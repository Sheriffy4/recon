...
def analyze_pcap_basic(filename):
    ...
    try:
        ...
        for i, pkt in enumerate(tcp_packets[:5]):
            if IP in pkt:
                print(f"  Packet {i+1}: {pkt[IP].src}:{pkt[TCP].sport} -> {pkt[IP].dst}:{pkt[TCP].dport}")
                print(f"    TTL: {pkt[IP].ttl}, Seq: {pkt[TCP].seq}, Flags: {pkt[TCP].flags}")
...
def compare_packets(zapret_pkt, recon_pkt, pair_num):
    ...
    print(f"Raw packet data exported to zapret_pkt_{pair_num}.hex and recon_pkt_{pair_num}.hex")
        ...
        if TCP in pkt and IP in pkt:
            ...
            if seq in seq_numbers:
                ...
                if ttl == 128:  # Windows default TTL
                    print(f"⚠ OS Retransmission detected: seq={seq}, TTL={ttl} at packet {i}")
            else:
