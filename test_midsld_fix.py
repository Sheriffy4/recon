...
def test_midsld_split_pos():
    ...
    try:
        ...
        logger.info(f"Тестирование fakeddisorder с split_pos='midsld'")
        ...
        for i, segment in enumerate(recipe):
            if not isinstance(segment, tuple) or len(segment) != 3:
                logger.error(f"❌ fakeddisorder с midsld: неправильная структура сегмента {i}")
                return False
        ...
        logger.info(f"✅ fakeddisorder с midsld: {len(recipe)} сегментов сгенерировано успешно")
