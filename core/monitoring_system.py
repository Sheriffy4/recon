...
class HealthChecker:
    ...
    async def check_tcp_connectivity(self, domain: str, port: int) -> Tuple[bool, float, Optional[str]]:
        ...
        start_time = time.time()
        try:
            reader, writer = await asyncio.wait_for(asyncio.open_connection(domain, port), timeout=self.timeout)
            writer.close()
...
class AutoRecoverySystem:
    ...
    async def attempt_recovery(self, health: ConnectionHealth, available_strategies: List[str]) -> bool:
        ...
        if self.learning_cache:
            optimized_strategies = self.learning_cache.get_smart_strategy_order(available_strategies, health.domain, health.ip)
        else:
    ...
    async def _test_strategy_recovery(self, health: ConnectionHealth, strategy: str) -> bool:
        ...
        try:
            reader, writer = await asyncio.wait_for(asyncio.open_connection(health.domain, health.port), timeout=3.0)
            writer.close()
            await writer.wait_closed()
            return True
        except Exception:
...
class MonitoringSystem:
    ...
    def __init__(self, config: MonitoringConfig, learning_cache=None, enable_modern_bypass: bool=True):
        ...
        if self.modern_bypass_enabled:
            ...
            self.pool_manager = None
            self.reliability_validator = None
        self.monitoring_stats = {'total_checks': 0, 'successful_recoveries': 0, 'failed_recoveries': 0, 'pool_strategy_uses': 0, 'registry_strategy_uses': 0, 'reliability_validations': 0}
    ...
    def add_site(self, domain: str, port: int=443, current_strategy: Optional[str]=None):
        ...
        try:
            ...
            ip = 'unknown'
        self.monitored_sites[site_key] = ConnectionHealth(domain=domain, ip=ip, port=port, is_accessible=False, response_time_ms=0.0, last_check=datetime.now(), current_strategy=current_strategy, bypass_active=current_strategy is not None)
    ...
    async def check_site_health(self, site_key: str) -> ConnectionHealth:
        ...
        is_accessible, response_time, error = await self.health_checker.check_http_connectivity(health.domain, health.port)
                ...
                try:
                    ...
                    for site_key, task in tasks:
                        try:
                            ...
                            if not health.is_accessible and health.consecutive_failures >= self.config.failure_threshold and self.config.enable_auto_recovery:
    ...
    async def _trigger_recovery(self, health: ConnectionHealth):
        ...
        if len(available_strategies) < 3:
            if self.config.enable_adaptive_strategies and self.learning_cache:
                ...
                default_strategies = ['--dpi-desync=multisplit --dpi-desync-split-count=3 --dpi-desync-split-seqovl=10 --dpi-desync-fooling=badsum', '--dpi-desync=fake,disorder --dpi-desync-split-pos=3 --dpi-desync-fooling=badsum --dpi-desync-ttl=3', '--dpi-desync=fake --dpi-desync-fooling=badsum --dpi-desync-ttl=2']
    ...
    async def _validate_recovery_strategies(self, health: ConnectionHealth, strategies: List[str]) -> List[str]:
        ...
        for strategy_str in strategies:
            try:
                strategy = BypassStrategy(id=f'recovery_{hash(strategy_str)}', name=f'Recovery strategy for {health.domain}', attacks=['tcp_fragmentation'], parameters={})
    ...
    async def _update_pool_after_recovery(self, health: ConnectionHealth):
        ...
        if not self.pool_manager or not health.current_strategy:
            return
        try:
            recovery_strategy = BypassStrategy(id=f'recovery_{health.domain}_{health.port}', name=f'Successful recovery strategy for {health.domain}', attacks=['tcp_fragmentation'], parameters={}, success_rate=1.0, last_tested=datetime.now())
            existing_strategy = self.pool_manager.get_strategy_for_domain(health.domain, health.port)
            if not existing_strategy:
                pool = self.pool_manager.create_pool(f'Recovery pool for {health.domain}', recovery_strategy, 'Auto-created after successful recovery')
    ...
    def get_status_report(self) -> dict:
        ...
        report = {'timestamp': datetime.now().isoformat(), 'total_sites': len(self.monitored_sites), 'accessible_sites': sum((1 for h in self.monitored_sites.values() if h.is_accessible)), 'sites_with_bypass': sum((1 for h in self.monitored_sites.values() if h.bypass_active)), 'average_response_time': 0.0, 'modern_bypass_enabled': self.modern_bypass_enabled, 'monitoring_stats': self.monitoring_stats.copy(), 'sites': {}}
