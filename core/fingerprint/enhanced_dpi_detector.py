...
class EnhancedDPIDetector:
    ...
    def detect_dpi_system(self, network_data: Dict[str, Any]) -> Optional[EnhancedDPISignature]:
        ...
        if best_match:
            ...
            LOG.info(f"DPI system detected: {best_match.dpi_type.value} (confidence: {highest_confidence:.2f})")
    ...
    def _detect_ja3s_fingerprinting(self, data: Dict[str, Any]) -> bool:
        ...
        return data.get("ja3s_detected", False) or "ja3s" in str(data.get("tls_analysis", "")).lower()
    ...
    def _detect_steganography_capability(self, data: Dict[str, Any]) -> bool:
        ...
        return data.get("steganography_detection", False) or data.get("hidden_data_detection", False)
    ...
    def _recommend_strategies_for_pattern(self, signature: EnhancedDPISignature) -> List[str]:
        ...
        if signature.tls_fingerprint_blocking:
            strategies.append("--dpi-desync=multisplit --dpi-desync-split-count=5 --dpi-desync-fooling=badsum")
        
        if signature.machine_learning_classification:
            strategies.append("--dpi-desync=fake,multidisorder --dpi-desync-split-pos=random --dpi-desync-fooling=badseq")
        ...
        if signature.obfuscation_detection:
            strategies.append("--dpi-desync=disorder --dpi-desync-split-pos=midsld --dpi-desync-fooling=md5sig")
