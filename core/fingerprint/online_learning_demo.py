...
from .online_learning import (
    OnlineLearningSystem, LearningMode, ABTestConfig
)
...
def simulate_classification_with_errors(true_type: str, confidence: float) -> tuple:
    ...
    dpi_types = [
        'UNKNOWN', 'ROSKOMNADZOR_TSPU', 'ROSKOMNADZOR_DPI', 
        'COMMERCIAL_DPI', 'FIREWALL_BASED', 'ISP_TRANSPARENT_PROXY',
        'CLOUDFLARE_PROTECTION', 'GOVERNMENT_CENSORSHIP'
    ]
...
def demo_basic_online_learning():
    ...
    metrics = trainer.train_model_with_evaluation(training_data[:50])  # Use subset for demo
    ...
    for i in range(30):
        ...
        if learned:
            print(f"   Example {i+1}: {predicted_type} -> {true_type} "
...
def demo_ab_testing():
    ...
    test_trainer.train_model_with_evaluation(training_data[10:50])  # Overlapping but different
    
    # Create online learning system
    online_learning = OnlineLearningSystem(
        ml_classifier=control_classifier,
        learning_mode=LearningMode.MODERATE
    )
    ...
    for i in range(25):  # More than min_samples to trigger conclusion
        ...
        print(f"   Request {i+1}: {model_used} model -> {predicted_type} "
