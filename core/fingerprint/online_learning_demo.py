...
def demo_basic_online_learning():
    ...
    for i in range(30):
        ...
        if learned:
            print(
                f"   Example {i + 1}: {predicted_type} -> {true_type} (conf: {confidence:.2f}) [LEARNED]"
            )
        else:
            print(
                f"   Example {i + 1}: {predicted_type} -> {true_type} (conf: {confidence:.2f}) [SKIPPED]"
            )
        time.sleep(0.1)
...
def demo_ab_testing():
    ...
    for i in range(25):
        ...
        print(
            f"   Request {i + 1}: {model_used} model -> {predicted_type} (actual: {actual_type}, conf: {confidence:.2f})"
        )
        time.sleep(0.05)
