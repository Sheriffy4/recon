...
def demo_create_legacy_data():
    ...
    mixed_data = {
        ...
        "mixed3.com": [
            "GOVERNMENT",
            0.95,
            {"dns_hijack": True, "header_filtering": True},
        ],
    }
...
def demo_legacy_data_migration(demo_dir, legacy_files):
    ...
    print("🔧 Initialized compatibility layer")
...
def demo_legacy_format_conversion():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "key": "minimal.com",
            "value": {"type": "COMMERCIAL", "score": 0.7},
        },
        {"name": "String Format", "key": "simple-site.com", "value": "GOVERNMENT"},
        {
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(
            f"   Value: {str(test_case['value'])[:60]}{'...' if len(str(test_case['value'])) > 60 else ''}"
        )
...
def demo_compatibility_wrapper():
    ...
    print("🔧 Created legacy compatibility wrapper")
    print(f"   Type: {type(wrapper).__name__}")

    # Test legacy interface methods
    test_domains = ["example.com", "blocked-site.com", "government-censored.com"]
...
def demo_advanced_to_legacy_conversion():
    ...
    print(
        f"🔄 Converting {len(advanced_fingerprints)} advanced fingerprints to legacy format:\n"
    )
    ...
    try:
        ...
        if migration_report["entries_migrated"] > 0:
            ...
            validation_report = compatibility_layer.validate_migration(
                legacy_files["pickle"], os.path.join(demo_dir, "cache")
            )
            ...
            print(
                f"   Validation errors: {len(validation_report['validation_errors'])}"
            )
            ...
            if validation_report["sample_comparisons"]:
                ...
                for comparison in validation_report["sample_comparisons"][:3]:
                    ...
                    print(
                        f"   Migrated Confidence: {comparison['migrated_confidence']:.2f}"
                    )
                    print()
            ...
            print("❌ No entries were migrated")
...
def demo_error_handling():
    ...
    try:
        ...
        print("1. Testing corrupted pickle file handling...")
        ...
        print(
            f"   ✅ Handled gracefully: {report['entries_migrated']} entries migrated"
        )
        ...
        print(
            f"   ✅ Fallback fingerprint created: {fallback_fp.get('dpi_type', 'N/A')}"
        )
...
def demo_performance_comparison():
    ...
    try:
        ...
        print("📊 Creating large legacy dataset...")
        large_data = {}
        for i in range(1000):
        ...
        print(
            f"   Entries per second: {report['entries_migrated'] / migration_time:.1f}"
        )
        print(
            f"   Success rate: {report['entries_migrated'] / len(large_data) * 100:.1f}%"
        )
        ...
        print(
            f"   Wrapper processed {len(test_domains)} domains in {wrapper_time:.3f}s"
        )
        print(
            f"   Average time per domain: {wrapper_time / len(test_domains) * 1000:.1f}ms"
        )
