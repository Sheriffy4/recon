...
class TrainingDataGenerator:
    ...
    def _create_variation(self, base_example: TrainingExample, variation_id: int) -> TrainingExample:
        ...
        numerical_keys = [
            ...
            'dns_resolution_time_ms', 'handshake_time_ms', 'rst_distance',
            'window_size_in_rst', 'tcp_option_len_limit', 'dpi_hop_distance'
        ]
...
class FeatureEngineer:
    ...
    def transform(self, metrics: Dict[str, Any]) -> Dict[str, float]:
        ...
        boolean_features = [
            ...
            'checksum_validation', 'quic_udp_blocked', 'stateful_inspection',
            'rate_limiting_detected', 'ml_detection_blocked', 'ip_level_blocked',
            'ech_blocked', 'tcp_option_splicing', 'large_payload_bypass',
            'ecn_support', 'http2_detection', 'http3_support', 'esni_support',
            'zero_rtt_blocked', 'dns_over_https_blocked', 'websocket_blocked'
        ]
        
        for feature in boolean_features:
            if feature in metrics:
                value = metrics[feature]
                engineered_features[feature] = 1.0 if value is True else 0.0 if value is False else -1.0
        
        # Categorical features (encoded)
        categorical_mappings = {
            'tls_version_sensitivity': {
                'blocks_tls13': 3.0, 'blocks_tls12': 2.0, 'blocks_tls11': 1.0,
                'blocks_all_tls': 4.0, 'no_version_preference': 0.0
            },
            'ipv6_handling': {
                'blocked': 2.0, 'throttled': 1.0, 'allowed': 0.0, 'not_applicable': -1.0
            },
            'tcp_keepalive_handling': {
                'reset': 2.0, 'strip': 1.0, 'forward': 0.0
            }
        }
    ...
    def _create_derived_features(self, metrics: Dict[str, Any]) -> Dict[str, float]:
        ...
        blocking_indicators = [
            'quic_udp_blocked', 'ech_blocked', 'zero_rtt_blocked',
            'dns_over_https_blocked', 'websocket_blocked', 'ip_level_blocked'
        ]
        blocking_count = sum(1 for indicator in blocking_indicators 
                           if metrics.get(indicator) is True)
        derived['blocking_intensity'] = blocking_count / len(blocking_indicators)
        
        # Technology support score
        tech_indicators = [
            'http2_detection', 'http3_support', 'esni_support', 'ecn_support'
        ]
