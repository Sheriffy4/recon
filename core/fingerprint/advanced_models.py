...
class DPIFingerprint:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for field_name, field_value in self.__dict__.items():
            if isinstance(field_value, Enum):
                ...
                result[field_name] = [(item[0].value if isinstance(item[0], Enum) else item[0], item[1]) 
    ...
    def get_recommended_strategies(self) -> List[str]:
        ...
        type_strategies = {
            DPIType.ROSKOMNADZOR_TSPU: [
                "tcp_fragmentation", "http_host_header_case", "tls_sni_fragmentation"
            ],
            DPIType.ROSKOMNADZOR_DPI: [
                "tcp_window_scaling", "http_method_override", "dns_over_https"
            ],
            DPIType.COMMERCIAL_DPI: [
                "tcp_options_manipulation", "http_chunked_encoding", "tls_version_downgrade"
            ],
            DPIType.FIREWALL_BASED: [
                "port_hopping", "protocol_tunneling", "packet_timing_manipulation"
            ],
            DPIType.ISP_TRANSPARENT_PROXY: [
                "http_proxy_bypass", "https_upgrade", "alternative_dns"
            ],
            DPIType.CLOUDFLARE_PROTECTION: [
                "user_agent_rotation", "request_header_randomization", "connection_pooling"
            ],
            DPIType.GOVERNMENT_CENSORSHIP: [
                "domain_fronting", "encrypted_sni", "traffic_obfuscation"
            ]
        }
    ...
    def get_summary(self) -> str:
        ...
        return (f"{self.dpi_type.value.replace('_', ' ').title()} "
    ...
    def merge_with(self, other: 'DPIFingerprint') -> 'DPIFingerprint':
        ...
        if other.connection_reset_timing > 0 and (self.connection_reset_timing == 0 or 
                                                  other.connection_reset_timing < self.connection_reset_timing):
        ...
        self.http_method_restrictions = list(set(self.http_method_restrictions + other.http_method_restrictions))
        self.protocol_whitelist = list(set(self.protocol_whitelist + other.protocol_whitelist))
        self.analysis_methods_used = list(set(self.analysis_methods_used + other.analysis_methods_used))
        
        # Update reliability score (weighted average)
        total_weight = self.reliability_score + other.reliability_score
        if total_weight > 0:
            self.reliability_score = (self.reliability_score * self.reliability_score + 
                                    other.reliability_score * other.reliability_score) / total_weight
