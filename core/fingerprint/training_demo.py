...
def demo_feature_engineering(training_data):
    ...
    boolean_features = [
        ...
        if k in sample_metrics and isinstance(sample_metrics[k], bool)
    ]
...
def demo_model_training(training_data):
    ...
    try:
        metrics = trainer.train_model_with_evaluation(
            training_data=training_data, test_size=0.2, cv_folds=3
        )

        print("\nTraining completed!")
        print(f"Model accuracy: {metrics.accuracy:.3f}")
        print(
            f"Cross-validation score: {metrics.cross_val_mean:.3f} ± {metrics.cross_val_std:.3f}"
        )
        ...
        for i in range(min(3, len(training_data))):
            ...
            predicted_type, confidence = trainer.ml_classifier.classify_dpi(
                sample["metrics"]
            )
            ...
            print(
                f"  {status} Predicted: {predicted_type} ({confidence:.3f}) | Actual: {actual_type}"
            )
...
def demo_evaluation_and_reporting(trainer):
    ...
    print("  - demo_training_report.txt")
    print("  - demo_dpi_classifier.joblib")
