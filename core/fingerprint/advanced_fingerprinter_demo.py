...
async def demo_ml_feature_extraction():
    ...
    try:
        ...
        feature_categories = {
            'TCP Features': [k for k in features.keys() if k.startswith(('tcp_', 'rst_', 'connection_', 'handshake_', 'mss_', 'sequence_'))],
            'HTTP Features': [k for k in features.keys() if k.startswith(('http_', 'content_', 'user_agent', 'host_header', 'redirect_', 'keep_alive', 'chunked_'))],
            'DNS Features': [k for k in features.keys() if k.startswith(('dns_', 'doh_', 'dot_', 'edns_', 'recursive_'))],
            'General Features': [k for k in features.keys() if k.startswith(('supports_', 'geographic_', 'time_based', 'packet_', 'protocol_', 'analysis_'))]
        }
