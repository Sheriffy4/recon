...
async def demo_basic_fingerprinting():
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.pkl') as f:
        cache_file = f.name
    try:
        config = FingerprintingConfig(cache_ttl=300, enable_ml=False, enable_cache=True, timeout=5.0, retry_attempts=1)
        async with AdvancedFingerprinter(config=config, cache_file=cache_file) as fingerprinter:
            ...
            test_targets = [('127.0.0.1', 80), ('127.0.0.1', 443), ('127.0.0.1', 53)]
            ...
            for target, port in test_targets:
                ...
                try:
                    ...
                    print(f"Analysis Methods: {', '.join(fingerprint.analysis_methods_used)}")
                    findings = []
                    if fingerprint.rst_injection_detected:
                        findings.append(f'RST injection ({fingerprint.rst_source_analysis})')
                    ...
                    if fingerprint.content_inspection_depth > 0:
                        findings.append(f'Content inspection (depth: {fingerprint.content_inspection_depth})')
                    if findings:
...
async def demo_heuristic_classification():
    ...
    try:
        ...
        test_patterns = [{'name': 'Roskomnadzor TSPU (Fast RST)', 'fingerprint': DPIFingerprint(target='blocked-site.com:443', rst_injection_detected=True, dns_hijacking_detected=True, http_header_filtering=True, connection_reset_timing=25.0)}, {'name': 'Roskomnadzor DPI (Slow RST)', 'fingerprint': DPIFingerprint(target='blocked-site2.com:443', rst_injection_detected=True, dns_hijacking_detected=True, http_header_filtering=True, connection_reset_timing=150.0)}, {'name': 'Commercial DPI', 'fingerprint': DPIFingerprint(target='corporate-site.com:443', content_inspection_depth=2000, user_agent_filtering=True, content_type_filtering=True, http_header_filtering=True)}, {'name': 'ISP Transparent Proxy', 'fingerprint': DPIFingerprint(target='redirected-site.com:443', redirect_injection=True, http_response_modification=True, rst_injection_detected=False)}, {'name': 'Firewall-based Blocking', 'fingerprint': DPIFingerprint(target='firewall-blocked.com:443', rst_injection_detected=True, dns_hijacking_detected=False, protocol_whitelist=['http', 'https', 'dns'])}, {'name': 'Unknown/Clean Connection', 'fingerprint': DPIFingerprint(target='clean-site.com:443')}]
        ...
        for pattern in test_patterns:
            ...
            print(f'  Difficulty Score: {fingerprint.calculate_evasion_difficulty():.2f}')
            characteristics = []
            if fingerprint.rst_injection_detected:
                characteristics.append(f'RST injection ({fingerprint.connection_reset_timing:.0f}ms)')
            if fingerprint.dns_hijacking_detected:
                characteristics.append('DNS hijacking')
            if fingerprint.http_header_filtering:
                characteristics.append('Header filtering')
            if fingerprint.content_inspection_depth > 0:
                characteristics.append(f'Content inspection ({fingerprint.content_inspection_depth})')
            if fingerprint.redirect_injection:
                characteristics.append('Redirect injection')
            if len(fingerprint.protocol_whitelist) > 0:
                characteristics.append(f'Protocol whitelist ({len(fingerprint.protocol_whitelist)})')
            if characteristics:
...
async def demo_ml_feature_extraction():
    ...
    try:
        ...
        fingerprint = DPIFingerprint(target='complex-dpi.com:443', dpi_type=DPIType.COMMERCIAL_DPI, confidence=0.85, rst_injection_detected=True, rst_source_analysis='middlebox', tcp_window_manipulation=True, sequence_number_anomalies=False, tcp_options_filtering=True, connection_reset_timing=75.0, handshake_anomalies=['window_anomaly', 'option_anomaly'], fragmentation_handling='modified', mss_clamping_detected=True, tcp_timestamp_manipulation=False, http_header_filtering=True, content_inspection_depth=1500, user_agent_filtering=True, host_header_manipulation=False, http_method_restrictions=['POST', 'PUT', 'DELETE'], content_type_filtering=True, redirect_injection=False, http_response_modification=True, keep_alive_manipulation=True, chunked_encoding_handling='blocked', dns_hijacking_detected=True, dns_response_modification=True, dns_query_filtering=False, doh_blocking=True, dot_blocking=False, dns_cache_poisoning=False, dns_timeout_manipulation=True, recursive_resolver_blocking=False, dns_over_tcp_blocking=True, edns_support=False, supports_ipv6=False, ip_fragmentation_handling='blocked', packet_size_limitations=1200, protocol_whitelist=['http', 'https'], geographic_restrictions=True, time_based_filtering=False, analysis_duration=3.5, reliability_score=0.9, analysis_methods_used=['tcp_analysis', 'http_analysis', 'dns_analysis'])
        ...
        print()
        feature_categories = {'TCP Features': [k for k in features.keys() if k.startswith(('tcp_', 'rst_', 'connection_', 'handshake_', 'mss_', 'sequence_'))], 'HTTP Features': [k for k in features.keys() if k.startswith(('http_', 'content_', 'user_agent', 'host_header', 'redirect_', 'keep_alive', 'chunked_'))], 'DNS Features': [k for k in features.keys() if k.startswith(('dns_', 'doh_', 'dot_', 'edns_', 'recursive_'))], 'General Features': [k for k in features.keys() if k.startswith(('supports_', 'geographic_', 'time_based', 'packet_', 'protocol_', 'analysis_'))]}
...
async def demo_cache_functionality():
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.pkl') as f:
        cache_file = f.name
    try:
        config = FingerprintingConfig(cache_ttl=10, enable_cache=True)
        async with AdvancedFingerprinter(config=config, cache_file=cache_file) as fingerprinter:
            if not fingerprinter.cache:
                print('Cache is not available for this demo')
                return
            print('Testing cache functionality...')
            test_fingerprints = [DPIFingerprint(target='cache-test-1.com:443', dpi_type=DPIType.ROSKOMNADZOR_TSPU, confidence=0.9), DPIFingerprint(target='cache-test-2.com:443', dpi_type=DPIType.COMMERCIAL_DPI, confidence=0.8), DPIFingerprint(target='cache-test-3.com:443', dpi_type=DPIType.ISP_TRANSPARENT_PROXY, confidence=0.7)]
