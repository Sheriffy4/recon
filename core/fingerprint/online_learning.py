...
from .ml_classifier import MLClassifier, MLClassificationError
...
class OnlineLearningSystem:
    ...
    def add_learning_example(self, 
        ...
        with self._lock:
            ...
            if not self._should_learn_from_example(confidence, predicted_type, actual_type):
                LOG.debug(f"Skipping learning example: confidence={confidence:.3f}, "
            ...
            LOG.info(f"Added learning example: {predicted_type} -> {actual_type} "
    ...
    def _perform_incremental_update(self):
        ...
        try:
            ...
            LOG.info(f"Incremental update completed. "
    ...
    def _update_performance_monitoring(self, example: LearningExample):
        ...
        if len(self.performance_history) >= self.performance_window_size:
            ...
            correct_predictions = sum(1 for ex in self.performance_history 
    ...
    def _should_trigger_retraining(self) -> bool:
        ...
        if (self.baseline_performance is None or 
            len(self.performance_history) < self.performance_window_size):
            return False
        ...
        if performance_drop > self.retraining_threshold:
            LOG.warning(f"Performance degradation detected: {performance_drop:.3f} "
    ...
    def _evaluate_current_performance(self) -> PerformanceMetrics:
        ...
        if len(self.performance_history) == 0:
            return PerformanceMetrics(
                accuracy=0.0, f1_score=0.0, confidence_distribution={},
                prediction_counts={}, timestamp=time.time(), sample_size=0
            )
    ...
    def start_ab_test(self, config: ABTestConfig) -> bool:
        ...
        try:
            ...
            LOG.info(f"Started A/B test '{config.test_name}' with {config.traffic_split:.1%} "
    ...
    def _conclude_ab_test(self) -> ABTestResults:
        ...
        statistical_significance = abs(improvement) / max(0.01, np.sqrt(
        ...
        LOG.info(f"A/B test concluded: {results.recommendation} "
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'baseline_performance': self.baseline_performance.to_dict() if self.baseline_performance else None,
