...
class OnlineLearningSystem:
    ...
    def add_learning_example(
        ...
        with self._lock:
            ...
            LOG.info(
                f"Added learning example: {predicted_type} -> {actual_type} (confidence: {confidence:.3f}, source: {source})"
            )
    ...
    def _perform_incremental_update(self):
        ...
        try:
            ...
            LOG.info(
                f"Incremental update completed. Performance change: {post_update_performance.accuracy - pre_update_performance.accuracy:.3f}"
            )
    ...
    def _should_trigger_retraining(self) -> bool:
        ...
        if performance_drop > self.retraining_threshold:
            LOG.warning(
                f"Performance degradation detected: {performance_drop:.3f} (threshold: {self.retraining_threshold})"
            )
            return True
        return False
    ...
    def start_ab_test(self, config: ABTestConfig) -> bool:
        ...
        try:
            ...
            LOG.info(
                f"Started A/B test '{config.test_name}' with {config.traffic_split:.1%} traffic to test model"
            )
            return True
        except Exception as e:
    ...
    def _conclude_ab_test(self) -> ABTestResults:
        ...
        LOG.info(
            f"A/B test concluded: {results.recommendation} (improvement: {improvement:.3f}, significance: {statistical_significance:.2f})"
        )
        return results
