...
class MonitoringIntegration:
    ...
    def __init__(self, fingerprinter: AdvancedFingerprinter, monitoring_config: Optional[MonitoringConfig]=None):
        """
        ...
        self.monitor = DPIBehaviorMonitor(fingerprinter=fingerprinter, config=self.config, alert_callback=self._handle_monitoring_alert)
        self.alert_handlers: List[Callable[[MonitoringAlert], None]] = []
        self.behavior_change_handlers: List[Callable[[str, DPIFingerprint, DPIFingerprint], None]] = []
        self.strategy_update_handlers: List[Callable[[str, List[str]], None]] = []
        self.integration_stats = {'alerts_processed': 0, 'behavior_changes_processed': 0, 'strategy_updates_triggered': 0, 'integration_errors': 0}
    ...
    def add_behavior_change_handler(self, handler: Callable[[str, DPIFingerprint, DPIFingerprint], None]):
            ...
            if recommended_strategies:
                ...
                for handler in self.strategy_update_handlers:
                    try:
                        ...
                        self.logger.error(f'Error in strategy update handler {handler.__name__}: {e}')
                        self.integration_stats['integration_errors'] += 1
        except Exception as e:
    ...
    def _generate_strategy_recommendations(self, alert: MonitoringAlert) -> List[str]:
        ...
        if fingerprint.dpi_type.value == 'roskomnadzor_tspu':
            recommendations.extend(['--dpi-desync=multisplit --dpi-desync-split-count=3 --dpi-desync-split-seqovl=10', '--dpi-desync=fake,disorder --dpi-desync-split-pos=3 --dpi-desync-fooling=badsum', '--dpi-desync=fake --dpi-desync-fooling=badsum --dpi-desync-ttl=2'])
        elif fingerprint.dpi_type.value == 'commercial_dpi':
            recommendations.extend(['--dpi-desync=fake --dpi-desync-fooling=badsum --dpi-desync-ttl=3', '--dpi-desync=split2 --dpi-desync-split-pos=2', '--dpi-desync=disorder --dpi-desync-fooling=badseq'])
        elif fingerprint.dpi_type.value == 'firewall_based':
            recommendations.extend(['--dpi-desync=fake --dpi-desync-ttl=1', '--dpi-desync=split2 --dpi-desync-split-pos=1', '--dpi-desync=disorder'])
        if fingerprint.rst_injection_detected:
            recommendations.append('--dpi-desync=fake --dpi-desync-fooling=badsum --dpi-desync-ttl=1')
        ...
        try:
            ...
            if change:
                ...
                for handler in self.behavior_change_handlers:
                    try:
                        handler(change.target, change.old_fingerprint, change.new_fingerprint)
                    except Exception as e:
                        self.logger.error(f'Error in behavior change handler {handler.__name__}: {e}')
                        self.integration_stats['integration_errors'] += 1
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {'monitoring': monitor_status, 'integration': {'alert_handlers': len(self.alert_handlers), 'behavior_change_handlers': len(self.behavior_change_handlers), 'strategy_update_handlers': len(self.strategy_update_handlers), 'stats': self.integration_stats.copy()}}
    ...
    def get_target_recommendations(self, domain: str, port: int=443) -> Dict[str, Any]:
        ...
        try:
            ...
            mock_alert = MonitoringAlert(id='temp', target=f'{domain}:{port}', timestamp=datetime.now(), severity=alert.AlertSeverity.MEDIUM, title='Strategy Recommendation Request', description='Generating recommendations', fingerprint=fingerprint)
            recommendations = self._generate_strategy_recommendations(mock_alert)
            return {'target': f'{domain}:{port}', 'dpi_type': fingerprint.dpi_type.value, 'confidence': fingerprint.confidence, 'recommendations': recommendations, 'analysis': {'rst_injection': fingerprint.rst_injection_detected, 'http_filtering': fingerprint.http_header_filtering, 'dns_hijacking': fingerprint.dns_hijacking_detected, 'content_inspection': fingerprint.content_inspection_depth > 0}}
        except Exception as e:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'status': overall_health, 'timestamp': datetime.now().isoformat(), 'components': {'fingerprinter': fp_health, 'monitor': {'state': monitor_status['state'], 'monitored_targets': monitor_status['monitored_targets'], 'active_tasks': monitor_status['active_tasks'], 'behavior_changes': monitor_status['behavior_changes'], 'active_alerts': monitor_status['active_alerts']}, 'integration': {'handlers_registered': {'alert_handlers': len(self.alert_handlers), 'behavior_change_handlers': len(self.behavior_change_handlers), 'strategy_update_handlers': len(self.strategy_update_handlers)}, 'stats': self.integration_stats.copy()}}}
        except Exception as e:
            self.logger.error(f'Error in health check: {e}')
            return {'status': 'error', 'timestamp': datetime.now().isoformat(), 'error': str(e)}

def create_monitoring_integration(fingerprinter: AdvancedFingerprinter, config: Optional[MonitoringConfig]=None) -> MonitoringIntegration:
    """
...
async def setup_basic_monitoring(fingerprinter: AdvancedFingerprinter, targets: List[tuple], alert_handler: Optional[Callable[[MonitoringAlert], None]]=None) -> MonitoringIntegration:
    """
...
def default_behavior_change_handler(target: str, old_fp: Optional[DPIFingerprint], new_fp: DPIFingerprint):
    """Default behavior change handler that logs changes"""
    if old_fp:
        logger.info(f'🔄 DPI behavior change detected for {target}')
        logger.info(f'   Old DPI type: {old_fp.dpi_type.value} (confidence: {old_fp.confidence:.2f})')
        logger.info(f'   New DPI type: {new_fp.dpi_type.value} (confidence: {new_fp.confidence:.2f})')
    else:
