...
from .models import Fingerprint, DPIClassification, DPIFamily
...
class UltimateDPIClassifier:
    ...
    # Comprehensive signature database (all experts combined + new)
    DPI_SIGNATURES = {
        # === Russian/CIS Region ===
        "TSPU": {
            "vendor": "TSPU/Beeline",
            ...
            "conditions": [
                ...
                # NEW: Extended features for enhanced classification
                (
        ...
        "Roskomnadzor": {
            ...
            "confidence": 0.92,
            "conditions": [
                (lambda fp: fp.ip_level_blocked is True, "IP-level blocking", 0.30),
                (lambda fp: (fp.dpi_hop_distance or 99) <= 3, "Very close DPI", 0.20),
                (lambda fp: fp.quic_udp_blocked is True, "QUIC blocked", 0.15),
                (lambda fp: fp.supports_ip_frag is False, "IP frag blocked", 0.10),
                (lambda fp: fp.stateful_inspection is True, "Stateful", 0.10),
                (lambda fp: fp.dns_over_https_blocked is True, "DoH blocked", 0.10),
        ...
        "GFW": {
            ...
            "conditions": [
                ...
                # NEW: Extended features for GFW detection
                (
                    lambda fp: hasattr(fp, "rst_ttl_distance")
        ...
        "Iran_NIN": {
            ...
            "confidence": 0.88,
            "conditions": [
                (lambda fp: fp.esni_support is False, "ESNI blocked", 0.20),
                (lambda fp: fp.ech_blocked is True, "ECH blocked", 0.20),
                (lambda fp: fp.quic_udp_blocked is True, "QUIC blocked", 0.15),
                (lambda fp: fp.ml_detection_blocked is True, "ML detection", 0.15),
                (lambda fp: (fp.dpi_hop_distance or 99) <= 4, "Close DPI", 0.10),
                (
        ...
        "Sandvine": {
            ...
            "conditions": [
                ...
                (lambda fp: fp.http2_detection is True, "HTTP/2 detection", 0.10),
                (lambda fp: fp.rate_limiting_detected is True, "Rate limiting", 0.15),
                (lambda fp: fp.ml_detection_blocked is True, "ML detection", 0.10),
            ],
        },
        "PaloAlto": {
            ...
            "conditions": [
                ...
                # NEW: Extended features for PaloAlto detection
                (
        ...
        "FortiGate": {
            ...
            "conditions": [
                ...
                (lambda fp: fp.certificate_validation is True, "Cert validation", 0.10),
            ],
        },
        "Cisco_ASA": {
            "vendor": "Cisco",
            ...
            "conditions": [
                ...
                (lambda fp: fp.grpc_blocked is True, "gRPC blocked", 0.10),
            ],
        },
        # === Cloud/CDN ===
        "Cloudflare": {
            ...
            "confidence": 0.88,
            "conditions": [
                (lambda fp: (fp.rst_latency_ms or 0) > 100, "High latency", 0.20),
                ...
                (lambda fp: fp.http2_detection is True, "HTTP/2 support", 0.10),
                (lambda fp: fp.http3_support is True, "HTTP/3 support", 0.10),
                (lambda fp: fp.tcp_fast_open_support is True, "TFO support", 0.10),
                (lambda fp: fp.zero_rtt_blocked is False, "0-RTT allowed", 0.10),
                (lambda fp: fp.websocket_blocked is False, "WebSocket allowed", 0.10),
            ],
        },
        "Akamai": {
            "vendor": "Akamai",
            ...
            "confidence": 0.86,
            "conditions": [
                (lambda fp: fp.http3_support is True, "HTTP/3 support", 0.20),
                (
                    lambda fp: (fp.payload_entropy_sensitivity or 0.0) > 0.7,
                    "Entropy sensitive",
                    0.15,
                ...
                (lambda fp: fp.tcp_fast_open_support is True, "TFO support", 0.10),
                (lambda fp: fp.ecn_support is True, "ECN support", 0.10),
        ...
        "Zscaler": {
            ...
            "confidence": 0.84,
            "conditions": [
                (lambda fp: (fp.rst_latency_ms or 0) > 80, "Cloud latency", 0.20),
                (lambda fp: fp.dns_over_https_blocked is True, "DoH blocked", 0.15),
        ...
        "Cisco_Umbrella": {
            ...
            "confidence": 0.83,
            "conditions": [
                (lambda fp: fp.dpi_hop_distance is None, "Cloud-based", 0.20),
                (lambda fp: fp.rst_from_target is False, "RST from middlebox", 0.15),
                (lambda fp: fp.quic_udp_blocked is True, "QUIC blocked", 0.15),
                (lambda fp: fp.ech_blocked is True, "ECH blocked", 0.15),
                (lambda fp: fp.dns_over_https_blocked is True, "DoH blocked", 0.20),
        ...
        "OpenDPI": {
            ...
            "confidence": 0.75,
            "conditions": [
                (lambda fp: 60 <= (fp.rst_ttl or 0) <= 64, "Linux TTL", 0.20),
                (lambda fp: fp.stateful_inspection is True, "Stateful", 0.25),
                (lambda fp: fp.checksum_validation is False, "No checksum check", 0.20),
                (lambda fp: fp.tcp_fast_open_support is False, "No TFO", 0.15),
                (lambda fp: fp.rate_limiting_detected is False, "No rate limit", 0.10),
                (lambda fp: fp.ml_detection_blocked is False, "No ML detection", 0.10),
            ],
        },
        # === Generic fallback ===
        "Generic_DPI": {
    ...
    def __init__(self, ml_enabled: bool = True, debug: bool = False):
        ...
        self.debug = debug  # Сохраняем флаг
        self.ml_model = None
        self.feature_names = []
    ...
    def classify(self, fp: Fingerprint) -> DPIClassification:
        ...
        # FIX: Correctly check if the model is enabled AND has been fitted
        ...
        if isinstance(classification.family, str):
            try:
                ...
                # If conversion fails, find by value
                for family in DPIFamily:
                    if family.value == classification.family:
                        fp.dpi_family = family
                        break
                else:
                    # If still not found, keep as string (handled by get_summary)
        ...
        LOG.info(
            f"DPI Classification: {classification.dpi_type} "
            f"({classification.vendor}) [{classification.confidence:.0%}]"
        )
    ...
    def _ml_classify(self, fp: Fingerprint) -> Optional[DPIClassification]:
        ...
        try:
            ...
            if confidence > 0.6:  # Threshold for ML prediction
                # Get signature info
    ...
    def _extract_features(self, fp: Fingerprint) -> np.ndarray:
        ...
        # NEW: Extended categorical features
        features.append(
        ...
        # NEW: Timing pattern features (aggregate statistics)
        ...
        if timing_patterns:
            ...
            features.extend([-1, -1])  # No timing data available

        # NEW: Content filtering indicator count
    ...
    def _get_recommendations(self, dpi_type: str, fp: Fingerprint) -> List[str]:
        ...
        # Feature-based recommendations с реальными именами
        ...
        if fp.stateful_inspection:
            # Заменяем концепции на реальные атаки
    ...
    def _load_or_train_ml_model(self):
        ...
        if os.path.exists(model_path):
            try:
                ...
                # FIX: If we load a model, we assume it's fitted
    ...
    def _train_new_model(self):
        ...
        # FIX: Model is created but not fitted yet
        self.is_model_fitted = False
    ...
    def update_model_with_feedback(
        ...
        try:
            ...
            if hasattr(self.ml_model, "partial_fit"):
                ...
                self.is_model_fitted = True  # FIX: Mark as fitted
