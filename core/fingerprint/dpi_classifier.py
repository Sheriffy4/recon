...
class DPIClassificationEngine:
    ...
    def _calculate_signature_match(self, fingerprint, signature: DPISignature) -> float:
        ...
        if signature.behavioral_markers:
            behavior_match = self._match_behavioral_markers(fingerprint, signature.behavioral_markers)
    ...
    def assess_vulnerabilities(self, fingerprint, dpi_signature: DPISignature) -> VulnerabilityAssessment:
        """Оценка уязвимостей DPI системы"""
    ...
    def _assess_fragmentation_vulnerabilities(self, fingerprint, signature: DPISignature) -> List[str]:
    ...
    def _calculate_evasion_difficulty(self, assessment: VulnerabilityAssessment) -> EvasionDifficulty:
    ...
    def _generate_attack_recommendations(self, fingerprint, signature: DPISignature, 
                                       assessment: VulnerabilityAssessment) -> List[Tuple[str, float]]:
    ...
    def _generate_parameter_recommendations(self, fingerprint, signature: DPISignature) -> Dict[str, Any]:
    ...
    def _calculate_assessment_confidence(self, assessment: VulnerabilityAssessment) -> float:
        ...
        if assessment.recommended_attacks:
            avg_probability = sum(prob for _, prob in assessment.recommended_attacks) / len(assessment.recommended_attacks)
            confidence_factors.append(avg_probability * 0.4)
        
        return sum(confidence_factors) if confidence_factors else 0.1
    
    def create_provenance_record(self, method: str, created_by: str, 
                               source_data: Dict[str, Any] = None) -> ProvenanceRecord:
    ...
    def calculate_confidence_score(self, fingerprint, classification_result: Tuple[DPISignature, float],
                                 vulnerability_assessment: VulnerabilityAssessment) -> float:
...
if __name__ == "__main__":
    ...
    print(f"🔍 Уязвимости фрагментации: {len(vulnerability_assessment.fragmentation_vulnerabilities)}")
