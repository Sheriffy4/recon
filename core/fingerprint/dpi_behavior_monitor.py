...
class BehaviorAnalyzer:
    ...
    def analyze_behavior_change(
        self, old_fp: Optional[DPIFingerprint], new_fp: DPIFingerprint
    ...
    def _analyze_specific_changes(
        self, old_fp: DPIFingerprint, new_fp: DPIFingerprint
    ) -> Dict[str, Any]:
    ...
    def _generate_alert_description(self, change: BehaviorChange) -> str:
        ...
        if (
            ...
            and change.details["removed_blocking_methods"]
        ):
...
class DPIBehaviorMonitor:
    ...
    def _load_persisted_data(self):
        ...
        try:
                    ...
                    with open(behavior_file, "r", encoding="utf-8") as f:
                        ...
                        for item in data:
                            change = BehaviorChange(
                                ...
                                old_fingerprint=(
                                    DPIFingerprint.from_dict(item["old_fingerprint"])
            ...
            self.logger.info(
                f"Loaded {len(self.behavior_changes)} behavior changes and {len(self.alerts)} alerts"
            )
        ...
        try:
            while self.state in [MonitoringState.RUNNING, MonitoringState.PAUSED]:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Monitoring cycle for {target_key} completed in {cycle_time:.2f}s"
                    )
    ...
    async def _handle_behavior_change(self, change: BehaviorChange):
        ...
        if self.config.enable_strategy_testing and change.change_type in [
            "enhanced_blocking",
            "new_blocking",
            "unknown_pattern",
        ]:
    ...
    def _cleanup_old_data(self):
        ...
        self.behavior_changes = [
            change for change in self.behavior_changes if change.timestamp > cutoff_date
        ]
