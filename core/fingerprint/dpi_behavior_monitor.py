...
class BehaviorChange:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'target': self.target, 'timestamp': self.timestamp.isoformat(), 'change_type': self.change_type, 'old_fingerprint': self.old_fingerprint.to_dict() if self.old_fingerprint else None, 'new_fingerprint': self.new_fingerprint.to_dict(), 'confidence': self.confidence, 'details': self.details}
...
class MonitoringAlert:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'id': self.id, 'target': self.target, 'timestamp': self.timestamp.isoformat(), 'severity': self.severity.value, 'title': self.title, 'description': self.description, 'fingerprint': self.fingerprint.to_dict(), 'suggested_actions': self.suggested_actions, 'acknowledged': self.acknowledged, 'resolved': self.resolved}
...
class PerformanceMonitor:
    ...
    def is_system_overloaded(self, cpu_threshold: float, memory_threshold: float) -> bool:
        ...
        return self._cpu_usage > cpu_threshold or self._memory_usage > memory_threshold

    def get_adaptive_interval(self, base_interval: int, min_interval: int, max_interval: int, cpu_threshold: float, memory_threshold: float) -> int:
...
class BehaviorAnalyzer:
    ...
    def _load_known_patterns(self):
        ...
        self._known_patterns = {'roskomnadzor_tspu': {'rst_injection_detected', 'dns_hijacking_detected', 'http_header_filtering', 'fast_connection_reset'}, 'roskomnadzor_dpi': {'rst_injection_detected', 'dns_hijacking_detected', 'content_inspection_depth_high', 'tcp_window_manipulation'}, 'commercial_dpi': {'content_inspection_depth_high', 'user_agent_filtering', 'content_type_filtering', 'protocol_analysis'}, 'firewall_based': {'rst_injection_detected', 'protocol_whitelist', 'port_blocking', 'ip_filtering'}, 'cloudflare_protection': {'user_agent_filtering', 'http_header_filtering', 'redirect_injection', 'challenge_response'}}

    def analyze_behavior_change(self, old_fp: Optional[DPIFingerprint], new_fp: DPIFingerprint) -> Optional[BehaviorChange]:
        """Analyze fingerprint changes and detect behavior modifications"""
        if not old_fp:
            change_type = self._classify_new_behavior(new_fp)
            return BehaviorChange(target=new_fp.target, timestamp=datetime.now(), change_type=change_type, old_fingerprint=None, new_fingerprint=new_fp, confidence=new_fp.confidence, details={'reason': 'new_target_analysis'})
        ...
        if similarity < self.config.fingerprint_similarity_threshold:
            ...
            return BehaviorChange(target=new_fp.target, timestamp=datetime.now(), change_type=change_type, old_fingerprint=old_fp, new_fingerprint=new_fp, confidence=1.0 - similarity, details=change_details)
        return None

    def _calculate_fingerprint_similarity(self, fp1: DPIFingerprint, fp2: DPIFingerprint) -> float:
        """Calculate similarity between two fingerprints"""
        indicators = ['rst_injection_detected', 'tcp_window_manipulation', 'sequence_number_anomalies', 'http_header_filtering', 'user_agent_filtering', 'content_type_filtering', 'dns_hijacking_detected', 'dns_response_modification', 'doh_blocking']
        matches = 0
        total = 0
    ...
    def _analyze_specific_changes(self, old_fp: DPIFingerprint, new_fp: DPIFingerprint) -> Dict[str, Any]:
        ...
        changes = {'dpi_type_changed': old_fp.dpi_type != new_fp.dpi_type, 'confidence_change': new_fp.confidence - old_fp.confidence, 'new_blocking_methods': [], 'removed_blocking_methods': [], 'behavior_modifications': []}
        blocking_indicators = ['rst_injection_detected', 'tcp_window_manipulation', 'http_header_filtering', 'dns_hijacking_detected', 'user_agent_filtering', 'content_type_filtering']
        ...
        if old_fp.connection_reset_timing != new_fp.connection_reset_timing:
            changes['behavior_modifications'].append({'type': 'connection_timing_change', 'old_value': old_fp.connection_reset_timing, 'new_value': new_fp.connection_reset_timing})
        if old_fp.content_inspection_depth != new_fp.content_inspection_depth:
            changes['behavior_modifications'].append({'type': 'inspection_depth_change', 'old_value': old_fp.content_inspection_depth, 'new_value': new_fp.content_inspection_depth})
        return changes
    ...
    def generate_alert(self, behavior_change: BehaviorChange) -> Optional[MonitoringAlert]:
        ...
        alert_id = hashlib.md5(f'{behavior_change.target}_{behavior_change.timestamp.isoformat()}_{behavior_change.change_type}'.encode()).hexdigest()[:12]
        alert = MonitoringAlert(id=alert_id, target=behavior_change.target, timestamp=behavior_change.timestamp, severity=severity, title=self._generate_alert_title(behavior_change), description=self._generate_alert_description(behavior_change), fingerprint=behavior_change.new_fingerprint, suggested_actions=self._generate_suggested_actions(behavior_change))
        return alert
    ...
    def _generate_alert_title(self, change: BehaviorChange) -> str:
        ...
        titles = {'unknown_pattern': f'Unknown DPI pattern detected for {change.target}', 'enhanced_blocking': f'Enhanced blocking detected for {change.target}', 'reduced_blocking': f'Reduced blocking detected for {change.target}', 'dpi_type_change': f'DPI type change detected for {change.target}', 'new_blocking': f'New blocking behavior for {change.target}', 'behavior_modification': f'DPI behavior modification for {change.target}'}
    ...
    def _generate_suggested_actions(self, change: BehaviorChange) -> List[str]:
        ...
        actions = []
        if change.change_type == 'unknown_pattern':
            actions.extend(['Manually analyze the new DPI pattern', 'Update ML training data with new pattern', 'Test existing strategies against new pattern'])
        elif change.change_type == 'enhanced_blocking':
            actions.extend(['Test advanced bypass strategies', 'Update strategy effectiveness ratings', 'Consider alternative protocols'])
        elif change.change_type == 'reduced_blocking':
            actions.extend(['Test if simpler strategies now work', 'Update strategy recommendations', 'Monitor for potential trap/honeypot behavior'])
        elif change.change_type == 'dpi_type_change':
            actions.extend(['Re-evaluate strategy selection for new DPI type', 'Update fingerprint cache', 'Test type-specific strategies'])
...
class DPIBehaviorMonitor:
    ...
    def __init__(self, fingerprinter: AdvancedFingerprinter, config: Optional[MonitoringConfig]=None, alert_callback: Optional[Callable[[MonitoringAlert], None]]=None):
        """
        ...
        self.stats = {'monitoring_cycles': 0, 'behavior_changes_detected': 0, 'alerts_generated': 0, 'fingerprints_updated': 0, 'strategy_tests_performed': 0, 'total_monitoring_time': 0.0, 'last_cycle_time': 0.0}
            ...
            if self.config.save_behavior_changes:
                ...
                if behavior_file.exists():
                    with open(behavior_file, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        for item in data:
                            change = BehaviorChange(target=item['target'], timestamp=datetime.fromisoformat(item['timestamp']), change_type=item['change_type'], old_fingerprint=DPIFingerprint.from_dict(item['old_fingerprint']) if item['old_fingerprint'] else None, new_fingerprint=DPIFingerprint.from_dict(item['new_fingerprint']), confidence=item['confidence'], details=item['details'])
            ...
            if self.config.enable_alerts:
                ...
                if alerts_file.exists():
                    with open(alerts_file, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        for item in data:
                            alert = MonitoringAlert(id=item['id'], target=item['target'], timestamp=datetime.fromisoformat(item['timestamp']), severity=AlertSeverity(item['severity']), title=item['title'], description=item['description'], fingerprint=DPIFingerprint.from_dict(item['fingerprint']), suggested_actions=item['suggested_actions'], acknowledged=item['acknowledged'], resolved=item['resolved'])
        ...
        try:
            ...
            if self.config.enable_alerts and self.alerts:
                ...
                with open(alerts_file, 'w', encoding='utf-8') as f:
                    data = [alert.to_dict() for alert in self.alerts[-1000:]]
                    json.dump(data, f, indent=2, ensure_ascii=False)
        except Exception as e:
    ...
    async def _monitor_target(self, target: str, port: int):
        ...
        try:
            while self.state in [MonitoringState.RUNNING, MonitoringState.PAUSED]:
                ...
                try:
                    interval = self.performance_monitor.get_adaptive_interval(self.config.check_interval_seconds, self.config.min_check_interval, self.config.max_check_interval, self.config.performance_threshold_cpu, self.config.performance_threshold_memory)
                    old_fingerprint = self.monitored_targets.get(target_key)
                    new_fingerprint = await self.fingerprinter.fingerprint_target(target, port, force_refresh=True)
                    self.monitored_targets[target_key] = new_fingerprint
                    behavior_change = self.behavior_analyzer.analyze_behavior_change(old_fingerprint, new_fingerprint)
                    if behavior_change:
    ...
    async def _handle_behavior_change(self, change: BehaviorChange):
        ...
        if self.config.enable_strategy_testing and change.change_type in ['enhanced_blocking', 'new_blocking', 'unknown_pattern']:
    ...
    def _cleanup_old_data(self):
        ...
        self.behavior_changes = [change for change in self.behavior_changes if change.timestamp > cutoff_date]
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {'state': self.state.value, 'monitored_targets': len(self.monitored_targets), 'active_tasks': len(self.monitoring_tasks), 'behavior_changes': len(self.behavior_changes), 'active_alerts': len([a for a in self.alerts if not a.resolved]), 'total_alerts': len(self.alerts), 'stats': self.stats.copy(), 'config': self.config.to_dict()}
    ...
    def get_target_status(self, target: str, port: int=443) -> Optional[Dict[str, Any]]:
        ...
        return {'target': target_key, 'current_fingerprint': fingerprint.to_dict(), 'behavior_changes': len(target_changes), 'recent_changes': [c.to_dict() for c in target_changes[-5:]], 'active_alerts': len([a for a in target_alerts if not a.resolved]), 'total_alerts': len(target_alerts), 'last_check': fingerprint.timestamp}
    ...
    def get_alerts(self, target: Optional[str]=None, severity: Optional[AlertSeverity]=None, unresolved_only: bool=False) -> List[MonitoringAlert]:
