...
class PacketAnalyzer:
    ...
    def _analyze_rst_characteristics(self, fp: Fingerprint, rst_packets: List[Packet]) -> Fingerprint:
    ...
    def _analyze_tcp_behavior(self, fp: Fingerprint, packets: List[Packet]) -> Fingerprint:
        ...
        if len(seq_numbers) > 1:
            ...
            if seq_diffs:
                fp.ml_features['seq_randomness'] = statistics.stdev(seq_diffs) if len(seq_diffs) > 1 else 0
        return fp
...
class BehaviorAnalyzer:
    ...
    def analyze_behavior(self, fingerprint: EnhancedFingerprint, attack_results: Dict[str, List[float]]) -> DPIBehaviorProfile:
        """
    ...
    def _analyze_detection_patterns(self, fp: EnhancedFingerprint) -> Dict[str, Any]:
        ...
        patterns = {'signature_based': False, 'behavioral_analysis': False, 'ml_detection': False, 'statistical_analysis': False, 'protocol_validation': False, 'timing_analysis': False}
        ...
        if 'timing_sensitivity' in fp.timing_sensitivity and fp.timing_sensitivity['timing_sensitivity'] > 0.7:
            patterns['timing_analysis'] = True
        return patterns

    def _analyze_temporal_patterns(self, fp: EnhancedFingerprint, attack_results: Dict[str, List[float]]) -> Dict[str, List[float]]:
        """Analyze temporal patterns in DPI behavior."""
        patterns = {'hourly_effectiveness': [], 'burst_tolerance': [], 'sustained_rate': [], 'detection_delay': []}
        ...
        for technique, results in attack_results.items():
            if len(results) > 10:
                ...
                burst_effectiveness = sum(burst_results) / len(burst_results) if burst_results else 0
                sustained_effectiveness = sum(sustained_results) / len(sustained_results) if sustained_results else 0
    ...
    def _analyze_size_sensitivity(self, fp: EnhancedFingerprint) -> Dict[int, float]:
        ...
        for size in test_sizes:
            if size < 256:
                ...
                effectiveness = base_effectiveness * 1.3 if fp.large_payload_bypass else base_effectiveness * 0.9
            else:
                effectiveness = base_effectiveness
    ...
    def _detect_traffic_shaping(self, fp: EnhancedFingerprint) -> bool:
        ...
        indicators = [fp.rate_limiting_detected, fp.timing_sensitivity.get('burst_tolerance', 1.0) < 0.5, 'qos' in str(fp.classification_reasons).lower()]
    ...
    def _update_baseline(self, profile: DPIBehaviorProfile):
        ...
        for technique, effectiveness in profile.evasion_effectiveness.items():
            if technique not in baseline['evasion_effectiveness']:
                baseline['evasion_effectiveness'][technique] = {'values': [], 'mean': 0, 'std': 0}
...
async def comprehensive_analysis(target_ip: str, target_port: int=443, packets: List[Packet]=None, domain: str=None) -> EnhancedFingerprint:
    """
    ...
    efp = EnhancedFingerprint(domain=domain or target_ip, ip_addresses=[target_ip], timestamp=datetime.now())
    if packets:
    ...
    efp.technique_success_rates = {'tcp_segmentation': 0.7 if not efp.stateful_inspection else 0.4, 'ip_fragmentation': 0.8 if efp.supports_ip_frag else 0.1, 'payload_encryption': 0.9 if not efp.ml_detection_blocked else 0.6, 'timing_evasion': 0.6 if efp.rate_limiting_detected else 0.8, 'checksum_confusion': 0.9 if not efp.checksum_validation else 0.0}
    efp.network_path = _trace_network_path(target_ip)
    efp.asn_info = _get_asn_info(target_ip)
    LOG.info(f'Comprehensive analysis complete: {classification.dpi_type} [{classification.confidence:.0%}]')
    return efp
