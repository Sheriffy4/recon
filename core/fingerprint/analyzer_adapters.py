        ...
        try:
            ...
            if isinstance(result_tuple, tuple) and len(result_tuple) == 2:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Could not map predicted DPI type '{dpi_type_str}' to enum. Falling back to UNKNOWN."
                    )
...
class RealEffectivenessTesterAdapter(BaseAnalyzerAdapter):
    ...
    def __init__(self, timeout: float = 30.0):
        ...
        if not EFFECTIVENESS_TESTER_AVAILABLE:
            raise AnalyzerError(
                f"RealEffectivenessTester not available: {EFFECTIVENESS_TESTER_IMPORT_ERROR}"
            )
