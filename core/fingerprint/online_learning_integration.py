# recon/core/fingerprint/online_learning_integration.py
"""
...
class FeedbackData:
    ...
    feedback_source: str  # 'user_manual', 'validation_test', 'expert_review'
    timestamp: float
...
class OnlineLearningIntegrator:
    ...
    def __init__(self, 
        ...
        # Initialize online learning system if enabled
        if self.enable_online_learning:
            self.online_learning = OnlineLearningSystem(
                ml_classifier=ml_classifier,
                learning_mode=learning_mode,
                buffer_size=500,  # Larger buffer for production
    ...
    def classify_with_learning(self, metrics: Dict[str, Any], 
                             target: str = None) -> Tuple[str, float, List[Tuple[str, float]]]:
        """
        ...
        if self.online_learning and self.online_learning.active_ab_test:
            # Use A/B testing if active
            dpi_type, confidence, model_used = self.online_learning.classify_with_ab_test(metrics)
            alternatives = []  # Simplified for A/B testing
            
            LOG.debug(f"A/B test classification: {dpi_type} (confidence: {confidence:.3f}, "
                     f"model: {model_used})")
        else:
    ...
    def add_user_feedback(self, feedback: FeedbackData) -> bool:
        ...
        try:
            ...
            # Adjust confidence based on feedback confidence
            ...
            if learned:
                ...
                LOG.info(f"User feedback processed: {predicted_type} -> {feedback.user_reported_type} "
                        f"for target {feedback.target}")
            ...
            if self.online_learning.active_ab_test:
                self.online_learning.record_ab_test_result(
                    ...
                    actual_type=feedback.user_reported_type,
                    confidence=adjusted_confidence,
                    model_used='control'  # Assume control for user feedback
                )
    ...
    def add_validation_result(self, 
                            ...
                            validated_type: str,
                            confidence: float,
                            validation_method: str = "automated") -> bool:
        """
        ...
        try:
            ...
            if learned:
                self.stats['learning_examples_added'] += 1
                LOG.debug(f"Validation result processed: {predicted_type} -> {validated_type} "
                         f"for {target}")
    ...
    def start_model_ab_test(self, 
                          test_model_path: str,
                          test_name: str = None,
                          traffic_split: float = 0.1,
                          min_samples: int = 100,
                          success_threshold: float = 0.05) -> bool:
        """
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        if learning_stats['baseline_performance'] and len(self.online_learning.performance_history) > 10:
            recent_accuracy = sum(1 for ex in list(self.online_learning.performance_history)[-10:] 
                                if ex.predicted_type == ex.actual_type) / 10
    ...
    def _extract_metrics_from_fingerprint(self, fingerprint: DPIFingerprint) -> Dict[str, Any]:
        ...
        """
        # Convert fingerprint to metrics dictionary
        # This maps the fingerprint fields to the expected metrics format
        ...
        metrics = {
            ...
            'dns_resolution_time_ms': 30.0,  # Default value
            'handshake_time_ms': 80.0,       # Default value
            
            # TCP metrics
            'rst_ttl': 63,  # Default value
            'rst_distance': 10,  # Default value
            'window_size_in_rst': 0,
            'tcp_option_len_limit': 40,
            'dpi_hop_distance': 8,
            
            # Boolean features from fingerprint
            ...
            'tls_version_sensitivity': 'blocks_tls13' if getattr(fingerprint, 'http_header_filtering', False) else 'no_version_preference',
            'ipv6_handling': 'allowed' if getattr(fingerprint, 'supports_ipv6', True) else 'blocked',
...
def create_online_learning_integrator(ml_classifier: MLClassifier, 
                                    enable_online_learning: bool = True,
                                    learning_mode: LearningMode = LearningMode.MODERATE) -> OnlineLearningIntegrator:
    """
