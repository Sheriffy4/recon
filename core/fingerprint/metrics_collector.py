...
class ComprehensiveMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'target': self.target, 'timestamp': self.timestamp, 'timing': {'latency_ms': self.timing.latency_ms, 'jitter_ms': self.timing.jitter_ms, 'packet_timing': self.timing.packet_timing, 'connection_time_ms': self.timing.connection_time_ms, 'first_byte_time_ms': self.timing.first_byte_time_ms, 'total_time_ms': self.timing.total_time_ms, 'timeout_occurred': self.timing.timeout_occurred, 'retransmission_count': self.timing.retransmission_count}, 'network': {'packet_loss_rate': self.network.packet_loss_rate, 'out_of_order_packets': self.network.out_of_order_packets, 'duplicate_packets': self.network.duplicate_packets, 'fragmented_packets': self.network.fragmented_packets, 'mtu_discovery_blocked': self.network.mtu_discovery_blocked, 'icmp_responses': self.network.icmp_responses, 'tcp_window_scaling': self.network.tcp_window_scaling, 'tcp_options': self.network.tcp_options}, 'protocols': {proto: {'protocol': metrics.protocol, 'success_rate': metrics.success_rate, 'error_codes': metrics.error_codes, 'response_sizes': metrics.response_sizes, 'header_modifications': metrics.header_modifications, 'content_modifications': metrics.content_modifications, 'redirect_responses': metrics.redirect_responses, 'blocked_responses': metrics.blocked_responses} for proto, metrics in self.protocols.items()}, 'raw_data': self.raw_data, 'collection_errors': self.collection_errors, 'reliability_score': self.reliability_score}
...
class TimingMetricsCollector(BaseMetricsCollector):
    ...
    async def collect_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        for i in range(self.samples):
            ...
            for retry in range(max_retries):
                try:
                    ...
                    reader, writer = await asyncio.wait_for(asyncio.open_connection(target, port), timeout=adjusted_timeout)
        ...
        self.timing_history.append({'timestamp': time.time(), 'latency_ms': latency_ms, 'jitter_ms': jitter_ms})
        return {'latency_ms': latency_ms, 'jitter_ms': jitter_ms, 'packet_timing': timing_data, 'connection_time_ms': avg_connection_time, 'first_byte_time_ms': avg_first_byte_time, 'total_time_ms': statistics.mean(timing_data) if timing_data else 0.0, 'timeout_occurred': any((t >= self.timeout * 1000 for t in timing_data)), 'retransmission_count': 0, 'samples_collected': len(timing_data), 'success_rate': len([t for t in timing_data if t < self.timeout * 1000]) / self.samples if self.samples > 0 else 0.0}
    ...
    def get_timing_trends(self) -> Dict[str, Any]:
        ...
        return {'latency_trend': 'increasing' if recent_latencies[-1] > recent_latencies[0] else 'decreasing', 'jitter_trend': 'increasing' if recent_jitters[-1] > recent_jitters[0] else 'decreasing', 'stability_score': 1.0 - statistics.stdev(recent_latencies) / statistics.mean(recent_latencies) if recent_latencies and statistics.mean(recent_latencies) > 0 else 0.0}
...
class NetworkMetricsCollector(BaseMetricsCollector):
    ...
    async def collect_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        metrics = {'packet_loss_rate': 0.0, 'out_of_order_packets': 0, 'duplicate_packets': 0, 'fragmented_packets': 0, 'mtu_discovery_blocked': False, 'icmp_responses': [], 'tcp_window_scaling': False, 'tcp_options': []}
        try:
            metrics['mtu_discovery_blocked'] = await self._test_mtu_discovery(target, port)
    ...
    async def _estimate_packet_loss(self, target: str, port: int) -> float:
        ...
        for _ in range(attempts):
            try:
                reader, writer = await asyncio.wait_for(asyncio.open_connection(target, port), timeout=self.timeout / 2)
                successes += 1
                writer.close()
                await writer.wait_closed()
            except Exception:
...
class ProtocolMetricsCollector(BaseMetricsCollector):
    ...
    def __init__(self, timeout: float=10.0):
        super().__init__(timeout)
        self.protocol_handlers = {'http': self._collect_http_metrics, 'https': self._collect_https_metrics, 'dns': self._collect_dns_metrics, 'tcp': self._collect_tcp_metrics}

    async def collect_metrics(self, target: str, port: int, protocol: str='auto', **kwargs) -> Dict[str, Any]:
        """Collect protocol-specific metrics"""
        if protocol == 'auto':
    ...
    def _detect_protocol(self, port: int) -> str:
        ...
        port_mapping = {80: 'http', 443: 'https', 53: 'dns', 8080: 'http', 8443: 'https'}
    ...
    async def _collect_http_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        return {'protocol': 'http', 'success_rate': metrics.success_rate, 'error_codes': metrics.error_codes, 'response_sizes': metrics.response_sizes, 'header_modifications': {}, 'content_modifications': False, 'redirect_responses': metrics.redirect_responses, 'blocked_responses': metrics.blocked_responses, 'avg_response_time_ms': statistics.mean(response_times) if response_times else 0.0, 'response_time_jitter_ms': statistics.stdev(response_times) if len(response_times) > 1 else 0.0}
    ...
    async def _collect_https_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            for retry in range(max_retries):
                try:
                    ...
                    start_time = time.perf_counter()
                    adjusted_timeout = self.timeout * (1 + retry * 0.5)
                    reader, writer = await asyncio.wait_for(asyncio.open_connection(target, port, ssl=context), timeout=adjusted_timeout)
                    handshake_time = (time.perf_counter() - start_time) * 1000
                    ssl_object = writer.get_extra_info('ssl_object')
                    if ssl_object:
                        cipher = ssl_object.cipher()
                        version = ssl_object.version()
                        metrics.raw_data = {'tls_version': version, 'cipher_suite': cipher[0] if cipher else None, 'handshake_time_ms': handshake_time}
                    metrics.success_rate = 1.0
                    writer.close()
                    await writer.wait_closed()
                    return {'protocol': 'https', 'success_rate': 1.0, 'error_codes': [], 'response_sizes': [], 'header_modifications': {}, 'content_modifications': False, 'redirect_responses': 0, 'blocked_responses': 0, 'tls_handshake_time_ms': handshake_time, 'raw_data': metrics.raw_data}
            ...
            return {'protocol': 'https', 'success_rate': 0.0, 'error_codes': [], 'response_sizes': [], 'header_modifications': {}, 'content_modifications': False, 'redirect_responses': 0, 'blocked_responses': 1, 'tls_handshake_time_ms': 0.0, 'error': str(last_error)}
            handshake_time = (time.perf_counter() - start_time) * 1000
            ssl_object = writer.get_extra_info('ssl_object')
            if ssl_object:
                cipher = ssl_object.cipher()
        ...
        return {'protocol': 'https', 'success_rate': metrics.success_rate, 'error_codes': [], 'response_sizes': [], 'header_modifications': {}, 'content_modifications': False, 'redirect_responses': 0, 'blocked_responses': 1 if metrics.success_rate == 0.0 else 0, 'tls_handshake_time_ms': metrics.raw_data.get('handshake_time_ms', 0.0) if hasattr(metrics, 'raw_data') else 0.0}

    async def _collect_dns_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics.success_rate = 0.0
        return {'protocol': 'dns', 'success_rate': metrics.success_rate, 'error_codes': [], 'response_sizes': [], 'header_modifications': {}, 'content_modifications': False, 'redirect_responses': 0, 'blocked_responses': 1 if metrics.success_rate == 0.0 else 0}

    async def _collect_tcp_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            writer.close()
            await writer.wait_closed()
            return {'protocol': 'tcp', 'success_rate': 1.0, 'error_codes': [], 'response_sizes': [], 'header_modifications': {}, 'content_modifications': False, 'redirect_responses': 0, 'blocked_responses': 0, 'connection_time_ms': connection_time}
        except Exception as e:
            self.logger.debug(f'TCP connection failed: {e}')
            return {'protocol': 'tcp', 'success_rate': 0.0, 'error_codes': [], 'response_sizes': [], 'header_modifications': {}, 'content_modifications': False, 'redirect_responses': 0, 'blocked_responses': 1, 'connection_time_ms': 0.0}

    async def _collect_generic_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
...
class MetricsCollector:
    ...
    async def collect_comprehensive_metrics(self, target: str, port: int=443, protocols: Optional[List[str]]=None, include_timing: bool=True, include_network: bool=True, include_protocol: bool=True) -> ComprehensiveMetrics:
        """
        ...
        """
        self.logger.info(f'Starting comprehensive metrics collection for {target}:{port}')
        metrics = ComprehensiveMetrics(target=target)
        collection_tasks = []
        try:
            if include_timing:
                collection_tasks.append(self._collect_with_error_handling('timing', self.timing_collector.collect_metrics(target, port)))
            if include_network:
                collection_tasks.append(self._collect_with_error_handling('network', self.network_collector.collect_metrics(target, port)))
            if include_protocol:
                if protocols is None:
                    protocols = [self.protocol_collector._detect_protocol(port)]
                for protocol in protocols:
                    collection_tasks.append(self._collect_with_error_handling(f'protocol_{protocol}', self.protocol_collector.collect_metrics(target, port, protocol)))
            ...
            for i, result in enumerate(results):
                ...
                if task_name == 'timing' and task_result:
                    timing_fields = {'latency_ms', 'jitter_ms', 'packet_timing', 'connection_time_ms', 'first_byte_time_ms', 'total_time_ms', 'timeout_occurred', 'retransmission_count'}
                    filtered_result = {k: v for k, v in task_result.items() if k in timing_fields}
                    metrics.timing = TimingMetrics(**filtered_result)
                elif task_name == 'network' and task_result:
                    network_fields = {'packet_loss_rate', 'out_of_order_packets', 'duplicate_packets', 'fragmented_packets', 'mtu_discovery_blocked', 'icmp_responses', 'tcp_window_scaling', 'tcp_options'}
                    filtered_result = {k: v for k, v in task_result.items() if k in network_fields}
                    ...
                    protocol_fields = {'protocol', 'success_rate', 'error_codes', 'response_sizes', 'header_modifications', 'content_modifications', 'redirect_responses', 'blocked_responses'}
                    filtered_result = {k: v for k, v in task_result.items() if k in protocol_fields}
                    metrics.protocols[protocol_name] = ProtocolMetrics(**filtered_result)
            metrics.reliability_score = self._calculate_reliability_score(metrics)
            metrics.raw_data = {'collection_timestamp': time.time(), 'target': target, 'port': port, 'protocols_tested': list(metrics.protocols.keys()), 'timing_samples': len(metrics.timing.packet_timing), 'errors_encountered': len(metrics.collection_errors)}
            self.logger.info(f'Metrics collection complete. Reliability: {metrics.reliability_score:.2f}, Errors: {len(metrics.collection_errors)}')
        except Exception as e:
    ...
    async def _collect_with_error_handling(self, task_name: str, coro) -> Tuple[str, Optional[Dict[str, Any]]]:
    ...
    def validate_comprehensive_metrics(self, metrics: ComprehensiveMetrics) -> List[str]:
        ...
        for protocol, protocol_metrics in metrics.protocols.items():
            if not 0.0 <= protocol_metrics.success_rate <= 1.0:
                errors.append(f'Success rate for {protocol} must be between 0.0 and 1.0')
    ...
    def aggregate_metrics(self, metrics_list: List[ComprehensiveMetrics]) -> ComprehensiveMetrics:
        ...
        for metrics in metrics_list:
            if metrics.timing.latency_ms > 0:
                all_latencies.append(metrics.timing.latency_ms)
            if metrics.timing.jitter_ms > 0:
                all_jitters.append(metrics.timing.jitter_ms)
            all_packet_timings.extend(metrics.timing.packet_timing)
        if all_latencies:
        ...
        for metrics in metrics_list:
            for protocol, protocol_metrics in metrics.protocols.items():
                protocol_aggregates[protocol].append(protocol_metrics)
        for protocol, protocol_list in protocol_aggregates.items():
            success_rates = [p.success_rate for p in protocol_list]
            aggregated.protocols[protocol] = ProtocolMetrics(protocol=protocol, success_rate=statistics.mean(success_rates) if success_rates else 0.0)
        all_errors = []
        for metrics in metrics_list:
            all_errors.extend(metrics.collection_errors)
