# recon/core/fingerprint/metrics_collector.py
"""
...
class NetworkMetricsCollector(BaseMetricsCollector):
    ...
    async def collect_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Estimate packet loss through multiple connection attempts
    ...
    async def _test_mtu_discovery(self, target: str, port: int) -> bool:
        ...
        try:
            # This would require raw socket access for proper implementation
            # For now, return a heuristic based on connection behavior
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(target, port),
                timeout=self.timeout
            )
            
            # Send a large packet and see if it gets through
            large_data = b"X" * 1400  # Close to standard MTU
            writer.write(large_data)
            await writer.drain()
            
            writer.close()
            await writer.wait_closed()
            return False  # If we get here, MTU discovery likely works
            
        except Exception:
            return True  # Assume blocked if connection fails
    
    async def _test_tcp_options(self, target: str, port: int) -> List[str]:
        """Test which TCP options are supported"""
        # This would require raw socket implementation for full functionality
        # For now, return common options that can be inferred
        try:
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(target, port),
                timeout=self.timeout
            ...
            # Basic connection successful, assume standard options
    ...
    async def _test_fragmentation(self, target: str, port: int) -> int:
        ...
        # This would require packet capture for accurate measurement
    ...
    async def _estimate_packet_loss(self, target: str, port: int) -> float:
        ...
        for _ in range(attempts):
            try:
                reader, writer = await asyncio.wait_for(
                    asyncio.open_connection(target, port),
                    timeout=self.timeout / 2  # Shorter timeout for loss estimation
                )
                successes += 1
                writer.close()
                await writer.wait_closed()
...
class ProtocolMetricsCollector(BaseMetricsCollector):
    ...
    async def _collect_http_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            'error_codes': metrics.error_codes,
            'response_sizes': metrics.response_sizes,
            'header_modifications': {},  # Would need deeper analysis
            'content_modifications': False,  # Would need content comparison
            'redirect_responses': metrics.redirect_responses,
            'blocked_responses': metrics.blocked_responses,
            'avg_response_time_ms': statistics.mean(response_times) if response_times else 0.0,
            'response_time_jitter_ms': statistics.stdev(response_times) if len(response_times) > 1 else 0.0
        }
    ...
    async def _collect_https_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            'tls_handshake_time_ms': metrics.raw_data.get('handshake_time_ms', 0.0) if hasattr(metrics, 'raw_data') else 0.0
        }
    
    async def _collect_dns_metrics(self, target: str, port: int, **kwargs) -> Dict[str, Any]:
        """Collect DNS-specific metrics"""
        # Basic DNS query metrics
...
class MetricsCollector:
        ...
        try:
            ...
            if include_timing:
                collection_tasks.append(
                    self._collect_with_error_handling(
                        'timing',
                        self.timing_collector.collect_metrics(target, port)
                    )
                )
            ...
            if include_network:
                collection_tasks.append(
                    self._collect_with_error_handling(
                        'network',
                        self.network_collector.collect_metrics(target, port)
                    )
                )
                ...
                for protocol in protocols:
                    collection_tasks.append(
                        self._collect_with_error_handling(
                            ...
                            self.protocol_collector.collect_metrics(target, port, protocol)
                        )
                    )
            ...
            for i, result in enumerate(results):
                ...
                if task_name == 'timing' and task_result:
                    # Filter out extra fields that aren't part of TimingMetrics
                    timing_fields = {
                        'latency_ms', 'jitter_ms', 'packet_timing', 'connection_time_ms',
                        'first_byte_time_ms', 'total_time_ms', 'timeout_occurred', 'retransmission_count'
                    }
                    ...
                    # Filter out extra fields that aren't part of NetworkMetrics
                    network_fields = {
                        'packet_loss_rate', 'out_of_order_packets', 'duplicate_packets',
                        'fragmented_packets', 'mtu_discovery_blocked', 'icmp_responses',
                        'tcp_window_scaling', 'tcp_options'
                    }
                    ...
                    # Filter out extra fields that aren't part of ProtocolMetrics
                    protocol_fields = {
                        'protocol', 'success_rate', 'error_codes', 'response_sizes',
                        'header_modifications', 'content_modifications', 'redirect_responses',
                        'blocked_responses'
                    }
            ...
            self.logger.info(
                f"Metrics collection complete. Reliability: {metrics.reliability_score:.2f}, "
                f"Errors: {len(metrics.collection_errors)}"
            )
    ...
    def _calculate_reliability_score(self, metrics: ComprehensiveMetrics) -> float:
        ...
        if metrics.timing.packet_timing:
            timing_score = min(1.0, len(metrics.timing.packet_timing) / 10.0)  # Full score at 10+ samples
            if not metrics.timing.timeout_occurred:
                timing_score *= 1.2  # Bonus for no timeouts
    ...
    def aggregate_metrics(self, metrics_list: List[ComprehensiveMetrics]) -> ComprehensiveMetrics:
        ...
        # Aggregate network metrics (take worst case for most metrics)
        ...
        aggregated.collection_errors = list(set(all_errors))  # Remove duplicates
