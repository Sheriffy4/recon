...
class MetricsCollector:
    ...
    async def collect_comprehensive_metrics(
        ...
        try:
            ...
            if include_timing:
                collection_tasks.append(
                    self._collect_with_error_handling(
                        "timing", self.timing_collector.collect_metrics(target, port)
                    )
                )
            ...
            if include_network:
                collection_tasks.append(
                    self._collect_with_error_handling(
                        "network", self.network_collector.collect_metrics(target, port)
                    )
                )
    ...
    def _calculate_reliability_score(self, metrics: ComprehensiveMetrics) -> float:
        ...
        if metrics.timing.packet_timing:
            timing_score = min(
                1.0, len(metrics.timing.packet_timing) / 10.0
            )  # Full score at 10+ samples
