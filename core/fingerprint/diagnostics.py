...
class DiagnosticLogger:
    ...
    def log_fingerprinting_start(self, target: str):
        ...
        self.logger.info(
            "Starting fingerprinting operation",
            extra={'fingerprint_target': target}
        )
    
    def log_fingerprinting_complete(self, target: str, fingerprint: DPIFingerprint, duration: float):
    ...
    def log_analyzer_result(self, analyzer: str, target: str, result: Dict[str, Any], duration: float):
        """Log analyzer result."""
        self.logger.debug(
...
class DiagnosticSystem:
    ...
    def record_fingerprinting_operation(self, target: str, success: bool, duration: float, 
        ...
        with self.lock:
            ...
            if success:
                ...
                if fingerprint:
                    self.fingerprinting_stats['dpi_type_distribution'][fingerprint.dpi_type.value] += 1
            else:
            ...
            self.fingerprinting_stats['average_duration'] = (current_avg * (total - 1) + duration) / total
        
        # Record metrics
        self.metrics_collector.record_metric("fingerprinting_duration", duration, "seconds", 
                                            {"target": target, "success": str(success)})
        
        if success and fingerprint:
            self.metrics_collector.record_metric("fingerprinting_confidence", fingerprint.confidence, 
                                                "score", {"dpi_type": fingerprint.dpi_type.value})
    ...
    def record_analyzer_operation(self, analyzer: str, target: str, duration: float, 
                                success: bool, result: Optional[Dict[str, Any]] = None):
        """Record analyzer operation metrics."""
        self.metrics_collector.record_metric(f"analyzer_{analyzer}_duration", duration, "seconds",
                                            {"target": target, "success": str(success)})
    ...
    def record_ml_classification(self, target: str, duration: float, prediction: Dict[str, Any]):
        ...
        self.metrics_collector.record_metric("ml_classification_duration", duration, "seconds",
                                            {"target": target})
        
        if 'confidence' in prediction:
            self.metrics_collector.record_metric("ml_classification_confidence", 
                                                prediction['confidence'], "score",
                                                {"dpi_type": prediction.get('dpi_type', 'unknown')})
        
        self.logger.log_ml_classification(target, prediction, duration)
    
    def record_cache_operation(self, operation: str, target: str, hit: bool = None, duration: float = None):
        """Record cache operation metrics."""
        if duration is not None:
            self.metrics_collector.record_metric(f"cache_{operation}_duration", duration, "seconds",
                                                {"target": target})
    ...
    def generate_diagnostic_report(self) -> DiagnosticReport:
        ...
        return DiagnosticReport(
            ...
            performance_metrics=[
                PerformanceMetric(name=name, value=stats.get('latest', 0), unit="", 
