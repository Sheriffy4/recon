...
def demo_default_configuration():
    ...
    print(
        f"   Max Concurrent Fingerprints: {config.performance.max_concurrent_fingerprints}"
    )
...
def demo_analyzer_configuration():
    ...
    print("🛠️  Analyzer Management Operations:")
...
def demo_feature_flags():
    ...
    print(
        f"   Experimental Features: {config.is_feature_enabled('experimental_features')}"
    )
...
def demo_configuration_serialization():
    ...
    config.disable_analyzer("dns")
    config.enable_feature("experimental_features")
    config.custom_settings = {"custom_param": "custom_value", "nested": {"param": 42}}
...
def demo_runtime_configuration_updates():
    ...
    print("   Updating analyzer configurations...")
    ...
    print(
        f"   Experimental Features: {manager.config.is_feature_enabled('experimental_features')}"
    )
    print(
        f"   Cache Compression: {manager.config.is_feature_enabled('cache_compression')}"
    )
...
def demo_performance_tuning_scenarios():
    ...
    print("🚀 Performance Tuning Scenarios:")
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        enabled_analyzers = [
            name for name, cfg in config.analyzers.items() if cfg.enabled
        ]
...
def demo_global_configuration_management():
    ...
    print(
        f"   Experimental Features: {updated_config.is_feature_enabled('experimental_features')}"
    )
