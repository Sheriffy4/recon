...
class BehavioralProber:
    ...
    async def _probe_timing_analysis(
        ...
        def probe():
            try:
                ...
                if len(successful_samples) >= 5:
                    ...
                    self.logger.debug(
                        f"Timing analysis: avg_connect={avg_connect:.2f}ms, "
    ...
    async def _probe_session_fingerprinting(
        self, result: BehavioralProbeResult, target_ip: str, port: int
    ):
        """
        ...
        def probe():
            try:
                ...
                self.logger.debug(
                    f"Session fingerprinting: tracking={result.session_tracking_detected}, "
    ...
    async def _probe_dpi_adaptation(
        ...
        def probe():
            try:
                ...
                if len(successful_attempts) >= 5:
                    ...
                    if len(response_times) >= 5:
                        ...
                        if late_avg > early_avg * 1.5:  # 50% increase suggests learning
                            ...
                            result.adaptation_time_window = (
                                len(successful_attempts) * 1.0
                            )  # seconds
                ...
                self.logger.debug(
                    f"DPI adaptation: learning={result.dpi_learning_detected}, "
    ...
    async def _probe_connection_patterns(
        self, result: BehavioralProbeResult, target_ip: str, port: int
    ):
        """
        ...
        def probe():
            try:
                            ...
                            rate_limit_results.append(
                                {
                                    ...
                                    "connect_time": connect_time * 1000,
                                    "success": len(response) > 0,
                                    "rate_limited": b"429" in response
                                    or b"rate" in response.lower(),
                                }
                            )
                        except socket.timeout:
                            rate_limit_results.append(
                ...
                self.logger.debug(
                    f"Connection patterns: max_concurrent={max_concurrent}, "
