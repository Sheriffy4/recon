...
class FingerprintAccuracyValidator:
    ...
    async def run_comprehensive_validation(self) -> ValidationSummary:
        ...
        LOG.info(
            f"Starting comprehensive fingerprint validation with {len(self.test_cases)} test cases"
        )
        ...
        for test_case in self.test_cases:
            ...
            if result.dpi_type_accurate:
                ...
                LOG.warning(
                    f"  ✗ DPI type mismatch: expected {result.expected_dpi_type}, got {result.detected_dpi_type}"
                )
            ...
            if result.confidence_accurate:
                ...
                LOG.warning(
                    f"  ✗ Low confidence: {result.actual_confidence:.2f} < {result.expected_confidence:.2f}"
                )
    ...
    async def _run_single_test_case(
        ...
        try:
            ...
            fingerprint_result = await self._simulate_fingerprinting(
                test_case.domain, test_case.network_simulation_data
            )
            ...
            if fingerprint_result:
                ...
                dpi_type_accurate = self._is_dpi_type_accurate(
                    test_case.expected_dpi_type, detected_dpi_type
                )
                ...
                strategy_accuracy = self._evaluate_strategy_accuracy(
                    test_case.expected_strategies, strategy_recommendations
                )
    ...
    async def _simulate_fingerprinting(
        ...
        if self.fingerprint_integrator:
            try:
                ...
                result = await self.fingerprint_integrator.fingerprint_target(
                    domain=domain, target_ip="1.2.3.4"
                )
    ...
    def _simulate_fingerprint_from_data(
        ...
        if (
            network_data.get("rst_ttl") in [60, 61, 62, 63, 64]
            and not network_data.get("rst_from_target", True)
            and network_data.get("stateful_inspection", False)
        ):
        ...
        # Sandvine detection
            ...
            network_data.get("rst_ttl") == 128
            and network_data.get("checksum_validation", False)
            and network_data.get("application_layer_inspection", False)
        ):
            dpi_type = "sandvine"
            confidence = 0.75
        ...
        # Great Firewall detection
            ...
            not network_data.get("supports_ip_frag", True)
            and network_data.get("ja3_fingerprint_detected", False)
            and network_data.get("machine_learning_classification", False)
        ):
            dpi_type = "gfw"
            confidence = 0.78
        ...
        # Cloudflare detection
            ...
            network_data.get("cdn_edge_detection", False)
            and network_data.get("load_balancer_fingerprinting", False)
            and network_data.get("http2_frame_analysis", False)
        ):
        ...
        # AWS WAF detection
            ...
            and network_data.get("machine_learning_classification", False)
            and network_data.get("geo_blocking_patterns", False)
        ):
            dpi_type = "aws_waf"
            confidence = 0.80
        ...
        # Palo Alto detection
            ...
            network_data.get("threat_intelligence_integration", False)
            and network_data.get("zero_day_detection_capability", False)
            and network_data.get("tunnel_detection_capability", False)
        ):
    ...
    def _evaluate_strategy_accuracy(
        self, expected_strategies: List[str], recommended_strategies: List[str]
    ) -> float:
    ...
    def _generate_validation_summary(self, total_time: float) -> ValidationSummary:
        ...
        confidence_accuracies = [
            r.actual_confidence for r in self.test_results if r.confidence_accurate
        ]
        average_confidence_accuracy = (
            statistics.mean(confidence_accuracies) if confidence_accuracies else 0.0
        )

        strategy_accuracies = [r.strategy_accuracy for r in self.test_results]
        average_strategy_accuracy = (
            statistics.mean(strategy_accuracies) if strategy_accuracies else 0.0
        )
    ...
    def get_accuracy_trends(self) -> Dict[str, Any]:
        ...
        trends = {
            ...
            - previous.average_strategy_accuracy,
    ...
    def get_problematic_test_cases(self) -> List[AccuracyTestResult]:
        ...
        return [
            ...
            if not result.dpi_type_accurate
            or not result.confidence_accurate
            or result.strategy_accuracy < 0.5
        ]

    # Example usage
    async def validate_strategy_recommendations(
        self, test_cases: Optional[List[ValidationTestCase]] = None
    ) -> Dict[str, Any]:
        """
        ...
        for test_case in test_cases:
            try:
                ...
                validation_results["detailed_results"].append(
                    {"test_id": test_case.test_id, "error": str(e)}
                )

        # Calculate overall metrics
        if validation_results["strategy_accuracy_scores"]:
            validation_results["average_f1_score"] = statistics.mean(
        ...
        LOG.info(
            f"Strategy validation complete: {validation_results['accuracy_rate']:.2%} accuracy, "
    ...
    try:
        ...
        if problematic:
            ...
            for case in problematic[:3]:  # Show first 3
                LOG.warning(
                    f"  - {case.test_id}: Expected {case.expected_dpi_type}, got {case.detected_dpi_type}"
                )
