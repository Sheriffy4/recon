...
try:
    from scapy.all import (
        IP, TCP, Raw, sr1, send, sr, fragment, 
...
class AdvancedTCPProber:
    ...
    async def _probe_packet_reordering(self, result: AdvancedTCPProbeResult, target_ip: str, port: int):
        """
    ...
    async def _probe_ip_fragmentation_overlap(self, result: AdvancedTCPProbeResult, target_ip: str, port: int):
        """
        ...
        def probe():
            try:
                ...
                # Create base packet
                base_pkt = IP(dst=target_ip) / TCP(
                    dport=port, sport=random.randint(10000, 65000),
                    flags="S"
                ) / Raw(large_payload)
                ...
                if len(fragments) >= 2:
                    ...
                    self.logger.debug(f"IP fragmentation overlap test: {result.ip_fragmentation_overlap_handling}")
    ...
    async def _probe_exotic_tcp_flags(self, result: AdvancedTCPProbeResult, target_ip: str, port: int):
        """
        ...
        def probe():
            try:
                ...
                for i, options in enumerate(exotic_options):
                    try:
                        pkt = IP(dst=target_ip) / TCP(
                            sport=sport + i, dport=port, 
                            flags="S", options=options
                        )
            ...
            try:
                ...
                if rst_ttls and synack_ttls:
                    ...
                    if max_rst_ttl > 0:
                        ...
                        self.logger.debug(f"DPI distance estimated at {result.dpi_distance_hops} hops")
                ...
                if normal_response and normal_response.haslayer(IP):
                    ...
                    if response_ttl not in [64, 63, 62, 61, 128, 127, 126, 125, 255, 254, 253, 252]:
                        result.ttl_manipulation_detected = True
