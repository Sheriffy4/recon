# recon/core/fingerprint/model_trainer.py
"""
...
from .training_data import TrainingDataGenerator, FeatureEngineer
...
class ModelTrainer:
    ...
    def prepare_training_data(self, include_synthetic: bool = True, 
                            save_to_file: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def train_model_with_evaluation(self, training_data: Optional[List[Dict[str, Any]]] = None,
                                  test_size: float = 0.2, cv_folds: int = 5,
                                  random_state: int = 42) -> ModelEvaluationMetrics:
        """
        ...
        if min_class_count < 2:
            LOG.warning(f"Some classes have only 1 example, disabling stratification")
        ...
        evaluation_metrics = self._calculate_evaluation_metrics(
            model, X, y, X_test, y_test, y_pred, y_pred_proba,
            feature_names, cv_folds, random_state, min_class_count
        )
        
        # Update ML classifier with trained model
    ...
    def _calculate_evaluation_metrics(self, model, X, y, X_test, y_test, y_pred, y_pred_proba,
                                    feature_names: List[str], cv_folds: int, 
                                    random_state: int, min_class_count: int) -> ModelEvaluationMetrics:
        ...
        # Cross-validation - adjust folds if necessary
        n_samples = len(X)
    ...
    def get_feature_importance_report(self, top_n: int = 10) -> Dict[str, Any]:
        ...
        sorted_features = sorted(
            self.evaluation_metrics.feature_importance.items(),
            key=lambda x: x[1],
            reverse=True
        )
    ...
    def quick_train_and_evaluate(self, save_results: bool = True) -> ModelEvaluationMetrics:
        ...
        training_data = self.prepare_training_data(
            ...
            save_to_file="training_data.json" if save_results else None
        )
