# recon/core/fingerprint/dpi_monitor_demo.py
"""
...
class DPIMonitoringDemo:
    ...
    def setup_components(self):
        ...
        self.fingerprinter = AdvancedFingerprinter(
            config=fingerprint_config, cache_file="demo_fingerprint_cache.pkl"
        )
        ...
        monitoring_config = MonitoringConfig(
            ...
            behavior_change_confidence_threshold=0.6,
            unknown_pattern_threshold=0.4,
            enable_alerts=True,
            alert_retention_days=1,  # Short retention for demo
            max_alerts_per_target=3,
            enable_strategy_testing=True,
            strategy_test_timeout=5.0,
            max_strategies_to_test=2,
            save_behavior_changes=True,
            behavior_log_file="demo_behavior_changes.json",
            alerts_file="demo_alerts.json",
        )

        # Create behavior monitor with alert callback
    ...
    async def demo_basic_monitoring(self):
        ...
        # Add some targets for monitoring
    ...
    async def demo_force_checks(self):
        ...
        for domain, port in test_targets:
            ...
            try:
                ...
                if target_status:
                    logger.info(
                        f"  Target status: {target_status['behavior_changes']} changes, "
                        f"{target_status['active_alerts']} active alerts"
                    )
    ...
    async def demo_performance_monitoring(self):
        ...
        adaptive_interval = perf_monitor.get_adaptive_interval(
            base_interval, min_interval, max_interval, cpu_threshold, memory_threshold
        )

        logger.info(f"Base monitoring interval: {base_interval}s")
        logger.info(f"Adaptive interval: {adaptive_interval}s")

        is_overloaded = perf_monitor.is_system_overloaded(
        ...
        if is_overloaded:
            logger.info(
                "  System is under high load - monitoring frequency will be reduced"
            )
        else:
    ...
    async def demo_monitoring_statistics(self):
        ...
        logger.info(
            f"  Behavior changes detected: {stats['behavior_changes_detected']}"
        )
        ...
        # Show fingerprinter stats if available
        try:
            ...
            logger.info(
                f"  Fallback classifications: {fp_stats['fallback_classifications']}"
            )
