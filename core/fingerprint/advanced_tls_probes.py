...
class AdvancedTLSProber:
    ...
    async def _probe_clienthello_size_sensitivity(self, result: AdvancedTLSProbeResult, 
                                                target: str, target_ip: str, port: int):
        """
        ...
        def probe():
            try:
                ...
                successful_sizes = [size for size, res in size_results.items() 
                                  if res.get("status") == "success"]
                failed_sizes = [size for size, res in size_results.items() 
                ...
                if failed_sizes and successful_sizes:
                    self.logger.debug(f"ClientHello size sensitivity detected: "
    ...
    async def _probe_ech_support(self, result: AdvancedTLSProbeResult, 
                                target: str, target_ip: str, port: int):
        """
        ...
        def probe():
            try:
                ...
                self.logger.debug(f"ECH probe results: support={result.ech_support_detected}, "
    ...
    async def _probe_http2_support(self, result: AdvancedTLSProbeResult, 
                                 target: str, target_ip: str, port: int):
        """
        ...
        def probe():
            try:
                ...
                self.logger.debug(f"HTTP/2 support: {result.http2_support}, "
    ...
    async def _probe_dirty_http_traffic(self, result: AdvancedTLSProbeResult, 
                                      target: str, target_ip: str, port: int):
        """
        ...
        def probe():
            try:
                ...
                dirty_tests = {
                    ...
                    "long_uri": b"GET /" + b"A" * 2000 + b" HTTP/1.1\r\nHost: " + target.encode() + b"\r\n\r\n",
                        ...
                        try:
                            ...
                            if response:
                                ...
                                if b"HTTP/" in response:
                                    ...
                                    elif b"200" in response or b"301" in response or b"302" in response:
                ...
                rejected_tests = [test for test, result_str in result.dirty_http_tolerance.items() 
                                if "rejected" in result_str or "reset" in result_str]
                
                if rejected_tests:
                    result.http_header_filtering = rejected_tests
                
                self.logger.debug(f"Dirty HTTP tolerance results: {len(result.dirty_http_tolerance)} tests, "
