...
async def demo_dns_analysis():
    ...
    test_domains = ['google.com', 'facebook.com', 'twitter.com', 'blocked-test-domain.example']
    ...
    for domain in test_domains:
        ...
        try:
            ...
            print(f"- DNS Response Modification: {results['dns_response_modification']}")
            ...
            print(f"- Recursive Resolver Blocking: {results['recursive_resolver_blocking']}")
            print(f"- DNS Timeout Manipulation: {results['dns_timeout_manipulation']}")
            print(f"- EDNS Support: {results['edns_support']}")
            blocking_indicators = [results['dns_hijacking_detected'], results['dns_response_modification'], results['doh_blocking'], results['dot_blocking'], results['dns_cache_poisoning'], results['dns_over_tcp_blocking'], results['recursive_resolver_blocking'], results['dns_timeout_manipulation']]
            ...
            print(f'- Blocking Percentage: {blocking_count / total_indicators * 100:.1f}%')
            if blocking_count == 0:
...
def demo_response_analysis():
    ...
    suspicious_response = DNSResponse(timestamp=time.time(), query=test_query, response_time=0.1, status_code=0, answers=['0.0.0.0'], flags={'qr': True, 'aa': False})
    private_response = DNSResponse(timestamp=time.time(), query=test_query, response_time=0.1, status_code=0, answers=['192.168.1.1'], flags={'qr': True, 'aa': False})
