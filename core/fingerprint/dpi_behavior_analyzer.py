...
class DPIBehaviorAnalyzer:
    ...
    async def analyze(self, fingerprint: DPIFingerprint, force_all: bool=False) -> DPIFingerprint:
        ...
        try:
            ...
            self.logger.debug(f"Selected tests: {(tests_to_run if tests_to_run != 'all' else 'all tests')}")
    ...
    async def _analyze_timing_sensitivity(self, fingerprint: DPIFingerprint):
        ...
        if success_rates:
            ...
            fingerprint.raw_metrics['timing_analysis'] = {'test_intervals': timing_tests, 'success_rates': success_rates, 'variance': variance}
    ...
    async def _analyze_tcp_state_depth(self, fingerprint: DPIFingerprint):
        ...
        if state_tests:
            ...
            fingerprint.raw_metrics['state_tracking'] = {'tests': dict(state_tests), 'is_stateful': fingerprint.is_stateful}
    ...
    async def _identify_pattern_engine(self, fingerprint: DPIFingerprint):
        ...
        try:
            reader, writer = await asyncio.open_connection(host, port)
            blocked_host = host
            variations = [blocked_host, blocked_host.upper(), f'pre{blocked_host}post', ''.join((c + ' ' for c in blocked_host))]
        ...
        if pattern_tests:
            ...
            structure_tests = [result for t, _, result in pattern_tests if t == 'protocol_structure']
            results = {'string_matching_hits': len([x for x in string_tests if not x]), 'structure_analysis_hits': len([x for x in structure_tests if not x]), 'pattern_tests': pattern_tests}
    ...
    async def _analyze_block_patterns(self, fingerprint: DPIFingerprint):
        ...
        if block_patterns:
            ...
            if block_type == 'rst':
                ...
                fingerprint.is_stateful = False
            fingerprint.raw_metrics['block_analysis'] = {'primary_block_type': block_type, 'block_time': block_time, 'patterns_detected': block_patterns, 'behavioral_markers': {'is_stateful': fingerprint.is_stateful, 'timing_sensitivity': fingerprint.timing_sensitivity, 'rst_ttl': fingerprint.rst_ttl}}
    ...
    async def refine_fingerprint(self, fingerprint: DPIFingerprint, test_results: Dict[str, Any]) -> DPIFingerprint:
        """
        ...
        try:
            ...
            for strategy in successful_strategies:
                if 'fragment' in strategy:
                    ...
                    fingerprint.timing_sensitivity = min(1.0, (fingerprint.timing_sensitivity or 0.0) + 0.3)
            ...
            success_rate = len(successful_strategies) / (len(successful_strategies) + len(failed_strategies)) if successful_strategies or failed_strategies else 0
            fingerprint.confidence = min(1.0, fingerprint.confidence + success_rate * 0.1)
            fingerprint.raw_metrics['strategy_testing'] = {'successful_strategies': successful_strategies, 'failed_strategies': failed_strategies, 'success_rate': success_rate}
