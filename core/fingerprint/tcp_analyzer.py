...
class TCPAnalyzer:
    ...
    def __init__(self, timeout: float = 10.0, max_attempts: int = 10):
        ...
        # Analysis parameters
        self.rst_timing_threshold_ms = 100  # RST responses faster than this are suspicious
    ...
    async def analyze_tcp_behavior(self, target: str, port: int = 443) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_raw_sockets:
                ...
                self.logger.warning("Skipping fragmentation analysis - raw sockets not available")
            ...
            self.logger.info(f"TCP analysis complete for {target}:{port} (reliability: {result.reliability_score:.2f})")
    ...
    async def _analyze_basic_connections(self, result: TCPAnalysisResult, target_ip: str, port: int):
        ...
        if rst_count > successful_connections:
            ...
            self.logger.info(f"RST injection suspected: {rst_count} RSTs vs {successful_connections} successes")
    ...
    async def _analyze_rst_injection(self, result: TCPAnalysisResult, target_ip: str, port: int):
        """Analyze RST injection patterns and source"""
        self.logger.debug("Analyzing RST injection patterns")
        
        if not self.use_raw_sockets:
            self.logger.warning("RST injection analysis requires raw sockets - using limited analysis")
        ...
        if rst_timings:
            ...
            if len(fast_rsts) > len(rst_timings) * 0.7:
                ...
                self.logger.info(f"Middlebox RST injection detected (avg timing: {sum(rst_timings)/len(rst_timings):.1f}ms)")
    ...
    async def _analyze_rst_injection_limited(self, result: TCPAnalysisResult, target_ip: str, port: int):
        ...
        if rst_count > 0:
            ...
            if result.rst_timing_patterns:
                avg_rst_time = sum(result.rst_timing_patterns) / len(result.rst_timing_patterns)
    ...
    async def _analyze_window_manipulation(self, result: TCPAnalysisResult, target_ip: str, port: int):
        ...
        for window_size in test_windows:
            try:
                ...
                try:
                    ...
                    # Test window scaling
                    if self.use_raw_sockets:
                        scaling_test = await self._test_window_scaling(target_ip, port, window_size)
    ...
    async def _test_window_scaling(self, target_ip: str, port: int, window_size: int) -> bool:
    ...
    async def _analyze_sequence_numbers(self, result: TCPAnalysisResult, target_ip: str, port: int):
        ...
        for i in range(min(5, self.max_attempts)):
            try:
                ...
                if sock:
                    ...
                    seq_numbers.append(random.randint(1000000, 4000000000))  # Placeholder
                    ack_numbers.append(random.randint(1000000, 4000000000))  # Placeholder
        ...
        if len(seq_numbers) > 2:
            ...
            seq_diffs = [abs(seq_numbers[i+1] - seq_numbers[i]) for i in range(len(seq_numbers)-1)]
            if seq_diffs:
                ...
                if max_diff - min_diff < avg_diff * 0.1:
                    ...
                    result.seq_prediction_difficulty = 0.2  # Low difficulty if predictable
                else:
                    result.seq_prediction_difficulty = min(1.0, (max_diff - min_diff) / avg_diff)
    ...
    async def _test_mss_clamping(self, result: TCPAnalysisResult, target_ip: str, port: int):
            ...
            if mss_responses:
                ...
                for sent_mss, received_mss in mss_responses:
                    if received_mss < sent_mss and received_mss <= 1460:
                        ...
                        self.logger.info(f"MSS clamping detected: {sent_mss} -> {received_mss}")
                        break
    ...
    async def _test_syn_flood_protection(self, result: TCPAnalysisResult, target_ip: str, port: int):
        ...
        try:
            ...
            if response_rate < 0.5:
                ...
                self.logger.info(f"SYN flood protection detected (response rate: {response_rate:.1%})")
