...
class TCPAnalyzer:
    ...
    async def analyze_tcp_behavior(self, target: str, port: int=443) -> Dict[str, Any]:
        ...
        probes = [self._probe_rst_injection(result, target_ip, port), self._probe_tcp_options_and_timing(result, target_ip, port), self._probe_fragmentation(result, target_ip, port)]
    ...
    async def _run_probe(self, probe_func, *args):
        ...
        for i in range(self.max_attempts):
            try:
                ...
                self.logger.debug(f'Probe {probe_func.__name__} attempt {i + 1} failed: {e}')
    ...
    async def _probe_rst_injection(self, result: TCPAnalysisResult, target_ip: str, port: int):
        ...
        def probe():
            response = sr1(IP(dst=target_ip) / TCP(dport=port, flags='S'), timeout=self.timeout, verbose=0)
            if response and response.haslayer(TCP) and response.getlayer(TCP).flags & 4:
                result.rst_injection_detected = True
        await self._run_probe(lambda: asyncio.get_event_loop().run_in_executor(None, probe))

    async def _probe_tcp_options_and_timing(self, result: TCPAnalysisResult, target_ip: str, port: int):
        """Probes various TCP options and measures timing."""

        def probe():
            syn_packet = IP(dst=target_ip) / TCP(dport=port, sport=random.randint(1024, 65535), flags='S', options=[('MSS', 1460), ('SAckOK', b''), ('Timestamp', (0, 0)), ('WScale', 10)])
    ...
    async def _probe_fragmentation(self, result: TCPAnalysisResult, target_ip: str, port: int):
        ...
        def probe():
            payload = b'X' * 2000
            frag1 = IP(dst=target_ip, flags='MF', frag=0) / TCP(dport=port, flags='S') / payload[:1000]
            frag2 = IP(dst=target_ip, frag=125) / payload[1000:]
            send(frag1, verbose=0)
            time.sleep(0.1)
            response = sr1(IP(dst=target_ip) / TCP(dport=port, sport=frag1[TCP].sport, flags='A', seq=frag1.seq + 1, ack=1), timeout=self.timeout, verbose=0)
            if response and response.haslayer(TCP) and response.getlayer(TCP).flags & 18:
