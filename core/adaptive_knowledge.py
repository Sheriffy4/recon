...
class AdaptiveKnowledgeBase:
        ...
        with self._lock:
            ...
            for record_dict in domain_data["strategies"]:
                ...
                if (
                    ...
                    and record.strategy_params == strategy_params
                ):
            ...
            if hasattr(metrics, "connect_time_ms") and metrics.connect_time_ms > 0:
                if strategy_record.avg_connect_ms is None:
                    ...
                    strategy_record.avg_connect_ms = (
                        ...
                        + metrics.connect_time_ms
                    ) / strategy_record.success_count

            # Update effective_against
            if hasattr(metrics, "block_type") and metrics.block_type:
                block_type_str = (
                    metrics.block_type.value
                    if hasattr(metrics.block_type, "value")
                    else str(metrics.block_type)
                )
            ...
            if domain_data["preferred_strategy"] is None:
                ...
                if (
                    current_preferred
                    and strategy_record.success_rate() > current_preferred.success_rate()
                ):
                    domain_data["preferred_strategy"] = strategy_name
            ...
            LOG.info(
                f"Recorded success for {domain}: {strategy_name} "
    ...
    def record_failure(
        ...
        with self._lock:
            ...
            if domain_data["preferred_strategy"] == strategy_name:
                if strategy_record.success_rate() < 0.5:
                    ...
                    if best_record:
                        ...
                        LOG.info(
                            f"Updated preferred strategy for {domain}: {best_record.strategy_name} "
            ...
            LOG.debug(
                f"Recorded failure for {domain}: {strategy_name} "
                f"(success_rate: {strategy_record.success_rate():.2%})"
            )

    def get_strategies_for_domain(
        self, domain: str, block_type: Optional[BlockType] = None
    ) -> List[StrategyRecord]:
        """
        ...
        with self._lock:
            ...
            if records:
                best_record = max(
                    records,
                    key=lambda r: (r.success_rate(), -r.avg_connect_ms if r.avg_connect_ms else 0),
                )
    ...
    def get_prioritized_strategies(
        ...
        def sort_key(record: StrategyRecord) -> tuple:
            ...
            is_preferred = (
                1 if preferred_strategy and record.strategy_name == preferred_strategy else 0
            )
    ...
    def get_fallback_strategies(self, domain: str, failed_strategy: str) -> List[StrategyRecord]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            total_successes = sum(
                sum(StrategyRecord.from_dict(s).success_count for s in d["strategies"])
                for d in self._data.values()
            )
            total_failures = sum(
                sum(StrategyRecord.from_dict(s).failure_count for s in d["strategies"])
                for d in self._data.values()
            )
            verified_strategies = sum(
                sum(1 for s in d["strategies"] if StrategyRecord.from_dict(s).verified)
                for d in self._data.values()
            )
