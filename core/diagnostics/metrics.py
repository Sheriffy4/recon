...
class MetricsCollector:
    ...
    def get_histogram_stats(self, name: str, labels: Optional[Dict[str, str]]=None) -> Dict[str, float]:
        ...
        with self._lock:
            ...
            if not points:
                return {'count': 0, 'min': 0.0, 'max': 0.0, 'mean': 0.0, 'median': 0.0, 'p95': 0.0, 'p99': 0.0}
            values = [p.value for p in points]
            return {'count': len(values), 'min': min(values), 'max': max(values), 'mean': statistics.mean(values), 'median': statistics.median(values), 'p95': self._percentile(values, 0.95), 'p99': self._percentile(values, 0.99)}

    def get_stats(self) -> Dict[str, Any]:
        """Получение всей статистики."""
        with self._lock:
            stats = {'name': self.name, 'uptime': time.time() - self._start_time, 'counters': dict(self._counters), 'gauges': dict(self._gauges), 'histograms': {}}
