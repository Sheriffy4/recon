...
class DoHMetrics:
    ...
    def record_query(self, success: bool, resolution_time_ms: float, cache_hit: bool = False) -> None:
        """Записывает запрос."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'min_resolution_time_ms': self.min_resolution_time_ms if self.min_resolution_time_ms != float('inf') else 0.0,
...
class PCAPMetrics:
    ...
    def record_capture(self, success: bool, packets: int = 0, bytes_captured: int = 0, duration_ms: float = 0.0) -> None:
        """Записывает захват."""
...
class DiagnosticsMetricsCollector:
    ...
    def record_strategy_success(self, domain: str, strategy_type: str, latency_ms: float) -> None:
        ...
        if key not in self.strategy_metrics:
            self.strategy_metrics[key] = StrategyMetrics(
    ...
    def record_doh_query(self, provider: str, success: bool, resolution_time_ms: float, cache_hit: bool = False) -> None:
        """
        ...
        self.logger.debug(f"{status}{cache_status} DoH query: {provider} - {resolution_time_ms:.1f}ms")
    
    def record_pcap_capture(self, success: bool, packets: int = 0, bytes_captured: int = 0, duration_ms: float = 0.0) -> None:
        """
        ...
        status = "✅" if success else "❌"
        self.logger.debug(f"{status} PCAP capture: {packets} packets, {bytes_captured} bytes, {duration_ms:.1f}ms")
    
    def get_strategy_metrics(self, domain: Optional[str] = None, strategy_type: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def _cleanup_history(self) -> None:
        ...
        self.strategy_success_history = [
            h for h in self.strategy_success_history
            if datetime.fromisoformat(h['timestamp']) > cutoff_time
        ]
        
        # Clean DoH history
        self.doh_query_history = [
            h for h in self.doh_query_history
            if datetime.fromisoformat(h['timestamp']) > cutoff_time
        ]
        
        # Clean PCAP history
        self.pcap_capture_history = [
            h for h in self.pcap_capture_history
            if datetime.fromisoformat(h['timestamp']) > cutoff_time
        ]
