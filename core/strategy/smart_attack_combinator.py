...
class SmartAttackCombinator:
    ...
    def _build_compatibility_matrix(self) -> Dict[Tuple[str, str], AttackCompatibility]:
        ...
        compatibility_rules = [
            ...
            ("fake", "multisplit", True, 0.85, 0.15, "Fake packet усиливает множественную фрагментацию"),
            ("disorder", "multisplit", True, 0.8, 0.1, "Disorder + multisplit создают сложную структуру"),
            ...
            ("multisplit", "multidisorder", True, 0.6, 0.0, "Сложная комбинация, требует осторожности"),
            ("fake", "disorder", True, 0.5, 0.0, "Возможны конфликты в механизмах"),
            ("split", "multisplit", True, 0.4, 0.0, "Дублирование функциональности"),
    ...
    def generate_attack_combinations(self,
                                   available_attacks: List[str],
                                   max_combination_size: int = 3,
                                   min_compatibility_score: float = 0.5) -> List[CombinationStrategy]:
        """
    ...
    def _generate_combinations_of_size(self,
                                     attacks: List[str],
                                     size: int,
                                     min_compatibility_score: float) -> List[CombinationStrategy]:
    ...
    def _calculate_heuristic_compatibility(self, attack1: str, attack2: str) -> AttackCompatibility:
        ...
        score = category_compatibility.get((cat1, cat2), 
                category_compatibility.get((cat2, cat1), 0.5))
    ...
    def _determine_execution_order(self, attacks: List[str]) -> List[str]:
        ...
        sorted_attacks = sorted(
            attacks, 
            key=lambda attack: execution_priorities.get(attack, 5)
        )
        ...
        for i in range(len(sorted_attacks) - 1):
            ...
            if compatibility.recommended_order:
                ...
                if compatibility.recommended_order != [attack1, attack2]:
                    # Меняем местами
                    sorted_attacks[i], sorted_attacks[i + 1] = sorted_attacks[i + 1], sorted_attacks[i]
    ...
    def _generate_combination_parameters(self, attacks: List[str]) -> Dict[str, Any]:
        ...
        if any("fake" in attack for attack in attacks):
            base_params.update({
                "ttl": 1,  # Короткий TTL для fake packets
                "fooling": "badseq"
            })
        ...
        if any("split" in attack for attack in attacks):
            base_params.update({
                "split_pos": 3,
                "split_count": 4 if "multisplit" in str(attacks) else 2
            })
    ...
    def _calculate_combination_effectiveness(self,
                                          attacks: List[str],
                                          compatibility_result: Dict[str, Any]) -> float:
        ...
        categories = set(self.attack_categories.get(attack, AttackCategory.FRAGMENTATION) 
    ...
    def optimize_combination_parameters(self,
                                      combination: CombinationStrategy,
                                      fingerprint: Optional[Any] = None) -> Dict[str, Any]:
    ...
    def rank_combinations_by_effectiveness(self,
                                         combinations: List[CombinationStrategy],
                                         fingerprint: Optional[Any] = None) -> List[CombinationStrategy]:
    ...
    def _calculate_dpi_match_bonus(self,
                                 combination: CombinationStrategy,
        ...
        try:
            ...
            if hasattr(fingerprint, 'known_weaknesses'):
                for weakness in fingerprint.known_weaknesses:
                    if "fragmentation" in weakness and any("split" in attack for attack in combination.attacks):
                        bonus += 0.05
    ...
    def get_combination_explanation(self, combination: CombinationStrategy) -> str:
        ...
        explanation_parts.append(f"   Ожидаемая эффективность: {combination.expected_effectiveness:.2f}")
