...
try:
    from ..fingerprint.unified_fingerprinter import (
...
class IntelligentStrategyGenerator:
    ...
    def load_recon_summary(self, summary_file: str) -> bool:
        ...
        try:
            ...
            LOG.info(
                f"Loaded recon summary with {len(self.historical_effectiveness)} strategy effectiveness records"
            )
            return True
    ...
    async def analyze_pcap(self, pcap_file: str) -> bool:
        ...
        try:
            ...
            LOG.info(
                f"PCAP analysis complete: {len(self.pcap_analysis_data.rst_triggers)} RST triggers found"
            )
            return True
    ...
    async def generate_intelligent_strategies(
        self, target_domain: str, count: int = 10, include_experimental: bool = False
    ) -> List[IntelligentStrategyRecommendation]:
        """
        ...
        if rule_recommendations:
            try:
                rule_result = self.rule_engine.evaluate_fingerprint(fingerprint_data)
                combinator_strategies = (
                    self.combinator.suggest_combinations_from_rule_recommendations(
                        rule_recommendations,
        ...
        if self.historical_effectiveness and len(recommendations) < count:
            historical_strategies = self._generate_historical_strategies(
        ...
        if recommendations:
            self.generation_stats["avg_confidence"] = statistics.mean(
                [r.confidence_score for r in recommendations]
            )
        self.generation_stats["recommendations_created"] += 1

        LOG.info(
            f"Generated {len(recommendations)} intelligent strategies using {len(data_sources_used)} data sources"
        )
    ...
    def _analyze_historical_effectiveness(self, target_domain: str) -> Dict[str, Any]:
        ...
        per_target = (
            self.recon_summary_data.get("best_strategy", {})
            .get("engine_telemetry_full", {})
            .get("per_target", {})
        )
        successful_targets = []
        failed_targets = []
        ...
        insights["target_success_rate"] = (
            len(successful_targets) / (len(successful_targets) + len(failed_targets))
            if (successful_targets or failed_targets)
            else 0.0
        )
    ...
    def _create_intelligent_recommendation(
        self,
        strategy_name: str,
        strategy_config: Dict[str, Any],
        ...
        if fingerprint_data:
            ...
            if dpi_confidence > 0.7:
                reasoning.append(
                    f"High-confidence DPI fingerprint (confidence: {dpi_confidence:.2f})"
                )
        ...
        if historical_insights:
            ...
            if best_historical and strategy_name in best_historical:
                ...
                reasoning.append(
                    f"Similar strategy historically successful (success rate: {historical_success:.2f})"
                )
        ...
        if pcap_insights:
            ...
            if (
                pcap_insights.get("fragmentation_vulnerable", False)
                and "multisplit" in strategy_name
            ):
    ...
    def _generate_pcap_specific_strategies(
        self, count: int
    ) -> List[IntelligentStrategyRecommendation]:
    ...
    def _generate_historical_strategies(
        self, count: int
    ) -> List[IntelligentStrategyRecommendation]:
        ...
        sorted_strategies = sorted(
            self.historical_effectiveness.items(), key=lambda x: x[1], reverse=True
        )
