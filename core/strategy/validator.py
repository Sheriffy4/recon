...
class CompatibilityResult:
    ...
    def add_difference(self, field: str, testing_value: Any, service_value: Any, severity: str = "warning") -> None:
...
class StrategyValidator:
    ...
    def validate_strategy(self, strategy: Any) -> ValidationResult:
        ...
        try:
            ...
            self.logger.debug(f"Strategy validation completed: {strategy_type}, valid={result.is_valid}")
    ...
    def _validate_parameters(self, strategy_type: str, params: Dict[str, Any], result: ValidationResult) -> bool:
    ...
    def _validate_parameter_ranges(self, strategy_type: str, params: Dict[str, Any], result: ValidationResult) -> bool:
    ...
    def _validate_parameter_combinations(self, strategy_type: str, params: Dict[str, Any], result: ValidationResult) -> bool:
            ...
            if 'overlap_size' in params and 'split_pos' in params:
                ...
                if isinstance(overlap_size, int) and isinstance(split_pos, int):
                    if overlap_size >= split_pos:
                        result.add_error(f"overlap_size ({overlap_size}) must be < split_pos ({split_pos})")
                        return False
    ...
    def validate_compatibility(self, 
                             ...
                             service_strategy: Any) -> CompatibilityResult:
        """
        ...
        try:
            ...
            if testing_type != service_type:
                ...
                result.recommendations.append(f"Strategy types must match: {testing_type} != {service_type}")
            ...
            for key in all_keys:
                ...
                if testing_value != service_value:
                    # Determine severity
                    severity = self._determine_difference_severity(key, testing_value, service_value)
            ...
            if result.is_compatible:
                ...
                result.recommendations.append("Strategies are NOT compatible - critical differences found")
    ...
    def test_strategy_application(self, 
                                 ...
                                 domain: str,
                                 timeout: float = 10.0,
                                 capture_pcap: bool = True) -> TestResult:
        """
        ...
        try:
            ...
            if self.bypass_engine:
                ...
                return TestResult(
                    ...
                    domain=domain,
                    strategy_type=strategy_type,
                    error_message="No bypass engine available for testing" if validation_result.is_valid else None
                )
    ...
    def _test_with_bypass_engine(self, 
                                strategy_dict: Dict[str, Any], 
                                domain: str,
                                timeout: float,
                                capture_pcap: bool) -> TestResult:
        """
        ...
        try:
            ...
            if capture_pcap and self.pcap_analyzer:
                pcap_file = Path(f"test_pcap_{domain}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pcap")
    ...
    def revalidate_strategies(self, 
                            strategies: Dict[str, Any],
                            remove_invalid: bool = False) -> Dict[str, ValidationResult]:
        """
        ...
        for domain, strategy in strategies.items():
            try:
                ...
                results[domain] = ValidationResult(
                    is_valid=False,
                    errors=[f"Revalidation exception: {str(e)}"]
                )
                invalid_domains.append(domain)
    ...
    def mark_strategy_failed(self, domain: str) -> None:
        ...
        self._failed_strategies[domain] = self._failed_strategies.get(domain, 0) + 1
        self.logger.debug(
            f"Marked strategy for {domain} as failed "
            f"(count: {self._failed_strategies[domain]})"
        )
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'broken_strategies': sum(1 for count in self._failed_strategies.values() if count >= self.max_failures),
            ...
            'failed_strategies': [
                {
                    'domain': domain,
                    'failure_count': count,
                    'is_broken': count >= self.max_failures
                }
    ...
    def start_periodic_revalidation(self, 
                                   strategies: Dict[str, Any],
                    ...
                    for domain, result in results.items():
                        if not result.is_valid:
                            ...
                            if self.is_strategy_broken(domain):
                                broken_message = f"Strategy for {domain} is broken (too many failures)"
        ...
        self._revalidation_thread = threading.Thread(
            target=revalidation_loop,
            daemon=True,
            name="StrategyRevalidation"
        )
    ...
    def schedule_revalidation(self, 
                            strategies: Dict[str, Any],
                            schedule_time: str,
                            remove_invalid: bool = True,
                            notification_callback: Optional[callable] = None) -> None:
        ...
        def scheduled_revalidation_loop():
            ...
            while self._revalidation_active:
                try:
                    ...
                    self.logger.info(f"Next revalidation scheduled for {next_run} (in {wait_seconds:.0f}s)")
        ...
        self._revalidation_active = True
        self._revalidation_thread = threading.Thread(
            target=scheduled_revalidation_loop,
            daemon=True,
            name="ScheduledStrategyRevalidation"
        )
        self._revalidation_thread.start()
...
def create_strategy_validator(strategy_loader=None, 
                             pcap_analyzer=None,
                             bypass_engine=None,
                             debug: bool = False) -> StrategyValidator:
    """
