...
class StrategyIntentEngine:
    ...
    def __init__(self):
        ...
        self.rule_stats = {"evaluations": 0, "intents_generated": 0, "fallback_used": 0}
    ...
    def _build_intent_registry(self) -> Dict[str, StrategyIntent]:
        ...
        intents = {
            ...
            "fake_sni": StrategyIntent(
                ...
                parameter_ranges={"fake_domain": ["example.com", "google.com"], "ttl": [1, 2, 3]},
            ),
            ...
            "ip_fragmentation": StrategyIntent(
                ...
                parameter_ranges={"fragment_size": [8, 16, 32, 64]},
            ),
            ...
            "short_ttl_decoy": StrategyIntent(
                ...
                parameter_ranges={"ttl": [1, 2, 3], "fooling_method": ["badsum", "badseq"]},
            ),
            ...
            "sequence_overlap": StrategyIntent(
                ...
                parameter_ranges={"overlap_size": [1, 2, 4, 8]},
            ),
            ...
            "timing_manipulation": StrategyIntent(
                ...
                parameter_ranges={"delay_ms": [10, 50, 100, 200], "jitter_enabled": [True, False]},
            ),
            ...
            "payload_obfuscation": StrategyIntent(
                ...
                parameter_ranges={"obfuscation_method": ["xor", "base64", "compression"]},
            ),
            ...
            "ipv6_fallback": StrategyIntent(
                ...
                parameter_ranges={"prefer_ipv6": [True, False], "fallback_timeout": [3, 5, 10]},
            ),
        }
    ...
    def propose_intents(
        self, fingerprint, failure_report: Optional[Dict[str, Any]] = None
    ) -> List[StrategyIntent]:
        """
    ...
    def _has_sni_dependency(self, fingerprint) -> bool:
        ...
        return (
            fingerprint.behavioral_signatures.get("sni_filtering", False)
            or fingerprint.detection_layer.value == "l7_tls"
            or any("sni" in weakness for weakness in fingerprint.known_weaknesses)
        )
    ...
    def _intents_from_failure(self, failure_report: Dict[str, Any]) -> List[StrategyIntent]:
        ...
        if root_cause == "dpi_active_rst_injection":
            intents.extend(
                [self.intent_registry["short_ttl_decoy"], self.intent_registry["sequence_overlap"]]
            )
            ...
            intents.extend([self.intent_registry["conceal_sni"], self.intent_registry["fake_sni"]])
    ...
    def get_intents_by_category(self, category: IntentCategory) -> List[StrategyIntent]:
        ...
        category_mapping = {
            ...
            IntentCategory.PROTOCOL_EVASION: [
    ...
    def explain_intent_selection(
        self, fingerprint, selected_intents: List[StrategyIntent]
    ) -> Dict[str, str]:
        ...
        for intent in selected_intents:
                ...
                if met_conditions:
                    ...
                    if filtered_conditions:
                        explanation_parts.append(
                            f"- Выполнены условия: {', '.join(filtered_conditions)}"
                        )
            ...
            if intent.side_effects:
                ...
                if filtered_effects:
                    explanation_parts.append(
                        f"- Возможные побочные эффекты: {', '.join(filtered_effects)}"
                    )
    ...
    def _check_precondition(self, fingerprint, condition: str) -> bool:
        ...
        condition_checks = {
            "sni_filtering_detected": lambda fp: fp.behavioral_signatures.get(
                "sni_filtering", False
            ),
            "vulnerable_to_fragmentation": lambda fp: "vulnerable_to_fragmentation"
            in fp.known_weaknesses,
            ...
            "timing_sensitive_dpi": lambda fp: fp.behavioral_signatures.get(
            ...
            "deep_content_inspection": lambda fp: fp.behavioral_signatures.get(
                "deep_content_inspection", False
            ),
            "ip_fragmentation_allowed": lambda fp: not fp.behavioral_signatures.get(
                "blocks_ip_fragmentation", False
            ),
        }
    ...
    def update_intent_from_failure(self, intent_key: str, failure_report: Dict[str, Any]):
    ...
    def generate_explanations(
        ...
        for intent in selected_intents:
            ...
            if intent.side_effects:
                ...
                if filtered_effects:
                    explanation_parts.append(
                        f"   ⚠️ Побочные эффекты: {', '.join(filtered_effects)}"
                    )
    ...
    def _analyze_dpi_match(self, fingerprint, intent: StrategyIntent) -> List[str]:
        ...
        try:
            ...
            if hasattr(fingerprint, "dpi_type"):
                ...
                if (
                    intent.key in ["packet_reordering", "out_of_order_decoy"]
                    and dpi_type == "stateless"
                ):
                    reasons.append("stateless DPI уязвим к изменению порядка")
                elif (
                    intent.key in ["sequence_overlap", "record_fragmentation"]
                    and dpi_type == "stateful"
                ):
            ...
            if hasattr(fingerprint, "behavioral_signatures"):
                ...
                if intent.key == "record_fragmentation" and signatures.get(
                    "reassembles_fragments", False
                ):
                    reasons.append("DPI не собирает фрагменты")

                if intent.key == "payload_obfuscation" and signatures.get(
    ...
    def create_fallback_intents_for_unknown_dpi(self, fingerprint) -> List[StrategyIntent]:
    ...
    def _get_conservative_parameters(self, intent_key: str) -> Dict[str, Any]:
        ...
        conservative_params = {
            "basic_fragmentation": {"split_count": [2, 4], "split_pos": ["random", "fixed"]},
            "simple_reordering": {"split_pos": [2, 3], "disorder_method": ["simple"]},
            "basic_sni_concealment": {"split_pos": ["sni"], "fooling_method": ["badsum", "badseq"]},
        }

        return conservative_params.get(intent_key, {})

    def from_keys(self, keys: List[str], base_weight: float = 0.9) -> List[StrategyIntent]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_intent_priority": sum(
                intent.priority for intent in self.intent_registry.values()
            )
            / len(self.intent_registry),
        }
...
if __name__ == "__main__":
    ...
    test_fingerprint = DPIFingerprint(
        ...
        behavioral_signatures={"sni_filtering": True, "deep_content_inspection": False},
