...
class EnhancedRSTAnalyzer:
        ...
        try:
            ...
            if self.rst_analyzer:
                try:
                    ...
                    LOG.info(
                        f"RST analysis found {len(self.pcap_analysis_results.get('rst_triggers', []))} triggers"
                    )
                except Exception as e:
    ...
    def _convert_to_second_pass_strategy(
        self, intelligent_strategy: IntelligentStrategyRecommendation
    ) -> Optional[SecondPassStrategy]:
    ...
    def _deduplicate_strategies(
        self, strategies: List[SecondPassStrategy]
    ) -> List[SecondPassStrategy]:
    ...
    async def _test_second_pass_strategies(self, target_sites: List[str]):
        ...
        for i, strategy in enumerate(self.second_pass_strategies):
            try:
                LOG.info(
                    f"Testing strategy {i+1}/{len(self.second_pass_strategies)}: {strategy.strategy_name}"
                )
    ...
    async def _test_single_strategy(
        self, strategy: SecondPassStrategy, target_sites: List[str]
    ) -> SecondPassResult:
        ...
        telemetry_data = {"sites_tested": len(test_sites), "individual_results": {}}
    ...
    def _compile_analysis_results(self) -> Dict[str, Any]:
        ...
        if self.strategy_generator and self.strategy_generator.recon_summary_data:
            first_pass_success_rate = self.strategy_generator.recon_summary_data.get(
                "success_rate", 0.0
            )
...
async def enhance_rst_analysis(
    recon_summary_file: str = "recon_summary.json",
    pcap_file: str = "out2.pcap",
    ...
    results = await analyzer.run_enhanced_analysis(
        target_sites, max_strategies, test_strategies=True
    )
