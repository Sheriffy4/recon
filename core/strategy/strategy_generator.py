...
class StrategyGenerator:
    ...
    async def generate_strategies(self,
                                ...
                                enable_combinations: bool = True) -> List[GeneratedStrategy]:
        """
    ...
    async def _generate_single_strategies(self,
                                        intents: List[Any],
                                        fingerprint: Optional[Any]) -> List[GeneratedStrategy]:
    ...
    async def _generate_combination_strategies(self,
                                            intents: List[Any],
                                            fingerprint: Optional[Any]) -> List[GeneratedStrategy]:
        ...
        for intent in intents:
            ...
            intent_attacks = [m.attack_name for m in attack_mappings 
        ...
        for combo_strategy in combination_strategies:
            ...
            source_intents = []
            for intent in intents:
                if any(attack in intent_to_attacks.get(intent.key, []) for attack in combo_strategy.attacks):
    ...
    async def _optimize_strategy_parameters(self,
        ...
        for strategy in top_strategies:
            try:
                ...
                optimization_method = OptimizationMethod.DPI_ADAPTIVE if fingerprint else OptimizationMethod.PRESET_GOOD_VALUES
                ...
                if optimization_result.improvement_score > 0.05:  # Минимальное улучшение 5%
                    optimized_strategy = GeneratedStrategy(
                        ...
                        expected_success_rate=strategy.expected_success_rate * (1 + optimization_result.improvement_score),
                    ...
                    LOG.debug(f"Оптимизирована стратегия {strategy.name}, улучшение: {optimization_result.improvement_score:.2f}")
                else:
        ...
        self.generation_stats["by_method"][GenerationMethod.OPTIMIZATION.value] += len(optimized_strategies)
    ...
    def _adapt_parameters_for_dpi(self,
        ...
        try:
            ...
            if hasattr(fingerprint, 'dpi_type'):
                ...
                if dpi_type == "stateless":
                    ...
                    if "split_count" in adapted_params:
                        adapted_params["split_count"] = min(16, adapted_params.get("split_count", 4) * 2)
            ...
            if hasattr(fingerprint, 'dpi_mode'):
                ...
                if dpi_mode == "active_rst":
                    ...
                    if "split_count" in adapted_params:
                        adapted_params["split_count"] = max(2, adapted_params.get("split_count", 4) // 2)
                    ...
                    if param in adapted_params and isinstance(value_range, list) and value_range:
                        ...
                        if isinstance(value_range[0], (int, float)):
                            ...
                            if fingerprint.confidence > 0.7:
                                ...
                                adapted_params[param] = value_range[len(value_range)//2]  # Средние параметры
                        else:
    ...
    def _generate_attack_combinations(self,
                                    attacks: List[str],
                                    max_combo_size: int = 2) -> List[List[str]]:
    ...
    def _generate_combination_parameters(self,
                                       attacks: List[str],
    ...
    def _calculate_combination_success(self,
                                     attacks: List[str],
                                     source_intents: List[str],
        ...
        expected_success = (base_success + combination_bonus - complexity_penalty) * fingerprint_modifier
    ...
    def _optimize_parameters_for_dpi(self,
                                   base_params: Dict[str, Any],
                                   attacks: List[str],
                                   fingerprint: Optional[Any]) -> Dict[str, Any]:
            ...
            if hasattr(fingerprint, 'dpi_type'):
                ...
                if dpi_type == "stateless":
                    ...
                    if "split_count" in optimized_params:
                        optimized_params["split_count"] = min(16, optimized_params["split_count"] * 2)
            ...
            if hasattr(fingerprint, 'known_weaknesses'):
                for weakness in fingerprint.known_weaknesses:
                    if "fragmentation" in weakness:
                        optimized_params["split_count"] = max(8, optimized_params.get("split_count", 4))
    ...
    def _calculate_dpi_compatibility_bonus(self,
                                         strategy: GeneratedStrategy,
                                         fingerprint: Any) -> float:
        ...
        try:
            ...
            if hasattr(fingerprint, 'dpi_type'):
                ...
                if dpi_type == "stateless" and any("disorder" in attack for attack in strategy.attack_combination):
                    bonus += 0.1
                elif dpi_type == "stateful" and any("fake" in attack for attack in strategy.attack_combination):
                    bonus += 0.1
    ...
    def generate_strategy_explanations(self, strategies: List[GeneratedStrategy]) -> Dict[str, str]:
        ...
        for strategy in strategies:
            ...
            explanation_parts.append(f"   Ожидаемая эффективность: {strategy.expected_success_rate:.2f}")
    ...
    def get_generation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                max(1, self.generation_stats["successful_tests"] + self.generation_stats["failed_tests"])
            ),
            "components_loaded": {
    ...
    def update_strategy_test_result(self, strategy_name: str, success: bool, details: Dict[str, Any]):
        ...
        LOG.info(f"Обновлен результат тестирования {strategy_name}: {'успех' if success else 'неудача'}")
