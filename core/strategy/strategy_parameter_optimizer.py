...
class StrategyParameterOptimizer:
    ...
    def _define_good_values(self) -> Dict[str, Dict[str, Any]]:
        ...
        good_values = {
            ...
            "fake_attack": {"ttl": 1, "split_pos": "sni", "fooling": "badseq", "window_div": 6},
            "multisplit_attack": {
                "split_count": 8,
                "split_pos": "sni",
                "fooling": "badsum",
            ...
            "social_media": {"split_pos": "sni", "split_count": 8, "fooling": "badsum", "ttl": 2},
            "video_streaming": {"split_count": 4, "split_pos": 3, "fooling": "badseq", "ttl": 3},
            "messaging": {"ttl": 1, "split_pos": 2, "fooling": "badseq", "repeats": 2},
    ...
    def _build_dpi_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        rules = {
            ...
            "stateful": {
                ...
                "avoid_parameters": {"disorder_method": "stateful DPI может восстановить порядок"},
            },
            ...
            "active_rst": {
                ...
                "boost_parameters": {"delay_ms": 10},  # Небольшая задержка может помочь
            },
    ...
    def optimize_parameters(
        ...
        method: OptimizationMethod = OptimizationMethod.DPI_ADAPTIVE,
    ) -> OptimizationResult:
        """
        ...
        confidence = self._calculate_optimization_confidence(method, fingerprint, improvement_score)
        ...
        LOG.info(
            f"Оптимизация завершена за {optimization_time:.3f}с, улучшение: {improvement_score:.2f}"
        )

        return result

    def _optimize_for_dpi(
        self, base_parameters: Dict[str, Any], attack_names: List[str], fingerprint: Any
    ) -> Dict[str, Any]:
    ...
    def _apply_good_values(
        self, base_parameters: Dict[str, Any], attack_names: List[str], fingerprint: Optional[Any]
    ) -> Dict[str, Any]:
        ...
        if good_values_key in self.good_values:
            ...
            for param, value in good_params.items():
                if param not in optimized or optimized[param] is None:
                    ...
                    optimized[param] = self._blend_parameter_values(optimized[param], value, param)
    ...
    def _heuristic_optimization(
        self, base_parameters: Dict[str, Any], attack_names: List[str]
    ) -> Dict[str, Any]:
        ...
        for attack_name in attack_names:
            if "fake" in attack_name:
                optimized.update({"ttl": 1, "fooling": "badseq", "split_pos": "sni"})

            elif "multisplit" in attack_name:
                optimized.update({"split_count": 8, "split_pos": "sni", "fooling": "badsum"})

            elif "disorder" in attack_name:
                optimized.update(
                    {"split_pos": 3, "disorder_method": "reverse", "fooling": "badsum"}
                )

            elif "seqovl" in attack_name:
                optimized.update(
                    {"split_seqovl": 336, "split_pos": 76, "fooling": "badseq", "ttl": 1}
                )
            ...
            if param_name in optimized or random.random() < 0.3:  # 30% шанс добавить новый параметр
                if param_range.values:
                    ...
                    if isinstance(param_range.min_value, int):
                        optimized[param_name] = random.randint(
                            param_range.min_value, param_range.max_value
                        )
                    else:
                        optimized[param_name] = random.uniform(
                            param_range.min_value, param_range.max_value
                        )

        return optimized

    def _select_good_values_key(self, attack_names: List[str], fingerprint: Optional[Any]) -> str:
        ...
        if fingerprint:
            ...
            if hasattr(fingerprint, "domain"):
                ...
                if any(
                    social in domain for social in ["twitter", "instagram", "facebook", "tiktok"]
                ):
                    return "social_media"
    ...
    def _blend_parameter_values(self, existing_value: Any, good_value: Any, param_name: str) -> Any:
        ...
        if isinstance(existing_value, (int, float)) and isinstance(good_value, (int, float)):
            if param_name in ["ttl"]:
                ...
                return (
                    (existing_value + good_value) // 2
                    if isinstance(existing_value, int)
                    else (existing_value + good_value) / 2
                )
    ...
    def _calculate_optimization_confidence(
        self, method: OptimizationMethod, fingerprint: Optional[Any], improvement_score: float
    ) -> float:
    ...
    def _update_optimization_stats(self, method: OptimizationMethod, improvement_score: float):
        ...
        self.optimization_stats["average_improvement"] = (
            current_avg * (total_opts - 1) + improvement_score
        ) / total_opts
    ...
    def generate_parameter_explanation(self, optimization_result: OptimizationResult) -> str:
        ...
        explanation_parts.append(
            f"🔧 Оптимизация параметров ({optimization_result.optimization_method}):"
        )
...
if __name__ == "__main__":
    ...
    # Тестовые параметры
    base_params = {"split_pos": 3, "ttl": 3, "fooling": "badsum"}
    ...
    result = optimizer.optimize_parameters(
        base_params, attacks, method=OptimizationMethod.PRESET_GOOD_VALUES
    )
