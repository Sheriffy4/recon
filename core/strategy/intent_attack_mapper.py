...
class IntentAttackMapper:
    ...
    def generate_strategies_from_intents(
        self, intents: List[Any], fingerprint: Optional[Any] = None
        ...
        for intent in intents:
            ...
            for mapping in attack_mappings:
                ...
                adapted_params = self._adapt_parameters(mapping.parameters, fingerprint, intent)

                # Вычисляем ожидаемую эффективность
                expected_success = self._calculate_expected_success(intent, mapping, fingerprint)
    ...
    def _calculate_expected_success(
        self, intent: Any, mapping: AttackMapping, fingerprint: Optional[Any]
    ) -> float:
    ...
    def get_mapping_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "registry_attacks_count": (
                len(self.attack_registry.list_attacks()) if self.attack_registry else 0
            ),
        }
