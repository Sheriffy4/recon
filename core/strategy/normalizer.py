...
class ParameterNormalizer:
    ...
    # Keep superset for compatibility with generators and engine validators.
    ...
    def normalize(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 0) Canonicalize legacy aliases to canonical keys (do NOT delete legacy keys)
        if "split_position" in normalized and "split_pos" not in normalized:
            normalized["split_pos"] = normalized["split_position"]
            transformations.append("split_position -> split_pos (alias)")
        if "split_cnt" in normalized and "split_count" not in normalized:
            normalized["split_count"] = normalized["split_cnt"]
            transformations.append("split_cnt -> split_count (alias)")
        if "split_seqovl" in normalized and "overlap_size" not in normalized:
            normalized["overlap_size"] = normalized["split_seqovl"]
            transformations.append("split_seqovl -> overlap_size (alias)")
        if "overlap_size" in normalized and "split_seqovl" not in normalized:
            normalized["split_seqovl"] = normalized["overlap_size"]
            transformations.append("overlap_size -> split_seqovl (mirror)")

        # 1) Canonical parameter is fooling; accept fooling_methods as alias
        if "fooling" not in normalized and "fooling_methods" in normalized:
            normalized["fooling"] = normalized["fooling_methods"]
            transformations.append("fooling_methods -> fooling (alias)")

        # 2) Normalize fooling and create fooling_methods mirror
        ...
        if "fooling" in normalized:
            # keep canonical fooling as-is (str or list), but build fooling_methods list mirror
            fm = _as_list(normalized.get("fooling"))
            normalized["fooling_methods"] = fm
            transformations.append(f"fooling -> fooling_methods={fm} (mirror)")

        # 3) Resolve fake_ttl alias to ttl (keep both)
        if "fake_ttl" in normalized and "ttl" not in normalized:
            normalized["ttl"] = normalized["fake_ttl"]
            transformations.append(f"fake_ttl={normalized['fake_ttl']} -> ttl={normalized['ttl']} (mirror)")
        if "ttl" in normalized and "fake_ttl" not in normalized:
            normalized["fake_ttl"] = normalized["ttl"]
            transformations.append(f"ttl={normalized['ttl']} -> fake_ttl={normalized['fake_ttl']} (mirror)")

        # 4) Apply defaults ONLY if absent (canonical first)
        if "fooling" not in normalized and "fooling_methods" not in normalized:
            normalized["fooling"] = ["badsum"]
            normalized["fooling_methods"] = ["badsum"]
            transformations.append("fooling not specified -> ['badsum'] (default)")

        if "fake_mode" not in normalized:
            # No fake_mode specified - use default
            normalized["fake_mode"] = "single"
            transformations.append("fake_mode not specified → fake_mode='single' (default)")
    ...
    def validate(self, params: Dict[str, Any]) -> None:
        ...
        if "split_pos" in params:
            ...
            # split_pos can be special token or positive integer
            if isinstance(split_pos, int) and split_pos <= 0:
                ...
                errors.append(
                    ValidationError(
                        ...
                        parameter_name="split_pos",
                        expected='int or one of {"sni","cipher","midsld","random"}',
    ...
    def detect_conflicts(self, params: Dict[str, Any], attacks: List[str]) -> List[str]:
        """
        ...
        if unimplemented:
            warnings.append(f"⚠️ Unimplemented attacks will be skipped: {unimplemented}")
