...
from recon.core.quic_handler import QuicHandler
...
class BypassTechniques:
    ...
    def apply_badsum_fooling(packet_data: bytearray) -> bytearray:
        ...
        if len(packet_data) > tcp_checksum_pos + 1:
            packet_data[tcp_checksum_pos:tcp_checksum_pos + 2] = struct.pack('!H', 57005)
        return packet_data
    ...
    def apply_md5sig_fooling(packet_data: bytearray) -> bytearray:
        ...
        tcp_checksum_pos = ip_header_len + 16
        if len(packet_data) > tcp_checksum_pos + 1:
            packet_data[tcp_checksum_pos:tcp_checksum_pos + 2] = struct.pack('!H', 48879)
        return packet_data
...
class BypassEngine:
    ...
    def __init__(self, debug=True):
        ...
        self.stats = {'packets_captured': 0, 'tls_packets_bypassed': 0, 'quic_packets_bypassed': 0, 'fragments_sent': 0, 'fake_packets_sent': 0}
    ...
    def _config_to_strategy_task(self, config: dict) -> dict:
            ...
            if split_count > 0:
                if split_count <= 3:
                    ...
                    for i in range(split_count):
                        ...
                        last_pos += gap
            return {'type': 'multisplit', 'params': {'ttl': ttl, 'split_pos': split_pos, 'positions': positions, 'overlap_size': overlap, 'fooling': fooling, 'window_div': 2, 'tcp_flags': {'psh': True, 'ack': True}, 'ipid_step': 2048, 'delay_ms': 5}}
        elif desync_method in ('fake', 'fakeddisorder', 'seqovl'):
            base_params = {'ttl': ttl, 'split_pos': split_pos, 'window_div': 8, 'tcp_flags': {'psh': True, 'ack': True}, 'ipid_step': 2048}
            if fooling == 'badsum':
        ...
        return {'type': 'fakedisorder', 'params': {'ttl': ttl, 'split_pos': split_pos, 'window_div': 8, 'tcp_flags': {'psh': True, 'ack': True}, 'ipid_step': 2048}}
    ...
    def _is_target_ip(self, ip_str: str, target_ips: Set[str]) -> bool:
        ...
        if ip_str in target_ips:
            return True
        cdn_prefixes = {'104.', '172.64.', '172.67.', '162.158.', '162.159.', '104.16.', '104.17.', '104.18.', '104.19.', '104.20.', '104.21.', '104.22.', '104.23.', '104.24.', '104.25.', '104.26.', '104.27.', '104.28.', '104.29.', '104.30.', '151.101.', '199.232.', '23.', '104.', '184.', '2.16.', '95.100.', '185.199.', '87.240.', '93.186.', '54.192.', '54.230.', '54.239.', '54.182.', '216.58.', '172.217.', '142.250.', '172.253.', '13.107.', '40.96.', '40.97.', '40.98.', '40.99.', '77.88.', '5.255.', '128.140.', '217.20.', '51.89.', '51.91.', '104.131.', '104.236.', '91.108.', '149.154.'}
    ...
    def _is_tls_clienthello(self, payload: Optional[bytes]) -> bool:
        ...
        return payload and len(payload) > 6 and (payload[0] == 22) and (payload[5] == 1)
    ...
    def _send_segments(self, original_packet, w, segments: List[Tuple[bytes, int]]):
        ...
        try:
            ...
            for i, (segment_data, seq_offset) in enumerate(segments):
                ...
                new_ip_id = ip_id + i * self.current_params.get('ipid_step', 2048) & 65535
