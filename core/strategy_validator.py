...
class StrategyValidator:
    ...
    def validate_strategy(
        ...
        if not strategy_name:
            strategy_name = (
                strategy_log.get("strategy_name", "unknown") if strategy_log else "unknown"
            )

        self.logger.info(f"Validating strategy '{strategy_name}' for domain {domain}")
        ...
        try:
            ...
            # Compare operations
            result = self._compare_operations(strategy_name, expected_ops, actual_ops, pcap_result)
    ...
    def _pcap_only_analysis(
        self, strategy_name: str, pcap_result: PCAPAnalysisResult, domain: str
    ) -> ValidationResult:
        """
        ...
        operation_details = {
            ...
            "rst_packets": (
                len(pcap_result.rst_packets) if hasattr(pcap_result, "rst_packets") else 0
            ),
        }
        ...
        if operation_details["split_detected"]:
            ...
            self.logger.info(
                f"   Split: position={split_info.position}, fragments={split_info.fragment_count}"
            )
    ...
    def _extract_actual_operations(self, pcap_result: PCAPAnalysisResult) -> List[str]:
        ...
        if pcap_result.split_info and pcap_result.split_info.detected:
            split_info = pcap_result.split_info
            actual.append(
                f"split:position={split_info.position}," f"count={split_info.fragment_count}"
            )
    ...
    def generate_report(self) -> str:
        ...
        report_lines = ["=" * 80, "STRATEGY VALIDATION REPORT", "=" * 80, ""]
    ...
    def save_results(self, output_file: Path):
        ...
        try:
            results_data = {
                ...
                "summary": {
                    "valid": sum(
                        1 for r in self.validation_results if r.status == ValidationStatus.VALID
                    ),
                    "invalid": sum(
                        1 for r in self.validation_results if r.status == ValidationStatus.INVALID
                    ),
                    "partial": sum(
                        1 for r in self.validation_results if r.status == ValidationStatus.PARTIAL
                    ),
                    "unknown": sum(
                        1 for r in self.validation_results if r.status == ValidationStatus.UNKNOWN
                    ),
                },
