...
class StrategyValidator:
    ...
    def __init__(self, rule_engine: Optional[StrategyRuleEngine] = None,
    ...
    def add_manual_strategy(self, name: str, strategy: Dict[str, Any], 
                          expected_success_rate: float, description: str = ""):
    ...
    async def test_strategy_effectiveness(self, strategy: Dict[str, Any], 
                                        test_sites: List[str],
                                        hybrid_engine=None) -> StrategyTestResult:
        """
        ...
        if hybrid_engine is None:
            success_count, avg_latency, failed_sites = self._simulate_strategy_test(strategy, test_sites)
        else:
        ...
        LOG.info(f"Strategy test result: {success_count}/{len(test_sites)} success rate: {result.success_rate:.2%}")
        return result
    
    def _simulate_strategy_test(self, strategy: Dict[str, Any], test_sites: List[str]) -> Tuple[int, float, List[str]]:
        """
    ...
    async def _real_strategy_test(self, strategy: Dict[str, Any], test_sites: List[str], 
                                hybrid_engine) -> Tuple[int, float, List[str]]:
        """
        ...
        try:
            ...
            dns_cache = {site.replace('https://', '').replace('http://', ''): '1.1.1.1' 
                        for site in full_sites}
            target_ips = set(dns_cache.values())
            
            # Test strategy
            result_status, success_count, total_count, avg_latency = await hybrid_engine.execute_strategy_real_world(
                strategy=strategy,
    ...
    async def validate_generated_strategies(self, fingerprint: DPIFingerprint,
                                          test_sites: List[str],
                                          hybrid_engine=None) -> ValidationReport:
        """
        ...
        if self.rule_engine:
            ...
            alternatives = self.rule_engine.generate_multiple_strategies(fingerprint, count=3)
            generated_strategies.extend(alternatives[1:])  # Skip primary (already added)
        
        # Generate strategies using combinator
        if self.combinator:
            suggestions = self.combinator.suggest_combinations_for_fingerprint(fingerprint)
        ...
        for strategy in generated_strategies:
            result = await self.test_strategy_effectiveness(strategy, test_sites, hybrid_engine)
        ...
        best_generated = max(generated_results, key=lambda r: r.success_rate) if generated_results else None
        best_manual = max(manual_results, key=lambda r: r.success_rate) if manual_results else None
        
        # Generate improvement suggestions
        suggestions = self._generate_improvement_suggestions(generated_results, manual_results)
        ...
        LOG.info(f"Validation complete. Best generated: {best_generated.success_rate:.2%}, "
                f"Best manual: {best_manual.success_rate:.2%}")
        
        return report
    
    def _generate_improvement_suggestions(self, generated_results: List[StrategyTestResult],
                                        manual_results: List[StrategyTestResult]) -> List[str]:
        ...
        if performance_gap > 0.1:  # 10% gap
            suggestions.append(f"Generated strategies underperform by {performance_gap:.1%}. "
            ...
            if manual_ttl != generated_ttl:
                suggestions.append(f"Consider using TTL={manual_ttl} instead of TTL={generated_ttl}")
            ...
            if manual_fooling != generated_fooling:
                ...
                if missing:
                    suggestions.append(f"Consider adding fooling methods: {list(missing)}")
                if extra:
                    suggestions.append(f"Consider removing fooling methods: {list(extra)}")
        
        elif performance_gap < -0.05:  # Generated is better by 5%
            suggestions.append("Generated strategies outperform manual ones. "
                             "Consider updating manual strategy database.")
        
        # Latency analysis
        avg_generated_latency = sum(r.avg_latency for r in generated_results) / len(generated_results)
        avg_manual_latency = sum(r.avg_latency for r in manual_results) / len(manual_results)
        
        if avg_generated_latency > avg_manual_latency * 1.2:  # 20% slower
            suggestions.append("Generated strategies have higher latency. "
                             "Consider optimizing for performance.")
        
        return suggestions
    
    def _compare_performance(self, generated_results: List[StrategyTestResult],
                           manual_results: List[StrategyTestResult]) -> Dict[str, float]:
        """Compare performance metrics between generated and manual strategies"""
        
        comparison = {}
        
        if generated_results:
            comparison["avg_generated_success_rate"] = sum(r.success_rate for r in generated_results) / len(generated_results)
            comparison["max_generated_success_rate"] = max(r.success_rate for r in generated_results)
            comparison["avg_generated_latency"] = sum(r.avg_latency for r in generated_results) / len(generated_results)
        
        if manual_results:
            comparison["avg_manual_success_rate"] = sum(r.success_rate for r in manual_results) / len(manual_results)
            comparison["max_manual_success_rate"] = max(r.success_rate for r in manual_results)
            comparison["avg_manual_latency"] = sum(r.avg_latency for r in manual_results) / len(manual_results)
        
        if generated_results and manual_results:
            comparison["success_rate_improvement"] = (
                comparison["max_generated_success_rate"] - comparison["max_manual_success_rate"]
            )
...
if __name__ == "__main__":
    ...
    if DEPENDENCIES_AVAILABLE:
        ...
        async def run_validation():
            report = await validator.validate_generated_strategies(test_fingerprint, test_sites)
            ...
            if report.best_generated:
                print(f"Best generated success rate: {report.best_generated.success_rate:.2%}")
            
            if report.best_manual:
                print(f"Best manual success rate: {report.best_manual.success_rate:.2%}")
