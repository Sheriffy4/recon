...
class AttackParityMetricsCollector:
    ...
    def __init__(self, retention_hours: int = 24, auto_save: bool = True,
    ...
    def record_compliance(self, domain: str, score: int, max_score: int,
                         issues_count: int, expected_attacks: List[str],
                         detected_attacks: List[str], mode: str = "production"):
        """
        ...
        logger.info(f"Recorded compliance metric: {domain} - {percentage:.1f}% ({score}/{max_score})")
    
    def record_attack_detection(self, attack_type: str, total_attempts: int,
        ...
        """
        detection_rate = (successful_detections / total_attempts * 100) if total_attempts > 0 else 0.0
    ...
    def record_strategy_application(self, domain: str, strategy_id: str,
                                   attacks: List[str], success: bool,
                                   error_message: Optional[str],
                                   application_time_ms: float,
                                   mode: str = "production"):
        """
        ...
        logger.info(f"Recorded application metric: {domain} - {status} ({application_time_ms:.1f}ms)")
    
    def record_pcap_validation(self, pcap_file: str, validation_success: bool,
                              error_type: Optional[str], error_message: Optional[str],
                              packets_analyzed: int, streams_found: int,
                              clienthello_found: bool, validation_time_ms: float):
        """
        ...
        logger.info(f"Recorded validation metric: {pcap_file} - {status} ({validation_time_ms:.1f}ms)")
    ...
    def get_summary(self, time_window_minutes: int = 60) -> MetricsSummary:
        ...
        with self._lock:
            ...
            if recent_compliance:
                ...
                summary.average_compliance_score = sum(m.percentage for m in recent_compliance) / len(recent_compliance)
                summary.perfect_compliance_count = sum(1 for m in recent_compliance if m.percentage == 100.0)
                summary.failed_compliance_count = sum(1 for m in recent_compliance if m.percentage < 50.0)
            
            # Attack detection metrics
            if recent_detection:
                summary.total_attack_detections = sum(m.total_attempts for m in recent_detection)
                summary.successful_detections = sum(m.successful_detections for m in recent_detection)
                summary.failed_detections = sum(m.failed_detections for m in recent_detection)
                if summary.total_attack_detections > 0:
                    summary.overall_detection_rate = (summary.successful_detections / summary.total_attack_detections * 100)
            ...
            if recent_application:
                ...
                if summary.total_strategy_applications > 0:
                    summary.application_success_rate = (summary.successful_applications / summary.total_strategy_applications * 100)
                ...
                for m in recent_application:
                    if not m.success and m.error_message:
                        ...
                        summary.failures_by_error_type[error_type] = summary.failures_by_error_type.get(error_type, 0) + 1
                
                # Average application time
                summary.average_application_time_ms = sum(m.application_time_ms for m in recent_application) / len(recent_application)
            
            # PCAP validation metrics
            if recent_validation:
                summary.total_pcap_validations = len(recent_validation)
                summary.successful_validations = sum(1 for m in recent_validation if m.validation_success)
                summary.failed_validations = sum(1 for m in recent_validation if not m.validation_success)
                if summary.total_pcap_validations > 0:
                    summary.validation_success_rate = (summary.successful_validations / summary.total_pcap_validations * 100)
                
                # Errors by type
                for m in recent_validation:
                    if not m.validation_success and m.error_type:
                        summary.errors_by_type[m.error_type] = summary.errors_by_type.get(m.error_type, 0) + 1
                
                # Average validation time
                summary.average_validation_time_ms = sum(m.validation_time_ms for m in recent_validation) / len(recent_validation)
            
            return summary
    
    def get_compliance_history(self, domain: Optional[str] = None, limit: int = 100) -> List[ComplianceMetric]:
    ...
    def get_detection_history(self, attack_type: Optional[str] = None, limit: int = 100) -> List[AttackDetectionMetric]:
    ...
    def get_application_history(self, domain: Optional[str] = None, limit: int = 100) -> List[StrategyApplicationMetric]:
    ...
    def _load_metrics(self):
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.compliance_metrics)} compliance, "
                       f"{len(self.detection_metrics)} detection, "
                       f"{len(self.application_metrics)} application, "
                       f"{len(self.validation_metrics)} validation metrics")
        except Exception as e:
