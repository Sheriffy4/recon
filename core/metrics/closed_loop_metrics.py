...
class ClosedLoopMetricsCollector:
    ...
    def record_iteration_start(self, domain: str, iteration_number: int):
        ...
        with self._lock:
            ...
            LOG.debug(
                f"📊 Итерация {self.metrics.iterations_count}: {domain} (#{iteration_number})"
            )
    ...
    def record_strategies_generated(self, strategies_count: int):
        ...
        with self._lock:
            ...
            if self._strategies_per_iteration_history:
                self.metrics.strategies_generated_per_iteration = sum(
                    self._strategies_per_iteration_history
                ) / len(self._strategies_per_iteration_history)

            LOG.debug(
                f"📊 Сгенерировано {strategies_count} стратегий, "
    ...
    def record_pattern_match(self, pattern_id: str, matched: bool = True):
        ...
        with self._lock:
            if matched:
                ...
                LOG.debug(
                    f"📊 Паттерн совпал: {pattern_id} (всего совпадений: {self.metrics.pattern_matches_total})"
                )
    ...
    def record_pattern_success(self, pattern_id: str, success: bool):
        ...
        with self._lock:
            ...
            LOG.debug(
                f"📊 Паттерн {pattern_id}: успех={success}, "
    ...
    def record_iteration_success(self, domain: str, iterations_to_success: int):
        ...
        with self._lock:
            ...
            if self._success_history:
                self.metrics.average_iterations_to_success = sum(self._success_history) / len(
                    self._success_history
                )

            LOG.info(
                f"📊 Успех для {domain} за {iterations_to_success} итераций, "
    ...
    def update_from_adaptive_engine(self, adaptive_engine):
        ...
        try:
            with self._lock:
                ...
                if (
                    hasattr(adaptive_engine, "knowledge_accumulator")
                    and adaptive_engine.knowledge_accumulator
                ):
        ...
        try:
            with self._lock:
                ...
                for pattern in knowledge_accumulator.get_all_patterns():
                    ...
                    if total_count > 0:
                        self.metrics.success_rate_by_pattern[pattern.id] = (
    ...
    def get_metrics(self) -> ClosedLoopMetrics:
        ...
        with self._lock:
            ...
            metrics_copy.strategies_generated_per_iteration = (
        ...
        try:
            ...
            with self._lock:
                data = {
                    ...
                    "recent_iterations": (
                        list(self._iteration_history)[-10:] if self._iteration_history else []
                    ),
                }
    ...
    def get_summary_report(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            top_patterns = sorted(
                metrics.success_rate_by_pattern.items(), key=lambda x: x[1], reverse=True
            )[:5]
            ...
            return {
                ...
                "top_patterns": [
                    {"pattern_id": pid, "success_rate": rate} for pid, rate in top_patterns
                ],
