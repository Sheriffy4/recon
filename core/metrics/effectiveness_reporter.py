...
class RuleEffectivenessStats:
    ...
    def calculate_effectiveness_score(self) -> float:
        ...
        total_score = (
            success_component + usage_component + universality_component + recency_component
        )
...
class EffectivenessReport:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for field_name in [
            "top_rules_by_success_rate",
            "top_rules_by_usage",
            "top_rules_by_effectiveness",
        ]:
...
class EffectivenessReporter:
    ...
    def analyze_rule_effectiveness(self, knowledge_accumulator) -> List[RuleEffectivenessStats]:
        ...
        try:
            ...
            for pattern in patterns:
                stats = RuleEffectivenessStats(rule_id=pattern.id, description=pattern.description)

                # Извлекаем метрики из метаданных
                metadata = pattern.metadata

                stats.total_applications = metadata.get("success_count", 0) + metadata.get(
                    "failure_count", 0
                )
                ...
                # Intent'ы
                stats.recommended_intents = [rec.get("intent", "") for rec in pattern.recommend]
    ...
    def generate_effectiveness_report(
        self, knowledge_accumulator, top_count: int = 10
    ) -> EffectivenessReport:
        """
        ...
        rules_by_usage = sorted(rule_stats, key=lambda x: x.total_applications, reverse=True)
        ...
        rules_by_effectiveness = sorted(
            rule_stats, key=lambda x: rule.effectiveness_score, reverse=True
        )
        ...
        LOG.info(
            f"Сгенерирован отчет об эффективности: {report.total_rules} правил, "
    ...
    def _generate_recommendations(self, rule_stats: List[RuleEffectivenessStats]) -> List[str]:
        ...
        low_performance_rules = [
            r for r in rule_stats if r.total_applications > 5 and r.success_rate < 0.3
        ]
    ...
    def export_report_json(
        self, report: EffectivenessReport, filename: Optional[str] = None
    ) -> str:
        """
    ...
    def generate_top_rules_visualization(
        self, report: EffectivenessReport, filename: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                ...
                f.write("-" * 50 + "\n")
                f.write(
                    f"{'Ранг':<4} {'ID правила':<20} {'Успешность':<12} {'Применений':<12} {'Доменов':<8}\n"
                )
                f.write("-" * 50 + "\n")

                for i, rule in enumerate(report.top_rules_by_success_rate, 1):
                    f.write(
                        f"{i:<4} {rule.rule_id[:19]:<20} {rule.success_rate:.1%}{'':>4} "
                ...
                f.write("-" * 50 + "\n")
                f.write(
                    f"{'Ранг':<4} {'ID правила':<20} {'Применений':<12} {'Успешность':<12} {'Доменов':<8}\n"
                )
                f.write("-" * 50 + "\n")

                for i, rule in enumerate(report.top_rules_by_usage, 1):
                    f.write(
                        f"{i:<4} {rule.rule_id[:19]:<20} {rule.total_applications:<12} "
