...
class SegmentWorkflowIntegration:
    ...
    async def _execute_single_scenario(
        ...
        try:
            ...
            if attack_result.status == AttackStatus.SUCCESS:
                ...
                result.issues_encountered.append(f"Attack {attack_type} failed: {attack_result.error}")
            ...
            execution_detail = {
                ...
                'effectiveness_score': effectiveness if attack_result.status == AttackStatus.SUCCESS else 0,
                'timestamp': time.time()
            }
    ...
    def _calculate_final_metrics(self, result: WorkflowExecutionResult):
        ...
        if result.effectiveness_scores:
            result.average_effectiveness_score = sum(result.effectiveness_scores) / len(result.effectiveness_scores)
