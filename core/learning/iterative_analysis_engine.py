...
try:
    ...
    from ..strategy_failure_analyzer import StrategyFailureAnalyzer, FailureReport, FailureCause
    from ..fingerprint.dpi_fingerprint_service import DPIFingerprintService, DPIFingerprint
...
class IterativeAnalysisEngine:
    ...
    async def analyze_pcap_iteration(
        self,
        domain: str,
        pcap_files: List[str],
        iteration_number: int = 1,
        previous_context: Optional[IterationContext] = None,
    ) -> IterationContext:
        """
        ...
        if previous_context:
            context.previous_strategies = [asdict(s) for s in previous_context.generated_strategies]
        ...
        try:
            LOG.info(
                f"Starting PCAP iteration analysis for {domain} (iteration {iteration_number})"
            )
            ...
            correlations = await self._correlate_with_previous_data(
                domain, analysis_results, previous_context
            )
            ...
            LOG.info(
                f"PCAP iteration analysis completed for {domain} in {context.processing_time:.2f}s"
            )
    ...
    async def correlate_dpi_patterns(
        self, fingerprint: DPIFingerprint, pcap_analysis: PCAPAnalysisResult, domain: str
    ) -> CorrelationResult:
        """
        ...
        try:
            ...
            timing_correlations = await self._correlate_timing_patterns(fingerprint, pcap_analysis)
            ...
            LOG.info(
                f"DPI pattern correlation completed with strength {correlation.correlation_strength:.3f}"
            )
    ...
    async def generate_adaptive_strategies(
        self, domain: str, iteration_contexts: List[IterationContext], target_count: int = 5
        ...
        """
        LOG.info(
            f"Generating adaptive strategies for {domain} based on {len(iteration_contexts)} iterations"
        )
    ...
    async def update_knowledge_base(
        ...
        """
        LOG.info(
            f"Updating knowledge base for {domain} with {len(strategy_results)} strategy results"
        )
        ...
        try:
            ...
            for strategy, success, metadata in strategy_results:
                await self._update_strategy_effectiveness(domain, strategy, success, metadata)
    ...
    async def evaluate_strategy_effectiveness(
        ...
        if strategy_key in self.effectiveness_metrics:
            ...
            metrics = StrategyEffectivenessMetrics(strategy_id=strategy.strategy_id, domain=domain)
        ...
        try:
            ...
            LOG.info(
                f"Strategy effectiveness evaluation completed: success_rate={metrics.success_rate:.3f}"
            )
    ...
    async def _analyze_pcap_files(
        self, pcap_files: List[str], context: IterationContext
    ...
    async def _correlate_with_previous_data(
        self,
        domain: str,
        analysis_results: List[PCAPAnalysisResult],
        previous_context: Optional[IterationContext],
    ) -> Dict[str, Any]:
        ...
        correlations["pattern_consistency"] = len(current_patterns & previous_patterns) / max(
            len(current_patterns | previous_patterns), 1
        )
    ...
    async def _generate_iteration_insights(self, context: IterationContext) -> List[str]:
        ...
        if blocking_types:
            ...
            insights.append(
                f"Most common blocking type: {most_common[0]} ({most_common[1]} occurrences)"
            )

        # Анализ корреляций
        if context.correlations.get("pattern_consistency", 0) > 0.8:
            insights.append(
                "High pattern consistency with previous iterations - DPI behavior is stable"
            )
    ...
    async def _correlate_behavioral_patterns(
        self, fingerprint: DPIFingerprint, pcap_analysis: PCAPAnalysisResult
    ) -> Dict[str, float]:
    ...
    async def _correlate_timing_patterns(
        self, fingerprint: DPIFingerprint, pcap_analysis: PCAPAnalysisResult
    ) -> Dict[str, float]:
        ...
        if pcap_timings and fingerprint_timing > 0:
            ...
            correlations["timing_consistency"] = max(
                0, 1.0 - timing_diff / max(fingerprint_timing, avg_pcap_timing)
            )
        else:
    ...
    async def _correlate_blocking_patterns(
        self, fingerprint: DPIFingerprint, pcap_analysis: PCAPAnalysisResult
    ) -> Dict[str, float]:
        """Корреляция паттернов блокировки."""
        correlations = {}
    ...
    def _calculate_correlation_strength(
        self, behavioral: Dict[str, float], timing: Dict[str, float], patterns: Dict[str, float]
    ) -> float:
        """Расчет общей силы корреляции."""
        all_correlations = []
    ...
    async def _generate_confirmed_hypotheses(
        self,
    ...
    async def _generate_new_hypotheses(
        ...
        for flow in pcap_analysis.flows:
            if flow.blocking_type.value not in fingerprint.behavioral_signatures.get(
                "observed_blocking_types", []
            ):
                new_blocking_types.add(flow.blocking_type.value)

        if new_blocking_types:
            hypotheses.append(
                f"DPI may have evolved to use new blocking methods: {', '.join(new_blocking_types)}"
            )
    ...
    async def _generate_strategy_implications(self, correlation: CorrelationResult) -> List[str]:
        ...
        if correlation.behavioral_correlations.get("blocking_type_match", 0) > 0.8:
            implications.append(
                "Strategies targeting the confirmed blocking type should be prioritized"
            )
    ...
    async def _analyze_accumulated_knowledge(
        self, domain: str, iteration_contexts: List[IterationContext]
    ) -> Dict[str, Any]:
        """Анализ накопленных знаний."""
        knowledge = {
    ...
    async def _identify_effective_patterns(
        self, domain: str, accumulated_knowledge: Dict[str, Any]
        ...
        for blocking_type, count in blocking_patterns.items():
            if count >= 2:  # Паттерн наблюдался минимум в 2 итерациях
                pattern = KnowledgePattern(
                    ...
                    confidence=min(
                        count / len(accumulated_knowledge.get("evolution_timeline", [])), 1.0
                    ),
                )
    ...
    async def _generate_strategy_from_pattern(
        self, domain: str, pattern: KnowledgePattern, accumulated_knowledge: Dict[str, Any]
        ...
        try:
                        ...
                        type(
                            ...
                            (),
                            {
                                "blocking_type": type(
                                    "BlockingType",
                                    (),
                                    {"value": pattern.features.get("blocking_type", "unknown")},
                                )(),
            ...
            strategies = await self.strategy_generator.generate_strategies_from_analysis(
                mock_analysis
            )
    ...
    async def _generate_experimental_strategies(
        self, domain: str, accumulated_knowledge: Dict[str, Any], count: int
    ) -> List[GeneratedStrategy]:
    ...
    async def _rank_strategies_by_effectiveness(
        self,
        domain: str,
    ...
    async def _update_strategy_effectiveness(
        self, domain: str, strategy: GeneratedStrategy, success: bool, metadata: Dict[str, Any]
    ):
        ...
        if strategy_key not in self.effectiveness_metrics:
            self.effectiveness_metrics[strategy_key] = StrategyEffectivenessMetrics(
                strategy_id=strategy.strategy_id, domain=domain
            )
    ...
    async def _extract_patterns_from_results(
        self,
        domain: str,
        strategy_results: List[Tuple[GeneratedStrategy, bool, Dict[str, Any]]],
        iteration_context: IterationContext,
    ...
    async def _merge_pattern_knowledge(
        self, existing_pattern: KnowledgePattern, new_pattern: KnowledgePattern
    ):
        """Слияние знаний паттернов."""
        existing_pattern.observed_count += new_pattern.observed_count
        existing_pattern.last_observed = max(
            existing_pattern.last_observed or datetime.min,
        ...
        if new_pattern.success_rate > 0:
            ...
            existing_pattern.success_rate = (
                ...
                + new_pattern.success_rate * new_pattern.observed_count
            ) / total_observations
    ...
    async def _calculate_knowledge_gain(
        self,
        domain: str,
        strategy_results: List[Tuple[GeneratedStrategy, bool, Dict[str, Any]]],
        iteration_context: IterationContext,
        ...
        correlation_strength = sum(iteration_context.correlations.values()) / max(
            len(iteration_context.correlations), 1
        )
        knowledge_gain += correlation_strength * 0.3

        return knowledge_gain

    async def _analyze_historical_effectiveness(
        self, metrics: StrategyEffectivenessMetrics, historical_data: Dict[str, Any]
    ):
    ...
    async def _predict_strategy_effectiveness(
        self,
        domain: str,
        strategy: GeneratedStrategy,
        current_metrics: StrategyEffectivenessMetrics,
    ) -> Dict[str, Any]:
