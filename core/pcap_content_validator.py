...
class ValidationIssue:
    ...
    def __str__(self) -> str:
        return (f"[{self.severity.upper()}] {self.category} at packet {self.packet_index}: "
...
class PCAPContentValidator:
    ...
    def __init__(self):
        ...
        if not SCAPY_AVAILABLE:
            raise ImportError("Scapy is required for PCAP validation. Install with: pip install scapy")
    ...
    def _validate_connection_sequences(
        self,
        conn_packets: List[Tuple[int, Packet]],
        result: PCAPValidationResult
    ):
        ...
        for idx, pkt in tcp_packets:
            ...
            if tcp_chksum != 0 or ip_chksum != 0:
                try:
                    ...
                    recalc_tcp_chksum = pkt_copy[TCP].chksum if hasattr(pkt_copy[TCP], 'chksum') else 0
