...
class SmartCache:
    ...
    def _init_persistent_cache(self):
        ...
        try:
            self.db_connection = sqlite3.connect(
                self.persistent_cache_path, check_same_thread=False
            )
    ...
    def _enforce_limits(self):
        ...
        with self.lock:
            ...
            if (
                ...
                and self.stats.entry_count <= self.max_entries
            ):
                return
            ...
            for key, entry in entries_by_access:
                if (
                    self.stats.size_bytes <= self.max_memory_bytes * 0.8
                    and self.stats.entry_count <= self.max_entries * 0.8
                ):
                    break
    ...
    def put(
        ...
        with self.lock:
            ...
            if (
                self.stats.size_bytes > self.max_memory_bytes
                or self.stats.entry_count > self.max_entries
            ):
                self._enforce_limits()
    ...
    def invalidate_by_tag(self, tag: str):
        ...
        with self.lock:
            ...
            if self.db_connection:
                try:
                    self.db_connection.execute(
                        "DELETE FROM cache_entries WHERE tags LIKE ?", (f'%"{tag}"%',)
                    )
                    self.db_connection.commit()
                except Exception as e:
...
class FingerprintCache(SmartCache):
    ...
    def put_fingerprint(
        ...
        tags = {f"domain:{domain}", f"port:{port}", "fingerprint"}
...
class StrategyCache(SmartCache):
    ...
    def put_strategy_result(self, domain: str, strategy_hash: str, result: Dict[str, Any]):
        ...
        tags = {f"domain:{domain}", f"strategy:{strategy_hash}", "strategy_result"}
