...
class StrategyManager:
    ...
    def __init__(
        ...
        self.domain_rules_file = Path(env_path or domain_rules_file)
        ...
        if self.strategies_file.exists():
            try:
                ...
                for domain, strategy_data in data.get("domain_strategies", {}).items():
                    self.domain_strategies[domain] = DomainStrategy.from_dict(strategy_data)

                self.logger.info(f"Loaded {len(self.domain_strategies)} domain strategies")
                return
    ...
    def save_legacy_format(self):
        ...
        try:
            ...
            best_strategy = max(self.domain_strategies.values(), key=lambda s: s.success_rate)
    ...
    def _parse_strategy_to_domain_rules(self, strategy_str: str) -> Optional[Dict[str, Any]]:
        ...
        return {"type": strategy_type, "params": params, "attacks": attacks}
            ...
            for domain, strategy_obj in self.domain_strategies.items():
                ...
                if converted:
                    ...
                    if not domain.startswith("*.") and not domain.startswith("www."):
                        ...
                        wildcard_entry["metadata"][
                            "strategy_id"
                        ] = f"{wildcard_domain}_{strategy_type}_discovered"
    ...
    def get_strategies_for_service(self) -> Dict[str, str]:
        ...
        return {domain: strategy.strategy for domain, strategy in self.domain_strategies.items()}
