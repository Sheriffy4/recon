...
import aiohttp
from recon import config as main_config_module
...
def _find_tool_path(tool_name: str, base_path: str) -> str:
    ...
    tool_path_rel = ''
    if os.name == 'nt':
        path_map = {'zapret': main_config_module.ZAPRET_NT_PATH, 'goodbyedpi': main_config_module.GOODBYEDPI_NT_PATH}
        name_map = {'zapret': main_config_module.ZAPRET_NT_TOOL_NAME, 'goodbyedpi': main_config_module.GOODBYEDPI_NT_TOOL_NAME}
        tool_path_rel = os.path.join(path_map.get(tool_name, ''), name_map.get(tool_name, ''))
    else:
        found_in_path = shutil.which(main_config_module.ZAPRET_LINUX_TOOL_NAME)
        if found_in_path:
            return found_in_path
        tool_path_rel = os.path.join(main_config_module.ZAPRET_LINUX_PATH, main_config_module.ZAPRET_LINUX_TOOL_NAME)
    absolute_path = os.path.join(base_path, tool_path_rel)
    if not os.path.exists(absolute_path):
        raise FileNotFoundError(f'Tool executable not found at expected path: {absolute_path}')
    return absolute_path

async def _test_single_site(session: aiohttp.ClientSession, site: str, semaphore: asyncio.Semaphore) -> Tuple[str, str, float, int, str]:
    """
    ...
    async with semaphore:
        ...
        try:
            ...
            async with session.get(site, headers=main_config_module.HEADERS, timeout=timeout, ssl=False, allow_redirects=True) as response:
                status = 'WORKING'
                http_status = response.status
                await response.content.readexactly(1)
                error_detail = 'OK'
        except (aiohttp.ClientPayloadError, asyncio.TimeoutError, ConnectionResetError) as e:
            status = 'WORKING'
...
class AppLevelTester:
    ...
    def start_tool(self, strategy_str: str) -> bool:
        ...
        if os.name == 'nt':
            ...
            try:
                LOG.info(f"Starting '{self.tool_path}' with args: {' '.join(final_args)}")
                self.process = subprocess.Popen([self.tool_path] + final_args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, cwd=os.path.dirname(self.tool_path))
                time.sleep(1.5)
                if self.process.poll() is not None:
                    LOG.error(f"Tool '{self.tool_name}' terminated immediately after start.")
                    return False
                LOG.info(f"Tool '{self.tool_name}' started with PID: {self.process.pid}")
                return True
            except Exception:
                LOG.exception(f"Failed to start tool '{self.tool_name}':")
                return False
        else:
            LOG.error('Linux service management is not fully implemented in this version of the tester.')
            return False
    ...
    def stop_tool(self):
        ...
        if os.name == 'nt':
            try:
                subprocess.run(['taskkill', '/F', '/T', '/PID', str(self.process.pid)], capture_output=True, check=False, timeout=5)
                self.process.wait(timeout=5)
            except (subprocess.TimeoutExpired, Exception) as e:
                LOG.warning(f'Could not gracefully stop process {self.process.pid}, killing. Error: {e}')
                self.process.kill()
        else:
    ...
    async def test_strategy(self, sites: List[str]) -> Dict[str, Tuple]:
        ...
        """
        results = {}
        connector = aiohttp.TCPConnector(limit=100, limit_per_host=25, ttl_dns_cache=300, ssl=False, force_close=True)
        try:
