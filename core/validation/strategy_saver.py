...
from core.test_result_models import TestVerdict, SaveResult, VerdictType
...
class StrategySaver:
    ...
    def save_strategy(
        ...
        if verdict.verdict != VerdictType.SUCCESS:
            LOG.debug(
                f"Skipping save for {domain}: verdict is {verdict.verdict.value}, not SUCCESS"
            )
            return SaveResult(
                success=False,
                files_updated=[],
        ...
        with self.save_lock:
            ...
            try:
                ...
                try:
                    if self._save_to_adaptive_knowledge(
                        domain, strategy_name, parameters, success_rate, verified
                    ):
                        files_updated.append(str(self.adaptive_knowledge_path))
                        # Log in consistent format (Requirement 10.4)
                        LOG.info(
                            f"Saved strategy: [{strategy_name}] to [{self.adaptive_knowledge_path.name}]"
                        )
                ...
                try:
                    if self._save_to_domain_rules(domain, strategy_name, parameters, attacks or []):
                        ...
                        # Log in consistent format (Requirement 10.4)
                        LOG.info(
                            f"Saved strategy: [{strategy_name}] to [{self.domain_rules_path.name}]"
                        )
                except Exception as e:
                ...
                # Task 8.4: Log errors but don't fail test
                try:
                    if self._save_to_domain_strategies(
                        domain, strategy_name, parameters, success_rate
                    ):
                        files_updated.append(str(self.domain_strategies_path))
                        # Log in consistent format (Requirement 10.4)
                        LOG.info(
                            f"Saved strategy: [{strategy_name}] to [{self.domain_strategies_path.name}]"
                        )
                except Exception as e:
                ...
                if files_updated:
                    LOG.info(
                        f"Saved strategy {strategy_name} for {domain} to {len(files_updated)} files"
                    )
                    if errors:
                        LOG.warning(
                            f"Partial save success with {len(errors)} errors: {'; '.join(errors)}"
                        )
                else:
    ...
    def _save_to_adaptive_knowledge(
        ...
        try:
            ...
            if domain not in data:
                data[domain] = {"strategies": [], "preferred_strategy": None, "block_type": None}
            ...
            for i, strategy in enumerate(domain_data["strategies"]):
                if (
                    strategy.get("strategy_name") == strategy_name
                    and strategy.get("strategy_params") == parameters
                ):
                    # Update existing
                    strategy["success_count"] = strategy.get("success_count", 0) + 1
                    strategy["last_success_ts"] = time.time()
                    strategy["verified"] = verified
                    strategy["verification_ts"] = (
                        time.time() if verified else strategy.get("verification_ts")
                    )
    ...
    def _save_to_domain_rules(
        self, domain: str, strategy_name: str, parameters: Dict[str, Any], attacks: List[str]
    ) -> bool:
        """
        ...
        try:
            ...
            data = self._load_json_file(
                ...
                default={"version": "1.0", "last_updated": "", "domain_rules": {}},
            )
    ...
    def _save_to_domain_strategies(
        self, domain: str, strategy_name: str, parameters: Dict[str, Any], success_rate: float
    ) -> bool:
        """
        ...
        try:
            # Load existing data
            data = self._load_json_file(
                self.domain_strategies_path,
                default={"version": "2.0", "last_updated": "", "domain_strategies": {}},
            )
