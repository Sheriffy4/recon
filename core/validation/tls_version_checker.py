...
class TLSVersionChecker:
    ...
    def extract_tls_version(payload: bytes) -> Optional[str]:
        ...
        try:
            ...
            return TLSVersionChecker.TLS_VERSIONS.get(
                version_bytes, f"Unknown (0x{version_bytes:04X})"
            )
    ...
    @staticmethod
    def check_consistency(test_hello: bytes, bypass_hello: bytes) -> Tuple[bool, dict]:
        """
    ...
    @staticmethod
    def validate_split_pos_for_versions(split_pos: int, tls12_size: int, tls13_size: int) -> bool:
        """
