    ...
    def check_compliance(
        ...
        if METRICS_AVAILABLE:
            try:
                ...
                if detected.split:
                    detected_attack_list.append('multisplit' if detected.fragment_count > 2 else 'split')
            ...
            if detected.split:
                ...
                if split_pos == 'sni':
                    if not detected.split_near_sni:
                        score = max(0, score - 2)
                        issues.append(
                            "Split position not near SNI as expected"
                        )
