...
class StrategyValidator:
    ...
    def validate(
        self, declared_strategy_name: str, pcap_analysis: PCAPAnalysisResult
    ) -> ValidationResult:
        """
        ...
        if pcap_analysis is None:
            self.logger.error(
                "❌ Edge case: pcap_analysis is None, setting applied_strategy='unknown'"
            )
        ...
        try:
            ...
            if pcap_analysis.executed_attacks_from_log:
                ...
                self.logger.info(
                    f"✅ Using executed attacks from metadata: {applied_strategy_name}"
                )
                ...
                if metadata and "parameters" in metadata:
                    ...
                    self.logger.info(
                        f"✅ Loaded executed parameters from metadata: {executed_params}"
                    )
            ...
            strategy_match = StrategyNameNormalizer.are_equivalent(
                declared_strategy_name, applied_strategy_name
            )
            ...
            try:
                ...
                parameter_count = len(
                    [v for v in pcap_analysis.parameters.values() if v is not None]
                )

                # Use attack registry defaults for missing parameters
                self._fill_missing_parameters(detected_attacks, pcap_analysis.parameters)
                ...
                parameter_count = 0
                # Use all defaults
                self._fill_missing_parameters(detected_attacks, {})
            ...
            if missing_components:
                errors.append(f"Missing component attacks: {', '.join(missing_components)}")
                recommendations.append(
    ...
    def _check_completeness(self, declared_attacks: List[str], detected_attacks: List[str]) -> bool:
        """
    ...
    def _determine_applied_strategy(self, detected_attacks: List[str]) -> str:
        ...
        if not main_attacks:
            self.logger.debug(
                f"🔧 Only fooling attacks present → returning '{detected_attacks[0]}'"
            )
    ...
    def _fill_missing_parameters(
        self, detected_attacks: List[str], extracted_parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
            ...
            if attack in self.attack_registry:
                ...
                for param_name, default_value in attack_defaults.items():
                    if param_name not in complete_params or complete_params[param_name] is None:
                        ...
                        self.logger.debug(f"📝 Using default for {param_name}: {default_value}")

        return complete_params

    def _find_missing_components(
        self, declared_attacks: List[str], detected_attacks: List[str]
    ) -> List[str]:
        """
    ...
    def _get_default_registry(self) -> Dict[str, Any]:
        ...
        """
        return {
            "split": {"default_params": {"split_pos": 3, "split_count": 2}},
            "fake": {"default_params": {"fake_ttl": 1, "fake_count": 1}},
            "disorder": {"default_params": {"disorder_count": 2}},
            "multisplit": {"default_params": {"split_positions": [3, 5], "split_count": 3}},
            "seqovl": {"default_params": {"overlap_size": 1}},
            "badsum": {"default_params": {}},
            "badseq": {"default_params": {}},
            "ttl_manipulation": {"default_params": {"ttl": 1}},
        }
