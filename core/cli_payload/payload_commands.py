...
def format_payload_list(manager: PayloadManager, console) -> None:
    ...
    for payload_type, infos in sorted(by_type.items(), key=lambda x: x[0].value):
        ...
        for info in sorted(infos, key=lambda x: (x.source, x.domain or "")):
            ...
            console.print(f"  • {domain_str:30} {source_str:15} {size_str:10} {file_str}")
...
async def cmd_payload_capture(args, console) -> int:
    ...
    console.print("[bold cyan]Payload Capturer - Capture ClientHello[/bold cyan]")
    ...
    payload_info = manager.add_payload(
        data=result.payload, payload_type=PayloadType.TLS, domain=domain, source="captured"
    )

    console.print("[bold green]✓ Payload saved successfully[/bold green]")
    ...
    console.print(
        f"[dim]  python cli.py payload test {domain} --payload {payload_info.file_path}[/dim]"
    )
...
async def cmd_payload_test(args, console) -> int:
    ...
    console.print("[bold cyan]Payload Tester - Test Strategy with Payload[/bold cyan]")
    console.print(f"[dim]Domain: {domain}[/dim]")
    ...
    try:
        ...
        console.print("[bold red]✗ Invalid payload parameter[/bold red]")
    ...
    if isinstance(parsed, bytes):
        ...
        try:
            ...
            console.print("[bold red]✗ Failed to read payload file[/bold red]")
            console.print(f"[red]Error: {e}[/red]")
            return 1
    ...
    if payload_bytes is None:
        console.print("[bold red]✗ Failed to resolve payload[/bold red]")
        return 1
    ...
    try:
        ...
        console.print("[bold red]✗ Failed to import bypass engine[/bold red]")
        console.print(f"[red]Error: {e}[/red]")
        return 1
    ...
    console.print("[dim]Strategy: fake_disorder with custom payload[/dim]")
