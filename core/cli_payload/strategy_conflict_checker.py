...
def load_domain_rules(domain_rules_path: str = "domain_rules.json") -> Dict[str, Any]:
    ...
    try:
        if Path(domain_rules_path).exists():
            with open(domain_rules_path, "r", encoding="utf-8") as f:
                ...
                if (
                    ...
                    and "domain_rules" in data
                    and isinstance(data["domain_rules"], dict)
                ):
        ...
        LOG.debug("Error loading domain rules: %s", e, exc_info=True)
        return {}
...
def print_strategy_conflicts(conflicts: List[Dict[str, Any]], verbose: bool = False):
    ...
    for i, conflict in enumerate(conflicts, 1):
        ...
        if conflict["conflict_type"] == "different_strategy_types":
            ...
            print("  💡 Consider removing subdomain strategy to use parent domain strategy")
            ...
            print("  💡 Consider testing if parent domain parameters work for subdomain")
...
def check_parent_domain_during_testing(
    domain: str, domain_rules_path: str = "domain_rules.json"
) -> bool:
    """
    ...
    if parent_exists and parent_strategy:
        ...
        print("   before creating a subdomain-specific strategy.")
        print("")
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed strategy information"
    )
    parser.add_argument(
        "--check-domain", help="Check if a specific domain has a parent domain strategy"
    )
