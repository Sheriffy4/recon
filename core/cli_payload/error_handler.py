...
class CLIErrorHandler:
    ...
    def categorize_error(self, error: Exception, context: Optional[ErrorContext] = None) -> ErrorCategory:
        ...
        if any(keyword in error_str for keyword in [
            ...
            "refused", "reset", "socket", "ssl", "tls"
        ]):
            return ErrorCategory.NETWORK
        
        # Permission errors
        if isinstance(error, PermissionError) or any(keyword in error_str for keyword in [
            "permission", "access denied", "administrator", "privilege", "forbidden"
        ]):
        ...
        # Validation errors
        if isinstance(error, (ValueError, TypeError)) or any(keyword in error_str for keyword in [
            "invalid", "validation", "format", "parse", "decode"
        ]):
            return ErrorCategory.VALIDATION
        
        # Configuration errors
        if any(keyword in error_str for keyword in [
            "config", "setting", "parameter", "argument", "option"
        ]):
            return ErrorCategory.CONFIGURATION
        
        # System errors
        if isinstance(error, (OSError, SystemError)) or any(keyword in error_str for keyword in [
            "system", "platform", "driver", "kernel", "hardware"
        ]):
            return ErrorCategory.SYSTEM
        
        return ErrorCategory.UNKNOWN
    
    def get_error_suggestions(self, error: Exception, category: ErrorCategory, 
                            context: Optional[ErrorContext] = None) -> List[str]:
    ...
    def _display_error(self, error: Exception, severity: ErrorSeverity, 
                      category: ErrorCategory, template: Dict[str, str],
                      suggestions: List[str], context: Optional[ErrorContext]):
    ...
    def _display_error_rich(self, error: Exception, severity: ErrorSeverity,
                           category: ErrorCategory, template: Dict[str, str],
                           suggestions: List[str], context: Optional[ErrorContext]):
        ...
        if context and context.operation:
            title += f" - {context.operation}"
        
        message_parts = [f"[{color}]{str(error)}[/{color}]"]
        
        if context and context.user_action:
            message_parts.append(f"\n[dim]While: {context.user_action}[/dim]")
        ...
        if self.debug and context and context.debug_info:
            ...
            for key, value in context.debug_info.items():
                message_parts.append(f"  [dim]{key}: {value}[/dim]")
    ...
    def _display_error_plain(self, error: Exception, severity: ErrorSeverity,
                            category: ErrorCategory, template: Dict[str, str],
                            suggestions: List[str], context: Optional[ErrorContext]):
        """Display error using plain text formatting"""
        # Create header
        header = f"{template['icon']} {template['title']}"
        if context and context.operation:
            header += f" - {context.operation}"
        ...
        if context and context.user_action:
            print(f"While: {context.user_action}")
...
def handle_cli_error(error: Exception, operation: str, component: str = "CLI",
