...
# Scapy is optional: do not fail module import if it's not installed.
...
class StrategyDiffTool:
    ...
    def _resolve_expected_strategy(self, domain: str) -> Optional[Dict[str, Any]]:
        """
        Try to resolve expected strategy by:
        1) exact domain
        2) wildcard *.base.tld (last 2 labels)
        3) parent domain chain
        """
        if not domain:
            return None
        ...
        if len(parts) >= 2:
            ...
            if wildcard in self.domain_rules:
                return self.domain_rules.get(wildcard)
        if len(parts) > 2:
            for i in range(1, len(parts) - 1):
                parent = ".".join(parts[i:])
                if parent in self.domain_rules:
                    return self.domain_rules.get(parent)
                wildcard_parent = "*." + parent
                if wildcard_parent in self.domain_rules:
                    return self.domain_rules.get(wildcard_parent)
        return None

    def compare_strategies(
        self, domain: str, actual_strategy: Dict[str, Any]
        ...
        """
        expected_strategy = self._resolve_expected_strategy(domain)
...
class PCAPStrategyAnalyzer:
    ...
    def analyze_pcap(
        self, pcap_path: str, expected_strategy: Dict[str, Any], target_domain: str
    ) -> Dict[str, Any]:
        """
        ...
        if not SCAPY_AVAILABLE:
            return {
                "success": False,
                "error": f"Scapy is not available: {_SCAPY_IMPORT_ERROR}. Install scapy to analyze PCAP.",
            }
        try:
            packets = rdpcap(pcap_path)
        except Exception as e:
            return {"success": False, "error": f"Failed to read PCAP: {e}"}
        ...
        if strategy_type in ["split", "multisplit"]:
            ...
            analysis["findings"].append(
                {"type": "warning", "message": f"Unknown strategy type: {strategy_type}"}
            )
    ...
    def _find_client_hello_packets(self, packets: List, target_domain: str) -> List[Dict]:
        ...
        for i, pkt in enumerate(packets):
            ...
            if payload[0] == 0x16 and payload[1] == 0x03:  # TLS Handshake
                ...
                if target_domain.lower().encode() in payload.lower():
    ...
    def _analyze_split_strategy(self, client_hellos: List[Dict], params: Dict, analysis: Dict):
        """Analyze split/multisplit strategy application."""
        _ = params.get("split_pos", 3)  # Not used in current analysis
    ...
    def _analyze_disorder_strategy(self, client_hellos: List[Dict], params: Dict, analysis: Dict):
        ...
        if seqs[0] < seqs[1]:
            ...
            analysis["findings"].append(
                {"type": "success", "message": "Packets in correct disorder order"}
            )

    def _analyze_fake_strategy(self, client_hellos: List[Dict], params: Dict, analysis: Dict):
        ...
        if not real_packets:
            analysis["findings"].append(
                {"type": "error", "severity": "critical", "message": "No real packets found"}
            )
...
class StrategyFailureReportGenerator:
    ...
    def _safe_filename(text: str) -> str:
        ...
        return "".join(
            c if c.isalnum() or c in ("-", "_", ".") else "_" for c in (text or "unknown")
        )
    ...
    def generate_failure_report(
        ...
        report_file = (
            self.output_dir / f"failure_report_{self._safe_filename(domain)}_{timestamp}.md"
        )
        ...
        if diffs:
            ...
            if critical_diffs:
                recommendations.append(
                    "1. **Critical parameter mismatches detected** - Verify domain_rules.json configuration"
                )
                recommendations.append(
                    "2. Re-run strategy discovery: `python cli.py auto " + domain + "`"
                )
        ...
        if pcap_analysis and not pcap_analysis.get("strategy_applied_correctly"):
            recommendations.append(
                "5. **PCAP analysis shows issues** - Check packet sending implementation"
            )
        ...
        report.append(
            f"2. Compare testing vs production mode: `python cli.py compare-modes {domain}`"
        )
        report.append(f"3. Re-test strategy: `python cli.py auto {domain}`")
        report.append("4. Check service logs: `tail -f recon_service.log`")
...
class VerboseStrategyLogger:
    ...
    def _write_to_file(self, record: StrategyApplicationRecord):
        ...
        try:
            ...
            LOG.debug("Failed to write to log file: %s", e, exc_info=True)
