...
class TestHybridEngineFingerprinting:
    ...
    def test_data(self):
        ...
        return {
            ...
            "ips": {"1.2.3.4", "5.6.7.8"},
            "dns_cache": {
                "blocked-site.com": "1.2.3.4",
                "another-blocked.com": "5.6.7.8",
            },
            "strategies": [
    ...
    async def test_fingerprint_target_success(self, hybrid_engine, mock_fingerprint):
        ...
        hybrid_engine.advanced_fingerprinter.fingerprint_target.assert_called_once_with(
            target="blocked-site.com", port=443, force_refresh=False
        )
    ...
    def test_adapt_strategies_for_fingerprint_roskomnadzor_tspu(
        self, hybrid_engine, mock_fingerprint
    ):
        ...
        low_ttl_found = any(
            "--dpi-desync-ttl=1" in s or "--dpi-desync-ttl=2" in s for s in adapted
        )
        ...
        assert (
            fake_disorder_found
        ), "Fake+disorder strategies should be present for TSPU"

    def test_adapt_strategies_for_fingerprint_commercial_dpi(
        self, hybrid_engine, mock_fingerprint
    ):
        ...
        assert (
            multisplit_found
        ), "Multisplit strategies should be added for commercial DPI"
    ...
    def test_prioritize_strategies(self, hybrid_engine):
        ...
        prioritized = hybrid_engine._prioritize_strategies(
            strategies, priority_patterns
        )
    ...
    @pytest.mark.asyncio
    async def test_test_strategies_hybrid_with_fingerprinting(
        self, hybrid_engine, mock_fingerprint, test_data
    ):
        ...
        hybrid_engine.fingerprint_target.assert_called_once_with(
            test_data["domain"], test_data["port"]
        )
    ...
    @pytest.mark.asyncio
    async def test_test_strategies_hybrid_without_fingerprinting(
        self, hybrid_engine, test_data
    ):
