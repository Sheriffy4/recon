...
class HybridEngine:
    ...
    async def execute_strategy_real_world(
        ...
        try:
            # FIX: Force the engine to use the exact task provided, bypassing complex internal logic.
            # The original `start` method ignores the strategy_map in some cases.
            # `start_with_strategy` is the direct way to test a single, specific strategy.
            # FIX: Force the engine to use the exact task provided, bypassing complex internal logic.
            # The original `start` method ignores the strategy_map in some cases.
            # `start_with_strategy` is the direct way to test a single, specific strategy.
            if hasattr(bypass_engine, "start_with_strategy") and callable(getattr(bypass_engine, "start_with_strategy")):
                LOG.info(f"Forcing engine to start directly with strategy: {self._task_to_str(engine_task)}")
                bypass_thread = bypass_engine.start_with_strategy(target_ips, dns_cache, engine_task)
            else:
                # Fallback to the old method if start_with_strategy is not available
                LOG.warning("BypassEngine does not have 'start_with_strategy' method, falling back to default start.")
