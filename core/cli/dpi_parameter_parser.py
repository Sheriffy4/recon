...
from ..bypass.strategies.config_models import DPIConfig, SplitConfig, FoolingConfig
...
class DPIParameterParser:
    ...
    def __init__(self):
        ...
        self.supported_split_positions = ['sni', 'cipher', 'midsld']  # Special positions
    ...
    def add_dpi_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            '--dpi-desync-ttl',
            type=int,
            help='TTL value for fake packets'
        )
    ...
    def parse_dpi_config(self, args: argparse.Namespace) -> DPIConfig:
        ...
        try:
            ...
            raise ConfigurationError('dpi_config', str(args), f"Configuration parsing failed: {e}")
    ...
    def _validate_config(self, config: DPIConfig) -> None:
        ...
        if config.desync_mode not in self.supported_desync_modes:
            raise ConfigurationError(
                'desync_mode', config.desync_mode,
                f"Unsupported desync mode. Supported: {self.supported_desync_modes}"
            )
        ...
        if config.ttl is not None and (config.ttl < 1 or config.ttl > 255):
            raise ConfigurationError(
                'ttl', config.ttl, "TTL must be between 1 and 255"
            )
        ...
        if config.split_count is not None and (config.split_count < 2 or config.split_count > 20):
            raise ConfigurationError(
                'split_count', config.split_count, "Split count must be between 2 and 20"
            )
