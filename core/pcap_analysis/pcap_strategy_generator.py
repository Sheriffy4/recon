...
class PCAPStrategyGenerator:
    ...
    async def generate_strategies(
        ...
        try:
            ...
            LOG.info(
                f"Генерация завершена: {len(strategies)} стратегий за "
    ...
    async def _generate_primary_strategies(
        self, pcap_analysis: PCAPAnalysisResult
        ...
        strategy_mappings = await self.blocking_mapper.get_strategies_for_blocking_type(
            blocking_type
        )
    ...
    async def _generate_behavior_strategies(
        self, pcap_analysis: PCAPAnalysisResult
    ) -> List[GeneratedStrategy]:
    ...
    async def _generate_signature_strategies(
        self, pcap_analysis: PCAPAnalysisResult
    ) -> List[GeneratedStrategy]:
        """Генерация стратегий на основе DPI сигнатур."""
        strategies = []
        ...
        for signature in pcap_analysis.dpi_signatures:
            signature_strategies = await self._generate_strategies_for_signature(
                signature, pcap_analysis
            )
            strategies.extend(signature_strategies)

        return strategies

    async def _generate_strategies_for_signature(
        self, signature: DPISignature, pcap_analysis: PCAPAnalysisResult
    ) -> List[GeneratedStrategy]:
        ...
        if signature.signature_type == "rst_pattern":
            ...
            timing_strategies = await self._generate_timing_bypass_strategies(
                signature, pcap_analysis
            )
            ...
            content_strategies = await self._generate_content_bypass_strategies(
                signature, pcap_analysis
            )
    ...
    async def _generate_rst_bypass_strategies(
        self, signature: DPISignature, pcap_analysis: PCAPAnalysisResult
    ) -> List[GeneratedStrategy]:
    ...
    async def _generate_timing_bypass_strategies(
        self, signature: DPISignature, pcap_analysis: PCAPAnalysisResult
    ) -> List[GeneratedStrategy]:
    ...
    async def _generate_content_bypass_strategies(
        self, signature: DPISignature, pcap_analysis: PCAPAnalysisResult
    ) -> List[GeneratedStrategy]:
    ...
    async def _generate_combo_strategies(
        self, pcap_analysis: PCAPAnalysisResult, base_strategies: List[GeneratedStrategy]
    ) -> List[GeneratedStrategy]:
        """Генерация комбинированных стратегий."""
        combo_strategies = []
        ...
        high_priority_strategies = [
            s for s in base_strategies if s.priority == StrategyPriority.HIGH
        ]
        ...
        for i, strategy1 in enumerate(high_priority_strategies):
            for strategy2 in high_priority_strategies[i + 1 :]:
                if self._can_combine_strategies(strategy1, strategy2):
                    combo_strategy = await self._create_combo_strategy(
                        strategy1, strategy2, pcap_analysis
                    )
                    if combo_strategy:
    ...
    async def _create_combo_strategy(
        self,
        strategy1: GeneratedStrategy,
        strategy2: GeneratedStrategy,
    ...
    async def _optimize_strategy_parameters(
        self, strategies: List[GeneratedStrategy], pcap_analysis: PCAPAnalysisResult
    ) -> List[GeneratedStrategy]:
        ...
        for strategy in strategies:
            optimized_strategy = await self.parameter_optimizer.optimize_parameters(
...
class BlockingTypeMapper:
    ...
    async def get_strategies_for_blocking_type(
            ...
            BlockingType.RST_INJECTION: [
                {
                    ...
                    "parameters": {
                        "ttl": {
                            "default_value": 1,
                            "confidence": 0.8,
                            "rationale": "Low TTL to expire before DPI",
                        },
                        "fooling": {
                            "default_value": "badseq",
                            "confidence": 0.7,
                            "rationale": "Bad sequence numbers",
                        },
                    },
                },
                {
                    ...
                    "parameters": {
                        "split_pos": {
                            "default_value": 3,
                            "confidence": 0.7,
                            "rationale": "Early split position",
                        }
                    },
                },
                {
                    ...
                    "parameters": {
                        "fooling": {
                            "default_value": "badsum",
                            "confidence": 0.8,
                            "rationale": "Invalid checksums",
                        },
                        "ttl": {
                            "default_value": 2,
                            "confidence": 0.6,
                            "rationale": "Short TTL for fake packets",
                        },
                    },
                },
            ],
            BlockingType.SNI_FILTERING: [
                {
                    ...
                    "parameters": {
                        "split_tls": {
                            "default_value": "sni",
                            "confidence": 0.9,
                            "rationale": "Split at SNI extension",
                        },
                        "split_count": {
                            "default_value": 5,
                            "confidence": 0.7,
                            "rationale": "Multiple fragments",
                        },
                    },
                },
                {
                    ...
                    "parameters": {
                        "fake_tls": {
                            "default_value": "0x160301",
                            "confidence": 0.8,
                            "rationale": "Fake TLS record",
                        },
                        "fooling": {
                            "default_value": "badsum",
                            "confidence": 0.7,
                            "rationale": "Invalid checksum",
                        },
                    },
                },
                {
                    ...
                    "parameters": {
                        "split_pos": {
                            "default_value": 10,
                            "confidence": 0.6,
                            "rationale": "Split within TLS header",
                        }
                    },
                },
            ],
            BlockingType.TLS_HANDSHAKE_BLOCKING: [
                {
                    ...
                    "parameters": {
                        "split_count": {
                            "default_value": 10,
                            "confidence": 0.7,
                            "rationale": "Heavy fragmentation",
                        },
                        "split_tls": {
                            "default_value": "chello",
                            "confidence": 0.8,
                            "rationale": "Split ClientHello",
                        },
                    },
                },
                {
                    ...
                    "parameters": {
                        "fake_tls": {
                            "default_value": "0x16030300",
                            "confidence": 0.7,
                            "rationale": "Fake handshake",
                        },
                        "fooling": {
                            "default_value": "badseq",
                            "confidence": 0.6,
                            "rationale": "Bad sequence",
                        },
                    },
                },
            ],
            BlockingType.FRAGMENTATION_REASSEMBLY: [
                {
                    ...
                    "parameters": {
                        "split_pos": {
                            "default_value": 2,
                            "confidence": 0.7,
                            "rationale": "Early reorder position",
                        }
                    },
                },
                {
            ...
            BlockingType.CONNECTION_TIMEOUT: [
                {
                    ...
                    "parameters": {
                        "fooling": {
                            "default_value": "badseq",
                            "confidence": 0.5,
                            "rationale": "Bad sequence for SYN",
                        }
                    },
                }
            ],
    ...
    async def get_strategies_for_dpi_behavior(
        self, dpi_behavior: DPIBehavior
    ) -> List[Dict[str, Any]]:
            ...
            DPIBehavior.ACTIVE_RST_INJECTION: [
                {
                    ...
                    "parameters": {
                        "ttl": {
                            "default_value": 1,
                            "confidence": 0.9,
                            "rationale": "Expire before RST source",
                        }
                    },
                }
            ],
            ...
            DPIBehavior.DEEP_PACKET_INSPECTION: [
                {
                    ...
                    "parameters": {
                        "split_count": {
                            "default_value": 20,
                            "confidence": 0.6,
                            "rationale": "Very heavy fragmentation",
                        }
                    },
                }
            ],
...
class ParameterOptimizer:
    ...
    async def optimize_parameters(
        self, strategy: GeneratedStrategy, pcap_analysis: PCAPAnalysisResult
    ) -> GeneratedStrategy:
    ...
    async def _optimize_ttl_parameter(
        self, strategy: GeneratedStrategy, pcap_analysis: PCAPAnalysisResult
    ) -> Optional[int]:
    ...
    async def _optimize_split_parameter(
        self, strategy: GeneratedStrategy, pcap_analysis: PCAPAnalysisResult
    ) -> Optional[int]:
        """Оптимизация split позиции."""
        # Анализ размеров пакетов для определения оптимальной позиции split
        if pcap_analysis.flows:
            avg_payload_size = sum(
                f.total_bytes / max(f.packet_count, 1) for f in pcap_analysis.flows
            ) / len(pcap_analysis.flows)
    ...
    async def _optimize_fooling_parameter(
        self, strategy: GeneratedStrategy, pcap_analysis: PCAPAnalysisResult
    ) -> Optional[str]:
...
class PriorityCalculator:
    ...
    async def calculate_priority(
        ...
        optimized_params = sum(
            1 for p in strategy.parameters.values() if p.source == "pcap_optimization"
        )
...
class StrategyValidator:
    ...
    async def validate_strategy(
        ...
        validation_result = {"is_valid": True, "score": 0.0, "details": {}, "issues": []}
    ...
    async def _validate_parameters(self, strategy: GeneratedStrategy) -> Dict[str, Any]:
        ...
        param_validation = {"score": 1.0, "valid_params": [], "invalid_params": [], "issues": []}
    ...
    async def _validate_blocking_match(
        self, strategy: GeneratedStrategy, pcap_analysis: PCAPAnalysisResult
    ) -> Dict[str, Any]:
        """Валидация соответствия стратегии типу блокировки."""

        blocking_validation = {"score": 0.5, "matches": [], "mismatches": []}
    ...
    async def _validate_logic(self, strategy: GeneratedStrategy) -> Dict[str, Any]:
        ...
        logic_validation = {"score": 1.0, "logical_issues": []}
...
async def generate_strategies_from_pcap(
    pcap_analysis: PCAPAnalysisResult, config: Optional[Dict[str, Any]] = None
) -> StrategyGenerationResult:
    """
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        for strategy in result.strategies:
            print(
                f"- {strategy.name} ({strategy.attack_name}) - "
