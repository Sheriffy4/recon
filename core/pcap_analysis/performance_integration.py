...
from .streaming_processor import StreamingPcapProcessor, StreamingConfig, AsyncStreamingProcessor
from .memory_optimizer import MemoryOptimizer, MemoryEfficientComparator, OptimizedPacketStorage
from .parallel_processor import ParallelPcapAnalyzer, ParallelConfig, AsyncParallelProcessor
...
class HighPerformancePcapAnalyzer:
    ...
    def _init_components(self):
        ...
        if self.config.enable_streaming:
            streaming_config = StreamingConfig(
                ...
                progress_callback=self._progress_callback if self.config.enable_progress_reporting else None
            )
    ...
    def analyze_single_pcap(self, pcap_file: str, 
        ...
        try:
            ...
            if self.cached_analyzer:
                ...
                result = self.cached_analyzer.cached_comparison(
                    recon_pcap, zapret_pcap, comparison_func,
    ...
    def batch_analyze_with_optimization(self, pcap_files: List[str], 
        ...
        """
        logger.info(f"Starting batch analysis of {len(pcap_files)} files (batch size: {batch_size})")
    ...
    if existing_pcaps:
        ...
        for pcap_file in existing_pcaps[:1]:  # Test with first file
            result = analyzer.analyze_single_pcap(
                pcap_file, 
                analysis_functions=[count_packets, analyze_protocols]
            )
