...
def demo_sequence_analysis():
    ...
    for i, packet in enumerate(packets):
        ...
        if analysis.is_fake:
            fake_count += 1
            print(f"   Packet {i}: FAKE (confidence: {analysis.confidence:.2f}) - {analysis.indicators}")
        else:
    ...
    if overlap_analysis.overlaps_detected:
        for i, overlap in enumerate(overlap_analysis.overlaps_detected):
            print(f"   Overlap {i+1}: {overlap['overlap_bytes']} bytes at packet {overlap['packet_index']}")
