    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "report_metadata": {
                ...
                "strategy_used": (
                    self.strategy_used.to_dict() if self.strategy_used else None
                ),
            },
            "executive_summary": (
            ...
            "analysis_results": {
                "comparison_summary": (
                    self.comparison_result.get_summary()
...
class AnalysisReporter:
    ...
    def generate_comprehensive_report(
        ...
        report = AnalysisReport(
            ...
            zapret_pcap=(
                comparison_result.zapret_file if comparison_result else "unknown"
            ),
        ...
        report.priority_matrix = self._create_priority_matrix(
    ...
    def _generate_executive_summary(
        ...
        similarity_score = (
            comparison_result.similarity_score if comparison_result else 0.0
        )
        critical_count = len(
...
"""
        ...
        if result.sequence_differences:
            ...
            for i, diff in enumerate(result.sequence_differences[:5], 1):
                ...
                content += (
                    f"- **Description**: {diff.get('description', 'No description')}\n"
                )
...
"""
        ...
        for category, diffs in diff_groups.items():
            critical_count = len(
                [d for d in diffs if d.impact_level == ImpactLevel.CRITICAL]
            )
...
"""
        ...
        section = ReportSection(
            ...
            data={
                ...
                "cause_types": list(
                    set(c.cause_type.value for c in report.root_causes)
                ),
            },
        )
...
"""
            ...
            if fix.test_cases:
                content += (
                    f"- **Test Cases**: {len(fix.test_cases)} test cases required\n"
                )
...
"""
        ...
        if successful_validations:
            avg_domains_tested = statistics.mean(
                [v.domains_tested for v in successful_validations]
            )
            avg_success_rate = statistics.mean(
                [v.success_rate for v in successful_validations]
            )
...
"""
    ...
    def _generate_visualizations(self, report: AnalysisReport):
        ...
        if report.critical_differences:
            ttl_diffs = [
                d
                for d in report.critical_differences
                if d.category == DifferenceCategory.TTL
            ]
            if ttl_diffs:
        ...
        if report.generated_fixes:
            visualizations["fix_priority_matrix"] = self._create_fix_priority_matrix(
                report.generated_fixes
            )
    ...
    def _create_ttl_pattern_viz(
        self, ttl_differences: List[CriticalDifference]
    ) -> Dict[str, Any]:
    ...
    def _calculate_success_probability(
        ...
        critical_penalty = (
            len([d for d in differences if d.impact_level == ImpactLevel.CRITICAL])
            * 0.1
        )
        high_penalty = (
            len([d for d in differences if d.impact_level == ImpactLevel.HIGH]) * 0.05
        )
    ...
    def _generate_immediate_actions(
        ...
        critical_diffs = [
            d for d in differences if d.impact_level == ImpactLevel.CRITICAL
        ]

        if critical_diffs:
            actions.append(
                f"Address {len(critical_diffs)} critical differences immediately"
            )

        # Actions based on root causes
        blocking_causes = [
            c for c in root_causes if c.blocking_severity in ["CRITICAL", "HIGH"]
        ]
        ...
        sequence_issues = [
            d for d in differences if d.category == DifferenceCategory.SEQUENCE
        ]
        if sequence_issues:
            actions.append("Review packet sequence generation logic")

        strategy_issues = [
            d for d in differences if d.category == DifferenceCategory.STRATEGY
        ]
        if strategy_issues:
