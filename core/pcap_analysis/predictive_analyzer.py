...
class PredictiveAnalyzer:
    ...
    def _combine_predictions(self, 
                           ...
                           risk_pred: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if success_rates and confidences:
                ...
                if total_weight > 0:
                    weighted_success = sum(rate * conf for rate, conf in zip(success_rates, confidences)) / total_weight
            ...
            combined["predicted_success_rate"] *= (1.0 - risk_factor * 0.3)  # Reduce by up to 30% based on risk
    ...
    def predict_parameter_optimization(self, 
        ...
        try:
            ...
            current_success_rate = current_prediction["overall_prediction"].get("predicted_success_rate", 0.0)
...
class EffectivenessModel:
    ...
    def predict(self, strategy_params: Dict[str, Any], historical_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _calculate_strategy_similarity(self, params1: Dict[str, Any], result: Dict[str, Any]) -> float:
        ...
        try:
            ...
            # TTL similarity
            if "ttl" in params1:
                if f"ttl={params1['ttl']}" in strategy_str or f"ttl={params1['ttl']})" in strategy_str:
                    similarity += 0.3
                factors += 1
            
            # Split position similarity
            if "split_pos" in params1:
                if f"split_pos={params1['split_pos']}" in strategy_str or f"split-pos={params1['split_pos']}" in strategy_str:
                    similarity += 0.3
                factors += 1
...
class RiskAssessmentModel:
    ...
    def assess_risk(self, strategy_params: Dict[str, Any], historical_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "ttl" in strategy_params:
                ...
                ttl_failures = sum(1 for r in failed_results if f"ttl={ttl}" in r.get("strategy", ""))
                ...
                if ttl_total > 0:
                    ...
                    if ttl_failure_rate > 0.7:
                        risk_factors.append(f"TTL={ttl} has high failure rate ({ttl_failure_rate:.1%})")
                        risk_score += 0.3
                    elif ttl_failure_rate > 0.4:
                        risk_factors.append(f"TTL={ttl} has moderate failure rate ({ttl_failure_rate:.1%})")
                        risk_score += 0.1
...
class OptimizationModel:
    ...
    def suggest_optimizations(self, strategy_params: Dict[str, Any], historical_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "split_pos" in strategy_params:
                    ...
                    if "split_pos=" in strategy_str or "split-pos=" in strategy_str:
                        try:
                            if "split_pos=" in strategy_str:
                                split_part = strategy_str.split("split_pos=")[1].split()[0].rstrip(',)')
                            else:
                                split_part = strategy_str.split("split-pos=")[1].split()[0].rstrip(',)')
                ...
                if split_success_rates:
                    best_split = max(split_success_rates.items(), key=lambda x: statistics.mean(x[1]))
