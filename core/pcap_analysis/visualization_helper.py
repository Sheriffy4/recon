...
class VisualizationHelper:
    ...
    def create_timing_difference_chart(
        ...
        avg_difference = statistics.mean(timing_data['differences']) if timing_data['differences'] else 0
    ...
    def create_fix_priority_matrix(
        self,
        fixes: List[CodeFix]
    ) -> VisualizationData:
        """
            ...
            'fixes': [
                {
                    ...
                    'priority_score': f.confidence * (1.0 - self._calculate_risk_score(f.risk_level))
                }
                for f in fixes
            ]
        }
    ...
    def create_difference_category_breakdown(
        ...
        for diff in differences:
            ...
            category_data[category]['by_impact'][impact] = category_data[category]['by_impact'].get(impact, 0) + 1
        ...
        breakdown_data = {
            ...
            'category_totals': [category_data[cat]['total'] for cat in category_data.keys()],
        ...
        config = {
            ...
            'statistics': {
                ...
                'most_common_category': max(category_data.keys(), key=lambda k: category_data[k]['total']),
    ...
    def create_checksum_analysis_chart(
        ...
        zapret_fake_invalid = len([p for p in zapret_packets if not p.checksum_valid and p.ttl <= 5])
