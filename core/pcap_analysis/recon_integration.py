...
try:
    from core.strategy import (
        StrategyRuleEngine, IntelligentStrategyGenerator, EnhancedRSTAnalyzer,
        RULE_ENGINE_AVAILABLE, INTELLIGENT_GENERATOR_AVAILABLE, ENHANCED_RST_AVAILABLE as STRATEGY_RST_AVAILABLE
    )
except ImportError:
...
class ReconIntegrationManager:
    ...
    def _load_historical_data(self) -> Dict[str, Any]:
        ...
        try:
            if os.path.exists(self.recon_summary_file):
                ...
                LOG.info(f"Loaded historical data: {len(historical_data['all_results'])} strategy results")
            else:
    ...
    def run_integrated_analysis(self, 
                               ...
                               include_strategy_generation: bool = True) -> Dict[str, Any]:
        """
        ...
        LOG.info(f"Integrated analysis completed in {results['analysis_metadata']['duration_seconds']:.2f}s")
        return results
    ...
    def _analyze_historical_context(self, comparison_result, target_domain: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if target_domain:
                context["domain_specific_data"] = self._analyze_domain_specific_history(target_domain)
    ...
    def _is_strategy_relevant(self, strategy: str, comparison_result) -> bool:
        ...
        if "ttl" in strategy.lower():
            ...
            ttl_differences = [d for d in comparison_result.sequence_differences 
        ...
        if "split" in strategy.lower():
            ...
            split_differences = [d for d in comparison_result.parameter_differences 
    ...
    def _extract_failure_factors(self, failed_strategies: List[Dict]) -> List[str]:
        ...
        no_sites_working = sum(1 for s in failed_strategies 
                              if s.get("result_status") == "NO_SITES_WORKING")
    ...
    def _generate_historical_recommendations(self, relevant_strategies: List[Dict], 
                                           effectiveness_patterns: Dict) -> List[str]:
    ...
    def _run_integrated_rst_analysis(self, pcap_file: str, target_domain: str = None) -> Dict[str, Any]:
    ...
    def _generate_integrated_strategies(self, comparison_result, historical_context: Dict, 
    ...
    def _generate_integration_insights(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pcap_strategies = results.get("strategy_recommendations", {}).get("pcap_based_strategies", [])
        history_strategies = results.get("strategy_recommendations", {}).get("history_based_strategies", [])
        combined_strategies = results.get("strategy_recommendations", {}).get("combined_strategies", [])
        ...
        try:
            ...
            integration_entry = {
                ...
                "results_summary": {
                    ...
                    "integration_components": analysis_results.get("analysis_metadata", {}).get("integration_components", []),
                    "actionable_fixes_count": len(analysis_results.get("actionable_fixes", [])),
                    "strategy_recommendations_count": len(analysis_results.get("strategy_recommendations", {}).get("combined_strategies", []))
                }
            }
            ...
            current_summary["metadata"]["integration_analysis_count"] = len(current_summary["integration_analysis"])
            
            # Save updated summary
            with open(self.recon_summary_file, 'w', encoding='utf-8') as f:
                json.dump(current_summary, f, indent=2, ensure_ascii=False)
            
            LOG.info(f"Updated recon_summary.json with integration analysis results")
            return True
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "historical_data_status": {
                ...
                "effectiveness_data_available": bool(self.historical_data.get("strategy_effectiveness"))
            },
...
def create_recon_integration_manager(recon_summary_file: str = "recon_summary.json",
                                    pcap_directory: str = ".",
                                    debug_mode: bool = False) -> ReconIntegrationManager:
    """
