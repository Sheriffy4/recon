...
class StreamingPcapProcessor:
                    ...
                    try:
                        if packet.haslayer('TCP') and packet.haslayer('IP'):
                            ...
                            packet_info = PacketInfo(
                                ...
                                payload_hex=bytes(tcp_layer.payload).hex() if tcp_layer.payload else "",
    ...
    def stream_packets(self, pcap_file: str, prefer_dpkt: bool = True) -> Iterator[PacketInfo]:
        ...
        try:
            ...
            logger.info(f"Completed streaming processing. Processed {self._processed_packets} packets")
            if self.config.enable_gc_optimization:
                gc.collect()
    ...
    def _is_tls_client_hello(self, payload: bytes) -> bool:
        ...
        try:
            return (payload[0] == 0x16 and  # TLS Handshake
...
if __name__ == "__main__":
    ...
    if Path(test_pcap).exists():
        ...
        end_time = time.time()
        print(f"Streaming processing completed: {packet_count} packets in {end_time - start_time:.2f}s")
    else:
