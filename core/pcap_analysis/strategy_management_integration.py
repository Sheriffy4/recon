...
class StrategyManagementIntegration:
    ...
    def _load_current_strategies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(self.recon_summary_file):
                ...
                strategies["strategy_effectiveness"] = summary_data.get("strategy_effectiveness", {})
    ...
    def integrate_pcap_strategies(self, 
        ...
        try:
            ...
            unified_strategies = self._unify_strategies(pcap_strategies, management_strategies, target_domain)
    ...
    def _extract_pcap_strategies(self, pcap_analysis_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for strategy_type in ["pcap_based_strategies", "history_based_strategies", "combined_strategies"]:
                ...
                for strategy in strategies:
                    pcap_strategies.append({
                        ...
                        "pcap_evidence": self._extract_pcap_evidence(strategy, pcap_analysis_results)
                    })
            ...
            for fix in actionable_fixes:
                if fix.get("type") == "strategy_implementation":
                    pcap_strategies.append({
                        "strategy_command": fix.get("description", "").replace("Apply strategy: ", ""),
    ...
    def _extract_pcap_evidence(self, strategy: Dict[str, Any], pcap_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_intelligent_strategies(self, target_domain: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            strategies.append({
                ...
                "intelligence_factors": ["historical_data", "pattern_recognition", "effectiveness_prediction"]
            })
        ...
        try:
            ...
            for strategy_config in active_strategies:
                ...
                strategies.append({
                    ...
                    "config_metadata": strategy_config if isinstance(strategy_config, dict) else {}
                })
    ...
    def _group_similar_strategies(self, strategies: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _create_unified_strategy(self, 
                               group_strategies: List[Dict[str, Any]],
                               target_domain: str = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if total_weight > 0:
                ...
                final_confidence = sum(s.get("confidence", 0.0) for s in group_strategies) / len(group_strategies)
    ...
    def _update_strategy_system(self, 
    ...
    def _update_recon_summary_integration(self, 
                                        unified_strategies: List[Dict[str, Any]],
                                        pcap_analysis_results: Dict[str, Any]):
        ...
        try:
            ...
            integration_entry = {
                ...
                "pcap_analysis_summary": {
                    "similarity_score": pcap_analysis_results.get("pcap_comparison", {}).get("similarity_score", 0.0),
                    "critical_issues_count": len(pcap_analysis_results.get("actionable_fixes", [])),
                    "recommendations_count": len(pcap_analysis_results.get("strategy_recommendations", {}).get("combined_strategies", []))
                }
            }
            ...
            current_summary["metadata"]["strategy_integration_count"] = len(current_summary["strategy_integration"])
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "configuration_status": {
                ...
                "active_strategies_count": len(self.current_strategies.get("active_strategies", []))
            },
