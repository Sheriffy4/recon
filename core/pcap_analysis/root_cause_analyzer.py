...
class RootCauseAnalyzer:
    ...
    def analyze_failure_causes(self, differences: List[CriticalDifference], 
                             ...
                             anomalies: List[Anomaly] = None) -> List[RootCause]:
    ...
    def correlate_with_historical_data(self, causes: List[RootCause], 
                                     summary_data: Dict[str, Any]) -> List[CorrelatedCause]:
    ...
    def validate_hypotheses(self, hypotheses: List[Hypothesis],
                          recon_packets: List[PacketInfo] = None,
                          zapret_packets: List[PacketInfo] = None) -> List[ValidatedHypothesis]:
    ...
    def _analyze_difference_causes(self, differences: List[CriticalDifference]) -> List[RootCause]:
    ...
    def _create_cause_from_difference(self, diff: CriticalDifference) -> Optional[RootCause]:
    ...
    def _analyze_pattern_causes(self, patterns: List[EvasionPattern], 
                              anomalies: List[Anomaly]) -> List[RootCause]:
    ...
    def _create_cause_from_missing_technique(self, technique: EvasionTechnique) -> RootCause:
        ...
        cause_type = technique_mapping.get(technique, RootCauseType.STRATEGY_PARAMETER_MISMATCH)
    ...
    def _analyze_missing_patterns(self, patterns: List[EvasionPattern]) -> List[RootCause]:
        """Analyze causes from missing expected patterns."""
        causes = []
        
        # Check for expected fakeddisorder pattern
        has_fake_injection = any(p.technique == EvasionTechnique.FAKE_PACKET_INJECTION 
                               for p in patterns if p.confidence >= 0.7)
        has_payload_splitting = any(p.technique == EvasionTechnique.PAYLOAD_SPLITTING 
                                  for p in patterns if p.confidence >= 0.7)
    ...
    def _correlate_single_cause(self, cause: RootCause, 
                              summary_data: Dict[str, Any]) -> CorrelatedCause:
        ...
        correlation.pattern_frequency = len(correlation.historical_matches) / max(1, total_strategies)
    ...
    def _find_historical_matches(self, cause: RootCause, 
                               failing_strategies: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _strategy_matches_cause(self, strategy: Dict[str, Any], cause: RootCause) -> bool:
        ...
        if cause.cause_type == RootCauseType.MISSING_FAKE_PACKETS:
            ...
            return 'fooling=' in strategy_str or 'badsum' in strategy_str or 'badseq' in strategy_str
        
        elif cause.cause_type == RootCauseType.ENGINE_TELEMETRY_ANOMALY:
            return (telemetry.get('segments_sent', 0) == 0 and 
                   telemetry.get('fake_packets_sent', 0) == 0)
        
        return False
    
    def _calculate_correlation_strength(self, cause: RootCause, 
    ...
    def _calculate_success_rate_impact(self, cause: RootCause, 
                                     summary_data: Dict[str, Any]) -> float:
    ...
    def _group_causes_by_type(self, causes: List[RootCause]) -> Dict[RootCauseType, List[RootCause]]:
    ...
    def _generate_hypothesis_for_group(self, cause_type: RootCauseType, 
                                     causes: List[RootCause]) -> Optional[Hypothesis]:
    ...
    def _generate_combined_hypotheses(self, causes: List[RootCause]) -> List[Hypothesis]:
        """Generate hypotheses that combine multiple related causes."""
        combined_hypotheses = []
        
        # Look for fakeddisorder-specific combination
        fake_packet_causes = [c for c in causes if c.cause_type == RootCauseType.MISSING_FAKE_PACKETS]
    ...
    def _validate_single_hypothesis(self, hypothesis: Hypothesis,
                                   recon_packets: List[PacketInfo] = None,
                                   zapret_packets: List[PacketInfo] = None) -> ValidatedHypothesis:
        """Validate a single hypothesis against available evidence."""
        validation = ValidatedHypothesis(
            hypothesis=hypothesis,
        ...
        total_evidence = len(validation.supporting_evidence) + len(validation.contradicting_evidence)
        if total_evidence > 0:
    ...
    def _validate_against_pcap(self, hypothesis: Hypothesis,
                             recon_packets: List[PacketInfo],
                             zapret_packets: List[PacketInfo]) -> Dict[str, List[Evidence]]:
    ...
    def _validate_against_historical_data(self, hypothesis: Hypothesis) -> Dict[str, List[Evidence]]:
        ...
        failing_strategies = self._historical_data.get('strategy_effectiveness', {}).get('top_failing', [])
        ...
        for cause in causes:
            ...
            if cause.cause_type not in seen_types:
                ...
                if existing.evidence:
                    existing.confidence = statistics.mean([e.confidence for e in existing.evidence])
    ...
    def _determine_affected_components(self, cause_type: RootCauseType) -> List[str]:
        ...
        component_mapping = {
            RootCauseType.MISSING_FAKE_PACKETS: ["fake_packet_generator", "attack_engine", "packet_sender"],
            RootCauseType.INCORRECT_TTL: ["packet_builder", "fake_packet_generator"],
            RootCauseType.WRONG_SPLIT_POSITION: ["payload_splitter", "segment_generator"],
            RootCauseType.MISSING_FOOLING_METHOD: ["checksum_corruptor", "sequence_manipulator"],
            RootCauseType.SEQUENCE_OVERLAP_ERROR: ["sequence_calculator", "segment_generator"],
            RootCauseType.TIMING_ISSUES: ["packet_scheduler", "timing_controller"],
            RootCauseType.CHECKSUM_VALIDATION_ERROR: ["checksum_calculator", "packet_validator"],
            RootCauseType.PACKET_ORDER_ERROR: ["packet_scheduler", "sequence_manager"],
            RootCauseType.STRATEGY_PARAMETER_MISMATCH: ["strategy_parser", "parameter_validator"],
            RootCauseType.ENGINE_TELEMETRY_ANOMALY: ["telemetry_collector", "metrics_reporter"]
        }
    ...
    def _generate_fixes_for_cause_type(self, cause_type: RootCauseType, 
                                     diff: CriticalDifference) -> List[str]:
