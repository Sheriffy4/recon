...
class SystemValidator:
    ...
    async def _validate_domain_strategy(self, domain: str, strategy: StrategyConfig) -> DomainValidationResult:
    ...
    def _generate_recommendations(self, domain_results: List[DomainValidationResult], 
        ...
        worst_strategy = min(strategy_success.items(), 
                           key=lambda x: x[1]["success"] / x[1]["total"])
        ...
        if performance["average_domain_time"] > 10:
            recommendations.append("Consider optimizing domain validation time (currently > 10s per domain)")
        ...
        if x_com_results:
            x_com_success_rate = sum(1 for r in x_com_results if r.success) / len(x_com_results) * 100
    ...
    async def _save_report(self, report: SystemValidationReport):
        ...
        with open(text_file, 'w') as f:
            ...
            for result in report.domain_results:
                status = "✓" if result.success else "✗"
                f.write(f"{status} {result.domain} ({result.strategy_name}): {result.response_time:.2f}s")
    ...
    async def validate_x_com_specifically(self) -> Dict[str, any]:
        ...
        return {
            ...
            "pcap_comparison": {
                ...
                "similarity_score": getattr(pcap_comparison_result, 'similarity_score', 0) if pcap_comparison_result else 0,
                "packet_differences": len(getattr(pcap_comparison_result, 'sequence_differences', [])) if pcap_comparison_result else 0
            },
            "strategy_analysis": {
