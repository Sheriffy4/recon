...
from .error_handling import PCAPParsingError, AnalysisError, ErrorCategory, ErrorSeverity, PartialResult
...
class GracefulPCAPParser:
    ...
    def parse_with_degradation(
        ...
        try:
            ...
            if not file_info.corruption_detected:
                try:
                    ...
                    return PartialResult(
                        ...
                        data=packets,
                        completeness=1.0,
                        metadata={"parsing_method": "normal", "file_info": file_info.__dict__}
                    )
                except Exception as e:
            ...
            return PartialResult(
                ...
                data=None,
                errors=[PCAPParsingError(
                    f"All parsing strategies failed for {filepath}",
                    filepath
                )],
                completeness=0.0,
        ...
        try:
            ...
            with PcapReader(filepath) as pcap_reader:
                for packet in pcap_reader:
                    ...
                    try:
                        if TCP in packet:
                            packet_info = self._extract_packet_info(packet, total_count - 1)
                            if packet_info:
                                packet_infos.append(packet_info)
                    except Exception as e:
                        skipped_count += 1
    ...
    def _raw_packet_extraction(self, filepath: str, file_info: PCAPFileInfo) -> PartialResult:
        ...
        try:
            ...
            return PartialResult(
                ...
                completeness=0.4,
                metadata={"parsing_method": "raw_extraction", "packets_extracted": len(packet_infos)}
            )
    ...
    def _extract_basic_info_raw(self, packet_data: bytes, timestamp: float, index: int) -> Optional[PacketInfo]:
        ...
        try:
            ...
            version_ihl, tos, total_len, id_field, flags_frag, ttl, protocol, checksum, src_ip, dst_ip = struct.unpack(
            ...
            src_port, dst_port, seq_num, ack_num, offset_flags, window, tcp_checksum, urgent = struct.unpack(
                '!HHIIHHH', tcp_header
            )
    ...
    def get_parsing_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (self.parsing_stats["successful_parses"] + self.parsing_stats["partial_parses"]) / total,
