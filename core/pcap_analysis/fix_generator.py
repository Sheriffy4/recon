...
from .root_cause_analyzer import RootCause, RootCauseType, Evidence
...
from .packet_sequence_analyzer import FakePacketAnalysis, SplitPositionAnalysis
...
class FixGenerator:
    ...
    def create_strategy_patches(self, strategy_differences: List[StrategyDifference]) -> List[StrategyPatch]:
        """
    ...
    def generate_packet_sequence_fixes(self, sequence_analysis: FakePacketAnalysis) -> List[SequenceFix]:
        """
...
def test_sequence_fix_regression():
    ...
    def export_fixes(self, output_file: str) -> None:
        ...
        export_data = {
            ...
            "summary": {
                ...
                "high_confidence_fixes": len([f for f in self.generated_fixes if f.confidence >= 0.8])
            }
        }
    ...
    def get_fix_summary(self) -> Dict[str, Any]:
        ...
        return {
            "total_fixes": len(self.generated_fixes),
            "fix_types": {fix_type.value: len([f for f in self.generated_fixes if f.fix_type == fix_type]) 
                         for fix_type in FixType},
            "risk_levels": {risk.value: len([f for f in self.generated_fixes if f.risk_level == risk]) 
                           for risk in RiskLevel},
            "high_confidence_fixes": [f.fix_id for f in self.generated_fixes if f.confidence >= 0.8],
            "critical_fixes": [f.fix_id for f in self.generated_fixes if f.risk_level == RiskLevel.CRITICAL]
        }