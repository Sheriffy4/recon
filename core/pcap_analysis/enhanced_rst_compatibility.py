...
class EnhancedRSTCompatibilityLayer:
    ...
    def __init__(
        ...
        self.integration_manager = integration_manager or ReconIntegrationManager(
            recon_summary_file
        )
        ...
        if ENHANCED_RST_FINDER_AVAILABLE:
            try:
                ...
                self.enhanced_rst_finder = EnhancedRSTTriggerFinder(
                    pcap_file="dummy.pcap", recon_summary_file=recon_summary_file
                )
    ...
    def run_enhanced_pcap_analysis(
        self,
        pcap_file: str,
        recon_pcap: str = None,
    ...
    def _integrate_analysis_results(
        ...
        pcap_comparison_results: Dict[str, Any],
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            cross_validated_strategies = self._cross_validate_strategies(
    ...
    def _cross_validate_strategies(
        self, rst_strategies: List[Dict], pcap_strategies: List[Dict]
    ) -> List[Dict]:
        ...
        for rst_strategy in rst_strategies:
            ...
            for pcap_strategy in pcap_strategies:
                ...
                if self._strategies_similar(rst_cmd, pcap_cmd):
                    ...
                    combined_confidence = (
                        rst_strategy["confidence"] + pcap_strategy["confidence"]
                    ) / 2
                    ...
                    cross_validated.append(
                        {
                            ...
                            "reasoning": "Validated by both RST analysis and PCAP comparison",
        ...
        for rst_strategy in rst_strategies:
            if not any(
                self._strategies_similar(rst_strategy["strategy"], cv["strategy"])
        ...
        for pcap_strategy in pcap_strategies:
            if not any(
                self._strategies_similar(pcap_strategy["strategy"], cv["strategy"])
                for cv in cross_validated
            ):
                cross_validated.append(
                    {
    ...
    def _calculate_integrated_confidence(
        ...
        if pcap_strategies:
            scores["pcap_analysis_confidence"] = sum(
                s["confidence"] for s in pcap_strategies
            ) / len(pcap_strategies)

        # Cross-validation confidence
        cross_validated_high = [
            s for s in cross_validated if s.get("validation_strength") == "HIGH"
        ]
        if cross_validated_high:
            scores["cross_validation_confidence"] = sum(
                s["confidence"] for s in cross_validated_high
            ) / len(cross_validated_high)

        # Overall confidence (weighted average)
        weights = {"rst": 0.3, "pcap": 0.3, "cross_validation": 0.4}
    ...
    def _generate_compatibility_report(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report["performance_metrics"] = {
            ...
            "cross_validation_rate": len(
                [
                    s
                    for s in results.get("integrated_results", {}).get(
                        "strategy_recommendations", []
                    )
                    if s.get("validation_strength") == "HIGH"
                ]
            )
            / max(
                1,
...
def create_enhanced_rst_compatibility_layer(
    ...
    return EnhancedRSTCompatibilityLayer(
        integration_manager=integration_manager, recon_summary_file=recon_summary_file
    )
