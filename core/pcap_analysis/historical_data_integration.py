...
from .learning_engine import LearningEngine, PatternDatabase
...
class HistoricalDataIntegration:
    ...
    def __init__(self, 
        ...
        LOG.info(f"Historical Data Integration initialized with {len(self.historical_data.get('all_results', []))} historical records")
    ...
    def _load_historical_data(self):
        ...
        try:
            if os.path.exists(self.recon_summary_file):
                ...
                self.historical_data["strategy_effectiveness"] = data.get("strategy_effectiveness", {})
            ...
            for strategy, results in strategy_results.items():
                ...
                if success_rates:
                    self.effectiveness_trends["strategy_performance"][strategy] = {
                        ...
                        "std_deviation": statistics.stdev(success_rates) if len(success_rates) > 1 else 0.0,
                        "total_attempts": len(results),
                        "successful_attempts": len([r for r in results if r.get("success_rate", 0.0) > 0])
                    }
    ...
    def _analyze_parameter_effectiveness(self, all_results: List[Dict[str, Any]]):
        ...
        for result in all_results:
            ...
            if "fooling=" in strategy:
                try:
                    ...
                    fooling_methods = fooling_part.strip('[]').replace("'", "").split(',')
            ...
            if "fake" in strategy.lower():
                if "disorder" in strategy.lower():
                    parameter_analysis["strategy_type_effectiveness"]["fake_disorder"].append(success_rate)
                else:
        ...
        for param_type, param_data in parameter_analysis.items():
            ...
            for param_value, success_rates in param_data.items():
                if success_rates:
                    self.effectiveness_trends["parameter_effectiveness"][param_type][param_value] = {
                        "average_success_rate": statistics.mean(success_rates),
                        "max_success_rate": max(success_rates),
    ...
    def _analyze_success_patterns(self):
        ...
        try:
            ...
            successful_strategies = [(r.get("strategy", ""), r.get("success_rate", 0.0)) 
    ...
    def _analyze_success_parameters(self, successful_results: List[Dict[str, Any]]):
        ...
        for result in successful_results:
            ...
            if "fake" in strategy.lower():
                if "disorder" in strategy.lower():
                    parameter_successes["strategy_type_successes"]["fake_disorder"].append(success_rate)
                else:
    ...
    def _generate_success_factors(self, successful_results: List[Dict[str, Any]]):
        ...
        if ttl_successes:
            ...
            success_factors.append(f"TTL={best_ttl[0]} shows highest success rate ({best_ttl[1]['average_success_rate']:.1%})")
        ...
        if split_successes:
            ...
            success_factors.append(f"split_pos={best_split[0]} shows highest success rate ({best_split[1]['average_success_rate']:.1%})")
        
        # Fooling method success factors
        fooling_successes = success_params.get("fooling_successes", {})
        if fooling_successes:
            best_fooling = max(fooling_successes.items(), key=lambda x: x[1]["average_success_rate"])
            success_factors.append(f"fooling={best_fooling[0]} shows highest success rate ({best_fooling[1]['average_success_rate']:.1%})")
        ...
        if type_successes:
            ...
            success_factors.append(f"{best_type[0]} strategy type shows highest success rate ({best_type[1]['average_success_rate']:.1%})")
        
        self.success_patterns["success_factors"] = success_factors
    
    def get_historical_context_for_pcap_analysis(self, 
        ...
        try:
            ...
            context["relevant_historical_strategies"] = self._find_relevant_strategies(pcap_analysis_results)
            
            # Generate parameter recommendations based on history
            context["parameter_recommendations"] = self._generate_parameter_recommendations(pcap_analysis_results)
            ...
            context["success_predictions"] = self._predict_success_from_history(pcap_analysis_results)
            
            # Generate historical insights
            context["historical_insights"] = self._generate_historical_insights(pcap_analysis_results)
    ...
    def _find_relevant_strategies(self, pcap_analysis_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            ttl_issues = any("ttl" in str(issue).lower() for issue in pcap_comparison.get("critical_issues", []))
            
            # Look for split-related issues
            split_issues = any("split" in str(issue).lower() for issue in pcap_comparison.get("parameter_differences", []))
            
            # Look for fake packet issues
            fake_issues = any("fake" in str(issue).lower() for issue in pcap_comparison.get("sequence_differences", []))
            ...
            for result in all_results:
                ...
                if strategy_params:
                    ...
                    if "split_pos" in strategy_params and f"split_pos={strategy_params['split_pos']}" in strategy:
                        relevance_score += 0.4
            ...
            relevant_strategies.sort(key=lambda x: (x["relevance_score"], x["success_rate"]), reverse=True)
    ...
    def _generate_parameter_recommendations(self, pcap_analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            ttl_successes = success_params.get("ttl_successes", {})
            if ttl_successes:
                best_ttl_item = max(ttl_successes.items(), key=lambda x: x[1]["average_success_rate"])
            ...
            split_successes = success_params.get("split_pos_successes", {})
            if split_successes:
                best_split_item = max(split_successes.items(), key=lambda x: x[1]["average_success_rate"])
            ...
            fooling_successes = success_params.get("fooling_successes", {})
            if fooling_successes:
                best_fooling_item = max(fooling_successes.items(), key=lambda x: x[1]["average_success_rate"])
            ...
            type_successes = success_params.get("strategy_type_successes", {})
            if type_successes:
                best_type_item = max(type_successes.items(), key=lambda x: x[1]["average_success_rate"])
        ...
        try:
            ...
            for ttl_value, failure_count in ttl_failures.items():
                if failure_count > 2:  # Threshold for warning
                    warnings.append(f"Warning: TTL={ttl_value} has failed {failure_count} times historically")
            ...
            for split_pos, failure_count in split_failures.items():
                if failure_count > 2:
                    warnings.append(f"Warning: split_pos={split_pos} has failed {failure_count} times historically")
            ...
            for fooling_method, failure_count in fooling_failures.items():
                if failure_count > 2:
                    warnings.append(f"Warning: fooling={fooling_method} has failed {failure_count} times historically")
    ...
    def _predict_success_from_history(self, pcap_analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if all_results:
                ...
                if successful_results:
                    success_rates = [r.get("success_rate", 0.0) for r in successful_results]
                    predictions["overall_success_probability"] = statistics.mean(success_rates)
                    predictions["confidence_level"] = "MEDIUM" if len(successful_results) > 2 else "LOW"
                    ...
                    for param, rates in param_success.items():
                        predictions["parameter_success_probabilities"][f"ttl_{param}"] = statistics.mean(rates)
                
                predictions["overall_success_probability"] = len(successful_results) / len(all_results)
            ...
            for param_type, param_data in success_params.items():
                ...
                for param_value, stats in param_data.items():
                    success_prob = stats["successful_attempts"] / stats["attempts"] if stats["attempts"] > 0 else 0.0
                    predictions["parameter_success_probabilities"][param_type][param_value] = success_prob
    ...
    def learn_from_successful_fix(self, 
    ...
    def optimize_strategy_parameters(self, 
                                   ...
                                   target_success_rate: float = 0.8) -> Dict[str, Any]:
        """
    ...
    def _update_historical_data_with_learning(self, 
                                            fix_data: Dict[str, Any], 
    ...
    def _save_learning_data_to_summary(self, learning_entry: Dict[str, Any]):
        ...
        try:
            ...
            summary_data["metadata"]["total_learning_entries"] = len(summary_data["learning_history"])
    ...
    def export_learning_knowledge(self, export_file: str = "historical_learning_export.json") -> bool:
        """
        ...
        try:
            ...
            LOG.info(f"Historical learning knowledge exported successfully")
    ...
    def import_learning_knowledge(self, import_file: str) -> bool:
        ...
        try:
            ...
            LOG.info(f"Historical learning knowledge imported successfully")
    ...
    def _generate_historical_insights(self, pcap_analysis_results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if strategy_performance:
                ...
                insights.append(f"Best Historical Strategy: {best_strategy[0]} "
            ...
            if common_failures:
                ...
                insights.append(f"Most Failed Strategy: {most_failed['strategy']} "
                              f"({most_failed['failure_count']} failures)")
            
            # Data quality insights
            total_results = len(self.historical_data.get("all_results", []))
            successful_results = len([r for r in self.historical_data.get("all_results", []) 
                                    if r.get("success_rate", 0.0) > 0])
            
            if total_results > 0:
                success_rate = successful_results / total_results
                insights.append(f"Historical Data: {total_results} total attempts, "
    ...
    def update_historical_data(self, new_analysis_results: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            integration_entry = {
                ...
                "results_summary": {
                    "similarity_score": new_analysis_results.get("pcap_comparison", {}).get("similarity_score", 0.0),
                    "critical_issues_count": len(new_analysis_results.get("actionable_fixes", [])),
                    "historical_strategies_found": len(new_analysis_results.get("historical_context", {}).get("relevant_historical_strategies", [])),
                    "success_predictions": new_analysis_results.get("historical_context", {}).get("success_predictions", {})
                },
                "historical_insights": new_analysis_results.get("historical_context", {}).get("historical_insights", [])
            }
            ...
            current_data["metadata"]["pcap_integration_count"] = len(current_data["pcap_integration_history"])
    ...
    def get_historical_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "data_statistics": {
                ...
                "successful_strategies": len([r for r in self.historical_data.get("all_results", []) 
                                            if r.get("success_rate", 0.0) > 0]),
                "unique_strategies": len(set(r.get("strategy", "") for r in self.historical_data.get("all_results", []))),
            ...
            "analysis_capabilities": {
                ...
                "parameter_analysis": bool(self.effectiveness_trends.get("parameter_effectiveness"))
            },
            "insights_available": {
                ...
                "parameter_recommendations": len(self.effectiveness_trends.get("parameter_effectiveness", {}))
            }
        }
...
def create_historical_data_integration(
    ...
    return HistoricalDataIntegration(
        ...
        backup_history_days=backup_history_days
    )