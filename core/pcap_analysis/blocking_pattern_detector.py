...
try:
    from scapy.all import (
        rdpcap, TCP, IP, IPv6, Raw, TLS, UDP, DNS, ICMP,
...
try:
    from .intelligent_pcap_analyzer import (
        BlockingType, DPIBehavior, PacketAnalysis, FlowAnalysis
    )
...
class BlockingPatternDetector:
    ...
    async def analyze_blocking_patterns(self, 
                                      domain: str,
                                      pcap_file: str,
                                      flow_analyses: Optional[List[FlowAnalysis]] = None) -> BlockingPatternAnalysis:
        """
        ...
        # Проверяем кэш
        cache_key = f"{domain}_{Path(pcap_file).stat().st_mtime}"
        if cache_key in self.analysis_cache:
            LOG.debug(f"Using cached analysis for {domain}")
        ...
        try:
            ...
            # Анализ RST инъекций
            analysis.rst_analysis = await self._analyze_rst_injections(
                domain, flows, packets
            )
            
            # Анализ TLS handshake
            analysis.tls_analysis = await self._analyze_tls_handshake(
                domain, flows, packets
            )
            
            # Анализ DNS манипуляций
            analysis.dns_analysis = await self._analyze_dns_manipulations(
                domain, flows, packets
            )
            
            # Анализ HTTP редиректов
            analysis.http_analysis = await self._analyze_http_redirects(
                domain, flows, packets
            )
            ...
            analysis.evasion_recommendations = await self._generate_evasion_recommendations(analysis)
            ...
            LOG.info(f"Blocking pattern analysis completed for {domain} in {analysis.analysis_duration_ms:.1f}ms")
    ...
    async def detect_rst_injection_patterns(self, 
                                          domain: str,
                                          packets: List[Any]) -> RSTInjectionAnalysis:
        """
        ...
        analysis = RSTInjectionAnalysis(
            detected=False,
            injection_type=RST_InjectionType.IMMEDIATE,
            timing_ms=0.0
        )
    ...
    async def analyze_tls_handshake_interruption(self, 
                                               domain: str,
                                               packets: List[Any]) -> TLSHandshakeAnalysis:
        """
        ...
        try:
                            ...
                            if hasattr(msg, 'ext') and msg.ext:
                                for ext in msg.ext:
                                    if hasattr(ext, 'servernames') and ext.servernames:
                                        analysis.client_hello_sni = ext.servernames[0].servername.decode()
            ...
            if completed_stages:
                analysis.last_successful_stage = max(completed_stages, key=lambda s: handshake_stages[s])
            ...
            if not analysis.completed:
                ...
                if analysis.failure_stage == TLSHandshakeStage.SERVER_HELLO:
                    if analysis.client_hello_sni and domain.lower() in analysis.client_hello_sni.lower():
                ...
                rst_after_client_hello = any(
                    TCP in p and p[TCP].flags.R and float(p.time) > handshake_stages.get(TLSHandshakeStage.CLIENT_HELLO, 0)
                    for p in packets
                )
    ...
    async def detect_dns_manipulations(self, 
                                     domain: str,
                                     packets: List[Any]) -> DNSManipulationAnalysis:
        """
        Детектор DNS манипуляций и подмены ответов.
        ...
        try:
            ...
            for query in dns_queries:
                ...
                matching_responses = [
                    r for r in dns_responses 
                    if r[DNS].id == query_id and float(r.time) > query_time
                ]
    ...
    async def analyze_http_redirects(self, 
                                   domain: str,
                                   packets: List[Any]) -> HTTPRedirectAnalysis:
        """
        Анализ HTTP/HTTPS редиректов и блокировок по содержимому.
        ...
        try:
            ...
            for packet in http_packets:
                ...
                # Поиск редиректов
                if 'HTTP/1.' in payload and ('301' in payload or '302' in payload or '307' in payload):
                    analysis.detected = True
                ...
                block_indicators = [
                    'access denied', 'blocked', 'restricted', 'forbidden',
                    'заблокирован', 'доступ запрещен', 'ограничен доступ',
                    'roskomnadzor', 'rkn.gov.ru', 'blocked by', 'access restricted'
                ]
                ...
                for indicator in block_indicators:
                    if indicator in payload_lower:
                        ...
                        elif any(isp in payload_lower for isp in ['beeline', 'mts', 'megafon', 'rostelecom']):
                ...
                if '<script' in payload_lower and ('location.href' in payload_lower or 'window.location' in payload_lower):
                    analysis.detected = True
                    analysis.redirect_type = "javascript"
                
                # Поиск meta refresh редиректов
                meta_refresh_match = re.search(r'<meta[^>]*http-equiv=["\']refresh["\'][^>]*content=["\']([^"\']+)["\']', payload, re.IGNORECASE)
    ...
    def classify_dpi_aggressiveness(self, analysis: BlockingPatternAnalysis) -> DPIAggressivenessLevel:
        """
    ...
    def _load_blocking_patterns(self):
        ...
        self.blocking_patterns = {
            "government_block_ips": [
                "127.0.0.1", "0.0.0.0", "10.0.0.1", "192.168.1.1"
            ],
            "common_block_pages": [
                "access denied", "blocked", "restricted", "forbidden"
            ],
            "suspicious_ttl_ranges": [
                (1, 32),   # Очень низкий TTL
                (250, 255) # Очень высокий TTL
            ]
        }
    ...
    async def _analyze_rst_injections(self, domain: str, flows: Dict[str, List[Any]], packets: List[Any]) -> RSTInjectionAnalysis:
        """Анализ RST инъекций."""
        return await self.detect_rst_injection_patterns(domain, packets)
    
    async def _analyze_tls_handshake(self, domain: str, flows: Dict[str, List[Any]], packets: List[Any]) -> TLSHandshakeAnalysis:
        """Анализ TLS handshake."""
        return await self.analyze_tls_handshake_interruption(domain, packets)
    
    async def _analyze_dns_manipulations(self, domain: str, flows: Dict[str, List[Any]], packets: List[Any]) -> DNSManipulationAnalysis:
        """Анализ DNS манипуляций."""
        return await self.detect_dns_manipulations(domain, packets)
    
    async def _analyze_http_redirects(self, domain: str, flows: Dict[str, List[Any]], packets: List[Any]) -> HTTPRedirectAnalysis:
        """Анализ HTTP редиректов."""
    ...
    def _assess_dpi_aggressiveness(self, analysis: BlockingPatternAnalysis) -> DPIAggressivenessLevel:
        """Оценка агрессивности DPI."""
    ...
    async def _generate_evasion_recommendations(self, analysis: BlockingPatternAnalysis) -> List[str]:
        """Генерация рекомендаций по обходу."""
        recommendations = []
        ...
        if analysis.dpi_aggressiveness in [DPIAggressivenessLevel.AGGRESSIVE, DPIAggressivenessLevel.EXTREME]:
            recommendations.extend([
    ...
    def _create_dpi_signature(self, analysis: BlockingPatternAnalysis) -> Dict[str, Any]:
        ...
        if analysis.tls_analysis:
            signature["characteristics"]["tls_behavior"] = {
                ...
                "failure_stage": analysis.tls_analysis.failure_stage.value if analysis.tls_analysis.failure_stage else None
            }
    ...
    def _classify_rst_injection_type(self, rst_packets: List[Any], all_packets: List[Any]) -> RST_InjectionType:
