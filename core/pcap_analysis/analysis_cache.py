...
class MemoryCache:
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        with self._lock:
            ...
            if (metadata.ttl_seconds is not None and 
                current_time - metadata.created_at > metadata.ttl_seconds):
                # Entry expired
    ...
    def clear(self):
        ...
        with self._lock:
            ...
            self._stats = CacheStats(0, 0, self._stats.hit_count, self._stats.miss_count, 
                                   self._stats.hit_rate, self._stats.eviction_count)
            ...
            try:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """, (key, value_hash, data_blob, current_time, current_time,
                         1, size_bytes, ttl_seconds))
...
class CachedAnalyzer:
    ...
    def cached_pcap_analysis(self, pcap_file: str, analysis_func: Callable, 
                           ttl_seconds: Optional[float] = 3600) -> Any:
    ...
    def cached_packet_analysis(self, packets: List[PacketInfo], analysis_func: Callable,
                             analysis_type: str, ttl_seconds: Optional[float] = 1800) -> Any:
    ...
    def cached_comparison(self, recon_pcap: str, zapret_pcap: str, 
                         comparison_func: Callable, ttl_seconds: Optional[float] = 3600) -> ComparisonResult:
