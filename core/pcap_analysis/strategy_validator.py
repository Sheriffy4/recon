...
class EffectivenessResult:
    ...
    def __post_init__(self):
        """Calculate derived metrics."""
        self.success_rate = self.successful_domains / self.total_domains if self.total_domains > 0 else 0.0
...
class BeforeAfterComparison:
    ...
    def __post_init__(self):
        ...
        domain_change = abs(self.after_result.successful_domains - self.before_result.successful_domains) > 2
...
class DomainSelector:
    ...
    def _categorize_domain(self, domain: str) -> str:
        ...
        if any(social in domain for social in ['x.com', 'twitter.com', 'facebook.com', 'instagram.com', 'telegram.org']):
            return "social"
            ...
            return "torrent"
        elif any(cdn in domain for cdn in ['twimg.com', 'fbcdn.net', 'cdninstagram.com', 'cloudflare.net', 'fastly.com']):
            return "cdn"
        elif 'api.' in domain:
            return "api"
        else:
    ...
    def select_test_domains(self, count: int = 5, categories: Optional[List[str]] = None, 
                          priorities: Optional[List[int]] = None) -> List[TestDomain]:
...
class StrategyValidator:
    ...
    def _analyze_test_result(self, stdout: bytes, stderr: bytes, return_code: int) -> bool:
        ...
        # Success indicators
        success_indicators = [
            'success', 'bypass successful', 'connection established',
    ...
    async def test_strategy_effectiveness(self, strategy: StrategyConfig, 
                                        domains: Optional[List[str]] = None) -> EffectivenessResult:
        ...
        for category in category_counts:
            performance_breakdown[category] = category_successes[category] / category_counts[category]
    ...
    async def _test_domain_with_strategy_config(self, domain: TestDomain, 
    ...
    async def compare_before_after(self, original_strategy: StrategyConfig, 
                                 fixed_strategy: StrategyConfig,
                                 domains: Optional[List[str]] = None) -> BeforeAfterComparison:
    ...
    async def generate_pcap_for_validation(self, strategy: StrategyConfig, 
    ...
    def get_validation_summary(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        average_domain_success_rate = sum(domain_success_rates) / len(domain_success_rates) if domain_success_rates else 0.0
        ...
        return {
            ...
            'overall_domain_success_rate': total_domains_successful / total_domains_tested if total_domains_tested > 0 else 0.0
        }