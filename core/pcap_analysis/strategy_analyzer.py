...
class StrategyAnalyzer:
    ...
    def parse_strategy_from_pcap(self, packets: List[PacketInfo], domain: str = "") -> StrategyConfig:
    ...
    def _detect_fake_disorder_pattern(self, packets: List[PacketInfo]) -> Optional[FakeDisorderPattern]:
    ...
    def _analyze_fake_disorder_sequence(self, packets: List[PacketInfo], 
                                      fake_idx: int, fake_packet: PacketInfo) -> Optional[FakeDisorderPattern]:
        ...
        for i in range(fake_idx + 1, min(fake_idx + 5, len(packets))):
            packet = packets[i]
            if (packet.ttl > 30 and  # Normal TTL
                packet.payload_length > 0 and  # Has payload
                packet.src_ip == fake_packet.src_ip and  # Same connection
                packet.dst_ip == fake_packet.dst_ip):
    ...
    def _calculate_split_and_overlap(self, fake_packet: PacketInfo, 
                                   real_segments: List[PacketInfo]) -> Tuple[Optional[int], int]:
    ...
    def _detect_disorder_pattern(self, packets: List[PacketInfo]) -> Optional[Dict[str, Any]]:
        ...
        for i in range(1, len(packets)):
            if (packets[i].sequence_num < packets[i-1].sequence_num and
                packets[i].src_ip == packets[i-1].src_ip):
    ...
    def compare_strategies(self, recon_config: StrategyConfig, 
                         zapret_config: StrategyConfig) -> StrategyComparison:
        ...
        if recon_config.dpi_desync != zapret_config.dpi_desync:
            differences.append(StrategyDifference(
        ...
        if recon_config.split_pos != zapret_config.split_pos:
            impact = "CRITICAL" if abs((recon_config.split_pos or 0) - (zapret_config.split_pos or 0)) > 2 else "HIGH"
            differences.append(StrategyDifference(
                parameter="split_pos",
    ...
    def validate_strategy_parameters(self, config: StrategyConfig) -> Dict[str, Any]:
        ...
        if config.ttl is not None:
            if config.ttl < 1 or config.ttl > 255:
                ...
                validation_result['warnings'].append(f"High TTL ({config.ttl}) for fake strategy may not be effective")
        ...
        if config.split_pos is not None:
            if config.split_pos < 1:
                ...
                validation_result['warnings'].append(f"Large split position ({config.split_pos}) may be ineffective")
        
        # Validate strategy combinations
        if config.has_strategy('fake') and not config.fooling:
            validation_result['warnings'].append("Fake strategy without fooling methods may be ineffective")
        
        if config.has_strategy('fakeddisorder') and config.split_pos is None:
            validation_result['errors'].append("Fake disorder strategy requires split_pos parameter")
            validation_result['valid'] = False
