...
class AdaptiveEnginePCAPIntegration:
    ...
    async def analyze_failure_with_pcap(
        self, domain: str, pcap_file: str
    ) -> Tuple[PCAPAnalysisResult, StrategyGenerationResult]:
        ...
        try:
            ...
            await self._cache_analysis_result(
                domain, pcap_file, pcap_analysis, strategy_generation.strategies
            )

            LOG.info(
                f"Анализ завершен: {len(strategy_generation.strategies)} стратегий сгенерировано"
            )
            ...
            empty_strategies = StrategyGenerationResult(
                pcap_file=pcap_file, generation_timestamp=datetime.now(), total_strategies=0
            )
    ...
    async def _handle_strategy_failure(
        self, domain: str, strategy: Any, test_result: Any, pcap_file: Optional[str] = None
    ...
    async def _generate_pcap_strategies(
    ...
    async def _cache_analysis_result(
        ...
        if len(self.analysis_cache) > self.max_cache_entries:
            ...
            oldest_keys = sorted(
                self.analysis_cache.keys(), key=lambda k: self.analysis_cache[k].timestamp
    ...
    async def _correlate_with_historical_data(self, domain: str, pcap_analysis: PCAPAnalysisResult):
        ...
        try:
            ...
            blocking_pattern = (
                f"{pcap_analysis.primary_blocking_type.value}_{pcap_analysis.dpi_behavior.value}"
            )
    ...
    async def _filter_relevant_strategies(
        self, strategies: List[GeneratedStrategy], failed_strategy: Any, test_result: Any
    ) -> List[GeneratedStrategy]:
    ...
    async def _update_failure_correlations(
        self, domain: str, failed_strategy: Any, pcap_analysis: PCAPAnalysisResult
    ):
                    ...
                    if strategy_name in correlation.successful_strategies:
                        ...
                        if correlation.sample_count > 0:
                            correlation.success_rate = (
                                len(correlation.successful_strategies) / correlation.sample_count
                            )
    ...
    async def _find_relevant_cached_analyses(self, domain: str) -> List[PCAPAnalysisCache]:
        ...
        relevant_analyses.sort(key=lambda x: (x.usage_count, x.timestamp), reverse=True)

        return relevant_analyses[:5]  # Максимум 5 релевантных анализов

    async def _adapt_strategies_to_context(
        self, strategies: List[GeneratedStrategy], context: Dict[str, Any]
    ) -> List[GeneratedStrategy]:
    ...
    async def _deduplicate_and_rank_strategies(
        self, strategies: List[GeneratedStrategy]
    ) -> List[GeneratedStrategy]:
        ...
        unique_strategies.sort(key=lambda s: (s.priority.value, s.confidence), reverse=True)
...
async def create_pcap_integration(
...
async def integrate_pcap_with_adaptive_engine(
    adaptive_engine, config: Optional[Dict[str, Any]] = None
) -> bool:
