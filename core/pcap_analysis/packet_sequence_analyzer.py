...
class PacketSequenceAnalyzer:
    ...
    def detect_fake_packet(self, packet: PacketInfo, context_packets: List[PacketInfo], 
                          position: int) -> FakePacketAnalysis:
        """
    ...
    def detect_split_positions(self, packets: List[PacketInfo]) -> SplitPositionAnalysis:
        ...
        if len(analysis.detected_splits) > 1:
            ...
            fake_nearby = any(
                ...
                for i in range(max(0, analysis.detected_splits[0] - 2),
        ...
        if analysis.expected_position and analysis.actual_positions:
            accuracy_scores = [
                1.0 - abs(pos - analysis.expected_position) / max(analysis.expected_position, pos, 1)
    ...
    def calculate_overlap_sizes(self, packets: List[PacketInfo]) -> OverlapAnalysis:
        ...
        if analysis.overlaps_detected:
            ...
            accurate_overlaps = [
                o for o in analysis.overlaps_detected
    ...
    def analyze_timing_patterns(self, packets: List[PacketInfo]) -> TimingAnalysis:
        ...
        analysis.delay_variance = statistics.variance(analysis.inter_packet_delays) if len(analysis.inter_packet_delays) > 1 else 0.0
    ...
    def _calculate_zapret_compliance(self, analysis: FakeDisorderAnalysis, 
                                   packets: List[PacketInfo]) -> float:
        """
    ...
    def compare_sequences(self, recon_packets: List[PacketInfo], 
    ...
    def get_analysis_summary(self, packets: List[PacketInfo]) -> Dict[str, Any]:
        ...
        return {
            ...
            'quality_score': (fake_disorder.zapret_compliance + splits.split_accuracy + overlaps.overlap_accuracy) / 3
        }