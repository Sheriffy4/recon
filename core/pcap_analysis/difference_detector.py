...
class DetectionConfig:
    ...
    def __post_init__(self):
        ...
        if self.critical_packet_types is None:
            self.critical_packet_types = {'ClientHello', 'fake_packet', 'disorder_packet'}
...
class DifferenceDetector:
    ...
    def detect_critical_differences(self, comparison: ComparisonResult) -> List[CriticalDifference]:
        ...
        self.logger.info(f"Detected {len(differences)} differences, "
                        f"{len([d for d in differences if d.impact_level == ImpactLevel.CRITICAL])} critical")
        
        return prioritized
    
    def _detect_sequence_differences(self, comparison: ComparisonResult) -> List[CriticalDifference]:
        ...
        for conn_key in recon_connections:
            ...
            fake_diff = self._detect_fake_packet_sequence_issues(recon_conn_packets, zapret_conn_packets, conn_key)
            if fake_diff:
    ...
    def _detect_checksum_differences(self, comparison: ComparisonResult) -> List[CriticalDifference]:
    ...
    def _detect_ttl_differences(self, comparison: ComparisonResult) -> List[CriticalDifference]:
        ...
        if recon_fake_ttls and zapret_fake_ttls:
            recon_fake_ttl = statistics.mode(recon_fake_ttls) if recon_fake_ttls else None
            zapret_fake_ttl = statistics.mode(zapret_fake_ttls) if zapret_fake_ttls else None
            
            if recon_fake_ttl and zapret_fake_ttl and abs(recon_fake_ttl - zapret_fake_ttl) >= self.config.ttl_difference_threshold:
                diff = CriticalDifference(
    ...
    def _detect_strategy_differences(self, comparison: ComparisonResult) -> List[CriticalDifference]:
        ...
        for param, (recon_val, zapret_val) in strategy_diffs.items():
            ...
            diff.add_evidence(
                "strategy_inference",
                f"Strategy parameter inferred from packet analysis",
                {
                    'parameter': param,
    ...
    def _detect_flag_differences(self, comparison: ComparisonResult) -> List[CriticalDifference]:
        ...
        # Compare patterns
        for flag_combo in set(recon_flag_patterns.keys()) | set(zapret_flag_patterns.keys()):
    ...
    def _detect_ordering_differences(self, comparison: ComparisonResult) -> List[CriticalDifference]:
        ...
        if len(recon_packets) != len(zapret_packets):
            diff = CriticalDifference(
                ...
                description=f"Packet count mismatch",
    ...
    def prioritize_differences(self, differences: List[CriticalDifference]) -> List[CriticalDifference]:
        """
        ...
        prioritized = sorted(
            differences,
            key=lambda d: (-d.calculate_severity_score(), d.fix_priority, -d.confidence)
        )
        
        return prioritized
    
    def categorize_differences(self, differences: List[CriticalDifference]) -> Dict[DifferenceCategory, List[CriticalDifference]]:
        """
    ...
    def group_related_differences(self, differences: List[CriticalDifference]) -> List[DifferenceGroup]:
    ...
    def _detect_sequence_gaps(self, recon_packets: List[PacketInfo], 
                            zapret_packets: List[PacketInfo], conn_key: str) -> Optional[CriticalDifference]:
    ...
    def _detect_fake_packet_sequence_issues(self, recon_packets: List[PacketInfo], 
                                          zapret_packets: List[PacketInfo], conn_key: str) -> Optional[CriticalDifference]:
    ...
    def _compare_strategies(self, recon_strategy: Dict[str, Any], 
                          zapret_strategy: Dict[str, Any]) -> Dict[str, Tuple[Any, Any]]:
        """Compare inferred strategies."""
        differences = {}
    ...
    def _assess_fix_risk(self, difference: CriticalDifference) -> str:
        ...
        if difference.category in [DifferenceCategory.STRATEGY, DifferenceCategory.SEQUENCE]:
            return "MEDIUM"
