...
class PatternDatabase:
        ...
        try:
            ...
            for pattern_id, pattern in self.patterns["failure_patterns"].items():
                if self._pattern_matches_query(pattern, query):
                    matches["failure_patterns"].append({
                        "id": pattern_id,
                        **pattern
                    })
            ...
            for pattern_id, pattern in self.patterns["success_patterns"].items():
                if self._pattern_matches_query(pattern, query):
                    matches["success_patterns"].append({
                        "id": pattern_id,
                        **pattern
                    })
    ...
    def update_pattern_success(self, pattern_id: str, success: bool, category: str = "success_patterns"):
...
class LearningEngine:
    ...
    def _update_pattern_database(self, learning_patterns: Dict[str, Any]):
        ...
        try:
            ...
            for solution in learning_patterns.get("solution_patterns", []):
                ...
                self.pattern_db.add_success_pattern(pattern_id, {
            ...
            self.pattern_db.add_fix_pattern(fix_id, {
                "fix_type": learning_patterns.get("solution_patterns", [{}])[0].get("fix_type", "unknown"),
                "problem_types": [p.get("type") for p in learning_patterns.get("problem_patterns", [])],
                "solution": learning_patterns.get("solution_patterns", [{}])[0].get("changes", {}),
    ...
    def _adapt_analysis_parameters(self, learning_patterns: Dict[str, Any]):
        ...
        try:
            ...
            if success_rate > 0.8:
                ...
                adaptation["adaptations_made"].append("Increased confidence threshold for similar patterns")
            elif success_rate < 0.3:
                # Low success rate - decrease confidence
                adaptation["adaptations_made"].append("Decreased confidence threshold for similar patterns")
            
            # Adapt parameter preferences
            param_patterns = learning_patterns.get("parameter_patterns", {})
            if param_patterns.get("ttl"):
                adaptation["adaptations_made"].append(f"Learned TTL preference: {param_patterns['ttl']}")
            
            if param_patterns.get("split_pos"):
                adaptation["adaptations_made"].append(f"Learned split_pos preference: {param_patterns['split_pos']}")
    ...
    def predict_strategy_effectiveness(self, 
        ...
        try:
            ...
            if success_patterns:
                ...
                prediction["confidence"] = min(len(success_patterns) / 5.0, 1.0)  # More patterns = higher confidence
    ...
    def _add_prediction_reasoning(self, 
        ...
        try:
            ...
            success_patterns = matching_patterns.get("success_patterns", [])
            if success_patterns:
                avg_success = statistics.mean([p.get("success_rate", 0.0) for p in success_patterns])
        ...
        try:
            ...
            if success_patterns:
                ...
                if best_params.get("ttl") and best_params["ttl"] != strategy_params.get("ttl"):
                    recommendations.append(f"Consider using TTL={best_params['ttl']} (success rate: {best_pattern.get('success_rate', 0.0):.1%})")
                
                # Split position recommendations
                if best_params.get("split_pos") and best_params["split_pos"] != strategy_params.get("split_pos"):
                    recommendations.append(f"Consider using split_pos={best_params['split_pos']} (success rate: {best_pattern.get('success_rate', 0.0):.1%})")
            ...
            if fix_patterns:
                ...
                if successful_fixes:
                    ...
                    recommendations.append(f"Consider applying {best_fix.get('fix_type', 'unknown')} fix (successful {best_fix.get('success_count', 0)} times)")
            
            # Warnings based on failure patterns
            failure_patterns = matching_patterns.get("failure_patterns", [])
            if failure_patterns:
                for pattern in failure_patterns[:3]:  # Top 3 failure patterns
                    recommendations.append(f"Warning: {pattern.get('description', 'Unknown issue')} - {pattern.get('solution', 'No solution available')}")
