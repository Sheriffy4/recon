...
from .progress_reporter import (
    DetailedProgressReporter, AsyncProgressReporter, 
    create_analysis_progress, create_batch_progress
)
from .interactive_menu import DifferenceReviewMenu, FixReviewMenu, confirm_action
...
Examples:
  ...
  %(prog)s analyze recon_x.pcap zapret_x.pcap --report-only
        ...
        parser.add_argument('--help-topic', type=str,
        ...
        compare_parser.add_argument('--interactive', '-i', action='store_true',
                                  help='Enable interactive mode for fix review')
        compare_parser.add_argument('--auto-apply', '-a', action='store_true',
                                  help='Automatically apply low-risk fixes')
        compare_parser.add_argument('--strategy-params', type=str,
                                  help='Strategy parameters JSON file')
        ...
        analyze_parser.add_argument('--report-only', action='store_true',
        ...
        validate_parser.add_argument('--test-domains', nargs='+',
                                   help='Test domains for validation')
        
        return parser
    
    async def run_single_analysis(self, recon_pcap: str, zapret_pcap: str,
                                output_dir: Optional[str] = None,
                                strategy_params: Optional[Dict] = None,
                                auto_apply_fixes: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            progress.complete_step(1, f"Loaded {len(comparison_result.recon_packets)} recon packets, {len(comparison_result.zapret_packets)} zapret packets")
            ...
            if getattr(self, 'interactive', False):
                ...
                # Review differences
                approved_differences, diff_results = self.difference_review.review_differences(differences)
                ...
                progress.complete_step(7, f"Approved {len(differences)} differences, {len(fixes)} fixes")
            else:
    ...
    async def run_batch_command(self, args) -> int:
        ...
        try:
            ...
            # Save batch results
            output_dir = getattr(args, 'output_dir', None) or batch_config.get('output_base_dir', './batch_results')
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    async def run_validate_command(self, args) -> int:
        ...
        try:
            ...
            for fix in fixes:
                ...
                print(f"Fix {fix.get('file_path', 'unknown')}: "
