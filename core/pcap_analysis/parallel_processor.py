...
class ParallelTaskManager:
    ...
    def _execute_with_processes(
        self, tasks: List[Tuple[Callable, tuple]], task_ids: List[str]
    ) -> List[TaskResult]:
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(
                future_to_task.keys(), timeout=self.config.timeout_seconds
            ):
...
class ParallelPcapAnalyzer:
    ...
    def analyze_multiple_pcaps(self, pcap_files: List[str]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Parallel analysis completed: {successful_analyses}/{len(pcap_files)} successful"
        )
        ...
        return {
            ...
            "summary": {
                ...
                "parallel_speedup": sum(r.execution_time for r in results)
                / max(total_execution_time, 0.001),
            },
        }
...
class AsyncParallelProcessor:
    ...
    async def stream_multiple_pcaps_async(
