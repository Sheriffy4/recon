    ...
    def execute_attack(
        ...
        try:
            ...
            if self.config.simulation_mode:
                ...
                result = self._execute_real_attack(attack_class, mapped_params, target_ip, target_port)
    ...
    def _stop_pcap_capture(self, pcap_file: Path):
        ...
        if self._captured_packets:
            try:
                ...
                self.logger.info(f"Saved {len(self._captured_packets)} packets to {pcap_file}")
            except Exception as e:
