...
class PacketConverter:
    ...
    def _build_packet_info(packet: 'pydivert.Packet', protocol_specific_data: Dict[str, Any]) -> PacketInfo:
        ...
        base_data = {'src_ip': packet.src_addr, 'dst_ip': packet.dst_addr, 'direction': PacketDirection.OUTBOUND if packet.is_outbound else PacketDirection.INBOUND, 'ip_ttl': ttl, 'raw_data': bytes(packet.raw), 'interface': packet.interface, 'metadata': {'source': 'pydivert', 'original_packet': packet}}
    ...
    def from_pydivert(packet: 'pydivert.Packet') -> Optional[PacketInfo]:
        ...
        try:
            ...
            if packet.tcp:
                ...
                protocol_specific_data = {'src_port': packet.tcp.src_port, 'dst_port': packet.tcp.dst_port, 'tcp_seq': packet.tcp.seq_num, 'tcp_ack': packet.tcp.ack_num, 'tcp_window': packet.tcp.window_size, 'tcp_urgent': packet.tcp.urg_ptr, 'tcp_flags': PacketConverter._get_tcp_flags_string(packet.tcp), 'payload': payload, 'protocol': protocol}
            elif packet.udp:
                protocol = ProtocolType.UDP
                payload = bytes(packet.udp.payload) if packet.udp.payload else b''
                protocol_specific_data = {'src_port': packet.udp.src_port, 'dst_port': packet.udp.dst_port, 'payload': payload, 'protocol': protocol}
            else:
                return None
            base_data = {'src_ip': packet.src_addr, 'dst_ip': packet.dst_addr, 'direction': PacketDirection.OUTBOUND if packet.is_outbound else PacketDirection.INBOUND, 'ip_ttl': packet.ipv4.ttl if packet.ipv4 else packet.ipv6.hop_limit if packet.ipv6 else 64, 'raw_data': bytes(packet.raw), 'interface': packet.interface, 'payload_size': len(payload), 'metadata': {'source_lib': 'pydivert', 'original_packet_ref': id(packet)}}
            all_data = {**base_data, **protocol_specific_data}
            return PacketInfo(**all_data)
    ...
    def from_scapy(packet: Any) -> Optional[PacketInfo]:
        ...
        try:
            ...
            return PacketInfo(src_ip=ip_layer.src, dst_ip=ip_layer.dst, ip_version=ip_layer.version, ip_ttl=ip_layer.ttl, ip_id=ip_layer.id, ip_flags=ip_layer.flags, ip_options=bytes(ip_layer.options) if ip_layer.options else b'', src_port=src_port, dst_port=dst_port, protocol=protocol, tcp_seq=tcp_seq, tcp_ack=tcp_ack, tcp_flags=tcp_flags, tcp_window=tcp_window, tcp_urgent=tcp_urgent, tcp_options=tcp_options, direction=direction, payload=payload, payload_size=len(payload), raw_data=bytes(packet), metadata={'original_packet': packet, 'packet_type': 'scapy'})
        except Exception as e:
            import logging
    ...
    def from_raw_bytes(data: bytes, direction: PacketDirection=PacketDirection.OUTBOUND) -> Optional[PacketInfo]:
        ...
        try:
            ...
            if ip_protocol == 6:
                ...
                return PacketInfo(src_ip=src_ip, dst_ip=dst_ip, ip_version=4, ip_ttl=ip_ttl, ip_id=ip_id, ip_flags=ip_flags, src_port=src_port, dst_port=dst_port, protocol=ProtocolType.TCP, tcp_seq=tcp_seq, tcp_ack=tcp_ack, tcp_flags=tcp_flags, tcp_window=tcp_window, tcp_urgent=tcp_urgent, direction=direction, payload=payload, payload_size=len(payload), raw_data=data, metadata={'packet_type': 'raw'})
            elif ip_protocol == 17:
                ...
                payload = data[payload_start:] if len(data) > payload_start else b''
                return PacketInfo(src_ip=src_ip, dst_ip=dst_ip, ip_version=4, ip_ttl=ip_ttl, ip_id=ip_id, ip_flags=ip_flags, src_port=src_port, dst_port=dst_port, protocol=ProtocolType.UDP, direction=direction, payload=payload, payload_size=len(payload), raw_data=data, metadata={'packet_type': 'raw'})
            else:
                return None
        except Exception as e:
            import logging
    ...
    def _get_tcp_flags_string(tcp_header) -> str:
        ...
        flags = []
        flag_map = [('C', 'cwr'), ('E', 'ece'), ('U', 'urg'), ('A', 'ack'), ('P', 'psh'), ('R', 'rst'), ('S', 'syn'), ('F', 'fin')]
    ...
    def to_attack_context(packet_info: PacketInfo) -> 'AttackContext':
        ...
        return AttackContext(dst_ip=packet_info.dst_ip, dst_port=packet_info.dst_port, src_ip=packet_info.src_ip, src_port=packet_info.src_port, seq=packet_info.tcp_seq, ack=packet_info.tcp_ack, flags=packet_info.tcp_flags or 'PA', payload=packet_info.payload, protocol='tcp' if packet_info.is_tcp else 'udp', engine_type='native_pydivert')