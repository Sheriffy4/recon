...
from .dns.robust_dns_handler import RobustDNSHandler
...
class CDNResolver:
    ...
    def get_cdn_domains(self, blocked_domain: str) -> List[str]:
        ...
        if blocked_domain in self.CDN_MAPPINGS:
            ...
            # Check for subdomain or partial matches
    ...
    def resolve_cdn_domains(self, blocked_domain: str) -> CDNResolutionResult:
        ...
        # Resolve all CDN domains concurrently
        resolved_ips = self.dns_handler.resolve_multiple_domains(cdn_domains)

        # Count successful and failed resolutions
    ...
    def validate_cdn_resolution(
        ...
        if not result.cdn_domains:
            return True  # No CDN domains to validate
        ...
        if not is_valid:
            LOG.warning(
                f"CDN resolution validation failed for {blocked_domain}: "
                f"success rate {success_rate:.2f} < {min_success_rate}"
            )
    ...
    def _is_domain_match(self, domain: str, pattern: str) -> bool:
        ...
        # Subdomain match (e.g., www.instagram.com matches instagram.com)
        if domain.endswith("." + pattern):
            return True

        # Pattern subdomain match (e.g., instagram.com matches www.instagram.com)
