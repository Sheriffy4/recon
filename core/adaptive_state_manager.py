...
class DomainState:
    ...
    def get_best_strategy(self) -> Optional[StrategyRecord]:
        ...
        return max(
            valid_strategies, key=lambda s: (s.success_rate, s.confidence, -s.get_age_days())
        )
...
class AdaptiveStateManager:
    ...
    def load_state(self) -> bool:
        ...
        try:
            ...
            if version != DataVersion.CURRENT.value:
                LOG.warning(
                    f"Версия данных {version} отличается от текущей {DataVersion.CURRENT.value}"
                )
            ...
            LOG.info(
                f"✅ Загружено состояние: {len(self.domains)} доменов, {self._count_total_strategies()} стратегий"
            )
            return True
    ...
    def save_state(self, create_backup: bool = True) -> bool:
        ...
        try:
            ...
            LOG.info(
                f"💾 Состояние сохранено: {len(self.domains)} доменов, {self._count_total_strategies()} стратегий"
            )
            return True
    ...
    def export_to_recon_service_format(self, output_file: str) -> bool:
        ...
        try:
            ...
            LOG.info(
                f"📤 Экспорт в формат recon_service: {len(recon_data)} доменов в {output_file}"
            )
            return True
    ...
    def cleanup_expired_data(self, max_age_days: int = 30) -> int:
        ...
        for domain_state in self.domains.values():
            ...
            domain_state.strategies = [
                s for s in domain_state.strategies if not s.is_expired(max_age_days)
            ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "file_size_bytes": (
                self.strategies_file.stat().st_size if self.strategies_file.exists() else 0
            ),
        ...
        if from_version == "1.0":
            ...
            for domain, strategy_data in data.items():
                ...
                if isinstance(strategy_data, dict) and "name" in strategy_data:
                    ...
                    LOG.info(
                        f"Мигрируем стратегию {strategy_data.get('name')} для {domain}, устанавливаем last_tested: {current_time}"
                    )
                    domain_state = {
                        "domain": domain,
                        "strategies": [
                            {
                                ...
                                "test_count": 1,
                                "found_at": strategy_data.get(
                                    "found_at", datetime.now().isoformat()
                                ),
                                "last_tested": current_time,  # Обновляем на текущее время
                                "engine_version": strategy_data.get(
                                    "engine_version", "adaptive_v1.0"
                                ),
                                "confidence": 1.0,
...
def load_best_strategy(
    domain: str, state_manager: Optional[AdaptiveStateManager] = None
...
if __name__ == "__main__":
    ...
    # Сохраняем стратегию
    success = state_manager.save_strategy(
        "example.com", strategy, execution_time=2.5, trials_count=3
    )
