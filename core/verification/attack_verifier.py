...
from core.packet.raw_packet_engine import (
    RawPacket,
    RawPacketEngine,
    ProtocolType,
    IPHeader,
...
class DisorderVerification:
    ...
    packet_order: List[Tuple[int, float]] = field(
        default_factory=list
    )  # Порядок пакетов (seq, timestamp)
    ...
    def generate_report(self) -> str:
        ...
        if self.differences:
            ...
            for i, diff in enumerate(self.differences, 1):
                lines.append(
                    f"  {i}. {diff.get('type', 'Unknown')}: {diff.get('description', 'No description')}"
                )
                if "file" in diff:
...
class AttackApplicationVerifier:
        ...
        try:
            ...
            if not packets:
                ...
                return FakeAttackVerification(
                    fake_packet_found=False, is_correct=False, issues=["No packets found in PCAP"]
                )
            ...
            return FakeAttackVerification(
                fake_packet_found=False, is_correct=False, issues=[f"Verification error: {str(e)}"]
            )

    def verify_multisplit_attack(
        self, pcap_file: str, expected_positions: List[int]
        ...
        try:
            ...
            if is_correct:
                ...
                self.logger.warning(
                    f"⚠️ Multisplit атака применена некорректно: {len(issues)} проблем"
                )
    ...
    def verify_disorder_attack(self, pcap_file: str) -> DisorderVerification:
        ...
        try:
            ...
            packets = self._read_pcap(pcap_file)
            if not packets:
                return DisorderVerification(is_correct=False, issues=["No packets found in PCAP"])
            ...
            if len(data_packets) < 2:
                return DisorderVerification(
                    is_correct=False,
                    issues=[
                        f"Not enough data packets for disorder attack (found {len(data_packets)}, need at least 2)"
                    ],
                )
            ...
            if is_correct:
                ...
                self.logger.warning(f"⚠️ Disorder атака применена некорректно: {', '.join(issues)}")

            return result

        except Exception as e:
            self.logger.error(f"❌ Ошибка верификации disorder атаки: {e}", exc_info=True)
            return DisorderVerification(is_correct=False, issues=[f"Verification error: {str(e)}"])

    def compare_attack_application(
        self, testing_pcap: str, service_pcap: str, attack_type: str
    ) -> ComparisonReport:
        """
        ...
        try:
            ...
            if attack_type == "fake":
                ...
                if set(testing_verification.found_positions) != set(
                    service_verification.found_positions
                ):
                    differences.append(
                        {
            ...
            if is_identical:
                ...
                self.logger.warning(
                    f"⚠️ {attack_type} атака применяется по-разному: {len(differences)} различий"
                )
            ...
            return ComparisonReport(
                ...
                attack_type=attack_type,
                differences=[
                    {"type": "comparison_error", "description": f"Comparison failed: {str(e)}"}
                ],
                is_identical=False,
