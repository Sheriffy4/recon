...
from recon.core.bypass.attacks.attack_registry import AttackRegistry
        ...
        if dpi_context["active_dpi"]:
            ...
            if combo:
                combo["params"]["stages"].append(
                    {
                        "type": segmentation_tech,
                        "params": self._get_attack_default_params(segmentation_tech),
                    }
                )
                combos.append(combo)
            ...
            if len(ml_predictions) >= 2:
                ...
                if tech1 in config.RACE_TECHS and conf1 > 0.7:
                    ...
                    delivery_tech = (
                        tech1
                        if tech1 in config.SEGMENTATION_TECHS + config.OBFUSCATION_TECHS
                        else "tcp_fakeddisorder"
                    )
                available_fooling = [
                    t
                    for t in config.TECH_LIBRARY
                    if "fooling" in t and t not in [race_tech, delivery_tech]
                ]
    ...
    def generate_plan(
        ...
        for item in plan:
            if "stages" in item.get("params", {}):
                item_key = (
                    item["type"],
                    tuple((s["type"] for s in item["params"]["stages"])),
                )
            else:
