...
class IPv4Packet(Packet):
    ...
    def parse(cls, raw: bytes) -> 'IPv4Packet':
        ...
        return cls(version=version, ihl=ihl, tos=fields[1], total_length=fields[2], id=fields[3], flags=fields[4] >> 13, frag_offset=fields[4] & 8191, ttl=fields[5], protocol=fields[6], checksum=fields[7], src_addr='.'.join((str(b) for b in fields[8])), dst_addr='.'.join((str(b) for b in fields[9])), options=options, payload=payload)

    def serialize(self) -> bytes:
        header = struct.pack('!BBHHHBBH4s4s', (self.version << 4) + self.ihl, self.tos, self.total_length, self.id, (self.flags << 13) + self.frag_offset, self.ttl, self.protocol, self.checksum, bytes(map(int, self.src_addr.split('.'))), bytes(map(int, self.dst_addr.split('.'))))
    ...
    def clone(self) -> 'IPv4Packet':
        return IPv4Packet(version=self.version, ihl=self.ihl, tos=self.tos, total_length=self.total_length, id=self.id, flags=self.flags, frag_offset=self.frag_offset, ttl=self.ttl, protocol=self.protocol, checksum=self.checksum, src_addr=self.src_addr, dst_addr=self.dst_addr, options=self.options, payload=self.payload)
...
class TCPPacket(Packet):
    ...
    def parse(cls, raw: bytes) -> 'TCPPacket':
        ...
        options = raw[20:header_length] if header_length > 20 else b''
        payload = raw[header_length:]
        return cls(src_port=fields[0], dst_port=fields[1], seq_num=fields[2], ack_num=fields[3], data_offset=data_offset, reserved=fields[4] >> 1 & 7, flags=fields[5], window=fields[6], checksum=fields[7], urgent_ptr=fields[8], options=options, payload=payload)
    ...
    def update_checksum(self, ip_packet: IPv4Packet):
        self.checksum = 0
        pseudo_header = struct.pack('!4s4sHH', bytes(map(int, ip_packet.src_addr.split('.'))), bytes(map(int, ip_packet.dst_addr.split('.'))), ip_packet.protocol, len(self.serialize()))
...
class UDPPacket(Packet):
    ...
    def parse(cls, raw: bytes) -> 'UDPPacket':
        ...
        return cls(src_port=fields[0], dst_port=fields[1], length=fields[2], checksum=fields[3], payload=raw[8:])
    ...
    def clone(self) -> 'UDPPacket':
        return UDPPacket(src_port=self.src_port, dst_port=self.dst_port, length=self.length, checksum=self.checksum, payload=self.payload)

    def update_checksum(self, ip_packet: IPv4Packet):
        self.checksum = 0
        self.length = 8 + len(self.payload)
        pseudo_header = struct.pack('!4s4sHH', bytes(map(int, ip_packet.src_addr.split('.'))), bytes(map(int, ip_packet.dst_addr.split('.'))), 17, self.length)
        packet = pseudo_header + self.serialize()
        if len(packet) % 2 == 1:
            packet += b'\x00'
        words = struct.unpack(f'!{len(packet) // 2}H', packet)
