...
class ConfigMigrationTool:
    ...
    def analyze_configuration(self, config_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis['wildcard_opportunities'] = self._find_wildcard_opportunities(domain_strategies)
    ...
    def optimize_configuration(self, config_file: str, output_file: str = None) -> bool:
        ...
        try:
            ...
            logger.info(f"Reduced {len(config.domain_strategies)} rules to {len(optimized_config.domain_strategies)}")
    ...
    def _find_wildcard_opportunities(self, domain_strategies: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if analysis['current_version'] != '3.0':
            recommendations.append(f"Upgrade from version {analysis['current_version']} to 3.0 for enhanced features")
        
        if analysis['wildcard_opportunities']:
            total_savings = sum(opp['potential_savings'] for opp in analysis['wildcard_opportunities'])
        ...
        if analysis['domain_count'] > 50:
            recommendations.append("Consider using IP-based rules for better performance with large rule sets")
    ...
    def _apply_optimizations(self, config: StrategyConfiguration, 
                           optimizations: Dict[str, Any]) -> StrategyConfiguration:
...
def main():
    ...
    parser.add_argument('command', choices=['migrate', 'analyze', 'optimize'],
    ...
    if args.command == 'migrate':
        ...
        if analysis['wildcard_opportunities']:
            ...
            for opp in analysis['wildcard_opportunities']:
                print(f"  {opp['wildcard_pattern']}: {opp['potential_savings']} rules can be consolidated")
