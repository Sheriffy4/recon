...
from .strategy_config_manager import StrategyConfiguration, StrategyRule, StrategyMetadata
...
class StrategyValidator:
    ...
    def _validate_performance(self, config: StrategyConfiguration):
        ...
        for pattern, rule in config.domain_strategies.items():
            if rule.metadata.success_rate > 0:
                ...
                # Check latency
                if (rule.metadata.avg_latency_ms > 0 and 
                    rule.metadata.avg_latency_ms > self.PERFORMANCE_THRESHOLDS['max_latency_ms']):
                    self.issues.append(ValidationIssue(
                        severity='warning',
                        category='performance',
    ...
    def suggest_improvements(self, config: StrategyConfiguration) -> List[str]:
        ...
        if config.domain_strategies:
            success_rates = [rule.metadata.success_rate for rule in config.domain_strategies.values() 
                           if rule.metadata.success_rate > 0]
