...
class CachingConfig:
    ...
    fingerprint_cache: Dict[str, Any] = field(default_factory=lambda: {
        "max_memory_mb": 50,
    ...
    strategy_cache: Dict[str, Any] = field(default_factory=lambda: {
        "max_memory_mb": 30,
...
class FingerprintingConfig:
    ...
    analysis_levels: Dict[str, bool] = field(default_factory=lambda: {
        "basic": True,
        "advanced": True,
    ...
    component_timeouts: Dict[str, float] = field(default_factory=lambda: {
        "tcp_analyzer": 10.0,
        "http_analyzer": 15.0,
        "dns_analyzer": 10.0,
        "tls_analyzer": 20.0,
...
def apply_environment_overrides(config: PerformanceConfig) -> PerformanceConfig:
    ...
    if os.getenv('RECON_FINGERPRINT_DEEP_ANALYSIS'):
        config.fingerprinting.analysis_levels['deep'] = os.getenv('RECON_FINGERPRINT_DEEP_ANALYSIS').lower() == 'true'
