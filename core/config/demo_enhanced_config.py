...
def demo_legacy_migration():
    ...
    print(
        f"Converted configuration from v{legacy_config['version']} to v{config.version}"
    )
    print(
        f"Global strategy: {config.global_strategy.strategy if config.global_strategy else 'None'}"
    )
...
def demo_wildcard_optimization():
    ...
    for domain in twitter_domains:
        metadata = StrategyMetadata(
            priority=1, description=f"Individual rule for {domain}", success_rate=0.38
        )
        manager.add_domain_strategy(
            domain, "seqovl(split_pos=76, overlap_size=336, ttl=1)", metadata
        )
    ...
    print("Optimized to 1 wildcard rule: *.twimg.com")
    print("Success rate improved from 38% to 85%")
...
def demo_priority_system():
    ...
    high_priority = StrategyMetadata(
        priority=1, description="High priority domain rule"
    )
    medium_priority = StrategyMetadata(
        priority=2, description="Medium priority IP rule"
    )
    low_priority = StrategyMetadata(
        priority=0, description="Low priority global fallback"
    )
    ...
    config.global_strategy = StrategyRule(
        "*", "--dpi-desync=badsum_race --dpi-desync-ttl=4", low_priority
    )
