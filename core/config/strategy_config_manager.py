...
class StrategyConfigManager:
    ...
    def load_configuration(self, config_file: Optional[str] = None) -> StrategyConfiguration:
        ...
        try:
            ...
            logger.info(
                f"Loaded configuration v{config.version} with {len(config.domain_strategies)} domain rules"
            )
            return config
    ...
    def add_domain_strategy(
        self, pattern: str, strategy: str, metadata: Optional[StrategyMetadata] = None
    ) -> None:
        """
        ...
        if metadata is None:
            metadata = StrategyMetadata(created_at=datetime.now().isoformat())
        else:
            metadata.updated_at = datetime.now().isoformat()

        rule = StrategyRule(pattern=pattern, strategy=strategy, metadata=metadata)
    ...
    def _create_default_configuration(self) -> StrategyConfiguration:
        ...
        config = StrategyConfiguration(global_strategy=global_strategy)
    ...
    def _parse_v3_config(self, raw_config: Dict[str, Any]) -> StrategyConfiguration:
        ...
        if global_strategy_data:
            if isinstance(global_strategy_data, str):
                ...
                config.global_strategy = StrategyRule(
                    pattern="*", strategy=global_strategy_data, metadata=metadata
                )
            else:
    ...
    def _validate_configuration(self, config: StrategyConfiguration) -> None:
        ...
        for pattern, rule in config.domain_strategies.items():
            ...
            if not self.validate_strategy_syntax(rule.strategy):
                logger.warning(
                    f"Potentially invalid strategy syntax for {pattern}: {rule.strategy}"
                )
