...
class ComponentRegistry:
    ...
    def register_all_components(self) -> None:
        ...
        try:
            ...
            self.logger.info(f'✅ Successfully registered {len(self._registered_components)} components')
    ...
    def _register_performance_components(self) -> None:
        ...
        try:
            ...
            self.container.register_singleton(ISegmentPerformanceOptimizer, SegmentPerformanceOptimizer)
            self.container.register_singleton(SegmentPerformanceOptimizer, SegmentPerformanceOptimizer)
            self._registered_components['ISegmentPerformanceOptimizer'] = ISegmentPerformanceOptimizer
            ...
            self.container.register_factory(ISegmentPerformanceOptimizer, lambda container: MockSegmentPerformanceOptimizer(), ServiceLifetime.SINGLETON)
            self._registered_components['ISegmentPerformanceOptimizer'] = ISegmentPerformanceOptimizer
    ...
    def _register_validation_components(self) -> None:
        ...
        try:
            from core.bypass.validation.dpi_effectiveness_validator import DPIEffectivenessValidator
            ...
            self.container.register_factory(IDPIEffectivenessValidator, lambda container: MockDPIEffectivenessValidator(), ServiceLifetime.SINGLETON)
            self._registered_components['IDPIEffectivenessValidator'] = IDPIEffectivenessValidator
    ...
    def _register_testing_components(self) -> None:
        ...
        try:
            from core.bypass.attacks.proper_testing_methodology import ProperTestingMethodology
            ...
            self.container.register_factory(IProperTestingMethodology, lambda container: MockProperTestingMethodology(), ServiceLifetime.SINGLETON)
            self._registered_components['IProperTestingMethodology'] = IProperTestingMethodology
    ...
    def _register_integration_components(self) -> None:
        ...
        try:
            ...
            self.container.register_factory(IAttackAdapter, lambda container: MockAttackAdapter(), ServiceLifetime.SINGLETON)
            self.container.register_factory(IResultProcessor, lambda container: MockResultProcessor(), ServiceLifetime.TRANSIENT)
            self.container.register_factory(IStrategyMapper, lambda container: MockStrategyMapper(), ServiceLifetime.SINGLETON)
            self._registered_components['IAttackAdapter'] = IAttackAdapter
            self._registered_components['IResultProcessor'] = IResultProcessor
            self._registered_components['IStrategyMapper'] = IStrategyMapper
    ...
    def _register_engine_components(self) -> None:
        ...
        def create_packet_processing_engine(container: DIContainer) -> PacketProcessingEngine:
        ...
        self.container.register_factory(PacketProcessingEngine, create_packet_processing_engine, ServiceLifetime.SINGLETON)
    ...
    def _register_ml_components(self) -> None:
        ...
        try:
            ...
            self.container.register_factory(IEvolutionarySearcher, lambda container: MockEvolutionarySearcher(), ServiceLifetime.SINGLETON)
            self._registered_components['IEvolutionarySearcher'] = IEvolutionarySearcher

    def register_custom_component(self, interface_type: Type[T], implementation_type: Type[T], lifetime: ServiceLifetime=ServiceLifetime.TRANSIENT, name: Optional[str]=None) -> None:
        """
    ...
    def validate_registrations(self) -> Dict[str, Any]:
        ...
        validation_results = {'total_components': len(self._registered_components), 'valid_components': 0, 'invalid_components': 0, 'validation_errors': []}
    ...
    def create_health_report(self) -> Dict[str, Any]:
        ...
        health_report = {'registry_status': 'healthy', 'total_components': len(self._registered_components), 'container_health': self.container.probe_health(), 'component_details': {}, 'validation_results': self.validate_registrations()}
        for name, component_type in self._registered_components.items():
            try:
                info = self.container.get_service_info(component_type)
                health_report['component_details'][name] = {'registered': True, 'info': info, 'resolvable': self.container.is_registered(component_type)}
            except Exception as e:
                health_report['component_details'][name] = {'registered': False, 'error': str(e), 'resolvable': False}
        if health_report['validation_results']['invalid_components'] > 0:
            health_report['registry_status'] = 'degraded'
        return health_report

def create_default_registry(container: Optional[DIContainer]=None) -> ComponentRegistry:
    """
...
__all__ = ['ComponentRegistry', 'ISegmentPerformanceOptimizer', 'IDPIEffectivenessValidator', 'IProperTestingMethodology', 'IAttackAdapter', 'IResultProcessor', 'IStrategyMapper', 'IEvolutionarySearcher', 'create_default_registry', 'get_global_registry']
