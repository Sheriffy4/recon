...
class CLIIntegration:
    ...
    async def initialize_services(self) -> CLIServices:
        ...
        try:
            ...
            self.services = CLIServices(fingerprint_engine=fingerprint_engine, prober=prober, classifier=classifier, attack_adapter=attack_adapter, effectiveness_tester=effectiveness_tester, learning_memory=learning_memory, strategy_generator=None, strategy_saver=strategy_saver, packet_processing_engine=packet_processing_engine, evolutionary_searcher=evolutionary_searcher, closed_loop_manager=closed_loop_manager, result_processor=result_processor, diagnostic_system=diagnostic_system, performance_optimizer=performance_optimizer)
    ...
    def create_domain_specific_services(self, domain: str, domain_ip: str, port: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            domain_services = {'domain': domain, 'domain_ip': domain_ip, 'port': port, 'fingerprint_engine': self.services.fingerprint_engine, 'prober': self.services.prober, 'classifier': self.services.classifier, 'attack_adapter': self.services.attack_adapter, 'effectiveness_tester': self.services.effectiveness_tester, 'learning_memory': self.services.learning_memory, 'strategy_generator': self.services.strategy_generator, 'strategy_saver': self.services.strategy_saver, 'result_processor': self.services.result_processor}
    ...
    async def create_strategy_generator_for_fingerprint(self, fingerprint_dict: Dict[str, Any]) -> IStrategyGenerator:
        """
        ...
        try:
            ...
            strategy_generator = AdvancedStrategyGenerator(attack_registry=attack_registry, domain_strategies=domain_strategies, strategy_predictor=strategy_predictor, parameter_optimizer=parameter_optimizer, fingerprint_dict=fingerprint_dict, history=[], max_strategies=self.args.count, enable_ml_prediction=not self.args.no_ml if hasattr(self.args, 'no_ml') else True)
            return strategy_generator
        except Exception as e:
            self._logger.error(f'Failed to create strategy generator for fingerprint: {e}')
            raise

    async def create_effectiveness_tester_for_optimization(self) -> IEffectivenessTester:
        """
        ...
        try:
            ...
            return RealEffectivenessTester(timeout=getattr(self.args, 'optimization_timeout', 120.0) / 10, max_retries=1)
        except Exception as e:
...
def create_fallback_services(args) -> Dict[str, Any]:
    ...
    try:
        ...
        from core.diagnostic_system import DiagnosticSystem
        from core.optimization.performance_optimizer import PerformanceOptimizer
        services['diagnostic_system'] = DiagnosticSystem(attack_adapter=services['attack_adapter'], debug=args.debug)
