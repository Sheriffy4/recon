...
class ValidationResult:
    ...
    def get_critical_issues(self) -> List[ValidationDetail]:
        ...
        return [
            ...
            if d.severity == ValidationSeverity.CRITICAL and not d.passed
        ]
    ...
    def get_errors(self) -> List[ValidationDetail]:
        ...
        return [
            d
            for d in self.details
            if d.severity == ValidationSeverity.ERROR and not d.passed
        ]
    ...
    def get_warnings(self) -> List[ValidationDetail]:
        ...
        return [
            d
            for d in self.details
            if d.severity == ValidationSeverity.WARNING and not d.passed
        ]
...
class PacketValidator:
    ...
    def _apply_spec_validation_rules(
        ...
        for category, rules in spec.validation_rules.items():
            for rule in rules:
                try:
                    # Evaluate rule
                    rule_passed = self._evaluate_validation_rule(
                        rule, packets, params, spec
                    )
    ...
    def _validate_fakeddisorder_sequence(
        ...
        for i in range(len(real_packets_sorted) - 1):
            ...
            expected_next_seq = (
                current_packet.sequence_num + current_packet.payload_length
            )
    ...
    def _validate_overlap(
        ...
        for i in range(len(packets) - 1):
            ...
            if (
                next_pkt.sequence_num >= overlap_start
                and next_pkt.sequence_num < overlap_end
            ):
                result.add_detail(
                    ValidationDetail(
                        aspect="sequence_numbers",
    ...
    def _validate_all_good_checksums(
        self, packets: List[PacketData], result: ValidationResult
    ):
        """
    ...
    def _detect_windivert_recalculation(
        self, packets: List[PacketData], result: ValidationResult
    ):
        """
    ...
    def _validate_packet_order(
        ...
        if attack_name == "fakeddisorder":
            ...
            is_ordered = all(
                seq_nums[i] <= seq_nums[i + 1] for i in range(len(seq_nums) - 1)
            )
        ...
        for i in range(max_packets):
            ...
            if i < len(expected_packets) and i < len(actual_packets):
                ...
                if "ttl" in expected and expected["ttl"] != actual.ttl:
                    differences.append(
                        f"TTL: expected {expected['ttl']}, got {actual.ttl}"
                    )
                ...
                if (
                    "sequence_num" in expected
                    and expected["sequence_num"] != actual.sequence_num
                ):
                    differences.append(
    ...
    def _generate_html_diff(
        ...
        for i in range(max_packets):
            ...
            if i < len(expected_packets) and i < len(actual_packets):
                ...
                html.append(
                    "<td colspan='4'>Expected packet but not found in actual</td>"
                )
                html.append("</tr>")
            else:
                html.append("<tr class='diff'>")
