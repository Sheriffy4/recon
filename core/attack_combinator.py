...
class AttackCombinator:
    ...
    def __init__(self, 
        ...
        self.logger.info(f"AttackCombinator initialized with {len(self.attack_strategies)} strategies")
    ...
    async def test_multiple_attacks_parallel(self, 
                                           domain: str, 
                                           target_ip: str,
                                           attack_list: Optional[List[str]] = None,
                                           max_parallel: Optional[int] = None) -> List[AttackResult]:
        """
        ...
        self.logger.info(f"Testing {len(attack_list)} attacks in parallel on {domain} ({target_ip})")
    ...
    async def _test_single_attack(self, 
        ...
        try:
            ...
            success, latency_ms, rst_packets, connection_established, error = await self._execute_attack(
                engine_task, domain, target_ip
            )
            ...
            self.logger.debug(f"Attack {attack_name} result: success={success}, latency={latency_ms:.1f}ms")
            return result
    ...
    async def _execute_attack(self, 
                            ...
                            domain: str,
                            target_ip: str) -> Tuple[bool, float, int, bool, Optional[str]]:
        """
    ...
    def _calculate_attack_score(self, attack_name: str, pattern_metrics: Dict[str, AdaptiveMetrics]) -> float:
        """
        ...
        success_rate = metrics.recent_success_rate if len(metrics.recent_results) >= 3 else metrics.success_rate
        ...
        if len(metrics.recent_results) >= 3:
            recent_boost = (metrics.recent_success_rate - metrics.success_rate) * self.config["recency_weight"] * 50
            score += recent_boost
        ...
        if "twitter" in attack_name and ("twimg.com" in pattern_metrics or "x.com" in pattern_metrics):
            score += 15
    ...
    async def execute_attack_chain(self, 
                                 chain_name: str,
                                 domain: str,
                                 target_ip: str) -> List[AttackResult]:
        """
        ...
        for i, attack_name in enumerate(chain.attacks):
            ...
            self.logger.info(f"Chain progress: {successful}/{len(results)} success rate: {current_success_rate:.2f}")
            
            # Check if we've met the success threshold
            if current_success_rate >= chain.success_threshold:
                self.logger.info(f"Chain success threshold met ({current_success_rate:.2f} >= {chain.success_threshold})")
                break
        ...
        if current_success_rate < chain.success_threshold and chain.fallback_strategy:
            ...
            fallback_result = await self._test_single_attack(chain.fallback_strategy, domain, target_ip)
    ...
    def get_best_strategy_for_domain(self, domain: str) -> Tuple[str, float]:
        ...
        for strategy_name, metrics in pattern_metrics.items():
            rate = metrics.recent_success_rate if len(metrics.recent_results) >= 3 else metrics.success_rate
            if rate > best_rate:
                best_rate = rate
    ...
    def get_comprehensive_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "domain_performance": {},
            "recent_activity": len([r for r in self.results_history 
        ...
        for result in self.results_history:
            ...
            strategy_stats[result.strategy_type]["avg_latency"] = (
                (current_avg * (attempts - 1) + result.latency_ms) / attempts
            )
    ...
    def load_metrics(self, filepath: str) -> bool:
        ...
        try:
            ...
            self.global_metrics.recent_results = deque(global_data.get("recent_results", []), maxlen=10)
    ...
    def __str__(self) -> str:
        ...
        return (f"AttackCombinator(strategies={len(self.attack_strategies)}, "
