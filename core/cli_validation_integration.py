    ...
    for i, strategy in enumerate(strategies):
        try:
            result = orchestrator.validate_strategy(
                strategy, check_attack_availability=check_attack_availability
            )
...
def format_strategy_validation_output(
    validation_summary: Dict[str, Any], use_colors: bool = True, verbose: bool = False
) -> str:
    ...
    if verbose and validation_summary["results"]:
        ...
        for i, result in enumerate(validation_summary["results"]):
            ...
            if use_colors:
                status_icon = (
                    f"\033[92m{status_icon}\033[0m"
                    if result.passed
                    else f"\033[91m{status_icon}\033[0m"
                )
...
def validate_strategy_string(
    strategy_string: str, check_attack_availability: bool = True
) -> StrategyValidationResult:
    ...
    try:
        ...
        strategy_dict = {"type": parsed.attack_type, **parsed.params}

        orchestrator = CLIValidationOrchestrator()
        result = orchestrator.validate_strategy(
            strategy_dict, check_attack_availability=check_attack_availability
        )
...
def validate_and_report_strategies(
    ...
    validation_summary = validate_generated_strategies(
        strategies, check_attack_availability=True, output_dir=output_dir
    )
    ...
    if console:
        ...
        output = format_strategy_validation_output(
            validation_summary, use_colors=True, verbose=verbose
        )
        print(output)
