...
class StrategyIntegrationHelper:
    ...
    def get_statistics_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "wildcard_rules": sum(1 for rule in self.selector.domain_rules.values() if rule.is_wildcard),
            "exact_rules": sum(1 for rule in self.selector.domain_rules.values() if not rule.is_wildcard)
        }
