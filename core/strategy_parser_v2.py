...
class StrategyParserV2:
    def __init__(self):
        ...
        self.known_attacks = {'fake', 'split', 'disorder', 'disorder2', 'multisplit', 'multidisorder', 'fakeddisorder', 'seqovl'}
    ...
    def _parse_function_style(self, strategy: str) -> ParsedStrategy:
        ...
        return ParsedStrategy(attack_type=attack_name, params=params, raw_string=strategy, syntax_type='function')
    ...
    def _parse_zapret_style(self, strategy: str) -> ParsedStrategy:
        ...
        for param_name in ['ttl', 'autottl', 'split-pos', 'split-count', 'split-seqovl', 'repeats']:
        ...
        return ParsedStrategy(attack_type=attack_type, params=params, raw_string=strategy, syntax_type='zapret')
...
class ParameterValidator:
    ...
    def __init__(self):
        ...
        self.param_specs = {
            ...
            'fooling': {
                ...
                'allowed_values': ['badsum', 'md5sig', 'badseq', 'hopbyhop', 'datanoack']
            },
            'fake_sni': {
                'type': str,
            ...
            'fake_seq': {
                'type': int,
                'min': 0,
                'description': 'Fake sequence number'
            },
            'enabled': {
                'type': bool,
        ...
        self.attack_requirements = {
            ...
            'multidisorder': {
                'required': ['split_pos'],
                'optional': ['ttl', 'autottl', 'overlap_size', 'fooling', 'repeats', 'split_seqovl'],
                'description': 'Multiple disorder attacks'
            },
            'fakeddisorder': {
                'required': ['split_pos'],
                'optional': ['ttl', 'autottl', 'fake_ttl', 'overlap_size', 'fooling', 'fake_sni', 'repeats'],
                'description': 'Send fake packet then real packets in disorder'
            },
            'fake': {
                'required': [],
                'optional': ['ttl', 'autottl', 'fake_ttl', 'fooling', 'fake_sni', 'repeats'],
    ...
    def validate(self, parsed: ParsedStrategy) -> bool:
        ...
        if parsed.attack_type not in self.attack_requirements:
            warnings.append(f"Unknown attack type '{parsed.attack_type}' - validation may be incomplete")
        ...
        for param_name, param_value in parsed.params.items():
            param_errors = self._validate_parameter(param_name, param_value, parsed.attack_type)
