...
class WinDivertDriverManager:
    ...
    def diagnose_driver_issues(self) -> DriverStatus:
        ...
        recommendations = self._generate_recommendations(
            files_present, is_installed, is_running, pydivert_works, 
            service_error, pydivert_error
        )
    ...
    def _check_driver_files(self) -> bool:
        ...
        files_in_current = [
            current_dir / filename for filename in self.required_files
        ]
    ...
    def _generate_recommendations(
        ...
        if not pydivert_works:
            if "administrator" in (pydivert_error or "").lower():
                ...
                recommendations.append(
                    "Driver may be corrupted - reinstall PyDivert"
                )
                recommendations.append(
    ...
    def _log_diagnosis_results(self, status: DriverStatus) -> None:
        ...
        self.logger.info(f"  Service Installed: {'✅' if status.is_installed else '❌'}")
    ...
    def _reinstall_pydivert(self) -> bool:
        ...
        try:
            ...
            # Install again
            result = subprocess.run(
                [sys.executable, "-m", "pip", "install", "pydivert", "--force-reinstall"],
                capture_output=True,
                text=True,
                timeout=120
            )
