...
class StrategyCombinator:
    ...
    def _load_combination_rules(self):
        ...
        self.combination_rules = {
            ...
            "commercial_dpi_bypass": ["multisplit_base", "badsum_fooling", "high_ttl"],
            ...
            "simple_fake": ["fake_base", "badsum_fooling", "high_ttl"],
            "seqovl_basic": ["seqovl_base", "badsum_fooling"],
        }
    ...
    def _check_compatibility(self, components: List[str]) -> Tuple[bool, List[str]]:
        ...
        for comp_name in components:
            ...
            for other_name in components:
                ...
                if (
                    comp.attack_type == "ttl_manipulation"
                ...
                if (
                    comp.attack_type != "fooling"
                    and comp.attack_type != "ttl_manipulation"
                    and comp.attack_type != "parameter"
                ):
                    if (
                        other_comp.attack_type not in comp.compatibility
    ...
    def combine_components(self, component_names: List[str]) -> Optional[Dict[str, Any]]:
        ...
        strategy = {"type": main_attack, "params": combined_params}
    ...
    def suggest_combinations_for_fingerprint(
        ...
        if fingerprint.dpi_type == DPIType.ROSKOMNADZOR_TSPU:
            suggestions.extend(
                [
                    (
                        "roskomnadzor_aggressive",
                        self.get_predefined_combination("roskomnadzor_aggressive"),
                    ),
                    (
                        "roskomnadzor_conservative",
                        self.get_predefined_combination("roskomnadzor_conservative"),
                    ),
                    (
                        "disorder_md5sig_combo",
                        self.get_predefined_combination("disorder_md5sig_combo"),
                    ),
                ]
            )
            ...
            suggestions.extend(
                [
                    (
                        ...
                        self.get_predefined_combination("commercial_dpi_bypass"),
                    ),
                    (
                        "commercial_dpi_advanced",
                        self.get_predefined_combination("commercial_dpi_advanced"),
                    ),
                    (
                        "multisplit_aggressive",
                        self.get_predefined_combination("multisplit_aggressive"),
                    ),
                ]
            )
        ...
        if fingerprint.vulnerable_to_bad_checksum_race:
            suggestions.append(
                (
                    "disorder_badsum_combo",
                    self.get_predefined_combination("disorder_badsum_combo"),
                )
            )
    ...
    def suggest_combinations_from_rule_recommendations(
        self,
        ...
        for technique in rule_recommendations:
            ...
            elif "fooling" in technique or technique in [
                "badsum_fooling",
                "md5sig_fooling",
                "badseq_fooling",
            ]:
        ...
        high_confidence_techniques = [
            t for t in rule_recommendations if technique_confidences.get(t, 0.0) > 0.8
        ]

        if len(high_confidence_techniques) >= 2:
            # Try to create a multi-technique combination
            if (
                "tcp_multisplit" in high_confidence_techniques
                and "badsum_fooling" in high_confidence_techniques
            ):
...
if __name__ == "__main__":
    ...
    custom = combinator.create_custom_combination(
        "fakeddisorder", ttl=64, fooling=["badsum", "md5sig"]
    )
