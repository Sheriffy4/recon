...
class StrategyPayloadIntegration:
    ...
    def __init__(
        ...
        if payload_manager is not None:
            ...
            self._manager = PayloadManager(payload_dir=payload_dir, bundled_dir=bundled_dir)
    ...
    def get_payload_for_domain(
        self, domain: str, payload_type: PayloadType = PayloadType.TLS
    ) -> Optional[bytes]:
        """
    ...
    def generate_payload_variations(
        self, base_strategy: Dict[str, Any], domain: Optional[str] = None, max_variations: int = 3
    ) -> List[Dict[str, Any]]:
        """
        ...
        supports_fake = (
            attack_type.startswith("fake")
            or ",fake" in attack_type
            or "_fake" in attack_type
            or attack_type in self.FAKE_PAYLOAD_ATTACKS
        )
    ...
    def enhance_strategies_with_payloads(
        ...
        for strategy in strategies:
            if include_variations:
                ...
                variations = self.generate_payload_variations(strategy, domain, max_variations=2)
    ...
    def get_strategy_payload_info(self, strategy: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    def format_strategy_for_zapret(
        self, strategy: Dict[str, Any], include_payload: bool = True
    ) -> str:
        """
