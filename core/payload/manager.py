...
class PayloadManager:
    ...
    def __init__(self, payload_dir: Optional[Path] = None, bundled_dir: Optional[Path] = None):
        ...
        # Index file path
        self._index_path = (
            self.payload_dir.parent / "payload_index.json"
            if self.payload_dir.parent.exists()
            else None
        )
    ...
    def _load_file(self, file_path: Path, source: str) -> Optional[Tuple[bytes, PayloadInfo]]:
        ...
        try:
            ...
            if not validation.valid:
                logger.warning(f"Invalid payload in {file_path}: {', '.join(validation.errors)}")
    ...
    def list_payloads(self, payload_type: Optional[PayloadType] = None) -> List[PayloadInfo]:
        """
        ...
        return [info for info in self._payload_infos if info.payload_type == payload_type]

    def resolve_placeholder(
        self, placeholder: str, domain: Optional[str] = None
    ) -> Optional[bytes]:
        """
    ...
    def get_payload_info(
        self, payload_type: PayloadType, domain: Optional[str] = None
