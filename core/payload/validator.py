...
class PayloadValidator:
    ...
    def validate(self, data: bytes) -> ValidationResult:
        ...
        if not data:
            return ValidationResult(
                valid=False, payload_type=PayloadType.UNKNOWN, errors=["Empty payload"]
            )
    ...
    def validate_tls_clienthello(self, data: bytes) -> ValidationResult:
        ...
        if len(data) < self.MIN_TLS_RECORD_SIZE:
            ...
            return ValidationResult(valid=False, payload_type=PayloadType.TLS, errors=errors)
        ...
        if version_major != 0x03:
            errors.append(
                f"Invalid TLS version major byte: 0x{version_major:02x} " f"(expected 0x03)"
            )
            return ValidationResult(valid=False, payload_type=PayloadType.TLS, errors=errors)

        # Warn about unusual minor versions
        if version_minor not in (0x01, 0x02, 0x03):
            warnings.append(f"Unusual TLS version minor byte: 0x{version_minor:02x}")
        ...
        if len(data) < self.MIN_TLS_RECORD_SIZE + 1:
            errors.append(f"Payload too short for handshake header: {len(data)} bytes")
            return ValidationResult(valid=False, payload_type=PayloadType.TLS, errors=errors)
        ...
        return ValidationResult(
            valid=True, payload_type=PayloadType.TLS, errors=errors, warnings=warnings
        )
    ...
    def validate_http_request(self, data: bytes) -> ValidationResult:
        ...
        if len(data) < 10:
            ...
            return ValidationResult(valid=False, payload_type=PayloadType.HTTP, errors=errors)
        ...
        return ValidationResult(
            valid=True, payload_type=PayloadType.HTTP, errors=errors, warnings=warnings
        )
    ...
    def validate_quic_initial(self, data: bytes) -> ValidationResult:
        ...
        if len(data) < 5:
            ...
            return ValidationResult(valid=False, payload_type=PayloadType.QUIC, errors=errors)
        ...
        return ValidationResult(
            valid=True, payload_type=PayloadType.QUIC, errors=errors, warnings=warnings
        )
