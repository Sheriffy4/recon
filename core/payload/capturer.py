...
class PayloadCapturer:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                payload = await loop.run_in_executor(
                    None,
                    self._capture_clienthello_sync,
                    domain,
                    port,
                    timeout
                )
                ...
                if not validation.valid:
                    logger.warning(
                        f"Captured payload failed validation: {validation.errors}"
                    )
                ...
                logger.info(
                    f"Successfully captured {len(payload)} bytes "
                    f"ClientHello from {domain}"
                )
    ...
    async def capture_quic_initial(
        self,
        domain: str,
        port: int = 443
    ) -> CaptureResult:
        ...
        return CaptureResult(
            ...
            payload=None,
            domain=domain,
            error="QUIC capture not yet implemented"
        )
