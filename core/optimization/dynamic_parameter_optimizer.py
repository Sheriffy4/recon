# recon/core/optimization/dynamic_parameter_optimizer.py
import time
import random
import logging
import asyncio
...
class DynamicParameterOptimizer:
    ...
    def generate_parameter_ranges(self, attack_name: str) -> Dict[str, ParameterRange]:
            ...
            if attack_class:
                try:
                    ...
                    if category in self._parameter_ranges:
                        self.logger.info(
                            f"Using category-based ranges for {attack_name} (category: {category})"
                        )
        ...
        # Возвращаем диапазоны по умолчанию, если ничего не найдено
    ...
    async def optimize_parameters(
        ...
        self.logger.info(
            f"Starting parameter optimization for {attack_name} using {strategy.value}"
        )
        ...
        try:
            ...
            if strategy == OptimizationStrategy.GRID_SEARCH:
                ...
                param_combinations = await self._generate_bayesian_combinations(
                    param_ranges, max_iterations, attack_name
                )
            elif strategy == OptimizationStrategy.EVOLUTIONARY:
                param_combinations = await self._generate_evolutionary_combinations(
                    param_ranges, max_iterations, context, attack_name
                )
            else:
            ...
            for iteration, params in enumerate(param_combinations):
                ...
                if time.time() - start_time > timeout_seconds:
                    self.logger.warning(
                        f"Optimization timeout reached after {iteration} iterations"
                    )
                    break
                ...
                try:
                    ...
                    if effectiveness > best_effectiveness:
                        ...
                        self.logger.info(
                            f"New best parameters for {attack_name}: "
                            f"effectiveness={effectiveness:.3f}, params={params}"
                        )
            ...
            result = OptimizationResult(
                ...
                total_tests=len(effectiveness_history),
            ...
            self.logger.info(
                f"Parameter optimization completed for {attack_name}: "
                f"best_effectiveness={best_effectiveness:.3f}, "
                f"total_tests={len(effectiveness_history)}, "
    ...
    async def _test_parameters(
        self, attack_name: str, parameters: Dict[str, Any], context: "AttackContext"
    ) -> float:
        """
        ...
        try:
            ...
            from ..bypass.attacks.registry import AttackRegistry
            from ..bypass.attacks.base import AttackContext, AttackStatus
            ...
            # Test effectiveness using real HTTP requests
    ...
    def _generate_grid_search_combinations(
        ...
        # Generate cartesian product (limited by max_combinations)
        import itertools
    ...
    async def _generate_bayesian_combinations(
        self,
        ...
        # Simplified Bayesian optimization using historical data
        combinations = []
    ...
    async def _generate_evolutionary_combinations(
        self,
        param_ranges: Dict[str, ParameterRange],
        max_combinations: int,
        context: "AttackContext",
        ...
        for generation in range(generations):
            ...
            # Evaluate population (simplified - would normally test all)
            # For efficiency, we'll use heuristics to estimate fitness
            fitness_scores = []
    ...
    def _create_parameter_variation(
        self, base_params: Dict[str, Any], param_ranges: Dict[str, ParameterRange]
    ) -> Dict[str, Any]:
    ...
    def _estimate_parameter_fitness(
        self, params: Dict[str, Any], param_ranges: Dict[str, ParameterRange]
    ) -> float:
        """Estimate parameter fitness using heuristics (for evolutionary algorithm)."""
        fitness = 0.5  # Base fitness

        # Heuristic: smaller segments often work better
        if "split_pos" in params and isinstance(params["split_pos"], int):
            if 1 <= params["split_pos"] <= 5:
                fitness += 0.2

        # Heuristic: moderate TTL values often work better
        if "ttl" in params and isinstance(params["ttl"], int):
            if 3 <= params["ttl"] <= 8:
                fitness += 0.1

        # Heuristic: certain combinations work well together
        if "fooling" in params and "split_pos" in params:
            if params.get("fooling") in ["badsum", "badseq"] and isinstance(
                params.get("split_pos"), int
            ):
                fitness += 0.15
    ...
    def _initialize_parameter_ranges(self) -> Dict[str, Dict[str, ParameterRange]]:
        ...
        ranges["tcp"] = {
            "split_pos": ParameterRange(
                "split_pos", "choice", choices=[1, 2, 3, 4, 5, 10, "midsld"], default=3
            ),
            "positions": ParameterRange(
                "positions",
                "choice",
                choices=[[1, 3], [1, 3, 10], [2, 5, 8]],
                default=[1, 3, 10],
            ),
            "overlap_size": ParameterRange(
                "overlap_size", "int", min_value=5, max_value=50, step=5, default=10
            ),
            "window_size": ParameterRange(
                "window_size", "int", min_value=1, max_value=16, default=1
            ),
        }

        # TCP Manipulation attacks
        ranges["tcp_manipulation"] = {
            "window_scale": ParameterRange(
                "window_scale", "int", min_value=1, max_value=8, default=2
            ),
            "urgent_data_size": ParameterRange(
                "urgent_data_size", "int", min_value=1, max_value=10, default=2
            ),
        ...
        ranges["tls"] = {
            ...
            "extension_padding": ParameterRange(
                "extension_padding",
                "int",
                min_value=0,
                max_value=512,
        ...
        # HTTP attacks
        ranges["http"] = {
            "header_case": ParameterRange(
                "header_case",
                "choice",
                choices=["upper", "lower", "mixed", "random"],
                default="mixed",
            ),
            "method": ParameterRange(
                "method",
                "choice",
                choices=["GET", "POST", "PUT", "HEAD"],
                default="GET",
            ),
            "user_agent_type": ParameterRange(
                "user_agent_type",
                "choice",
        ...
        # HTTP/2 attacks
        ranges["http2"] = {
            "frame_size": ParameterRange(
                "frame_size", "int", min_value=1, max_value=16384, step=64, default=1024
            ),
            "stream_priority": ParameterRange(
                "stream_priority", "int", min_value=0, max_value=255, default=0
            ),
            "window_update_size": ParameterRange(
                "window_update_size", "int", min_value=1, max_value=65535, default=65535
            ),
        }

        # QUIC attacks
        ranges["quic"] = {
            "packet_size": ParameterRange(
                "packet_size", "int", min_value=64, max_value=1200, step=64, default=512
            ),
            "connection_id_length": ParameterRange(
                "connection_id_length", "int", min_value=4, max_value=18, default=8
            ),
            "migration_frequency": ParameterRange(
                "migration_frequency", "int", min_value=1, max_value=10, default=3
            ),
        }

        # Payload attacks
        ranges["payload"] = {
            "encryption_key_size": ParameterRange(
                "encryption_key_size", "choice", choices=[8, 16, 32], default=16
            ),
            "obfuscation_rounds": ParameterRange(
                "obfuscation_rounds", "int", min_value=1, max_value=5, default=2
            ),
            "noise_ratio": ParameterRange(
                "noise_ratio",
                "float",
                min_value=0.1,
                max_value=0.5,
        ...
        # Tunneling attacks
        ranges["tunneling"] = {
            "tunnel_protocol": ParameterRange(
                "tunnel_protocol",
                "choice",
                choices=["dns", "icmp", "http"],
                default="dns",
            ),
            "chunk_size": ParameterRange(
                "chunk_size", "int", min_value=32, max_value=512, step=32, default=128
            ),
            "encoding_type": ParameterRange(
                "encoding_type",
                "choice",
                choices=["base64", "base32", "hex"],
                default="base64",
            ),
        }
        ...
        # Combo attacks
        ranges["combo"] = {
            "max_iterations": ParameterRange(
                "max_iterations", "int", min_value=1, max_value=10, default=3
            ),
            "detection_threshold": ParameterRange(
                "detection_threshold",
                "float",
                min_value=0.3,
                max_value=0.9,
            ...
            "adaptation_level": ParameterRange(
                "adaptation_level",
                "choice",
        ...
        # Default ranges for unknown attacks
        ranges["default"] = {
            "split_pos": ParameterRange(
                "split_pos", "int", min_value=1, max_value=10, default=3
            ),
        ...
        # Specific attack ranges (override category ranges)
