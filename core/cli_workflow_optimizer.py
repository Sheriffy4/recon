# recon/core/cli_workflow_optimizer.py
"""
...
class FingerprintCache:
    ...
    cache_ttl: float = 3600.0  # 1 hour TTL
    ...
    def clear_expired(self):
        ...
        for fp_hash in expired_hashes:
            ...
            # Remove from domain and IP mappings
            domains_to_remove = [
                domain
...
class CLIWorkflowOptimizer:
    ...
    def optimize_domain_grouping(
        ...
        # Group domains by IP address first (same IP likely has same DPI)
        ...
        # For each IP group, check if we already have a fingerprint
        optimized_groups: Dict[str, List[str]] = {}

        for ip, ip_domains in ip_groups.items():
            # Check if we have a cached fingerprint for this IP
            cached_fp = self.workflow_state.fingerprint_cache.get_fingerprint_for_ip(ip)

            if cached_fp:
                # Use cached fingerprint for all domains with this IP
                ...
                # Mark all domains as having this fingerprint
                ...
                # Need to fingerprint this IP group
                # Use first domain as representative for fingerprinting
    ...
    def should_skip_evolutionary_search(self, current_effectiveness: float) -> bool:
        ...
        # Skip if we already have very high effectiveness
        ...
        # Skip if we're in single strategy mode
    ...
    def optimize_parameter_testing_order(
        ...
        # Score attacks based on fingerprint characteristics
        attack_scores = {}

        for attack_name in attack_names:
            score = 0.0

            # Score based on DPI type
            ...
            # Score based on blocking patterns
    ...
    def should_use_fast_mode(
        ...
        # Use fast mode for large domain counts
        if domain_count > 50:
            LOG.info(f"Using fast mode due to large domain count ({domain_count})")
            return True

        # Use fast mode if time budget is tight
        if time_budget_seconds and time_budget_seconds < 300:  # Less than 5 minutes
            LOG.info(
                f"Using fast mode due to tight time budget ({time_budget_seconds}s)"
            )
            return True

        # Use fast mode in certain execution modes
