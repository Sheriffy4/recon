...
class ParameterMapper:
    ...
    def map_parameters(
        ...
        try:
            ...
            if not mappings:
                self.logger.debug(f"No parameter mappings for {attack_name}, using empty constructor")
                return {}
    ...
    def validate_parameters(
        ...
        try:
            ...
            if not mappings:
                if params:
                    self.logger.debug(f"{attack_name} takes no constructor params, but {len(params)} provided")
                return errors
