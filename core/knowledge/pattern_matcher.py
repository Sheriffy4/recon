...
class PatternMatcher:
    ...
    def apply_knowledge_rules(
        ...
        if cached_result is not None:
            LOG.debug(
                f"Использован кэшированный результат сопоставления (время: {cache_time:.3f}s)"
            )
        ...
        if not matching_patterns:
            LOG.debug(
                f"Нет совпадающих правил, используем fallback (время поиска: {matching_time:.3f}s)"
            )
        ...
        matching_patterns.sort(key=lambda p: p.metadata.get("confidence", 0.5), reverse=True)
        sort_time = time.time() - sort_start_time

        # Ограничиваем количество правил
        max_patterns = self.knowledge_accumulator.global_settings.get("max_patterns_per_match", 3)
        ...
        LOG.info(
            f"Найдено {len(matching_patterns)} совпадающих правил, "
    ...
    def _cache_result(self, cache_key: str, result: Tuple[List[str], Dict[str, Any]]):
        ...
        if len(self.match_cache) > 1000:
            ...
            sorted_items = sorted(
                self.match_cache.items(), key=lambda x: x[1][1]  # Сортируем по timestamp
            )
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        now = datetime.now()
        valid_entries = sum(
            1 for _, (_, timestamp) in self.match_cache.items() if now - timestamp < self.cache_ttl
        )
    ...
    def explain_match(
        self, pattern: PatternRule, failure_report: Any, context: Dict[str, Any]  # FailureReport
    ) -> Dict[str, Any]:
        """
