...
class KnowledgeAccumulator:
            ...
            for pattern_data in data.get("patterns", []):
                try:
                    ...
                    if pattern.validate():
                        ...
                        LOG.warning(f"Невалидное правило пропущено: {pattern_data.get('id', 'unknown')}")
    ...
    def save_patterns(self, force: bool = False):
        ...
        with self._save_lock:
            ...
            if not force and self._update_count < self._batch_size and time_since_last_save.total_seconds() < self._batch_timeout:
                # Добавляем в очередь pending updates
                LOG.debug(f"Отложено сохранение: {self._update_count}/{self._batch_size} обновлений, "
    ...
    def _perform_save(self):
        ...
        try:
            ...
            is_testing = (os.getenv("PYTEST_CURRENT_TEST") is not None or 
                         os.getenv("TESTING") is not None or
                         "test" in str(self.rules_file).lower())
            ...
            LOG.info(f"💾 Batch сохранение: {len(self.patterns)} правил, "
    ...
    def update_success_pattern(self, 
        ...
        if matching_pattern:
            ...
            total = (matching_pattern.metadata.get("success_count", 0) + 
            ...
            LOG.info(f"Обновлено правило {matching_pattern.id}: "
    ...
    def update_failure_pattern(self,
                              failure_report: Any,  # FailureReport
                              failed_strategy: Any,
                              context: Dict[str, Any]):
        ...
        if matching_pattern:
            ...
            LOG.info(f"Обновлено правило {matching_pattern.id} после неудачи: "
                    f"confidence={matching_pattern.metadata['confidence']:.2f}")
        else:
    ...
    def _find_matching_pattern(self,
                              failure_report: Any,  # FailureReport
                              context: Dict[str, Any]) -> Optional[PatternRule]:
    ...
    def _create_pattern_from_success(self,
                                    failure_report: Any,  # FailureReport
                                    successful_strategy: Any,
                                    context: Dict[str, Any]) -> PatternRule:
        """Создание нового правила из успешного обхода."""
        pattern_id = f"auto_{failure_report.root_cause.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def prune_low_confidence_patterns(self, min_confidence: float = None):
        ...
        self.patterns = [
            ...
            if p.metadata.get("confidence", 0.5) >= min_confidence
        ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        avg_confidence = sum(p.metadata.get("confidence", 0.5) for p in self.patterns) / len(self.patterns)
    ...
    def _add_pending_update(self, update_info: Dict[str, Any]):
        ...
        with self._save_lock:
            ...
            LOG.debug(f"Добавлено обновление в batch: {update_info['type']} "
    ...
    def _check_and_save(self):
        ...
        with self._save_lock:
            ...
            should_save = (
                self._update_count >= self._batch_size or
                time_since_last_save.total_seconds() >= self._batch_timeout
            )
            
            if should_save:
                LOG.debug(f"Условия для batch сохранения выполнены: "
    ...
    def flush_pending_updates(self):
        ...
        with self._save_lock:
            if self._pending_updates:
                LOG.info(f"Принудительное сохранение {len(self._pending_updates)} отложенных обновлений")
                self._perform_save()
            else:
