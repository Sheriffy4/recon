...
class PacketModifier:
    ...
    def corrupt_checksum(self, packet: RawPacket, method: str = "badsum") -> RawPacket:
        """
    ...
    def corrupt_sequence(self, packet: RawPacket, method: str = "offset") -> RawPacket:
        """
        ...
        self.logger.debug(
            f"🔧 Corrupted TCP sequence (badseq, method={method}): " f"{old_seq} → {corrupted_seq}"
        )
    ...
    def _build_pseudo_header(self, packet: RawPacket) -> bytes:
        ...
        pseudo_header = struct.pack(
            "!IIBBH", src_ip, dst_ip, 0, 6, tcp_length  # Zero  # Protocol (TCP)
        )
