...
class ScapyMigrationTool:
    ...
    def convert_scapy_code(self, code: str) -> str:
        ...
        try:
            ...
            converted = re.sub(r"IP\s*\(", "engine.build_ip_packet(", converted)

            converted = re.sub(r"TCP\s*\(", "engine.build_tcp_packet(", converted)

            # Замена функций отправки
            converted = re.sub(r"send\s*\(", "await engine.inject_packet(", converted)
                ...
                if total_files > 0:
                    ...
                    if conversion_rate >= 90:
                        ...
                        report["recommendations"].append(
                            "Требуется дополнительная работа по миграции"
                        )
