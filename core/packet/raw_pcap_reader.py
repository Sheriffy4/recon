...
class RawPCAPReader:
    ...
    def _read_next_packet(self, file_handle, header: PCAPHeader) -> Optional[RawPacket]:
        ...
        if incl_len > header.snaplen:
            raise CorruptedPacketError(
                f"Packet size exceeds snaplen: {incl_len} > {header.snaplen}"
            )
        ...
        if len(packet_data) < incl_len:
            raise CorruptedPacketError(
                f"Incomplete packet data: expected {incl_len}, got {len(packet_data)}"
            )
    ...
    def read_pcap_file(self, filepath: str) -> List[RawPacket]:
        ...
        try:
            ...
            if file_size > self.MAX_FILE_SIZE:
                self.logger.warning(
                    f"⚠️ PCAP файл слишком большой ({file_size} bytes), используется потоковая обработка"
                )
            ...
            if pcap_format == "pcapng":
                self.logger.warning(
                    "⚠️ PCAPNG формат пока не полностью поддерживается, используется базовый парсинг"
                )
                ...
                while True:
                    try:
                        ...
                        if packet_count >= self.MAX_PACKETS_IN_MEMORY:
                            self.logger.info(
                                f"ℹ️ Достигнут лимит пакетов в памяти ({self.MAX_PACKETS_IN_MEMORY}), используется потоковая обработка"
                            )
                        ...
                        skipped_count += 1
                        self.logger.warning(
                            f"⚠️ Пропущен поврежденный пакет #{packet_count + skipped_count}: {e}"
                        )
                        continue
                    except EOFError:
                        break
    ...
    def iterate_packets(self, filepath: str) -> Iterator[RawPacket]:
        ...
        try:
            ...
            if file_size > self.MAX_FILE_SIZE:
                self.logger.info(
                    f"ℹ️ Большой PCAP файл ({file_size} bytes), используется потоковая обработка"
                )
            ...
            with open(filepath, "rb") as f:
                ...
                self.logger.debug(
                    f"✅ Обработано {packet_count} пакетов (пропущено {skipped_count})"
                )
