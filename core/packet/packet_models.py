...
class IPHeader:
    ...
    def from_bytes(cls, data: bytes) -> 'IPHeader':
        ...
        version_ihl, tos, total_len, identification, flags_fragment, ttl, protocol, checksum = struct.unpack('!BBHHHBBH', data[:12])
...
class TCPHeader:
    ...
    def from_bytes(cls, data: bytes) -> 'TCPHeader':
        ...
        src_port, dst_port, seq, ack, header_len_flags, window, checksum, urgent = struct.unpack('!HHLLHHHH', data[:20])
...
class UDPHeader:
    ...
    def to_bytes(self) -> bytes:
        ...
        return struct.pack('!HHHH', self.source_port, self.destination_port, self.length, self.checksum)
...
class RawPacket:
    ...
    def is_tls(self) -> bool:
        ...
        return (self.is_tcp and self.destination_port == 443 and 
                len(self.payload) > 6 and self.payload[0] == 0x16)
    
    @property
    def is_http(self) -> bool:
        """Проверяет, является ли пакет HTTP."""
        return (self.is_tcp and self.destination_port == 80 and
                self.payload.startswith((b'GET ', b'POST ', b'PUT ', b'DELETE ', b'HEAD ')))
    
    @property
    def is_quic(self) -> bool:
        """Проверяет, является ли пакет QUIC."""
        return (self.is_udp and self.destination_port == 443 and
                len(self.payload) > 1 and (self.payload[0] & 0x80) != 0)
...
class PacketStatistics:
    ...
    def add_packet(self, packet_size: int, modified: bool = False, technique_applied: bool = False) -> None:
