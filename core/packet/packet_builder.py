...
from .packet_models import (
    EthernetHeader, IPv4Header, IPv6Header, TCPHeader, UDPHeader,
    ICMPHeader, DNSHeader, HTTPHeader, TLSHeader
)
...
class PacketBuilder:
    ...
    def ethernet(self, dst_mac: str = "00:00:00:00:00:00", 
                 src_mac: str = "00:00:00:00:00:00", 
                 ethertype: int = 0x0800) -> 'PacketBuilder':
        """Add Ethernet header."""
    ...
    def ipv4(self, src_ip: str, dst_ip: str, protocol: int = 6,
             ttl: int = 64, flags: int = 0x4000, 
             identification: Optional[int] = None) -> 'PacketBuilder':
        """Add IPv4 header."""
    ...
    def ipv6(self, src_ip: str, dst_ip: str, next_header: int = 6,
             hop_limit: int = 64) -> 'PacketBuilder':
        """Add IPv6 header."""
        header = IPv6Header(
            version=6,
            traffic_class=0,
    ...
    def tcp(self, src_port: int, dst_port: int, seq: Optional[int] = None,
            ack: Optional[int] = None, flags: int = 0x18,
            window: int = 8192) -> 'PacketBuilder':
        """Add TCP header."""
        if seq is None:
    ...
    def icmp(self, icmp_type: int = 8, code: int = 0,
             identification: Optional[int] = None,
             sequence: Optional[int] = None) -> 'PacketBuilder':
        """Add ICMP header."""
        if identification is None:
            identification = random.randint(1, 65535)
        if sequence is None:
    ...
    def dns(self, transaction_id: Optional[int] = None,
            flags: int = 0x0100, questions: int = 1,
            answers: int = 0) -> 'PacketBuilder':
        """Add DNS header."""
    ...
    def http(self, method: str = "GET", path: str = "/",
             version: str = "HTTP/1.1", headers: Optional[Dict[str, str]] = None) -> 'PacketBuilder':
        """Add HTTP header."""
        if headers is None:
            headers = {}
    ...
    def tls(self, content_type: int = 22, version: int = 0x0303,
            length: int = 0) -> 'PacketBuilder':
        """Add TLS header."""
        header = TLSHeader(
    ...
    def _build_tcp(self, header: TCPHeader) -> bytes:
        """Build TCP header."""
        data_offset_flags = (header.data_offset << 12) | (header.reserved << 6) | header.flags
...
class FragmentedPacketBuilder:
    ...
    def fragment(self) -> List[bytes]:
        ...
        while offset < len(original_packet):
            fragment_size = min(self.mtu - 20, len(original_packet) - offset)  # 20 bytes for IP header
...
class PacketModifier:
    ...
    @staticmethod
    def modify_payload(packet_data: bytes, new_payload: bytes, 
...
def create_http_request(src_ip: str, dst_ip: str, src_port: int, dst_port: int,
...
def create_dns_query(src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                    domain: str) -> bytes:
