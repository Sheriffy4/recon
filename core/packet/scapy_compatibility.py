...
class IP(PacketLayer):
    ...
    def __bytes__(self) -> bytes:
        ...
        packet = engine.create_tcp_packet(self.src, self.dst, 0, 0, ttl=self.ttl)
    ...
    def __truediv__(self, other):
        ...
        elif isinstance(other, UDP):
            return IPUDPPacket(self, other)
...
class UDP(PacketLayer):
    """Эмуляция UDP слоя Scapy."""
...
class IPTCPPacket:
    ...
    def __init__(self, ip_layer: IP, tcp_layer: TCP, payload: Optional[Raw] = None):
        ...
        self.layers.append(LayerInfo("TCP", tcp_layer))
        if payload:
            self.layers.append(LayerInfo("Raw", payload))
...
class IPUDPPacket:
    """Комбинированный IP+UDP пакет."""

    def __init__(self, ip_layer: IP, udp_layer: UDP, payload: Optional[Raw] = None):
        self.ip = ip_layer
        self.udp = udp_layer
        self.payload = payload
        self._raw_packet = None
        ...
        self.layers = []
        self.layers.append(LayerInfo("IP", ip_layer))
        self.layers.append(LayerInfo("UDP", udp_layer))
        if payload:
            self.layers.append(LayerInfo("Raw", payload))
    ...
    def __truediv__(self, other):
        """Добавление payload."""
        if isinstance(other, Raw):
            return IPUDPPacket(self.ip, self.udp, other)
        else:
            raise ScapyCompatibilityError(f"Unsupported payload type: {type(other)}")
