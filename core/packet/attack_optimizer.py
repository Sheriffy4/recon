...
class AttackOptimizer:
    ...
    async def optimize_tcp_fragmentation(
    ...
    async def generate_improved_strategies(
        self, attack_type: AttackType, optimized_params: List[AttackParameters]
    ) -> List[Dict[str, Any]]:
...
async def quick_attack_optimization() -> None:
    ...
    for i, strategy in enumerate(top_strategies, 1):
        print(
            f"{i}. {strategy.get('type', 'unknown')} - {strategy.get('expected_success_rate', 0):.1%} успех"
        )
