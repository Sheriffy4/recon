...
class TCPHeader:
    ...
    def pack(self) -> bytes:
        ...
        offset_reserved_flags = (self.data_offset << 12) | (self.reserved << 6) | self.flags
    ...
    def unpack(cls, data: bytes) -> 'TCPHeader':
        ...
        # Распаковываем основные поля
        unpacked = struct.unpack("!HHIIHHHHH", data[:20])
...
class RawPacketEngine:
    ...
    def create_tcp_packet(self, src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                         seq_num: int = 0, ack_num: int = 0, flags: int = 0,
                         payload: bytes = b'', ttl: int = 64,
                         window_size: int = 65535) -> RawPacket:
        """Создает TCP пакет."""
        try:
    ...
    def create_syn_packet(self, src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                         seq_num: int = 0, ttl: int = 64) -> RawPacket:
        """Создает TCP SYN пакет."""
        return self.create_tcp_packet(
            src_ip, dst_ip, src_port, dst_port,
            seq_num=seq_num, flags=TCPHeader.FLAG_SYN, ttl=ttl
        )

    def create_syn_ack_packet(self, src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                             seq_num: int = 0, ack_num: int = 0, ttl: int = 64) -> RawPacket:
        """Создает TCP SYN-ACK пакет."""
        return self.create_tcp_packet(
            src_ip, dst_ip, src_port, dst_port,
            seq_num=seq_num, ack_num=ack_num,
            flags=TCPHeader.FLAG_SYN | TCPHeader.FLAG_ACK, ttl=ttl
        )

    def create_ack_packet(self, src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                         seq_num: int = 0, ack_num: int = 0, ttl: int = 64) -> RawPacket:
        """Создает TCP ACK пакет."""
        return self.create_tcp_packet(
            src_ip, dst_ip, src_port, dst_port,
            seq_num=seq_num, ack_num=ack_num,
            flags=TCPHeader.FLAG_ACK, ttl=ttl
        )

    def create_psh_ack_packet(self, src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                             seq_num: int = 0, ack_num: int = 0, payload: bytes = b'',
                             ttl: int = 64) -> RawPacket:
    ...
    def parse_packet(self, data: bytes) -> RawPacket:
        """Парсит сырой пакет."""
        try:
            if len(data) < 20:
            ...
            packet = RawPacket(
                ...
                protocol=ProtocolType(ip_header.protocol) if ip_header.protocol in [1, 6, 17] else None
            )
    ...
    def fragment_packet(self, packet: RawPacket, fragment_size: int) -> List[RawPacket]:
        ...
        try:
            ...
            while offset < len(payload_data):
                ...
                fragment = RawPacket(
                    ...
                    payload=fragment_data if offset == 0 else None  # Payload только в первом фрагменте
                )
    ...
    def get_packet_info(self, packet: RawPacket) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(packet.data) >= 20:
                ...
                # TCP информация
                if (packet.protocol == ProtocolType.TCP and
                    len(packet.data) >= 40):
...
def create_tcp_syn(src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                   seq_num: int = 0, ttl: int = 64) -> RawPacket:
...
def create_tcp_packet_with_payload(src_ip: str, dst_ip: str, src_port: int, dst_port: int,
                                  payload: bytes, seq_num: int = 0, ack_num: int = 0,
                                  ttl: int = 64) -> RawPacket:
    """Создает TCP пакет с данными (совместимость с существующим кодом)."""
    engine = RawPacketEngine()
    return engine.create_psh_ack_packet(src_ip, dst_ip, src_port, dst_port,
                                       seq_num, ack_num, payload, ttl)
