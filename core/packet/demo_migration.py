...
def tcp_scan(target_ip, ports):
    ...
    async def demo_compatibility_layer(self):
        ...
        print(f"  TCP пакет: {tcp_packet.sport} -> {tcp_packet.dport}, флаги: {tcp_packet.flags}")
        ...
        # Создание UDP пакета
        print("\nСоздание UDP пакета:")
        udp_packet = self.scapy_compat.IP(dst="8.8.8.8") / self.scapy_compat.UDP(dport=53)
...
def send_syn_packet(target_ip, target_port):
    ...
    return True
        ...
        backup_path = self.migration_tool.backup_scapy_code("demo_file.py", original_code)
        ...
        scapy_style = self.scapy_compat.IP(dst="192.168.1.1") / self.scapy_compat.TCP(dport=80)
        ...
        is_valid = (
            len(scapy_bytes) > 0 and 
            len(raw_bytes) > 0 and
            abs(len(scapy_bytes) - len(raw_bytes)) < 100  # Допустимая разница
        )
    ...
    async def demo_performance_comparison(self):
        ...
        for i in range(iterations):
            packet = self.scapy_compat.IP(dst="192.168.1.1") / self.scapy_compat.TCP(dport=80 + i)
            _ = bytes(packet)
        ...
        if raw_time < compat_time:
            ...
            print(f"  ⚠️ Слой совместимости быстрее")
...
if __name__ == "__main__":
    asyncio.run(main()).build_tcp_packet(
                source_port=12345 + i,
                dest_port=80,
                seq_num=1000,
                ack_num=0,
        ...
        # Результаты
        print(f"\nРезультаты производительности ({iterations} итераций):")
        print(f"  Побайтовая обработка: {raw_time:.3f} с ({raw_time/iterations*1000:.2f} мс/пакет)")
        print(f"  Слой совместимости: {compat_time:.3f} с ({compat_time/iterations*1000:.2f} мс/пакет)")
        ...
        if raw_time < compat_time * 0.8:
            ...
            print("  ✅ Миграция даст умеренное улучшение производительности")
        else:
            print("  ⚠️ Используйте слой совместимости для плавного перехода")
...
if __name__ == "__main__":
    asyncio.run(main()).build_tcp_packet(
        ...
        # Результаты
        print(f"\nРезультаты производительности ({iterations} итераций):")
        print(f"  Побайтовая обработка: {raw_time:.3f} сек ({raw_time/iterations*1000:.2f} мс/пакет)")
        print(f"  Слой совместимости: {compat_time:.3f} сек ({compat_time/iterations*1000:.2f} мс/пакет)")
        ...
        # Создаем много пакетов для теста памяти
        packets = []
        for i in range(100):
            packet = await self.raw_engine.build_tcp_packet(
                source_port=12345 + i,
        ...
        print(f"  Пиковое использование: {peak/1024:.1f} KB")
        print(f"  Текущее использование: {current/1024:.1f} KB")
        
        # Рекомендации по миграции
        print(f"\n📋 Рекомендации по миграции:")
        recommendations = self.migration_tool.generate_migration_recommendations()
...
if __name__ == "__main__":
    asyncio.run(main()).build_tcp_packet(
        ...
        if raw_time < compat_time * 0.8:
            ...
            print("  ⚠️ Требуется дополнительная оптимизация побайтовой обработки")
        
        print("  📋 Этапы миграции:")
        print("    1. Создать резервные копии существующего кода")
        print("    2. Заменить простые операции на побайтовые")
        print("    3. Использовать слой совместимости для сложных случаев")
        print("    4. Постепенно оптимизировать критические участки")
        print("    5. Провести полное тестирование")
