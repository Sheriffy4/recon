...
class IPv4Handler(ProtocolHandler):
    ...
    def parse(self, data: bytes, offset: int = 0) -> Tuple[IPv4Header, int]:
        ...
        version_ihl, tos, total_length, identification, flags_fragment, ttl, protocol, checksum, src_ip, dst_ip = \
    ...
    def build(self, header: IPv4Header) -> bytes:
        ...
        ip_header = struct.pack('!BBHHHBBH4s4s',
            ...
            header.ttl, header.protocol, header.checksum,
            src_ip, dst_ip
        )
...
class IPv6Handler(ProtocolHandler):
    ...
    def parse(self, data: bytes, offset: int = 0) -> Tuple[IPv6Header, int]:
        ...
        version_class_label, payload_length, next_header, hop_limit = \
            struct.unpack('!IHBB', ipv6_data[:8])
    ...
    def build(self, header: IPv6Header) -> bytes:
        """Build IPv6 header."""
        version_class_label = (header.version << 28) | (header.traffic_class << 20) | header.flow_label
        ...
        return struct.pack('!IHBB16s16s',
            version_class_label, header.payload_length,
            header.next_header, header.hop_limit,
            src_ip, dst_ip
        )
...
class DNSHandler(ProtocolHandler):
    ...
    def build(self, header: DNSHeader) -> bytes:
        ...
        return struct.pack('!HHHHHH',
            header.transaction_id, header.flags,
            header.questions, header.answers,
            header.authority, header.additional
        )
