...
async def test_raw_packet_engine():
    ...
    try:
        ...
        print(
            f"✅ TCP пакет создан: {tcp_packet.source_port} -> {tcp_packet.dest_port}"
        )
...
send(packet)
        ...
        if (
            "from recon.core.packet" in converted_code
            and "scapy" not in converted_code.lower()
        ):
...
async def test_packet_compatibility():
    ...
    try:
        ...
        if parsed_packet:
            print("✅ Scapy пакет распарсен побайтовым движком")
        ...
        try:
            ...
            print("✅ Побайтовый пакет обработан слоем совместимости")
        except Exception as e:
