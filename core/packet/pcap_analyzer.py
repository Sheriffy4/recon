...
class PCAPAnalyzer:
        ...
        try:
            with open(pcap_file, "rb") as f:
                ...
                while (
                    ...
                    # Анализируем пакет
                    packet_info = await self.parse_packet_data(
                        packet_data, ts_sec + ts_usec / 1000000
                    )
                    if packet_info:
    ...
    async def analyze_packets(self, packets: List[PacketInfo]) -> Dict[str, Any]:
        ...
        for packet in packets:
            # Подсчет протоколов
            analysis["protocol_distribution"][packet.protocol] = (
                analysis["protocol_distribution"].get(packet.protocol, 0) + 1
            )
