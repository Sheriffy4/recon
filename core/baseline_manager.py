...
class BaselineManager:
    ...
    def __init__(self, baselines_dir: Optional[Path] = None, enable_cache: bool = True):
        ...
        self._baseline_cache: Dict[str, tuple[BaselineReport, float]] = {}  # name -> (report, timestamp)
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        valid_entries = sum(
            1 for _, cache_time in self._baseline_cache.values()
        ...
        if baseline.packet_count > 0 and current.packet_count < baseline.packet_count * 0.8:
            return Regression(
                ...
                details={
                    'baseline_packet_count': baseline.packet_count,
                    'current_packet_count': current.packet_count,
                    'decrease_percent': ((baseline.packet_count - current.packet_count) / baseline.packet_count) * 100
                }
            )
    ...
    def _generate_comparison_summary(self, result: ComparisonResult) -> str:
        ...
        if result.regressions:
            ...
            for reg in result.regressions:
                lines.append(f"  [{reg.severity.value.upper()}] {reg.attack_name}: {reg.description}")
