...
class OperationLogger:
    ...
    def start_strategy_log(self, strategy_name: str, domain: str, 
    ...
    def log_operation(self, strategy_id: str, operation_type: str, 
                     parameters: Dict[str, Any], segment_number: int,
        ...
        with self._lock:
            if strategy_id in self._current_logs:
                ...
                self.logger.warning(
                    f"⚠️ Strategy ID not found: {strategy_id[:8]}, "
                    f"operation not logged"
                )
        
        return operation_id
    
    def end_strategy_log(self, strategy_id: str, save_to_file: bool = True) -> Optional[StrategyLog]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            total_operations = sum(
                len(log.operations) for log in self._current_logs.values()
            )
...
def log_operation(strategy_id: str, operation_type: str, 
                 parameters: Dict[str, Any], segment_number: int,
                 correlation_id: Optional[str] = None) -> str:
    """
