...
class StrategyInterpreter:
    ...
    def _extract_int_param(self, strategy_str: str, param_name: str, default: Optional[int] = None) -> Optional[int]:
        ...
        try:
            ...
            if param_name == 'autottl':
                if not (1 <= value <= 255):
                    self.logger.error(f"Invalid autottl value {value}. Must be 1-255. Using default.")
                    return 2
            elif param_name == 'ttl':
                if not (1 <= value <= 255):
    ...
    def _extract_str_param(self, strategy_str: str, param_name: str, default: Optional[str] = None) -> Optional[str]:
    ...
    def validate_strategy(self, strategy: ZapretStrategy) -> bool:
        ...
        if not strategy.methods and not strategy.fooling:
            self.logger.error(f"No valid DPI methods or fooling techniques found in strategy: '{strategy.raw_strategy}'")
            return False
        ...
        if DPIMethod.FAKEDDISORDER in strategy.methods:
            if strategy.split_seqovl is None:
                self.logger.error("fakeddisorder requires split-seqovl (0 if FAKE present, else >0).")
                return False
