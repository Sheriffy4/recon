...
class AsyncOptimizer:
    ...
    def __init__(self, max_workers: int = 4, connection_pool_size: int = 20):
        ...
        self.thread_pool = concurrent.futures.ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="AsyncOptimizer"
        )
        ...
        async with self._session_lock:
            if self.http_session is None or self.http_session.closed:
                ...
                self.http_session = aiohttp.ClientSession(
                    connector=connector,
                    timeout=timeout,
                    headers={"User-Agent": "recon-async-optimizer/1.0"},
                )
    ...
    async def execute(self) -> List[Any]:
        ...
        async with async_timeout(self.timeout_seconds):
            return await optimizer.batch_operations(
                self.operations, max_concurrent=self.max_concurrent
            )
...
class AsyncUtils:
    ...
    async def gather_with_limit(
        *operations: Awaitable, limit: int = 10, return_exceptions: bool = True
    ) -> List[Any]:
