...
class CLIValidationOrchestrator:
    ...
    def validate_strategy(
        self, strategy: Dict[str, Any], check_attack_availability: bool = True
        ...
        result = StrategyValidationResult(passed=True, strategy=strategy)
            ...
            if attack_info:
                ...
                for param_name, param_value in strategy.items():
                    ...
                    if param_info:
                        ...
                        param_errors = validator._validate_parameter(
                            param_name, param_value, attack_type
                        )
                        if param_errors:
                ...
                result.warnings.append(
                    f"No attack info found for '{attack_type}', skipping parameter validation"
                )
    ...
    def compare_with_baseline(
        self, current_results: List[Dict[str, Any]], baseline_name: Optional[str] = None
    ) -> ComparisonResult:
        """
        ...
        comparison = self.baseline_manager.compare_with_baseline(
            current_report, baseline_name=baseline_name
        )
    ...
    def format_validation_output(
        ...
        overall_status = (
            f"{GREEN}{CHECK} ALL VALIDATIONS PASSED{RESET}"
            if all_passed
            else f"{RED}{CROSS} VALIDATION FAILURES DETECTED{RESET}"
        )
        ...
        if report.pcap_validation:
            ...
            status = (
                f"{GREEN}{CHECK} PASSED{RESET}"
                if report.pcap_validation.passed
                else f"{RED}{CROSS} FAILED{RESET}"
            )
        ...
        if report.strategy_validation:
            ...
            status = (
                f"{GREEN}{CHECK} PASSED{RESET}"
                if report.strategy_validation.passed
                else f"{RED}{CROSS} FAILED{RESET}"
            )

            lines.append(f"  {BOLD}Status:{RESET} {status}")
            lines.append(
                f"  {BOLD}Strategy Type:{RESET} {report.strategy_validation.strategy.get('type', 'unknown')}"
            )
        ...
        if report.baseline_comparison:
            ...
            lines.append(
                f"  {BOLD}Regressions:{RESET} {regressions_color}{regressions_count}{RESET}"
            )
            lines.append(
    ...
    def _save_comparison_report(self, comparison: ComparisonResult, output_path: Path):
    ...
    def save_validation_report_json(
        self, report: CLIValidationReport, output_path: Optional[Path] = None
    ) -> Path:
        """
    ...
    def format_validation_output_rich(self, report: CLIValidationReport, console=None) -> None:
        ...
        try:
            ...
            if report.strategy_validation:
                ...
                strategy_table.add_row(
                    "Warnings",
                    f"[{warnings_color}]{len(report.strategy_validation.warnings)}[/{warnings_color}]",
                )
            ...
            if report.baseline_comparison:
                ...
                baseline_table.add_row(
                    "Regressions",
                    f"[{regressions_color}]{len(report.baseline_comparison.regressions)}[/{regressions_color}]",
                )
                ...
                if report.baseline_comparison.regressions:
                    ...
                    for reg in report.baseline_comparison.regressions:
                        console.print(
                            f"  [red][{reg.severity.value.upper()}][/red] {reg.attack_name}: {reg.description}"
                        )
...
def create_cli_validator(
    baselines_dir: Optional[Path] = None, output_dir: Optional[Path] = None
