...
class DPIChangeNotifier:
    ...
    def _setup_notification_channels(self):
        ...
        if channels_config.get("webhook", {}).get("enabled", False):
            ...
            self.notification_channels.append(
                WebhookNotificationChannel(webhook_config)
            )
        ...
        if channels_config.get("file", {}).get("enabled", True):
            ...
            self.notification_channels.append(
                FileNotificationChannel(file_config)
            )
    ...
    async def detect_and_notify_changes(self, domain: str, current_state: Dict[str, Any], 
                                      source: str = "unknown"):
    ...
    def _detect_changes(self, domain: str, old_state: Dict[str, Any], 
                       new_state: Dict[str, Any], source: str) -> List[DPIChangeEvent]:
    ...
    async def _send_notifications(self, change: DPIChangeEvent):
        ...
        for channel in self.notification_channels:
            task = asyncio.create_task(
                channel.send_notification(change)
            )
    ...
    async def send_notification(self, change: DPIChangeEvent) -> bool:
        ...
        try:
            ...
            if (self.file_path.exists() and 
                self.file_path.stat().st_size > self.max_file_size_mb * 1024 * 1024):
                # Ротация файла
                backup_path = self.file_path.with_suffix(f".{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl")
...
class ConsoleNotificationChannel(NotificationChannel):
    ...
    async def send_notification(self, change: DPIChangeEvent) -> bool:
        ...
        try:
            ...
            message = (f"{icon} DPI Change: {change.change_type.value} "
