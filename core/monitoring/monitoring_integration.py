...
try:
    ...
    from core.monitoring.models import MonitoringConfig as BaseMonitoringConfig
    from core.monitoring.real_time_monitor import (
        RealTimeMonitor,
        MonitoringConfig as RTMConfig,
        TrafficEvent,
    )
...
class EnhancedMonitoringSystem:
    ...
    def __init__(
        self, config: IntegratedMonitoringConfig, adaptive_engine: Optional[AdaptiveEngine] = None
    ):
        ...
        self.base_monitor = MonitoringSystem(config.base_monitoring, enable_modern_bypass=True)

        self.real_time_monitor = RealTimeMonitor(
            config.real_time_monitoring, adaptive_engine=adaptive_engine
        )
    ...
    async def _find_correlations_for_domain(
        self, domain: str, site_info: Dict[str, Any], rtm_events: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        domain_events = [
            event
            for event in rtm_events
            if event.get("domain") == domain
            or (
                event.get("dest_ip") == site_info.get("ip")
                and event.get("dest_port") == site_info.get("port")
            )
        ]
    ...
    async def _analyze_event_correlation(
        self, rtm_event: Dict[str, Any], is_accessible: bool, consecutive_failures: int
        ...
        if (
            event_type in ["connection_blocked", "rst_injection", "tls_handshake_fail"]
        ...
        elif event_type == "connection_success" and not is_accessible and consecutive_failures >= 1:
    ...
    async def _handle_correlations(self, domain: str, correlations: List[Dict[str, Any]]):
        ...
        for correlation in correlations:
            ...
            LOG.info(
                f"🔗 Correlation found for {domain}: {correlation_type} (confidence: {confidence:.2f})"
            )

            # Выполняем рекомендуемые действия
            if (
                recommended_action == "trigger_adaptive_calibration"
                and self.config.enable_adaptive_triggers
            ):
        ...
        try:
            ...
            calibration_task = asyncio.create_task(
                self.adaptive_engine.find_best_strategy(
                    domain, progress_callback=lambda msg: LOG.info(f"Calibration {domain}: {msg}")
                )
            )
    ...
    async def _cross_validate_domain(self, domain: str, port: int, block_event: Dict[str, Any]):
        ...
        if not health.is_accessible:
            ...
            if self.config.enable_adaptive_triggers and self.adaptive_engine:
                await self._trigger_adaptive_calibration(
                    domain,
                    {
...
if __name__ == "__main__":
    ...
    async def test_integrated_monitoring():
        ...
        try:
            ...
            print(
                f"  Correlations found: {final_status['integration']['statistics']['correlation_matches']}"
            )
            print(
                f"  Adaptive triggers: {final_status['integration']['statistics']['adaptive_triggers']}"
            )
            print(
                f"  Cross-validations: {final_status['integration']['statistics']['cross_validations']}"
            )
