...
class OnlineMLPredictor:
    ...
    def extract_features(
        self, blocking_event: BlockingEvent, domain_history: Dict[str, Any]
    ) -> List[float]:
    ...
    def add_training_sample(
        self, features: List[float], strategy_type: StrategyType, success: bool
    ):
    ...
    def predict_success_rate(
        self, features: List[float], strategy_type: StrategyType
    ) -> Tuple[float, float]:
...
class ABTestManager:
    ...
    def record_test_result(self, domain: str, result: ABTestResult):
        ...
        new_avg = (old_avg * (perf["total_count"] - 1) + result.response_time_ms) / perf[
            "total_count"
        ]
...
class AdaptiveOnlineStrategyGenerator:
    ...
    def generate_strategies_for_blocking(
        self, blocking_event: BlockingEvent, count: int = 3
    ...
    def _generate_type_based_strategies(
    ...
    def _generate_ml_predicted_strategies(
        self, features: List[float], blocking_event: BlockingEvent
    ) -> List[StrategyCandidate]:
    ...
    def _create_strategy_candidate(
        self,
        strategy_type: StrategyType,
        parameters: Dict[str, Any],
        predicted_success_rate: float,
    ...
    def _generate_parameters_for_type(
        self, strategy_type: StrategyType, blocking_event: BlockingEvent
    ) -> Dict[str, Any]:
        ...
        if strategy_type == StrategyType.TCP_FRAGMENTATION:
            ...
            return {"disorder_count": random.randint(1, 3), "delay_ms": random.randint(5, 50)}

        elif strategy_type == StrategyType.TIMING_ATTACKS:
            return {"delay_ms": random.randint(10, 100), "jitter_ms": random.randint(1, 10)}
    ...
    def record_strategy_result(
        ...
        domain_hist["avg_response_time"] = (old_avg * (total - 1) + response_time_ms) / total
    ...
    def start_ab_test(
        self, domain: str, strategy_a: StrategyCandidate, strategy_b: StrategyCandidate
    ):
