...
from .real_time_traffic_analyzer import (
    RealTimeTrafficAnalyzer, BlockingEvent, BlockingType, TrafficEvent
)
from .adaptive_online_strategy_generator import (
    AdaptiveOnlineStrategyGenerator, StrategyCandidate, ABTestResult, StrategyType
)

try:
    from core.monitoring_system import MonitoringSystem, ConnectionHealth
...
class NotificationManager:
    ...
    def check_and_send_alerts(self, metrics: OnlineAnalysisMetrics, 
                            recent_events: List[BlockingEvent]):
        ...
        if len(recent_events) > 0:
            ...
            if blocking_rate > self.alert_thresholds['blocking_rate_threshold']:
                alerts.append({
                    ...
                    'details': {'blocking_rate': blocking_rate, 'events_count': len(recent_events)}
                })
        ...
        if metrics.strategies_tested > 10:
            ...
            if failure_rate > self.alert_thresholds['strategy_failure_threshold']:
                alerts.append({
                    ...
                    'details': {'failure_rate': failure_rate, 'total_tested': metrics.strategies_tested}
                })
        
        # Проверяем задержки обнаружения
        if metrics.avg_detection_time_ms > self.alert_thresholds['detection_delay_threshold'] * 1000:
            alerts.append({
                'type': 'slow_detection',
                'severity': 'warning',
...
class OnlineAnalysisIntegration:
    ...
    def __init__(self, 
        ...
        self.strategy_generator = AdaptiveOnlineStrategyGenerator(
            strategy_cache_file=self.config.get('strategy_cache_file', 'online_strategies_cache.json')
        )
    ...
    def _on_blocking_detected(self, blocking_event: BlockingEvent):
        ...
        if self.config.get('auto_strategy_switching', True):
            ...
            threading.Thread(
                target=self._handle_blocking_async,
                args=(blocking_event,),
                daemon=True
            ).start()
    ...
    def _handle_blocking_async(self, blocking_event: BlockingEvent):
        ...
        try:
            ...
            strategies = self.strategy_generator.generate_strategies_for_blocking(
                blocking_event, 
                count=self.config.get('max_strategies_per_blocking', 3)
            )
            ...
            self.metrics.avg_strategy_generation_time_ms = (
                (self.metrics.avg_strategy_generation_time_ms * 
                 (self.metrics.strategies_generated - len(strategies)) + generation_time) /
            ...
            for strategy in strategies:
                ...
                if success:
                    ...
                    self.logger.info(f"Successfully bypassed {blocking_event.domain} with {strategy.id}")
                    break
                else:
