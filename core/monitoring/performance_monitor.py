...
class PerformanceMonitor:
    ...
    def _collect_metrics(self) -> PerformanceMetrics:
        ...
        avg_fingerprint_time = (
            ...
            if fingerprint_timings else 0.0
        )
        ...
        fingerprint_success_rate = (
            fingerprint_successes / fingerprint_total
            if fingerprint_total > 0 else 0.0
        )
        ...
        try:
            active_connections = len([f for f in self.process.open_files() if f.path.startswith('/proc')])
        except:
    ...
    def record_operation(self, component: str, operation: str, duration_ms: float, success: bool, error: Optional[str] = None):
    ...
    def get_component_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for component, metrics in self.component_metrics.items():
            summary[component] = {
                "success_rate": metrics.success_count / metrics.operation_count if metrics.operation_count > 0 else 0,
                "avg_duration_ms": metrics.avg_duration_ms,
                "total_operations": metrics.operation_count,
                "last_error": metrics.last_error,
                "time_since_last_success": time.time() - metrics.last_success if metrics.last_success else None
            }
        return summary
