...
try:
    ...
    from core.monitoring.models import MonitoringConfig, ConnectionHealth
...
class EnhancedMonitoringSystem:
    ...
    def __init__(
        self, config: Optional[Dict[str, Any]] = None, enable_online_analysis: bool = True
    ):
        ...
        if MONITORING_SYSTEM_AVAILABLE:
            ...
            self.base_monitoring = MonitoringSystem(
                config=monitoring_config, enable_modern_bypass=True
            )
        else:
            ...
            try:
                self.online_analysis = OnlineAnalysisIntegration(
                    ...
                    config_file=self.config.get(
                        "online_analysis_config", "online_analysis_config.json"
                    ),
                )
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        if self.online_analysis:
            ...
            self.enhanced_metrics["total_blocking_events"] = online_metrics.get(
                "blocking_events_detected", 0
            )
            self.enhanced_metrics["automatic_recoveries"] = online_metrics.get(
                "successful_bypasses", 0
            )
            self.enhanced_metrics["online_analysis_uptime"] = online_metrics.get(
                "uptime_seconds", 0.0
            )
    ...
    def get_recent_events(self, seconds: int = 300) -> List[Dict[str, Any]]:
        ...
        if self.online_analysis:
            ...
            blocking_events = self.online_analysis.traffic_analyzer.get_recent_blocking_events(
                seconds
            )
            ...
            strategy_switches = self.online_analysis.strategy_orchestrator.get_recent_switches(
                seconds
            )
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if self.online_analysis:
            ...
            report["summary"] = {
                ...
                "avg_strategy_generation_time_ms": online_metrics.get(
                    "avg_strategy_generation_time_ms", 0.0
                ),
...
def create_enhanced_monitoring_system(
    config_file: str = "enhanced_monitoring_config.json",
    ...
    return EnhancedMonitoringSystem(
        config=config, enable_online_analysis=config.get("enable_online_analysis", True)
    )
