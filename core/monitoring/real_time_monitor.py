...
class ConnectionTracker:
    ...
    def update_connection(self, src_ip: str, src_port: int, dst_ip: str, dst_port: int, 
    ...
    def get_connection_info(self, src_ip: str, src_port: int, dst_ip: str, dst_port: int) -> Optional[Dict[str, Any]]:
...
class TrafficAnalyzer:
    ...
    def analyze_packet(self, packet) -> List[TrafficEvent]:
        ...
        try:
            ...
            self.connection_tracker.update_connection(src_ip, src_port, dst_ip, dst_port, packet_info)
            ...
            self.stats["analysis_time_ms"] = (
                (self.stats["analysis_time_ms"] * (self.stats["packets_analyzed"] - 1) + analysis_time) /
                self.stats["packets_analyzed"]
            ) if self.stats["packets_analyzed"] > 0 else analysis_time
        
        return events
    
    def _analyze_for_blocks(self, src_ip: str, src_port: int, dst_ip: str, dst_port: int, 
                           packet_info: Dict[str, Any]) -> List[TrafficEvent]:
            ...
            if connection_duration < self.config.rst_injection_threshold:
                events.append(TrafficEvent(
                    ...
                    evidence={
                        ...
                        "packets_exchanged": conn_info["packets_sent"] + conn_info["packets_received"]
                    }
                ))
        ...
        if packet_info["has_tls"] and conn_info["tls_handshake_started"]:
            ...
            if (connection_duration > 5.0 and 
                not conn_info["tls_handshake_completed"] and 
                conn_info["packets_sent"] > 3):
...
class RealTimeMonitor:
    ...
    async def _handle_traffic_event(self, event: TrafficEvent):
        ...
        if event.event_type in [TrafficEventType.CONNECTION_BLOCKED, 
    ...
    async def _handle_blocking_event(self, event: TrafficEvent):
        ...
        LOG.warning(f"🚨 Detected blocking: {event.block_type.value if event.block_type else 'unknown'} "
        ...
        if (self.config.auto_trigger_calibration and 
            self.adaptive_engine and 
            event.domain):
    ...
    async def _cleanup_loop(self):
        ...
        while self.is_running:
            try:
                ...
                old_notifications = [
                    key for key, timestamp in self.notification_cache.items()
                ...
                old_blocks = [
                    key for key, event in self.detected_blocks.items()
                ...
                if old_notifications or old_blocks:
                    LOG.debug(f"Cleaned up {len(old_notifications)} notifications and {len(old_blocks)} block events")
    ...
    def get_status_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                **self.monitor_stats,
                **self.traffic_analyzer.get_statistics()
            },
            "current_state": {
...
class MonitoringSystemIntegration:
    ...
    def get_combined_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "integration": {
                "active": True,
                "synchronized_domains": 0  # Можно добавить подсчет
            }
        }
        ...
        try:
            ...
            for i in range(30):  # 30 секунд
                ...
                if i % 10 == 0:
                    ...
                    print(f"Status: {status['statistics']['packets_captured']} packets captured, "
