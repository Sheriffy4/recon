...
class RealTimeTrafficAnalyzer:
    ...
    def _cleanup_old_connections(self):
        ...
        old_flows = [
            flow_id for flow_id, flow in self.connection_flows.items()
                    ...
                    if suspicion_score > 0.6:
                        alert = TrafficAlert(
                            ...
                            details={
                                ...
                                "flow_duration": (flow.last_activity - flow.start_time).total_seconds()
                            },
        ...
        try:
            ...
            for packet in dns_packets:
                if "DNS Response" in packet.payload_snippet:
                    ...
                    if "NXDOMAIN" in packet.payload_snippet or "127.0.0.1" in packet.payload_snippet:
                        alert = TrafficAlert(
    ...
    def _detect_suspicious_redirect(self, packets: List[PacketInfo]) -> List[TrafficAlert]:
        ...
        try:
            http_packets = [p for p in packets if p.protocol == "TCP" and 
                          (p.dst_port == 80 or p.src_port == 80) and 
                          "HTTP" in p.payload_snippet]
            
            for packet in http_packets:
                if any(code in packet.payload_snippet for code in ["301", "302", "303"]):
                    # Проверяем на подозрительные редиректы
                    if any(keyword in packet.payload_snippet.lower() 
    ...
    def _handle_alert(self, alert: TrafficAlert):
        ...
        try:
            ...
            # Логируем алерт
            LOG.info(f"🚨 {alert.event_type.value}: {alert.target_ip} "
    ...
    def _is_alert_in_cooldown(self, alert: TrafficAlert) -> bool:
        ...
        for recent_alert in self.recent_alerts:
            if (recent_alert.event_type == alert.event_type and
                recent_alert.target_ip == alert.target_ip and
    ...
    def remove_event_handler(self, event_type: TrafficEvent, handler: Callable[[TrafficAlert], None]):
