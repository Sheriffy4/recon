...
class ClosedLoopManager(IClosedLoopManager):
    ...
    def __init__(self, fingerprint_engine: IFingerprintEngine, strategy_generator: IStrategyGenerator, effectiveness_tester: IEffectivenessTester, learning_memory: ILearningMemory, attack_adapter: IAttackAdapter, strategy_saver: IStrategySaver, failure_analyzer: Optional[FailureAnalyzer]=None):
        """
    ...
    async def run_closed_loop(self, domain: str, port: int=443, max_iterations: Optional[int]=None) -> ClosedLoopResult:
        """
        ...
        self.logger.info(f'Configuration: max_iterations={self.max_iterations}, convergence_threshold={self.convergence_threshold}')
        result = ClosedLoopResult(domain=domain, total_iterations=0, final_effectiveness=0.0, best_strategy=None)
        self.current_iteration = 0
        self.best_effectiveness = 0.0
        self.baseline_effectiveness = 0.0
        self.iterations_without_improvement = 0
        ...
        try:
            ...
            self.logger.info(f'Baseline established: success={baseline_result.success}, latency={baseline_result.latency_ms:.1f}ms')
            ...
            current_fingerprint = await self.fingerprint_engine.create_comprehensive_fingerprint_with_extended_metrics(domain=domain, force_refresh=True)
            behavioral_profile = await self.fingerprint_engine.analyze_dpi_behavior(domain, current_fingerprint)
            self.logger.info(f'Behavioral profile created: {len(behavioral_profile.identified_weaknesses)} weaknesses identified')
            fingerprint_hash = self.learning_memory._generate_fingerprint_hash(current_fingerprint.__dict__)
            learning_history = await self.learning_memory.load_learning_history(fingerprint_hash)
            if learning_history:
                self.logger.info(f'Loaded learning history: {len(learning_history.successful_attacks)} successful attacks, best effectiveness: {learning_history.best_effectiveness:.2f}')
            for iteration in range(self.max_iterations):
                self.current_iteration = iteration + 1
                self.logger.info(f'=== Closed Loop Iteration {self.current_iteration}/{self.max_iterations} ===')
                self.logger.info('=== PHASE 2: PLAN ===')
                iteration_result = await self._run_enhanced_iteration(domain, port, current_fingerprint, behavioral_profile, learning_history, baseline_result, cumulative_failure_analysis)
                ...
                if iteration_result.failure_analysis:
                    cumulative_failure_analysis = await self._integrate_failure_analysis(cumulative_failure_analysis, iteration_result.failure_analysis)
                    self.logger.info(f'Failure analysis: {len(iteration_result.failure_analysis.failure_patterns)} patterns identified')
                if iteration_result.best_effectiveness > self.best_effectiveness:
                    ...
                    self.iterations_without_improvement = 0
                    self.logger.info(f'New best effectiveness: {self.best_effectiveness:.2f} (improvement: +{improvement:.2f})')
                else:
                    self.iterations_without_improvement += 1
                    self.logger.info(f'No improvement in iteration {self.current_iteration} ({self.iterations_without_improvement} consecutive)')
                ...
                if iteration < self.max_iterations - 1:
                    ...
                    current_fingerprint = await self.fingerprint_engine.refine_fingerprint(current_fingerprint, getattr(iteration_result, 'test_results', []), learning_insights={'failure_patterns': cumulative_failure_analysis.failure_patterns if cumulative_failure_analysis else [], 'successful_techniques': [r.bypass.attack_name for r in all_test_results if getattr(r, 'bypass_effective', False)], 'behavioral_insights': behavioral_profile.__dict__})
                    behavioral_profile = await self.fingerprint_engine.analyze_dpi_behavior(domain, current_fingerprint)
                    await self._update_learning_memory_with_behavioral_insights(fingerprint_hash, behavioral_profile, iteration_result)
                    self.logger.info('Fingerprint and behavioral profile refined for next iteration')
            result.total_time_seconds = time.time() - start_time
            result.convergence_achieved = self.best_effectiveness >= self.convergence_threshold
            await self._generate_final_analysis_summary(result, behavioral_profile, cumulative_failure_analysis)
            self.logger.info(f'Enhanced closed loop completed: {result.total_iterations} iterations, final effectiveness: {result.final_effectiveness:.2f}, time: {result.total_time_seconds:.1f}s')
            return result
    ...
    async def _run_enhanced_iteration(self, domain: str, port: int, fingerprint: EnhancedFingerprint, behavioral_profile: Any, learning_history: Optional[Any], baseline_result: Any, previous_failure_analysis: Optional[FailureAnalysisResult]=None) -> ClosedLoopIteration:
        """
        ...
        iteration_result = ClosedLoopIteration(iteration_number=self.current_iteration, fingerprint=fingerprint, strategies_generated=0, strategies_tested=0, best_effectiveness=0.0, improvement_over_baseline=0.0)
        try:
            if previous_failure_analysis:
                self.logger.info('Generating strategies with failure analysis insights')
                strategies = self.strategy_generator.generate_strategies_with_failure_analysis(count=self.strategies_per_iteration, failure_analysis=previous_failure_analysis, use_parameter_ranges=True)
            else:
                self.logger.info('Generating strategies with behavioral profile insights')
                strategies = self.strategy_generator.generate_strategies(count=self.strategies_per_iteration, use_parameter_ranges=True)
            iteration_result.strategies_generated = len(strategies)
            prioritized_strategies = self._prioritize_strategies_by_behavioral_profile(strategies, behavioral_profile)
            ...
            self.logger.info(f'Generated {iteration_result.strategies_generated} strategies, testing top {iteration_result.strategies_tested}')
            self.logger.info('=== PHASE 3: EXECUTE ===')
            test_results = []
            best_strategy = None
            for i, strategy in enumerate(strategies_to_test):
                self.logger.info(f"Testing strategy {i + 1}/{len(strategies_to_test)}: {strategy['name']}")
                try:
                    attack_result = await self.attack_adapter.execute_attack(strategy['name'], strategy.get('params', {}), domain, port)
                    effectiveness_result = await self.effectiveness_tester.test_with_bypass(domain, port, attack_result)
                    test_results.append(effectiveness_result)
                    if effectiveness_result.effectiveness_score > iteration_result.best_effectiveness:
                        iteration_result.best_effectiveness = effectiveness_result.effectiveness_score
                        best_strategy = strategy
                        self.logger.info(f"New best in iteration: {effectiveness_result.effectiveness_score:.2f} with {strategy['name']}")
                except Exception as e:
            ...
            iteration_result.improvement_over_baseline = iteration_result.best_effectiveness - self.baseline_effectiveness
            if test_results:
                failure_analysis = await self.failure_analyzer.analyze_iteration_results(test_results, fingerprint, behavioral_profile)
                iteration_result.failure_analysis = failure_analysis
                strategic_recommendations = await self._generate_strategic_recommendations(failure_analysis, behavioral_profile, learning_history)
                iteration_result.strategic_recommendations = strategic_recommendations
                iteration_result.analysis_notes.extend([f'Tested {len(test_results)} strategies', f'Best effectiveness: {iteration_result.best_effectiveness:.2f}', f'Improvement over baseline: {iteration_result.improvement_over_baseline:.2f}', f'Failure patterns identified: {len(failure_analysis.failure_patterns)}', f'Strategic recommendations: {len(strategic_recommendations)}'])
                for pattern in failure_analysis.failure_patterns[:3]:
                    self.logger.info(f'Failure pattern: {pattern.pattern_type} affecting {len(pattern.affected_techniques)} techniques')
            ...
            if learning_history and test_results:
                await self._update_learning_memory_from_iteration(fingerprint, test_results, iteration_result)
            iteration_time = time.time() - iteration_start
            self.logger.info(f'Iteration {self.current_iteration} completed in {iteration_time:.1f}s: best_effectiveness={iteration_result.best_effectiveness:.2f}')
    ...
    async def _run_single_iteration(self, domain: str, port: int, fingerprint: EnhancedFingerprint, learning_history: Optional[Any], baseline_result: Any) -> ClosedLoopIteration:
        """
        ...
        if learning_history:
            for attack_name, success_rate in learning_history.successful_attacks.items():
                history_list.append({'attack_name': attack_name, 'success_rate': success_rate, 'parameters': learning_history.optimal_parameters.get(attack_name, {})})
        strategy_gen = AdvancedStrategyGenerator(fingerprint_dict=fingerprint_dict, history=history_list, doh_success=False)
        ...
        if previous_failure_analysis:
            strategies = strategy_gen.generate_strategies_with_failure_analysis(count=self.strategies_per_iteration, failure_analysis=previous_failure_analysis, use_parameter_ranges=True)
        ...
        self.logger.info(f'Generated {len(strategies)} strategies for iteration {self.current_iteration}')
        tested_strategies = 0
        ...
        for i, strategy in enumerate(strategies_to_test):
            try:
                self.logger.debug(f"Testing strategy {i + 1}/{len(strategies_to_test)}: {strategy['name']}")
                adaptive_attack = LearningAdaptiveAttack(effectiveness_tester=self.effectiveness_tester, learning_memory=self.learning_memory)
                attack_result = await self._execute_strategy_with_adaptive_attack(domain, port, strategy, adaptive_attack, fingerprint)
                if attack_result:
        ...
        if effectiveness_results:
            avg_effectiveness = sum((r.effectiveness_score for r in effectiveness_results)) / len(effectiveness_results)
            analysis_notes.append(f'Tested {tested_strategies} strategies, avg effectiveness: {avg_effectiveness:.2f}')
            failure_analysis = self.failure_analyzer.analyze_closed_loop_failures(effectiveness_results)
            strategic_recommendations = failure_analysis.strategic_recommendations
            if failure_analysis.detected_patterns:
                self.logger.info(f'Detected {len(failure_analysis.detected_patterns)} failure patterns')
                for pattern in failure_analysis.detected_patterns:
                    self.logger.debug(f'Pattern: {pattern.pattern_type} (confidence: {pattern.confidence:.2f})')
            if failure_analysis.dpi_behavior_insights:
                ...
                if 'recommended_classification' in insights:
                    analysis_notes.append(f"DPI classification: {insights['recommended_classification']}")
            fingerprint_hash = self.learning_memory._generate_fingerprint_hash(fingerprint_dict)
            for result in effectiveness_results:
                await self.learning_memory.save_learning_result(fingerprint_hash=fingerprint_hash, attack_name=result.bypass.attack_name or 'unknown', effectiveness_score=result.effectiveness_score, parameters={}, success=result.bypass_effective, latency_ms=result.bypass.latency_ms)
        else:
            analysis_notes.append('No strategies could be tested successfully')
            failure_analysis = FailureAnalysisResult(total_failures=tested_strategies, failure_breakdown={}, detected_patterns=[], strategic_recommendations=['No test results available - check network connectivity'])
        improvement_over_baseline = best_effectiveness - self.baseline_effectiveness
        iteration_result = ClosedLoopIteration(iteration_number=self.current_iteration, fingerprint=fingerprint, strategies_generated=len(strategies), strategies_tested=tested_strategies, best_effectiveness=best_effectiveness, improvement_over_baseline=improvement_over_baseline, analysis_notes=analysis_notes, failure_analysis=failure_analysis, strategic_recommendations=strategic_recommendations, timestamp=time.time())
        self._previous_failure_analysis = failure_analysis
        iteration_time = time.time() - iteration_start
        self.logger.info(f'Iteration {self.current_iteration} completed in {iteration_time:.1f}s: best_effectiveness={best_effectiveness:.2f}, improvement={improvement_over_baseline:.2f}')
        if strategic_recommendations:
            self.logger.info(f"Strategic recommendations: {'; '.join(strategic_recommendations[:3])}")
        return iteration_result

    async def _execute_strategy_with_adaptive_attack(self, domain: str, port: int, strategy: Dict[str, Any], adaptive_attack: LearningAdaptiveAttack, fingerprint: EnhancedFingerprint) -> Optional[EffectivenessResult]:
        """
    ...
    async def should_continue_loop(self, current_results: ClosedLoopResult) -> Tuple[bool, str]:
        ...
        if self.best_effectiveness >= self.convergence_threshold:
            return (False, f'Convergence achieved (effectiveness: {self.best_effectiveness:.2f})')
        ...
        if self.iterations_without_improvement >= self.max_iterations_without_improvement:
            return (False, f'No improvement for {self.iterations_without_improvement} iterations')
        if self.current_iteration >= 2 and self.best_effectiveness <= self.baseline_effectiveness + self.improvement_threshold:
            return (False, f'Insufficient improvement over baseline ({self.best_effectiveness:.2f} vs {self.baseline_effectiveness:.2f})')
        return (True, 'Continuing optimization')

    async def _refine_fingerprint_from_results(self, current_fingerprint: EnhancedFingerprint, test_results: List[EffectivenessResult]) -> EnhancedFingerprint:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {'current_iteration': self.current_iteration, 'best_effectiveness': self.best_effectiveness, 'baseline_effectiveness': self.baseline_effectiveness, 'iterations_without_improvement': self.iterations_without_improvement, 'configuration': {'max_iterations': self.max_iterations, 'convergence_threshold': self.convergence_threshold, 'improvement_threshold': self.improvement_threshold, 'strategies_per_iteration': self.strategies_per_iteration, 'max_strategies_to_test': self.max_strategies_to_test}}

    def _prioritize_strategies_by_behavioral_profile(self, strategies: List[Dict[str, Any]], behavioral_profile: Any) -> List[Dict[str, Any]]:
        """
        ...
        def calculate_strategy_priority(strategy: Dict[str, Any]) -> float:
            ...
            for weakness in behavioral_profile.identified_weaknesses:
                if 'fragmentation' in weakness.lower() and 'fragmentation' in strategy_name.lower():
                    ...
                    priority_score += 0.3
                elif 'checksum' in weakness.lower() and ('badsum' in strategy_name.lower() or 'checksum' in strategy_name.lower()):
                    priority_score += 0.3
            ...
            if not behavioral_profile.checksum_validation and 'badsum' in strategy_name.lower():
                priority_score += 0.2
            if behavioral_profile.ech_support is False and ('ech' in strategy_name.lower() or 'modern' in strategy_name.lower()):
                priority_score += 0.2
            if behavioral_profile.ml_detection and 'mimicry' in strategy_name.lower():
                priority_score += 0.3
            return min(priority_score, 1.0)
        strategy_priorities = [(strategy, calculate_strategy_priority(strategy)) for strategy in strategies]
    ...
    async def _integrate_failure_analysis(self, cumulative_analysis: Optional[FailureAnalysisResult], new_analysis: FailureAnalysisResult) -> FailureAnalysisResult:
        """
        ...
        for new_pattern in new_analysis.failure_patterns:
            ...
            for existing_pattern in merged_patterns:
                if existing_pattern.pattern_type == new_pattern.pattern_type and len(set(existing_pattern.affected_techniques) & set(new_pattern.affected_techniques)) > 0:
                    existing_pattern.affected_techniques.extend(new_pattern.affected_techniques)
                    existing_pattern.affected_techniques = list(set(existing_pattern.affected_techniques))
                    existing_pattern.confidence = max(existing_pattern.confidence, new_pattern.confidence)
                    similar_found = True
                    break
            if not similar_found:
                merged_patterns.append(new_pattern)
        merged_analysis = FailureAnalysisResult(failure_patterns=merged_patterns, next_iteration_focus=new_analysis.next_iteration_focus, strategic_insights=cumulative_analysis.strategic_insights + new_analysis.strategic_insights, confidence_score=max(cumulative_analysis.confidence_score, new_analysis.confidence_score))
        return merged_analysis

    async def _generate_strategic_recommendations(self, failure_analysis: FailureAnalysisResult, behavioral_profile: Any, learning_history: Optional[Any]) -> List[str]:
        """
        ...
        for pattern in failure_analysis.failure_patterns:
            if pattern.pattern_type == 'consistent_blocking':
                recommendations.append(f'Consider alternative attack categories - {len(pattern.affected_techniques)} techniques consistently blocked')
            elif pattern.pattern_type == 'timing_sensitive':
                recommendations.append('Focus on timing-based attacks - DPI shows timing sensitivity')
        ...
        if len(behavioral_profile.anti_evasion_techniques) >= 3:
            recommendations.append('Use multi-layer combination attacks against sophisticated anti-evasion')
        if learning_history and hasattr(learning_history, 'successful_attacks'):
            ...
            if successful_categories:
                recommendations.append(f"Focus on previously successful categories: {', '.join(successful_categories)}")
        return recommendations[:5]

    async def _update_learning_memory_with_behavioral_insights(self, fingerprint_hash: str, behavioral_profile: Any, iteration_result: ClosedLoopIteration):
        """
        ...
        """
        try:
            behavioral_insights = {'dpi_capabilities': {'supports_ip_frag': behavioral_profile.supports_ip_frag, 'checksum_validation': behavioral_profile.checksum_validation, 'rst_latency_ms': behavioral_profile.rst_latency_ms, 'ech_support': behavioral_profile.ech_support}, 'detection_sophistication': {'ml_detection': behavioral_profile.ml_detection, 'behavioral_analysis': behavioral_profile.behavioral_analysis, 'statistical_analysis': behavioral_profile.statistical_analysis}, 'identified_weaknesses': behavioral_profile.identified_weaknesses, 'anti_evasion_techniques': behavioral_profile.anti_evasion_techniques}
            await self.learning_memory.store_behavioral_insights(fingerprint_hash, behavioral_insights)
            self.logger.debug(f'Updated learning memory with behavioral insights for {fingerprint_hash}')
        except Exception as e:
            self.logger.error(f'Failed to update learning memory with behavioral insights: {e}')

    async def _update_learning_memory_from_iteration(self, fingerprint: EnhancedFingerprint, test_results: List[Any], iteration_result: ClosedLoopIteration):
        """
        ...
        try:
            ...
            for result in test_results:
                if hasattr(result, 'bypass_effective') and result.bypass_effective:
                    attack_name = result.bypass.attack_name
                    effectiveness = result.effectiveness_score
                    await self.learning_memory.save_learning_result(fingerprint_hash, attack_name, effectiveness, getattr(result.bypass, 'parameters', {}))
            if iteration_result.strategic_recommendations:
                await self.learning_memory.store_strategic_insights(fingerprint_hash, iteration_result.strategic_recommendations)
            self.logger.debug(f'Updated learning memory from iteration {iteration_result.iteration_number}')
        except Exception as e:
            self.logger.error(f'Failed to update learning memory from iteration: {e}')

    async def _generate_final_analysis_summary(self, result: ClosedLoopResult, behavioral_profile: Any, cumulative_failure_analysis: Optional[FailureAnalysisResult]):
        """
        ...
        """
        try:
            result.analysis_summary.append(f'DPI Behavioral Profile: {len(behavioral_profile.identified_weaknesses)} weaknesses, {len(behavioral_profile.anti_evasion_techniques)} anti-evasion techniques')
            if cumulative_failure_analysis:
                result.analysis_summary.append(f'Failure Analysis: {len(cumulative_failure_analysis.failure_patterns)} patterns identified')
                for pattern in cumulative_failure_analysis.failure_patterns[:3]:
                    result.analysis_summary.append(f'  - {pattern.pattern_type}: {len(pattern.affected_techniques)} techniques affected')
            if result.convergence_achieved:
                result.analysis_summary.append(f'Convergence achieved with {result.final_effectiveness:.2f} effectiveness')
            else:
                result.analysis_summary.append(f'Convergence not achieved - final effectiveness: {result.final_effectiveness:.2f}')
            if result.iterations:
                best_iteration = max(result.iterations, key=lambda x: x.best_effectiveness)
                result.analysis_summary.append(f'Best iteration: #{best_iteration.iteration_number} with {best_iteration.best_effectiveness:.2f} effectiveness')
