...
class StrategyPredictionIntegrator:
    ...
    def __init__(
        ...
        self.enable_fingerprinting = (
            enable_fingerprinting and FINGERPRINT_INTEGRATION_AVAILABLE
        )
        self.enable_evolutionary = (
            enable_evolutionary and EVOLUTIONARY_OPTIMIZATION_AVAILABLE
        )
        self.predictor = None
        ...
        if self.enable_evolutionary:
            try:
                ...
                LOG.info(
                    "Evolutionary optimization integrator initialized successfully"
                )
            except Exception as e:
    ...
    def predict_best_strategy(
        ...
        dpi_profile: Optional[DPIBehaviorProfile] = None,
        ...
        if self.enable_evolutionary and self.evolutionary_integrator and domain:
            try:
                optimized_strategy = (
                    self.evolutionary_integrator.get_optimized_strategy(domain)
                )
                ...
                elif self.evolutionary_integrator.suggest_optimization_for_domains(
                    [domain]
                ):
                    self.evolutionary_integrator.start_background_optimization(
                        domains=[domain], target_ips={target_ip}
                    )
                    LOG.debug(
                        f"Started background evolutionary optimization for {domain}"
                    )
            except Exception as e:
        ...
        if self.enable_fingerprinting and self.fingerprint_integrator and domain:
            try:
                cached_fp = self.fingerprint_integrator.get_cached_fingerprint(
                    domain, target_ip
                )
                if cached_fp:
                    ...
                    self.fingerprint_integrator.start_background_fingerprinting(
                        domain, target_ip
                    )
                    LOG.debug(
        ...
        if self.enable_ml and self.predictor and enhanced_dpi_profile:
            try:
                try:
                    from core.integration.performance_integration import (
                    ...
                    pass
                prediction = self.predictor.predict_strategy_categories(
                    enhanced_dpi_profile
                )
                ...
                LOG.info(
                    f"ML strategy prediction for {target_ip}: {recommendation.primary_strategy} (confidence: {recommendation.confidence:.2f})"
                )
                return recommendation
            except Exception as e:
                LOG.error(f"ML prediction failed for {target_ip}: {e}")
        recommendation = self._rule_based_prediction(
            target_ip, enhanced_fingerprint or fingerprint
        )
        self.cache[cache_key] = (recommendation, time.time())
        LOG.info(
            f"Rule-based strategy prediction for {target_ip}: {recommendation.primary_strategy}"
        )
        return recommendation

    def _convert_ml_prediction(
        ...
        primary_category = (
            prediction.recommended_categories[0]
            if prediction.recommended_categories
            else "tcp_segmentation"
        )
        primary_strategy = category_to_method.get(
            primary_category, "tcp_window_scaling"
        )
        fallback_strategies = []
    ...
    def _rule_based_prediction(
        self, target_ip: str, fingerprint: Optional[Dict]
    ) -> StrategyRecommendation:
    ...
    def _convert_optimized_strategy(
        self, optimized_strategy: Dict[str, Any], domain: str
    ) -> StrategyRecommendation:
    ...
    def update_strategy_effectiveness(
        self, target_ip: str, strategy: str, success: bool, latency_ms: float = 0
    ):
        """Update strategy effectiveness data for learning."""
        LOG.info(
            f"Strategy feedback for {target_ip}: {strategy} -> {('success' if success else 'failure')} (latency: {latency_ms}ms)"
        )
