...
class EvolutionaryOptimizationIntegrator:
    ...
    def start_background_optimization(self, 
        ...
        for existing_task in self.optimization_tasks.values():
            if (existing_task.status == 'running' and 
                set(existing_task.domains) == set(domains)):
    ...
    async def _run_background_optimization(self, task_id: str):
        ...
        try:
            ...
            fingerprint_dict = {
                domain: {"dpi_type": "generic", "confidence": 0.5}
                for domain in domains
            }
            ...
            LOG.info(f"Evolutionary optimization completed for task {task_id}. "
    ...
    def cleanup_old_tasks(self, max_age_hours: int = 24):
        ...
        for task_id, task in self.optimization_tasks.items():
            if (task.status in ['completed', 'failed'] and 
                task.start_time and task.start_time < cutoff_time):
    ...
    def suggest_optimization_for_domains(self, domains: List[str]) -> bool:
        ...
        for task in self.optimization_tasks.values():
            if (task.status in ['pending', 'running'] and 
                any(domain in task.domains for domain in domains)):
                return False
...
class SimplifiedStrategyGenerator:
    ...
    def generate_strategies(self, count: int = 10) -> List[Dict[str, Any]]:
        ...
        strategy_templates = [
            ...
            {"name": "tcp_timestamp_manipulation", "params": {"timestamp_offset": 1000}},
