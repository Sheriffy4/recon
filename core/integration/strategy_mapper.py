# recon/core/integration/strategy_mapper.py
"""
Strategy Mapper
...
from .integration_config import AttackMapping, StrategyMappingError
...
class StrategyMapper:
    ...
    def __init__(self, mapping_file: Optional[str] = None):
        ...
        # Initialize parameter converters
    ...
    def map_legacy_strategy(self, strategy: Dict[str, Any]) -> List[AttackMapping]:
        ...
        try:
            ...
            # If all else fails, suggest generic attacks
    ...
    def convert_parameters(
        ...
        try:
            ...
            # Get converter for this attack
            converter = self.parameter_converters.get(
                attack_name, self._default_parameter_converter
            )
            ...
            LOG.debug(
                f"Converted {len(legacy_params)} legacy params to {len(converted)} attack params"
            )
            return converted
    ...
    def suggest_alternatives(self, failed_strategy: Dict[str, Any]) -> List[str]:
        ...
        # Get alternatives based on strategy type
        alternatives = []
        ...
        # Add protocol-specific alternatives
        if protocol == "tcp":
    ...
    def _load_default_mappings(self):
        ...
        # TCP Segmentation mappings
        self.mapping_rules["tcp_segment"] = AttackMapping(
            legacy_strategy="tcp_segment",
            attack_names=["tcp_basic_segmentation"],
            parameter_mapping={"size": "segment_size", "delay": "segment_delay"},
            confidence=0.9,
            fallback_attacks=["tcp_random_segmentation", "tcp_timing_manipulation"],
        )

        self.mapping_rules["tcp_split"] = AttackMapping(
            legacy_strategy="tcp_split",
            attack_names=["tcp_random_segmentation"],
            parameter_mapping={"parts": "segment_count", "delay": "segment_delay"},
            confidence=0.85,
            fallback_attacks=["tcp_basic_segmentation"],
        )

        # TCP Manipulation mappings
        self.mapping_rules["tcp_flags"] = AttackMapping(
            legacy_strategy="tcp_flags",
            attack_names=["tcp_flag_manipulation"],
            parameter_mapping={"flags": "tcp_flags", "window": "window_size"},
            confidence=0.9,
            fallback_attacks=["tcp_window_manipulation"],
        )

        # IP Fragmentation mappings
        self.mapping_rules["ip_fragment"] = AttackMapping(
            legacy_strategy="ip_fragment",
            attack_names=["ip_basic_fragmentation"],
            parameter_mapping={"size": "fragment_size", "overlap": "fragment_overlap"},
            confidence=0.9,
            fallback_attacks=["ip_random_fragmentation"],
        )

        # TLS mappings
        self.mapping_rules["tls_split"] = AttackMapping(
            legacy_strategy="tls_split",
            attack_names=["tls_record_split"],
            parameter_mapping={"size": "record_size"},
            confidence=0.85,
            fallback_attacks=["tls_sni_manipulation"],
        )

        # HTTP mappings
        self.mapping_rules["http_case"] = AttackMapping(
            legacy_strategy="http_case",
            attack_names=["http_header_case"],
            parameter_mapping={"case_type": "case_strategy"},
            confidence=0.9,
            fallback_attacks=["http_method_case"],
        )

        # Payload mappings
        self.mapping_rules["payload_encrypt"] = AttackMapping(
            legacy_strategy="payload_encrypt",
            attack_names=["payload_xor_encryption"],
            parameter_mapping={"key": "encryption_key"},
            confidence=0.8,
            fallback_attacks=["payload_base64_obfuscation"],
        )

        # Combo mappings
        self.mapping_rules["combo_tcp_http"] = AttackMapping(
            legacy_strategy="combo_tcp_http",
            attack_names=["tcp_http_combo"],
            parameter_mapping={"tcp_size": "segment_size", "http_case": "header_case"},
            confidence=0.85,
            fallback_attacks=["tcp_basic_segmentation", "http_header_case"],
        )

        # Zapret strategy mappings
        self.mapping_rules["zapret_strategy"] = AttackMapping(
            legacy_strategy="zapret_strategy",
            attack_names=["zapret_strategy"],
            parameter_mapping={
                "split_seqovl": "split_seqovl",
                "ttl": "base_ttl",
                "repeats": "repeats",
                "autottl": "auto_ttl",
                "fake_tls": "fake_tls_data",
                "fooling": "fooling_method",
            },
            confidence=0.95,
            fallback_attacks=["tcp_basic_segmentation", "tcp_flag_manipulation"],
        )

        # TTL fake race mapping (legacy zapret mapping)
        self.mapping_rules["ttl_fake_race"] = AttackMapping(
            legacy_strategy="ttl_fake_race",
            attack_names=["zapret_strategy"],
            parameter_mapping={
                "split_seqovl": "split_seqovl",
                "ttl": "base_ttl",
    ...
    def _load_mapping_file(self, mapping_file: str):
        ...
        try:
            ...
            for strategy_name, mapping_data in data.items():
                self.mapping_rules[strategy_name] = AttackMapping(
                    legacy_strategy=strategy_name,
                    attack_names=mapping_data["attack_names"],
                    parameter_mapping=mapping_data.get("parameter_mapping", {}),
                    confidence=mapping_data.get("confidence", 0.5),
                    fallback_attacks=mapping_data.get("fallback_attacks", []),
                )
    ...
    def _try_fallback_mapping(
        ...
        fallback_mappings = {
            "tcp": ["tcp_basic_segmentation"],
            "udp": ["dns_subdomain_tunneling"],
            "icmp": ["icmp_data_tunneling"],
            "http": ["http_header_case"],
            "tls": ["tls_record_split"],
        }

        if protocol in fallback_mappings:
            return [
                AttackMapping(
                    legacy_strategy=f"fallback_{protocol}",
                    attack_names=fallback_mappings[protocol],
                    parameter_mapping={},
                    confidence=0.3,
                    fallback_attacks=[],
                )
    ...
    def _get_generic_mappings(self, strategy: Dict[str, Any]) -> List[AttackMapping]:
        ...
        return [
            AttackMapping(
                legacy_strategy="generic",
                attack_names=["tcp_basic_segmentation"],
                parameter_mapping={},
                confidence=0.1,
                fallback_attacks=["tcp_random_segmentation", "ip_basic_fragmentation"],
            )
        ]
