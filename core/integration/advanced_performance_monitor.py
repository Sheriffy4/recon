...
class AdvancedPerformanceMonitor:
    ...
    async def record_attack_performance(self, 
                                      attack_name: str,
                                      execution_time_ms: float,
                                      success: bool,
                                      effectiveness_score: float,
        ...
        try:
            ...
            LOG.debug(f"Recorded performance metrics for {attack_name}: "
    ...
    async def record_system_performance(self) -> SystemPerformanceMetrics:
        ...
        try:
            ...
            recent_attacks = [m for m in self.attack_metrics_history 
        ...
        try:
            ...
            analysis = {
                ...
                'best_attack': max(attack_analysis.items(), 
                                 key=lambda x: x[1]['success_rate_percent'])[0] if attack_analysis else None,
    ...
    async def get_system_health_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Performance trends
            execution_time_trend = self._calculate_trend([m.average_execution_time_ms for m in recent_system_metrics])
            effectiveness_trend = self._calculate_trend([m.average_effectiveness_score for m in recent_system_metrics])
            error_rate_trend = self._calculate_trend([m.error_rate_percent for m in recent_system_metrics])
            ...
            health_report = {
                ...
                'recommendations': self._generate_health_recommendations(latest_metrics, recent_alerts),
                'uptime_hours': (current_time - self.current_metrics['start_time']).total_seconds() / 3600
            }
    ...
    async def get_performance_alerts(self, severity_filter: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    async def export_performance_data(self, 
        ...
        try:
            ...
            filtered_attack_metrics = [
                asdict(m) for m in self.attack_metrics_history
            ...
            export_data = {
                ...
                'performance_summary': {
                    ...
                    'unique_attack_types': len(set(m['attack_name'] for m in filtered_attack_metrics)),
                    'unique_dpi_types': len(set(m['dpi_type'] for m in filtered_attack_metrics)),
                    'overall_success_rate': (sum(1 for m in filtered_attack_metrics if m['success']) / 
                                           len(filtered_attack_metrics) * 100) if filtered_attack_metrics else 0
                }
            }
        ...
        try:
            ...
            if metrics.execution_time_ms > self.thresholds['max_execution_time_ms']:
                alert = PerformanceAlert(
                    ...
                    severity='high' if metrics.execution_time_ms > self.thresholds['max_execution_time_ms'] * 2 else 'medium',
                    message=f"High execution time for {metrics.attack_name}: {metrics.execution_time_ms:.1f}ms",
                    timestamp=datetime.now(),
                    metrics={'execution_time_ms': metrics.execution_time_ms, 'attack_name': metrics.attack_name},
            ...
            if metrics.memory_usage_mb > self.thresholds['max_memory_usage_mb']:
                alert = PerformanceAlert(
                    ...
                    timestamp=datetime.now(),
                    metrics={'memory_usage_mb': metrics.memory_usage_mb, 'attack_name': metrics.attack_name},
            ...
            if metrics.network_latency_ms > self.thresholds['max_network_latency_ms']:
                alert = PerformanceAlert(
                    ...
                    timestamp=datetime.now(),
                    metrics={'network_latency_ms': metrics.network_latency_ms, 'target_domain': metrics.target_domain},
    ...
    async def _check_system_alerts(self, system_metrics: SystemPerformanceMetrics) -> None:
        ...
        try:
            ...
            success_rate = (system_metrics.successful_attacks / system_metrics.total_attacks_executed * 100) if system_metrics.total_attacks_executed > 0 else 100
    ...
    def _calculate_system_health_score(self, metrics: SystemPerformanceMetrics) -> float:
        ...
        try:
            ...
            success_rate = (metrics.successful_attacks / metrics.total_attacks_executed * 100) if metrics.total_attacks_executed > 0 else 100
            if success_rate < 90:
    ...
    def _generate_health_recommendations(self, 
        ...
        try:
            ...
            success_rate = (metrics.successful_attacks / metrics.total_attacks_executed * 100) if metrics.total_attacks_executed > 0 else 100
            if success_rate < 70:
                recommendations.append("Consider reviewing attack configurations and target compatibility")
            
            # Performance recommendations
            if metrics.average_execution_time_ms > 3000:
                recommendations.append("Optimize attack execution performance or increase timeout thresholds")
            ...
            if metrics.cpu_usage_percent > 80:
                recommendations.append("Consider reducing concurrent attack execution or optimizing CPU usage")
            
            if metrics.total_memory_usage_mb > 400:
                recommendations.append("Monitor memory usage and consider implementing memory optimization")
            ...
            if 'low_success_rate' in alert_types:
                recommendations.append("Review attack selection algorithms and DPI detection accuracy")
