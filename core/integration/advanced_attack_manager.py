...
class AdvancedAttack(ABC):
    ...
    def __init__(self, config: AdvancedAttackConfig):
        ...
        self.enabled = True
        self.stats = {'executions': 0, 'successes': 0, 'total_latency_ms': 0.0, 'last_execution': None}
...
class AdvancedAttackManager:
    ...
    async def initialize_advanced_attacks(self) -> bool:
        ...
        try:
            ...
            LOG.info(f'Advanced attacks system initialized with {len(self.registered_attacks)} attacks')
            return True
        except Exception as e:
    ...
    async def select_optimal_attack(self, target_info: TargetInfo, dpi_signature: DPISignature) -> Optional[AdvancedAttack]:
        """
        ...
        """
        LOG.info(f'Selecting optimal attack for {target_info.domain} (DPI: {dpi_signature.dpi_type})')
        ml_recommendation = None
        if self.enable_ml_integration and self.strategy_integrator:
            try:
                ml_recommendation = self.strategy_integrator.predict_best_strategy(target_ip=target_info.ip, domain=target_info.domain, fingerprint=dpi_signature.fingerprint_data)
        ...
        for attack_name, attack in self.registered_attacks.items():
            ...
            if target_info.protocol not in attack.config.target_protocols:
                continue
            if attack.config.dpi_signatures != ['all'] and dpi_signature.dpi_type not in attack.config.dpi_signatures:
                continue
    ...
    async def execute_attack_with_ml_feedback(self, attack: AdvancedAttack, target: str) -> AdvancedAttackResult:
        """
        ...
        try:
            ...
            if self.performance_integrator:
                self.performance_integrator.record_attack_executed(result.latency_ms, result.success)
            self.execution_history.append({'attack_name': attack.config.name, 'target': target, 'result': result, 'timestamp': datetime.now()})
            if result.adaptation_suggestions:
                await self._apply_adaptations(attack, result.adaptation_suggestions)
            execution_time = (time.time() - start_time) * 1000
            LOG.info(f"Advanced attack completed: {attack.config.name} -> {('SUCCESS' if result.success else 'FAILURE')} ({execution_time:.1f}ms)")
            return result
        except Exception as e:
            LOG.error(f'Advanced attack execution failed: {e}')
            error_result = AdvancedAttackResult(attack_name=attack.config.name, success=False, latency_ms=(time.time() - start_time) * 1000, effectiveness_score=0.0, ml_feedback=MLFeedback(attack_name=attack.config.name, success=False, latency_ms=0.0, effectiveness_score=0.0, failure_reason=str(e), adaptation_suggestions=[]), learning_data=LearningData(target_signature=target, attack_parameters={}, effectiveness=0.0, context={}, timestamp=datetime.now()), performance_metrics=PerformanceMetrics(execution_time_ms=0.0, memory_usage_mb=0.0, cpu_usage_percent=0.0, network_overhead_bytes=0, success_rate=0.0), adaptation_suggestions=[], error_message=str(e))
    ...
    async def _get_dpi_signature(self, target_info: TargetInfo) -> DPISignature:
        ...
        if self.enable_ml_integration and self.fingerprint_integrator:
            try:
                fingerprint_result = await self.fingerprint_integrator.fingerprint_target(target_info.domain, target_info.ip)
                return DPISignature(dpi_type=fingerprint_result.dpi_type, sophistication_level=self._determine_sophistication_level(fingerprint_result), capabilities=self._extract_capabilities(fingerprint_result), confidence=fingerprint_result.confidence, fingerprint_data=fingerprint_result.fingerprint_data)
            except Exception as e:
                LOG.error(f'Fingerprinting failed: {e}')
        return DPISignature(dpi_type='unknown', sophistication_level='basic', capabilities=['basic_filtering'], confidence=0.3, fingerprint_data={})
    ...
    async def _create_attack_context(self, target_info: TargetInfo, dpi_signature: DPISignature) -> AttackContext:
        """Create attack context with all available information."""
        ml_prediction = None
        if self.enable_ml_integration and self.strategy_integrator:
            try:
                ml_prediction = self.strategy_integrator.predict_best_strategy(target_info.ip, target_info.domain, dpi_signature.fingerprint_data)
            except Exception as e:
                LOG.debug(f'ML prediction failed: {e}')
        fingerprint_data = None
        if self.enable_ml_integration and self.fingerprint_integrator:
            try:
                fingerprint_data = await self.fingerprint_integrator.fingerprint_target(target_info.domain, target_info.ip)
            except Exception as e:
        ...
        return AttackContext(target_info=target_info, dpi_signature=dpi_signature, ml_prediction=ml_prediction, fingerprint_data=fingerprint_data, historical_data=historical_data, performance_constraints=performance_constraints)
    ...
    def _get_historical_data(self, domain: str) -> Optional[Dict[str, Any]]:
        ...
        return {'total_attempts': len(domain_history), 'successful_attempts': len(successful_attacks), 'success_rate': len(successful_attacks) / len(domain_history) if domain_history else 0.0, 'most_successful_attack': self._get_most_successful_attack(successful_attacks), 'last_attempt': domain_history[-1]['timestamp'] if domain_history else None}
    ...
    def _get_performance_constraints(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {'max_cpu_usage': 80.0, 'max_memory_usage': 500.0, 'max_latency_ms': 2000.0, 'current_cpu': metrics.cpu_usage_percent, 'current_memory': metrics.memory_usage_mb}
        except Exception as e:
            LOG.debug(f'Failed to get performance constraints: {e}')
            return None

    async def _apply_adaptations(self, attack: AdvancedAttack, suggestions: List[AdaptationSuggestion]):
        """Apply adaptation suggestions to attack."""
        LOG.info(f'Applying {len(suggestions)} adaptations to {attack.config.name}')
        for suggestion in suggestions:
            try:
                feedback = MLFeedback(attack_name=attack.config.name, success=False, latency_ms=0.0, effectiveness_score=0.0, failure_reason=suggestion.reason, adaptation_suggestions=[suggestion.parameter])
                await attack.adapt_from_feedback(feedback)
                LOG.debug(f'Applied adaptation: {suggestion.parameter} -> {suggestion.suggested_value}')
            except Exception as e:
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {'registered_attacks': len(self.registered_attacks), 'enabled_attacks': len([a for a in self.registered_attacks.values() if a.enabled]), 'total_executions': sum((a.stats['executions'] for a in self.registered_attacks.values())), 'total_successes': sum((a.stats['successes'] for a in self.registered_attacks.values())), 'execution_history_size': len(self.execution_history), 'ml_integration_enabled': self.enable_ml_integration}
        attack_metrics = {}
        for name, attack in self.registered_attacks.items():
            attack_metrics[name] = {'executions': attack.stats['executions'], 'successes': attack.stats['successes'], 'success_rate': attack.get_success_rate(), 'average_latency_ms': attack.get_average_latency(), 'enabled': attack.enabled, 'priority': attack.config.priority, 'complexity': attack.config.complexity}
        metrics['attacks'] = attack_metrics
        if self.enable_ml_integration:
            if self.performance_integrator:
                try:
                    phase1_metrics = self.performance_integrator.get_current_metrics()
                    metrics['phase1_performance'] = {'cpu_usage_percent': phase1_metrics.cpu_usage_percent, 'memory_usage_mb': phase1_metrics.memory_usage_mb, 'ml_predictions_per_second': phase1_metrics.ml_predictions_per_second, 'fingerprints_per_second': phase1_metrics.fingerprints_per_second}
                except Exception as e:
