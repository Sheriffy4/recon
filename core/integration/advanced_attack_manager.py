...
try:
    from .strategy_prediction_integration import get_strategy_integrator, StrategyRecommendation
...
class DPISignature:
    ...
    sophistication_level: str = "basic"  # 'basic', 'intermediate', 'advanced', 'sophisticated'
...
class AdvancedAttackManager:
    ...
    async def select_optimal_attack(self, 
                                  target_info: TargetInfo, 
                                  dpi_signature: DPISignature) -> Optional[AdvancedAttack]:
        """
        ...
        for attack_name, attack in self.registered_attacks.items():
            ...
            if (attack.config.dpi_signatures != ["all"] and 
                dpi_signature.dpi_type not in attack.config.dpi_signatures):
                continue
        ...
        def attack_score(attack_tuple):
            ...
            priority_score = 1.0 / attack.config.priority  # Lower priority number = higher score
    ...
    async def execute_attack_with_ml_feedback(self, 
                                            attack: AdvancedAttack, 
                                            target: str) -> AdvancedAttackResult:
        """
    ...
    async def _create_attack_context(self, 
                                   target_info: TargetInfo, 
                                   dpi_signature: DPISignature) -> AttackContext:
        ...
        if self.enable_ml_integration and self.strategy_integrator:
            try:
                ml_prediction = self.strategy_integrator.predict_best_strategy(
                    target_info.ip,
                    target_info.domain,
                    dpi_signature.fingerprint_data
                )
            except Exception as e:
    ...
    def _get_historical_data(self, domain: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'success_rate': len(successful_attacks) / len(domain_history) if domain_history else 0.0,
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'total_executions': sum(a.stats['executions'] for a in self.registered_attacks.values()),
            'total_successes': sum(a.stats['successes'] for a in self.registered_attacks.values()),
