# recon/core/integration/result_processor.py
"""
Result Processor
...
from .integration_config import PerformanceMetrics
...
class ResultProcessor:
    ...
    def _build_attack_map(self) -> Dict[str, str]:
        ...
        for name, attack_class in all_attacks.items():
            try:
                ...
                # Skip abstract classes or classes that cannot be instantiated without args
                continue
        return attack_map
    ...
    def process_attack_result(
        ...
        try:
            ...
            # Determine the simple strategy name for reporting
            strategy_name = self._extract_strategy_name(attack_name, task_data, result)

            # Prioritize real-world effectiveness metrics if available
            ...
            if result.metadata and "bypass_results" in result.metadata:
                ...
                # Use latency from the successful path
                if bypass_effective:
            ...
            # Final success is a combination of attack execution and real-world effectiveness
            ...
            # Add derived metrics for deeper analysis
    ...
    def _extract_strategy_name(
        ...
        """
        try:
            # Handle dynamic_combo strategies by creating a descriptive name
