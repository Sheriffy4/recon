...
class ResultProcessor:
    ...
    def process_attack_result(self, result: Optional[AttackResult], attack_name: str='unknown', task_data: Dict[str, Any]=None) -> Dict[str, Any]:
        """
        ...
        try:
            if result is None:
                ...
                return self._create_error_result(attack_name, 'Attack returned None instead of AttackResult')
            ...
            if result.metadata and 'bypass_results' in result.metadata:
                ...
                if bypass_effective:
                    final_latency = bypass_results.get('bypass', {}).get('latency_ms', result.latency_ms)
                elif bypass_results.get('baseline', {}).get('success', False):
                    final_latency = bypass_results.get('baseline', {}).get('latency_ms', result.latency_ms)
            final_success = result.status == AttackStatus.SUCCESS and bypass_effective
            legacy_result = {'success': final_success, 'status': result.status.value, 'latency_ms': final_latency, 'bypass_effective': bypass_effective, 'packets_sent': result.packets_sent, 'bytes_sent': result.bytes_sent, 'connection_established': result.connection_established, 'data_transmitted': result.data_transmitted, 'error_message': result.error_message, 'timestamp': time.time(), 'attack_name': strategy_name, 'original_attack_name': attack_name, 'metadata': result.metadata or {}}
            ...
            LOG.debug(f'Processed result for {attack_name}: status={result.status.value}, effective={bypass_effective}')
    ...
    def aggregate_results(self, results: List[AttackResult], attack_names: List[str]=None) -> Dict[str, Any]:
        """
        ...
        try:
            processed_results = [self.process_attack_result(res, name) for res, name in zip(results, attack_names or [f'attack_{i}' for i in range(len(results))])]
            ...
            aggregate = {'total_attacks': len(results), 'successful_attacks': len(successful_results), 'failed_attacks': len(results) - len(successful_results), 'success_rate': len(successful_results) / len(results) if results else 0.0, 'total_latency_ms': sum((r['latency_ms'] for r in processed_results)), 'average_latency_ms': mean(latencies) if latencies else 0.0, 'median_latency_ms': median(latencies) if latencies else 0.0, 'max_latency_ms': max(latencies) if latencies else 0.0, 'min_latency_ms': min(latencies) if latencies else 0.0, 'total_packets_sent': sum((r['packets_sent'] for r in processed_results)), 'total_bytes_sent': sum((r['bytes_sent'] for r in processed_results)), 'average_throughput_bps': mean([r['throughput_bps'] for r in successful_results]) if successful_results else 0.0, 'average_efficiency_score': mean([r['efficiency_score'] for r in successful_results]) if successful_results else 0.0, 'status_breakdown': self._calculate_status_breakdown(results), 'attack_breakdown': self._calculate_attack_breakdown(processed_results), 'aggregation_timestamp': time.time()}
            LOG.info(f"Aggregated {len(results)} attack results: {aggregate['success_rate']:.2%} success rate")
            return aggregate
        except Exception as e:
            LOG.error(f'Failed to aggregate attack results: {e}')
            return self._create_error_aggregate(str(e))

    def _extract_strategy_name(self, attack_name: str, task_data: Dict[str, Any]=None, result: AttackResult=None) -> str:
        """
        Extracts a simple, user-friendly strategy name for reporting.
        Handles dynamic_combo strategies by inspecting their stages.
        """
        try:
            if attack_name == 'dynamic_combo' or (task_data and task_data.get('type') == 'dynamic_combo'):
                return self._extract_dynamic_combo_name(task_data or result.metadata.get('task', {}))
            ...
            if result and result.metadata:
                return result.metadata.get('strategy_type') or result.metadata.get('technique_applied', attack_name)
            return attack_name
        except Exception as e:
    ...
    def _create_error_result(self, attack_name: str, error_message: str) -> Dict[str, Any]:
        ...
        return {'success': False, 'status': 'error', 'latency_ms': 0, 'bypass_effective': False, 'packets_sent': 0, 'bytes_sent': 0, 'connection_established': False, 'data_transmitted': False, 'error_message': error_message, 'timestamp': time.time(), 'attack_name': attack_name, 'original_attack_name': attack_name, 'throughput_bps': 0, 'efficiency_score': 0}

    def _create_empty_aggregate(self) -> Dict[str, Any]:
        """Creates an empty aggregate result dictionary."""
        return {'total_attacks': 0, 'successful_attacks': 0, 'success_rate': 0.0, 'average_latency_ms': 0.0, 'status_breakdown': {}, 'attack_breakdown': {}, 'aggregation_timestamp': time.time()}
