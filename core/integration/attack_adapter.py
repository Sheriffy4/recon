...
class AttackAdapter:
    ...
    def __init__(self, attack_registry: Optional[AttackRegistry]=None, integration_config: Optional[IntegrationConfig]=None):
        ...
        self.registry = self.attack_registry
    ...
    def _safe_create_attack_result(self, status_name: str, error_message: str='', technique_used: str='', **kwargs):
        ...
        try:
            if self._cached_attack_result and self._cached_attack_status:
                ...
                return self._cached_attack_result(status=status, error_message=error_message, technique_used=technique_used, **kwargs)
        except Exception:
            pass
        try:
            from recon.core.bypass.attacks.base import AttackResult, AttackStatus
        ...
        self.execution_stats = {'total_executions': 0, 'successful_executions': 0, 'failed_executions': 0, 'total_execution_time': 0.0, 'cache_hits': 0, 'cache_misses': 0, 'raw_packet_executions': 0, 'packet_executor_successes': 0}
        ...
        self.dry_run_stats = {'total_dry_runs': 0, 'segments_simulated': 0, 'validation_errors': 0, 'simulation_time': 0.0}
    _PARAM_ALIASES = {'tcp_multisplit': {'split_count': ('positions', lambda v: list(range(1, int(v) * 2, 2))), 'split_positions': ('positions', lambda v: [int(x) for x in v] if isinstance(v, (list, tuple)) else [int(v)]), 'split_pos': ('positions', lambda v: [int(v)]), 'seq_overlap': ('overlap_size', int), 'overlap': ('overlap_size', int)}, 'tcp_multidisorder': {'split_count': ('positions', lambda v: list(range(1, int(v) * 2, 2))), 'split_positions': ('positions', lambda v: [int(x) for x in v] if isinstance(v, (list, tuple)) else [int(v)]), 'split_pos': ('positions', lambda v: [int(v)])}, 'tcp_seqovl': {'seq_overlap': ('overlap_size', int), 'split_pos': ('split_pos', int)}, 'tcp_fakeddisorder': {'split_positions': ('split_pos', lambda v: int(v[0]) if isinstance(v, (list, tuple)) and v else int(v)), 'split_pos': ('split_pos', int)}, 'badsum_race': {'race_delay_ms': ('delay_ms', int), 'ttl': ('ttl', int)}}

    def _normalize_attack_params_for_name(self, attack_name: str, provided: Dict[str, Any], expected_keys: Optional[set]=None) -> Dict[str, Any]:
        """
    ...
    def execute_legacy_technique(self, technique_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            legacy_strategy = {'name': technique_name, 'type': technique_name, 'params': params}
            ...
            LOG.info(f'Legacy technique {technique_name} mapped to {attack_name} with {mapping.confidence:.2%} confidence')
    ...
    async def execute_attack_by_name(self, attack_name: str, context: 'AttackContext', strategy_params: Optional[Dict[str, Any]]=None) -> 'AttackResult':
        """
        Находит, создает и выполняет атаку по ее имени, передавая параметры
        через обновленный контекст.
        """
        self.logger.debug(f"Executing attack '{attack_name}' for {context.dst_ip}:{context.dst_port}")
        ...
        try:
            ...
            if inspect.iscoroutinefunction(attack_instance.execute):
                ...
                result = await loop.run_in_executor(None, attack_instance.execute, execution_context)
            return result
        except Exception as e:
            self.logger.error(f"Exception during execution of attack '{attack_name}': {e}", exc_info=True)
            return AttackResult(status=AttackStatus.ERROR, error_message=f'Exception during execution: {e}', technique_used=attack_name)

    async def execute_attack_sequence(self, stages: List[Dict[str, Any]], initial_context: AttackContext) -> AttackResult:
        """
    ...
    def _aggregate_combo_results(self, results: List[AttackResult], successful_stages: List[str], all_stages: List[Dict]) -> AttackResult:
        """Агрегирует результаты комбо-атаки."""
        if not results:
            return self._safe_create_attack_result(status_name='ERROR', error_message='No stages were executed.')
        ...
        if len(successful_stages) < len(all_stages):
            ...
            final_result.error_message = f"Combo failed at stage '{failed_stage_name}'. " + (final_result.error_message or '')
    ...
    def execute_attack_parallel(self, attacks: List[str], context: AttackContext) -> List[AttackResult]:
        """
        ...
        try:
            ...
            if len(attacks) > self.config.parallel_execution_limit:
                raise AttackExecutionError(f'Too many parallel attacks: {len(attacks)} > {self.config.parallel_execution_limit}')
            future_to_attack = {}
            for attack_name in attacks:
                future = self.executor.submit(self.execute_attack_by_name, attack_name, context)
                future_to_attack[future] = attack_name
            results = []
            for future in as_completed(future_to_attack, timeout=self.config.attack_timeout_seconds):
                ...
                try:
                    ...
                    error_result = self._safe_create_attack_result(status_name='ERROR', error_message=str(e), latency_ms=0)
    ...
    def get_available_attacks(self, category: Optional[str]=None, protocol: Optional[str]=None) -> List[str]:
        ...
        try:
            ...
            LOG.debug(f'Found {len(attack_names)} attacks (category: {category}, protocol: {protocol})')
            return attack_names
        except Exception as e:
    ...
    def get_attack_info(self, attack_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not attack:
                return None
            return {'name': attack.name, 'category': attack.category, 'description': attack.description, 'supported_protocols': attack.supported_protocols, 'class_name': attack.__class__.__name__}
        except Exception as e:
    ...
    def _create_attack_context_from_legacy(self, legacy_params: Dict[str, Any], converted_params: Dict[str, Any]) -> AttackContext:
        ...
        context = AttackContext(dst_ip=dst_ip, dst_port=dst_port, payload=payload, protocol=protocol, params=converted_params)
    ...
    def _validate_attack_context(self, attack, context: AttackContext):
        ...
        if context.protocol not in attack.supported_protocols:
            raise AttackExecutionError(f'Attack {attack.name} does not support protocol {context.protocol}. Supported: {attack.supported_protocols}')
    ...
    def _execute_with_timeout(self, attack, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            future.cancel()
            return self._safe_create_attack_result(status_name='TIMEOUT', error_message=f'Attack timed out after {self.config.attack_timeout_seconds}s', latency_ms=self.config.attack_timeout_seconds * 1000)
        ...
        try:
            ...
            for k, v in context.params.items():
                if isinstance(v, (list, dict)):
                    ...
                    hashable_params[k] = v
            context_str = f'{context.dst_ip}:{context.dst_port}:{context.protocol}:{len(context.payload)}:{hash(frozenset(hashable_params.items()))}'
    ...
    def _update_context_from_result(self, context: AttackContext, result: AttackResult) -> AttackContext:
    ...
    async def _execute_raw_packets(self, context: AttackContext, result: AttackResult) -> bool:
        ...
        try:
            ...
            if result.metadata.get('segments'):
                success = await asyncio.to_thread(self.packet_executor.execute_attack_session, context, result)
    ...
    async def _execute_scapy_packets(self, context: AttackContext, packets: List[Any]) -> bool:
        ...
        try:
            ...
            if not segments:
                ...
                return False
            mock_result = self._safe_create_attack_result(status_name='SUCCESS', technique_used='scapy_conversion', metadata={'segments': segments})
            if mock_result:
                mock_result.success = True
            success = await asyncio.to_thread(self.packet_executor.execute_attack_session, context, mock_result)
            return success
        except Exception as e:
    ...
    def get_execution_stats(self) -> Dict[str, Any]:
        ...
        if stats['raw_packet_executions'] > 0:
            stats['raw_packet_success_rate'] = stats['packet_executor_successes'] / stats['raw_packet_executions']
        else:
    ...
    async def _execute_dry_run(self, attack, context: AttackContext, attack_name: str) -> AttackResult:
        ...
        try:
            ...
            result = self._safe_create_attack_result(status_name='SUCCESS', technique_used=attack_name, latency_ms=0.0, metadata={'dry_run': True, 'simulation_mode': True, 'attack_name': attack_name, 'context_summary': self._create_context_summary(context)})
            try:
            ...
            return self._safe_create_attack_result(status_name='FAILED', technique_used=attack_name, error_message=f'Dry run failed: {e}', latency_ms=(time.time() - start_time) * 1000, metadata={'dry_run': True, 'simulation_mode': True, 'dry_run_error': str(e)})
                ...
                if len(segment) >= 3:
                    ...
                    if isinstance(options, dict):
                        ...
                        if options.get('delay_ms', 0) > 0:
                            timing_delays += 1
            validation_result['segment_analysis'] = {'total_segments': len(segments), 'total_payload_size': total_payload_size, 'ttl_modifications': ttl_modifications, 'checksum_corruptions': checksum_corruptions, 'timing_delays': timing_delays}
        except Exception as e:
    ...
    def _create_context_summary(self, context: AttackContext) -> dict:
        ...
        """
        return {'dst_ip': context.dst_ip, 'dst_port': context.dst_port, 'protocol': context.protocol, 'payload_size': len(context.payload) if context.payload else 0, 'domain': context.domain, 'params_count': len(context.params) if context.params else 0, 'has_tcp_session': hasattr(context, 'tcp_seq') and context.tcp_seq is not None}
