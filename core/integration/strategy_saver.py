...
class StrategySaver:
    ...
    def __init__(self, strategy_file: str='best_strategy.json', max_strategies_per_fingerprint: int=5):
        """
    ...
    def validate_strategy_effectiveness(self, strategy: Dict[str, Any]) -> bool:
        ...
        try:
            if strategy.get('bypass_effective') is True:
                LOG.info(f"Strategy '{strategy.get('attack_name')}' confirmed effective by real-world test.")
            ...
            if success_rate < self.min_success_rate:
                LOG.debug(f'Strategy rejected: low success rate {success_rate:.2f} < {self.min_success_rate:.2f}')
                return False
            avg_latency = strategy.get('avg_latency_ms', float('inf'))
            if avg_latency > self.max_latency_threshold:
                LOG.debug(f'Strategy rejected: high latency {avg_latency:.1f}ms > {self.max_latency_threshold:.1f}ms')
                return False
            if self._is_false_positive(strategy):
                LOG.debug('Strategy rejected: appears to be a false positive')
                return False
            LOG.info(f"Strategy validated as effective based on metrics: {strategy.get('attack_name', 'unknown')}")
            return True
        except Exception as e:
    ...
    def validate_strategy_completeness(self, strategy: Dict[str, Any]) -> bool:
        ...
        for field in required_fields:
            if field not in strategy or not strategy[field]:
                LOG.warning(f"Strategy validation failed: missing or empty required field '{field}'.")
                return False
        return True
    ...
    def format_strategy_for_service(self, strategy: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            formatted = {'domains': strategy.get('domains', []), 'fingerprint_summary': strategy.get('fingerprint_summary', 'N/A'), 'metrics': {'success_rate': float(strategy.get('success_rate', 0)), 'avg_latency_ms': float(strategy.get('avg_latency_ms', 0)), 'bypass_effective': strategy.get('bypass_effective', False)}, 'timestamp': time.time(), 'version': '3.1'}
    ...
    def _load_existing_strategies(self) -> Dict[str, Any]:
        ...
        return {'metadata': {'version': '3.1', 'last_updated': time.time()}, 'strategies_by_fingerprint': {}}

    def _merge_strategies(self, existing_config: Dict[str, Any], new_strategies: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for strategy in new_strategies:
                ...
                LOG.info(f'Updated strategies for fingerprint {fp_hash}. Now have {len(trimmed_strategies)} unique options (limit: {self.max_strategies_per_fingerprint}).')
    ...
    def _save_to_file(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            LOG.info(f'Successfully saved strategies for {fingerprint_count} fingerprints to {self.strategy_file}')
            return True
        except Exception as e:
    ...
    def _is_false_positive(self, strategy: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success_rate == 1.0 and avg_latency < 100 and (not is_quic_attack):
                LOG.warning(f"Suspicious result for '{attack_name}': perfect success with very low latency.")
                return True
            return False
        except Exception as e:
    ...
    def _sort_strategies_by_effectiveness(self, strategies: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
