...
class ServiceIntegrationManager:
    ...
    def load_strategies_for_service(self) -> Dict[str, ServiceStrategy]:
        ...
        try:
            ...
            if not Path(self.strategy_file).exists():
                LOG.warning(
                    f"Strategy file {self.strategy_file} not found, returning empty strategies"
                )
                return {}
            ...
            if (
                current_time - self.last_load_time
            ) < self.strategy_cache_ttl and self.loaded_strategies:
    ...
    def _convert_to_service_strategy(
        self, fingerprint_hash: str, strategy_data: Dict
    ) -> Optional[ServiceStrategy]:
        """
    ...
    def _validate_strategy_data(self, strategy: ServiceStrategy) -> bool:
        ...
        try:
            ...
            if strategy.success_rate < self.min_success_rate:
                LOG.warning(
                    f"Strategy {strategy.fingerprint_hash} has low success rate: {strategy.success_rate}"
                )
                return False
            ...
            if not strategy.bypass_effective:
                LOG.warning(
                    f"Strategy {strategy.fingerprint_hash} is not marked as bypass effective"
                )
                return False
    ...
    def _update_domain_mapping(self, strategy: ServiceStrategy) -> None:
        ...
        try:
            ...
            LOG.error(
                f"Error updating domain mapping for strategy {strategy.fingerprint_hash}: {e}"
            )
