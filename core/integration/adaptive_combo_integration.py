...
try:
    ...
    from core.bypass.attacks.base import AttackContext as BaseAttackContext, AttackResult as BaseAttackResult
...
class AdaptiveComboAttackIntegration(AdvancedAttack):
    ...
    async def execute(self, target: str, context: AttackContext) -> AdvancedAttackResult:
        ...
        try:
            ...
            LOG.info(f"Adaptive combo attack completed: {('SUCCESS' if result.success else 'FAILURE')} ({execution_time:.1f}ms)")
            return result
        except Exception as e:
    ...
    async def adapt_from_feedback(self, feedback: MLFeedback) -> None:
        ...
        try:
            ...
            LOG.debug(f'Adaptation completed: aggressiveness={self.adaptation_aggressiveness:.2f}, threshold={self.detection_threshold:.2f}')
        except Exception as e:
    ...
    async def get_effectiveness_metrics(self) -> Dict[str, float]:
        ...
        try:
            base_metrics = {'success_rate': self.get_success_rate(), 'average_latency_ms': self.get_average_latency(), 'adaptation_aggressiveness': self.adaptation_aggressiveness, 'detection_threshold': self.detection_threshold, 'learning_rate': self.learning_rate}
            if self.state_cache:
                total_iterations = sum((state.iteration for state in self.state_cache.values()))
                total_techniques = sum((len(state.techniques_tried) for state in self.state_cache.values()))
                avg_success_rate = sum((state.success_rate for state in self.state_cache.values())) / len(self.state_cache)
                base_metrics.update({'total_adaptations': total_iterations, 'total_techniques_tried': total_techniques, 'average_state_success_rate': avg_success_rate, 'cached_states': len(self.state_cache)})
            if self.learning_data_cache:
                total_learning_entries = sum((len(entries) for entries in self.learning_data_cache.values()))
    ...
    def _get_target_key(self, target: str, context: AttackContext) -> str:
        ...
        key_components = [target, context.target_info.domain, context.dpi_signature.dpi_type, str(context.dpi_signature.sophistication_level)]
    ...
    async def _apply_ml_predictions(self, ml_prediction, state: AdaptiveComboState):
        ...
        try:
            ...
            LOG.debug(f'Applied ML predictions: strategy={state.current_strategy}, iterations={self.max_iterations}')
        except Exception as e:
            LOG.error(f'Failed to apply ML predictions: {e}')

    async def _apply_historical_learning(self, historical_data: Dict[str, Any], state: AdaptiveComboState):
    ...
    async def _convert_to_base_context(self, context: AttackContext, state: AdaptiveComboState) -> BaseAttackContext:
        """Convert Phase 2 context to base attack context."""
        try:
            payload = f'GET / HTTP/1.1\r\nHost: {context.target_info.domain}\r\n\r\n'.encode()
            base_context = BaseAttackContext(payload=payload, dst_ip=context.target_info.ip, dst_port=context.target_info.port, domain=context.target_info.domain, params={'max_iterations': self.max_iterations, 'detection_threshold': self.detection_threshold, 'adaptation_aggressiveness': self.adaptation_aggressiveness, 'current_strategy': state.current_strategy, 'techniques_tried': state.techniques_tried.copy()})
            ...
            return BaseAttackContext(payload=b'GET / HTTP/1.1\r\n\r\n', dst_ip=context.target_info.ip, dst_port=context.target_info.port)

    async def _convert_from_base_result(self, base_result: BaseAttackResult, state: AdaptiveComboState, target_key: str) -> AdvancedAttackResult:
        ...
        try:
            ...
            ml_feedback = MLFeedback(attack_name=self.config.name, success=base_result.status.name == 'SUCCESS', latency_ms=base_result.latency_ms, effectiveness_score=final_success_rate, failure_reason=base_result.error_message if base_result.error_message else None, adaptation_suggestions=self._generate_adaptation_suggestions(state, detection_score))
            learning_data = LearningData(target_signature=target_key, attack_parameters={'max_iterations': self.max_iterations, 'detection_threshold': self.detection_threshold, 'adaptation_aggressiveness': self.adaptation_aggressiveness, 'techniques_tried': techniques_tried}, effectiveness=final_success_rate, context={'adaptation_iterations': adaptation_iterations, 'detection_score': detection_score, 'strategy': state.current_strategy}, timestamp=datetime.now())
            performance_metrics = PerformanceMetrics(execution_time_ms=base_result.latency_ms, memory_usage_mb=0.0, cpu_usage_percent=0.0, network_overhead_bytes=base_result.bytes_sent, success_rate=final_success_rate)
            adaptation_suggestions = self._generate_adaptation_suggestions(state, detection_score)
            return AdvancedAttackResult(attack_name=self.config.name, success=base_result.status.name == 'SUCCESS', latency_ms=base_result.latency_ms, effectiveness_score=final_success_rate, ml_feedback=ml_feedback, learning_data=learning_data, performance_metrics=performance_metrics, adaptation_suggestions=adaptation_suggestions)
        except Exception as e:
            LOG.error(f'Failed to convert result: {e}')
            return self._create_error_result(str(e), 0)

    def _generate_adaptation_suggestions(self, state: AdaptiveComboState, detection_score: float) -> List[AdaptationSuggestion]:
        """Generate adaptation suggestions based on current state."""
        suggestions = []
        try:
            if detection_score > 0.8 and self.max_iterations < 8:
                suggestions.append(AdaptationSuggestion(parameter='max_iterations', current_value=self.max_iterations, suggested_value=self.max_iterations + 1, reason='High detection score requires more adaptation iterations', confidence=0.8))
            if state.success_rate < 0.4:
                suggestions.append(AdaptationSuggestion(parameter='detection_threshold', current_value=self.detection_threshold, suggested_value=max(0.3, self.detection_threshold * 0.9), reason='Low success rate suggests threshold is too high', confidence=0.7))
            if len(state.techniques_tried) > 5 and state.success_rate < 0.6:
                suggestions.append(AdaptationSuggestion(parameter='adaptation_aggressiveness', current_value=self.adaptation_aggressiveness, suggested_value=min(1.0, self.adaptation_aggressiveness * 1.2), reason='Many techniques tried with low success - increase aggressiveness', confidence=0.6))
        except Exception as e:
    ...
    async def _update_state_and_learning(self, state: AdaptiveComboState, result: AdvancedAttackResult, target_key: str):
        """Update state and learning data after execution."""
        try:
            state.adaptation_history.append({'timestamp': datetime.now(), 'success': result.success, 'effectiveness': result.effectiveness_score, 'techniques': state.techniques_tried.copy(), 'iterations': state.iteration})
            ...
            self.learning_data_cache[target_key].append({'timestamp': datetime.now(), 'learning_data': result.learning_data, 'effectiveness': result.effectiveness_score})
    ...
    async def _fallback_execution(self, target: str, context: AttackContext, state: AdaptiveComboState) -> AdvancedAttackResult:
        ...
        try:
            ...
            ml_feedback = MLFeedback(attack_name=self.config.name, success=success, latency_ms=latency, effectiveness_score=effectiveness, failure_reason=None if success else 'DPI too sophisticated for fallback', adaptation_suggestions=['increase_aggressiveness'] if not success else [])
            learning_data = LearningData(target_signature=target, attack_parameters={'fallback': True}, effectiveness=effectiveness, context={'sophistication': sophistication}, timestamp=datetime.now())
            performance_metrics = PerformanceMetrics(execution_time_ms=latency, memory_usage_mb=1.0, cpu_usage_percent=5.0, network_overhead_bytes=1024, success_rate=effectiveness)
            return AdvancedAttackResult(attack_name=self.config.name, success=success, latency_ms=latency, effectiveness_score=effectiveness, ml_feedback=ml_feedback, learning_data=learning_data, performance_metrics=performance_metrics, adaptation_suggestions=[])
        except Exception as e:
            return self._create_error_result(str(e), time.time() - start_time)
    ...
    def _create_error_result(self, error_message: str, execution_time: float) -> AdvancedAttackResult:
        ...
        ml_feedback = MLFeedback(attack_name=self.config.name, success=False, latency_ms=execution_time * 1000 if execution_time > 0 else 0, effectiveness_score=0.0, failure_reason=error_message, adaptation_suggestions=['retry', 'fallback'])
        ...
        performance_metrics = PerformanceMetrics(execution_time_ms=execution_time * 1000 if execution_time > 0 else 0, memory_usage_mb=0.0, cpu_usage_percent=0.0, network_overhead_bytes=0, success_rate=0.0)
...
def create_adaptive_combo_integration() -> AdaptiveComboAttackIntegration:
    ...
    config = AdvancedAttackConfig(name='adaptive_combo', priority=1, complexity='High', expected_improvement='25-35%', target_protocols=['tcp', 'http', 'https'], dpi_signatures=['complex_dpi', 'sophisticated_dpi', 'ai_dpi', 'all'], ml_integration=True, learning_enabled=True)
