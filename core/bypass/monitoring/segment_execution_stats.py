...
class SegmentExecutionStatsCollector:
    ...
    def start_segment_execution(self, segment_id: int, session_id: str, 
                              payload_size: int, seq_offset: int, 
                              options: Dict[str, Any]) -> SegmentExecutionMetrics:
    ...
    def update_segment_phase(self, metrics: SegmentExecutionMetrics, 
                           phase: ExecutionPhase, duration_ms: float):
    ...
    def complete_segment_execution(self, metrics: SegmentExecutionMetrics, 
    ...
    def get_segment_metrics(self, session_id: str, segment_id: int) -> Optional[SegmentExecutionMetrics]:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if recent_segments:
                successful_segments = [s for s in recent_segments if s.status == ExecutionStatus.SUCCESS]
                recent_success_rate = (len(successful_segments) / len(recent_segments)) * 100
                
                if successful_segments:
                    recent_avg_time = statistics.mean([s.total_execution_time_ms for s in successful_segments])
    ...
    def _update_global_stats_for_segment(self, metrics: SegmentExecutionMetrics):
        ...
        if metrics.status == ExecutionStatus.SUCCESS:
            ...
            if metrics.error_message:
                ...
                self._global_stats.common_errors[error_type] = self._global_stats.common_errors.get(error_type, 0) + 1
    ...
    def _finalize_session_stats(self, session_stats: SessionExecutionStats):
        ...
        if session_stats.total_segments > 0:
            ...
            session_segments = [s for s in self._completed_segments 
            ...
            if session_segments:
                ...
                timing_errors = [abs(s.timing_accuracy_error_ms) for s in session_segments 
                ...
                if timing_errors:
                    ...
                    avg_requested_delay = statistics.mean([
                        s.options.get('delay_ms', 0) for s in session_segments 
                        if s.options.get('delay_ms', 0) > 0
                    ]) if any(s.options.get('delay_ms', 0) > 0 for s in session_segments) else 1.0
                    
                    if avg_requested_delay > 0:
                        error_percent = (session_stats.avg_timing_accuracy_error_ms / avg_requested_delay) * 100
        ...
        if session_stats.end_time:
            ...
            if session_duration > 0:
                session_stats.throughput_segments_per_sec = session_stats.total_segments / session_duration
                session_stats.throughput_bytes_per_sec = session_stats.total_payload_bytes / session_duration
    ...
    def _update_global_realtime_stats(self):
        ...
        if completed_sessions:
            session_durations = [s.total_execution_time_ms for s in completed_sessions if s.is_completed]
        ...
        if current_time - self._last_throughput_calculation > 1.0:  # Update every second
            recent_segments = [s for s in self._completed_segments 
                             if current_time - s.start_time < 60]  # Last minute
            
            if len(recent_segments) > 1:
                time_span = recent_segments[-1].start_time - recent_segments[0].start_time
                if time_span > 0:
                    self._global_stats.global_throughput_segments_per_sec = len(recent_segments) / time_span
    ...
    def _get_timing_analysis(self) -> Dict[str, Any]:
        ...
        construction_times = [s.construction_time_ms for s in recent_segments if s.construction_time_ms > 0]
        transmission_times = [s.transmission_time_ms for s in recent_segments if s.transmission_time_ms > 0]
        total_times = [s.total_execution_time_ms for s in recent_segments]
        timing_errors = [abs(s.timing_accuracy_error_ms) for s in recent_segments if s.timing_accuracy_error_ms != 0]
    ...
    def _get_modification_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "checksum_corruptions": {
                ...
                "percentage": (checksum_corrupted / total_segments) * 100 if total_segments > 0 else 0
            },
            "tcp_flags_modifications": {
                "count": tcp_flags_modified,
                "percentage": (tcp_flags_modified / total_segments) * 100 if total_segments > 0 else 0
            },
            "window_size_modifications": {
                "count": window_size_modified,
                "percentage": (window_size_modified / total_segments) * 100 if total_segments > 0 else 0
            },
    ...
    def _get_error_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate_percent": (status_counts.get('failed', 0) / total_segments) * 100 if total_segments > 0 else 0,
            "success_rate_percent": (status_counts.get('success', 0) / total_segments) * 100 if total_segments > 0 else 0,
            "total_segments_analyzed": total_segments
        }
