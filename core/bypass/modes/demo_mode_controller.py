...
def demo_mode_controller():
    ...
    for mode, info in available_modes.items():
        ...
        print(
            f"    Performance: {info.performance_level}, Stability: {info.stability_level}"
        )
    print()
    ...
    try:
        ...
        if controller.is_mode_available(OperationMode.NATIVE):
            ...
            if result:
                print(
                    f"✓ Successfully switched to {controller.get_current_mode().value}"
                )
            else:
...
def demo_auto_fallback():
    ...
    if result:
        print(
            f"✓ Auto-fallback successful, now in {controller.get_current_mode().value} mode"
        )
    else:
