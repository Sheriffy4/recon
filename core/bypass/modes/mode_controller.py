...
try:
    from recon.core.bypass.modes.capability_detector import CapabilityDetector, CapabilityLevel
    from recon.core.bypass.modes.mode_transition import ModeTransitionManager
    from recon.core.bypass.modes.exceptions import ModeError, ModeTransitionError, UnsupportedModeError
except ImportError:
...
class ModeController:
    ...
    def switch_mode(self, target_mode: OperationMode, reason: str='Manual switch', force: bool=False) -> bool:
        """
        ...
        if target_mode == self.current_mode:
            ...
            return True
        self.logger.info(f'Switching from {self.current_mode.value} to {target_mode.value}: {reason}')
        try:
            success = self.transition_manager.transition_to_mode(target_mode.value, self.current_mode.value, reason)
            if success:
    ...
    def auto_fallback(self, error: Exception) -> bool:
        ...
        self.mode_failure_counts[self.current_mode] = self.mode_failure_counts.get(self.current_mode, 0) + 1
        new_mode = self.transition_manager.auto_fallback(self.current_mode.value, error, {'failure_count': self.mode_failure_counts[self.current_mode]})
        if new_mode:
            try:
    ...
    def get_mode_info(self, mode: Optional[OperationMode]=None) -> Dict[str, Any]:
        ...
        return {'mode': target_mode.value, 'available': mode_info.available, 'description': mode_info.description, 'capabilities': mode_info.capabilities, 'performance_level': mode_info.performance_level, 'stability_level': mode_info.stability_level, 'requirements': mode_info.requirements, 'failure_count': self.mode_failure_counts.get(target_mode, 0), 'last_health_check': self.last_health_check.get(target_mode), 'is_current': target_mode == self.current_mode}
    ...
    def _initialize_mode_info(self) -> None:
        ...
        self.mode_capabilities[OperationMode.NATIVE] = ModeInfo(mode=OperationMode.NATIVE, available=native_available, description='Direct packet interception using native OS capabilities', capabilities={'packet_modification': True, 'real_time_processing': True, 'low_latency': True, 'requires_admin': True}, performance_level='high', stability_level='stable', requirements={'admin_privileges': True, 'native_driver': True, 'platform_specific': True})
        emulated_available = self.capability_detector.is_emulated_mode_available()
        self.mode_capabilities[OperationMode.EMULATED] = ModeInfo(mode=OperationMode.EMULATED, available=emulated_available, description='Packet processing using Scapy emulation', capabilities={'packet_modification': True, 'real_time_processing': False, 'low_latency': False, 'requires_admin': False}, performance_level='medium', stability_level='stable', requirements={'scapy_library': True, 'python_environment': True})
        hybrid_available = native_available and emulated_available
        self.mode_capabilities[OperationMode.HYBRID] = ModeInfo(mode=OperationMode.HYBRID, available=hybrid_available, description='Combination of native and emulated processing', capabilities={'packet_modification': True, 'real_time_processing': True, 'low_latency': True, 'fallback_support': True, 'requires_admin': True}, performance_level='high', stability_level='beta', requirements={'native_and_emulated': True, 'admin_privileges': True})
        self.mode_capabilities[OperationMode.COMPATIBILITY] = ModeInfo(mode=OperationMode.COMPATIBILITY, available=True, description='Maximum compatibility mode with limited functionality', capabilities={'packet_modification': False, 'real_time_processing': False, 'low_latency': False, 'basic_functionality': True}, performance_level='low', stability_level='stable', requirements={})
        self.mode_capabilities[OperationMode.AUTO] = ModeInfo(mode=OperationMode.AUTO, available=True, description='Automatic mode selection based on capabilities', capabilities={'adaptive_selection': True, 'automatic_fallback': True}, performance_level='variable', stability_level='stable', requirements={})
    ...
    def _register_default_handlers(self) -> None:
        ...
        self.transition_manager.register_validation_handler('emulated', self._validate_emulated_mode)
    ...
    def _validate_native_mode(self) -> bool:
        ...
        try:
            ...
            return pydivert_cap and pydivert_cap.level in [CapabilityLevel.FULL, CapabilityLevel.PARTIAL]
        except Exception as e:
    ...
    def _validate_emulated_mode(self) -> bool:
        ...
        try:
            ...
            return scapy_cap and scapy_cap.level in [CapabilityLevel.FULL, CapabilityLevel.PARTIAL]
        except Exception as e:
