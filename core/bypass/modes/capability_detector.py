# recon/core/bypass/modes/capability_detector.py
"""
...
try:
    from .exceptions import CapabilityDetectionError
except ImportError:
    from exceptions import CapabilityDetectionError
...
class CapabilityLevel(Enum):
    ...
    FULL = "full"           # Full native support
    PARTIAL = "partial"     # Limited native support
    EMULATED = "emulated"   # Emulated support only
...
class CapabilityDetector:
    ...
    def is_native_mode_available(self) -> bool:
        ...
        # On Windows, native mode requires PyDivert
        if self._system_info['platform'] == 'Windows':
            ...
            return (pydivert_cap and pydivert_cap.level in [CapabilityLevel.FULL, CapabilityLevel.PARTIAL] and
                    admin_cap and admin_cap.requirements_met and
                    driver_cap and driver_cap.level != CapabilityLevel.UNAVAILABLE)
        
        # On Linux, native mode requires netfilter or raw sockets
            ...
            return (admin_cap and admin_cap.requirements_met and
                    ((netfilter_cap and netfilter_cap.level != CapabilityLevel.UNAVAILABLE) or
                     (raw_sockets_cap and raw_sockets_cap.level != CapabilityLevel.UNAVAILABLE)))
    ...
    def _detect_pydivert_capability(self) -> CapabilityInfo:
        ...
        try:
            ...
            # Check if we can create a handle (requires admin privileges)
            try:
                # Try to create a simple filter that should work
                test_handle = pydivert.WinDivert("false")  # Filter that matches nothing
                test_handle.close()
    ...
    def _detect_scapy_capability(self) -> CapabilityInfo:
        ...
        try:
            ...
            try:
                # Try to access Scapy configuration
                iface = conf.iface
    ...
    def _detect_netfilter_capability(self) -> CapabilityInfo:
        ...
        try:
            # Check if netfilterqueue is available
    ...
    def _detect_raw_sockets_capability(self) -> CapabilityInfo:
        ...
        try:
            ...
            try:
                if self._system_info['platform'] == 'Windows':
                    # On Windows, raw sockets have limitations
    ...
    def _detect_admin_privileges(self) -> CapabilityInfo:
        ...
        return CapabilityInfo(
            ...
            reason="Administrator privileges detected" if is_admin else "Administrator privileges required",
            details={
                'is_admin': is_admin,
