...
from recon.core.bypass.protocols.multi_port_handler import (
    MultiPortHandler, PortStrategy, ProtocolFamily, PortTestResult, BypassResult
)
...
class MultiPortBypassDemo:
    ...
    def demonstrate_port_strategy_configuration(self):
        ...
        print(f"  ✓ Custom strategy added for port 8443")
    ...
    async def demonstrate_domain_testing(self):
            ...
            if "example.com" in domain:
                if port == 80:
                    return PortTestResult(
                        port=80, accessible=True, response_time_ms=120.0,
                        protocol_detected="http", server_header="Apache/2.4.41",
                        block_type=BlockType.NONE
                    )
                elif port == 443:
                    return PortTestResult(
                        port=443, accessible=True, response_time_ms=180.0,
                        protocol_detected="https", tls_version="TLSv1.3",
                        block_type=BlockType.NONE
                    )
            
            elif "blocked" in domain:
                return PortTestResult(
                    port=port, accessible=False, response_time_ms=5000.0,
                    error_message="Connection timeout", block_type=BlockType.TIMEOUT
                )
            
            elif "partial" in domain:
                if port == 80:
                    return PortTestResult(
                        port=80, accessible=False, response_time_ms=100.0,
                        error_message="Connection refused", block_type=BlockType.CONNECTION_REFUSED
                    )
                elif port == 443:
                    return PortTestResult(
                        port=443, accessible=True, response_time_ms=250.0,
                        protocol_detected="https", tls_version="TLSv1.2",
                        block_type=BlockType.NONE
                    )
            
            # Default case
            return PortTestResult(
                port=port, accessible=True, response_time_ms=200.0,
                protocol_detected="tcp", block_type=BlockType.NONE
            )
        ...
        test_scenarios = [
            ...
            ("partial.example.com", "Partially blocked domain (HTTP blocked, HTTPS works)")
        ]
    ...
    async def demonstrate_strategy_application(self):
        ...
        async def mock_bypass_success(domain: str, port: int) -> PortTestResult:
            ...
            return PortTestResult(
                port=port, accessible=True, response_time_ms=150.0,
    ...
    def demonstrate_statistics_and_monitoring(self):
        ...
        cache_entries = [
            ("google.com:80", PortTestResult(port=80, accessible=True, response_time_ms=95.0)),
            ("google.com:443", PortTestResult(port=443, accessible=True, response_time_ms=120.0)),
            ("youtube.com:443", PortTestResult(port=443, accessible=True, response_time_ms=180.0)),
            ("twitter.com:80", PortTestResult(port=80, accessible=False, response_time_ms=5000.0)),
            ("twitter.com:443", PortTestResult(port=443, accessible=True, response_time_ms=200.0))
        ]
