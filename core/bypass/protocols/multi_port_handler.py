...
class MultiPortHandler:
    ...
    async def test_domain_accessibility(
        self, domain: str, ports: List[int] = None
    ) -> Dict[int, PortTestResult]:
        """
    ...
    async def _test_https_port(self, domain: str, port: int) -> PortTestResult:
        ...
        try:
            ...
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(domain, port, ssl=context), timeout=30
            )
    ...
    async def _test_http_port(self, domain: str, port: int) -> PortTestResult:
        ...
        try:
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(domain, port), timeout=15
            )
    ...
    async def apply_port_specific_strategy(
        self,
        domain: str,
        port: int,
        strategy_id: str,
        attacks: List[AttackDefinition] = None,
    ) -> BypassResult:
        """
    ...
    def _select_attacks_for_port(
        self, port: int, available_attacks: List[AttackDefinition]
    ...
    def get_optimal_port_for_domain(
        self, domain: str, test_results: Dict[int, PortTestResult] = None
    ) -> int:
        """
    ...
    def detect_protocol_requirements(
        self, domain: str, test_results: Dict[int, PortTestResult] = None
    ) -> List[int]:
        """
        ...
        return {
            **self.stats,
            ...
            "success_rate": (
                ...
                if self.stats["strategies_applied"] > 0
                else 0.0
            ),
        }
