...
class MultiPortHandler:
    ...
    def __init__(self):
        ...
        self.cache_ttl = 300
        self.stats = {'ports_tested': 0, 'strategies_applied': 0, 'successful_bypasses': 0, 'cache_hits': 0}

    def _initialize_default_strategies(self) -> None:
        """Initialize default port strategies."""
        self.port_strategies[80] = PortStrategy(port=80, protocol_family=ProtocolFamily.HTTP_FAMILY, preferred_attacks=['http_host_header_case', 'http_method_override', 'http_chunked_encoding', 'tcp_window_scaling'], default_timeout=15, requires_tls=False, supports_sni=False, validation_method='http_response')
        self.port_strategies[443] = PortStrategy(port=443, protocol_family=ProtocolFamily.SECURE_FAMILY, preferred_attacks=['tls_sni_fragmentation', 'tls_record_splitting', 'tls_handshake_split', 'tcp_segment_fragmentation'], default_timeout=30, requires_tls=True, supports_sni=True, validation_method='tls_handshake')
        self.port_strategies[53] = PortStrategy(port=53, protocol_family=ProtocolFamily.DNS_FAMILY, preferred_attacks=['dns_fragmentation', 'dns_case_randomization', 'dns_padding'], default_timeout=10, validation_method='dns_query')

    def _initialize_protocol_attacks(self) -> None:
        """Initialize protocol-specific attack mappings."""
        self.protocol_attacks = {ProtocolFamily.HTTP_FAMILY: ['http_host_header_case', 'http_method_override', 'http_chunked_encoding', 'http_pipeline_abuse', 'http_header_injection'], ProtocolFamily.SECURE_FAMILY: ['tls_sni_fragmentation', 'tls_record_splitting', 'tls_handshake_split', 'tls_version_rollback', 'tls_cipher_reorder'], ProtocolFamily.DNS_FAMILY: ['dns_fragmentation', 'dns_case_randomization', 'dns_padding', 'dns_over_https', 'dns_over_tls'], ProtocolFamily.MAIL_FAMILY: ['smtp_command_injection', 'pop3_timing_attack', 'imap_buffer_overflow'], ProtocolFamily.PLAIN_FAMILY: ['tcp_window_scaling', 'tcp_segment_fragmentation', 'tcp_rst_injection', 'payload_scrambling']}
    ...
    async def test_domain_accessibility(self, domain: str, ports: List[int]=None) -> Dict[int, PortTestResult]:
        ...
        for port, task in tasks:
            try:
                ...
                results[port] = PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message=str(e), block_type=BlockType.UNKNOWN)
        return results
    ...
    async def _test_single_port(self, domain: str, port: int) -> PortTestResult:
        ...
        try:
            ...
            self.logger.error(f'Error testing {domain}:{port}: {e}')
            return PortTestResult(port=port, accessible=False, response_time_ms=(time.time() - start_time) * 1000, error_message=str(e), block_type=BlockType.UNKNOWN)
    ...
    async def _test_https_port(self, domain: str, port: int) -> PortTestResult:
        ...
        try:
            ...
            reader, writer = await asyncio.wait_for(asyncio.open_connection(domain, port, ssl=context), timeout=30)
            tls_version = writer.get_extra_info('ssl_object').version()
            writer.close()
            await writer.wait_closed()
            return PortTestResult(port=port, accessible=True, response_time_ms=0.0, protocol_detected='https', tls_version=tls_version, block_type=BlockType.NONE)
        except asyncio.TimeoutError:
            return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message='Connection timeout', block_type=BlockType.TIMEOUT)
        except ConnectionRefusedError:
            return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message='Connection refused', block_type=BlockType.CONNECTION_REFUSED)
        except Exception as e:
            return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message=str(e), block_type=BlockType.TLS_HANDSHAKE_FAILURE)

    async def _test_http_port(self, domain: str, port: int) -> PortTestResult:
        """Test HTTP port accessibility."""
        try:
            reader, writer = await asyncio.wait_for(asyncio.open_connection(domain, port), timeout=15)
            ...
            if 'HTTP/' in response_str:
                ...
                for line in response_str.split('\r\n'):
                    if line.lower().startswith('server:'):
                        ...
                        break
                return PortTestResult(port=port, accessible=True, response_time_ms=0.0, protocol_detected='http', server_header=server_header, block_type=BlockType.NONE)
            else:
                return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message='Invalid HTTP response', block_type=BlockType.HTTP_BLOCK_PAGE)
        except asyncio.TimeoutError:
            return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message='Connection timeout', block_type=BlockType.TIMEOUT)
        except ConnectionRefusedError:
            return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message='Connection refused', block_type=BlockType.CONNECTION_REFUSED)
        except Exception as e:
            return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message=str(e), block_type=BlockType.HTTP_ERROR)

    async def _test_dns_port(self, domain: str, port: int) -> PortTestResult:
        """Test DNS port accessibility."""
        try:
            reader, writer = await asyncio.wait_for(asyncio.open_connection(domain, port), timeout=10)
            writer.close()
            await writer.wait_closed()
            return PortTestResult(port=port, accessible=True, response_time_ms=0.0, protocol_detected='dns', block_type=BlockType.NONE)
        except Exception as e:
            return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message=str(e), block_type=BlockType.CONNECTION_REFUSED)
    ...
    async def _test_generic_port(self, domain: str, port: int) -> PortTestResult:
        ...
        try:
            ...
            writer.close()
            await writer.wait_closed()
            return PortTestResult(port=port, accessible=True, response_time_ms=0.0, protocol_detected='tcp', block_type=BlockType.NONE)
        except Exception as e:
            return PortTestResult(port=port, accessible=False, response_time_ms=0.0, error_message=str(e), block_type=BlockType.CONNECTION_REFUSED)

    async def apply_port_specific_strategy(self, domain: str, port: int, strategy_id: str, attacks: List[AttackDefinition]=None) -> BypassResult:
        """
        ...
        try:
            ...
            if not selected_attacks:
                return BypassResult(success=False, port=port, strategy_used=strategy_id, execution_time_ms=(time.time() - start_time) * 1000, error_message='No suitable attacks found for this port')
            applied_attacks = []
            for attack in selected_attacks[:3]:
                try:
            ...
            return BypassResult(success=success, port=port, strategy_used=strategy_id, execution_time_ms=(time.time() - start_time) * 1000, attacks_applied=applied_attacks, metadata={'port_strategy': port_strategy.protocol_family.value, 'test_result': {'accessible': test_result.accessible, 'block_type': test_result.block_type.value if test_result.block_type else None, 'response_time_ms': test_result.response_time_ms}})
        except Exception as e:
            self.logger.error(f'Error applying strategy to {domain}:{port}: {e}')
            return BypassResult(success=False, port=port, strategy_used=strategy_id, execution_time_ms=(time.time() - start_time) * 1000, error_message=str(e))
    ...
    def _select_attacks_for_port(self, port: int, available_attacks: List[AttackDefinition]) -> List[AttackDefinition]:
        ...
        for attack in available_attacks:
            ...
            if attack.id in port_strategy.preferred_attacks:
                suitable_attacks.insert(0, attack)
    ...
    def detect_protocol_requirements(self, domain: str, test_results: Dict[int, PortTestResult]=None) -> List[int]:
        ...
        """
        if test_results is None:
            return [self.HTTP_PORT, self.HTTPS_PORT]
        required_ports = []
    ...
    def get_port_strategy(self, port: int) -> PortStrategy:
        ...
        return PortStrategy(port=port, protocol_family=protocol_family, default_timeout=30, validation_method='tcp_connect')
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {**self.stats, 'cache_size': len(self.port_test_cache), 'configured_ports': len(self.port_strategies), 'success_rate': self.stats['successful_bypasses'] / self.stats['strategies_applied'] if self.stats['strategies_applied'] > 0 else 0.0}
