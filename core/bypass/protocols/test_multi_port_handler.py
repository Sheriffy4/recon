# recon/core/bypass/protocols/test_multi_port_handler.py
...
class TestMultiPortHandler:
    ...
    def test_unknown_port_strategy(self, handler):
        ...
        # Test system port (< 1024)
        system_port_strategy = handler.get_port_strategy(22)  # SSH
    ...
    async def test_domain_accessibility_testing(self, handler):
        ...
        with patch.object(handler, '_test_single_port') as mock_test:
            # Mock test results for different ports
            mock_test.side_effect = [
                PortTestResult(port=80, accessible=True, response_time_ms=100.0, block_type=BlockType.NONE),
                PortTestResult(port=443, accessible=False, response_time_ms=5000.0, block_type=BlockType.TIMEOUT)
            ]
    ...
    def test_attack_selection_for_port(self, handler, sample_attacks):
        ...
        # Test HTTP port attack selection
        ...
        assert "tls_sni_fragmentation" not in http_attack_ids  # TLS attack shouldn't be selected for HTTP
        
        # Test HTTPS port attack selection
        ...
        assert "http_host_header_case" in https_attack_ids  # HTTP attack should work on HTTPS too
        
        # Test DNS port attack selection
        ...
        assert "http_host_header_case" not in dns_attack_ids  # HTTP attack shouldn't work on DNS
    ...
    async def test_port_specific_strategy_application(self, handler, sample_attacks):
        ...
        with patch.object(handler, '_test_single_port') as mock_test:
            ...
            mock_test.return_value = PortTestResult(
                port=443, accessible=True, response_time_ms=200.0, block_type=BlockType.NONE
            )
    ...
    async def test_strategy_application_failure(self, handler, sample_attacks):
        ...
        with patch.object(handler, '_test_single_port') as mock_test:
            # Mock failed bypass
            mock_test.return_value = PortTestResult(
                port=80, accessible=False, response_time_ms=5000.0, block_type=BlockType.TIMEOUT
            )
    ...
    def test_optimal_port_selection(self, handler):
        ...
        # Test with both ports accessible
        test_results = {
        ...
        assert optimal_port == 443  # HTTPS preferred over HTTP
        
        # Test with only HTTP accessible
        test_results = {
            80: PortTestResult(port=80, accessible=True, response_time_ms=100.0),
    ...
    def test_protocol_requirements_detection(self, handler):
        ...
        # Test with blocked but potentially required ports
        test_results = {
            80: PortTestResult(port=80, accessible=False, response_time_ms=5000.0, block_type=BlockType.TIMEOUT),
            443: PortTestResult(port=443, accessible=False, response_time_ms=5000.0, block_type=BlockType.RST_INJECTION)
        }
        required_ports = handler.detect_protocol_requirements("example.com", test_results)
        assert 80 in required_ports  # Timeout suggests port might be required
        assert 443 in required_ports  # RST injection suggests port might be required
    ...
    def test_statistics_tracking(self, handler):
        ...
        assert updated_stats['success_rate'] == 0.6  # 3/5
...
class TestPortTestResult:
    ...
    def test_port_test_result_initialization(self):
        ...
        assert result.protocol_detected == "https"
        assert result.tls_version == "TLSv1.3"
        assert result.block_type == BlockType.NONE  # Set by post_init
...
async def test_integration_multi_port_workflow():
    ...
    with patch.object(handler, '_test_single_port') as mock_test:
        # Mock successful tests for both ports
        mock_test.side_effect = [
            PortTestResult(port=80, accessible=True, response_time_ms=100.0, block_type=BlockType.NONE),
            PortTestResult(port=443, accessible=True, response_time_ms=150.0, block_type=BlockType.NONE),
            PortTestResult(port=443, accessible=True, response_time_ms=120.0, block_type=BlockType.NONE)  # For strategy application
        ]
        ...
        assert optimal_port == 443  # HTTPS preferred
        
        # Apply strategy to optimal port
