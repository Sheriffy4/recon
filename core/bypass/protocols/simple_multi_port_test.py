...
def test_handler_initialization():
    ...
    for port in [80, 443, 53]:
        ...
        print(f"✓ Port {port}: {strategy.protocol_family.value} family, "
...
def test_attack_selection():
    ...
    for port in test_ports:
        ...
        for attack in selected_attacks:
            print(f"  - {attack.name} (effectiveness: {attack.effectiveness_score:.1f})")
...
def test_port_strategy_management():
    ...
    print(f"✓ Added custom strategy for port 8443")
    ...
    print(f"✓ Custom strategy removed successfully")
    
    # Verify removal
    final_ports = handler.get_supported_ports()
    assert 8443 not in final_ports
    print(f"✓ Port 8443 no longer in supported ports")
...
async def test_strategy_application():
    ...
    print(f"✓ Strategy application result:")
