...
def create_engine(
    ...
    if engine_type is None:
        raise TypeError(
            "create_engine() missing required argument 'engine_type'. Use create_best_engine() for automatic engine selection, or provide a specific EngineType (e.g., EngineType.NATIVE_PYDIVERT)"
        )
    if config is None:
        config = EngineConfig()
