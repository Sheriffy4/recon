...
class PacketBuilder:
    ...
    def build_ip_header(
        ...
        header = (
            struct.pack(
                "!BBHHHBBH",
                (4 << 4) | ihl,  # Version (4) and IHL
                0,  # Type of Service
                total_length or header_length,  # Total Length (updated later if needed)
                identification or random.randint(0, 65535),  # Identification
                (flags << 13) | fragment_offset,  # Flags and Fragment Offset
                ttl,  # Time to Live
                protocol,  # Protocol
    ...
    def assemble_packet(
        ...
        **kwargs,
        ...
        # Build TCP header
        tcp_header = cls.build_tcp_header(
            src_port, dst_port, seq, ack, flags, window, urgent, tcp_options
        )
    ...
    def modify_packet(
        ...
        new_flags: Optional[Union[int, str]] = None,
        ...
        **kwargs,
        ...
        ip_hlen = (packet_data[0] & 0x0F) * 4
        tcp_start = ip_hlen
        tcp_hlen = ((packet_data[tcp_start + 12] >> 4) & 0xF) * 4
        ...
        seq = (
            ...
            else struct.unpack("!I", packet_data[tcp_start + 4 : tcp_start + 8])[0]
        )
        ack = (
            new_ack
            if new_ack is not None
            else struct.unpack("!I", packet_data[tcp_start + 8 : tcp_start + 12])[0]
        )
    ...
    @classmethod
    def fragment_packet(
        cls, packet_data: bytes, fragment_size: int, overlap_bytes: int = 0
    ) -> List[bytes]:
        ...
        while offset < len(payload):
            ...
            # Apply overlap for non-first fragments
