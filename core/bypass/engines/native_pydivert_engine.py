...
from ..attacks.segment_packet_builder import (
    SegmentPacketBuilder,
    SegmentPacketInfo,
    validate_segments_for_building,
)
from ..attacks.timing_controller import get_timing_controller
from ..diagnostics.segment_diagnostics import (
...
class NativePydivertEngine(BaseBypassEngine):
        ...
        try:
            ...
            while cursor + 4 <= extensions_end:
                ...
                if ext_type == 0x0000:  # SNI
                    ...
                    if ext_len < 5:
                        break  # 2 (list_len) + 1 (type) + 2 (name_len)
                ...
                try:
                    ...
                    if self.recipe_mode:
                        ...
                        if not self._recipe_consumed and self._should_process_packet(
                            ...
                            self._recipe_consumed = (
                                True  # Применяем рецепт только один раз
                            )
    ...
    def _execute_segments_orchestration(
        ...
        try:
            ...
            )  # Предполагается, что у вас есть такой компонент
    ...
    def _execute_segments_with_timing(
        ...
        try:
                ...
                try:
                    ...
                    if success:
                        ...
                        stats_collector.complete_segment_execution(
                            ...
                            != context.tcp_flags,  # TCP flags modified
                            packet_info.tcp_window
                            != context.tcp_window_size,  # Window size modified
            ...
            if session_stats:
                self.logger.info(
                    f"Session statistics: {session_stats.success_rate_percent:.1f}% success rate, "
            ...
            success_rate = (
                (successful_segments / total_segments) * 100
                if total_segments > 0
                else 0
            )
            self.logger.info(
