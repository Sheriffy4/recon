...
from .base import BaseBypassEngine, EngineConfig, EngineStats
from ..attacks.base import AttackResult, AttackStatus, AttackContext, SegmentTuple
from ..attacks.segment_packet_builder import SegmentPacketBuilder, SegmentPacketInfo, validate_segments_for_building
from ..attacks.timing_controller import PreciseTimingController, get_timing_controller
from ..diagnostics.segment_diagnostics import get_segment_diagnostic_logger, SegmentDiagnosticData
from ..monitoring.segment_execution_stats import (
    get_segment_stats_collector, SegmentExecutionMetrics, ExecutionPhase, ExecutionStatus
)
from core.packet_converter import PacketConverter
...
from ...integration.strategy_prediction_integration import get_strategy_integrator
from ...integration.performance_integration import get_performance_integrator
...
class NativePydivertEngine(BaseBypassEngine):
    ...
    def __init__(self, config: EngineConfig):
        ...
        # +++ ИЗМЕНЕНИЕ: Инициализируем AttackAdapter и PacketBuilder +++
    ...
    def start(
        ...
        # <-- Сохраняем весь пул IP в конфиг перехвата -->
        ...
        self.logger.info(f"Filter target IPs: {target_ips}") # Логируем правильные IP
    ...
    def _extract_sni(self, payload: bytes) -> Optional[str]:
        ...
        try:
            if len(payload) < 42 or payload[0] != 0x16 or payload[5] != 0x01:
                return None
            
            cursor = 43 # Начало Session ID
            ...
            while cursor + 4 <= extensions_end:
                ...
                if ext_type == 0x0000: # SNI
                    # Мы нашли расширение SNI, теперь парсим его содержимое
                    sni_data_start = cursor
                    
                    # Проверяем минимальную длину содержимого SNI
                    if ext_len < 5: break # 2 (list_len) + 1 (type) + 2 (name_len)

                    # Длина списка имен (обычно содержит только одно имя)
                    server_name_list_len = struct.unpack('!H', payload[sni_data_start:sni_data_start+2])[0]
                    
                    # Тип имени (0x00 для host_name)
                    ...
                    if server_name_type == 0x00:
                        ...
                        # Финальная проверка, что длина имени не выходит за пределы расширения
                        if name_start + server_name_len <= sni_data_start + ext_len:
                            domain_bytes = payload[name_start : name_start + server_name_len]
                            # Декодируем имя хоста и возвращаем
                            return domain_bytes.decode('utf-8', errors='ignore')

                # Переходим к следующему расширению
                cursor += ext_len

            # Если прошли все расширения и не нашли SNI
            return None

        except (struct.error, IndexError) as e:
            # Эти ошибки могут возникнуть при парсинге поврежденного или неожиданного пакета
    ...
    def _build_filter(self) -> str:
        ...
        # Используем генератор фильтров для корректного форматирования
        from core.windivert_filter import WinDivertFilterGenerator
        ...
        try:
            with self._global_lock:
                if self._global_handle is None:
                    ...
                    # Получаем список кандидатов для фильтра, от сложного к простому
                    filter_candidates = self._filter_gen.progressive_candidates(
                        target_ips, target_ports, direction="outbound", protocols=("tcp",)
                    )
            ...
            # --- ЭТАП 2: ОСНОВНОЙ ЦИКЛ ОБРАБОТКИ ПАКЕТОВ ---
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    if self.recipe_mode:
                        # Если мы в режиме рецепта, используем специальный обработчик
                        if not self._recipe_consumed and self._should_process_packet(packet):
                            self._recipe_consumed = True # Применяем рецепт только один раз
            ...
            # Логируем критическую ошибку, которая привела к остановке всего цикла
            self.logger.error(f"Критическая ошибка в цикле перехвата: {e}", exc_info=self.config.debug)
        
        finally:
            # --- ЭТАП 3: ГАРАНТИРОВАННАЯ ОЧИСТКА РЕСУРСОВ ---
    ...
    def _should_process_packet(self, packet: pydivert.Packet) -> bool:
        ...
        """
        # Пакет должен быть исходящим, иметь TCP заголовок и payload
        ...
        if packet.tcp.dst_port == 443:
            ...
            return len(payload) > 5 and payload[0] == 0x16 and payload[5] == 0x01
            
        # Проверяем на HTTP GET/POST
    ...
    def _process_packet_with_recipe(self, packet: pydivert.Packet) -> bool:
        ...
        # Создаем временный AttackResult для передачи в оркестратор
        from ..attacks.base import AttackResult, AttackStatus
        recipe_result = AttackResult(status=AttackStatus.SUCCESS, segments=self.segments_recipe)
        
        # Вызываем оркестратор напрямую
    ...
    def _process_packet_with_attack(self, packet: pydivert.Packet) -> bool:
        ...
        # --- НАЧАЛО ИЗМЕНЕНИЯ ---
        # Проверяем, не является ли это "рецептом" от тестера эффективности
        ...
        if strategy and strategy.get("type") == "segments_recipe":
            ...
            context = self._create_enhanced_attack_context(packet)
            if not context:
                return False # Отправляем оригинал
            
            # Создаем временный AttackResult для передачи в оркестратор
            from ..attacks.base import AttackResult, AttackStatus
            recipe_result = AttackResult(status=AttackStatus.SUCCESS, segments=strategy.get("segments"))
            
            # Вызываем оркестратор напрямую и выходим
            return self._execute_segments_orchestration(recipe_result, context, packet)
        
        # +++ PHASE 1: ML-based strategy prediction if no strategy found +++
        if not strategy:
            try:
                # +++ PHASE1-2: Try to get domain for enhanced fingerprinting +++
                domain = self._get_domain_for_ip(packet.dst_addr)
                
                # Use ML predictor to get strategy recommendation
                recommendation = self.strategy_integrator.predict_best_strategy(
                    packet.dst_addr, 
                    domain=domain
                )
                
                # Create strategy from ML recommendation
                strategy = {
                ...
                return False # No strategy available, send original
        
        if not strategy:
            return False

        # Create enhanced AttackContext with TCP session information
        context = self._create_enhanced_attack_context(packet)
        if not context:
        ...
        try:
            # +++ PHASE1-3: Measure attack execution time +++
            attack_start_time = time.time()
            
            future = asyncio.run_coroutine_threadsafe(
                self.attack_adapter.execute_attack_by_name(attack_name, context, strategy_params=strategy),
                self._loop
            )
            ...
            if attack_result.status == AttackStatus.SUCCESS:
                if attack_result.has_segments():
                    # Используем новую, более надежную функцию оркестрации
                    ...
                    return False # Отправляем оригинальный пакет
            else:
            ...
            # +++ PHASE1-3: Calculate attack latency +++
            attack_latency_ms = (time.time() - attack_start_time) * 1000
            
            # +++ NEW: Enhanced segments orchestration +++
            if attack_result.status == AttackStatus.SUCCESS:
                if attack_result.has_segments():
                    return self._execute_segments_orchestration(attack_result, context, packet)
                elif attack_result.modified_payload:
            ...
            # +++ PHASE1-3: Record attack execution for performance monitoring +++
            self.performance_integrator.record_attack_executed(attack_latency_ms, success)
            
            # +++ PHASE 1: ML feedback for strategy effectiveness +++
            if strategy.get("ml_predicted", False):
                try:
                    # Use measured latency if not provided in metadata
            ...
            if success:
                # Check if we have segments to send
                ...
                return True # We processed and sent packets, don't send original
            else:
            ...
            # +++ PHASE1-3: Record failed attack for performance monitoring +++
            self.performance_integrator.record_attack_executed(0, False)
            
            # +++ PHASE 1: ML feedback for failed attacks +++
            if strategy.get("ml_predicted", False):
                try:
                    self.strategy_integrator.update_strategy_effectiveness(
            ...
            return False # Error in attack, send original
    ...
    def start_with_segments_recipe(
        ...
        self._recipe_consumed = False # Сбрасываем флаг перед каждым запуском
        
        # Для режима рецепта нам нужна только одна цель
        target_port = 443 # Предполагаем HTTPS для теста
        
        self.interception_config = InterceptionConfig(target_ips=target_ips, target_ports={target_port})
        self.strategy_map = {} # Карта стратегий не используется в этом режиме
    ...
    def _create_enhanced_attack_context(self, packet: pydivert.Packet) -> Optional[AttackContext]:
        ...
        try:
            ...
            # Create enhanced context with TCP session info
            context = AttackContext(
                ...
                packet_id=1,  # Will be managed by attack
                session_established=True,  # Assume established for outbound packets
    ...
    def _execute_segments_orchestration(self, attack_result: AttackResult, context: AttackContext, original_packet: pydivert.Packet) -> bool:
        ...
        try:
            # Получаем segments из metadata (новый способ) или из прямого атрибута (старый способ)
            ...
            # Используем timing_controller для точных задержек
            timing_controller = get_timing_controller() # Предполагается, что у вас есть такой компонент
            
            for i, segment_info in enumerate(segments):
                payload_data, seq_offset, options = segment_info
                
                # 1. Применяем задержку
                delay_ms = options.get("delay_ms", 0)
                if delay_ms > 0:
                    timing_controller.delay(delay_ms)

                # 2. Собираем пакет с помощью SegmentPacketBuilder
    ...
    def _execute_segments_with_timing(
        ...
        segments: list, 
        context: AttackContext,
        original_packet: pydivert.Packet  # ADD THIS PARAMETER
    ) -> bool:
        """
        ...
        # Get controllers - FIX 2: timing_controller needs proper initialization
        timing_controller = get_timing_controller()  # Get the singleton instance
        ...
        try:
            for i, segment in enumerate(segments):
                ...
                try:
                    ...
                    # Update statistics for validation phase
                    ...
                    if not is_valid:
                        ...
                        # Complete segment statistics with failure
                    ...
                    # Update statistics for construction phase
                    stats_collector.update_segment_phase(
                        segment_metrics, ExecutionPhase.CONSTRUCTION, construction_time
                    )
                    
                    # Apply modifications and log them
                    ...
                    if delay_ms > 0:
                        ...
                        # Update statistics for timing phase
                        stats_collector.update_segment_phase(
                            segment_metrics, ExecutionPhase.TIMING, timing_phase_time
                        )
                    
                    # Transmission phase - NOW PASSING original_packet
                    ...
                    diagnostic_logger.log_transmission_phase(
                        segment_data, transmission_time, success, 
                        None if success else "Transmission failed"
                    )
                    
                    # Update statistics for transmission phase
                    stats_collector.update_segment_phase(
                    ...
                    if success:
                        ...
                        # Complete segment statistics with success
                        stats_collector.complete_segment_execution(
                            segment_metrics, 
                            ExecutionStatus.SUCCESS,
                            None,  # No error message
                            packet_info.packet_size,
                            packet_info.ttl != 64,  # TTL modified if not default
                            packet_info.checksum_corrupted,
                            packet_info.tcp_flags != context.tcp_flags,  # TCP flags modified
                            packet_info.tcp_window != context.tcp_window_size,  # Window size modified
                            timing_result.accuracy_error_ms if timing_result else 0.0
                        )
                        
                        # Log successful segment execution
                        ...
                        # Complete segment statistics with failure
                        stats_collector.complete_segment_execution(
                            segment_metrics, ExecutionStatus.FAILED, "Transmission failed"
                        )
                    ...
                    # Log error in diagnostics if segment_data exists
                    if segment_data:
                        diagnostic_logger.log_transmission_phase(
                            segment_data, 0.0, False, error_msg
                        )
                    
                    # Complete segment statistics with error
            ...
            # End diagnostic session and get summary
            ...
            if session_stats:
                self.logger.info(f"Session statistics: {session_stats.success_rate_percent:.1f}% success rate, "
                               f"{session_stats.avg_segment_time_ms:.2f}ms avg time, "
                               f"{session_stats.throughput_segments_per_sec:.1f} segments/sec")
            
            # Log basic execution summary
            ...
            # Try to end sessions gracefully
            try:
    ...
    def _apply_ttl_modification(self, packet_info: 'SegmentPacketInfo', ttl: int) -> None:
        ...
        """
        try:
            # TTL is already applied during packet construction
            # This method is for logging and validation
    ...
    def _apply_checksum_corruption(self, packet_info: 'SegmentPacketInfo') -> None:
        ...
        """
        try:
            # Checksum corruption is already applied during packet construction
            # This method is for logging and validation
    ...
    def _send_segment_packet(self, packet_info: 'SegmentPacketInfo', segment_num: int, original_packet: pydivert.Packet) -> bool:
        ...
        """
        try:
            # --- ИЗМЕНЕНИЕ 2: Создаем pydivert.Packet с правильными аргументами ---
    ...
    def _log_timing_execution(self, segment_num: int, timing_result) -> None:
        ...
        """
        self.logger.debug(
            f"Segment {segment_num} timing: requested={timing_result.requested_delay_ms:.3f}ms, "
            f"actual={timing_result.actual_delay_ms:.3f}ms, "
            f"error={timing_result.accuracy_error_ms:.3f}ms, "
            f"strategy={timing_result.strategy_used.value}"
        )
    ...
    def _log_segment_execution(self, segment_num: int, packet_info: 'SegmentPacketInfo', options: dict, execution_time_ms: float):
        ...
        if self.logger.isEnabledFor(logging.DEBUG):
            # Basic packet info
            self.logger.debug(
                f"Segment {segment_num}: {packet_info.packet_size} bytes, "
                f"seq={packet_info.tcp_seq}, ack={packet_info.tcp_ack}, "
                f"flags=0x{packet_info.tcp_flags:02x}, window={packet_info.tcp_window}"
            )
            
            # Modifications applied
            modifications = []
            if packet_info.ttl != 64:  # Default TTL
            ...
            # Performance metrics
            self.logger.debug(
                f"Segment {segment_num} performance: "
                f"build_time={packet_info.construction_time_ms:.3f}ms, "
                f"total_time={execution_time_ms:.3f}ms"
            )
        
        # Always log summary for info level
        if self.logger.isEnabledFor(logging.INFO):
            payload_size = len(options.get('payload_data', b'')) if 'payload_data' in options else 0
            self.logger.info(
                f"Segment {segment_num} executed: {payload_size} bytes payload, "
                f"{packet_info.packet_size} bytes total"
            )
    ...
    def stop(self) -> bool:
        ...
        # --- НАЧАЛО ИЗМЕНЕНИЯ: Убираем "Ellipsis" ---
    ...
    def get_stats(self) -> EngineStats:
        ...
        # Add segment builder statistics to engine stats if available
        if hasattr(self, 'segment_builder'):
            segment_stats = self.segment_builder.get_stats()
            # Add metadata field to stats if it doesn't exist
        ...
        # Add timing controller statistics - FIX: Handle dict return type
        try:
            ...
            if timing_data is not None:
                if isinstance(timing_data, dict):
                    ...
                    self.stats.metadata.update({
                        "timing_total_delays": timing_data.get("total_delays", timing_data.get("delays_total", 0)),
                        "timing_average_accuracy": timing_data.get("average_accuracy_percent", timing_data.get("avg_accuracy_percent", 0.0)),
                        "timing_errors": timing_data.get("timing_errors", timing_data.get("errors", 0)),
                        "timing_total_requested_ms": timing_data.get("total_requested_time_ms", 0.0),
                        "timing_total_actual_ms": timing_data.get("total_actual_time_ms", 0.0),
                    })
                else:
                    self.stats.metadata.update({
                        "timing_total_delays": getattr(timing_data, "total_delays", 0),
                        "timing_average_accuracy": getattr(timing_data, "average_accuracy_percent", getattr(timing_data, "avg_accuracy_percent", 0.0)),
                        "timing_errors": getattr(timing_data, "timing_errors", getattr(timing_data, "errors", 0)),
                        "timing_total_requested_ms": getattr(timing_data, "total_requested_time_ms", 0.0),
                        "timing_total_actual_ms": getattr(timing_data, "total_actual_time_ms", 0.0),
                    })
        except Exception as e:
    ...
    def get_segment_execution_report(self) -> Dict[str, Any]:
        ...
        # Add segment builder report
        if hasattr(self, 'segment_builder'):
            segment_stats = self.segment_builder.get_stats()
            report["segment_builder"] = segment_stats
        
        # Add timing controller report
        try:
            timing_controller = get_timing_controller()
        ...
        # Add segment execution statistics
        try:
            stats_collector = get_segment_stats_collector()
    ...
    async def execute_dry_run_test(self, attack_name: str, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Execute attack in dry run mode using attack adapter
            result = await self.attack_adapter.execute_attack_by_name(
                attack_name, context, dry_run=True
            )
            
            # Add engine-specific dry run metadata
    ...
    def _log_dry_run_results(self, result: AttackResult, attack_name: str, context: AttackContext):
        ...
        self.logger.info("=" * 60)
        self.logger.info(f"Engine: NativePyDivertEngine")
        ...
        if "segment_analysis" in result.metadata:
            ...
            self.logger.info(f"Segments Analysis:")
    ...
    def test_attack_scenarios(self, attack_scenarios: list) -> dict:
        ...
        for i, (attack_name, context) in enumerate(attack_scenarios):
            try:
                ...
                future = asyncio.run_coroutine_threadsafe(
                    self.execute_dry_run_test(attack_name, context),
                    self._loop
                )
    ...
    def _get_domain_for_ip(self, ip_address: str) -> Optional[str]:
        ...
        # Could add reverse DNS lookup here, but it might be slow
        # For now, return None to use IP-based prediction
        return None
    ...
    def _apply_attack_strategy(
        ...
        """
        try:
            # Create AttackContext from packet_info
            ...
            # Convert packet_data to Scapy packet for AttackAdapter
            ...
            # Use AttackAdapter to execute strategy
            if hasattr(self, 'attack_adapter') and self.attack_adapter:
                # Convert strategy to attack name
