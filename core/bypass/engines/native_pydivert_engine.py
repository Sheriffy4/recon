...
class NativePydivertEngine(BaseBypassEngine):
        ...
        try:
            ...
            while cursor + 4 <= extensions_end:
                ...
                if ext_type == 0:
                    ...
                    if ext_len < 5:
                        break
                    server_name_list_len = struct.unpack('!H', payload[sni_data_start:sni_data_start + 2])[0]
                    server_name_type = payload[sni_data_start + 2]
                    if server_name_type == 0:
                        name_len_start = sni_data_start + 3
                        server_name_len = struct.unpack('!H', payload[name_len_start:name_len_start + 2])[0]
    ...
    def _build_filter(self) -> str:
        ...
        try:
            return filter_gen.generate(target_ips=self.interception_config.target_ips, target_ports=self.interception_config.target_ports, direction='outbound', protocols=('tcp',))
        except Exception as e:
        ...
        try:
            with self._global_lock:
                if self._global_handle is None:
                    target_ips = self.interception_config.target_ips if self.interception_config else set()
                    target_ports = self.interception_config.target_ports if self.interception_config else {443}
                    filter_candidates = self._filter_gen.progressive_candidates(target_ips, target_ports, direction='outbound', protocols=('tcp',))
                    created = False
                    last_error = None
                    for filter_str in filter_candidates:
                        self.logger.debug(f'Попытка создать хендл WinDivert с фильтром: {filter_str}')
                        try:
                            self._global_handle = pydivert.WinDivert(filter_str)
                            self._global_handle.open()
                            self.logger.info(f'Успешно создан глобальный хендл с фильтром: {filter_str}')
                            created = True
                            break
                        except Exception as e:
                            last_error = e
                            self.logger.warning(f"Не удалось создать хендл с фильтром '{filter_str}': {e}")
                            self._global_handle = None
                    if not created:
                        self.logger.critical(f'КРИТИЧЕСКАЯ ОШИБКА: Не удалось создать ни один хендл WinDivert. Последняя ошибка: {last_error}')
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    was_handled = False
                    if self.recipe_mode:
                        if not self._recipe_consumed and self._should_process_packet(packet):
            ...
            self.logger.error(f'Критическая ошибка в цикле перехвата: {e}', exc_info=self.config.debug)
        finally:
    ...
    def start_with_strategy_or_segments(self, target_ips: Set[str], strategy_or_segments: Union[Dict[str, Dict[str, Any]], List[SegmentTuple]]) -> bool:
        """
    ...
    def _process_packet_with_attack(self, packet: pydivert.Packet) -> bool:
        ...
        if not strategy:
            try:
                ...
                recommendation = self.strategy_integrator.predict_best_strategy(packet.dst_addr, domain=domain)
                strategy = {'type': recommendation.primary_strategy, 'name': recommendation.primary_strategy, 'params': {}, 'ml_predicted': True, 'confidence': recommendation.confidence, 'fallbacks': recommendation.fallback_strategies, 'domain': domain}
                LOG.info(f'ML predicted strategy for {packet.dst_addr} ({domain}): {recommendation.primary_strategy} (confidence: {recommendation.confidence:.2f})')
            except Exception as e:
        ...
        try:
            ...
            future = asyncio.run_coroutine_threadsafe(self.attack_adapter.execute_attack_by_name(attack_name, context, strategy_params=strategy), self._loop)
            ...
            if attack_result.status == AttackStatus.SUCCESS:
                if attack_result.has_segments():
                    ...
                    self.logger.warning(f"Attack '{attack_name}' succeeded but produced no packets.")
                    return False
            else:
                self.logger.error(f"Attack '{attack_name}' failed: {attack_result.error_message}")
                return False
        except Exception as e:
            self.logger.error(f"Attack execution for '{attack_name}' raised an exception: {e}", exc_info=self.config.debug)
            return False
            ...
            if strategy.get('ml_predicted', False):
                try:
                    ...
                    self.strategy_integrator.update_strategy_effectiveness(packet.dst_addr, attack_name, success, feedback_latency)
                except Exception as e:
            ...
            if success:
                ...
                error_msg = attack_result.error_message if hasattr(attack_result, 'error_message') else 'No error message'
                self.logger.debug(f"Attack '{attack_name}' failed: {attack_result.status.value}. Error: {error_msg}")
            ...
            if strategy.get('ml_predicted', False):
                try:
                    self.strategy_integrator.update_strategy_effectiveness(packet.dst_addr, attack_name, False, 0)
                except Exception as e:
                    LOG.debug(f'Failed to update ML feedback for failure: {e}')
            return False

    def start_with_segments_recipe(self, target_ips: Set[str], segments_recipe: List[SegmentTuple]) -> bool:
        """
        ...
        self._recipe_consumed = False
        target_port = 443
        self.interception_config = InterceptionConfig(target_ips=target_ips, target_ports={target_port})
        self.strategy_map = {}
        self.stop_event.clear()
        self.intercept_thread = threading.Thread(target=self._intercept_loop, daemon=True, name='PydivertRecipeThread')
        self.intercept_thread.start()
        time.sleep(0.5)
        self.is_running = True
        self.stats.start_time = time.time()
    ...
    def _create_enhanced_attack_context(self, packet: pydivert.Packet) -> Optional[AttackContext]:
        ...
        try:
            ...
            context = AttackContext(dst_ip=packet.dst_addr, dst_port=packet.dst_port, src_ip=packet.src_addr, src_port=packet.src_port, domain=self._get_domain_for_ip(packet.dst_addr), payload=payload, tcp_seq=packet.tcp.seq_num if packet.tcp else 0, tcp_ack=packet.tcp.ack_num if packet.tcp else 0, tcp_flags=self._get_tcp_flags_int(packet), tcp_window_size=packet.tcp.window_size if packet.tcp else 65535, tcp_urgent_pointer=packet.tcp.urg_ptr if packet.tcp else 0, connection_id=f'{packet.src_addr}:{packet.src_port}->{packet.dst_addr}:{packet.dst_port}', packet_id=1, session_established=True, initial_seq=packet.tcp.seq_num if packet.tcp else 0)
            return context
        except Exception as e:
    ...
    def _execute_segments_orchestration(self, attack_result: AttackResult, context: AttackContext, original_packet: pydivert.Packet) -> bool:
        """Исполняет отправку сегментов с полным контролем над параметрами пакетов."""
        try:
            segments = attack_result.get_metadata('segments') or getattr(attack_result, 'segments', None)
            if not segments:
                self.logger.warning('No segments found in attack result for orchestration')
                return False
            self.logger.info(f'Orchestrating {len(segments)} segments for connection {context.connection_id}')
            ...
            for i, segment_info in enumerate(segments):
                ...
                packet_info = self.segment_builder.build_segment(payload=payload_data, seq_offset=seq_offset, options=options, context=context)
                pydivert_packet = pydivert.Packet(packet_info.packet_bytes, original_packet.interface, original_packet.direction)
    ...
    def _validate_segments_for_execution(self, segments: list, context: AttackContext) -> tuple[bool, Optional[str]]:
        """
    ...
    def _execute_segments_with_timing(self, segments: list, context: AttackContext, original_packet: pydivert.Packet) -> bool:
        """
        ...
        try:
            for i, segment in enumerate(segments):
                ...
                try:
                    ...
                    segment_data = diagnostic_logger.log_segment_start(session_id, i + 1, len(payload_data), seq_offset, options_dict)
                    segment_metrics = stats_collector.start_segment_execution(i + 1, session_id, len(payload_data), seq_offset, options_dict)
                    validation_start = time.time()
                    is_valid, error_msg = self._validate_single_segment(segment, context)
                    validation_time = (time.time() - validation_start) * 1000
                    diagnostic_logger.log_validation_phase(segment_data, validation_time, is_valid, error_msg)
                    ...
                    if not is_valid:
                        ...
                        stats_collector.complete_segment_execution(segment_metrics, ExecutionStatus.FAILED, error_msg)
                        continue
                    construction_start = time.time()
                    packet_info = self.segment_builder.build_segment(payload_data, seq_offset, options_dict, context)
                    ...
                    diagnostic_logger.log_transmission_phase(segment_data, transmission_time, success, None if success else 'Transmission failed')
                    ...
                    if success:
                        ...
                        stats_collector.complete_segment_execution(segment_metrics, ExecutionStatus.SUCCESS, None, packet_info.packet_size, packet_info.ttl != 64, packet_info.checksum_corrupted, packet_info.tcp_flags != context.tcp_flags, packet_info.tcp_window != context.tcp_window_size, timing_result.accuracy_error_ms if timing_result else 0.0)
                        ...
                        stats_collector.complete_segment_execution(segment_metrics, ExecutionStatus.FAILED, 'Transmission failed')
                except Exception as e:
                    error_msg = str(e)
                    self.logger.error(f'Failed to execute segment {i + 1}: {error_msg}')
                    if segment_data:
                        diagnostic_logger.log_transmission_phase(segment_data, 0.0, False, error_msg)
                    if 'segment_metrics' in locals():
                        stats_collector.complete_segment_execution(segment_metrics, ExecutionStatus.ERROR, error_msg)
                    continue
            ...
            if session_stats:
                self.logger.info(f'Session statistics: {session_stats.success_rate_percent:.1f}% success rate, {session_stats.avg_segment_time_ms:.2f}ms avg time, {session_stats.throughput_segments_per_sec:.1f} segments/sec')
            success_rate = successful_segments / total_segments * 100 if total_segments > 0 else 0
            self.logger.info(f'Segments execution completed: {successful_segments}/{total_segments} successful ({success_rate:.1f}%)')
    ...
    def _validate_single_segment(self, segment: tuple, context: AttackContext) -> tuple[bool, Optional[str]]:
        """
    ...
    def _apply_sequence_adjustment(self, packet_info: 'SegmentPacketInfo', seq_offset: int, context: AttackContext) -> None:
        """
        ...
        try:
            ...
            if packet_info.tcp_seq != expected_seq:
                self.logger.warning(f'Sequence number mismatch: expected {expected_seq}, got {packet_info.tcp_seq}')
            else:
                self.logger.debug(f'Sequence adjustment applied: base={context.tcp_seq}, offset={seq_offset}, final={packet_info.tcp_seq}')
        except Exception as e:
            self.logger.error(f'Sequence adjustment validation failed: {e}')

    def _send_segment_packet(self, packet_info: 'SegmentPacketInfo', segment_num: int, original_packet: pydivert.Packet) -> bool:
        """
        Send segment packet via PyDivert with error handling.
        """
        try:
            packet_obj = pydivert.Packet(packet_info.packet_bytes, original_packet.interface, original_packet.direction)
            self.windivert_handle.send(packet_obj)
            self.logger.debug(f'Segment {segment_num} sent successfully ({packet_info.packet_size} bytes)')
            return True
        except Exception as e:
    ...
    def _log_timing_execution(self, segment_num: int, timing_result) -> None:
        ...
        """
        self.logger.debug(f'Segment {segment_num} timing: requested={timing_result.requested_delay_ms:.3f}ms, actual={timing_result.actual_delay_ms:.3f}ms, error={timing_result.accuracy_error_ms:.3f}ms, strategy={timing_result.strategy_used.value}')

    def _log_segment_execution(self, segment_num: int, packet_info: 'SegmentPacketInfo', options: dict, execution_time_ms: float):
        """
        ...
        if self.logger.isEnabledFor(logging.DEBUG):
            self.logger.debug(f'Segment {segment_num}: {packet_info.packet_size} bytes, seq={packet_info.tcp_seq}, ack={packet_info.tcp_ack}, flags=0x{packet_info.tcp_flags:02x}, window={packet_info.tcp_window}')
            modifications = []
            ...
            if modifications:
                self.logger.debug(f"Segment {segment_num} modifications: {', '.join(modifications)}")
            self.logger.debug(f'Segment {segment_num} performance: build_time={packet_info.construction_time_ms:.3f}ms, total_time={execution_time_ms:.3f}ms')
        if self.logger.isEnabledFor(logging.INFO):
            payload_size = len(options.get('payload_data', b'')) if 'payload_data' in options else 0
            self.logger.info(f'Segment {segment_num} executed: {payload_size} bytes payload, {packet_info.packet_size} bytes total')

    def _send_modified_packet(self, original_packet: pydivert.Packet, modified_payload: bytes) -> bool:
        """
        ...
        """
        try:
            modified_packet = pydivert.Packet(original_packet.raw, original_packet.interface, original_packet.direction)
    ...
    def is_healthy(self) -> bool:
        if not self.is_running or not self.intercept_thread or (not self.intercept_thread.is_alive()):
            return False
    ...
    def get_stats(self) -> EngineStats:
        ...
        if hasattr(self, 'segment_builder'):
            ...
            if not hasattr(self.stats, 'metadata'):
                self.stats.metadata = {}
            self.stats.metadata.update({'segment_packets_built': segment_stats.get('packets_built', 0), 'segment_build_time_ms': segment_stats.get('total_build_time_ms', 0.0), 'segment_avg_build_time_ms': segment_stats.get('avg_build_time_ms', 0.0), 'segment_ttl_modifications': segment_stats.get('ttl_modifications', 0), 'segment_checksum_corruptions': segment_stats.get('checksum_corruptions', 0), 'segment_flag_modifications': segment_stats.get('flag_modifications', 0)})
        try:
            timing_controller = get_timing_controller()
            timing_data = None
            ...
            if timing_data is not None:
                if isinstance(timing_data, dict):
                    ...
                    self.stats.metadata.update({'timing_total_delays': timing_data.get('total_delays', timing_data.get('delays_total', 0)), 'timing_average_accuracy': timing_data.get('average_accuracy_percent', timing_data.get('avg_accuracy_percent', 0.0)), 'timing_errors': timing_data.get('timing_errors', timing_data.get('errors', 0)), 'timing_total_requested_ms': timing_data.get('total_requested_time_ms', 0.0), 'timing_total_actual_ms': timing_data.get('total_actual_time_ms', 0.0)})
                else:
                    self.stats.metadata.update({'timing_total_delays': getattr(timing_data, 'total_delays', 0), 'timing_average_accuracy': getattr(timing_data, 'average_accuracy_percent', getattr(timing_data, 'avg_accuracy_percent', 0.0)), 'timing_errors': getattr(timing_data, 'timing_errors', getattr(timing_data, 'errors', 0)), 'timing_total_requested_ms': getattr(timing_data, 'total_requested_time_ms', 0.0), 'timing_total_actual_ms': getattr(timing_data, 'total_actual_time_ms', 0.0)})
        except Exception as e:
    ...
    def get_segment_execution_report(self) -> Dict[str, Any]:
        ...
        """
        report = {'engine_stats': {'packets_processed': self.stats.packets_processed, 'packets_sent': self.stats.packets_sent, 'modified_packets': getattr(self.stats, 'modified_packets', 0), 'errors': self.stats.errors}}
        if hasattr(self, 'segment_builder'):
            segment_stats = self.segment_builder.get_stats()
    ...
    async def execute_dry_run_test(self, attack_name: str, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            result = await self.attack_adapter.execute_attack_by_name(attack_name, context, dry_run=True)
            result.metadata = result.metadata or {}
            result.metadata.update({'engine_dry_run': True, 'engine_type': 'native_pydivert', 'engine_stats': self.get_stats().to_dict()})
    ...
    def test_attack_scenarios(self, attack_scenarios: list) -> dict:
        ...
        for i, (attack_name, context) in enumerate(attack_scenarios):
            try:
                ...
                future = asyncio.run_coroutine_threadsafe(self.execute_dry_run_test(attack_name, context), self._loop)
                result = future.result(timeout=30.0)
                results[f'{attack_name}_{i}'] = {'attack_name': attack_name, 'status': result.status.value, 'success': result.status == AttackStatus.SUCCESS, 'simulation_time_ms': result.metadata.get('simulation_time_ms', 0), 'segments_valid': result.metadata.get('segments_valid', True), 'validation_errors': result.metadata.get('validation_errors', []), 'error_message': result.error_message}
            except Exception as e:
                self.logger.error(f'Scenario test failed for {attack_name}: {e}')
                results[f'{attack_name}_{i}'] = {'attack_name': attack_name, 'status': 'ERROR', 'success': False, 'error_message': str(e)}
        total_scenarios = len(attack_scenarios)
        successful_scenarios = sum((1 for r in results.values() if r.get('success', False)))
        summary = {'total_scenarios': total_scenarios, 'successful_scenarios': successful_scenarios, 'failed_scenarios': total_scenarios - successful_scenarios, 'success_rate': successful_scenarios / total_scenarios * 100 if total_scenarios > 0 else 0, 'results': results}
        self.logger.info(f"Attack scenario testing completed: {successful_scenarios}/{total_scenarios} successful ({summary['success_rate']:.1f}%)")
        return summary
    ...
    def get_diagnostic_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'diagnostic_system': global_stats, 'segment_execution_stats': {'total_sessions': segment_stats.total_sessions, 'active_sessions': segment_stats.active_sessions, 'completed_sessions': segment_stats.completed_sessions, 'total_segments_processed': segment_stats.total_segments_processed, 'successful_segments': segment_stats.total_successful_segments, 'failed_segments': segment_stats.total_failed_segments, 'global_success_rate_percent': segment_stats.global_success_rate_percent, 'global_throughput_segments_per_sec': segment_stats.global_throughput_segments_per_sec, 'global_avg_timing_accuracy_percent': segment_stats.global_avg_timing_accuracy_percent}, 'engine_integration': {'diagnostic_sessions_active': global_stats.get('active_sessions', 0), 'total_diagnostic_sessions': global_stats.get('total_sessions', 0), 'segments_diagnosed': global_stats.get('total_segments_processed', 0)}}
        except Exception as e:
    ...
    def get_recent_session_stats(self, count: int=10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [{'session_id': session.session_id, 'connection_id': session.connection_id, 'total_segments': session.total_segments, 'successful_segments': session.successful_segments, 'success_rate_percent': session.success_rate_percent, 'avg_segment_time_ms': session.avg_segment_time_ms, 'throughput_segments_per_sec': session.throughput_segments_per_sec, 'timing_accuracy_percent': session.timing_accuracy_percent, 'total_execution_time_ms': session.total_execution_time_ms} for session in recent_sessions]
        except Exception as e:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'throughput': {'segments_per_sec': global_stats.global_throughput_segments_per_sec, 'bytes_per_sec': global_stats.global_throughput_bytes_per_sec, 'avg_segments_per_session': global_stats.avg_segments_per_session}, 'timing': {'avg_accuracy_percent': global_stats.global_avg_timing_accuracy_percent, 'avg_construction_time_ms': global_stats.global_avg_construction_time_ms, 'avg_transmission_time_ms': global_stats.global_avg_transmission_time_ms}, 'reliability': {'success_rate_percent': global_stats.global_success_rate_percent, 'error_rate_percent': global_stats.error_rate_percent, 'total_sessions': global_stats.total_sessions, 'completed_sessions': global_stats.completed_sessions}, 'modifications': {'ttl_modifications': global_stats.total_ttl_modifications, 'checksum_corruptions': global_stats.total_checksum_corruptions, 'tcp_flags_modifications': global_stats.total_tcp_flags_modifications, 'window_size_modifications': global_stats.total_window_size_modifications}, 'recent_performance': performance_summary.get('recent_performance', {}), 'timing_analysis': performance_summary.get('timing_analysis', {}), 'modification_analysis': performance_summary.get('modification_analysis', {}), 'error_analysis': performance_summary.get('error_analysis', {})}
        except Exception as e:
    ...
    def _get_domain_for_ip(self, ip_address: str) -> Optional[str]:
        ...
        ip_to_domain = {'104.21.32.39': 'rutracker.org', '185.60.216.35': 'nnmclub.to', '31.13.24.36': 'instagram.com', '217.69.139.200': 'mail.ru'}
        if ip_address in ip_to_domain:
            return ip_to_domain[ip_address]
        return None

    def _apply_attack_strategy(self, packet_data: bytes, packet_info: dict, strategy: dict) -> Optional[bytes]:
        """
        ...
        try:
            ...
            context = AttackContext(dst_ip=packet_info.get('dst_ip', ''), dst_port=packet_info.get('dst_port', 0), src_ip=packet_info.get('src_ip', ''), src_port=packet_info.get('src_port', 0), payload=packet_data, protocol=packet_info.get('protocol', 'tcp'))
                    ...
                    if result and result.status == AttackStatus.SUCCESS:
                        ...
                        if modified_packets:
                            ...
                            if hasattr(modified_packet, 'raw'):
                                self.logger.debug(f"Applied strategy {strategy.get('method', 'unknown')}: packet modified")
                                return bytes(modified_packet.raw)
                            else:
                                return modified_packet
                        elif segments:
                            self.logger.debug(f"Applied strategy {strategy.get('method', 'unknown')}: generated {len(segments)} segments")
                            return None
                        else:
                            self.logger.debug(f"Strategy {strategy.get('method', 'unknown')} did not modify packet")
                            return None
                    else:
                        self.logger.debug(f"Strategy {strategy.get('method', 'unknown')} execution failed")
                        return None
                except asyncio.TimeoutError:
                    self.logger.warning(f"Strategy {strategy.get('method', 'unknown')} execution timed out")
                    return None
                except Exception as e:
                    self.logger.error(f"Strategy {strategy.get('method', 'unknown')} execution error: {e}")
                    return None
            else:
