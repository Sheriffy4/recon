...
class EngineTypeDetector:
    ...
    def get_recommended_engine(self) -> EngineType:
        ...
        if not available:
            self.logger.warning('No engines with all dependencies available, falling back to EXTERNAL_TOOL')
    ...
    def _detect_engine_type(self, engine_type: EngineType) -> EngineDetectionResult:
        ...
        if engine_type == EngineType.NATIVE_PYDIVERT:
            ...
            return EngineDetectionResult(engine_type=engine_type, available=False, score=0, dependencies_met=False, missing_dependencies=['Unknown engine type'], warnings=['Unknown engine type'])

    def _detect_pydivert_engine(self) -> EngineDetectionResult:
        """Detect PyDivert engine availability."""
        result = EngineDetectionResult(engine_type=EngineType.NATIVE_PYDIVERT, available=False, score=100, dependencies_met=False)
    ...
    def _detect_external_tool_engine(self) -> EngineDetectionResult:
        ...
        result = EngineDetectionResult(engine_type=EngineType.EXTERNAL_TOOL, available=True, score=50, dependencies_met=True, missing_dependencies=[])
        tools_found = []
        if self._check_external_tool('zapret'):
            tools_found.append('zapret')
        if not tools_found:
            result.warnings.append('No specific external tools detected, but engine should work with any compatible tool')
    ...
    def _detect_netfilter_engine(self) -> EngineDetectionResult:
        ...
        result = EngineDetectionResult(engine_type=EngineType.NATIVE_NETFILTER, available=False, score=80, dependencies_met=False)
        ...
        if self._check_netfilter_support():
            ...
            result.missing_dependencies.extend(['netfilter support', 'netfilter engine implementation'])
            result.warnings.append('Netfilter support not detected')
            result.installation_hints.extend(['Ensure netfilter is available on your Linux system', 'Netfilter engine is not yet implemented'])
        return result

    def _assess_system_capabilities(self) -> SystemCapabilities:
        """Assess comprehensive system capabilities."""
        capabilities = SystemCapabilities(platform=platform.system(), is_windows=platform.system() == 'Windows', is_linux=platform.system() == 'Linux', is_admin=self._check_admin_privileges(), python_version=sys.version)
        capabilities.available_packages = {'pydivert': self._check_package_availability('pydivert'), 'scapy': self._check_package_availability('scapy'), 'netfilterqueue': self._check_package_availability('netfilterqueue')}
        capabilities.network_interfaces = self._get_network_interfaces()
        capabilities.permissions = {'admin': capabilities.is_admin, 'network': capabilities.is_admin}
        return capabilities
    ...
    def _check_external_tool(self, tool_name: str) -> bool:
        ...
        try:
            result = subprocess.run(['where' if platform.system() == 'Windows' else 'which', tool_name], capture_output=True, text=True, timeout=5)
