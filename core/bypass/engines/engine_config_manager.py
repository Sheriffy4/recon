# recon/core/bypass/engines/engine_config_manager.py
"""
...
class EngineConfigManager:
    ...
    def get_default_engine_type(self) -> EngineType:
        ...
        Returns:
            Default engine type
        """
        # Check for explicit default in configuration
        ...
        # Get highest priority enabled engine
        available_profiles = [
            profile for profile in self._profiles.values() if profile.enabled
        ]

        if not available_profiles:
            # Fallback to platform-based default
    ...
    def get_engine_config_object(
        ...
        # Create EngineConfig with safe defaults
        return EngineConfig(
    ...
    def validate_config(
        self, config: Dict[str, Any], engine_type: Optional[EngineType] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if engine_type:
                ...
                if profile:
                    ...
                    # This could be extended with more specific validation rules
                    pass
    ...
    def get_fallback_order(
        ...
        # Start with preferred engine if specified
        if preferred_engine and preferred_engine in self._profiles:
            ...
            if profile.enabled:
                ...
                # Add its specific fallbacks
                fallback_order.extend(
                    [
                        et
                        for et in profile.fallback_engines
                        if et in self._profiles and self._profiles[et].enabled
                    ]
                )

        # Add remaining engines by priority
        remaining_engines = [
            profile.engine_type
            for profile in self._profiles.values()
            if profile.enabled and profile.engine_type not in fallback_order
        ]
    ...
    def get_configuration_state(self) -> Dict[str, Any]:
        ...
        """
        return {
            "loaded_from": [source.value for source in self._state.loaded_from],
            "config_files": self._state.config_files,
            ...
            "profiles": {
                ...
                for et, profile in self._profiles.items()
            },
    ...
    def _load_default_configuration(self):
        ...
        # PyDivert Engine Profile
        self._profiles[EngineType.NATIVE_PYDIVERT] = EngineConfigProfile(
            ...
            platform_specific={"Windows": {"packet_buffer_size": 65535}},
            description="High-performance Windows packet interception using PyDivert",
        )

        # External Tool Engine Profile
        self._profiles[EngineType.EXTERNAL_TOOL] = EngineConfigProfile(
            ...
            fallback_engines=[],
            platform_specific={
                "Windows": {"tool_name": "zapret"},
                "Linux": {"tool_name": "zapret"},
            },
            description="Cross-platform engine using external DPI bypass tools",
        )

        # Netfilter Engine Profile
        self._profiles[EngineType.NATIVE_NETFILTER] = EngineConfigProfile(
            engine_type=EngineType.NATIVE_NETFILTER,
            priority=80,  # High priority for Linux
            ...
            dependencies=["netfilterqueue"],
            fallback_engines=[EngineType.EXTERNAL_TOOL],
            platform_specific={"Linux": {"packet_buffer_size": 65535}},
            description="Linux netfilter-based packet interception (not implemented)",
        )

        # Global configuration defaults
        self._global_config = {
            "default_engine": None,  # Auto-detect
    ...
    def _load_config_file(self, config_path: Path):
        ...
        with open(config_path, "r") as f:
            if config_path.suffix.lower() == ".json":
                ...
                # Try JSON first, could add YAML support later
                config_data = json.load(f)
    ...
    def _get_platform_default(self) -> EngineType:
        ...
        if current_platform == "Windows":
            ...
            )  # Would be NATIVE_NETFILTER when implemented
        else:
