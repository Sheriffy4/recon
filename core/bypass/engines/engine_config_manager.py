...
class EngineConfigManager:
        ...
        if preferred_engine and preferred_engine in self._profiles:
            ...
            if profile.enabled:
                ...
                fallback_order.extend([
                    et for et in profile.fallback_engines 
                    if et in self._profiles and self._profiles[et].enabled
                ])
        
        # Add remaining engines by priority
        remaining_engines = [
            profile.engine_type for profile in self._profiles.values()
            if profile.enabled and profile.engine_type not in fallback_order
        ]
        
        # Sort by priority
        remaining_engines.sort(
            key=lambda et: self._profiles[et].priority, 
            reverse=True
        )
    ...
    def _get_platform_default(self) -> EngineType:
        ...
        if current_platform == "Windows":
            ...
            return EngineType.EXTERNAL_TOOL  # Would be NATIVE_NETFILTER when implemented
        else:
