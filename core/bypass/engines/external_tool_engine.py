# recon/core/bypass/engines/external_tool_engine.py
"""
...
class ExternalToolEngine(BaseBypassEngine):
    ...
    TOOL_CONFIGS = {
        "zapret": {
            "windows": {
                "executable": "winws.exe",
                "path": "bin",
                "base_args": ["--wf-l3=ipv4"],
            },
            "linux": {"executable": "nfqws", "path": "/opt/zapret", "base_args": []},
        },
        "goodbyedpi": {
            "windows": {"executable": "goodbyedpi.exe", "path": "bin", "base_args": []},
    ...
    def _build_command(
        ...
        # Start with executable and base args
        command = [str(tool_path)] + tool_config["base_args"]

        # Add port filters for Windows zapret
        if self.tool_name == "zapret" and system == "windows":
            # Collect all ports from strategies
            ports = set()
        ...
        # Convert strategy to command line args
        # This is simplified - real implementation would use proper converter
        if target_ips and strategy_map:
            # Get first strategy (tools usually apply same strategy to all)
            first_ip = next(iter(target_ips))
            strategy = strategy_map.get(first_ip, {})

            # Convert strategy dict to zapret/goodbyedpi args
    ...
    def _convert_strategy_to_args(self, strategy: Dict[str, Any]) -> List[str]:
        ...
        # This is where we'd use the synth function from zapret module
