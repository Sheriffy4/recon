# recon/core/bypass/engines/fallback_recovery.py
"""
...
from .error_handling import (
    BaseEngineError, EngineCreationError, EngineDependencyError,
    ErrorContext, ErrorSeverity, get_error_handler
)
from .config_models import EngineCreationRequest, EngineCreationResult
...
class FallbackStrategy_Interface(ABC):
    ...
    @abstractmethod
    def get_fallback_order(self, preferred_engine: Optional[EngineType],
                          available_engines: List[EngineType],
                          context: Optional[ErrorContext] = None) -> List[EngineType]:
        """Get the fallback order for engines."""
        pass
    
    @abstractmethod
    def should_attempt_fallback(self, error: BaseEngineError,
...
class DependencyBasedStrategy(FallbackStrategy_Interface):
    ...
    def get_fallback_order(self, preferred_engine: Optional[EngineType],
        ...
        # Get engines with all dependencies met first
        engines_with_deps = []
        ...
        # Prefer engines with dependencies, then others
...
class LeastRequirementsStrategy(FallbackStrategy_Interface):
    ...
    def get_fallback_order(self, preferred_engine: Optional[EngineType],
        ...
        # Sort by number of requirements (fewer = better for fallback)
        requirements_map = {
            EngineType.EXTERNAL_TOOL: 0,  # Fewest requirements
            EngineType.NATIVE_PYDIVERT: 2,  # Windows + PyDivert
            EngineType.NATIVE_NETFILTER: 2,  # Linux + netfilterqueue
        }
...
class FallbackRecoveryManager:
    ...
    def __init__(self, config_manager, detector, validator):
        ...
        self.fallback_timeout = 30.0  # seconds
        self.enable_mock_fallback = True
        self.retry_delays = [0.1, 0.5, 1.0, 2.0, 5.0]  # seconds
    
    def create_engine_with_fallback(self, request: EngineCreationRequest,
                                   engine_creator: Callable[[EngineType, EngineConfig], BaseBypassEngine],
                                   fallback_strategy: FallbackStrategy = FallbackStrategy.PRIORITY_ORDER) -> FallbackResult:
        """
        ...
        fallback_order = strategy.get_fallback_order(
            initial_engine_type, 
            available_engines,
            self._create_error_context(request)
        )
        
        # Ensure initial engine is first in order
        ...
        # Attempt engine creation with fallback
        for attempt_num, engine_type in enumerate(fallback_order, 1):
            ...
            if attempt.success and attempt.engine:
                ...
                if recovery_action == RecoveryAction.RETRY:
                    ...
                    break
                # Otherwise continue to next engine (FALLBACK/DEGRADE)
        
        # If all engines failed and mock fallback is enabled
        if not result.success and self.enable_mock_fallback:
            ...
            if mock_attempt.success:
                ...
                result.final_engine_type = None  # Mock doesn't have a real type
    ...
    def configure_fallback_behavior(self, max_attempts: Optional[int] = None,
        ...
        self.logger.info(f"Fallback behavior configured: max_attempts={self.max_fallback_attempts}, "
                        f"timeout={self.fallback_timeout}, enable_mock={self.enable_mock_fallback}")
    ...
    def _attempt_engine_creation(self, engine_type: EngineType, request: EngineCreationRequest,
                                engine_creator: Callable, attempt_num: int) -> FallbackAttempt:
        ...
        try:
            ...
            from .error_handling import create_error_from_exception
    ...
    def _attempt_mock_engine_creation(self, request: EngineCreationRequest, attempt_num: int) -> FallbackAttempt:
        ...
        attempt = FallbackAttempt(
            engine_type=None,  # Mock doesn't have a real type
            attempt_number=attempt_num,
            start_time=time.time()
        )
    ...
    def _determine_recovery_action(self, error: Optional[BaseEngineError],
                                 attempt_count: int,
                                 strategy: FallbackStrategy_Interface) -> RecoveryAction:
        """Determine the appropriate recovery action."""
        if not error:
            return RecoveryAction.FAIL
        
        # Check if strategy suggests fallback
...
# Global fallback recovery manager instance
