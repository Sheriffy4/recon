...
class MockEngine(BaseBypassEngine):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {'mock_engine': True, 'operation_count': self.operation_count, 'is_running': self.is_running, 'packets_processed': 0, 'packets_modified': 0, 'bytes_processed': 0, 'errors': 0}
...
class FallbackStrategy_Interface(ABC):
    ...
    @abstractmethod
    def get_fallback_order(self, preferred_engine: Optional[EngineType], available_engines: List[EngineType], context: Optional[ErrorContext]=None) -> List[EngineType]:
...
class LeastRequirementsStrategy(FallbackStrategy_Interface):
    ...
    def get_fallback_order(self, preferred_engine: Optional[EngineType], available_engines: List[EngineType], context: Optional[ErrorContext]=None) -> List[EngineType]:
        ...
        requirements_map = {EngineType.EXTERNAL_TOOL: 0, EngineType.NATIVE_PYDIVERT: 2, EngineType.NATIVE_NETFILTER: 2}
...
class FallbackRecoveryManager:
    ...
    def __init__(self, config_manager, detector, validator):
        ...
        self.strategies = {FallbackStrategy.PRIORITY_ORDER: PriorityOrderStrategy(config_manager), FallbackStrategy.DEPENDENCY_BASED: DependencyBasedStrategy(detector), FallbackStrategy.LEAST_REQUIREMENTS: LeastRequirementsStrategy()}
    ...
    def create_engine_with_fallback(self, request: EngineCreationRequest, engine_creator: Callable[[EngineType, EngineConfig], BaseBypassEngine], fallback_strategy: FallbackStrategy=FallbackStrategy.PRIORITY_ORDER) -> FallbackResult:
        """
        ...
        fallback_order = strategy.get_fallback_order(initial_engine_type, available_engines, self._create_error_context(request))
        if initial_engine_type and initial_engine_type not in fallback_order:
            ...
            fallback_order.insert(0, initial_engine_type)
        self.logger.info(f'Starting fallback process with order: {[e.value for e in fallback_order]}')
        for attempt_num, engine_type in enumerate(fallback_order, 1):
            ...
            if time.time() - start_time > self.fallback_timeout:
                ...
                break
            attempt = self._attempt_engine_creation(engine_type, request, engine_creator, attempt_num)
            ...
            if attempt.success and attempt.engine:
                ...
                recovery_action = self._determine_recovery_action(attempt.error, attempt_num, strategy)
    ...
    def configure_fallback_behavior(self, max_attempts: Optional[int]=None, timeout: Optional[float]=None, enable_mock: Optional[bool]=None, retry_delays: Optional[List[float]]=None):
        """
        ...
        self.logger.info(f'Fallback behavior configured: max_attempts={self.max_fallback_attempts}, timeout={self.fallback_timeout}, enable_mock={self.enable_mock_fallback}')

    def get_fallback_statistics(self) -> Dict[str, Any]:
        """Get statistics about fallback usage."""
        return {'max_attempts': self.max_fallback_attempts, 'timeout': self.fallback_timeout, 'mock_enabled': self.enable_mock_fallback, 'available_strategies': list(self.strategies.keys()), 'retry_delays': self.retry_delays}
    ...
    def test_fallback_chain(self, preferred_engine: Optional[EngineType]=None) -> Dict[str, Any]:
        ...
        for strategy_name, strategy in self.strategies.items():
            ...
            results[strategy_name.value] = {'fallback_order': [e.value for e in fallback_order], 'available_engines': [e.value for e in available_engines], 'total_options': len(fallback_order)}
        return results

    def _determine_initial_engine_type(self, request: EngineCreationRequest) -> Optional[EngineType]:
    ...
    def _attempt_engine_creation(self, engine_type: EngineType, request: EngineCreationRequest, engine_creator: Callable, attempt_num: int) -> FallbackAttempt:
        ...
        attempt = FallbackAttempt(engine_type=engine_type, attempt_number=attempt_num, start_time=time.time())
        try:
            self.logger.debug(f'Attempting to create {engine_type.value} engine (attempt {attempt_num})')
            ...
            self.logger.info(f'Successfully created {engine_type.value} engine on attempt {attempt_num}')
    ...
    def _attempt_mock_engine_creation(self, request: EngineCreationRequest, attempt_num: int) -> FallbackAttempt:
        """Attempt to create a mock engine."""
        attempt = FallbackAttempt(engine_type=None, attempt_number=attempt_num, start_time=time.time())
        try:
    ...
    def _determine_recovery_action(self, error: Optional[BaseEngineError], attempt_count: int, strategy: FallbackStrategy_Interface) -> RecoveryAction:
