...
class SerializableModel:
    ...
    def save_to_file(self, file_path: Union[str, Path], format: SerializationFormat=SerializationFormat.JSON):
        """Save to file."""
    ...
    @classmethod
    def load_from_file(cls, file_path: Union[str, Path], format: SerializationFormat=SerializationFormat.JSON) -> 'SerializableModel':
        """Load from file."""
        file_path = Path(file_path)
        if format == SerializationFormat.JSON:
...
class EnhancedEngineConfig(SerializableModel):
    ...
    def to_engine_config(self) -> EngineConfig:
        ...
        return EngineConfig(debug=self.debug, timeout=self.timeout, base_path=self.base_path, tool_name=self.tool_name, packet_buffer_size=self.packet_buffer_size, max_concurrent_connections=self.max_concurrent_connections, log_packets=self.log_packets)

    @classmethod
    def from_engine_config(cls, config: EngineConfig) -> 'EnhancedEngineConfig':
        """Create from standard EngineConfig."""
        return cls(debug=config.debug, timeout=config.timeout, base_path=config.base_path, tool_name=config.tool_name, packet_buffer_size=config.packet_buffer_size, max_concurrent_connections=config.max_concurrent_connections, log_packets=config.log_packets)
...
class EngineCreationResult(SerializableModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if 'engine' in data and data['engine'] is not None:
            data['engine'] = {'class_name': self.engine.__class__.__name__, 'engine_type': self.engine_type.value if self.engine_type else None}
        if isinstance(self.engine_type, EngineType):
            data['engine_type'] = self.engine_type.value
        return data
...
class ConfigurationManager:
    ...
    def serialize_profiles(self, profiles: Dict[EngineType, EngineConfigProfile], format: SerializationFormat=SerializationFormat.JSON) -> str:
    ...
    def deserialize_profiles(self, data: str, format: SerializationFormat=SerializationFormat.JSON) -> Dict[EngineType, EngineConfigProfile]:
    ...
    def validate_configuration_file(self, file_path: Union[str, Path]) -> ValidationResult:
        ...
        try:
            if not file_path.exists():
                result.add_issue(ValidationSeverity.ERROR, f'Configuration file does not exist: {file_path}', error_code='FILE_NOT_FOUND')
                return result
            with open(file_path, 'r') as f:
                content = f.read()
            try:
                ...
                result.add_issue(ValidationSeverity.ERROR, f'Invalid JSON format: {e}', error_code='INVALID_JSON')
                return result
            if not isinstance(config_data, dict):
                result.add_issue(ValidationSeverity.ERROR, 'Configuration must be a JSON object', error_code='INVALID_STRUCTURE')
                return result
            if 'profiles' in config_data:
                if not isinstance(config_data['profiles'], dict):
                    result.add_issue(ValidationSeverity.ERROR, 'Profiles section must be an object', field='profiles', error_code='INVALID_PROFILES')
                else:
                    for engine_name, profile_data in config_data['profiles'].items():
                        try:
                            EngineConfigProfile.from_dict(profile_data)
                        except Exception as e:
                            result.add_issue(ValidationSeverity.ERROR, f'Invalid profile for {engine_name}: {e}', field=f'profiles.{engine_name}', error_code='INVALID_PROFILE')
            if 'global' in config_data:
                if not isinstance(config_data['global'], dict):
                    result.add_issue(ValidationSeverity.ERROR, 'Global section must be an object', field='global', error_code='INVALID_GLOBAL')
        except Exception as e:
            result.add_issue(ValidationSeverity.ERROR, f'Error validating configuration file: {e}', error_code='VALIDATION_ERROR')
        return result

    def convert_format(self, input_file: Union[str, Path], output_file: Union[str, Path], input_format: SerializationFormat=SerializationFormat.JSON, output_format: SerializationFormat=SerializationFormat.JSON):
...
def create_engine_request(engine_type: Optional[Union[str, EngineType]]=None, **kwargs) -> EngineCreationRequest:
