...
class PacketProcessingEngine(BaseBypassEngine):
    ...
    def __init__(self, attack_adapter, fingerprint_engine, diagnostic_system, packet_processor: Optional[RobustPacketProcessor]=None, strategy_mapper: Optional[StrategyMapper]=None, result_processor: Optional[ResultProcessor]=None, performance_optimizer: Optional['PerformanceOptimizer']=None, combo_attacker: Optional['ComboAttacker']=None, config: Optional[EngineConfig]=None):
        ...
        if not all([attack_adapter, fingerprint_engine, diagnostic_system]):
            raise ValueError('AttackAdapter, UltimateAdvancedFingerprintEngine и DiagnosticSystem должны быть предоставлены через DI.')
        ...
        self.performance_optimizer = performance_optimizer or (PerformanceOptimizer(self) if PERFORMANCE_OPTIMIZATION_AVAILABLE else None)
    ...
    def apply_strategy(self, packet_info: PacketInfo, strategy: Dict[str, Any]) -> StrategyResult:
        ...
        try:
            payload_start = (packet_info.raw_data[0] & 15) * 4 + (packet_info.raw_data[(packet_info.raw_data[0] & 15) * 4 + 12] >> 4 & 15) * 4
            context = AttackContext(dst_ip=packet_info.dst_ip, dst_port=packet_info.dst_port, src_ip=packet_info.src_ip, src_port=packet_info.src_port, payload=packet_info.raw_data[payload_start:], protocol=packet_info.protocol.value if hasattr(packet_info.protocol, 'value') else 'tcp', params=strategy.get('params', {}))
            attack_name = strategy.get('type') or strategy.get('name')
            loop = asyncio.get_event_loop()
            if loop.is_running():
                future = asyncio.run_coroutine_threadsafe(self.attack_adapter.execute_attack_by_name(attack_name, context), loop)
                attack_result = future.result(timeout=10)
            else:
                attack_result = asyncio.run(self.attack_adapter.execute_attack_by_name(attack_name, context))
            return StrategyResult(success=attack_result.status == AttackStatus.SUCCESS, technique_used=attack_name, execution_time_ms=(time.time() - start_time) * 1000, packets_modified=attack_result.packets_sent, error_message=attack_result.error_message, metadata={'attack_result': attack_result})
        except Exception as e:
            self.logger.error(f"Ошибка применения стратегии '{strategy.get('type')}': {e}")
            return StrategyResult(success=False, technique_used=strategy.get('type', 'unknown'), execution_time_ms=(time.time() - start_time) * 1000, error_message=str(e))
    ...
    def _initialize_optional_components(self) -> None:
        ...
        if COMBO_ATTACKER_AVAILABLE:
            try:
                self.combo_attacker = ComboAttacker(attack_adapter=self.attack_adapter, debug=self.config.debug)
    ...
    def start(self, target_ips: Set[str], strategy_map: Dict[str, Dict]) -> Optional[threading.Thread]:
        ...
        self._thread = threading.Thread(target=self._run, args=(target_ips, strategy_map), daemon=True, name='PacketProcessingEngine-Thread')
        self._thread.start()
        return self._thread
    ...
    def update_strategy_map(self, new_strategy_map: Dict[str, Dict]):
        ...
        with self._lock:
            ...
            LOG.info(f'Карта стратегий обновлена. Новых/измененных записей: {len(new_strategy_map)}')
    ...
    def _process_packet(self, packet: pydivert.Packet, w: pydivert.WinDivert) -> None:
        ...
        loop = asyncio.get_event_loop()
        future = asyncio.run_coroutine_threadsafe(self.attack_adapter.execute_attack_by_name(strategy['type'], context), loop)
        attack_result = future.result(timeout=10)
    ...
    def _handle_attack_result(self, result: AttackResult, original_packet: pydivert.Packet, w: pydivert.WinDivert) -> PacketProcessingResult:
        ...
        if result.status != AttackStatus.SUCCESS:
            ...
            if meta_params_missing or meta_params_unexpected:
                LOG.warning(f'Strategy param issues: missing={meta_params_missing or []}, unexpected={meta_params_unexpected or []}')
            self._safe_send_packet(w, original_packet)
            return PacketProcessingResult(success=False, packets_sent=1, error_message=result.error_message, attack_used=result.technique_used)
        metadata = result.metadata or {}
        packets_sent = 0
        if 'segments' in metadata and metadata['segments']:
            packets_sent = self._send_attack_segments(w, original_packet, metadata['segments'])
            ...
            self._safe_send_packet(w, original_packet)
            packets_sent = 1
        return PacketProcessingResult(success=True, packets_sent=packets_sent, attack_used=result.technique_used, metadata=metadata)

    def _create_attack_context_from_packet(self, packet: pydivert.Packet, strategy: Dict) -> AttackContext:
        """Создает AttackContext из пакета PyDivert."""
        return AttackContext(dst_ip=packet.dst_addr, dst_port=packet.tcp.dst_port, src_ip=packet.src_addr, src_port=packet.tcp.sport, payload=bytes(packet.payload), protocol='tcp', seq=packet.tcp.seq, ack=packet.tcp.ack, params=strategy.get('params', {}).copy(), debug=self.config.debug)

    def _execute_strategy_through_adapter(self, strategy: Dict[str, Any], context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if strategy_type in available_attacks:
                ...
                legacy_result = self.attack_adapter.execute_legacy_technique(strategy_type, self._context_to_legacy_params(context))
    ...
    def _execute_combo_strategy(self, strategy: Dict[str, Any], context: AttackContext) -> AttackResult:
        """Выполняет combo стратегию."""
        if not self.combo_attacker:
            return AttackResult(status=AttackStatus.ERROR, error_message='ComboAttacker не доступен')
    ...
    def _send_attack_segments(self, w: pydivert.WinDivert, original_packet: pydivert.Packet, segments: List[Tuple]) -> int:
        ...
        for i, segment_info in enumerate(segments):
            ...
            packet_params = {'new_payload': data, 'new_seq': base_seq + seq_offset & 4294967295, 'new_flags': 'A' if i < len(segments) - 1 else 'PA'}
            packet_params.update(options)
            new_packet_raw = EnhancedPacketBuilder.assemble_tcp_packet(bytes(original_packet.raw), **packet_params)
            if self._send_raw_packet(w, new_packet_raw, original_packet):
                packets_sent += 1
                expected_seq = packet_params.get('new_seq')
                new_seq = expected_seq
                report = self.mod_validator.validate_segment(original_payload, data, expected_seq=expected_seq, new_seq=new_seq, is_last=i == len(segments) - 1)
                if report.success:
                    try:
    ...
    def _send_modified_payload(self, w: pydivert.WinDivert, original_packet: pydivert.Packet, modified_payload: bytes) -> bool:
        """Отправляет пакет с модифицированным payload."""
        original_payload = bytes(original_packet.payload)
        new_packet_raw = EnhancedPacketBuilder.assemble_tcp_packet(bytes(original_packet.raw), new_payload=modified_payload)
        sent = self._send_raw_packet(w, new_packet_raw, original_packet)
        if sent:
            report = self.mod_validator.validate_segment(original_payload, modified_payload, expected_seq=None, new_seq=None, is_last=True)
            if report.success:
                try:
                    self.metrics.increment_counter('packets_modified')
                except Exception:
    ...
    def _send_raw_packet(self, w: pydivert.WinDivert, packet_data: bytes, template_packet: pydivert.Packet) -> bool:
        ...
        try:
            packet = pydivert.Packet(packet_data, template_packet.interface, template_packet.direction)
        except Exception:
            try:
    ...
    def _get_or_create_fingerprint(self, domain: str, target_ips: List[str]) -> Any:
        ...
        fingerprint = self.fingerprint_engine.create_comprehensive_fingerprint(domain, target_ips)
        if fingerprint:
    ...
    def _context_to_legacy_params(self, context: AttackContext) -> Dict[str, Any]:
        ...
        legacy_params = {'target_ip': context.dst_ip, 'target_port': context.dst_port, 'src_ip': context.src_ip, 'src_port': context.src_port, 'payload': context.payload, 'protocol': context.protocol}
        if context.domain:
    ...
    def _legacy_result_to_attack_result(self, legacy_result: Dict[str, Any]) -> AttackResult:
        ...
        return AttackResult(status=status, error_message=legacy_result.get('error_message', ''), latency_ms=legacy_result.get('execution_time_ms', 0), metadata=legacy_result)
    ...
    def _apply_bad_checksum(self, packet_raw: bytes, checksum_value: int) -> bytes:
        ...
        if len(packet_bytes) > tcp_checksum_pos + 1:
            packet_bytes[tcp_checksum_pos:tcp_checksum_pos + 2] = struct.pack('!H', checksum_value)
        return bytes(packet_bytes)

    def _log_processing_result(self, packet: pydivert.Packet, strategy: Dict, result: PacketProcessingResult) -> None:
        ...
        try:
            ...
            pass
        self.diagnostic_system.log_packet_processing(packet=packet, action='bypassed' if result.success else 'failed', technique_used=result.attack_used or strategy.get('type', 'unknown'), strategy_type=strategy.get('type', 'unknown'), processing_time_ms=result.execution_time_ms, success=result.success, error_message=result.error_message, byte_level_info=result.metadata)
    ...
    def get_combined_stats(self) -> Dict[str, Any]:
        ...
        stats = {'engine_metrics': self.metrics.get_all_metrics(), 'packet_processor_stats': self.packet_processor.get_stats(), 'diagnostic_stats': self.diagnostic_system.get_stats(), 'fingerprint_stats': self.fingerprint_engine.get_stats(), 'packet_builder_stats': EnhancedPacketBuilder.get_performance_stats(), 'attack_adapter_stats': self.attack_adapter.get_execution_stats()}
        if self.performance_optimizer:
    ...
    def create_domain_fingerprint(self, domain: str, target_ips: List[str]=None):
        ...
        return self.fingerprint_engine.create_comprehensive_fingerprint(domain, target_ips)
...
def create_packet_processing_engine(config: Optional[EngineConfig]=None) -> PacketProcessingEngine:
    ...
    def get_di_status(self) -> Dict[str, Any]:
        ...
        return {'performance_optimizer_injected': self._performance_optimizer is not None, 'effectiveness_validator_injected': self._effectiveness_validator is not None, 'di_enabled': self.has_di_dependencies()}