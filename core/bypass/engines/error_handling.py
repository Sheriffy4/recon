...
class BaseEngineError(Exception):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "context": {
                "engine_type": self.context.engine_type.value if self.context.engine_type else None,
...
class EnginePlatformError(BaseEngineError):
    ...
    def __init__(self, message: str, required_platform: Optional[str] = None, 
                 current_platform: Optional[str] = None, **kwargs):
...
class ErrorHandler:
    ...
    def create_error_from_exception(self, exception: Exception, 
                                  ...
                                  context: Optional[ErrorContext] = None) -> BaseEngineError:
        """
    ...
    def get_resolution_suggestions(self, error_code: str, 
                                 context: Optional[ErrorContext] = None) -> List[ResolutionSuggestion]:
    ...
    def _enhance_suggestions(self, error: BaseEngineError):
        ...
        if error.category == ErrorCategory.DEPENDENCY:
            if not any(s.action.startswith("Check Python") for s in error.suggestions):
                error.suggestions.append(ResolutionSuggestion(
...
def create_dependency_error(message: str, missing_dependencies: Optional[List[str]] = None,
                          context: Optional[ErrorContext] = None) -> EngineDependencyError:
    """Create a dependency error."""
    return EngineDependencyError(message, missing_dependencies, context=context)


def create_permission_error(message: str, required_permissions: Optional[List[str]] = None,
                          context: Optional[ErrorContext] = None) -> EnginePermissionError:
    """Create a permission error."""
    return EnginePermissionError(message, required_permissions, context=context)


def create_platform_error(message: str, required_platform: Optional[str] = None,
                         context: Optional[ErrorContext] = None) -> EnginePlatformError:
...
def create_validation_error(message: str, validation_errors: Optional[List[str]] = None,
                          context: Optional[ErrorContext] = None) -> EngineValidationError:
