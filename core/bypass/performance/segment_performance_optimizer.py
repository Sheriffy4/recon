    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "reused": self._reused,
                "reuse_rate": (
                    self._reused / (self._allocated + self._reused)
                    if (self._allocated + self._reused) > 0
                    else 0.0
                ),
            }
...
class SegmentPerformanceOptimizer:
    ...
    def optimize_packet_construction(
        self, segments: List[Tuple[bytes, int, Dict[str, Any]]], packet_builder: Any
    ) -> List[bytes]:
        """
                ...
                if self.config.enable_memory_pooling and self.memory_pool:
                    ...
                    try:
                        packet = packet_builder.build_segment_packet(
                            payload, seq_offset, options, buffer
                        )
                    finally:
    ...
    def optimize_segment_execution(
        self,
        ...
        try:
            ...
            if execution_time * 1000 > self.config.performance_threshold_ms:
                self.logger.warning(
                    f"Segment execution exceeded threshold: {execution_time*1000:.2f}ms"
                )
    ...
    def _execute_batched(
        ...
        self._optimization_stats["batches_processed"] += (
            len(segments) + batch_size - 1
        ) // batch_size
        return metrics
    ...
    def suggest_optimizations(self, performance_stats: Dict[str, Any]) -> List[str]:
        ...
        if "performance_history" in performance_stats:
            ...
            if history["avg_time"] > self.config.performance_threshold_ms / 1000:
                suggestions.append(
                    "Average execution time exceeds threshold - consider enabling more optimizations"
                )
            if history["max_time"] > history["avg_time"] * 3:
                suggestions.append(
                    "High variance in execution times - investigate performance bottlenecks"
                )
...
def optimize_segments(
    segments: List[Tuple[bytes, int, Dict[str, Any]]], execution_func: Callable
) -> PerformanceMetrics:
    """
