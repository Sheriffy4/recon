...
from .performance_models import (
    SystemHealth, PerformanceMetrics, Alert, AlertSeverity,
    ProductionConfig
)
...
class ProductionMonitor:
    ...
    async def _monitor_performance_metrics(self) -> None:
        ...
        while self.monitoring_active:
            try:
                ...
                # Wait for next monitoring interval
                await asyncio.sleep(self.config.monitoring_interval * 2)  # Less frequent than health
    ...
    async def _create_alert(self, severity: AlertSeverity, title: str, message: str,
                          component: str, metrics: Dict[str, Any]) -> None:
        """Create a new alert."""
        try:
        ...
        while self.monitoring_active:
            try:
                ...
                while (self.alerts and 
                       self.alerts[0].resolved and
                       self.alerts[0].timestamp < cutoff_time):
                    self.alerts.popleft()
        ...
        try:
            for alert in self.alerts:
                if alert.id == alert_id:
                    ...
                    keys_to_remove = [
                        key for key, active_alert in self.active_alerts.items()
