...
from .performance_models import StrategyPerformance, OptimizationLevel
...
class StrategyOptimizer:
    ...
    def __init__(self, optimization_level: OptimizationLevel = OptimizationLevel.BALANCED):
        ...
        self.strategy_stats = defaultdict(lambda: {
            ...
            'total_time': 0.0,
            'last_used': None,
            'effectiveness_history': deque(maxlen=100)
        })
    ...
    async def _calculate_strategy_score(self, strategy_id: str, domain: str) -> float:
        ...
        try:
            ...
            # Execution time component (lower is better)
            avg_time = stats['total_time'] / max(total_attempts, 1)
            time_score = 1.0 / (1.0 + avg_time)  # Inverse relationship
            
            # Stability component (consistency of results)
            ...
            score = (
                success_rate * self.optimization_weights['success_rate'] +
                time_score * self.optimization_weights['execution_time'] +
    ...
    def _calculate_stability_score(self, effectiveness_history: deque) -> float:
        ...
        try:
            ...
            cv = std_val / mean_val
            stability_score = 1.0 / (1.0 + cv)  # Inverse relationship
    ...
    def _get_domain_specific_bonus(self, strategy_id: str, domain: str) -> float:
        ...
        # Check if this strategy has been successful for this domain before
    ...
    async def update_strategy_performance(self, strategy_id: str, domain: str, 
        ...
        try:
            ...
            # Update domain-specific mapping
            domain_key = f"{domain}:{strategy_id}"
            if domain_key not in self.domain_strategy_mapping:
                self.domain_strategy_mapping[domain_key] = {
                    'success_count': 0,
                    'failure_count': 0,
                    'success_rate': 0.0
                }
            ...
            for strategy_id in all_strategies:
                ...
                if total_attempts > 0:
                    ...
                    recommendation = {
                        ...
                        'effectiveness_score': await self._calculate_strategy_score(strategy_id, domain)
                    }
    ...
    async def optimize_algorithm_parameters(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Analyze success rates across different optimization levels
            level_performance = {}
            
            for level in OptimizationLevel:
                # Simulate performance with different weights
                temp_weights = {
                    OptimizationLevel.CONSERVATIVE: {
                        'success_rate': 0.5, 'execution_time': 0.2, 'stability': 0.3
                    },
                    OptimizationLevel.BALANCED: {
                        'success_rate': 0.4, 'execution_time': 0.3, 'stability': 0.3
                    },
                    OptimizationLevel.AGGRESSIVE: {
                        'success_rate': 0.6, 'execution_time': 0.3, 'stability': 0.1
                    },
                    OptimizationLevel.MAXIMUM: {
                        'success_rate': 0.7, 'execution_time': 0.3, 'stability': 0.0
                    }
                }[level]
                ...
                theoretical_score = (
                    avg_success_rate * temp_weights['success_rate'] +
                    (1.0 / (1.0 + avg_execution_time)) * temp_weights['execution_time'] +
                    0.7 * temp_weights['stability']  # Assume moderate stability
                )
            ...
            optimization_results = {
                'recommended_optimization_level': optimal_level[0],
                'level_scores': level_performance,
                'current_level': self.optimization_level.value,
                'improvement_potential': optimal_level[1] - level_performance[self.optimization_level.value],
