...
from recon.core.bypass.performance.performance_models import OptimizationLevel, ProductionConfig, AlertSeverity, PerformanceMetrics, SystemHealth, Alert
...
class TestPerformanceOptimizer:
    ...
    async def test_collect_performance_metrics(self, optimizer):
        ...
        with patch('psutil.cpu_percent', return_value=50.0), patch('psutil.virtual_memory') as mock_memory, patch('psutil.net_io_counters') as mock_network:
    ...
    async def test_optimize_performance(self, optimizer):
        ...
        metrics = PerformanceMetrics(attack_execution_time=2.0, strategy_selection_time=0.2, validation_time=1.0, memory_usage=80.0, cpu_usage=85.0, success_rate=60.0, throughput=5.0, latency=6.0)
    ...
    async def test_get_system_health(self, optimizer):
        ...
        with patch('psutil.cpu_percent', return_value=45.0), patch('psutil.virtual_memory') as mock_memory, patch('psutil.disk_usage') as mock_disk, patch('time.time', return_value=1000000), patch('psutil.boot_time', return_value=999000):
...
class TestStrategyOptimizer:
    ...
    async def test_optimize_strategy_selection(self, optimizer):
        ...
        await optimizer.update_strategy_performance('tcp_fragmentation', domain, True, 1.0, 0.8)
        await optimizer.update_strategy_performance('http_manipulation', domain, True, 0.5, 0.9)
    ...
    async def test_update_strategy_performance(self, optimizer):
        ...
        domain = 'test.com'
        await optimizer.update_strategy_performance(strategy_id, domain, True, 1.5, 0.85)
    ...
    async def test_get_strategy_recommendations(self, optimizer):
        ...
        for i, strategy in enumerate(strategies):
            await optimizer.update_strategy_performance(strategy, domain, True, 1.0 + i * 0.5, 0.8 + i * 0.05)
        recommendations = await optimizer.get_strategy_recommendations(domain)
        assert isinstance(recommendations, list)
        assert len(recommendations) == len(strategies)
        if len(recommendations) > 1:
            assert recommendations[0]['effectiveness_score'] >= recommendations[1]['effectiveness_score']

    @pytest.mark.asyncio
    async def test_optimize_algorithm_parameters(self, optimizer):
        """Test algorithm parameter optimization."""
        await optimizer.update_strategy_performance('test_strategy', 'test.com', True, 1.0, 0.8)
...
class TestProductionMonitor:
    ...
    @pytest.fixture
    def config(self):
        return ProductionConfig(optimization_level=OptimizationLevel.BALANCED, max_concurrent_attacks=10, resource_limits={'max_cpu_usage': 70.0}, monitoring_interval=1, alert_thresholds={'cpu_critical': 90.0, 'memory_critical': 85.0}, auto_scaling_enabled=False, backup_enabled=False, logging_level='INFO', performance_targets={})
    ...
    async def test_create_alert(self, monitor):
        """Test alert creation."""
        await monitor._create_alert(AlertSeverity.WARNING, 'Test Alert', 'This is a test alert', 'test_component', {'test_metric': 100})
    ...
    async def test_get_active_alerts(self, monitor):
        ...
        await monitor._create_alert(AlertSeverity.ERROR, 'Active Alert', 'This is an active alert', 'test', {})
    ...
    async def test_acknowledge_alert(self, monitor):
        ...
        await monitor._create_alert(AlertSeverity.INFO, 'Test Alert', 'Test message', 'test', {})
    ...
    async def test_resolve_alert(self, monitor):
        ...
        await monitor._create_alert(AlertSeverity.WARNING, 'Resolvable Alert', 'Test message', 'test', {})
        alert_id = monitor.alerts[0].id
...
class TestAlertingSystem:
    ...
    def alerting_config(self, temp_log_file):
        return {'email': {'enabled': False}, 'webhook': {'enabled': False}, 'file': {'enabled': True, 'log_file': temp_log_file}}
    ...
    async def test_send_alert(self, alerting_system, temp_log_file):
        """Test alert sending."""
        alert = Alert(id='test_alert', severity=AlertSeverity.WARNING, title='Test Alert', message='This is a test alert', component='test', metrics={'test': True})
    ...
    def test_add_suppression_rule(self, alerting_system):
        ...
        rule = {'component': 'test', 'severity': 'warning'}
    ...
    def test_add_escalation_rule(self, alerting_system):
        ...
        rule = {'severity': 'warning', 'title_prefix': '[ESCALATED]'}
    ...
    async def test_log_to_file(self, alerting_system, temp_log_file):
        ...
        alert = Alert(id='file_test', severity=AlertSeverity.ERROR, title='File Test Alert', message='Testing file logging', component='file_test', metrics={'file_test': True})
...
class TestIntegration:
    ...
    async def test_full_optimization_workflow(self):
        ...
        selected = await strategy_optimizer.optimize_strategy_selection('example.com', strategies)
        assert selected in strategies

    @pytest.mark.asyncio
    async def test_monitoring_and_alerting_integration(self):
        """Test monitoring and alerting integration."""
        config = ProductionConfig(optimization_level=OptimizationLevel.BALANCED, max_concurrent_attacks=5, resource_limits={}, monitoring_interval=1, alert_thresholds={'cpu_critical': 90.0}, auto_scaling_enabled=False, backup_enabled=False, logging_level='INFO', performance_targets={})
        fd, temp_log = tempfile.mkstemp(suffix='.log')
        os.close(fd)
        try:
            monitor = ProductionMonitor(config)
            alerting_config = {'file': {'enabled': True, 'log_file': temp_log}}
            alerting_system = AlertingSystem(alerting_config)

            async def alert_callback(alert):
                await alerting_system.send_alert(alert)
            monitor.add_alert_callback(alert_callback)
            await monitor._create_alert(AlertSeverity.WARNING, 'Integration Test', 'Testing integration', 'integration_test', {})
            assert len(monitor.alerts) == 1
