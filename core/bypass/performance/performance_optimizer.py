...
class PerformanceOptimizer:
    ...
    def __init__(self, optimization_level: OptimizationLevel=OptimizationLevel.BALANCED):
        ...
        self.thresholds = {OptimizationLevel.CONSERVATIVE: {'max_cpu_usage': 50.0, 'max_memory_usage': 60.0, 'min_success_rate': 85.0, 'max_latency': 5.0}, OptimizationLevel.BALANCED: {'max_cpu_usage': 70.0, 'max_memory_usage': 75.0, 'min_success_rate': 80.0, 'max_latency': 3.0}, OptimizationLevel.AGGRESSIVE: {'max_cpu_usage': 85.0, 'max_memory_usage': 85.0, 'min_success_rate': 75.0, 'max_latency': 2.0}, OptimizationLevel.MAXIMUM: {'max_cpu_usage': 95.0, 'max_memory_usage': 90.0, 'min_success_rate': 70.0, 'max_latency': 1.0}}
    ...
    async def collect_performance_metrics(self) -> PerformanceMetrics:
        ...
        try:
            ...
            metrics = PerformanceMetrics(attack_execution_time=self._get_average_attack_time(), strategy_selection_time=self._get_average_selection_time(), validation_time=self._get_average_validation_time(), memory_usage=memory_usage, cpu_usage=cpu_usage, success_rate=success_rate, throughput=throughput, latency=latency)
    ...
    async def optimize_performance(self, current_metrics: PerformanceMetrics) -> OptimizationResult:
        ...
        try:
            ...
            if current_metrics.cpu_usage > self.thresholds[self.optimization_level]['max_cpu_usage']:
                optimization_actions.extend(self._optimize_cpu_usage())
            if current_metrics.memory_usage > self.thresholds[self.optimization_level]['max_memory_usage']:
            ...
            if current_metrics.success_rate < self.thresholds[self.optimization_level]['min_success_rate']:
            ...
            result = OptimizationResult(original_metrics=original_metrics, optimized_metrics=optimized_metrics, improvement_percentage=improvement, optimization_actions=optimization_actions, recommendations=self._generate_recommendations(optimized_metrics))
    ...
    def _calculate_improvement(self, original: PerformanceMetrics, optimized: PerformanceMetrics) -> float:
        ...
        try:
            cpu_improvement = max(0, original.cpu_usage - optimized.cpu_usage) / original.cpu_usage * 100
            memory_improvement = max(0, original.memory_usage - optimized.memory_usage) / original.memory_usage * 100
            latency_improvement = max(0, original.latency - optimized.latency) / original.latency * 100
            success_improvement = max(0, optimized.success_rate - original.success_rate) / original.success_rate * 100
            total_improvement = cpu_improvement * 0.3 + memory_improvement * 0.2 + latency_improvement * 0.3 + success_improvement * 0.2
    ...
    def _calculate_throughput(self) -> float:
        ...
        return sum((1.0 / max(m.attack_execution_time, 0.1) for m in recent_metrics)) / len(recent_metrics)
    ...
    def _get_default_metrics(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(attack_execution_time=1.0, strategy_selection_time=0.1, validation_time=0.5, memory_usage=50.0, cpu_usage=30.0, success_rate=80.0, throughput=10.0, latency=1.0)
    ...
    async def get_system_health(self) -> SystemHealth:
        ...
        try:
            ...
            return SystemHealth(cpu_usage=cpu_usage, memory_usage=memory.percent, disk_usage=disk.percent, network_latency=self._calculate_average_latency(), active_attacks=len(self.strategy_performance), failed_attacks=0, system_load=load_avg, uptime=uptime)
        except Exception as e:
            self.logger.error(f'Error getting system health: {e}')
            return SystemHealth(cpu_usage=0.0, memory_usage=0.0, disk_usage=0.0, network_latency=0.0, active_attacks=0, failed_attacks=0, system_load=0.0, uptime=0.0)