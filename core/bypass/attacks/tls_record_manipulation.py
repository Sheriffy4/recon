# core/bypass/attacks/tls_record_manipulation.py
"""
...
class ClientHelloSplitAttack(AdvancedAttack):
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Check for TLS Handshake (0x16) record containing a ClientHello (0x01)
            if not (
                len(payload) > 9 and
                payload.startswith(b"\x16") and # Handshake record
                payload[5] == 0x01 # ClientHello message
            ):
                return AttackResult(
            ...
            # Construct two complete TLS records
            ...
            # The entire modified payload is sent as a single TCP segment
...
class TLSRecordPaddingAttack(AdvancedAttack):
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Update TLS record length if it's a complete record
            if len(payload) >= 5:
