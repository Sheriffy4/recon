...
from core.bypass.attacks.base import AttackResult, AttackContext, AttackStatus, SegmentTuple
...
class MultisplitSegmentProcessor:
    ...
    def create_multisplit_attack_result(
        ...
        if "multisplit" not in dpi_desync:
            return AttackResult(status=AttackStatus.INVALID_PARAMS, error_message="Not a multisplit strategy")
        ...
        try:
            ...
            return AttackResult(status=AttackStatus.INVALID_PARAMS, error_message=f"Invalid multisplit params: {e}")

        self.logger.info(f"Processing multisplit strategy: split_count={split_count}, seqovl={split_seqovl}, fooling={fooling}")
        ...
        if not segments:
            return AttackResult(status=AttackStatus.FAILED, error_message="Failed to create any segments.")
        ...
        self.logger.info(f"Created multisplit attack result with {len(segments)} segments.")
        for i, (p, offset, opt) in enumerate(segments):
            self.logger.debug(f"  Segment {i}: len={len(p)}, offset={offset}, options={opt}")
...
def create_multisplit_attack_result(
    ...
    """
    return _multisplit_processor.create_multisplit_attack_result(context, strategy_params)