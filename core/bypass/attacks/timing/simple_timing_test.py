...
def test_timing_attacks():
    ...
    print('=' * 50)
    context = AttackContext(dst_ip='93.184.216.34', dst_port=443, domain='example.com', payload=b'GET / HTTP/1.1\r\nHost: example.com\r\n\r\n')
    print('\n1. Testing Jitter Injection Attack')
    try:
        config = JitterConfiguration(jitter_type=JitterType.UNIFORM, jitter_amplitude_ms=5.0, packets_per_burst=3)
    ...
    try:
        config = DelayEvasionConfiguration(delay_pattern=DelayPattern.PROGRESSIVE, max_progression_steps=4, progression_factor=1.5)
    ...
    try:
        config = BurstConfiguration(burst_type=BurstType.FIXED_SIZE, default_burst_size=4, total_bursts=2, burst_interval_ms=10.0)
