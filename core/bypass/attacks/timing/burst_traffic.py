...
class BurstMetrics:
    ...
    def update_burst_metrics(self, burst_size: int, interval_ms: float, 
...
class BurstTrafficAttack(TimingAttackBase):
    ...
    def _calculate_burst_size(self, burst_index: int) -> int:
        ...
        if burst_type == BurstType.FIXED_SIZE:
            ...
            scaled_size = self.burst_config.min_burst_size + (
                fib_size % (self.burst_config.max_burst_size - self.burst_config.min_burst_size + 1)
            )
            return scaled_size
    ...
    def _calculate_burst_interval(self, burst_index: int) -> float:
        ...
        if timing == BurstTiming.FIXED_INTERVAL:
            ...
            if self.last_response_time > 0:
                ...
                return max(self.burst_config.min_interval_ms, 
    ...
    def _execute_sequential_bursts(self, context: AttackContext, 
    ...
    def _execute_concurrent_bursts(self, context: AttackContext, 
                                 burst_sequence: List[Tuple[int, float]], 
                                 timing_result: TimingResult) -> List[AttackResult]:
        """
        ...
        for stream_id in range(self.burst_config.concurrent_streams):
            ...
            future = self.executor.submit(
                ...
                context, burst_sequence, stream_id, stream_offset
            )
    ...
    def _execute_stream_bursts(self, context: AttackContext, 
                             burst_sequence: List[Tuple[int, float]], 
                             stream_id: int, offset_ms: float) -> List[AttackResult]:
        """
    ...
    def _execute_single_burst(self, context: AttackContext, burst_size: int, 
                            burst_index: int, stream_id: int = 0) -> AttackResult:
        """
            ...
            for packet_idx in range(burst_size):
                ...
                if packet_idx < burst_size - 1:
                    ...
                    if self.burst_config.intra_burst_jitter_ms > 0:
                        jitter = random.uniform(-self.burst_config.intra_burst_jitter_ms, 
    ...
    def _send_burst_packet(self, context: AttackContext, burst_index: int, 
                          packet_index: int, stream_id: int) -> AttackResult:
        """
    ...
    def get_burst_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "metrics": {
                ...
                "success_rate": (self.metrics.successful_bursts / max(1, self.metrics.bursts_sent)) * 100,
    ...
    def configure_burst_pattern(self, burst_type: BurstType = None, 
                              burst_timing: BurstTiming = None, **kwargs):
        """
