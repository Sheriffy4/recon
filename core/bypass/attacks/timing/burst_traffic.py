...
class BurstMetrics:
    ...
    def update_burst_metrics(self, burst_size: int, interval_ms: float, success: bool, response_time_ms: float):
        ...
        if self.burst_timings:
            self.avg_burst_interval_ms = sum(self.burst_timings) / len(self.burst_timings)
...
class BurstTrafficAttack(TimingAttackBase):
    ...
    def _execute_timing_attack(self, context: AttackContext, timing_result: TimingResult) -> AttackResult:
        ...
        try:
            ...
            if self.burst_config.concurrent_streams > 1:
                results = self._execute_concurrent_bursts(context, burst_sequence, timing_result)
            else:
                results = self._execute_sequential_bursts(context, burst_sequence, timing_result)
            ...
            timing_result.response_received = any((getattr(result, 'response_received', False) for result in results))
            result = AttackResult(status=AttackStatus.SUCCESS if success else AttackStatus.FAILURE, technique_used=f'burst_traffic_{self.burst_config.burst_type.value}', packets_sent=self.metrics.total_packets, bytes_sent=self.metrics.total_bytes, response_received=timing_result.response_received, latency_ms=self.metrics.avg_response_time_ms)
            return result
        except Exception as e:
            self.logger.error(f'Burst traffic attack failed: {e}')
            timing_result.success = False
            timing_result.error_message = str(e)
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), technique_used='burst_traffic_error')
    ...
    def _calculate_burst_size(self, burst_index: int) -> int:
        ...
        if burst_type == BurstType.FIXED_SIZE:
            ...
            scaled_size = self.burst_config.min_burst_size + fib_size % (self.burst_config.max_burst_size - self.burst_config.min_burst_size + 1)
            return scaled_size
        elif burst_type == BurstType.RANDOM:
            return random.randint(self.burst_config.min_burst_size, self.burst_config.max_burst_size)
        elif burst_type == BurstType.ADAPTIVE:
            if self.last_response_time > self.burst_config.adaptation_threshold_ms:
                self.current_burst_size = max(self.burst_config.min_burst_size, int(self.current_burst_size / self.burst_config.adaptation_factor))
            else:
                self.current_burst_size = min(self.burst_config.max_burst_size, int(self.current_burst_size * self.burst_config.adaptation_factor))
    ...
    def _calculate_burst_interval(self, burst_index: int) -> float:
        ...
        if timing == BurstTiming.FIXED_INTERVAL:
            ...
            return random.uniform(self.burst_config.min_interval_ms, self.burst_config.max_interval_ms)
            ...
            if self.last_response_time > 0:
                ...
                return max(self.burst_config.min_interval_ms, min(interval, self.burst_config.max_interval_ms))
            else:
                return self.burst_config.burst_interval_ms
        else:
            return self.burst_config.burst_interval_ms

    def _execute_sequential_bursts(self, context: AttackContext, burst_sequence: List[Tuple[int, float]], timing_result: TimingResult) -> List[AttackResult]:
        """
        ...
        for i, (burst_size, interval_ms) in enumerate(burst_sequence):
            ...
            timing_result.bytes_sent += burst_size * self.burst_config.burst_payload_size
            if i < len(burst_sequence) - 1 and interval_ms > 0:
                self.execute_delay(interval_ms, timing_result)
        return results

    def _execute_concurrent_bursts(self, context: AttackContext, burst_sequence: List[Tuple[int, float]], timing_result: TimingResult) -> List[AttackResult]:
        """
        ...
        for stream_id in range(self.burst_config.concurrent_streams):
            ...
            future = self.executor.submit(self._execute_stream_bursts, context, burst_sequence, stream_id, stream_offset)
    ...
    def _execute_stream_bursts(self, context: AttackContext, burst_sequence: List[Tuple[int, float]], stream_id: int, offset_ms: float) -> List[AttackResult]:
        """
    ...
    def _execute_single_burst(self, context: AttackContext, burst_size: int, burst_index: int, stream_id: int=0) -> AttackResult:
        """
        ...
        try:
            ...
            for packet_idx in range(burst_size):
                ...
                packet_result = self._send_burst_packet(packet_context, burst_index, packet_idx, stream_id)
                packets_sent += 1
                ...
                if packet_idx < burst_size - 1:
                    ...
                    if self.burst_config.intra_burst_jitter_ms > 0:
                        jitter = random.uniform(-self.burst_config.intra_burst_jitter_ms, self.burst_config.intra_burst_jitter_ms)
            ...
            return AttackResult(status=AttackStatus.SUCCESS, technique_used=f'burst_traffic_burst_{burst_index}_stream_{stream_id}', packets_sent=packets_sent, bytes_sent=bytes_sent, latency_ms=burst_duration_ms, response_received=True)
        except Exception as e:
            self.logger.error(f'Failed to execute burst {burst_index}: {e}')
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), technique_used=f'burst_traffic_error_{burst_index}')
    ...
    def _send_burst_packet(self, context: AttackContext, burst_index: int, packet_index: int, stream_id: int) -> AttackResult:
        """
        ...
        return AttackResult(status=AttackStatus.SUCCESS, packets_sent=1, bytes_sent=len(context.payload), latency_ms=network_delay, technique_used=f'burst_packet_b{burst_index}_p{packet_index}_s{stream_id}')
    ...
    def get_burst_statistics(self) -> Dict[str, Any]:
        ...
        """
        stats = {'burst_type': self.burst_config.burst_type.value, 'burst_timing': self.burst_config.burst_timing.value, 'concurrent_streams': self.burst_config.concurrent_streams, 'total_bursts_configured': self.burst_config.total_bursts, 'metrics': {'bursts_sent': self.metrics.bursts_sent, 'total_packets': self.metrics.total_packets, 'total_bytes': self.metrics.total_bytes, 'successful_bursts': self.metrics.successful_bursts, 'failed_bursts': self.metrics.failed_bursts, 'success_rate': self.metrics.successful_bursts / max(1, self.metrics.bursts_sent) * 100, 'avg_burst_size': self.metrics.avg_burst_size, 'avg_burst_interval_ms': self.metrics.avg_burst_interval_ms, 'avg_response_time_ms': self.metrics.avg_response_time_ms}}
        if self.metrics.burst_timings:
            total_time_s = sum(self.metrics.burst_timings) / 1000.0
            if total_time_s > 0:
                stats['metrics']['peak_throughput_pps'] = self.metrics.total_packets / total_time_s
                stats['metrics']['peak_throughput_bps'] = self.metrics.total_bytes / total_time_s
        stats.update(self.get_timing_statistics())
        return stats

    def configure_burst_pattern(self, burst_type: BurstType=None, burst_timing: BurstTiming=None, **kwargs):
        """
    ...
    def benchmark_burst_patterns(self, test_bursts: int=5) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for burst_type in BurstType:
            ...
            results[burst_type.value] = {'sequence_length': len(burst_sequence), 'total_packets': sum(burst_sizes), 'avg_burst_size': sum(burst_sizes) / len(burst_sizes) if burst_sizes else 0, 'min_burst_size': min(burst_sizes) if burst_sizes else 0, 'max_burst_size': max(burst_sizes) if burst_sizes else 0, 'avg_interval_ms': sum(intervals) / len(intervals) if intervals else 0, 'generation_time_ms': generation_time_ms, 'burst_sizes_preview': burst_sizes[:5]}
