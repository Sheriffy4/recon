...
def demo_jitter_injection():
    ...
    for pattern, results in benchmark_results.items():
        print(
            f"{pattern}: avg={results['avg_jitter_ms']:.2f}ms, range={results['jitter_range_ms']:.2f}ms, gen_time={results['avg_generation_time_ms']:.4f}ms"
        )
...
def demo_delay_evasion():
    ...
    for pattern, results in benchmark_results.items():
        print(
            f"{pattern}: total_delay={results['total_delay_ms']:.1f}ms, avg={results['avg_delay_ms']:.2f}ms, gen_time={results['generation_time_ms']:.4f}ms"
        )
...
def demo_burst_traffic():
    ...
    for pattern, results in benchmark_results.items():
        print(
            f"{pattern}: total_packets={results['total_packets']}, avg_size={results['avg_burst_size']:.1f}, gen_time={results['generation_time_ms']:.4f}ms"
        )
...
def demo_timing_controller():
    ...
    for delay_ms in test_delays:
        ...
        for strategy in [
            ...
            print(
                f"  {strategy.value}: actual={measurement.actual_delay_ms:.3f}ms, error={measurement.accuracy_error_ms:.3f}ms, accuracy={measurement.accuracy_percentage:.1f}%"
            )
    ...
    for strategy, results in benchmark_results.items():
        print(
            f"{strategy.value}: avg_accuracy={results['avg_accuracy']:.1f}%, avg_error={results['avg_error_ms']:.3f}ms, max_error={results['max_error_ms']:.3f}ms"
        )
...
def demo_performance_comparison():
    ...
    for result in results:
        print(
            f"{result['name']:<30} {result['avg_time_ms']:<12.2f} {result['avg_packets']:<10.1f} {result['avg_bytes']:<10.1f}"
        )
...
async def demo_async_timing():
    ...
    for delay_ms in test_delays:
        ...
        print(
            f"Async delay {delay_ms}ms: measured={measurement.actual_delay_ms:.3f}ms, wall_time={actual_time:.3f}ms, accuracy={measurement.accuracy_percentage:.1f}%"
        )
    ...
    for delay_id, measurement, wall_time in results:
        print(
            f"  Delay {delay_id}: {measurement.requested_delay_ms}ms -> {measurement.actual_delay_ms:.3f}ms (wall: {wall_time:.3f}ms)"
        )
