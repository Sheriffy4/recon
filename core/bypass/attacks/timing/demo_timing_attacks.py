...
from .timing_base import TimingConfiguration, TimingPattern
...
from .burst_traffic import BurstTrafficAttack, BurstConfiguration, BurstType, BurstTiming
...
def demo_jitter_injection():
    ...
    for pattern, results in benchmark_results.items():
        print(f"{pattern}: avg={results['avg_jitter_ms']:.2f}ms, "
...
def demo_delay_evasion():
    ...
    for pattern, results in benchmark_results.items():
        print(f"{pattern}: total_delay={results['total_delay_ms']:.1f}ms, "
...
def demo_burst_traffic():
    ...
    burst_configs = [
        (BurstType.FIXED_SIZE, BurstTiming.FIXED_INTERVAL, "Fixed Size, Fixed Interval"),
        (BurstType.EXPONENTIAL, BurstTiming.EXPONENTIAL_BACKOFF, "Exponential Size, Exponential Timing"),
        (BurstType.FIBONACCI, BurstTiming.RANDOM_INTERVAL, "Fibonacci Size, Random Timing"),
        (BurstType.RANDOM, BurstTiming.RESPONSE_BASED, "Random Size, Response-Based Timing"),
        (BurstType.ADAPTIVE, BurstTiming.VARIABLE_INTERVAL, "Adaptive Size, Variable Timing")
    ]
    ...
    for pattern, results in benchmark_results.items():
        print(f"{pattern}: total_packets={results['total_packets']}, "
...
def demo_performance_comparison():
    ...
    attacks = [
        ("Jitter Injection (Uniform)", JitterInjectionAttack(JitterConfiguration(
            jitter_type=JitterType.UNIFORM,
            jitter_amplitude_ms=10.0,
            packets_per_burst=5
        ))),
        ("Delay Evasion (Progressive)", DelayEvasionAttack(DelayEvasionConfiguration(
            delay_pattern=DelayPattern.PROGRESSIVE,
            max_progression_steps=5,
            progression_factor=1.5
        ))),
        ("Burst Traffic (Fixed)", BurstTrafficAttack(BurstConfiguration(
    ...
    for result in results:
        print(f"{result['name']:<30} {result['avg_time_ms']:<12.2f} "
...
async def demo_async_timing():
    ...
    for delay_ms in test_delays:
        ...
        print(f"Async delay {delay_ms}ms: "
    ...
    for delay_id, measurement, wall_time in results:
        print(f"  Delay {delay_id}: {measurement.requested_delay_ms}ms -> "
