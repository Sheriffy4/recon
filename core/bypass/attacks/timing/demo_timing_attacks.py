...
def create_demo_context() -> AttackContext:
    ...
    return AttackContext(dst_ip='93.184.216.34', dst_port=443, domain='example.com', payload=b'GET / HTTP/1.1\r\nHost: example.com\r\nUser-Agent: Mozilla/5.0\r\n\r\n')
...
def demo_jitter_injection():
    ...
    jitter_types = [(JitterType.UNIFORM, 'Uniform Random Jitter'), (JitterType.GAUSSIAN, 'Gaussian Distribution Jitter'), (JitterType.PERIODIC, 'Periodic Sine Wave Jitter'), (JitterType.SAWTOOTH, 'Sawtooth Wave Jitter'), (JitterType.ADAPTIVE, 'Adaptive Response-Based Jitter')]
    for jitter_type, description in jitter_types:
        print(f'\n--- {description} ---')
        config = JitterConfiguration(jitter_type=jitter_type, jitter_amplitude_ms=15.0, packets_per_burst=4, inter_packet_base_delay_ms=2.0)
    ...
    for pattern, results in benchmark_results.items():
        print(f"{pattern}: avg={results['avg_jitter_ms']:.2f}ms, range={results['jitter_range_ms']:.2f}ms, gen_time={results['avg_generation_time_ms']:.4f}ms")
...
def demo_delay_evasion():
    ...
    print('=' * 60)
    context = create_demo_context()
    delay_patterns = [(DelayPattern.PROGRESSIVE, 'Progressive Increasing Delays'), (DelayPattern.EXPONENTIAL, 'Exponential Backoff Delays'), (DelayPattern.FIBONACCI, 'Fibonacci Sequence Delays'), (DelayPattern.SINE_WAVE, 'Sine Wave Pattern Delays'), (DelayPattern.RANDOM_WALK, 'Random Walk Delays')]
    for pattern, description in delay_patterns:
        print(f'\n--- {description} ---')
        config = DelayEvasionConfiguration(delay_pattern=pattern, base_delay_ms=5.0, max_progression_steps=6, progression_factor=1.8, packets_per_delay=2)
    ...
    for pattern, results in benchmark_results.items():
        print(f"{pattern}: total_delay={results['total_delay_ms']:.1f}ms, avg={results['avg_delay_ms']:.2f}ms, gen_time={results['generation_time_ms']:.4f}ms")
...
def demo_burst_traffic():
    ...
    print('=' * 60)
    context = create_demo_context()
    burst_configs = [(BurstType.FIXED_SIZE, BurstTiming.FIXED_INTERVAL, 'Fixed Size, Fixed Interval'), (BurstType.EXPONENTIAL, BurstTiming.EXPONENTIAL_BACKOFF, 'Exponential Size, Exponential Timing'), (BurstType.FIBONACCI, BurstTiming.RANDOM_INTERVAL, 'Fibonacci Size, Random Timing'), (BurstType.RANDOM, BurstTiming.RESPONSE_BASED, 'Random Size, Response-Based Timing'), (BurstType.ADAPTIVE, BurstTiming.VARIABLE_INTERVAL, 'Adaptive Size, Variable Timing')]
    for burst_type, burst_timing, description in burst_configs:
        print(f'\n--- {description} ---')
        config = BurstConfiguration(burst_type=burst_type, burst_timing=burst_timing, min_burst_size=3, max_burst_size=12, default_burst_size=6, total_bursts=4, burst_interval_ms=25.0, min_interval_ms=10.0, max_interval_ms=50.0, intra_burst_delay_ms=0.5)
    ...
    config = BurstConfiguration(burst_type=BurstType.VARIABLE_SIZE, concurrent_streams=3, stream_offset_ms=5.0, total_bursts=3, default_burst_size=4)
    attack = BurstTrafficAttack(config)
    result = attack.execute(context)
    ...
    for pattern, results in benchmark_results.items():
        print(f"{pattern}: total_packets={results['total_packets']}, avg_size={results['avg_burst_size']:.1f}, gen_time={results['generation_time_ms']:.4f}ms")
...
def demo_timing_controller():
    ...
    for delay_ms in test_delays:
        ...
        for strategy in [TimingStrategy.SLEEP, TimingStrategy.BUSY_WAIT, TimingStrategy.HYBRID]:
            measurement = controller.delay(delay_ms, strategy)
            print(f'  {strategy.value}: actual={measurement.actual_delay_ms:.3f}ms, error={measurement.accuracy_error_ms:.3f}ms, accuracy={measurement.accuracy_percentage:.1f}%')
    ...
    for strategy, results in benchmark_results.items():
        print(f"{strategy.value}: avg_accuracy={results['avg_accuracy']:.1f}%, avg_error={results['avg_error_ms']:.3f}ms, max_error={results['max_error_ms']:.3f}ms")
...
def demo_performance_comparison():
    ...
    print('=' * 60)
    context = create_demo_context()
    attacks = [('Jitter Injection (Uniform)', JitterInjectionAttack(JitterConfiguration(jitter_type=JitterType.UNIFORM, jitter_amplitude_ms=10.0, packets_per_burst=5))), ('Delay Evasion (Progressive)', DelayEvasionAttack(DelayEvasionConfiguration(delay_pattern=DelayPattern.PROGRESSIVE, max_progression_steps=5, progression_factor=1.5))), ('Burst Traffic (Fixed)', BurstTrafficAttack(BurstConfiguration(burst_type=BurstType.FIXED_SIZE, default_burst_size=8, total_bursts=3)))]
    results = []
    for name, attack in attacks:
        ...
        results.append({'name': name, 'avg_time_ms': avg_execution_time, 'avg_packets': avg_packets_sent, 'avg_bytes': avg_bytes_sent})
    print('\n--- Performance Summary ---')
    print(f"{'Attack Type':<30} {'Time (ms)':<12} {'Packets':<10} {'Bytes':<10}")
    print('-' * 62)
    for result in results:
        print(f"{result['name']:<30} {result['avg_time_ms']:<12.2f} {result['avg_packets']:<10.1f} {result['avg_bytes']:<10.1f}")
...
async def demo_async_timing():
    ...
    for delay_ms in test_delays:
        ...
        print(f'Async delay {delay_ms}ms: measured={measurement.actual_delay_ms:.3f}ms, wall_time={actual_time:.3f}ms, accuracy={measurement.accuracy_percentage:.1f}%')
    ...
    tasks = [concurrent_delay(5.0, 1), concurrent_delay(10.0, 2), concurrent_delay(15.0, 3)]
    start_time = time.perf_counter()
    ...
    for delay_id, measurement, wall_time in results:
        print(f'  Delay {delay_id}: {measurement.requested_delay_ms}ms -> {measurement.actual_delay_ms:.3f}ms (wall: {wall_time:.3f}ms)')
