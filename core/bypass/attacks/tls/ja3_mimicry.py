...
class JA3FingerprintMimicryAttack(BaseAttack):
    ...
    def optional_params(self) -> Dict[str, Any]:
        ...
        return {"profile": "chrome_110_windows"}
    ...
    def _build_mimicked_client_hello_scapy(self, profile: CoherentProfile, domain: str) -> bytes:
        ...
        for ext_val in profile.extensions_order:
            if ext_val == 0:
                ...
                ext_list.append(
                    TLSExtALPN(protocols=[ALPNProtocol(protocol=p) for p in profile.alpn_protocols])
                )
            elif ext_val == 23:
                ...
                ext_list.append(
                    TLSExtKeyShare(client_shares=[KeyShareEntry(group=profile.supported_groups[0])])
                )
            elif ext_val == 65281:
...
class JA4FingerprintMimicryAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            return AttackResult(
                status=AttackStatus.ERROR, error_message=f"JA4 mimicry failed: {str(e)}"
            )
