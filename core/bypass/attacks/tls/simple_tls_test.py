...
def test_tls_payload_validation():
    ...
    def is_tls_record(payload: bytes) -> bool:
        ...
        return (content_type in [0x14, 0x15, 0x16, 0x17] and
...
def test_extension_manipulation():
    ...
    if offset < len(sample_hello):
        ...
        if offset + 2 <= len(sample_hello):
            cipher_suites_len = struct.unpack("!H", sample_hello[offset:offset + 2])[0]
            ...
            if offset < len(sample_hello):
                ...
                if offset + 2 <= len(sample_hello):
                    extensions_len = struct.unpack("!H", sample_hello[offset:offset + 2])[0]
                    print(f"✓ Found extensions at offset {offset}, length {extensions_len}")
                    
                    # Add fake extension
                    fake_ext = b"\x10\x00\x00\x04test"  # Type 0x1000, length 4, data "test"
                    
                    # Insert at beginning of extensions
                    modified_payload = (
                        sample_hello[:offset + 2] +  # Up to extensions length
                        struct.pack("!H", extensions_len + len(fake_ext)) +  # New length
                        fake_ext +  # Fake extension
                        sample_hello[offset + 2:]  # Original extensions
                    )
                    
                    assert len(modified_payload) > len(sample_hello), "Should be larger with fake extension"
