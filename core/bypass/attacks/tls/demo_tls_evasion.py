...
def demo_version_downgrade():
    ...
    for version in versions:
        ...
        if result.status == AttackStatus.SUCCESS:
            print(
                f"    - Target version bytes: {result.metadata.get('target_version_bytes', 'N/A')}"
            )
            print(
                f"    - Modified size: {result.metadata.get('modified_size', 'N/A')} bytes"
            )
        elif result.error_message:
            print(f"    - Error: {result.error_message}")
        print()
...
def demo_extension_manipulation():
    ...
    for manipulation_type in manipulation_types:
        ...
        if result.status == AttackStatus.SUCCESS:
            print(
                f"    - Original size: {result.metadata.get('original_size', 'N/A')} bytes"
            )
            print(
                f"    - Modified size: {result.metadata.get('modified_size', 'N/A')} bytes"
            )
            size_increase = result.metadata.get(
                "modified_size", 0
            ) - result.metadata.get("original_size", 0)
...
def demo_record_fragmentation():
    ...
    for frag_type, params in fragmentation_types:
        ...
        if result.status == AttackStatus.SUCCESS:
            print(f"    - Packets sent: {result.packets_sent}")
            print(
                f"    - Segments count: {result.metadata.get('segments_count', 'N/A')}"
            )
