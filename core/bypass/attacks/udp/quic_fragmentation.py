...
class QUICFragmentationAttack(UDPAttackBase):
    ...
    def _fragment_at_frame_boundaries(
        self,
        quic_packet: QUICPacket
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if self.config.preserve_header_in_first_fragment:
                first_fragment_size = min(
                    header_size + self.config.fragment_size,
                    len(quic_packet.raw_data)
                )
            ...
            while offset < len(quic_packet.raw_data):
                fragment_size = min(
                    ...
                    len(quic_packet.raw_data) - offset
                )
