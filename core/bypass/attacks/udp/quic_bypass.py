...
class QUICBypassAttack(BaseAttack):
    ...
    def __init__(
        self, name: str = "quic_bypass", config: Optional[QUICBypassConfig] = None
    ):
        super().__init__()
    ...
    async def _create_quic_bypass_segments(
        ...
        if self.config.packet_number_offset > 0 and len(payload) >= 16:
            ...
            segments.append(
                (bytes(offset_payload), 0, {"is_fake": True, "ttl": 3, "delay_ms": 5})
            )

        # Send original packet
        segments.append((payload, 0, {"is_fake": False, "delay_ms": 10}))
