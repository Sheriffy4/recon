...
from .attack_definition import AttackCategory, AttackComplexity, AttackStability
...
class TestWindowManipulationAttack(unittest.TestCase):
    ...
    def test_execute_success(self):
        ...
        # Check metadata
        self.assertEqual(result.get_metadata("fragmentation_type"), "window_manipulation")
...
class TestTCPOptionsModificationAttack(unittest.TestCase):
    ...
    def test_execute_success(self):
        ...
        # Check metadata
        self.assertEqual(result.get_metadata("fragmentation_type"), "tcp_options_modification")
    ...
    def test_create_timestamp_options(self):
        ...
        self.assertIsInstance(options, bytes)
        self.assertEqual(len(options), 10)  # kind(1) + length(1) + timestamp(4) + echo(4)
    ...
    def test_execute_with_different_options(self):
        ...
        options_types = ["mss", "window_scale", "timestamp", "sack_permitted", "md5_signature", "custom"]
...
class TestTCPFragmentationRegistration(unittest.TestCase):
    ...
    @patch('recon.core.bypass.attacks.tcp_fragmentation.get_modern_registry')
    def test_register_tcp_fragmentation_attacks(self, mock_get_registry):
        """Test registration of all TCP fragmentation attacks."""
        mock_registry = Mock()
        mock_registry.register_attack.return_value = True
        mock_get_registry.return_value = mock_registry
        
        # Import and call registration function
        from .tcp_fragmentation import register_tcp_fragmentation_attacks
        
        registered_count = register_tcp_fragmentation_attacks()
        
        # Should register 6 attacks
        self.assertEqual(registered_count, 6)
        self.assertEqual(mock_registry.register_attack.call_count, 6)
        
        # Verify attack definitions were registered
        call_args_list = mock_registry.register_attack.call_args_list
        registered_attack_ids = []
        
        for call_args in call_args_list:
            definition, attack_class = call_args[0]
            registered_attack_ids.append(definition.id)
            
            # Verify definition structure
            self.assertIsInstance(definition.id, str)
            self.assertIsInstance(definition.name, str)
            self.assertEqual(definition.category, AttackCategory.TCP_FRAGMENTATION)
            self.assertIn(definition.complexity, [AttackComplexity.SIMPLE, AttackComplexity.MODERATE, AttackComplexity.ADVANCED])
            self.assertIn(definition.stability, [AttackStability.STABLE, AttackStability.MODERATE])
            self.assertGreater(len(definition.test_cases), 0)
        
        # Verify all expected attacks were registered
        expected_attacks = [
            "simple_fragment",
            "fake_disorder", 
            "multisplit",
        ...
        for expected_attack in expected_attacks:
            self.assertIn(expected_attack, registered_attack_ids)
