    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if not 0 < split_pos < len(payload):
                ...
                segments = [(part1, 0, {'window_scale': window_scale}), (part2, split_pos, {'window_scale': window_scale * 2})]
            packets_sent = len(segments)
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'window_scale': window_scale, 'split_pos': split_pos, 'segments': segments if context.engine_type != 'local' else None})
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if modification_type == 'add_mss_abuse':
                ...
                return AttackResult(status=AttackStatus.INVALID_PARAMS, error_message=f'Invalid modification_type: {modification_type}')
            ...
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'modification_type': modification_type, 'split_pos': split_pos, 'segment_count': len(segments), 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if manipulation_type == 'overlap_forward':
                ...
                return AttackResult(status=AttackStatus.INVALID_PARAMS, error_message=f'Invalid manipulation_type: {manipulation_type}')
            packets_sent = len(segments)
            bytes_sent = sum((len(s[0]) for s in segments))
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'manipulation_type': manipulation_type, 'overlap_size': overlap_size if 'overlap' in manipulation_type else None, 'gap_size': gap_size if 'gap' in manipulation_type else None, 'split_pos': split_pos, 'segment_count': len(segments), 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
...
class TCPWindowManipulationAttack(ManipulationAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'manipulation_type': manipulation_type, 'split_pos': split_pos, 'segment_count': len(segments), 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
...
class TCPFragmentationAttack(ManipulationAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'frag_size': frag_size, 'fragments_count': len(segments), 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if not 0 < split_pos < len(payload):
                ...
                part1 = payload[:split_pos]
                part2 = payload[split_pos:]
                segments = [(part1, 0, {'urgent': True, 'urgent_size': urgent_data_size}), (part2, split_pos, {'urgent': False})]
            packets_sent = len(segments)
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'split_pos': split_pos, 'urgent_data_size': urgent_data_size, 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if not 0 < split_pos < len(payload):
                ...
                part1 = payload[:split_pos]
                part2 = payload[split_pos:]
                segments = [(part1, 0, {'tcp_options': padding_options}), (part2, split_pos, {'tcp_options': b''})]
            packets_sent = len(segments)
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'padding_size': padding_size, 'split_pos': split_pos, 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
...
class TCPMultiSplitAttack(ManipulationAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            self.logger.debug(f'Executing multisplit with count={split_count}, overlap={overlap_size}, fooling={fooling_methods}')
            if len(payload) < split_count:
                return AttackResult(status=AttackStatus.INVALID_PARAMS, error_message=f'Payload too small ({len(payload)} bytes) for {split_count} splits.')
            segments = []
            ...
            bytes_sent = sum((len(s[0]) for s in segments))
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'split_count': split_count, 'overlap_size': overlap_size, 'fooling_methods': fooling_methods, 'segment_count': len(segments), 'segments': segments})
        except Exception as e:
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if not 0 < split_pos < len(payload):
                ...
                part1 = payload[:split_pos]
                part2 = payload[split_pos:]
                segments = [(part1, 0, {'timestamp': ts_val1}), (part2, split_pos, {'timestamp': ts_val2})]
            packets_sent = len(segments)
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'split_pos': split_pos, 'timestamp1': ts_val1, 'timestamp2': ts_val2, 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if not 0 < split_pos < len(payload):
                ...
                part1 = payload[:split_pos]
                part2 = payload[split_pos:]
                segments = [(part1, 0, {'window_size': window_size}), (part2, split_pos, {'window_size': window_size})]
            packets_sent = len(segments)
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'window_size': window_size, 'split_pos': split_pos, 'segment_count': len(segments), 'attack_type': 'window_size_limit', 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)