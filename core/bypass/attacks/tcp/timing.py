    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            while offset < len(payload):
                ...
                offset += chunk_size
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=(time.time() - start_time) * 1000, packets_sent=len(segments), bytes_sent=len(payload), connection_established=True, data_transmitted=True, metadata={'drip_chunk_size': chunk_size, 'drip_delay_ms': delay_ms, 'packet_count': len(segments), 'randomized_timing': self.config.drip_randomize, 'segments': segments})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=f'Drip feed attack failed: {str(e)}', latency_ms=(time.time() - start_time) * 1000)
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if not 0 < split_pos < len(payload):
                ...
                segments = [(part1, 0, {'delay_ms': 0}), (part2, split_pos, {'delay_ms': delay_ms})]
            packets_sent = len(segments)
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'split_pos': split_pos, 'delay_ms': delay_ms, 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
...
class BurstTimingEvasionAttack(TimingAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            bytes_sent = len(payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'burst_size': burst_size, 'burst_delay_ms': burst_delay_ms, 'bursts_count': burst_count, 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)