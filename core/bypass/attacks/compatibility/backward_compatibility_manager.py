...
class CompatibilityMode(Enum):
    ...
    LEGACY_ONLY = "legacy_only"  # Use only modified_payload
    SEGMENTS_ONLY = "segments_only"  # Use only segments
    HYBRID = "hybrid"  # Use both with fallback
    AUTO_DETECT = "auto_detect"  # Automatically detect best mode
...
class BackwardCompatibilityManager:
    ...
    def _check_segments_support(self, attack: BaseAttack) -> bool:
        ...
        try:
            ...
            # Check if result metadata indicates segments support
    ...
    def _check_legacy_support(self, attack: BaseAttack) -> bool:
        ...
        try:
            ...
            # Check if result has modified_payload
            if hasattr(result, 'modified_payload') and result.modified_payload:
                return True
            
            # Check if result metadata indicates legacy support
    ...
    def execute_with_fallback(self, attack: BaseAttack, context: AttackContext, 
                            preferred_mode: CompatibilityMode = CompatibilityMode.AUTO_DETECT) -> AttackResult:
        """
        ...
        if execution_mode == CompatibilityMode.SEGMENTS_ONLY:
            ...
            # Fallback to best available mode
    ...
    def _execute_segments_mode(self, attack: BaseAttack, context: AttackContext, 
                             stats: Dict[str, int]) -> AttackResult:
    ...
    def _execute_legacy_mode(self, attack: BaseAttack, context: AttackContext, 
                           stats: Dict[str, int]) -> AttackResult:
        ...
        try:
            ...
            # Convert legacy result to segments if needed
            if not hasattr(result, '_segments') or not result._segments:
                if hasattr(result, 'modified_payload') and result.modified_payload:
                    # Convert modified_payload to single segment
                    result._segments = [(result.modified_payload, 0, {})]
                else:
                    # Use original payload as single segment
    ...
    def _execute_hybrid_mode(self, attack: BaseAttack, context: AttackContext, 
                           stats: Dict[str, int], report: CompatibilityReport) -> AttackResult:
        """Execute attack in hybrid mode with fallback."""
        # Try segments first if supported
        if report.has_segments_support:
            try:
                return self._execute_segments_mode(attack, context, stats)
            except Exception as e:
    ...
    def get_compatibility_stats(self) -> Dict[str, Any]:
        ...
        segments_supported = sum(1 for report in self._compatibility_cache.values() 
                               if report.has_segments_support)
        legacy_supported = sum(1 for report in self._compatibility_cache.values() 
                             if report.has_legacy_support)
        migration_required = sum(1 for report in self._compatibility_cache.values() 
        ...
        return {
            ...
            'migration_required': migration_required,
            'compatibility_percentage': (segments_supported / total_attacks * 100) if total_attacks > 0 else 0,
    ...
    def generate_migration_plan(self, attacks: List[BaseAttack]) -> Dict[str, Any]:
        ...
        """
        migration_plan = {
            'high_priority': [],  # Attacks that must be migrated
            'medium_priority': [],  # Attacks that should be migrated
            'low_priority': [],  # Attacks that can be migrated later
            'no_migration': [],  # Attacks that don't need migration
            'summary': {}
        }
    ...
    def validate_migration(self, attack: BaseAttack, test_contexts: List[AttackContext]) -> Dict[str, Any]:
        ...
        validation_results['success_rate'] = (
            ...
            if validation_results['test_count'] > 0 else 0
        )
