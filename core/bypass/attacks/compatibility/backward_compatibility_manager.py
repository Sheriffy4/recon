...
class BackwardCompatibilityManager:
    ...
    def _analyze_attack_compatibility(self, attack: BaseAttack) -> CompatibilityReport:
        ...
        return CompatibilityReport(attack_name=attack_name, has_segments_support=has_segments_support, has_legacy_support=has_legacy_support, recommended_mode=recommended_mode, migration_required=migration_required, migration_complexity=migration_complexity, issues=issues, recommendations=recommendations)

    def _check_segments_support(self, attack: BaseAttack) -> bool:
        """Check if attack supports segments architecture."""
        try:
            test_context = AttackContext(dst_ip='127.0.0.1', dst_port=80, payload=b'test', connection_id='compatibility_test')
    ...
    def execute_with_fallback(self, attack: BaseAttack, context: AttackContext, preferred_mode: CompatibilityMode=CompatibilityMode.AUTO_DETECT) -> AttackResult:
        """
        ...
        if attack_name not in self._fallback_stats:
            self._fallback_stats[attack_name] = {'segments_attempts': 0, 'segments_successes': 0, 'legacy_attempts': 0, 'legacy_successes': 0, 'fallback_used': 0}
    ...
    def _execute_segments_mode(self, attack: BaseAttack, context: AttackContext, stats: Dict[str, int]) -> AttackResult:
    ...
    def _execute_legacy_mode(self, attack: BaseAttack, context: AttackContext, stats: Dict[str, int]) -> AttackResult:
    ...
    def _execute_hybrid_mode(self, attack: BaseAttack, context: AttackContext, stats: Dict[str, int], report: CompatibilityReport) -> AttackResult:
        ...
        if report.has_legacy_support:
            return self._execute_legacy_mode(attack, context, stats)
        raise RuntimeError(f'No compatible execution mode available for {attack.__class__.__name__}')
    ...
    def get_compatibility_stats(self) -> Dict[str, Any]:
        ...
        segments_supported = sum((1 for report in self._compatibility_cache.values() if report.has_segments_support))
        legacy_supported = sum((1 for report in self._compatibility_cache.values() if report.has_legacy_support))
        migration_required = sum((1 for report in self._compatibility_cache.values() if report.migration_required))
        return {'total_attacks_analyzed': total_attacks, 'segments_supported': segments_supported, 'legacy_supported': legacy_supported, 'migration_required': migration_required, 'compatibility_percentage': segments_supported / total_attacks * 100 if total_attacks > 0 else 0, 'fallback_stats': dict(self._fallback_stats)}
    ...
    def generate_migration_plan(self, attacks: List[BaseAttack]) -> Dict[str, Any]:
        ...
        """
        migration_plan = {'high_priority': [], 'medium_priority': [], 'low_priority': [], 'no_migration': [], 'summary': {}}
        for attack in attacks:
            report = self.check_attack_compatibility(attack)
            attack_info = {'name': report.attack_name, 'complexity': report.migration_complexity, 'issues': report.issues, 'recommendations': report.recommendations}
        ...
        migration_plan['summary'] = {'total_attacks': len(attacks), 'high_priority_count': len(migration_plan['high_priority']), 'medium_priority_count': len(migration_plan['medium_priority']), 'low_priority_count': len(migration_plan['low_priority']), 'no_migration_count': len(migration_plan['no_migration']), 'estimated_effort': self._estimate_migration_effort(migration_plan)}
    ...
    def validate_migration(self, attack: BaseAttack, test_contexts: List[AttackContext]) -> Dict[str, Any]:
        """
        ...
        Returns:
            Validation results
        """
        validation_results = {'attack_name': attack.__class__.__name__, 'test_count': len(test_contexts), 'passed_tests': 0, 'failed_tests': 0, 'errors': [], 'warnings': [], 'performance_metrics': {}}
        for i, context in enumerate(test_contexts):
            try:
                ...
                if result.status != AttackStatus.SUCCESS:
                    validation_results['errors'].append(f'Test {i}: Attack failed with status {result.status}')
                    validation_results['failed_tests'] += 1
                    continue
                if not hasattr(result, '_segments') or not result._segments:
                    validation_results['warnings'].append(f'Test {i}: No segments generated')
                if execution_time > 0.1:
                    validation_results['warnings'].append(f'Test {i}: Slow execution ({execution_time:.3f}s)')
                ...
                validation_results['failed_tests'] += 1
        validation_results['success_rate'] = validation_results['passed_tests'] / validation_results['test_count'] * 100 if validation_results['test_count'] > 0 else 0
