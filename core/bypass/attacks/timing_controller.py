...
class PreciseTimingController:
    ...
    def delay(
        self, delay_ms: float, strategy: Optional[TimingStrategy] = None
    ) -> TimingMeasurement:
        """
        ...
        if self.logger.isEnabledFor(logging.DEBUG):
            self.logger.debug(
                f"Timing: requested={delay_ms:.3f}ms, actual={actual_delay_ms:.3f}ms, "
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.statistics.measurements:
            ...
            stats["recent_error_ms"] = statistics.mean(
                [abs(m.accuracy_error_ms) for m in recent_measurements]
            )
    ...
    def benchmark_strategies(
        self, test_delays: List[float]
    ) -> Dict[TimingStrategy, Dict[str, float]]:
        """
        ...
        for strategy in TimingStrategy:
            ...
            for delay_ms in test_delays:
                try:
                    ...
                    self.logger.warning(
                        f"Strategy {strategy.value} failed for delay {delay_ms}ms: {e}"
                    )
