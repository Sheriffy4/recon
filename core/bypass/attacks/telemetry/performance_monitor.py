    ...
    def _check_degradation(
        ...
        if current_success_rate < baseline.avg_success_rate * (1 - self.degradation_threshold):
            ...
            degradation = PerformanceDegradation(
                ...
                threshold_exceeded=(baseline.avg_success_rate - current_success_rate) / baseline.avg_success_rate,
                diagnostic_info={
                    'recent_failures': sum(1 for x in list(self._success_rates[attack_name])[-10:] if x == 0),
                    'baseline_samples': baseline.sample_count
                }
            )
        ...
        if throughput_pps < baseline.avg_throughput_pps * (1 - self.degradation_threshold):
            ...
            degradation = PerformanceDegradation(
                ...
                threshold_exceeded=(baseline.avg_throughput_pps - throughput_pps) / baseline.avg_throughput_pps,
                diagnostic_info={
        ...
        if current_error_rate > baseline.avg_error_rate * (1 + self.degradation_threshold):
            ...
            degradation = PerformanceDegradation(
                ...
                threshold_exceeded=(current_error_rate - baseline.avg_error_rate) / (baseline.avg_error_rate + 0.01),
                diagnostic_info={
                    'recent_errors': sum(1 for x in list(self._error_rates[attack_name])[-10:] if x == 1),
                    'baseline_samples': baseline.sample_count
                }
            )
