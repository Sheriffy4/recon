...
class AttackExecutionLogger:
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        success_count = sum(
            1 for e in self._execution_history if e.status == ExecutionStatus.SUCCESS
        )
        failure_count = sum(
            1 for e in self._execution_history if e.status == ExecutionStatus.FAILURE
        )
        error_count = sum(1 for e in self._execution_history if e.status == ExecutionStatus.ERROR)

        avg_time = sum(e.execution_time_ms for e in self._execution_history) / len(
