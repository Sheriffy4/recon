...
class PrometheusExporter(MetricsExporter):
    ...
    def export(self, snapshot: MetricsSnapshot) -> str:
        ...
        for attack_name, metrics in snapshot.attack_metrics.items():
            ...
            # Total executions
            lines.append(
                f"# HELP {self.namespace}_executions_total " f"Total number of attack executions"
            )
            ...
            # Failed executions
            lines.append(
                f"# HELP {self.namespace}_executions_failed " f"Number of failed attack executions"
            )
            ...
            # Error executions
            lines.append(
                f"# HELP {self.namespace}_executions_error " f"Number of error attack executions"
            )
            ...
            # Success rate
            lines.append(
                f"# HELP {self.namespace}_success_rate " f"Success rate of attack executions"
            )
            ...
            # Segments generated
            lines.append(
                f"# HELP {self.namespace}_segments_total " f"Total number of segments generated"
            )
            ...
            # Fallback count
            lines.append(
                f"# HELP {self.namespace}_fallback_total " f"Total number of fallback executions"
            )
            ...
            # Fallback rate
            lines.append(f"# HELP {self.namespace}_fallback_rate " f"Rate of fallback executions")
        ...
        lines.append(
            f"{self.namespace}_throughput_bytes_per_second " f"{throughput.bytes_per_second:.2f}"
        )
        lines.append("")
        ...
        lines.append(f"# HELP {self.namespace}_unique_attacks " f"Number of unique attack types")
        lines.append(f"# TYPE {self.namespace}_unique_attacks gauge")
        lines.append(f"{self.namespace}_unique_attacks " f'{global_stats.get("unique_attacks", 0)}')
        lines.append("")
        ...
        for snapshot in self._snapshots:
            for attack_name, metrics in snapshot.attack_metrics.items():
                if attack_name not in aggregated_attacks:
                    aggregated_attacks[attack_name] = AttackMetrics(attack_name=attack_name)
                ...
                agg.min_execution_time_ms = min(
                    agg.min_execution_time_ms, metrics.min_execution_time_ms
                )
                agg.max_execution_time_ms = max(
                    agg.max_execution_time_ms, metrics.max_execution_time_ms
                )
                agg.total_segments_generated += metrics.total_segments_generated
                agg.fallback_count += metrics.fallback_count

        # Aggregate throughput
        total_packets = sum(s.throughput_metrics.total_packets for s in self._snapshots)
        total_bytes = sum(s.throughput_metrics.total_bytes for s in self._snapshots)
