...
class TLSProtocolMimicryAttack(BaseAttack):
    ...
    def _generate_fake_certificate(self, server_name: str) -> bytes:
        ...
        cert_body = (
            ...
            b"\x02\x08" + random.randbytes(8) +  # Serial number
            b"\x30\x0d\x06\x09\x2a\x86\x48\x86\xf7\x0d\x01\x01\x0b\x05\x00" +  # Signature algorithm
            b"\x30\x10\x31\x0e\x30\x0c\x06\x03\x55\x04\x03\x0c\x05" + b"TestCA" +  # Issuer
            b"\x30\x1e\x17\x0d" + b"231201000000Z" + b"\x17\x0d" + b"241201000000Z" +  # Validity
            b"\x30\x20\x31\x1e\x30\x1c\x06\x03\x55\x04\x03\x0c\x15" + server_name.encode('utf-8')[:21] +  # Subject
            b"\x30\x82\x01\x22" + random.randbytes(290)  # Public key (fake)
        )
...
class SMTPProtocolMimicryAttack(BaseAttack):
    ...
    def _generate_smtp_conversation(self, payload: bytes, server: str, sender: str, recipient: str, use_tls: bool) -> List[bytes]:
        ...
        # Client EHLO
        packets.append(f"EHLO client.example.com\r\n".encode('utf-8'))
