...
class DemoTCPFragmentationAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        return AttackResult(status=AttackStatus.SUCCESS, technique_used='tcp_fragmentation', latency_ms=25.5, packets_sent=3, bytes_sent=len(context.payload) * 3)
...
class DemoHTTPManipulationAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        return AttackResult(status=AttackStatus.SUCCESS, technique_used='http_header_modification', latency_ms=15.2, packets_sent=2, bytes_sent=len(context.payload) * 2)

def create_demo_attack_definitions():
    """Create demo attack definitions with comprehensive metadata."""
    tcp_attack = AttackDefinition(id='demo_tcp_fragmentation', name='Demo TCP Fragmentation', description='Demonstrates TCP packet fragmentation to bypass DPI inspection', category=AttackCategory.TCP_FRAGMENTATION, complexity=AttackComplexity.MODERATE, stability=AttackStability.STABLE, parameters={'fragment_size': 64, 'fragment_delay_ms': 10, 'randomize_order': False}, default_parameters={'fragment_size': 64, 'fragment_delay_ms': 10, 'randomize_order': False}, required_parameters=['fragment_size'], compatibility=[CompatibilityMode.ZAPRET, CompatibilityMode.NATIVE], external_tool_mappings={'zapret': '--dpi-desync=split --dpi-desync-split-pos=64', 'goodbyedpi': '-f 64'}, stability_score=0.9, effectiveness_score=0.8, performance_score=0.85, supported_protocols=['tcp'], supported_ports=[80, 443, 8080], requires_handshake=True, documentation_url='https://example.com/tcp-fragmentation-docs', examples=[{'name': 'Basic fragmentation', 'parameters': {'fragment_size': 64, 'fragment_delay_ms': 10}, 'description': 'Basic TCP fragmentation with 64-byte fragments'}, {'name': 'Aggressive fragmentation', 'parameters': {'fragment_size': 32, 'fragment_delay_ms': 5, 'randomize_order': True}, 'description': 'Aggressive fragmentation with randomized packet order'}], author='Demo System', version='1.2.0')
    tcp_attack.add_tag('tcp')
    tcp_attack.add_tag('fragmentation')
    tcp_attack.add_tag('stable')
    tcp_attack.add_tag('recommended')
    tcp_test_basic = TestCase(id='tcp_basic_test', name='Basic TCP Fragmentation Test', description='Test basic TCP fragmentation functionality', target_domain='httpbin.org', expected_success=True, test_parameters={'fragment_size': 64}, timeout_seconds=30, validation_criteria=['http_response', 'content_check', 'timing_analysis'])
    tcp_test_aggressive = TestCase(id='tcp_aggressive_test', name='Aggressive TCP Fragmentation Test', description='Test aggressive TCP fragmentation with small fragments', target_domain='example.com', expected_success=True, test_parameters={'fragment_size': 32, 'randomize_order': True}, timeout_seconds=45)
    tcp_attack.add_test_case(tcp_test_basic)
    tcp_attack.add_test_case(tcp_test_aggressive)
    http_attack = AttackDefinition(id='demo_http_manipulation', name='Demo HTTP Header Manipulation', description='Demonstrates HTTP header modification to bypass DPI inspection', category=AttackCategory.HTTP_MANIPULATION, complexity=AttackComplexity.SIMPLE, stability=AttackStability.MOSTLY_STABLE, parameters={'header_case': 'mixed', 'add_fake_headers': True, 'host_header_split': False}, default_parameters={'header_case': 'mixed', 'add_fake_headers': True, 'host_header_split': False}, required_parameters=['header_case'], compatibility=[CompatibilityMode.GOODBYEDPI, CompatibilityMode.NATIVE], external_tool_mappings={'goodbyedpi': '-m --fake-gen 10', 'byebyedpi': '--fake-gen 10 --mod-http'}, stability_score=0.75, effectiveness_score=0.85, performance_score=0.9, supported_protocols=['tcp'], supported_ports=[80, 8080], requires_handshake=False, conflicts_with=['demo_tcp_fragmentation'], examples=[{'name': 'Mixed case headers', 'parameters': {'header_case': 'mixed', 'add_fake_headers': False}, 'description': 'Use mixed case HTTP headers'}], author='Demo System', version='1.0.1')
    http_attack.add_tag('http')
    http_attack.add_tag('headers')
    http_attack.add_tag('simple')
    http_test = TestCase(id='http_basic_test', name='Basic HTTP Manipulation Test', description='Test basic HTTP header manipulation', target_domain='httpbin.org', expected_success=True, test_parameters={'header_case': 'mixed'}, timeout_seconds=20)
...
def demonstrate_registry_operations():
    ...
    for attack_id in all_attacks:
        ...
        LOG.info(f'  - {attack_id}: {definition.name} (Category: {definition.category.value}, Complexity: {definition.complexity.value}, Overall Score: {definition.get_overall_score():.2f})')
    ...
    LOG.info(f"Enabled attacks after re-enable: {stats_after_enable['enabled_attacks']}")
...
def demonstrate_attack_definition_features():
    ...
    attack = AttackDefinition(id='feature_demo_attack', name='Feature Demo Attack', description='Demonstrates all AttackDefinition features', category=AttackCategory.EXPERIMENTAL, complexity=AttackComplexity.EXPERT, stability=AttackStability.EXPERIMENTAL, stability_score=0.6, effectiveness_score=0.9, performance_score=0.7)
    ...
    LOG.info(f'Compatible with NATIVE: {attack.is_compatible_with(CompatibilityMode.NATIVE)}')
    LOG.info(f'Compatible with ZAPRET: {attack.is_compatible_with(CompatibilityMode.ZAPRET)}')
    LOG.info(f'Compatible with GOODBYEDPI: {attack.is_compatible_with(CompatibilityMode.GOODBYEDPI)}')
