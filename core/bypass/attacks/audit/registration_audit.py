...
class AttackRegistrationAuditor:
    ...
    def audit_all_registrations(self) -> RegistrationAuditReport:
        ...
        all_attack_names = (
            set(decorator_attacks.keys()) | set(manual_attacks.keys()) | primitive_attacks
        )
        ...
        for attack_name in all_attack_names:
            ...
            if attack_name in decorator_attacks:
                ...
                status.decorator_priority = decorator_attacks[attack_name].get(
                    "priority", "UNKNOWN"
                )
            ...
            for attack_name in registry.list_attacks():
                ...
                if entry:
                    attacks[attack_name] = {
                        "priority": (
                            entry.priority.name
                            if hasattr(entry.priority, "name")
