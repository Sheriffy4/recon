...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run comprehensive attack implementation audit'
    )
    parser.add_argument(
        '--output', '-o',
    ...
    parser.add_argument(
        '--no-logs',
        action='store_true',
        help='Skip log file analysis (only analyze registry)'
    )
    parser.add_argument(
        '--log-paths',
        nargs='+',
        help='Specific log files to analyze (default: auto-discover)'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    try:
        ...
        report = auditor.generate_comprehensive_report(
            include_log_analysis=not args.no_logs,
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            if report.missing_implementations:
                ...
                f.write("Attacks ranked by implementation priority (based on fallback frequency and category):\n\n")
                
                priority_ranking = auditor.rank_attacks_by_priority(report)
                for i, (attack, score) in enumerate(priority_ranking[:20], 1):
                    fallback_count = report.fallback_frequency.get(attack, 0)
                    f.write(f"{i}. **{attack}** (priority score: {score}, fallbacks: {fallback_count})\n")
        ...
        if report.missing_implementations:
            logger.warning(f"Found {len(report.missing_implementations)} attacks needing advanced implementations")
            return 1
        else:
