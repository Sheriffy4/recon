...
from .advanced_base import AdvancedAttack, AdvancedAttackConfig
...
class StatefulFragmentationAttack(AdvancedAttack):
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if not payload or len(payload) < 20: # Ensure payload is large enough to split
                return AttackResult(
                    status=AttackStatus.INVALID_PARAMS,
                    error_message="Payload too small for stateful fragmentation.",
                )

            # Get parameters or use defaults from config
            ...
            # 1. First valid fragment
            part1 = payload[:split_pos]
            segment1_options = {}
            segment1 = (part1, 0, segment1_options)

            # 2. Garbage packet
            # The sequence number should be where the next packet *would* be.
            # We give it a bad checksum to ensure the host's kernel drops it.
            garbage_options = {"bad_checksum": True}
            garbage_seq_offset = len(part1)
            segment2 = (garbage_data, garbage_seq_offset, garbage_options)

            # 3. Second valid fragment
            part2 = payload[split_pos:]
            segment3_options = {}
            # The sequence offset for the second part must be correct, as if the garbage packet never existed.
...
class AdvancedOverlapAttack(AdvancedAttack):
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Segment 1: The payload for the DPI to see.
            segment1_options = {}
            segment1 = (dpi_payload, 0, segment1_options)

            # Segment 2: The real payload for the host to accept.
            # Sent at the same sequence offset to overwrite the DPI payload.
            segment2_options = {}
...
# --- New Registration with AdvancedAttackRegistry ---

# 1. Define configurations for the attacks
stateful_fragment_config = AdvancedAttackConfig(
    ...
    default_params={
        "split_pos": 10,
        "garbage_data": b"GARBAGE_PACKET_CONTENT",
    },
...
# 2. Register attacks with the global registry instance
try:
