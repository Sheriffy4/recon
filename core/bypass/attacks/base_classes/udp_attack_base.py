...
class QUICPacket:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.is_long_header:
            ...
            result.update({"spin_bit": self.spin_bit, "key_phase": self.key_phase})
...
class STUNMessage:
    ...
    def __init__(self, data: bytes):
        ...
        while offset + 4 <= len(data):
            ...
            self.attributes.append({"type": attr_type, "length": attr_length, "value": attr_value})
    ...
    def get_message_method(self) -> int:
        ...
        return (
            (self.message_type & 0x000F)
            | ((self.message_type >> 1) & 0x0070)
            | ((self.message_type >> 2) & 0x0F80)
        )
...
class UDPAttackBase(BaseAttack):
    ...
    def build_udp_packet(
        self, src_port: int, dst_port: int, payload: bytes, checksum: Optional[int] = None
    ) -> bytes:
        """
    ...
    def detect_quic(self, data: bytes) -> bool:
        ...
        if (first_byte & 0x80) != 0:
            ...
            if (
                ...
                or version == QUIC_VERSION_1
                or (version & 0xFF000000) == 0xFF000000
            ):
    ...
    def build_quic_initial_packet(
        self, version: int, dcid: bytes, scid: bytes, payload: bytes
    ) -> bytes:
        """
    ...
    def build_stun_message(
        ...
        header = struct.pack(
            "!HHI12s", message_type, message_length, STUN_MAGIC_COOKIE, transaction_id
        )
    ...
    def fragment_udp_payload(
        self, payload: bytes, fragment_size: int = DEFAULT_FRAGMENT_SIZE
    ) -> List[bytes]:
        """
