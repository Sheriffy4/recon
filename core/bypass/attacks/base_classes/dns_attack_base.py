...
class DNSQuestion:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {"name": self.name, "type": self.qtype, "class": self.qclass}
...
class DNSAttackBase(BaseAttack):
    ...
    def create_tunnel_domain(
        self, data: bytes, base_domain: str, scheme: str = ENCODING_BASE32
    ) -> str:
        """
        ...
        if len(tunnel_domain) > DNS_MAX_NAME_LENGTH:
            raise ValueError(
                f"Tunnel domain too long: {len(tunnel_domain)} > {DNS_MAX_NAME_LENGTH}"
            )

        return tunnel_domain

    def extract_tunnel_data(
        self, domain: str, base_domain: str, scheme: str = ENCODING_BASE32
    ) -> Optional[bytes]:
        """
