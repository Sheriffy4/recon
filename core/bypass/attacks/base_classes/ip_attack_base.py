...
class IPAttackBase(BaseAttack):
    ...
    def validate_ip_checksum(self, packet: IPPacket) -> bool:
        ...
        header = self._build_ip_header(
            packet.version, packet.ihl, packet.tos, packet.total_length,
            packet.identification, packet.flags, packet.fragment_offset,
            packet.ttl, packet.protocol, 0,  # checksum = 0 for calculation
            packet.src_ip, packet.dst_ip, packet.options
        )
    ...
    def build_ip_packet(
        ...
        header = self._build_ip_header(
            self.IP_VERSION, ihl, tos, total_length,
            identification, flags, fragment_offset,
            ttl, protocol, 0,  # checksum calculated later
