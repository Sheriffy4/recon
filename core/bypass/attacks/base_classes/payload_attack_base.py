...
class PayloadAttackBase(BaseAttack):
    ...
    def split_into_chunks(self, payload: bytes, chunk_size: int) -> List[bytes]:
        ...
        self.logger.debug(
            f"Split {len(payload)} bytes into {len(chunks)} chunks of size {chunk_size}"
        )
        return chunks

    def log_payload_operation(
        self, operation: str, original_size: int, modified_size: int, details: str = ""
    ):
        """
    ...
    def create_payload_result(
        ...
        # Log the operation
        self.log_payload_operation(operation, len(original_payload), len(modified_payload))
