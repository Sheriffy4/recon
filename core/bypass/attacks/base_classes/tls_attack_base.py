...
class TLSAttackBase(BaseAttack):
    ...
    def is_tls_client_hello(self, payload: bytes) -> bool:
        ...
        return (
            ...
            and payload[2] in [0x01, 0x02, 0x03, 0x04]  # TLS version minor
            and len(payload) > 5
        ...
        try:
            ...
            if offset < len(payload):
                ...
                while offset < extensions_end:
                    ...
                    extensions.append({"type": ext_type, "length": ext_length, "data": ext_data})
    ...
    def build_client_hello(self, parsed: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            client_hello += (
                struct.pack("!H", len(parsed["cipher_suites"])) + parsed["cipher_suites"]
            )
            client_hello += (
                bytes([len(parsed["compression_methods"])]) + parsed["compression_methods"]
            )
    ...
    def get_extension_by_type(
        self, extensions: List[Dict[str, Any]], ext_type: int
    ...
    def add_extension(
        self, extensions: List[Dict[str, Any]], ext_type: int, ext_data: bytes
    ) -> List[Dict[str, Any]]:
        """
        ...
        new_extensions.append({"type": ext_type, "length": len(ext_data), "data": ext_data})
    ...
    def validate_handshake(self, modified_payload: bytes) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if handshake_length != actual_handshake_length:
                return (
                    False,
                    f"Handshake length mismatch: {handshake_length} != {actual_handshake_length}",
                )
    ...
    def handle_tls_error(
        self, error: Exception, context: AttackContext, operation: str
    ) -> AttackResult:
        """
