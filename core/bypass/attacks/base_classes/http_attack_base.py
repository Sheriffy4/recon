...
class HTTPAttackBase(BaseAttack):
    ...
    def randomize_header_case(self, header_name: str, strategy: str = "random") -> str:
        ...
        if strategy == "upper":
            ...
            return "".join(
                [c.upper() if i % 2 == 0 else c.lower() for i, c in enumerate(header_name)]
            )
    ...
    def validate_http_compliance(self, payload: bytes) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if parsed["method"] in ["POST", "PUT", "PATCH"]:
                if (
                    "Content-Length" not in parsed["headers"]
                    and "Transfer-Encoding" not in parsed["headers"]
                ):
                    return (
                        False,
                        "POST/PUT/PATCH requires Content-Length or Transfer-Encoding header",
                    )
    ...
    def manipulate_http2_pseudo_headers(
        self, pseudo_headers: Dict[str, str], manipulation_type: str, **kwargs
    ) -> Dict[str, str]:
        """
            ...
            if ":method" in modified:
                ...
                if case_strategy == "upper":
                    ...
                    modified[":method"] = "".join(
                        [c.upper() if i % 2 == 0 else c.lower() for i, c in enumerate(method)]
                    )
