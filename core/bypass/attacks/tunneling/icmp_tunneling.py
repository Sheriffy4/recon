# recon/core/bypass/attacks/tunneling/icmp_tunneling.py
"""
...
class ICMPDataTunnelingAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Split payload into ICMP-sized chunks
            chunks = self._split_payload(
                payload, packet_size - 8
...
class ICMPTimestampTunnelingAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Split payload into 4-byte chunks (timestamp size)
    ...
    def _create_timestamp_packet(self, sequence: int, timestamp: int) -> bytes:
        ...
        originate_time = timestamp  # Use our data as timestamp
        receive_time = 0
        transmit_time = 0
...
class ICMPRedirectTunnelingAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Split payload into chunks that fit in ICMP redirect
            chunks = self._split_payload(
                payload, 28
            )  # Space for original IP header + 8 bytes
    ...
    def _create_redirect_packet(
        ...
        # ICMP redirect includes gateway address + original IP header
