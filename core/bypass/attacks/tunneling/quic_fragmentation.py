...
class QUICFragmentationAttack(BaseAttack):
    ...
    def _create_simple_tls_client_hello(self, domain: str) -> bytes:
        ...
        sni_ext = b'\x00\x00'
        sni_content = struct.pack('!H', len(server_name) + 5) + struct.pack('!H', len(server_name) + 3) + b'\x00' + struct.pack('!H', len(server_name)) + server_name
        sni_ext += struct.pack('!H', len(sni_content)) + sni_content
        extensions += sni_ext
        supported_versions_ext = b'\x00+'
        supported_versions_content = b'\x02\x03\x04'
        supported_versions_ext += struct.pack('!H', len(supported_versions_content)) + supported_versions_content
        extensions += supported_versions_ext
        quic_params_ext = b'\x009'
        quic_params = b'\x01\x04' + struct.pack('!I', 1048576) + b'\x04\x04' + struct.pack('!I', 1048576) + b'\x08\x02' + struct.pack('!H', 100)
        ...
        client_hello_body = tls_version + client_random + session_id_len + session_id + cipher_suites_len + cipher_suites + compression_methods + extensions_len + extensions
    ...
    def _create_quic_initial_packet(self, domain: str, payload_data: Optional[bytes]=None) -> bytes:
        ...
        if payload_data:
            ...
            stream_frame = stream_frame_type + stream_id + stream_offset + stream_length + payload_data
            frames += stream_frame
        min_packet_size = 1200
        current_size = 1 + 4 + 1 + dcid_len + 1 + scid_len + len(token_length) + 2 + 4 + len(frames) + 16
        ...
        header_flags |= 3
        packet = bytes([header_flags]) + version + bytes([dcid_len]) + dcid + bytes([scid_len]) + scid + token_length + length_field + packet_number + frames
