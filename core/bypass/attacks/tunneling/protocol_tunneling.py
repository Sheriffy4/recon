...
class HTTPTunnelingAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'tunnel_method': tunnel_method, 'tunnel_path': tunnel_path, 'encoding': encoding, 'original_size': len(payload), 'tunneled_size': len(http_request), 'segments': segments if context.engine_type != 'local' else None})
...
class WebSocketTunnelingAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'host_header': host_header, 'path': path, 'ws_key': ws_key, 'original_size': len(payload), 'tunneled_size': len(combined_payload), 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
...
class SSHTunnelingAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            bytes_sent = len(combined_payload)
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'ssh_version': ssh_version, 'original_size': len(payload), 'tunneled_size': len(combined_payload), 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
    ...
    def _create_encrypted_payload(self, payload: bytes) -> bytes:
        ...
        return struct.pack('!I', packet_length) + bytes([padding_length]) + bytes(encrypted) + padding
...
class VPNTunnelingAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            latency = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency, packets_sent=packets_sent, bytes_sent=bytes_sent, connection_established=True, data_transmitted=True, metadata={'vpn_type': vpn_type, 'original_size': len(payload), 'tunneled_size': len(tunneled_payload), 'segments': segments if context.engine_type != 'local' else None})
        except Exception as e:
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000)
    ...
    def _create_ipsec_packet(self, payload: bytes) -> bytes:
        ...
        next_header = b'\\x04'
        encrypted_data = bytes((b ^ 85 for b in payload + padding + bytes([pad_length]) + next_header))
