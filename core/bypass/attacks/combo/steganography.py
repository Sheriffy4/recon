...
class IPIDSteganographyAttack(BaseAttack):
    ...
    def _create_ip_packet_with_id_stego(
        ...
        tcp_header = struct.pack(
            ">HHIIBBHHH", 80, 8080, sequence, 0, 80, 24, 65535, 0, 0
        )
...
class CombinedFieldSteganographyAttack(BaseAttack):
    ...
    def _create_combined_stego_packet(
        ...
        ip_header = struct.pack(
            ...
            6,
            0,
            struct.pack("!I", 3232235521),
            struct.pack("!I", 3232235522),
        )
        checksum = self._calculate_ip_checksum(ip_header)
        ...
        tcp_header = struct.pack(
            ">HHIIBBHHH", 80, 8080, tcp_seq, 0, 128, 24, 65535, 0, 0
        )
        ts_val = base_timestamp
        ts_ecr = base_timestamp - 1000
...
class TimingChannelSteganographyAttack(BaseAttack):
    ...
    async def _encode_payload_in_timing(
        ...
        if method == "binary":
            ...
            segments = await self._encode_interval_timing(
                payload, base_delay, bit_delay
            )
        else:
            segments = await self._encode_binary_timing(payload, base_delay, bit_delay)
        return segments
...
class CovertChannelComboAttack(BaseAttack):
    ...
    async def _create_covert_channel_packets(
    ...
    def _create_protocol_field_packets(self, payload: bytes) -> List[tuple]:
        ...
        for i in range(0, len(payload), 4):
            ...
            tcp_header = struct.pack(
                ">HHIIBBHHH", 80, 8080, seq_num, 0, 80, 24, 65535, 0, 0
            )
    ...
    def _create_ip_packet_with_embedded_data(
        ...
        for field in fields:
            ...
            if field == "id" and data_offset + 2 <= len(data):
                ...
                fragment_offset = (
                    struct.unpack(">H", data[data_offset : data_offset + 2])[0] & 8191
                )
                data_offset += 2
...
class AdvancedTimingChannelSteganographyAttack(BaseAttack):
    ...
    async def _encode_payload_with_advanced_timing(
        ...
        if method == "binary":
            return await self._encode_advanced_binary_timing(
                payload, base_delay, bit_delay, precision
            )
        elif method == "differential":
            return await self._encode_differential_timing(
                payload, base_delay, bit_delay
            )
