...
class NativeComboEngine:
    ...
    def __init__(self):
        ...
        LOG.info(f'Native combo engine initialized with {len(self.attack_registry)} attacks and {len(self.combo_rules)} combo rules')

    def register_attack(self, name: str, attack_class: type, **default_params):
        """Register an attack for use in combinations."""
        self.attack_registry[name] = {'class': attack_class, 'default_params': default_params, 'usage_count': 0, 'success_count': 0}
    ...
    async def execute_combo(self, combo_name: str, context: AttackContext, custom_params: Optional[Dict[str, Any]]=None) -> ComboResult:
        """
        ...
        LOG.info(f"Combo '{combo_name}' completed: {result.successful_attacks}/{result.total_attacks} successful in {execution_time:.3f}s")
        return result

    async def _execute_sequential(self, rule: ComboRule, context: AttackContext, params: Dict[str, Any]) -> ComboResult:
        ...
        for i, attack_name in enumerate(rule.attacks):
            ...
            if attack_name in rule.required_attacks and (not attack_result.success):
                ...
                break
        return ComboResult(success=successful >= len(rule.attacks) * rule.min_success_rate, total_attacks=len(results), successful_attacks=successful, failed_attacks=len(results) - successful, execution_time_ms=0, attack_results=results)

    async def _execute_parallel(self, rule: ComboRule, context: AttackContext, params: Dict[str, Any]) -> ComboResult:
        ...
        if rule.timing == ComboTiming.SYNCHRONIZED:
            ...
            for i, (attack_name, result) in enumerate(zip([name for name, _ in tasks], task_results)):
                if isinstance(result, Exception):
                    LOG.error(f'Attack {attack_name} failed with exception: {result}')
                    attack_result = AttackResult(success=False, status=AttackStatus.ERROR, technique_used=attack_name, error_message=str(result))
                else:
            ...
            for i, (attack_name, task) in enumerate(tasks):
                ...
                try:
                    ...
                    results.append(AttackResult(success=False, status=AttackStatus.ERROR, technique_used=attack_name, error_message=str(e)))
        return ComboResult(success=successful >= len(rule.attacks) * rule.min_success_rate, total_attacks=len(results), successful_attacks=successful, failed_attacks=len(results) - successful, execution_time_ms=0, attack_results=results)

    async def _execute_conditional(self, rule: ComboRule, context: AttackContext, params: Dict[str, Any]) -> ComboResult:
        ...
        for attack_name in rule.attacks:
            ...
            if attack_result.success:
                ...
                if params.get('stop_on_first_success', False):
                    ...
                    break
        return ComboResult(success=successful > 0, total_attacks=len(results), successful_attacks=successful, failed_attacks=len(results) - successful, execution_time_ms=0, attack_results=results)

    async def _execute_layered(self, rule: ComboRule, context: AttackContext, params: Dict[str, Any]) -> ComboResult:
    ...
    async def _execute_adaptive(self, rule: ComboRule, context: AttackContext, params: Dict[str, Any]) -> ComboResult:
        ...
        for attempt in range(rule.max_adaptations + 1):
            ...
            for attack_name in rule.attacks:
                ...
                attack_result = await self._execute_single_attack(attack_name, context, adapted_params)
            ...
            if attempt < rule.max_adaptations:
                adaptations += 1
                LOG.info(f'Adapting combo strategy (attempt {attempt + 1}/{rule.max_adaptations + 1})')
                await asyncio.sleep(0.1)
        return ComboResult(success=successful >= len(rule.attacks) * rule.min_success_rate, total_attacks=len(results), successful_attacks=successful, failed_attacks=len(results) - successful, execution_time_ms=0, attack_results=results, adaptations_made=adaptations)

    async def _execute_single_attack(self, attack_name: str, context: AttackContext, params: Dict[str, Any]) -> AttackResult:
        """Execute a single attack."""
        if attack_name not in self.attack_registry:
            LOG.error(f'Unknown attack: {attack_name}')
            return AttackResult(success=False, status=AttackStatus.ERROR, technique_used=attack_name, error_message=f'Unknown attack: {attack_name}')
        ...
        try:
            ...
            return AttackResult(success=False, status=AttackStatus.ERROR, technique_used=attack_name, error_message=str(e))
    ...
    def _check_conditions(self, rule: ComboRule, context: AttackContext, previous_results: List[AttackResult]) -> bool:
    ...
    def _modify_context_for_layer(self, context: AttackContext, result: AttackResult) -> AttackContext:
        """Modify context based on layer result."""
        new_context = AttackContext(target_host=context.target_host, target_port=context.target_port, source_ip=context.source_ip, source_port=context.source_port, payload=context.payload)
        if hasattr(result, 'modified_packets') and result.modified_packets:
            new_context.payload = result.modified_packets[-1]
        return new_context

    def _adapt_parameters(self, base_params: Dict[str, Any], previous_results: List[AttackResult], attack_name: str) -> Dict[str, Any]:
        ...
        if failed_count > 0:
            if attack_name in adapted_params:
                ...
                if 'delay_ms' in attack_params:
                    attack_params['delay_ms'] = max(1, attack_params['delay_ms'] - failed_count * 10)
    ...
    def _update_stats(self, combo_name: str, result: ComboResult):
        ...
        if combo_name not in self.execution_stats:
            self.execution_stats[combo_name] = {'executions': 0, 'successes': 0, 'total_attacks': 0, 'successful_attacks': 0, 'avg_execution_time_ms': 0.0}
        ...
        new_avg = (old_avg * (stats['executions'] - 1) + result.execution_time_ms) / stats['executions']
    ...
    def _load_builtin_attacks(self):
        ...
        attack_modules = ['tcp.fooling', 'tcp.segmentation', 'tcp.manipulation', 'tcp.timing', 'tls.record_manipulation', 'tls.extension_attacks', 'tls.confusion', 'http.header_attacks', 'http.method_attacks', 'ip.fragmentation', 'ip.header_manipulation', 'payload.obfuscation', 'payload.encryption', 'payload.noise']
    ...
    def _load_builtin_combo_rules(self):
        ...
        zapret_rule = ComboRule(name='zapret_combo', attacks=['zapret'], mode=ComboMode.SEQUENTIAL, timing=ComboTiming.IMMEDIATE, parameters={'zapret': {'split_seqovl': 297, 'base_ttl': 51, 'repeats': 10, 'auto_ttl': True}})
        self.register_combo_rule(zapret_rule)
        tcp_combo = ComboRule(name='tcp_multi_layer', attacks=['tcp_segmentation', 'tcp_fooling', 'tcp_manipulation'], mode=ComboMode.LAYERED, timing=ComboTiming.STAGGERED, stagger_ms=50, min_success_rate=0.6)
        self.register_combo_rule(tcp_combo)
        tls_adaptive = ComboRule(name='tls_adaptive', attacks=['tls_record_manipulation', 'tls_extension_attacks'], mode=ComboMode.ADAPTIVE, timing=ComboTiming.BURST, adapt_on_failure=True, max_adaptations=3)
        self.register_combo_rule(tls_adaptive)
        payload_parallel = ComboRule(name='payload_parallel', attacks=['payload_obfuscation', 'payload_encryption', 'payload_noise'], mode=ComboMode.PARALLEL, timing=ComboTiming.SYNCHRONIZED, min_success_rate=0.3)
        self.register_combo_rule(payload_parallel)

    def get_statistics(self) -> Dict[str, Any]:
        """Get engine statistics."""
        return {'registered_attacks': len(self.attack_registry), 'registered_combos': len(self.combo_rules), 'execution_stats': self.execution_stats, 'attack_usage': {name: {'usage_count': info['usage_count'], 'success_count': info['success_count'], 'success_rate': info['success_count'] / info['usage_count'] if info['usage_count'] > 0 else 0.0} for name, info in self.attack_registry.items()}}
