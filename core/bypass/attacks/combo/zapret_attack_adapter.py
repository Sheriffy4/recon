...
from core.bypass.attacks.combo.zapret_integration import ZapretIntegration, get_zapret_integration
...
class ZapretAttackAdapter(BaseAttack):
    ...
    def __init__(self, config: Optional[ZapretAdapterConfig] = None):
        ...
        LOG.info(f"Zapret Attack Adapter initialized: mode={self.config.mode.value}, "
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            if self.config.mode in [ZapretAdapterMode.PRESET, ZapretAdapterMode.INTEGRATION, ZapretAdapterMode.AUTO]:
    ...
    async def _async_execute(self, context: AttackContext) -> AttackResult:
        ...
        Returns:
            AttackResult with execution results
        """
        LOG.info(f"Executing Zapret adapter for {context.dst_ip}:{context.dst_port} "
        ...
        try:
            ...
            LOG.info(f"Zapret adapter execution completed: status={result.status.value}, "
    ...
    async def _execute_with_mode(self, mode: ZapretAdapterMode, context: AttackContext) -> AttackResult:
    ...
    async def _execute_direct_mode(self, context: AttackContext) -> AttackResult:
        ...
        if self.config.enable_network_validation and hasattr(self.direct_strategy, 'execute_with_network_validation'):
            return await self.direct_strategy.execute_with_network_validation(context, strict_mode=False)
        else:
            return await self.direct_strategy.execute(context)

    def _post_process_result(self, result: AttackResult, mode: ZapretAdapterMode, start_time: float) -> AttackResult:
        ...
        class ZapretIntegrationContext:
            def __init__(self, original_context):
                ...
                for attr_name in dir(original_context):
                    if not attr_name.startswith('_') and not callable(getattr(original_context, attr_name)):
                        try:
    ...
    def get_configuration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'zapret_config': asdict(self.config.zapret_config) if self.config.zapret_config else None,
...
def create_zapret_adapter_with_config(zapret_config: Union[ZapretConfig, Dict[str, Any]], **kwargs) -> ZapretAttackAdapter:
