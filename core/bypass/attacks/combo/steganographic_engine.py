...
class SteganographicConfig:
    ...
    header_fields: List[str] = field(default_factory=lambda: ["user_agent", "accept", "cache_control"])
...
class SteganographicEngine(ABC):
    ...
    @abstractmethod
    def embed_data(self, packet_data: bytes, data_to_embed: bytes, context: Dict[str, Any]) -> Tuple[bytes, int]:
        """
    ...
    def update_stats(self, success: bool, bytes_embedded: int, efficiency: float):
        ...
        if total_successful > 0:
            ...
            self._embedding_stats['average_efficiency'] = (
                (current_avg * (total_successful - 1) + efficiency) / total_successful
            )
        ...
        try:
            ...
            for header_idx in header_indices:
                ...
                if ':' in line:
                    ...
                    embedded_value = self._embed_in_header_value(value.strip(), data_to_embed[data_index])
...
class MultiLayerSteganographicEngine(SteganographicEngine):
    ...
    def __init__(self, config: SteganographicConfig):
        ...
        self.engines = {
            ...
            SteganographicMethod.HEADER_MODIFICATION: HeaderModificationSteganographicEngine(config)
        }
    ...
    def can_embed_in_packet(self, packet_data: bytes, context: Dict[str, Any]) -> bool:
        ...
        return any(engine.can_embed_in_packet(packet_data, context) 
        ...
        try:
            ...
            for method in self.embedding_order:
                ...
                if engine.can_embed_in_packet(modified_packet, context):
                    modified_packet, embedded = engine.embed_data(modified_packet, remaining_data, context)
...
class SteganographicManager:
    ...
    def __init__(self, config: Optional[SteganographicConfig] = None):
        ...
        self.engines = {
            ...
            SteganographicMethod.HEADER_MODIFICATION: HeaderModificationSteganographicEngine(self.config),
