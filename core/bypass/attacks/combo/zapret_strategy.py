...
class ZapretStrategy(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            for _ in range(self.config.repeats):
                packet = self.packet_builder.create_tcp_packet(src_ip=context.src_ip or '127.0.0.1', dst_ip=context.dst_ip, src_port=context.src_port or random.randint(1024, 65535), dst_port=context.dst_port, seq=context.seq or random.randint(1, 10000), ack=context.ack or 0, flags='PA', payload=b'FAKE_ZAPRET_DATA')
                if packet is None:
                    continue
    ...
    async def execute_with_network_validation(self, context: AttackContext, strict_mode: bool=False) -> AttackResult:
        ...
        """
        LOG.info(f'Executing zapret strategy with network validation for {context.dst_ip}:{context.dst_port}')
        start_time = time.time()
        try:
            ...
            try:
                ...
                LOG.info(f'Baseline test for {context.domain}: success={baseline.success}, latency={baseline.latency_ms:.1f}ms')
                bypass = await tester.test_with_bypass(context.domain or context.dst_ip, context.dst_port, basic_result)
                LOG.info(f'Bypass test for {context.domain}: success={bypass.success}, latency={bypass.latency_ms:.1f}ms')
                effectiveness = await tester.compare_results(baseline, bypass)
                basic_result.metadata['bypass_results'] = {'baseline_success': baseline.success, 'bypass_success': bypass.success, 'bypass_effective': effectiveness.bypass_effective, 'improvement_type': effectiveness.improvement_type, 'effectiveness_score': effectiveness.effectiveness_score}
                ...
                if not effectiveness.bypass_effective and strict_mode:
                    ...
                    basic_result.error_message = 'Zapret bypass was not effective against detected blocking'
                    LOG.warning(f'Zapret bypass ineffective for {context.domain}: {effectiveness.improvement_type}')
                elif not effectiveness.bypass_effective:
                    basic_result.status = AttackStatus.SUCCESS if bypass.success else AttackStatus.BLOCKED
                    if not bypass.success:
                        basic_result.error_message = f'Domain remains blocked after zapret bypass: {baseline.block_type}'
                        LOG.warning(f'Zapret bypass failed for {context.domain}: domain still blocked')
                else:
                    basic_result.status = AttackStatus.SUCCESS
                    LOG.info(f'Zapret bypass effective for {context.domain}: {effectiveness.improvement_type}')
            finally:
                if hasattr(tester, 'close'):
                    await tester.close()
            execution_time = time.time() - start_time
            basic_result.latency_ms = execution_time * 1000
            LOG.info(f'Zapret strategy with validation completed in {execution_time:.3f}s: status={basic_result.status.value}, effective={effectiveness.bypass_effective}')
            return basic_result
        except Exception as e:
            LOG.error(f'Zapret strategy with validation failed: {e}')
            return AttackResult(status=AttackStatus.ERROR, error_message=str(e), latency_ms=(time.time() - start_time) * 1000, technique_used='zapret_strategy')

    async def _generate_fake_packets(self, context: AttackContext) -> List[bytes]:
        """Generate fake packets to confuse DPI."""
        fake_packets = []
        for i in range(3):
            ttl = self._calculate_optimal_ttl(context) if self.config.auto_ttl else self.config.base_ttl
            fake_payload = self._create_fake_tls_payload()
            fake_packet = self.packet_builder.build_tcp_packet(src_ip=context.src_ip or '192.168.1.100', dst_ip=context.dst_ip, src_port=context.src_port or random.randint(10000, 65000), dst_port=context.dst_port, seq=random.randint(1000000, 9999999), ack=0, flags='S', payload=fake_payload, ttl=ttl, ip_id=random.randint(1, 65535))
    ...
    async def _apply_sequence_overlap_split(self, context: AttackContext) -> List[bytes]:
        ...
        if len(main_payload) <= self.config.split_seqovl:
            packet = self.packet_builder.build_tcp_packet(src_ip=context.src_ip or '192.168.1.100', dst_ip=context.dst_ip, src_port=context.src_port or random.randint(10000, 65000), dst_port=context.dst_port, seq=random.randint(1000000, 9999999), ack=random.randint(1000000, 9999999), flags='PA', payload=main_payload, ttl=self.config.base_ttl)
        ...
        first_packet = self.packet_builder.build_tcp_packet(src_ip=context.src_ip or '192.168.1.100', dst_ip=context.dst_ip, src_port=context.src_port or random.randint(10000, 65000), dst_port=context.dst_port, seq=base_seq, ack=base_ack, flags='PA', payload=first_part, ttl=self.config.base_ttl)
        second_packet = self.packet_builder.build_tcp_packet(src_ip=context.src_ip or '192.168.1.100', dst_ip=context.dst_ip, src_port=context.src_port or random.randint(10000, 65000), dst_port=context.dst_port, seq=base_seq + split_pos, ack=base_ack, flags='PA', payload=second_part, ttl=self.config.base_ttl)
        split_packets.extend([first_packet, second_packet])
        LOG.debug(f'Applied sequence overlap split at position {split_pos} with {overlap_bytes} bytes overlap')
        return split_packets

    async def _apply_fake_disorder(self, context: AttackContext, existing_packets: List[bytes]) -> List[bytes]:
        ...
        for i in range(self.config.disorder_window):
            ...
            disorder_packet = self.packet_builder.build_tcp_packet(src_ip=context.src_ip or '192.168.1.100', dst_ip=context.dst_ip, src_port=context.src_port or random.randint(10000, 65000), dst_port=context.dst_port, seq=wrong_seq, ack=0, flags='R', payload=b'', ttl=1, ip_id=random.randint(1, 65535))
    ...
    async def _apply_repeats(self, packets: List[bytes]) -> List[bytes]:
        ...
        LOG.debug(f'Repeated pattern {self.config.repeats} times, total packets: {len(repeated_packets)}')
    ...
    def _create_fake_tls_payload(self) -> bytes:
        ...
        fake_tls = b'\x16\x03\x01\x00 \x01\x00\x00\x1c\x03\x03' + b'\x00' * 32 + b'\x00' + b'\x00\x02\x00/' + b'\x01\x00'
        return fake_tls
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {'packets_sent': self.packets_sent, 'fake_packets_sent': self.fake_packets_sent, 'disorder_packets_sent': self.disorder_packets_sent, 'config': {'desync_methods': self.config.desync_methods, 'split_seqovl': self.config.split_seqovl, 'ttl': self.config.base_ttl, 'repeats': self.config.repeats, 'fooling': self.config.fooling_method}}

def create_zapret_strategy(split_seqovl: int=297, ttl: int=51, repeats: int=10, auto_ttl: bool=True, **kwargs) -> ZapretStrategy:
