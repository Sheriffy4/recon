...
class SimpleAttackExecutor:
    ...
    def execute_strategy(self, strategy: Dict[str, Any]) -> AttackResult:
        ...
        try:
            ...
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency_ms, packets_sent=1, bytes_sent=len(strategy.get('payload', b'')), connection_established=True, data_transmitted=True, technique_used=attack_name, metadata={'strategy': attack_name, 'params': params, 'execution_method': 'simple_executor'})
        except Exception as e:
            self.logger.error(f'Strategy execution failed: {e}')
            return AttackResult(status=AttackStatus.ERROR, error_message=f'Strategy execution failed: {str(e)}', latency_ms=(time.time() - start_time) * 1000, technique_used=strategy.get('name', 'unknown'))
    ...
    def execute_attack(self, attack_type: str, context: 'AttackContext') -> AttackResult:
        ...
        try:
            ...
            latency_ms = (time.time() - start_time) * 1000
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=latency_ms, packets_sent=1, bytes_sent=len(context.payload) if hasattr(context, 'payload') else 0, connection_established=True, data_transmitted=True, technique_used=attack_type, metadata={'attack_type': attack_type, 'execution_method': 'simple_executor', 'segments': [(context.payload, 0, {})] if hasattr(context, 'payload') else []})
        except Exception as e:
            self.logger.error(f'Attack execution failed: {e}')
            return AttackResult(status=AttackStatus.ERROR, error_message=f'Attack execution failed: {str(e)}', latency_ms=(time.time() - start_time) * 1000, technique_used=attack_type)
