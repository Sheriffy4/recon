# recon/core/bypass/attacks/network_tester.py
"""
...
class NetworkTester:
            ...
            while True:
                try:
                    ...
                    if not data:
                        break
                    response += data
                    if len(response) > 10240:  # Limit response size
                        break
                except socket.timeout:
                    break
    ...
    def validate_attack_result(
        ...
        if attack_result.status != AttackStatus.SUCCESS:
            # If attack already failed, no need to test network
            return attack_result

        # Determine test type based on port and context
        ...
        if context.dst_port in [80, 443, 8080, 8443] and context.domain:
            # Use comparative bypass testing
            try:
                ...
                if "error" in bypass_results:
                    # Fallback to simple domain testing
                    ...
                    if bypass_effective:
                        ...
                        # Bypass was not effective, but in non-strict mode don't fail the attack
                        if not strict_mode:
                            attack_result.connection_established = baseline_data.get(
                                "success", False
                            )
                            attack_result.latency_ms = baseline_data.get(
                                "latency_ms", 0
                            )
                            if not isinstance(attack_result.metadata, dict):
                                attack_result.metadata = {}
            ...
            if success:
                ...
                # In non-strict mode, network failures don't necessarily mean attack failure
                if not strict_mode:
                    attack_result.connection_established = False
                    attack_result.latency_ms = latency_ms
                    attack_result.metadata["network_test"] = "tcp_failed_but_attack_ok"
                    attack_result.metadata["network_error"] = error_msg
                    # Don't change the status - keep it as SUCCESS
                else:
                    attack_result.status = AttackStatus.ERROR
                    attack_result.error_message = (
