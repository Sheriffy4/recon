...
class NetworkTester:
    ...
    def test_tcp_connection(self, context: AttackContext) -> Tuple[bool, float, Optional[str]]:
        """
        ...
        try:
            ...
            if result == 0:
                ...
                sock.close()
                return (False, latency_ms, f'Connection failed with error code {result}')
        except socket.timeout:
            return (False, self.timeout * 1000, 'Connection timeout')
        except socket.gaierror as e:
            return (False, (time.time() - start_time) * 1000, f'DNS resolution failed: {e}')
        except Exception as e:
            return (False, (time.time() - start_time) * 1000, f'Connection error: {e}')

    def test_http_request(self, context: AttackContext, use_ssl: bool=False) -> Tuple[bool, float, Optional[str], Optional[bytes]]:
        """
        ...
        try:
            ...
            return (False, (time.time() - start_time) * 1000, f'Request error: {e}', None)

    def validate_attack_result(self, context: AttackContext, attack_result: AttackResult, strict_mode: bool=False) -> AttackResult:
        """
        ...
        use_ssl = context.dst_port in [443, 8443] or (context.domain and context.domain.startswith('https://'))
        if context.dst_port in [80, 443, 8080, 8443] and context.domain:
            try:
                bypass_results = test_bypass_effectiveness_sync(context, attack_result, timeout=self.timeout)
                if 'error' in bypass_results:
                    success, latency_ms, error_msg = test_domain_accessibility_sync(context.domain, timeout=self.timeout)
                    if success:
                        ...
                        attack_result.metadata['network_test'] = 'domain_success_fallback'
                        ...
                        if not isinstance(attack_result.metadata, dict):
                            attack_result.metadata = {}
                        attack_result.metadata['network_test'] = 'domain_failed_but_attack_ok'
                        attack_result.metadata['network_error'] = error_msg
                        attack_result.metadata['domain_tested'] = context.domain
                    else:
                        attack_result.status = AttackStatus.ERROR
                        attack_result.error_message = error_msg or 'Domain accessibility test failed'
                        attack_result.connection_established = False
                    ...
                    if bypass_effective:
                        ...
                        attack_result.connection_established = baseline_data.get('success', False)
                        attack_result.latency_ms = baseline_data.get('latency_ms', 0)
                        if not isinstance(attack_result.metadata, dict):
                            attack_result.metadata = {}
                        attack_result.metadata['network_test'] = 'bypass_not_effective_but_attack_ok'
                        attack_result.metadata['bypass_results'] = bypass_results
                        attack_result.metadata['domain_tested'] = context.domain
                    else:
                        attack_result.status = AttackStatus.ERROR
                        attack_result.error_message = 'Bypass strategy was not effective'
                        attack_result.connection_established = False
                        attack_result.data_transmitted = False
                        attack_result.latency_ms = bypass_data.get('latency_ms', 0)
                        if not isinstance(attack_result.metadata, dict):
                ...
                success, latency_ms, error_msg, response = self.test_http_request(context, use_ssl)
                if success:
                    attack_result.connection_established = True
                    attack_result.data_transmitted = True
                    attack_result.response_received = True
                    ...
                    attack_result.metadata['response_size'] = len(response) if response else 0
                elif not strict_mode:
                    attack_result.connection_established = False
                    attack_result.latency_ms = latency_ms
                    attack_result.metadata['network_test'] = 'http_fallback_failed_but_attack_ok'
                    attack_result.metadata['network_error'] = error_msg
                else:
                    attack_result.status = AttackStatus.ERROR
                    attack_result.error_message = error_msg or 'HTTP fallback test failed'
                    attack_result.connection_established = False
                    attack_result.data_transmitted = False
                    attack_result.latency_ms = latency_ms
            ...
            if success:
                ...
                attack_result.status = AttackStatus.ERROR
                attack_result.error_message = error_msg or 'TCP connectivity test failed'
                attack_result.connection_established = False
                attack_result.latency_ms = latency_ms
                attack_result.metadata['network_test'] = 'tcp_failed'
        return attack_result

    def quick_connectivity_check(self, ip: str, port: int, timeout: float=2.0) -> bool:
        """
