...
class FakedDisorderAttack(BaseAttack):
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            segments = await self._create_segments(
                fake_payload, part1, part2, split_byte_pos
            )
            result = AttackResult(
            ...
            self.logger.info(
                f"FakedDisorderAttack created {len(segments)} segments: fake({len(fake_payload)}b) -> part2({len(part2)}b) -> part1({len(part1)}b)"
            )
            return result
    ...
    async def _create_segments(
        ...
        fake_options = {
            ...
            "flags": self.config.fake_tcp_flags if self.config.fake_tcp_flags else 24,
        }
