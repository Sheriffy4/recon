...
from core.bypass.attacks.safe_result_utils import (
...
class WindowScalingAttack(BaseAttack):
    ...
    def __init__(
        self, name: str = "window_scaling", config: Optional[WindowScalingConfig] = None
    ):
        super().__init__(name)
    ...
    def _validate_config(self):
        ...
        if not 2 <= self.config.segment_count <= 15:
            raise ValueError(
                f"segment_count must be between 2 and 15, got {self.config.segment_count}"
            )
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            self.logger.info(
                f"Executing WindowScalingAttack on {context.connection_id}"
            )
            ...
            self.logger.info(
                f"WindowScalingAttack created {len(segments)} segments with {self.config.window_pattern.value} pattern"
            )
            return result
    ...
    async def _create_window_segments(
        self, payload: bytes
    ...
    def _generate_window_pattern(self) -> List[int]:
        ...
        for i in range(self.config.segment_count):
            if (
                self.config.use_zero_window
                and random.random() < self.config.zero_window_probability
            ):
                window_size = 0
    ...
    def _create_segment_options(
        self, segment_index: int, window_size: int
    ) -> Dict[str, Any]:
    ...
    def estimate_effectiveness(self, context: AttackContext) -> float:
        ...
        if self.config.add_window_scaling:
            effectiveness += 0.05
        if self.config.window_pattern in [
            WindowPattern.OSCILLATING,
            WindowPattern.EXTREME_VALUES,
        ]:
            effectiveness += 0.1
        if self.config.vary_tcp_flags:
            effectiveness += 0.05
