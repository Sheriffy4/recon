...
class WindowScalingAttack(BaseAttack):
    ...
    def _validate_config(self):
        ...
        if self.config.min_window_size < 0 or self.config.min_window_size > 65535:
            raise ValueError('min_window_size must be between 0 and 65535')
        if self.config.max_window_size < 0 or self.config.max_window_size > 65535:
            raise ValueError('max_window_size must be between 0 and 65535')
        if self.config.min_window_size > self.config.max_window_size:
            raise ValueError('min_window_size must be <= max_window_size')
        if not 0.0 <= self.config.zero_window_probability <= 1.0:
            raise ValueError('zero_window_probability must be between 0.0 and 1.0')
        if not 0 <= self.config.window_scale_factor <= 14:
            raise ValueError('window_scale_factor must be between 0 and 14')
    ...
    def _create_segment_options(self, segment_index: int, window_size: int) -> Dict[str, Any]:
        ...
        options = {'delay_ms': self.config.base_delay_ms, 'window_size': window_size, 'ttl': 64}
        ...
        if self.config.add_window_scaling and segment_index == 0:
            options['tcp_options'] = {'window_scale': self.config.window_scale_factor}
    ...
    def get_attack_info(self) -> Dict[str, Any]:
        ...
        return {'name': self.name, 'type': 'window_scaling', 'description': 'Manipulates TCP window sizes and scaling to confuse window-based DPI analysis', 'technique': 'tcp_window_manipulation', 'effectiveness': 'medium_against_window_analysis_dpi', 'config': {'window_pattern': self.config.window_pattern.value, 'segment_count': self.config.segment_count, 'min_window_size': self.config.min_window_size, 'max_window_size': self.config.max_window_size, 'use_zero_window': self.config.use_zero_window, 'zero_window_probability': self.config.zero_window_probability, 'use_extreme_values': self.config.use_extreme_values, 'add_window_scaling': self.config.add_window_scaling, 'window_scale_factor': self.config.window_scale_factor}, 'window_patterns': [pattern.value for pattern in WindowPattern], 'advantages': ['Confuses window-based DPI analysis', 'Multiple window scaling patterns', 'Zero window advertisement support', 'Extreme value generation', 'TCP window scaling option support']}
    ...
    def get_required_capabilities(self) -> List[str]:
        ...
        capabilities = ['packet_construction', 'window_size_modification', 'timing_control', 'sequence_manipulation']
        if self.config.vary_tcp_flags:
...
def create_window_scaling_attack(name: str='window_scaling', window_pattern: WindowPattern=WindowPattern.RANDOM, segment_count: int=4, min_window_size: int=0, max_window_size: int=65535, use_zero_window: bool=True, zero_window_probability: float=0.2, use_extreme_values: bool=True, add_window_scaling: bool=True, window_scale_factor: int=7, vary_tcp_flags: bool=True, vary_ttl: bool=False) -> WindowScalingAttack:
    """Factory function to create WindowScalingAttack."""
    config = WindowScalingConfig(window_pattern=window_pattern, segment_count=segment_count, min_window_size=min_window_size, max_window_size=max_window_size, use_zero_window=use_zero_window, zero_window_probability=zero_window_probability, use_extreme_values=use_extreme_values, add_window_scaling=add_window_scaling, window_scale_factor=window_scale_factor, vary_tcp_flags=vary_tcp_flags, vary_ttl=vary_ttl)
...
def create_zero_window_attack() -> WindowScalingAttack:
    ...
    return create_window_scaling_attack(name='zero_window_attack', window_pattern=WindowPattern.ZERO_WINDOW, segment_count=5, use_zero_window=True, zero_window_probability=0.6, vary_tcp_flags=True)

def create_oscillating_window_attack() -> WindowScalingAttack:
    """Create oscillating window variant."""
    return create_window_scaling_attack(name='oscillating_window_attack', window_pattern=WindowPattern.OSCILLATING, segment_count=8, min_window_size=1024, max_window_size=32768, add_window_scaling=True, vary_tcp_flags=True)

def create_extreme_window_attack() -> WindowScalingAttack:
    """Create extreme window values variant."""
    return create_window_scaling_attack(name='extreme_window_attack', window_pattern=WindowPattern.EXTREME_VALUES, segment_count=6, use_extreme_values=True, use_zero_window=True, zero_window_probability=0.4, vary_tcp_flags=True, vary_ttl=True)