...
class MultisplitConfig:
    ...
    # Number of segments to create (2-20)
    split_count: int = 5
    ...
    # Overlap between segments in bytes (0 = no overlap)
    overlap_bytes: int = 0
    ...
    # Whether to randomize segment order
    randomize_order: bool = False

    # Whether to use different TTL values
    vary_ttl: bool = False

    # TTL range for variation (min, max)
    ttl_range: Tuple[int, int] = (60, 64)

    # Whether to vary TCP flags
    vary_tcp_flags: bool = False

    # Whether to vary window sizes
    vary_window_size: bool = False

    # Window size range for variation
    window_size_range: Tuple[int, int] = (32768, 65535)

    # Whether to add padding to segments
    add_padding: bool = False

    # Padding size range (min, max bytes)
    padding_range: Tuple[int, int] = (1, 5)

    # Whether to corrupt checksums on some segments
    corrupt_some_checksums: bool = False

    # Probability of checksum corruption (0.0-1.0)
...
class MultisplitAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Check if payload is large enough for splitting
            ...
            if len(context.payload) < min_required_size:
                return AttackResult(
                    ...
                    modified_payload=None,
                    metadata={
                        "error": f"Payload too small for {self.config.split_count} segments. "
                        f"Required: {min_required_size}, got: {len(context.payload)}"
                    },
                )
    ...
    def _calculate_segment_boundaries(self, payload_len: int) -> List[Tuple[int, int]]:
        ...
        if self.config.max_segment_size > 0:
            # Use max segment size to determine boundaries
            boundaries = self._calculate_size_based_boundaries(payload_len)
        else:
            # Use split count to determine boundaries
    ...
    def _calculate_count_based_boundaries(
        ...
        for i in range(self.config.split_count):
            # Add extra byte to first 'remainder' segments
    ...
    def _calculate_size_based_boundaries(
        ...
        while current_pos < payload_len:
            # Calculate segment size with some randomization
            if self.config.delay_variation_ms > 0:
    ...
    def _add_overlap_to_boundaries(
        ...
        for i, (start_pos, end_pos) in enumerate(boundaries):
            ...
            # Add overlap to the end (except for last segment)
            if i < len(boundaries) - 1:
                new_end = min(end_pos + self.config.overlap_bytes, payload_len)

            # Add overlap to the beginning (except for first segment)
            if i > 0:
    ...
    def _create_segment_options(
        ...
        if self.config.vary_ttl:
            ...
            options["ttl"] = 64  # Default TTL

        # Set TCP flags
        if self.config.vary_tcp_flags:
            # Vary between PSH+ACK, ACK, and PSH
    ...
    def _calculate_segment_delay(
        ...
        if self.config.exponential_backoff:
            # Exponential backoff: delay increases with segment index
    ...
    def _add_padding(self, segment_data: bytes) -> bytes:
        ...
        padding = b"\\x00" * padding_size  # Null byte padding
    ...
    def estimate_effectiveness(self, context: AttackContext) -> float:
        ...
        # Higher effectiveness for larger payloads (more room for splitting)
        if context.payload:
        ...
        # Higher effectiveness with overlap
        if self.config.overlap_bytes > 0:
            effectiveness += 0.05

        # Higher effectiveness with randomization
        if self.config.randomize_order:
            effectiveness += 0.05
        ...
        # Higher effectiveness with exponential backoff
        if self.config.exponential_backoff:
            effectiveness += 0.05
