...
class UrgentPointerManipulationAttack(BaseAttack):
    ...
    def __init__(
        ...
        config: Optional[UrgentPointerConfig] = None,
    ):
        super().__init__(name)
    ...
    def _validate_config(self):
        ...
        if self.config.fake_urgent_size < 0:
            raise ValueError(
                f"fake_urgent_size must be non-negative, got {self.config.fake_urgent_size}"
            )
        for seg_idx in self.config.urgent_segments:
            if not 0 <= seg_idx < self.config.segment_count:
                raise ValueError(
                    f"urgent_segments index {seg_idx} out of range for {self.config.segment_count} segments"
                )
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            self.logger.info(
                f"Executing UrgentPointerManipulationAttack on {context.connection_id}"
            )
            ...
            self.logger.info(
                f"UrgentPointerManipulationAttack created {len(segments)} segments with urgent manipulation"
            )
            return result
    ...
    async def _create_urgent_segments(
        self, payload: bytes
    ...
    def _generate_fake_urgent_data(self) -> bytes:
        ...
        fake_data = b"URG:" + bytes(
            [random.randint(65, 90) for _ in range(self.config.fake_urgent_size - 4)]
        )
        return fake_data
            ...
            if segment_index < len(self.config.urgent_pointer_values):
                ...
                urgent_pointer = random.choice(
                    [
                        ...
                        random.randint(
                            self.config.pointer_range[0], self.config.pointer_range[1]
                        ),
                    ]
                )
            else:
