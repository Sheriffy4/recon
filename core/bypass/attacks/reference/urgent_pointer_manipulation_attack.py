...
class UrgentPointerConfig:
    ...
    urgent_pointer_values: List[int] = None
...
class UrgentPointerManipulationAttack(BaseAttack):
    ...
    def __init__(self, name: str='urgent_pointer_manipulation', config: Optional[UrgentPointerConfig]=None):
        ...
        if self.config.urgent_segments is None:
            self.config.urgent_segments = [0, 2]
        if self.config.urgent_pointer_values is None:
            self.config.urgent_pointer_values = []
    ...
    def _create_segment_options(self, segment_index: int) -> Dict[str, Any]:
        ...
        if segment_index in self.config.urgent_segments:
            ...
            elif self.config.use_invalid_pointers:
                urgent_pointer = random.choice([0, 65535, random.randint(1000, 5000), random.randint(self.config.pointer_range[0], self.config.pointer_range[1])])
            else:
    ...
    def get_attack_info(self) -> Dict[str, Any]:
        ...
        return {'name': self.name, 'type': 'urgent_pointer_manipulation', 'description': 'Manipulates TCP urgent pointer and URG flag to confuse DPI systems', 'technique': 'tcp_urgent_manipulation', 'effectiveness': 'medium_against_urgent_unaware_dpi', 'config': {'segment_count': self.config.segment_count, 'urgent_segments': self.config.urgent_segments, 'use_invalid_pointers': self.config.use_invalid_pointers, 'add_fake_urgent_data': self.config.add_fake_urgent_data, 'fake_urgent_size': self.config.fake_urgent_size, 'vary_window_size': self.config.vary_window_size}, 'advantages': ["Exploits DPI systems that don't handle urgent data properly", 'Uses invalid urgent pointer values to cause confusion', 'Adds fake urgent data for additional obfuscation', 'Varies TCP parameters for diversity']}
    ...
    def get_required_capabilities(self) -> List[str]:
        ...
        capabilities = ['packet_construction', 'tcp_flags_modification', 'urgent_pointer_modification', 'timing_control', 'sequence_manipulation']
        if self.config.vary_window_size:
...
def create_urgent_pointer_attack(name: str='urgent_pointer_manipulation', segment_count: int=3, urgent_segments: List[int]=None, use_invalid_pointers: bool=True, add_fake_urgent_data: bool=True, fake_urgent_size: int=10, vary_window_size: bool=True) -> UrgentPointerManipulationAttack:
    """Factory function to create UrgentPointerManipulationAttack."""
    config = UrgentPointerConfig(segment_count=segment_count, urgent_segments=urgent_segments or [0, segment_count - 1], use_invalid_pointers=use_invalid_pointers, add_fake_urgent_data=add_fake_urgent_data, fake_urgent_size=fake_urgent_size, vary_window_size=vary_window_size)
...
def create_aggressive_urgent_attack() -> UrgentPointerManipulationAttack:
    ...
    return create_urgent_pointer_attack(name='aggressive_urgent_attack', segment_count=5, urgent_segments=[0, 1, 3, 4], use_invalid_pointers=True, add_fake_urgent_data=True, fake_urgent_size=15, vary_window_size=True)

def create_subtle_urgent_attack() -> UrgentPointerManipulationAttack:
    """Create subtle urgent pointer variant."""
    return create_urgent_pointer_attack(name='subtle_urgent_attack', segment_count=3, urgent_segments=[1], use_invalid_pointers=False, add_fake_urgent_data=False, vary_window_size=False)