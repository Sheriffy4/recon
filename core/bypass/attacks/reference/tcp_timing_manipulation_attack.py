...
class TCPTimingManipulationAttack(BaseAttack):
    ...
    def __init__(
        ...
        config: Optional[TCPTimingConfig] = None,
    ):
        super().__init__(name)
    ...
    def _validate_config(self):
        ...
        if self.config.min_delay_ms < 0:
            raise ValueError(
                f"min_delay_ms must be non-negative, got {self.config.min_delay_ms}"
            )
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            self.logger.info(
                f"Executing TCPTimingManipulationAttack on {context.connection_id}"
            )
            ...
            self.logger.info(
                f"TCPTimingManipulationAttack created {len(segments)} segments with {self.config.timing_pattern.value} pattern"
            )
            return result
    ...
    async def _create_timing_segments(
        ...
        for i in range(self.config.segment_count):
            ...
            if (
                self.config.simulate_congestion
                and random.random() < self.config.congestion_probability
            ):
    ...
    def _calculate_delay(self, segment_index: int, total_segments: int) -> float:
        ...
        if self.config.timing_pattern == TimingPattern.LINEAR:
            ...
            return self.config.min_delay_ms + normalized * (
                self.config.max_delay_ms - self.config.min_delay_ms
            )
        else:
    ...
    def _create_segment_options(
        self, segment_index: int, delay: float
    ) -> Dict[str, Any]:
        """
