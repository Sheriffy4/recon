...
class TCPTimingManipulationAttack(BaseAttack):
    ...
    def _validate_config(self):
        ...
        if not 2 <= self.config.segment_count <= 20:
            raise ValueError('segment_count must be between 2 and 20')
    ...
    def _calculate_delay(self, segment_index: int, total_segments: int) -> float:
        ...
        if self.config.timing_pattern == TimingPattern.LINEAR:
            ...
            if burst_position == 0 and segment_index > 0:
                return self.config.burst_delay_ms
            else:
                return self.config.inter_burst_delay_ms
    ...
    def _create_segment_options(self, segment_index: int, delay: float) -> Dict[str, Any]:
        ...
        """
        options = {'delay_ms': delay, 'flags': 24}
    ...
    def get_attack_info(self) -> Dict[str, Any]:
        ...
        return {'name': self.name, 'type': 'tcp_timing_manipulation', 'description': 'Manipulates TCP timing patterns to confuse timing-based DPI analysis', 'technique': 'timing_manipulation', 'effectiveness': 'high_against_timing_analysis_dpi', 'config': {'timing_pattern': self.config.timing_pattern.value, 'base_delay_ms': self.config.base_delay_ms, 'max_delay_ms': self.config.max_delay_ms, 'min_delay_ms': self.config.min_delay_ms, 'segment_count': self.config.segment_count, 'add_jitter': self.config.add_jitter, 'jitter_ms': self.config.jitter_ms, 'vary_window_size': self.config.vary_window_size, 'vary_ttl': self.config.vary_ttl, 'simulate_congestion': self.config.simulate_congestion}, 'timing_patterns': [pattern.value for pattern in TimingPattern], 'advantages': ['Confuses timing-based DPI analysis', 'Multiple timing patterns available', 'Configurable jitter and congestion simulation', 'TCP parameter variation', 'Burst pattern support']}
...
def create_tcp_timing_attack(name: str='tcp_timing_manipulation', timing_pattern: TimingPattern=TimingPattern.RANDOM, base_delay_ms: float=10.0, max_delay_ms: float=100.0, min_delay_ms: float=1.0, segment_count: int=4, add_jitter: bool=True, jitter_ms: float=2.0, vary_window_size: bool=True, vary_ttl: bool=False, simulate_congestion: bool=False) -> TCPTimingManipulationAttack:
    """Factory function to create TCPTimingManipulationAttack."""
    config = TCPTimingConfig(timing_pattern=timing_pattern, base_delay_ms=base_delay_ms, max_delay_ms=max_delay_ms, min_delay_ms=min_delay_ms, segment_count=segment_count, add_jitter=add_jitter, jitter_ms=jitter_ms, vary_window_size=vary_window_size, vary_ttl=vary_ttl, simulate_congestion=simulate_congestion)
...
def create_burst_timing_attack() -> TCPTimingManipulationAttack:
    ...
    return create_tcp_timing_attack(name='burst_timing_attack', timing_pattern=TimingPattern.BURST, segment_count=6, add_jitter=True, vary_window_size=True, simulate_congestion=True)

def create_fibonacci_timing_attack() -> TCPTimingManipulationAttack:
    """Create Fibonacci timing variant."""
    return create_tcp_timing_attack(name='fibonacci_timing_attack', timing_pattern=TimingPattern.FIBONACCI, segment_count=8, max_delay_ms=80.0, add_jitter=True, vary_ttl=True)

def create_congestion_simulation_attack() -> TCPTimingManipulationAttack:
    """Create congestion simulation variant."""
    return create_tcp_timing_attack(name='congestion_simulation_attack', timing_pattern=TimingPattern.RANDOM, segment_count=5, simulate_congestion=True, vary_window_size=True, jitter_ms=5.0)