...
async def test_cache_prevention():
    ...
    result = await attack.execute("example.com", {'technique': 'multiple_server_validation'})
    
    if result.success:
        print(f"  ✅ Cache Prevention Success: Consistent={result.data.get('consistent', 'N/A')}")
        return True
    else:
...
def test_attack_definitions():
    ...
    try:
        ...
        if len(definitions) == 4:
            print(f"  ✅ Attack Definitions Success: {len(definitions)} definitions loaded")
