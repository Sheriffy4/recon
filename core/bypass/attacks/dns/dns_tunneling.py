...
class DNSResponseReassembler:
    ...
    def add_fragment(self, session_id: int, seq_num: int, total_fragments: int, 
                    data: bytes, encoding: str) -> Optional[bytes]:
        """
...
class DNSTunnelingAttack(DNSAttackBase):
        ...
        try:
            ...
            if encoding not in [self.ENCODING_BASE32, self.ENCODING_BASE64, self.ENCODING_HEX]:
                return AttackResult(
                    ...
                    metadata={'supported_encodings': [self.ENCODING_BASE32, self.ENCODING_BASE64, self.ENCODING_HEX]}
                )
            ...
            if len(payload) > max_data_per_response:
                ...
                logger.info(f"Payload size {len(payload)} exceeds response limit, using multi-query sequence")
            ...
            for i, fragment in enumerate(fragments):
                try:
                    ...
                    tunnel_domain = self._build_tunnel_domain(encoded, seq_id, len(fragments), base_domain)
    ...
    def validate_params(self, params: Dict[str, Any]) -> bool:
        ...
        if 'encoding' in params:
            if params['encoding'] not in [self.ENCODING_BASE32, self.ENCODING_BASE64, self.ENCODING_HEX]:
                return False
    ...
    def _fragment_data(self, data: bytes, max_query_size: int, encoding: str, base_domain: str) -> List[bytes]:
        """
    ...
    def _build_tunnel_domain(self, encoded_data: str, seq_id: int, total_count: int, base_domain: str) -> str:
        """
    ...
    def split_for_response_limit(self, data: bytes, max_response_size: int, 
                                 encoding: str) -> List[bytes]:
        """
    ...
    def create_multi_query_sequence(self, data: bytes, base_domain: str, 
                                   encoding: str, max_response_size: int) -> List[bytes]:
        """
    ...
    def _build_multi_query_domain(self, encoded_data: str, session_id: int, 
                                  seq_num: int, total_count: int, 
                                  base_domain: str) -> str:
        """
    ...
    def reassemble_response_data(self, session_id: int, seq_num: int, 
                                total_fragments: int, data: bytes, 
                                encoding: str) -> Optional[bytes]:
        """
        ...
        Returns:
            Complete reassembled data if all fragments received, None otherwise
        """
        return self._reassembler.add_fragment(
            session_id, seq_num, total_fragments, data, encoding
        )
...
class DNSTXTTunnelingAttack(DNSAttackBase):
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if encoding not in [self.ENCODING_BASE32, self.ENCODING_BASE64, self.ENCODING_HEX]:
                return AttackResult(
                    status=AttackStatus.ERROR,
                    error_message=f"Unsupported encoding: {encoding}"
                )
            ...
            if not payload:
                return AttackResult(
                    status=AttackStatus.ERROR,
                    error_message="No payload data provided"
                )
            
            # Check if we need response size splitting
            response_chunks = self.split_for_txt_response_limit(payload, max_response_size, encoding)
            
            if len(response_chunks) > 1:
                # Use multi-query sequence for large payloads
                logger.info(f"TXT payload requires {len(response_chunks)} queries for response size limits")
                session_id = self._get_next_session_id()
    ...
    def split_for_txt_response_limit(self, data: bytes, max_response_size: int, 
                                     encoding: str) -> List[bytes]:
        """
    ...
    def reassemble_txt_response_data(self, session_id: int, seq_num: int, 
                                    total_fragments: int, data: bytes, 
                                    encoding: str) -> Optional[bytes]:
        """
...
class DNSNullTunnelingAttack(DNSAttackBase):
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            if len(response_chunks) > 1:
                # Use multi-query sequence for large payloads
                logger.info(f"NULL payload requires {len(response_chunks)} queries for response size limits")
                session_id = self._get_next_session_id()
    ...
    def reassemble_null_response_data(self, session_id: int, seq_num: int, 
        ...
        """
        return self._reassembler.add_fragment(
            session_id, seq_num, total_fragments, data, 'binary'
        )
