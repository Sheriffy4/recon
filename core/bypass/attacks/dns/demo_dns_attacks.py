...
class DNSAttackDemo:
    ...
    async def demo_doh_attacks(self):
        ...
        for provider in providers:
            ...
            if result.success:
                print(f"  ✅ Success: {result.data['resolved_ip']} (took {duration:.3f}s)")
        ...
        for format_name, use_json in formats:
            ...
            if result.success:
                print(f"  ✅ {format_name} format: {result.data['resolved_ip']} (took {duration:.3f}s)")
            else:
    ...
    async def demo_dot_attacks(self):
        ...
        for query_type in query_types:
            ...
            if result.success:
                print(f"  ✅ {query_type} record: {result.data['resolved_ip']} (took {duration:.3f}s)")
            else:
    ...
    async def demo_query_manipulation(self):
        ...
        for technique, extra_params in techniques:
            ...
            if result.success:
                ...
                print(f"     Manipulation applied: {result.metadata['manipulation_applied']}")
            else:
                print(f"  ❌ Failed: {result.error}")
    ...
    async def demo_performance_comparison(self):
        ...
        attacks = {
            ...
            'Query Manipulation': (DNSQueryManipulation(), {'technique': 'case_randomization'}),
            'Cache Prevention': (DNSCachePoisoningPrevention(), {'technique': 'multiple_server_validation'})
        }
        ...
        if successful_attacks:
            ...
            print(f"  🏆 Fastest successful attack: {fastest} ({results[fastest]['duration']:.3f}s)")
            
            avg_duration = sum(results[name]['duration'] for name in successful_attacks) / len(successful_attacks)
    ...
    async def demo_attack_definitions(self):
        ...
        for definition in definitions:
            ...
            print(f"   Scores: Stability={definition.stability_score:.2f}, "
            ...
            print(f"   Supported Protocols: {', '.join(definition.supported_protocols)}")
            print(f"   Supported Ports: {', '.join(map(str, definition.supported_ports))}")
    ...
    async def demo_concurrent_attacks(self):
        ...
        # Analyze results
        successful = sum(1 for r in results if isinstance(r, type(attacks[0][0].execute("test").__await__().__next__())) and hasattr(r, 'success') and r.success)
