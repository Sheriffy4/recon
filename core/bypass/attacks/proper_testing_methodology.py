...
class ProperTestingMethodology:
    ...
    async def test_strategy_effectiveness(
        ...
        self.logger.info(
            f"Baseline test: success={baseline_result.success}, latency={baseline_result.latency_ms:.1f}ms"
        )
        if baseline_result.success:
            ...
            self.logger.info(
                f"Bypass test: success={bypass_result.success}, latency={bypass_result.latency_ms:.1f}ms"
            )
    ...
    async def _test_with_system_interceptor(
        self, domain: str, port: int, strategy: Dict[str, Any], tool_name: str
    ) -> SystemTestResult:
        """Test domain with system interceptor enabled using internal engines."""
        from core.bypass.engines.factory import (
            create_engine,
            detect_best_engine,
            EngineType,
        )
        ...
        try:
            ...
            try:
                ...
                success = block_type == BlockType.NONE
                self.logger.debug(
                    f"HTTP request through interceptor: status={status_code}, success={success}"
                )
                return SystemTestResult(
                    domain=domain,
                    success=success,
