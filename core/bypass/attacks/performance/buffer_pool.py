...
class BufferPool:
    ...
    def __init__(
        ...
        self._preallocate_buffers(small_pool_size, medium_pool_size, large_pool_size)
    ...
    def _preallocate_buffers(self, small_count: int, medium_count: int, large_count: int) -> None:
        ...
        logger.debug(f"Pre-allocated {small_count + medium_count + large_count} buffers")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            stats = {
                ...
                "total_available": (
                    len(self._small_pool) + len(self._medium_pool) + len(self._large_pool)
                ),
            }
            ...
            if total_ops > 0:
                stats["reuse_rate_percent"] = stats["total_reused"] / total_ops * 100
            else:
    ...
    def optimize_pool_sizes(self) -> None:
        ...
        with self._lock:
            ...
            # Adjust pools
            self._adjust_pool_size(self._small_pool, optimal_small, self.SMALL_BUFFER_SIZE, "small")
            self._adjust_pool_size(
                self._medium_pool, optimal_medium, self.MEDIUM_BUFFER_SIZE, "medium"
            )
            self._adjust_pool_size(self._large_pool, optimal_large, self.LARGE_BUFFER_SIZE, "large")
    ...
    def _adjust_pool_size(
        self, pool: List[bytearray], target_size: int, buffer_size: int, pool_name: str
    ) -> None:
        ...
        if current_size < target_size:
            ...
            logger.debug(f"Expanded {pool_name} pool: {current_size} -> {target_size}")
        elif current_size > target_size:
            # Remove buffers
            del pool[target_size:]
            logger.debug(f"Shrunk {pool_name} pool: {current_size} -> {target_size}")
