...
class AttackInstanceCache:
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        with self._lock:
            ...
            logger.debug(
                f"Cache hit: {key} (access_count={entry.access_count})"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._cache:
                ...
                avg_age = sum(
                    time.time() - e.creation_time for e in self._cache.values()
                ) / len(self._cache)
    ...
    def resize(self, new_max_size: int) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Cache resized: {old_size} -> {new_max_size} "
                f"(current: {len(self._cache)})"
            )
