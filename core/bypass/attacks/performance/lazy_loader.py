...
class AttackLazyLoader:
    ...
    def __init__(
        self, cache_instances: bool = True, max_cache_size: int = 100, preload_critical: bool = True
    ):
        """
    ...
    def get_cached_instance(self, attack_class: type) -> Optional[Any]:
        ...
        if class_name in self._instance_cache:
            ...
            logger.debug(f"Cache hit for {class_name} (access_count={entry.access_count})")
    ...
    def _evict_least_used(self) -> None:
        ...
        lru_key = min(
            self._instance_cache.keys(), key=lambda k: self._instance_cache[k].last_access_time
        )

        evicted = self._instance_cache.pop(lru_key)
        logger.debug(f"Evicted {lru_key} from cache (access_count={evicted.access_count})")
    ...
    def record_module_load(
        ...
        if success:
            logger.debug(
                f"Loaded {module_path} in {load_time_ms:.2f}ms " f"({attacks_found} attacks)"
            )
        else:
            logger.warning(f"Failed to load {module_path}: {error_message}")
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        """
        total_accesses = sum(entry.access_count for entry in self._instance_cache.values())
    ...
    def get_most_used_attacks(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_entries = sorted(
            self._instance_cache.items(), key=lambda x: x[1].access_count, reverse=True
        )
...
def get_lazy_loader(
    cache_instances: bool = True, max_cache_size: int = 100, preload_critical: bool = True
) -> AttackLazyLoader:
    """
