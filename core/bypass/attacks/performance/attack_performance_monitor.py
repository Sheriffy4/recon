...
class AttackPerformanceMonitor:
    ...
    def _update_stats(self, metrics: AttackExecutionMetrics) -> None:
        ...
        if len(stats.execution_times) > 0:
            ...
            stats.p95_execution_time_ms = sorted_times[p95_idx] if p95_idx < len(sorted_times) else sorted_times[-1]
            stats.p99_execution_time_ms = sorted_times[p99_idx] if p99_idx < len(sorted_times) else sorted_times[-1]
        
        # Update memory stats
        stats.total_memory_used_mb += metrics.memory_used_mb
        stats.avg_memory_used_mb = (
            stats.total_memory_used_mb / stats.total_executions
        )
