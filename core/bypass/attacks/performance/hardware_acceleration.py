...
class HardwareAccelerator:
    ...
    def __init__(self, enable_acceleration: bool = True):
        ...
        logger.info(
            f"HardwareAccelerator initialized (acceleration={self._use_hardware})"
        )
    ...
    def compare_implementations(
        self,
        hardware_op: Callable,
        software_op: Callable,
        *args,
        iterations: int = 1000,
        **kwargs
    ) -> Dict[str, Any]:
        ...
        Returns:
            Comparison results
        """
        hw_metrics = self.measure_performance(
            hardware_op, *args, iterations=iterations, **kwargs
        )
        sw_metrics = self.measure_performance(
            software_op, *args, iterations=iterations, **kwargs
        )
        
        speedup = (
            sw_metrics["avg_time_ms"] / hw_metrics["avg_time_ms"]
            if hw_metrics["avg_time_ms"] > 0 else 0
        )
...
def get_hardware_accelerator(
    ...
    if _global_accelerator is None:
        _global_accelerator = HardwareAccelerator(
