...
class AttackRegistryAdapter:
    ...
    def __init__(self, registry: Optional[AttackRegistry] = None, 
        ...
        self.logger.info(f"AttackRegistryAdapter initialized with {len(self.enhanced_metadata)} attacks")
    ...
    def update_attack_effectiveness(self, attack_name: str, success: bool, 
                                  domain: str, response_time: float = 0.0,
        ...
        if success:
            ...
            self.logger.debug(f"Updated success for {canonical_name}: {effectiveness.success_rate:.2%}")
        else:
            reason = failure_reason or "unknown"
            effectiveness.update_failure(domain, reason)
            self.logger.debug(f"Updated failure for {canonical_name}: {effectiveness.success_rate:.2%}")
    ...
    def get_parameter_recommendations(self, attack_name: str, dpi_type: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def get_category_statistics(self) -> Dict[str, Dict[str, Any]]:
        ...
        for category in IntentCategory:
            ...
            successful_attempts = sum(
                self.effectiveness_data.get(attack, AttackEffectivenessData(attack)).successful_attempts
                for attack in attacks
            )
    ...
    def _build_enhanced_metadata(self):
        ...
        for attack_name in self.registry.list_attacks():
            ...
            priority_score = self._calculate_priority_score(attack_name, original_metadata, effectiveness)
            
            # Get DPI type recommendations
            dpi_types = self._get_dpi_type_recommendations(attack_name, original_metadata)
            
            # Get parameter recommendations
            param_recommendations = self._get_parameter_recommendations(attack_name, original_metadata)
    ...
    def _categorize_attack(self, attack_name: str, metadata: AttackMetadata) -> List[IntentCategory]:
    ...
    def _calculate_priority_score(self, attack_name: str, metadata: AttackMetadata, 
                                effectiveness: Optional[AttackEffectivenessData]) -> float:
    ...
    def _get_dpi_type_recommendations(self, attack_name: str, metadata: AttackMetadata) -> List[str]:
    ...
    def _get_parameter_recommendations(self, attack_name: str, metadata: AttackMetadata) -> Dict[str, Any]:
        """Get parameter recommendations for an attack"""
    ...
    def _build_intent_mapping(self) -> Dict[str, List[str]]:
        ...
        for intent_key in mapping:
            mapping[intent_key].sort(
                key=lambda name: self.enhanced_metadata[name].priority_score,
                reverse=True
            )
    ...
    def load_effectiveness_data(self):
        ...
        try:
            ...
            self.logger.info(f"Loaded effectiveness data for {len(self.effectiveness_data)} attacks")
...
def create_attack_registry_adapter(registry: Optional[AttackRegistry] = None,
                                 effectiveness_file: str = "attack_effectiveness.json") -> AttackRegistryAdapter:
    """
