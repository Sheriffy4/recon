...
def demo_attack_comparison():
    ...
    attacks = [
        ("Original", None, {}),
        ("Header Modification", HeaderModificationAttack(), {"case_modification": True}),
    ...
    for name, attack, params in attacks:
        ...
        if attack is None:
            ...
            print(f"Total bytes: {sum(len(seg[0]) for seg in result.segments) if result.has_segments() else 0}")
