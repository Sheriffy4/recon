...
class StartupConflictDetector:
    ...
    def load_domain_rules(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.domain_rules)} domain rules from {self.domain_rules_path}"
            )
            return True
        ...
        for ip, domains in ip_to_domains.items():
            if len(domains) > 1:
                ...
                for domain in domains:
                    ...
                    param_sig = {
                        k: strategy_params.get(k) for k in critical_params if k in strategy_params
                    }
            ...
            if parent_exists and parent_strategy:
                ...
                if subdomain_type != parent_type:
                    ...
                    param_diffs = self._compare_strategy_params(
                        subdomain_strategy.get("params", {}), parent_strategy.get("params", {})
                    )
    ...
    def _compare_strategy_params(
        ...
        for param in critical_params:
            if param in params1 or param in params2:
                ...
                if val1 != val2:
                    diffs.append({"param": param, "value1": val1, "value2": val2})
    ...
    def print_conflict_report(self, report: ConflictReport):
        ...
        if report.ip_conflicts:
            ...
            for i, conflict in enumerate(report.ip_conflicts, 1):
                ...
                logger.warning("   Domains on this IP:")
                ...
                logger.warning("   ℹ️  SNI-based selection will be used to apply correct strategy")
                logger.warning("")
        ...
        if report.parent_child_conflicts:
            ...
            for i, conflict in enumerate(report.parent_child_conflicts, 1):
                ...
                if conflict["conflict_type"] == "different_types":
                    logger.warning(
                        f"      Subdomain strategy: {conflict['subdomain_strategy_type']}"
                    )
                    ...
                    logger.warning("      Parameter differences:")
                    for diff in conflict["parameter_differences"]:
                        logger.warning(
                            f"         - {diff['param']}: subdomain={diff['value1']}, parent={diff['value2']}"
                        )
        ...
        if report.wildcard_conflicts:
            ...
            for i, conflict in enumerate(report.wildcard_conflicts, 1):
                ...
                logger.warning("      ℹ️  Specific domain rule takes precedence")
                logger.warning("")
    ...
    def _print_resolution_guide(self, report: ConflictReport):
        ...
        if report.ip_conflicts:
            ...
            logger.warning(
                "   2. If issues occur, consider using the same strategy for all domains on the IP"
            )
            logger.warning("   3. Test each domain individually: cli.py test <domain>")
            logger.warning(
                "   4. Monitor logs for 'Domain conflict detected' warnings during operation"
            )
        ...
        logger.warning(
            "   • Run conflict checker regularly: python -m core.cli.strategy_conflict_checker"
        )
        logger.warning("")
        logger.warning("=" * 80)
