...
if platform.system() == "Windows":
    ...
    class WindowsBypassEngine(IBypassEngine):
                ...
                if task_type == "fakeddisorder":
                    ...
                    if is_simple:
                        try:
                            ...
                            # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Проверяем zapret-совместимость
                            zapret_compatible = (split_pos == 3 and "badsum" in fooling_list)
                            
                            # ОТЛАДКА: Выводим параметры для диагностики
                            self.logger.info(f"🔍 DEBUG: split_pos={split_pos}, fooling_list={fooling_list}")
                            self.logger.info(f"🔍 DEBUG: zapret_compatible={zapret_compatible}")
                            
                            # ПРИНУДИТЕЛЬНАЯ АКТИВАЦИЯ для отладки
                            force_zapret = True  # Принудительно активируем zapret-style
                            
                            if zapret_compatible or force_zapret:
                                if force_zapret:
                                    self.logger.info("🚨 FORCE ZAPRET-STYLE ACTIVATED FOR DEBUG!")
                                else:
                                    self.logger.info("🎯 ZAPRET-COMPATIBLE CONDITIONS DETECTED!")
                                # ZAPRET-STYLE: Отправляем ТОЛЬКО 3 пакета как zapret
                                self.logger.info(f"🎯 ZAPRET-STYLE ACTIVATED: split_pos={split_pos} with fooling={fooling_list}")
                                
                                # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Используем TTL=1 для fake пакетов как в zapret
                                fake_ttl = 1  # Fake пакеты должны иметь TTL=1 чтобы не дойти до сервера
                                real_ttl = 3  # Real сегменты имеют TTL=3 как в zapret
                                
                                # 1) Отправляем ПОЛНЫЙ fake ClientHello с badsum (~500 байт)
                                self.logger.info(f"🎭 Sending FULL fake ClientHello with TTL={fake_ttl} and corrupted checksum...")
                                self._send_full_fake_zapret_style(packet, w, ttl=fake_ttl)
                                
                                # КРИТИЧЕСКОЕ: Добавляем микро-задержку как в zapret
                                import time
                                time.sleep(0.001)  # 1ms задержка между fake и real
                                
                                # 2) Отправляем реальные сегменты с TTL=3 (только 2 сегмента)
                                if self._send_real_segments_zapret_style(packet, w, payload, split_pos, real_ttl):
                                    self.logger.info("✅ ZAPRET-STYLE SUCCESS: Sent 3 packets total (1 fake + 2 real)")
                                    return
                                else:
                                    self.logger.error("❌ ZAPRET-STYLE FAILED: Real segments failed")
                                    # Не продолжаем выполнение, чтобы не отправлять лишние пакеты
                                    w.send(packet)
                                    return
                            else:
                                # Стандартный fakeddisorder (может отправлять 5 пакетов)
        ...
        def _send_full_fake_zapret_style(self, original_packet, w, ttl: int):
            ...
            КРИТИЧЕСКОЕ: Использует поддельный SNI вместо реального!
            """
            try:
                raw = bytearray(original_packet.raw)
                ip_hl = (raw[0] & 0x0F) * 4
                ...
                try:
                    ...
                    pass
                w.send(pkt)
                self.logger.debug(f"🎭 Sent FULL fake ClientHello with fake SNI: {fake_sni}, TTL={ttl}, bad_checksum=True")
