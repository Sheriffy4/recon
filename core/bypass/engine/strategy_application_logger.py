...
class StrategyApplicationLogger:
    ...
    def log_strategy_application(
        ...
        if params:
            LOG.info(f"Strategy Parameters:")
            for key, value in params.items():
                LOG.info(f"  - {key}: {value}")
        else:
            LOG.info(f"Strategy Parameters: None")
        ...
        if validation_result.valid:
            ...
            if validation_result.mismatches:
                LOG.error(f"Parameter Mismatches:")
        ...
        if match_type == 'parent':
            ...
            LOG.warning(f"⚠️ No matching rule found, using default strategy")
    ...
    def log_strategy_failure(
        ...
        params = strategy.get('params', {})
        if params:
            LOG.error(f"Strategy Parameters:")
            for key, value in params.items():
        ...
        if '.' in domain:
            ...
            if len(parts) > 2:  # Has subdomain
                ...
                LOG.error(f"  4. Try removing '{domain}' rule to use parent domain '{parent_domain}' strategy")
                LOG.error(f"     Edit domain_rules.json and remove the '{domain}' entry")
                LOG.error(f"  5. Check if DPI behavior has changed")
            else:
                LOG.error(f"  4. Check if DPI behavior has changed")
        else:
            LOG.error(f"  4. Check if DPI behavior has changed")
        
        # Enhanced warning for high retransmission count (Requirement 7.3, 8.1)
        if retransmissions >= 3:
            LOG.error(f"⚠️ CRITICAL: High retransmission count ({retransmissions}) indicates strategy is NOT working")
            LOG.error(f"   This strategy has failed the threshold and should be replaced immediately")
            LOG.error(f"   The connection is likely blocked by DPI despite bypass attempts")
    ...
    def log_retransmission_detected(
        ...
        params = strategy.get('params', {})
        if params:
            LOG.warning(f"Strategy Parameters:")
            for key, value in params.items():
                LOG.warning(f"  - {key}: {value}")
        
        # Provide progressive warnings based on retransmission count
        if retransmission_number == 1:
            LOG.warning(f"ℹ️  First retransmission - this may be normal network behavior")
        elif retransmission_number == 2:
            LOG.warning(f"⚠️  Second retransmission - strategy may not be working correctly")
        elif retransmission_number >= 3:
            LOG.warning(f"🚨 Third+ retransmission - strategy is likely failing")
            LOG.warning(f"   Consider re-testing or finding a new strategy")
    ...
    def log_testing_production_mismatch(
        ...
        LOG.error(f"Differences Detected:")
        ...
        LOG.error(f"  2. Verify that the strategy was saved correctly after testing")
        LOG.error(f"  3. Ensure no manual edits were made to domain_rules.json")
        LOG.error(f"  4. Re-run 'cli.py auto {domain}' to regenerate the strategy")
        LOG.error(f"  5. Check for parent domain conflicts (e.g., youtube.com vs www.youtube.com)")
    ...
    def log_multisplit_application(
        ...
        if fragment_count != split_count:
            ...
            LOG.warning(f"💡 This may indicate an issue with multisplit generation")
        else:
            LOG.info(f"✅ Fragment count matches expected split count")
        
        LOG.info("=" * 80)
    
    def log_first_connection(
        self,
        domain: str,
        strategy: Dict[str, Any],
        is_correct: bool
        ...
        if is_correct:
            LOG.info(f"✅ Correct strategy applied to first connection")
        else:
            LOG.error(f"❌ Incorrect strategy applied to first connection")
            LOG.error(f"💡 Recommendations:")
            LOG.error(f"  1. Check domain_rules.json for '{domain}'")
            LOG.error(f"  2. Verify strategy loading at service startup")
            LOG.error(f"  3. Check for parent domain conflicts")
