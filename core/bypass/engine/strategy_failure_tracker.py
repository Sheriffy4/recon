...
class StrategyFailureTracker:
    ...
    def _load_failure_data(self):
        ...
        try:
            ...
            if not rules_path.exists():
                LOG.warning("Domain rules file not found: %s", self.domain_rules_path)
                return
            ...
            LOG.error("Failed to load failure data: %s", e)

    def _atomic_write_json(self, path: Path, data: Dict[str, Any]) -> None:
        """Atomic write to JSON file using temp file + rename."""
    ...
    def _log_failure(self, record: StrategyFailureRecord):
        ...
        if record.failure_count == self.failure_threshold:
            LOG.warning("=" * 80)
            LOG.warning("STRATEGY FAILURE THRESHOLD REACHED")
            LOG.warning("=" * 80)
            ...
            LOG.warning("💡 Strategy may not be working correctly")
            LOG.warning("=" * 80)

        elif record.failure_count >= self.revalidation_threshold:
            LOG.error("=" * 80)
            LOG.error("STRATEGY REVALIDATION RECOMMENDED")
            LOG.error("=" * 80)
            ...
            LOG.error("  3. Check if DPI behavior has changed")
            LOG.error("  4. Consider using parent domain strategy")
            LOG.error("=" * 80)
        ...
        try:
            ...
            with self._file_lock:
                ...
                if domain in domain_rules:
                    ...
                    # Atomic write
                    self._atomic_write_json(rules_path, data)
    ...
    def get_domains_needing_revalidation(self) -> List[str]:
        ...
        """
        return [
            domain for domain, record in self.failure_records.items() if record.needs_revalidation
        ]
    ...
    def save_strategy_history(
        ...
        try:
            ...
            if domain in domain_rules:
                ...
                failure_count = self.failure_records.get(
                    domain,
        ...
        try:
            ...
            with self._file_lock:
                ...
                if domain in domain_rules:
                    ...
                    # Note: save_strategy_history will acquire lock again, but it's reentrant-safe
                    # because we release lock before calling it
                    pass  # Will call after releasing lock
            ...
            # Save old strategy to history (outside lock to avoid deadlock)
            if domain in domain_rules:
                old_strategy = domain_rules.get(domain)
                if old_strategy:
                    self.save_strategy_history(
                        domain, old_strategy, new_strategy, "Strategy revalidation - new strategy found"
                    )
