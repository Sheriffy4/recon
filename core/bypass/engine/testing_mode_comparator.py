...
class TestingModeComparator:
    ...
    def record_packet_sending(
        self, mode: PacketMode, strategy_type: str, domain: Optional[str] = None, **kwargs
    ) -> str:
        """
    ...
    def compare_modes(self, strategy_type: str, domain: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        testing_metric = self._find_latest_metric(self._testing_metrics, strategy_type, domain)
        production_metric = self._find_latest_metric(
            self._production_metrics, strategy_type, domain
        )

        if not testing_metric:
            self.logger.warning(f"⚠️  No testing mode metrics found for {strategy_type} ({domain})")
    ...
    def get_comparison_summary(self) -> Dict[str, Any]:
        ...
        identical_count = sum(1 for c in self._comparison_results if c.get("identical", False))
        ...
        return {
            ...
            "parity_percentage": (
                (identical_count / total_comparisons * 100) if total_comparisons > 0 else 0
            ),
