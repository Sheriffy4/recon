...
class EnhancedBypassEngineAdapter:
    ...
    def __init__(self, bypass_engine: WindowsBypassEngine, enable_traffic_analysis: bool = True):
        ...
        if enable_traffic_analysis:
            try:
                ...
                self.logger.warning("⚠️ Failed to initialize PCAP capturer: %s", e)
                self.enable_traffic_analysis = False
    ...
    def test_strategy_with_artifacts(
        self,
        domain: str,
        strategy: Dict[str, Any],
        ...
        if include_validation:
            ...
            if not validation_result.is_valid:
                self.logger.warning(
                    f"⚠️ Strategy validation failed: {validation_result.error_message}"
                )
    ...
    def _test_with_capture(
        self, domain: str, strategy: Dict[str, Any], timeout: float, artifacts: TrialArtifacts
    ) -> EnhancedTestResult:
        ...
        try:
            with self.pcap_capturer.capture_session(domain) as session:
                ...
                self.logger.info(
                    f"📊 Capture completed: {session.packets_captured} packets in {artifacts.capture_duration:.2f}s"
                )

                return EnhancedTestResult(
                    success=success, domain=domain, strategy=strategy, response_time=0, error=error
                )
    ...
    def _test_without_capture(
        self, domain: str, strategy: Dict[str, Any], artifacts: TrialArtifacts
    ) -> EnhancedTestResult:
        ...
        try:
            ...
            return EnhancedTestResult(
                success=False, domain=domain, strategy=strategy, response_time=0, error=str(e)
            )

    def _execute_strategy_test(
        self, domain: str, strategy: Dict[str, Any], timeout: float
    ...
    def _convert_strategy_to_task(self, strategy: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Support both old format (attack/type) and new format (attack_combination)
        attack_combination = strategy.get("attack_combination")
        if attack_combination:
            ...
            if isinstance(attack_combination, list) and len(attack_combination) > 0:
                ...
                # Store full attacks list for combo strategies
                attacks = attack_combination
            else:
                attack_type = "fake"
                attacks = None
        else:
            # Old format: fallback to attack/type
        ...
        task = {"type": attack_type, "params": params.copy(), "forced": True, "no_fallbacks": True}

        # Add attacks field for combo strategies
        if attacks and len(attacks) > 1:
            task["attacks"] = attacks
    ...
    def _test_domain_connectivity(self, domain: str, timeout: float) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"https://{domain}", timeout=timeout, verify=False, allow_redirects=True
            )
        ...
        stats.update(
            {
                ...
                "pcap_capture_available": (
                    self.pcap_capturer is not None and self.pcap_capturer.is_capture_available()
                    if self.pcap_capturer
                    else False
                ),
                "success_rate": self.stats["successful_tests"]
                / max(1, self.stats["tests_performed"]),
            }
        )
    ...
    def cleanup_artifacts(self):
        ...
        # Detach log handler to prevent duplicates/leaks on repeated adapter usage
        try:
            if self._log_handler is not None:
                logging.getLogger("BypassEngine").removeHandler(self._log_handler)
                self._log_handler = None
        except Exception:
            pass
...
def create_enhanced_bypass_adapter(
    bypass_engine: WindowsBypassEngine, enable_traffic_analysis: bool = True
