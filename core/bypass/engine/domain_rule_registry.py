...
class DomainRuleRegistry:
    ...
    def __init__(self, config_file: str = "domain_rules.json"):
        ...
        logger.info(f"DomainRuleRegistry initialized with config: {config_file}")
    ...
    def add_domain_rule(self, domain: str, strategy: Dict[str, Any], check_parent: bool = True) -> bool:
        """
        ...
        """
        try:
            if not domain or not isinstance(strategy, dict) or 'type' not in strategy:
            ...
            if not validator.validate_before_save(domain_lower, strategy):
                logger.error(f"❌ Strategy validation failed for '{domain_lower}' - missing required parameters")
    ...
    def _check_parent_domain_before_adding(self, domain: str, strategy: Dict[str, Any]):
        ...
        try:
            ...
            if parent_strategy:
                ...
                logger.warning(f"   A strategy already exists for the parent domain '{parent_key}'.")
                logger.warning(f"   Consider testing if the parent domain strategy works for '{domain}'")
                logger.warning(f"   before creating a subdomain-specific strategy.")
                logger.warning("")
                ...
                logger.warning("")
                logger.warning("   If the parent strategy works, you can skip creating this subdomain rule.")
                logger.warning("=" * 80)
            ...
            for ip, domains in ip_to_domains.items():
                if len(domains) > 1:
                    ...
                    if len(unique_strategies) > 1:
                        ...
                        logger.warning(f"   Multiple domains resolve to this IP with DIFFERENT strategies:")
                        for domain in domains:
                            strategy = self.domain_rules[domain]
                            logger.warning(f"   - {domain}: {strategy.get('type')} {strategy.get('params', {})}")
                        logger.warning(f"   ℹ️  Domain-based filtering will use SNI to select the correct strategy")
                        logger.warning(f"   ℹ️  If a strategy doesn't work, consider using the same strategy for all domains on this IP")
            
            if conflicts_found:
                logger.warning("=" * 80)
                logger.warning("⚠️ IP CONFLICTS SUMMARY:")
                logger.warning("   Some domains share the same IP but have different strategies.")
                logger.warning("   The system will use SNI (Server Name Indication) to apply the correct strategy.")
