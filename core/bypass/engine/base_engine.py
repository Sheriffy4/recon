...
class WindowsBypassEngine(IBypassEngine):
    ...
    def _recipe_to_specs(self, recipe: List[Tuple[bytes, int, dict]], payload: bytes) -> List[TCPSegmentSpec]:
        if not recipe or not isinstance(recipe, (list, tuple)):
            self.logger.error(f"_recipe_to_specs: Получен невалидный 'рецепт' (тип: {type(recipe)}).")
            return []
        ...
        for i, recipe_item in enumerate(recipe):
            try:
                if not isinstance(recipe_item, (list, tuple)) or len(recipe_item) != 3:
                    self.logger.error(f"Элемент рецепта #{i} имеет неверную структуру. Пропуск.")
                    continue
                    
                seg_payload, offset, opts = recipe_item
                
                if not isinstance(offset, int):
                    self.logger.error(f"Offset в элементе #{i} не является числом. Пропуск.")
                    continue
                if not isinstance(opts, dict):
                    self.logger.error(f"Опции в элементе #{i} не являются словарем. Пропуск.")
                    continue
                ...
                if (tcp_flags & 0x01):
                    if not opts.get("allow_fin", False):
                        self.logger.warning(f"🛡️ FIN-Sanitizer: Удален флаг FIN в сегменте #{i}.")
                        tcp_flags &= ~0x01
                
                valid_real_flags = [0x10, 0x18]
                if not is_fake and tcp_flags not in valid_real_flags:
                    self.logger.warning(f"🛡️ Flag-Normalizer: Нестандартные TCP флаги 0x{tcp_flags:02X} в реальном сегменте #{i}, нормализация до PSH+ACK (0x18).")
                    tcp_flags = 0x18
                ...
                spec = TCPSegmentSpec(
                    ...
                    seq_extra=int(opts.get("seq_extra", -1 if opts.get("corrupt_sequence") else 0)),
                    fooling_sni=opts.get("fooling_sni"),
                    is_fake=is_fake,
                    delay_ms_after=int(opts.get("delay_ms", opts.get("delay_ms_after", 0))) if i < total_items - 1 else 0,
                    preserve_window_size=bool(opts.get("preserve_window_size", not is_fake))
                )
                specs.append(spec)
                
            except Exception as e:
                self.logger.error(f"Ошибка при обработке элемента рецепта #{i}: {e}", exc_info=self.debug)
                continue
        
        if not specs:
            self.logger.error("Не удалось сгенерировать ни одного валидного сегмента из рецепта.")
            return []
        ...
        try:
            ...
            if L > 0:
                covered = [False] * L
                for s in specs:
                    if getattr(s, "is_fake", False): continue
                ...
                if holes:
                    self.logger.error(f"‼️ КРИТИЧЕСКАЯ ОШИБКА СБОРКИ! "
                                      f"Реальный payload имеет 'дыры' ({len(holes)} байт не покрыто). "
                                      f"Первые пропуски на позициях: {holes[:16]}")
                    if self.debug:
                        raise ValueError(f"TCP stream has holes at positions: {holes[:16]}")
        except Exception as e:
    ...
    def apply_bypass(self, packet: "pydivert.Packet", w: "pydivert.WinDivert", strategy_task: Dict, forced=True):
        """
        Унифицированный метод применения обхода с подавлением оригинального пакета.
        Эта логика теперь едина для режима службы и режима тестирования.
        """
        ...
        with self._lock:
            if flow_key in self._processed_flows and now - self._processed_flows[flow_key] < 1.5:
                self.logger.debug(f"🔪 Dropping subsequent packet for already processed flow {flow_key}")
            ...
            if len(self._processed_flows) > 2000:
                ...
                self._processed_flows = {k: t for k, t in self._processed_flows.items() if t >= cutoff}
        ...
        try:
            ...
            if autottl_offset is not None:
                ...
                race_attack_types = ("fakeddisorder", "multidisorder", "disorder", "disorder2", "seqovl", "fake_race", "fake")
            ...
            if task_type == "seqovl":
                dispatch_params['overlap_size'] = int(params.get("overlap_size", params.get("split_seqovl", 20)))
            
            try:
                self.logger.debug(f"🎯 Dispatching {task_type} attack via AttackDispatcher")
                ...
                self.logger.error(f"❌ Attack dispatch validation failed for '{task_type}': {e}")
                w.send(packet)
                return
            except Exception as e:
            ...
            if not recipe:
                self.logger.warning(f"Recipe for {task_type} was not generated. Forwarding original.")
                w.send(packet)
                return

            specs = self._recipe_to_specs(recipe, payload)
            if not specs:
                self.logger.error(f"Failed to convert recipe to specs for {task_type}. Forwarding original.")
                w.send(packet)
                return

            self.logger.info(f"📦 Packet sequence: {len(specs)} segments for {task_type}")
            
            ok = self._packet_sender.send_tcp_segments(w, packet, specs)
            if not ok:
                self.logger.error(f"❌ PacketSender failed for {task_type}. Forwarding original.")
                w.send(packet)
            # После успешной инъекции оригинальный пакет уже был уничтожен в начале функции.

        except Exception as e:
            self.logger.error(f"❌ CRITICAL ERROR in apply_bypass: {e}", exc_info=self.debug)
            try:
                w.send(packet)
            except Exception:
                pass
