...
class HierarchicalDomainMatcher:
    ...
    def __init__(self, domain_rules: Dict[str, Dict[str, Any]], default_strategy: Dict[str, Any]):
        ...
        logger.debug(f"HierarchicalDomainMatcher initialized with {len(domain_rules)} rules")
    
    def find_matching_rule(self, domain: str, strict: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[str], str]:
        """
        ...
        """
        if not domain:
            logger.info(f"📋 No domain provided, will apply default strategy")
            return None, None, 'none'
            
        return self._find_matching_rule_cached(domain, strict)  
  
    @lru_cache(maxsize=1000)
    def _find_matching_rule_cached(self, domain: str, strict: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[str], str]:
        """
        ...
        try:
            ...
            if domain in self.domain_rules:
                ...
                logger.info(f"📋 Exact subdomain match found: '{domain}' → rule '{domain}'")
                self._cache_hits += 1
            ...
            if not strict:
                ...
                for check_domain in domains_to_check[1:]:
                    if check_domain in self.domain_rules:
                        ...
                        logger.warning(f"📋 Parent domain fallback: '{domain}' → rule '{check_domain}'")
                        logger.warning(f"💡 Consider creating specific rule for '{domain}' or using wildcard '*.{check_domain}'")
                        self._cache_hits += 1
                    ...
                    if wildcard_pattern in self.domain_rules:
                        rule = self.domain_rules[wildcard_pattern]
                        logger.warning(f"📋 Parent wildcard fallback: '{domain}' → rule '{wildcard_pattern}'")
                        self._cache_hits += 1
                        return rule, wildcard_pattern, 'parent'
            
            # No rule found in hierarchy
            logger.info(f"📋 No matching rule found for domain '{domain}', will apply default strategy")
    ...
    def update_rules(self, domain_rules: Dict[str, Dict[str, Any]], default_strategy: Dict[str, Any]):
        """
