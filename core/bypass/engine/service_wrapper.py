...
class BypassEngineService:
    ...
    def start(self) -> bool:
        ...
        # Set strategy override via API to ensure normalization/parity with CLI
        self.engine.set_strategy_override(self.config.strategy)

        # Start engine in background thread
        self.service_thread = threading.Thread(
            target=self._run_engine, daemon=True, name="BypassEngineService"
        )
    ...
    def _run_engine(self) -> None:
        ...
        try:
            ...
            self.engine.start(
                ...
                # Pass normalized override (set_strategy_override may adjust ttl/fake_ttl/etc)
                strategy_override=self.engine.strategy_override,
            )
