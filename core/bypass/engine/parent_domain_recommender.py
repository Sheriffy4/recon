...
class ParentDomainRecommendation:
    ...
    def __init__(
        ...
        self.recommendation_type = (
            recommendation_type  # 'remove_subdomain', 'use_parent', 'test_parent'
        )
        self.reason = reason
...
class ParentDomainRecommender:
    ...
    def check_parent_domain_exists(
        self, domain: str
    ...
    def should_recommend_parent_domain(
        self, domain: str, failure_count: int
    ...
    def log_recommendation(self, recommendation: ParentDomainRecommendation):
        ...
        if recommendation.recommendation_type == "remove_subdomain":
            ...
            LOG.warning("💡 RECOMMENDATION: Remove subdomain strategy")
            LOG.warning(
                f"   The parent domain '{recommendation.parent_domain}' has a working strategy."
            )
            LOG.warning(
                "   Removing the subdomain strategy will simplify configuration and may improve reliability."
            )
            LOG.warning("")
            LOG.warning("   To apply this recommendation:")
            LOG.warning(f"   1. Edit {self.domain_rules_path}")
            LOG.warning(f"   2. Remove the entry for '{recommendation.subdomain}'")
            LOG.warning("   3. Restart the bypass service")
            LOG.warning("")
            LOG.warning("   Parent domain strategy details:")
            ...
            LOG.warning("💡 RECOMMENDATION: Test parent domain strategy")
            LOG.warning(f"   The subdomain '{recommendation.subdomain}' strategy is failing.")
            LOG.warning(
                f"   Consider testing the parent domain '{recommendation.parent_domain}' to find a working strategy."
            )
            LOG.warning("")
            LOG.warning("   To apply this recommendation:")
            LOG.warning(f"   1. Run: cli.py auto {recommendation.parent_domain}")
            LOG.warning(
                f"   2. If successful, remove the entry for '{recommendation.subdomain}' from {self.domain_rules_path}"
            )
            LOG.warning("   3. Restart the bypass service")
    ...
    def detect_and_recommend(
        self, domain: str, failure_count: int, strategy: Dict[str, Any]
        ...
        Requirements: 10.1, 10.4
        """
        should_recommend, recommendation = self.should_recommend_parent_domain(
            domain, failure_count
        )
    ...
    def log_strategy_conflicts(self, conflicts: List[Dict[str, Any]]):
        ...
        for i, conflict in enumerate(conflicts, 1):
            ...
            if conflict["conflict_type"] == "different_strategy_types":
                ...
                LOG.warning(
                    "  💡 Consider removing subdomain strategy to use parent domain strategy"
                )

            elif conflict["conflict_type"] == "different_parameters":
                LOG.warning(f"  Strategy Type: {conflict['subdomain_strategy_type']}")
                LOG.warning("  Parameter Differences:")
                for diff in conflict["parameter_differences"]:
                    LOG.warning(
                        f"    - {diff['param']}: subdomain={diff['subdomain_value']}, parent={diff['parent_value']}"
                    )
                LOG.warning("  💡 Consider testing if parent domain parameters work for subdomain")

        LOG.warning("")
        LOG.warning("💡 To resolve conflicts:")
        LOG.warning("   1. Test parent domain strategies with subdomains")
        LOG.warning("   2. Remove subdomain entries if parent strategy works")
        LOG.warning("   3. Use wildcard patterns (*.domain.com) for consistency")
        LOG.warning("=" * 80)
    ...
    def get_recommendation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations_by_type": {
                "remove_subdomain": sum(
                    1
                    for r in self.recommendations_made.values()
                    if r.recommendation_type == "remove_subdomain"
                ),
                "test_parent": sum(
                    1
                    for r in self.recommendations_made.values()
                    if r.recommendation_type == "test_parent"
                ),
            },
