"""
SNI Extractor component for runtime packet filtering - ULTIMATE VERSION
...
ULTIMATE FIXES:
- Normalizes hostname (lowercase, strip, rstrip("."))
- Optional IDNA encoding for consistent matching
"""
...
class SNIExtractor:
    ...
    ULTIMATE VERSION: Proper hostname normalization for consistent matching
    """
    ...
    def _parse_sni_extension(self, sni_data: bytes) -> Optional[str]:
        ...
        ULTIMATE FIX: Normalizes hostname for consistent matching
        ...
        while offset < len(sni_data) - 3:
            ...
            if name_type == 0x00:  # hostname type
                ...
                if offset + name_len <= len(sni_data):
                    ...
                    # ULTIMATE FIX: Normalize hostname for consistent matching
    ...
    def _normalize_hostname(self, hostname: str) -> Optional[str]:
        ...
        ULTIMATE FIX: Ensures consistent format for rule matching

        Args:
            hostname: Raw hostname from SNI
        ...
        try:
            ...
            # Remove port if present (shouldn't be in SNI, but just in case)
            if ":" in hostname:
            ...
            # Optional: Normalize IDN to punycode for consistent matching
            # Uncomment if you want IDNA normalization in fast extractor too
            # try:
    ...
    def _is_valid_hostname(self, hostname: str) -> bool:
        ...
        Args:
            hostname: Hostname to validate (should already be normalized/lowercase)
        ...
        # Basic character validation (lowercase only since we normalize)
...
_global_extractor = SNIExtractor()


def extract_sni_from_packet(payload: bytes) -> Optional[str]:
    """
    Module-level function to extract SNI from packet payload.
    ...
    Returns:
        SNI hostname string if found, None otherwise
    """
    return _global_extractor.extract_sni(payload)
