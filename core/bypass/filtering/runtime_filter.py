...
class RuntimePacketFilter:
    ...
    def __init__(self, config: Optional[FilterConfig] = None, 
        ...
        logger.info(f"RuntimePacketFilter initialized with mode={self.mode.value}, domains={len(self.domain_list)}")
            ...
            try:
                ...
                if not domain:
                    ...
                    if self.mode == FilterMode.NONE:
                        ...
                        logger.debug(f"No domain extracted from packet, skipping bypass (mode={self.mode.value})")
                        return False
    ...
    def extract_domain(self, packet) -> Optional[str]:
        ...
        with self.performance_monitor.start_domain_extraction():
            try:
                ...
                cache_key = hash(payload[:min(200, len(payload))])  # Use first 200 bytes for cache key
    ...
    def update_configuration(self, config: FilterConfig) -> None:
        ...
        logger.info(f"Configuration updated: mode={self.mode.value}, domains={len(self.domain_list)}")
