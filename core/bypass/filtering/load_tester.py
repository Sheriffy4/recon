...
class LoadTester:
    ...
    def __init__(self, config: Optional[LoadTestConfig] = None):
        ...
        logger.info(f"LoadTester initialized for {self.config.packet_count} packets, {self.config.concurrent_threads} threads")
    ...
    def _generate_test_data(self) -> None:
        ...
        logger.info(f"Generated {len(self._test_packets)} test packets and {len(self._test_domains)} domains")
    ...
    def _test_domain_list_scalability(self) -> None:
        ...
        for domain_count in self.config.domain_list_sizes:
            ...
            logger.info(f"Domain count {domain_count}: {throughput:.1f} pps, {avg_latency:.2f}ms latency")
    ...
    def _run_throughput_test(self) -> None:
        ...
        performance_monitor = PerformanceMonitor(
            max_samples=10000,
            alert_threshold_ms=self.config.max_latency_ms
        )
        ...
        try:
            ...
            logger.info(f"Throughput test completed: {throughput:.1f} pps, {self.results.avg_latency_ms:.2f}ms avg latency")
    ...
    def _validate_requirements(self) -> None:
        ...
        requirements_met = [
            ("Latency", self.results.latency_requirement_met, f"{self.results.avg_latency_ms:.2f}ms <= {self.config.max_latency_ms}ms"),
            ("Memory", self.results.memory_requirement_met, f"{self.results.peak_memory_mb:.1f}MB <= {self.config.max_memory_mb}MB"),
            ("CPU", self.results.cpu_requirement_met, f"{self.results.peak_cpu_percent:.1f}% <= {self.config.max_cpu_percent}%"),
            ("Throughput", self.results.throughput_requirement_met, f"{self.results.throughput_pps:.1f}pps >= {self.config.min_throughput_pps}pps")
        ]
    ...
    def generate_report(self) -> str:
        ...
        if self.results.domain_list_results:
            ...
            for domain_count, metrics in self.results.domain_list_results.items():
                report.append(f"  {domain_count:,} domains: {metrics['throughput_pps']:.1f} pps, {metrics['avg_latency_ms']:.2f}ms")
