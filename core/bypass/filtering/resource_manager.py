...
class ResourceManager:
    ...
    def __init__(self, limits: Optional[ResourceLimits] = None):
        ...
        logger.info(f"ResourceManager initialized with limits: memory={self.limits.memory_emergency_mb}MB, cpu={self.limits.cpu_emergency_percent}%")
    ...
    def _calculate_resource_state(self, memory_mb: float, cpu_percent: float) -> ResourceState:
        ...
        if (memory_mb >= self.limits.memory_emergency_mb or 
            cpu_percent >= self.limits.cpu_emergency_percent):
            return ResourceState.EMERGENCY
        
        # Check critical conditions
        if (memory_mb >= self.limits.memory_critical_mb or 
            cpu_percent >= self.limits.cpu_critical_percent):
            return ResourceState.CRITICAL
        
        # Check warning conditions
        if (memory_mb >= self.limits.memory_warning_mb or 
            cpu_percent >= self.limits.cpu_warning_percent):
    ...
    def add_state_change_callback(self, callback: Callable[[ResourceState, ResourceState], None]) -> None:
        """
