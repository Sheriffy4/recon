...
class SNIManipulator:
    ...
    def find_sni_position(packet: bytes) -> Optional[SNIPosition]:
        ...
        try:
            ...
            if sni_value_end > extension_end:
                LOG.error(f"SNI value end position {sni_value_end} exceeds extension end {extension_end}")
                return None
            ...
            LOG.debug(f"Found SNI '{sni_value}' at position {sni_value_start}-{sni_value_end} (TLS 1.{tls_version - 1})")
            return position
    ...
    @staticmethod
    def split_at_sni(packet: bytes, mode: str = "sni", position: Optional[int] = None) -> Tuple[bytes, bytes]:
        """
        ...
        try:
            ...
            if mode == "position" and position is not None:
                ...
                if position < 0 or position > len(packet):
                    raise ValueError(f"Invalid split position: {position} (packet length: {len(packet)})")
                split_pos = position
            else:
                ...
                if mode == "sni":
                    ...
                    raise ValueError(f"Invalid split mode: {mode}. Use 'sni', 'midsni', or 'position'")
            ...
            LOG.debug(f"Split packet at {mode} (position {split_pos}): {len(first_part)} + {len(second_part)} bytes")
    ...
    @staticmethod
    def split_with_tcp_update(packet_with_tcp: bytes, mode: str = "sni", position: Optional[int] = None, 
                              tcp_header_len: int = 20) -> Tuple[bytes, bytes]:
        """
        ...
        try:
            ...
            LOG.debug(f"Split with TCP update: seq {original_seq} -> {new_seq} (offset: {len(tls_first)})")
            LOG.debug(f"First packet: {len(first_packet)} bytes, Second packet: {len(second_packet)} bytes")
        ...
        try:
            ...
            if validate:
                ...
                if not new_sni_pos or new_sni_pos.sni_value != new_sni:
                    LOG.error(f"SNI verification failed: expected '{new_sni}', got '{new_sni_pos.sni_value if new_sni_pos else None}'")
                    raise ValueError("SNI change verification failed")
            
            LOG.debug(f"Changed SNI from '{sni_pos.sni_value}' to '{new_sni}' (length diff: {length_diff})")
            return new_packet
    ...
    @staticmethod
    def change_sni_with_tcp(packet_with_tcp: bytes, new_sni: str, tcp_header_len: int = 20, 
                           recalculate_checksum: bool = False) -> bytes:
        """
        ...
        try:
            ...
            LOG.debug(f"Changed SNI in TCP packet: {len(packet_with_tcp)} -> {len(new_packet)} bytes")
            return new_packet
    ...
    def _update_tls_lengths(packet: bytes, sni_pos: SNIPosition, length_diff: int) -> bytes:
        ...
        try:
            ...
            hello_len_pos = 6
            old_hello_len = struct.unpack("!I", b'\x00' + packet_array[hello_len_pos:hello_len_pos + 3])[0]
            ...
            record_len_pos = 3
            old_record_len = struct.unpack("!H", packet_array[record_len_pos:record_len_pos + 2])[0]
    ...
    def validate_tls_structure(packet: bytes) -> bool:
        ...
        try:
            ...
            if record_len + 5 != len(packet):
                LOG.debug(f"TLS structure validation failed: record length mismatch ({record_len + 5} != {len(packet)})")
                return False
            ...
            if sni_ext:
                sni_value = TLSParser.get_sni(packet)
                if not sni_value:
                    LOG.debug("TLS structure validation failed: SNI extension exists but value cannot be extracted")
                    return False
        ...
        try:
            ...
            if split_mode.isdigit():
                ...
                if 0 <= position <= len(packet):
                    ...
                    LOG.warning(f"Split position {position} out of range for packet length {len(packet)}")
                    return None
