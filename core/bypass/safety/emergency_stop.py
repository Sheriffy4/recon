...
class AttackStopController:
    ...
    def request_stop(self, reason: StopReason, description: str, priority: StopPriority = StopPriority.HIGH) -> None:
...
class EmergencyStopManager:
    ...
    def _initialize_default_conditions(self) -> None:
        ...
        self.add_stop_condition(StopCondition(
    ...
    def request_stop(self, attack_id: str, reason: StopReason, description: str, 
                    priority: StopPriority = StopPriority.HIGH) -> bool:
    ...
    def request_stop_all(self, reason: StopReason, description: str, 
                        priority: StopPriority = StopPriority.CRITICAL) -> int:
        ...
        with self._lock:
            ...
            if count > 0:
                ...
                LOG.critical(f"Emergency stop requested for all {count} active attacks: {description}")
            ...
            while self._monitoring_active:
                ...
                for condition in self._stop_conditions:
                    ...
                    if (condition.last_check and 
                        (current_time - condition.last_check).total_seconds() < condition.check_interval_seconds):
                        continue
                    ...
                    try:
                        ...
                        if should_stop:
                            ...
                            if condition.consecutive_failures >= condition.consecutive_failures_required:
    ...
    def _trigger_condition_stop(self, condition: StopCondition) -> None:
        ...
        LOG.critical(f"Emergency stop condition triggered: {condition.name} - {condition.description}")
        ...
        if condition.priority in [StopPriority.CRITICAL, StopPriority.IMMEDIATE]:
            ...
            LOG.warning(f"Stop condition {condition.name} triggered but not stopping attacks (priority: {condition.priority.name})")
