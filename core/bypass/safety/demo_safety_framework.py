...
class DemoAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        if self.behavior == 'normal':
            time.sleep(0.1)
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=100.0, packets_sent=1, bytes_sent=len(context.payload) if context.payload else 0, technique_used='demo_technique')
        elif self.behavior == 'slow':
            time.sleep(2.0)
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=2000.0, packets_sent=1, bytes_sent=len(context.payload) if context.payload else 0)
        elif self.behavior == 'memory_intensive':
            large_data = b'x' * (10 * 1024 * 1024)
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=100.0, packets_sent=1, bytes_sent=len(large_data), metadata={'large_data': large_data})
        elif self.behavior == 'network_heavy':
            time.sleep(0.1)
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=100.0, packets_sent=100, bytes_sent=1024 * 1024, technique_used='network_flooding')
        elif self.behavior == 'failure':
            raise Exception('Demo attack intentional failure')
        elif self.behavior == 'suspicious':
            return AttackResult(status=AttackStatus.SUCCESS, latency_ms=100.0, packets_sent=1, bytes_sent=100, metadata={'password': 'secret123', 'large_field': 'x' * 200000})
        else:
...
def demo_basic_safety():
    ...
    context = AttackContext(dst_ip='1.1.1.1', dst_port=443, payload=b'GET / HTTP/1.1\r\nHost: example.com\r\n\r\n')
    ...
    try:
        with controller.execute_attack_sync(attack, context) as record:
            ...
            if record.pre_validation:
                print(f'  - Pre-validation score: {record.pre_validation.safety_score:.2f}')
            if record.result:
...
def demo_resource_limits():
    ...
    print('=' * 60)
    config = SafetyConfiguration(resource_limits=ResourceLimits(max_memory_mb=5.0, max_execution_time_seconds=1.0, max_concurrent_attacks=1))
    controller = SafetyController(config)
...
def demo_validation_levels():
    ...
    print('=' * 60)
    validation_levels = [ValidationLevel.MINIMAL, ValidationLevel.STANDARD, ValidationLevel.STRICT, ValidationLevel.PARANOID]
    for level in validation_levels:
        print(f'\nTesting validation level: {level.value}')
        config = SafetyConfiguration(validation_level=level, fail_on_validation_errors=False)
        controller = SafetyController(config)
        ...
        try:
            with controller.execute_attack_sync(attack, context) as record:
                ...
                if record.post_validation:
                    print(f'  - Post-validation score: {record.post_validation.safety_score:.2f}')
                    if record.post_validation.recommendations:
                        print(f'  - Recommendations: {len(record.post_validation.recommendations)}')
        except SafetyError as e:
...
def demo_sandbox_violations():
    ...
    print('=' * 60)
    config = SafetyConfiguration(sandbox_constraints=SandboxConstraints(max_network_operations=2, forbidden_destinations={'malicious.com'}, forbidden_ports={22, 23}))
    controller = SafetyController(config)
...
def demo_emergency_stops():
    ...
    def trigger_emergency_stop():
        ...
        success = controller.emergency_stop_attack('demo_emergency', 'Demo emergency stop')
...
def demo_concurrent_attacks():
    ...
    print('=' * 60)
    config = SafetyConfiguration(resource_limits=ResourceLimits(max_concurrent_attacks=2))
    controller = SafetyController(config)
    ...
    threads = []
    for i in range(4):
        thread = threading.Thread(target=run_attack, args=[f'concurrent_attack_{i}'], daemon=True)
...
async def demo_async_execution():
    ...
    print('=' * 60)
    controller = SafetyController()
    attacks = [DemoAttack('async_attack_1', 'normal'), DemoAttack('async_attack_2', 'normal'), DemoAttack('async_attack_3', 'normal')]
...
def demo_comprehensive_safety():
    ...
    print('=' * 60)
    config = SafetyConfiguration(resource_limits=ResourceLimits(max_execution_time_seconds=10.0, max_memory_mb=100.0, max_concurrent_attacks=3, max_packets_per_second=500), sandbox_constraints=SandboxConstraints(max_network_operations=100, max_file_operations=10, allowed_ports={80, 443, 53, 8080, 8443}, forbidden_destinations={'malicious.com', 'evil.org'}), validation_level=ValidationLevel.STRICT, enable_pre_validation=True, enable_post_validation=True, fail_on_validation_errors=False, default_attack_timeout=5.0, log_all_executions=True)
    controller = SafetyController(config)
    print('Configuration:')
    print(f'  - Validation level: {config.validation_level.value}')
    print(f'  - Max execution time: {config.resource_limits.max_execution_time_seconds}s')
    print(f'  - Max concurrent attacks: {config.resource_limits.max_concurrent_attacks}')
    print(f'  - Max memory: {config.resource_limits.max_memory_mb}MB')
    test_cases = [('normal_attack', 'normal', 'Standard attack execution'), ('network_attack', 'network_heavy', 'Network-intensive attack'), ('suspicious_attack', 'suspicious', 'Attack with suspicious metadata')]
    for attack_id, behavior, description in test_cases:
        ...
        try:
            with controller.execute_attack_sync(attack, context) as record:
                ...
                print(f'  - Duration: {record.duration_seconds:.3f}s')
                print(f'  - Safety components active: R:{record.resource_monitor} S:{record.sandbox_monitor} E:{record.emergency_stop}')
                if record.pre_validation:
                    print(f'  - Pre-validation: {record.pre_validation.overall_result.value} (score: {record.pre_validation.safety_score:.2f})')
                if record.post_validation:
                    print(f'  - Post-validation: {record.post_validation.overall_result.value} (score: {record.post_validation.safety_score:.2f})')
    ...
    for record in history[-3:]:
        print(f"  - {record.attack_id}: {('✓' if record.success else '✗')} ({record.duration_seconds:.3f}s)")

def main():
    """Run all safety framework demonstrations."""
    print('🛡️  SAFETY FRAMEWORK DEMONSTRATION')
    print('=' * 80)
    print('This demo shows the comprehensive safety features of the modernized bypass engine.')
