...
class ValidationCheck:
    ...
    name: str
    description: str
    check_function: Callable[[BaseAttack, AttackContext, Optional[AttackResult]], Tuple[ValidationResult, str]]
    level: ValidationLevel
    category: str
    enabled: bool = True
    weight: float = 1.0

    def execute(self, attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]=None) -> Tuple[ValidationResult, str]:
...
class ValidationReport:
    ...
    def add_check_result(self, check_name: str, result: ValidationResult, message: str, weight: float=1.0) -> None:
    ...
    def is_safe_to_execute(self) -> bool:
        ...
        return self.overall_result in [ValidationResult.PASS, ValidationResult.WARN] and self.checks_failed == 0 and (self.checks_errored == 0)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'attack_id': self.attack_id, 'validation_level': self.validation_level.value, 'timestamp': self.timestamp.isoformat(), 'summary': {'checks_passed': self.checks_passed, 'checks_warned': self.checks_warned, 'checks_failed': self.checks_failed, 'checks_errored': self.checks_errored, 'overall_result': self.overall_result.value, 'safety_score': self.safety_score, 'safe_to_execute': self.is_safe_to_execute()}, 'check_results': [{'name': name, 'result': result.value, 'message': message} for name, result, message in self.check_results], 'recommendations': self.recommendations, 'critical_issues': self.get_critical_issues()}
...
class SafetyValidator:
    ...
    def _initialize_default_checks(self) -> None:
        ...
        def check_attack_structure(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
        ...
        self.add_check(ValidationCheck(name='attack_structure', description='Validate basic attack structure', check_function=check_attack_structure, level=ValidationLevel.MINIMAL, category='structure'))

        def check_context_safety(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            issues = []
            if not context.dst_ip:
                issues.append('Missing destination IP')
            elif context.dst_ip in ['127.0.0.1', 'localhost']:
                return (ValidationResult.WARN, 'Targeting localhost - ensure this is intentional')
            ...
            if context.params:
                for key, value in context.params.items():
                    if isinstance(value, str) and any((dangerous in value.lower() for dangerous in ['rm -rf', 'del /f', 'format c:'])):
        ...
        self.add_check(ValidationCheck(name='context_safety', description='Validate attack context for safety', check_function=check_context_safety, level=ValidationLevel.MINIMAL, category='context'))

        def check_attack_definition(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            attack_id = getattr(attack, 'id', 'unknown')
            if hasattr(attack, 'stability') and attack.stability == AttackStability.EXPERIMENTAL:
                return (ValidationResult.WARN, 'Attack marked as experimental - use with caution')
        ...
        self.add_check(ValidationCheck(name='attack_definition', description='Validate attack definition properties', check_function=check_attack_definition, level=ValidationLevel.STANDARD, category='definition'))

        def check_resource_prediction(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            warnings = []
            if context.payload:
                payload_size = len(context.payload)
                if payload_size > 10 * 1024:
                    warnings.append(f'Large payload may use significant memory: {payload_size} bytes')
            if context.params:
        ...
        self.add_check(ValidationCheck(name='resource_prediction', description='Predict resource usage for attack', check_function=check_resource_prediction, level=ValidationLevel.STANDARD, category='resources'))

        def check_network_safety(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            issues = []
            if context.dst_ip:
                try:
                    import ipaddress
            ...
            if context.dst_port in dangerous_ports:
                return (ValidationResult.WARN, f'Targeting potentially sensitive port {context.dst_port}')
        ...
        self.add_check(ValidationCheck(name='network_safety', description='Validate network safety aspects', check_function=check_network_safety, level=ValidationLevel.STANDARD, category='network'))

        def check_result_safety(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            if result is None:
            ...
            if hasattr(result, 'segments') and result.segments:
                total_payload_size = sum((len(segment[0]) for segment in result.segments if isinstance(segment, tuple) and len(segment) >= 1))
                if total_payload_size > 1024 * 1024:
                    issues.append(f'Very large total payload in segments: {total_payload_size} bytes')
            if issues:
                return (ValidationResult.FAIL, '; '.join(issues))
            return (ValidationResult.PASS, 'Result safety validation passed')
        self.add_check(ValidationCheck(name='result_safety', description='Validate attack result for safety', check_function=check_result_safety, level=ValidationLevel.STANDARD, category='result'))

        def check_deterministic_behavior(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            if context.params:
                for key, value in context.params.items():
                    if 'random' in key.lower() or 'rand' in key.lower():
                        return (ValidationResult.WARN, 'Attack uses random parameters - behavior may not be deterministic')
            return (ValidationResult.PASS, 'Deterministic behavior check passed')
        self.add_check(ValidationCheck(name='deterministic_behavior', description='Check for deterministic attack behavior', check_function=check_deterministic_behavior, level=ValidationLevel.STRICT, category='behavior'))

        def check_side_effects(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            warnings = []
            if hasattr(attack, 'modifies_global_state') and attack.modifies_global_state:
                warnings.append('Attack may modify global state')
            if context.params and any(('file' in str(key).lower() or 'path' in str(key).lower() for key in context.params.keys())):
                warnings.append('Attack may access file system')
            if context.params and any(('config' in str(key).lower() or 'setting' in str(key).lower() for key in context.params.keys())):
                warnings.append('Attack may modify network configuration')
            if warnings:
                return (ValidationResult.WARN, '; '.join(warnings))
            return (ValidationResult.PASS, 'Side effects check passed')
        self.add_check(ValidationCheck(name='side_effects', description='Check for potential side effects', check_function=check_side_effects, level=ValidationLevel.PARANOID, category='side_effects'))
    ...
    def validate_pre_execution(self, attack: BaseAttack, context: AttackContext) -> ValidationReport:
        ...
        with self._lock:
            applicable_checks = [check for check in self._checks if check.enabled and self._is_check_applicable(check)]
        ...
        LOG.info(f'Pre-execution validation for {attack_id}: {report.overall_result.value} (score: {report.safety_score:.2f})')
        return report

    def validate_post_execution(self, attack: BaseAttack, context: AttackContext, result: AttackResult) -> ValidationReport:
        ...
        if len(self._validation_history) > 1000:
            self._validation_history = self._validation_history[-1000:]
        LOG.info(f'Post-execution validation for {attack_id}: {report.overall_result.value} (score: {report.safety_score:.2f})')
        return report
    ...
    def _is_check_applicable(self, check: ValidationCheck) -> bool:
        ...
        level_order = [ValidationLevel.MINIMAL, ValidationLevel.STANDARD, ValidationLevel.STRICT, ValidationLevel.PARANOID]
    ...
    def _add_recommendations(self, report: ValidationReport) -> None:
        ...
        if report.checks_failed > 0:
            report.recommendations.append('Address all failed validation checks before executing attack')
        if report.checks_warned > 0:
            report.recommendations.append('Review warnings and consider if attack execution is appropriate')
        ...
        for check_name, result, message in report.check_results:
            if result == ValidationResult.WARN:
                if 'experimental' in message.lower():
                    report.recommendations.append('Monitor attack execution closely due to experimental nature')
                elif 'resource' in message.lower():
                    report.recommendations.append('Monitor system resources during attack execution')
                elif 'port' in message.lower():
                    report.recommendations.append('Verify target port is appropriate for your use case')
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            passed = sum((1 for r in self._validation_history if r.overall_result == ValidationResult.PASS))
            warned = sum((1 for r in self._validation_history if r.overall_result == ValidationResult.WARN))
            failed = sum((1 for r in self._validation_history if r.overall_result == ValidationResult.FAIL))
            errored = sum((1 for r in self._validation_history if r.overall_result == ValidationResult.ERROR))
            avg_safety_score = sum((r.safety_score for r in self._validation_history)) / total_validations
            return {'total_validations': total_validations, 'results': {'passed': passed, 'warned': warned, 'failed': failed, 'errored': errored}, 'average_safety_score': avg_safety_score, 'validation_level': self.validation_level.value, 'total_checks': len(self._checks), 'enabled_checks': sum((1 for c in self._checks if c.enabled))}
