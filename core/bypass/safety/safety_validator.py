# recon/core/bypass/safety/safety_validator.py
...
from ..attacks.attack_definition import AttackDefinition, AttackStability, AttackComplexity
from .exceptions import AttackValidationError, SafetyError
...
class ValidationLevel(Enum):
    ...
    STRICT = "strict"        # Strict safety validation
    PARANOID = "paranoid"    # Maximum safety validation
...
class ValidationCheck:
    ...
    def execute(self, attack: BaseAttack, context: AttackContext, 
               result: Optional[AttackResult] = None) -> Tuple[ValidationResult, str]:
...
class ValidationReport:
    ...
    def _calculate_safety_score(self) -> None:
        ...
        total_weight = len(self.check_results)  # Simplified: equal weight for all checks
        score = 0.0
    ...
    def is_safe_to_execute(self) -> bool:
        ...
        return (self.overall_result in [ValidationResult.PASS, ValidationResult.WARN] and
                self.checks_failed == 0 and self.checks_errored == 0)
...
class SafetyValidator:
    ...
    def _initialize_default_checks(self) -> None:
        ...
        # Basic attack structure validation
        ...
        def check_attack_definition(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            # This would require access to attack definition
            # For now, just check basic attack properties
            attack_id = getattr(attack, 'id', 'unknown')
            
            # Check if attack has been marked as unstable
        ...
        def check_resource_prediction(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            ...
            # Predict memory usage based on payload size
            if context.payload:
                payload_size = len(context.payload)
                if payload_size > 10 * 1024:  # 10KB
        ...
        def check_network_safety(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            ...
            if context.dst_ip:
                try:
                    ...
                    # Not a valid IP, might be domain name
                    pass
        ...
        def check_result_safety(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            ...
            if hasattr(result, 'segments') and result.segments:
                ...
                if total_payload_size > 1024 * 1024:  # 1MB
        ...
        # Strict validation checks (only for STRICT and PARANOID levels)
        def check_deterministic_behavior(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            # This would require multiple executions to verify deterministic behavior
            # For now, just check for random elements in parameters
            if context.params:
                for key, value in context.params.items():
        ...
        def check_side_effects(attack: BaseAttack, context: AttackContext, result: Optional[AttackResult]) -> Tuple[ValidationResult, str]:
            ...
            # Check if attack modifies global state
