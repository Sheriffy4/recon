...
class SandboxConstraints:
    ...
    # Network constraints
    allowed_destinations: Set[str] = field(default_factory=set)  # IP addresses or domains
        ...
        with self._lock:
            ...
            if self._counters["file_operations"] > self.constraints.max_file_operations:
                violation = SandboxViolation(
                    ...
                    attack_id=self.attack_id,
                    context={"operation": operation, "path": path, "count": self._counters["file_operations"]},
                    severity="high"
                )
            ...
            for forbidden in self.constraints.forbidden_paths:
                if path_str.startswith(forbidden):
                    violation = SandboxViolation(
                        ...
                        attack_id=self.attack_id,
                        context={"operation": operation, "path": path_str, "forbidden_path": forbidden},
                        severity="critical"
                    )
                    self.violations.append(violation)
                    raise SandboxViolationError(violation.description, violation.violation_type, self.attack_id)
        ...
        with self._lock:
            ...
            if self._counters["network_operations"] > self.constraints.max_network_operations:
                violation = SandboxViolation(
                    ...
                    attack_id=self.attack_id,
                    context={"operation": operation, "destination": destination, "port": port, "count": self._counters["network_operations"]},
                    severity="high"
                )
                self.violations.append(violation)
                raise SandboxViolationError(violation.description, violation.violation_type, self.attack_id)
            ...
            if destination in self.constraints.forbidden_destinations:
                violation = SandboxViolation(
                    ...
                    attack_id=self.attack_id,
                    context={"operation": operation, "destination": destination, "port": port},
                    severity="critical"
                )
                self.violations.append(violation)
                raise SandboxViolationError(violation.description, violation.violation_type, self.attack_id)
...
class AttackSandbox:
    ...
    @contextmanager
    def execute_attack(self, attack: BaseAttack, context: AttackContext, 
    ...
    def _validate_attack_safety(self, attack: BaseAttack, context: AttackContext, monitor: SandboxMonitor) -> None:
        ...
        if context.dst_port:
            ...
            if (monitor.constraints.allowed_ports and 
                context.dst_port not in monitor.constraints.allowed_ports):
