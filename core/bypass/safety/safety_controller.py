...
class SafetyConfiguration:
    ...
    sandbox_constraints: SandboxConstraints = field(default_factory=lambda: SandboxConstraints().get_default_constraints())
...
class ExecutionRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'attack_id': self.attack_id, 'start_time': self.start_time.isoformat(), 'end_time': self.end_time.isoformat() if self.end_time else None, 'duration_seconds': self.duration_seconds, 'safety_components': {'resource_monitor': self.resource_monitor, 'sandbox_monitor': self.sandbox_monitor, 'emergency_stop': self.emergency_stop}, 'validation': {'pre_validation_score': self.pre_validation.safety_score if self.pre_validation else None, 'post_validation_score': self.post_validation.safety_score if self.post_validation else None}, 'result': {'success': self.success, 'status': self.result.status.value if self.result else None, 'error_message': self.error_message}, 'safety_events': {'resource_violations': self.resource_violations, 'sandbox_violations': self.sandbox_violations, 'emergency_stops': self.emergency_stops}}
...
class SafetyController:
    ...
    def __init__(self, config: Optional[SafetyConfiguration]=None):
        ...
        self.resource_manager = ResourceManager(self.config.resource_limits) if self.config.enable_resource_monitoring else None
        self.sandbox = AttackSandbox(self.config.sandbox_constraints) if self.config.enable_sandboxing else None
        self.emergency_stop_manager = EmergencyStopManager() if self.config.enable_emergency_stops else None
        self.validator = SafetyValidator(self.config.validation_level)
        self._active_executions: Dict[str, ExecutionRecord] = {}
        self._execution_history: List[ExecutionRecord] = []
        self._lock = threading.RLock()
        self._stats = {'total_executions': 0, 'successful_executions': 0, 'failed_executions': 0, 'safety_violations': 0, 'emergency_stops': 0}
        ...
        if self.sandbox and self.emergency_stop_manager and self.config.auto_stop_on_violations:
            ...
            def on_sandbox_violation(violation):
                if violation.severity == 'critical':
                    self.emergency_stop_manager.request_stop(violation.attack_id, StopReason.SANDBOX_VIOLATION, f'Critical sandbox violation: {violation.description}', StopPriority.HIGH)
            self.sandbox.add_violation_callback(on_sandbox_violation)
        if self.emergency_stop_manager:

            def on_emergency_stop(stop_event):
                LOG.critical(f'Emergency stop triggered: {stop_event.attack_id} - {stop_event.description}')
                self._stats['emergency_stops'] += 1
            self.emergency_stop_manager.add_global_callback(on_emergency_stop)

    @contextmanager
    def execute_attack_sync(self, attack: BaseAttack, context: AttackContext, timeout: Optional[float]=None):
        """
        ...
        execution_timeout = min(timeout or self.config.default_attack_timeout, self.config.max_attack_timeout)
        ...
        try:
            if self.config.enable_pre_validation:
                ...
                if not pre_validation.is_safe_to_execute() and self.config.fail_on_validation_errors:
                    critical_issues = pre_validation.get_critical_issues()
                    raise AttackValidationError(f"Pre-execution validation failed: {'; '.join(critical_issues)}", critical_issues, attack_id)
            ...
            try:
                if emergency_controller and emergency_controller.is_stop_requested():
                    ...
                    raise EmergencyStopError(f"Emergency stop requested before execution: {stop_info['description']}", stop_info['reason'], attack_id)
                ...
                def execute_with_monitoring():
                    try:
                        ...
                        if sandbox_monitor:
                            sandbox_monitor.record_network_operation('execute', context.dst_ip, context.dst_port)
                ...
                if execution_thread.is_alive():
                    if emergency_controller:
                        emergency_controller.request_stop(StopReason.TIMEOUT, f'Attack execution timed out after {execution_timeout}s', StopPriority.HIGH)
                    raise AttackTimeoutError(f'Attack execution timed out after {execution_timeout}s', execution_timeout, attack_id)
                if exception_container[0]:
                    raise exception_container[0]
                result = result_container[0]
                if emergency_controller and emergency_controller.is_stop_requested():
                    stop_info = emergency_controller.get_stop_info()
                    raise EmergencyStopError(f"Emergency stop triggered during execution: {stop_info['description']}", stop_info['reason'], attack_id)
            except Exception as e:
            ...
            record.success = isinstance(result, AttackResult) and result.status == AttackStatus.SUCCESS
            LOG.info(f"Attack {attack_id} execution completed in {execution_time:.2f}s: {(result.status if result else 'No result')}")
            if self.config.enable_post_validation and result:
                ...
                if not post_validation.is_safe_to_execute() and self.config.fail_on_validation_errors:
                    critical_issues = post_validation.get_critical_issues()
                    raise AttackValidationError(f"Post-execution validation failed: {'; '.join(critical_issues)}", critical_issues, attack_id)
            ...
            with self._lock:
                ...
                if len(self._execution_history) > self.config.max_history_entries:
                    self._execution_history = self._execution_history[-self.config.max_history_entries:]
    ...
    @asynccontextmanager
    async def execute_attack_async(self, attack: BaseAttack, context: AttackContext, timeout: Optional[float]=None):
        """
    ...
    def validate_attack_safety(self, attack: BaseAttack, context: AttackContext) -> ValidationReport:
        """
    ...
    def emergency_stop_attack(self, attack_id: str, reason: str='Manual stop') -> bool:
        ...
        if not self.emergency_stop_manager:
            ...
            return False
        return self.emergency_stop_manager.request_stop(attack_id, StopReason.USER_REQUEST, reason, StopPriority.HIGH)
    ...
    def emergency_stop_all(self, reason: str='Manual stop all') -> int:
        ...
        if not self.emergency_stop_manager:
            LOG.warning('Emergency stop manager not enabled')
            return 0
        return self.emergency_stop_manager.request_stop_all(StopReason.USER_REQUEST, reason, StopPriority.CRITICAL)
    ...
    def get_safety_status(self) -> Dict[str, Any]:
        ...
        status = {'configuration': {'resource_monitoring': self.config.enable_resource_monitoring, 'sandboxing': self.config.enable_sandboxing, 'emergency_stops': self.config.enable_emergency_stops, 'validation_level': self.config.validation_level.value, 'pre_validation': self.config.enable_pre_validation, 'post_validation': self.config.enable_post_validation}, 'statistics': self._stats.copy(), 'active_executions': len(self._active_executions), 'components': {}}
        if self.resource_manager:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._execution_history:
                ...
                total_violations = sum((r.resource_violations + r.sandbox_violations for r in self._execution_history))
                stats.update({'history_success_rate': successful / len(self._execution_history), 'average_execution_time': avg_duration, 'total_safety_violations': total_violations})
            return stats
