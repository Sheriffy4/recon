...
class MetricsCollector:
    ...
    async def record_strategy_result(self, strategy_id: str, domain: str, success: bool, effectiveness: float):
        ...
        metrics.avg_effectiveness = (metrics.avg_effectiveness * (metrics.domain_count - 1) + effectiveness) / metrics.domain_count
    ...
    async def _update_domain_analytics(self, domain: str, strategy_id: str, success: bool, effectiveness: float):
        ...
        analytics.test_history.append({'timestamp': datetime.now().isoformat(), 'strategy_id': strategy_id, 'success': success, 'effectiveness': effectiveness})
    ...
    async def _store_attack_metrics(self, attack_id: str, metrics: AttackMetrics):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('\n            INSERT OR REPLACE INTO attack_metrics \n            (attack_id, success_count, failure_count, total_attempts, \n             avg_response_time, reliability_score, last_success, last_failure, updated_at)\n            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)\n        ', (attack_id, metrics.success_count, metrics.failure_count, metrics.total_attempts, metrics.avg_response_time, metrics.reliability_score, metrics.last_success.isoformat() if metrics.last_success else None, metrics.last_failure.isoformat() if metrics.last_failure else None, datetime.now().isoformat()))
        conn.commit()
        conn.close()
    ...
    async def _store_strategy_metrics(self, strategy_id: str, metrics: StrategyMetrics):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('\n            INSERT OR REPLACE INTO strategy_metrics \n            (strategy_id, domain_count, successful_domains, failed_domains,\n             avg_effectiveness, trend_direction, last_updated)\n            VALUES (?, ?, ?, ?, ?, ?, ?)\n        ', (strategy_id, metrics.domain_count, metrics.successful_domains, metrics.failed_domains, metrics.avg_effectiveness, metrics.trend_direction.value, metrics.last_updated.isoformat()))
        conn.commit()
        conn.close()
    ...
    async def _store_domain_analytics(self, key: str, analytics: DomainAnalytics):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('\n            INSERT OR REPLACE INTO domain_analytics \n            (domain, port, successful_strategies, failed_strategies, best_strategy,\n             avg_success_rate, last_tested, test_history)\n            VALUES (?, ?, ?, ?, ?, ?, ?, ?)\n        ', (analytics.domain, analytics.port, json.dumps(analytics.successful_strategies), json.dumps(analytics.failed_strategies), analytics.best_strategy, analytics.avg_success_rate, analytics.last_tested.isoformat() if analytics.last_tested else None, json.dumps(analytics.test_history)))
        conn.commit()
        conn.close()
    ...
    async def get_attack_metrics(self, attack_id: str) -> Optional[AttackMetrics]:
        ...
        conn.close()
        if row:
            metrics = AttackMetrics(attack_id=row[0], success_count=row[1], failure_count=row[2], total_attempts=row[3], avg_response_time=row[4], reliability_score=row[5], last_success=datetime.fromisoformat(row[6]) if row[6] else None, last_failure=datetime.fromisoformat(row[7]) if row[7] else None)
    ...
    async def get_strategy_metrics(self, strategy_id: str) -> Optional[StrategyMetrics]:
        ...
        conn.close()
        if row:
            metrics = StrategyMetrics(strategy_id=row[0], domain_count=row[1], successful_domains=row[2], failed_domains=row[3], avg_effectiveness=row[4], trend_direction=TrendDirection(row[5]), last_updated=datetime.fromisoformat(row[6]))
    ...
    async def get_realtime_metrics(self) -> RealtimeMetrics:
        ...
        for attack_id, metrics in self.attack_metrics.items():
            if metrics.last_failure and (now - metrics.last_failure).seconds < 3600:
                recent_failures.append({'attack_id': attack_id, 'timestamp': metrics.last_failure.isoformat(), 'failure_rate': metrics.failure_count / metrics.total_attempts if metrics.total_attempts > 0 else 0})
        top_attacks = sorted(self.attack_metrics.items(), key=lambda x: x[1].success_rate, reverse=True)[:5]
        top_strategies = sorted(self.strategy_metrics.items(), key=lambda x: x[1].success_rate, reverse=True)[:5]
        return RealtimeMetrics(timestamp=now, active_attacks=total_attacks, active_strategies=total_strategies, overall_success_rate=overall_success_rate, avg_response_time=avg_response_time, system_health=system_health, recent_failures=recent_failures[:10], top_performing_attacks=[attack_id for attack_id, _ in top_attacks], top_performing_strategies=[strategy_id for strategy_id, _ in top_strategies])

    async def get_metric_history(self, entity_id: str, metric_type: MetricType, hours: int=24) -> List[Dict[str, Any]]:
