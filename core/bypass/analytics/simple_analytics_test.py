    ...
    with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as f:
        with tempfile.TemporaryDirectory() as model_dir:
            ...
            try:
                ...
                print(
                    f"✓ System overview: {overview['system_metrics']['active_attacks']} attacks, {overview['system_metrics']['overall_success_rate']:.2f} success rate"
                )
                ...
                for attack in attacks[:2]:
                    ...
                    if prediction:
                        print(
                            f"  - Prediction for {attack}: {prediction['predicted_value']:.2f} (confidence: {prediction['confidence']:.2f})"
                        )
                    else:
                        print(
