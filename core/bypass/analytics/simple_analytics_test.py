...
# Правильные импорты в зависимости от того, как запускается тест
try:
    # При запуске как модуль
    ...
    from analytics_engine import AnalyticsEngine  # type: ignore[import-not-found]
    from analytics_models import MetricType  # type: ignore[import-not-found]
        ...
        with tempfile.TemporaryDirectory() as model_dir:
            ...
            try:
                ...
                for i, attack in enumerate(attacks):
                    ...
                    await engine.record_attack_result(
                        attack, success, response_time, "example.com"
                    )
                ...
                for attack in attacks:
                    ...
                    if analytics:
                        print(
                            f"  - {attack}: {analytics['metrics']['success_rate']:.2f} success rate"
                        )

                # Test system overview
                overview = await engine.get_system_overview()
                print(
                    f"✓ System overview: {overview['system_metrics']['active_attacks']} attacks, "
                ...
                print(
                    f"✓ Generated report: {len(report.attack_analytics)} attacks analyzed"
                )

                # Test analytics summary
                summary = await engine.get_analytics_summary()
                print(
                    f"✓ Analytics summary: {summary['summary']['system_health_status']} health status"
                )
                ...
                for attack in attacks[:2]:  # Test first 2 attacks
                    prediction = await engine.get_prediction(
                        attack, MetricType.SUCCESS_RATE
                    )
                    if prediction:
                        print(
                            f"  - Prediction for {attack}: {prediction['predicted_value']:.2f} "
                            f"(confidence: {prediction['confidence']:.2f})"
                        )
                    else:
                        print(
                            f"  - No prediction available for {attack} (insufficient data)"
                        )
