...
from .analytics_models import (
    PredictionResult, MetricType, AttackMetrics, StrategyMetrics
)
...
class MLPredictor:
    ...
    def __init__(self, metrics_collector: MetricsCollector, model_dir: str = "ml_models"):
        ...
        self.feature_columns = [
            ...
            'recent_response_time', 'total_attempts', 'failure_streak',
            'time_since_last_success', 'avg_effectiveness'
        ]
    ...
    async def _prepare_strategy_training_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for strategy_id, metrics in self.metrics_collector.strategy_metrics.items():
            ...
            features = [
                ...
                (datetime.now() - metrics.last_updated).total_seconds() / 3600,  # hours since update
    ...
    def _extract_features_from_history(self, history: List[Dict], metrics: AttackMetrics) -> List[float]:
        ...
        features = [
            ...
            (now - metrics.last_success).total_seconds() / 3600 if metrics.last_success else 24,  # time_since_last_success
            np.mean(recent_values) if recent_values else 0  # avg_effectiveness (same as success rate for attacks)
        ]
