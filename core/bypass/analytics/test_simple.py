...
class SimpleAnalytics:
    ...
    def get_system_overview(self):
        ...
        total_attack_successes = sum(
            data["successes"] for data in self.attacks.values()
        )
        total_attack_attempts = sum(
            data["successes"] + data["failures"] for data in self.attacks.values()
        )

        total_strategy_successes = sum(
            data["successes"] for data in self.strategies.values()
        )
        total_strategy_attempts = sum(
            data["successes"] + data["failures"] for data in self.strategies.values()
        )
        ...
        return {
            "attacks": {
                ...
                "success_rate": (
                    total_attack_successes / total_attack_attempts
            ...
            "strategies": {
                ...
                "success_rate": (
                    total_strategy_successes / total_strategy_attempts
        ...
        for attack_id in self.attacks:
            ...
            if stats and stats["success_rate"] < 0.5 and stats["total_attempts"] > 3:
                recommendations.append(
                    f"Review {attack_id} configuration (low success rate)"
                )
...
async def test_analytics():
    ...
    for attack in attacks:
        ...
        if stats:
            print(
                f"  {attack}: {stats['success_rate']:.2%} success, "
    ...
    for strategy in strategies:
        ...
        if stats:
            print(
                f"  {strategy}: {stats['success_rate']:.2%} success, "
    ...
    print(
        f"  Attacks: {overview['attacks']['count']} total, "
        f"{overview['attacks']['success_rate']:.2%} success rate"
    )
    print(
        f"  Strategies: {overview['strategies']['count']} total, "
    ...
    if report["top_attacks"]:
        print(
            f"    Best attack: {report['top_attacks'][0][0]} "
            f"({report['top_attacks'][0][1]:.2%})"
        )

    if report["top_strategies"]:
        print(
            f"    Best strategy: {report['top_strategies'][0][0]} "
...
async def main():
    ...
    print(
        f"  - {overview['strategies']['success_rate']:.1%} overall strategy success rate"
    )
