...
class AnalyticsEngine:
    ...
    def __init__(self, db_path: str='analytics.db', model_dir: str='ml_models'):
        ...
        self.reporting_dashboard = ReportingDashboard(self.metrics_collector, self.performance_tracker, self.ml_predictor)
        self._running = False
        self._tasks = []
        ...
        while self._running:
            try:
                ...
                if self._running:
                    ...
                    await self.reporting_dashboard.generate_comprehensive_report(1)
        ...
        while self._running:
            try:
                ...
                if self._running:
                    ...
                    await self.metrics_collector.cleanup_old_data(30)
    ...
    async def record_attack_result(self, attack_id: str, success: bool, response_time: float, domain: str=None):
        """Record attack execution result"""
        await self.metrics_collector.record_attack_result(attack_id, success, response_time, domain)

    async def record_strategy_result(self, strategy_id: str, domain: str, success: bool, effectiveness: float):
        """Record strategy application result"""
        await self.metrics_collector.record_strategy_result(strategy_id, domain, success, effectiveness)
    ...
    async def get_attack_analytics(self, attack_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {'attack_id': attack_id, 'metrics': {'success_rate': metrics.success_rate, 'total_attempts': metrics.total_attempts, 'avg_response_time': metrics.avg_response_time, 'reliability_score': metrics.reliability_score, 'last_success': metrics.last_success.isoformat() if metrics.last_success else None, 'last_failure': metrics.last_failure.isoformat() if metrics.last_failure else None}, 'performance': performance, 'predictions': {'success_rate': {'value': success_prediction.predicted_value if success_prediction else None, 'confidence': success_prediction.confidence if success_prediction else None}, 'response_time': {'value': response_prediction.predicted_value if response_prediction else None, 'confidence': response_prediction.confidence if response_prediction else None}}}
    ...
    async def get_strategy_analytics(self, strategy_id: str) -> Optional[Dict[str, Any]]:
        ...
        effectiveness_prediction = await self.ml_predictor.predict_strategy_effectiveness(strategy_id)
        return {'strategy_id': strategy_id, 'metrics': {'success_rate': metrics.success_rate, 'domain_count': metrics.domain_count, 'successful_domains': metrics.successful_domains, 'failed_domains': metrics.failed_domains, 'avg_effectiveness': metrics.avg_effectiveness, 'trend_direction': metrics.trend_direction.value, 'last_updated': metrics.last_updated.isoformat()}, 'performance': performance, 'prediction': {'effectiveness': {'value': effectiveness_prediction.predicted_value if effectiveness_prediction else None, 'confidence': effectiveness_prediction.confidence if effectiveness_prediction else None}}}
    ...
    async def get_system_overview(self) -> Dict[str, Any]:
        ...
        top_strategies = await self.performance_tracker.get_top_performers(MetricType.STRATEGY_PERFORMANCE, 5)
        performance_report = await self.performance_tracker.generate_performance_report()
        return {'timestamp': realtime_metrics.timestamp.isoformat(), 'system_metrics': {'active_attacks': realtime_metrics.active_attacks, 'active_strategies': realtime_metrics.active_strategies, 'overall_success_rate': realtime_metrics.overall_success_rate, 'avg_response_time': realtime_metrics.avg_response_time, 'system_health': realtime_metrics.system_health}, 'top_performers': {'attacks': top_attacks, 'strategies': top_strategies}, 'recent_issues': {'failures': realtime_metrics.recent_failures, 'declining_entities': performance_report['declining_entities'], 'volatile_entities': performance_report['volatile_entities']}, 'recommendations': performance_report['recommendations']}
    ...
    async def get_trend_analysis(self, entity_id: str, metric_type: MetricType, hours: int=168) -> Dict[str, Any]:
    ...
    async def get_prediction(self, entity_id: str, metric_type: MetricType, hours_ahead: int=1) -> Optional[Dict[str, Any]]:
        ...
        if metric_type == MetricType.SUCCESS_RATE:
            ...
            prediction = await self.ml_predictor.predict_strategy_effectiveness(entity_id, hours_ahead)
        else:
            return None
        if prediction:
            return {'entity_id': prediction.entity_id, 'metric_type': prediction.metric_type.value, 'predicted_value': prediction.predicted_value, 'confidence': prediction.confidence, 'prediction_horizon': prediction.prediction_horizon, 'created_at': prediction.created_at.isoformat()}
        return None
    ...
    async def get_historical_data(self, entity_id: str, metric_type: MetricType, hours: int=24) -> List[Dict[str, Any]]:
    ...
    async def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        total_entities = system_overview['system_metrics']['active_attacks'] + system_overview['system_metrics']['active_strategies']
        ...
        if system_overview['system_metrics']['system_health'] < 0.4:
            health_status = 'poor'
        return {'summary': {'total_entities_monitored': total_entities, 'overall_success_rate': system_overview['system_metrics']['overall_success_rate'], 'system_health_status': health_status, 'active_issues': len(system_overview['recent_issues']['failures']), 'recommendations_count': len(system_overview['recommendations'])}, 'key_metrics': {'success_rate': system_overview['system_metrics']['overall_success_rate'], 'avg_response_time': system_overview['system_metrics']['avg_response_time'], 'system_health': system_overview['system_metrics']['system_health']}, 'status': {'analytics_engine_running': self._running, 'ml_models_trained': len(self.ml_predictor.trained_models), 'performance_tracking_active': self.performance_tracker._running}}