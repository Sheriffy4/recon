...
async def simulate_bypass_activity(engine: AnalyticsEngine, duration_minutes: int = 5):
    ...
    attacks = [
        "tcp_fragmentation_basic", "tcp_fragmentation_advanced",
        "http_header_manipulation", "http_chunked_encoding",
        "tls_handshake_manipulation", "tls_record_fragmentation",
        "dns_over_https", "dns_cache_poisoning",
        "packet_timing_jitter", "packet_delay_injection",
        "protocol_tunneling", "payload_encryption"
    ]
    
    strategies = [
        "basic_tcp_strategy", "advanced_http_strategy", 
        "tls_evasion_strategy", "dns_tunneling_strategy",
        "timing_based_strategy", "obfuscation_strategy"
    ]
...
async def demonstrate_analytics_features(engine: AnalyticsEngine):
    ...
    for attack_id in attack_ids:
        ...
        if prediction:
            print(f"   {attack_id}: {prediction['predicted_value']:.2%} predicted success rate "
...
async def demonstrate_reporting_features(engine: AnalyticsEngine):
    ...
    if stats['most_successful_attack']:
        print(f"   Best Attack: {stats['most_successful_attack']['id']} "
              f"({stats['most_successful_attack']['success_rate']:.2%})")
    
    if stats['most_successful_strategy']:
        print(f"   Best Strategy: {stats['most_successful_strategy']['id']} "
