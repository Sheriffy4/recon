...
async def demonstrate_analytics_features(engine: AnalyticsEngine):
    ...
    for attack_id in attack_ids:
        ...
        if prediction:
            print(
                f"   {attack_id}: {prediction['predicted_value']:.2%} predicted success rate (confidence: {prediction['confidence']:.2%})"
            )
        else:
...
async def demonstrate_reporting_features(engine: AnalyticsEngine):
    ...
    if stats["most_successful_attack"]:
        print(
            f"   Best Attack: {stats['most_successful_attack']['id']} ({stats['most_successful_attack']['success_rate']:.2%})"
        )
    if stats["most_successful_strategy"]:
        print(
            f"   Best Strategy: {stats['most_successful_strategy']['id']} ({stats['most_successful_strategy']['success_rate']:.2%})"
        )
