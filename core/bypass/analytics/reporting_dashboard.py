...
class ReportingDashboard:
    ...
    async def _calculate_summary_stats(
        self, attack_analytics: Dict, strategy_analytics: Dict, domain_analytics: Dict
    ) -> Dict[str, Any]:
        ...
        health_factors = [
            stats["overall_success_rate"],
            (
                min(1.0, 1.0 / max(stats["avg_response_time"], 0.1))
                if stats["avg_response_time"] > 0
                else 1.0
            ),
            min(1.0, len(attack_analytics) / 50.0),
        ]
        stats["system_health_score"] = sum(health_factors) / len(health_factors)
        return stats

    async def _generate_recommendations(
        self, attack_analytics: Dict, strategy_analytics: Dict, performance_trends: List
    ) -> List[str]:
        ...
        if attack_analytics:
            low_performing_attacks = [
                (attack_id, metrics)
                for attack_id, metrics in attack_analytics.items()
        ...
        if strategy_analytics:
            ineffective_strategies = [
                strategy_id
                for strategy_id, metrics in strategy_analytics.items()
    ...
    async def get_realtime_dashboard_data(self) -> Dict[str, Any]:
        ...
        dashboard_data = {
            ...
            "recent_issues": {"failures": realtime_metrics.recent_failures},
    ...
    async def _generate_entity_recommendations(
        self, entity_id: str, trend: "PerformanceTrend"
    ) -> List[str]:
        ...
        if trend.trend_direction == TrendDirection.DECLINING:
            if trend.metric_type == MetricType.SUCCESS_RATE:
                recommendations.append(
                    f"Success rate declining for {entity_id} - review configuration"
                )
            elif trend.metric_type == MetricType.RESPONSE_TIME:
                recommendations.append(
                    f"Response time increasing for {entity_id} - check performance"
                )
        elif trend.trend_direction == TrendDirection.VOLATILE:
            recommendations.append(
                f"Performance is unstable for {entity_id} - investigate stability"
            )
