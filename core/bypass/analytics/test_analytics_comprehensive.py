...
class TestMetricsCollector:
    ...
    async def test_record_attack_result(self, metrics_collector):
        ...
        await metrics_collector.record_attack_result(
            "tcp_frag_1", True, 1.5, "example.com"
        )
    ...
    async def test_record_strategy_result(self, metrics_collector):
        ...
        await metrics_collector.record_strategy_result(
            "strategy_1", "example.com", True, 0.85
        )
    ...
    async def test_top_performers(self, performance_tracker):
        ...
        if top_performers:
            ...
            for i in range(len(top_performers) - 1):
                assert (
                    top_performers[i]["current_value"]
                    >= top_performers[i + 1]["current_value"]
                )
...
class TestReportingDashboard:
    ...
    async def test_trend_report_generation(self, reporting_dashboard):
        ...
        report = await dashboard.generate_trend_report(
            attack_id, MetricType.SUCCESS_RATE, 24
        )
        ...
        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as f:
            with tempfile.TemporaryDirectory() as model_dir:
                ...
                try:
                    ...
                    assert overview["system_metrics"]["active_strategies"] == len(
                        strategies
                    )
    ...
    async def test_error_handling(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as f:
            ...
            try:
                ...
                await engine.record_attack_result(
                    "", True, -1.0
                )  # Invalid attack ID and response time
                ...
                prediction = await engine.get_prediction(
                    "non_existent", MetricType.SUCCESS_RATE
                )
