...
class PrimitivesAuditor:
    ...
    def _test_fakeddisorder_edge_cases(self) -> Dict[str, Any]:
        ...
        if len(segments3) >= 1:
            ...
            # Should clamp to 0
            if fake_offset != 0:
                issues.append(f"Negative overlap handling failed: expected offset=0, got {fake_offset}")
    ...
    def _analyze_packet_differences(self, zapret_packets, recon_packets) -> Dict[str, Any]:
        ...
        for i in range(min(3, len(zapret_tls), len(recon_tls))):
            ...
            if IP in z_pkt and IP in r_pkt:
                ...
                if z_ip.id != r_ip.id:
                    differences['ip_header_diffs'].append(f"Packet {i}: IP ID differs (zapret: {z_ip.id}, recon: {r_ip.id})")
                
                if z_ip.flags != r_ip.flags:
                    differences['ip_header_diffs'].append(f"Packet {i}: IP flags differ (zapret: {z_ip.flags}, recon: {r_ip.flags})")
                
                if z_ip.ttl != r_ip.ttl:
                    differences['ip_header_diffs'].append(f"Packet {i}: TTL differs (zapret: {z_ip.ttl}, recon: {r_ip.ttl})")
            ...
            if TCP in z_pkt and TCP in r_pkt:
                ...
                if z_tcp.window != r_tcp.window:
                    differences['tcp_header_diffs'].append(f"Packet {i}: Window size differs (zapret: {z_tcp.window}, recon: {r_tcp.window})")
                
                if z_tcp.flags != r_tcp.flags:
                    differences['tcp_header_diffs'].append(f"Packet {i}: TCP flags differ (zapret: {z_tcp.flags}, recon: {r_tcp.flags})")
                ...
                if len(z_options) != len(r_options):
                    differences['tcp_options_diffs'].append(f"Packet {i}: TCP options count differs (zapret: {len(z_options)}, recon: {len(r_options)})")
    ...
    def _test_multisplit(self) -> Dict[str, Any]:
        ...
        if len(segments) != 4:  # Should create 4 segments: [0:10], [10:20], [20:30], [30:]
        ...
        for i, (expected_payload, expected_offset) in enumerate(expected_segments):
            if i < len(segments):
                ...
                if actual_offset != expected_offset:
                    issues.append(f"Segment {i} offset mismatch: expected {expected_offset}, got {actual_offset}")
        ...
        segments_oob = BypassTechniques.apply_multisplit(self.test_payload, [5, len(self.test_payload) + 10])
    ...
    def _create_mock_tcp_packet(self) -> bytearray:
        ...
        tcp_header = bytearray([
            ...
            0x00, 0x00,  # Checksum (will be modified)
    ...
    def _test_badsum_fooling(self, packet: bytearray) -> Dict[str, Any]:
        ...
        original_checksum = struct.unpack('!H', packet[36:38])[0]  # TCP checksum position
    ...
    def audit_other_attacks(self) -> Dict[str, Any]:
        ...
        results = {
            'tlsrec_split_test': None,
            'wssize_limit_test': None,
            'issues': []
        }
    ...
    def _test_wssize_limit(self) -> Dict[str, Any]:
        ...
        for i, (chunk, offset) in enumerate(segments):
            ...
            if len(chunk) != expected_size:
                issues.append(f"Segment {i} size mismatch: expected {expected_size}, got {len(chunk)}")
        ...
        if len(segments_1) != len(self.test_payload):
            issues.append(f"Window size 1 should create {len(self.test_payload)} segments, got {len(segments_1)}")
    ...
    def _generate_fakeddisorder_pcap(self) -> str:
        ...
        for i, (payload, offset, opts) in enumerate(segments):
            pkt = IP(dst="192.168.1.100", ttl=opts.get('ttl', 64)) / TCP(
                dport=443,
                seq=base_seq + offset,
    ...
    def _generate_seqovl_pcap(self) -> str:
        ...
        segments = BypassTechniques.apply_seqovl(self.test_payload, split_pos=15, overlap_size=5)
    ...
    def run_full_audit(self) -> Dict[str, Any]:
        ...
        audit_results['summary'] = {
            ...
            'critical_issues': [issue for issue in all_issues if 'TTL' in issue or 'checksum' in issue]
        }
