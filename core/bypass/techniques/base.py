...
class RaceConditionTechnique(BaseTechnique):
    ...
    @classmethod
    def create_fake_packet(cls, packet_data: bytes, fake_payload: Optional[bytes]=None, ttl: Optional[int]=None, modify_checksum: bool=False) -> bytes:
...
class TimingTechnique(BaseTechnique):
    ...
    @classmethod
    def calculate_delays(cls, num_segments: int, total_time_ms: float, pattern: str='uniform') -> List[float]:
