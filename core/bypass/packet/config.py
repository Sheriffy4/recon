...
class PacketConfig:
    ...
    def validate(self) -> list[str]:
        ...
        if self.badseq_offset_max > 0xFFFFFFFF:
            errors.append(
                f"badseq_offset_max must fit in 32-bit space, got {self.badseq_offset_max}"
            )
        ...
        if self.badseq_offset == -1 or self.badseq_offset == 0xFFFFFFFF:
            errors.append(
                "badseq_offset=-1 creates sequence overlaps! " "Use 0x10000000 (268435456) instead."
            )
...
class PacketConfigManager:
    ...
    def _load_configuration(self):
        ...
        try:
            ...
            if not packet_data:
                self.logger.info("No 'packet' section in configuration. Using defaults.")
                self._config = PacketConfig()
            else:
