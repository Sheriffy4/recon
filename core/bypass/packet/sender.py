...
class PacketSender:
    ...
    def __init__(
        self, builder: PacketBuilder, logger: logging.Logger, inject_mark: int
    ):
    ...
    def _strip_fin_and_normalize(
        ...
        try:
            if (
                not pkt_bytes
                or not original_packet
                or not hasattr(original_packet, "raw")
            ):
                return pkt_bytes
    ...
    def send_tcp_segments(
        self, w, original_packet, specs, window_div=1, ipid_step=2048
    ):
        """
        ...
        try:
            ...
            if not hasattr(original_packet, "raw") or len(original_packet.raw) < 6:
                self.logger.error(
                    "send_tcp_segments: original_packet missing or invalid raw data"
                )
                return False
            ...
            self.logger.error(
                f"send_tcp_segments: Unexpected error - {e}", exc_info=True
            )
            return False
    ...
    def send_udp_datagrams(
        ...
        try:
            ...
            for i, (data, delay_ms) in enumerate(datagrams):
                ip_id = (base_ip_id + i * ipid_step) & 0xFFFF
                pkt_bytes = self.builder.build_udp_datagram(
                    original_packet, data, ip_id=ip_id
                )
                if not pkt_bytes:
                    self.logger.error(
                        f"Datagram {i} build failed, aborting send sequence."
                    )
                    return False
                if not self.safe_send(w, pkt_bytes, original_packet):
                    self.logger.error(
                        f"Datagram {i} send failed, aborting send sequence."
                    )
                    return False
                if delay_ms > 0:
    ...
    def safe_send(
        ...
        try:
            ...
            if getattr(e, "winerror", None) == 258 and allow_fix_checksums:
                self.logger.debug(
                    "WinDivert send timeout (258). Retrying with checksum helper..."
                )
                time.sleep(0.001)
                try:
                    buf = bytearray(pkt_bytes)
    ...
    def _create_tcp_retransmission_blocker(self, original_packet: "pydivert.Packet"):
        ...
        try:
            ...
            self.logger.debug(
                f"🛡️ Creating TCP retransmission blocker with filter: {filter_str}"
            )

            blocker = pydivert.WinDivert(
                filter_str, layer=pydivert.Layer.NETWORK, priority=-100
            )
            blocker.open()
        ...
        try:
            ...
            while not stop_event.is_set():
                try:
                    ...
                    if packet.tcp and (
                        packet.tcp.syn or packet.tcp.fin or packet.tcp.rst
                    ):
                        blocker.send(packet)
                        continue
    ...
    def _send_tcp_segments_threaded(
        ...
        try:
                    ...
                    with self._create_tcp_retransmission_blocker(original_packet):
                        ...
                        for i, (pkt, spec) in enumerate(packets_to_send):
                            ...
                            if not self._batch_safe_send(
                                w, pkt, allow_fix_checksums=allow_fix
                            ):
            ...
            if result_container["error"]:
                self.logger.error(
                    f"Threaded packet sending failed: {result_container['error']}"
                )
                return False
