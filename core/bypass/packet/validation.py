...
def detect_sequence_overlap(
    ...
    if fake_end_64 <= real_seq_64:
        log.debug(
            f"No overlap: FAKE ends (0x{fake_end:08X}) before REAL starts (0x{real_seq:08X})"
        )
        return False
    
    if real_end_64 <= fake_seq_64:
        log.debug(
            f"No overlap: REAL ends (0x{real_end:08X}) before FAKE starts (0x{fake_seq:08X})"
        )
        return False
...
def validate_packet_sequences(
    ...
    if has_overlap:
        ...
        report_lines.append(
            f"💡 Recommendation: Use seq_offset=0x{safe_offset:08X} "
            f"instead of current offset"
        )
        ...
        if seq_diff > 0x80000000:  # Negative offset
            seq_offset = seq_diff - 0x100000000
            report_lines.append(
                f"Sequence difference: {seq_offset} bytes (FAKE before REAL)"
            )
        else:
            report_lines.append(
                f"Sequence difference: +{seq_diff} bytes (FAKE after REAL)"
            )
