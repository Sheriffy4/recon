...
def demo_legacy_migration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        legacy_config = {'strategy': '--dpi-desync=multisplit --dpi-desync-split-count=5 --dpi-desync-split-seqovl=20 --dpi-desync-fooling=badsum --dpi-desync-split-pos=4', 'result_status': 'PARTIAL_SUCCESS', 'successful_sites': 1, 'total_sites': 4, 'success_rate': 0.25, 'avg_latency_ms': 465.984582901001, 'fingerprint_used': True, 'dpi_type': 'unknown', 'dpi_confidence': 0.0}
        ...
        custom_strategy = BypassStrategy(id='demo_custom_strategy', name='Demo Custom Strategy', attacks=['http_host_case', 'tls_sni_modification', 'tcp_fragmentation'], parameters={'host_case_mix': True, 'sni_case_randomization': True, 'split_count': 3}, target_ports=[80, 443])
        custom_pool = manager.create_pool('demo_custom_pool', 'Demo Custom Pool', custom_strategy, 'Custom pool created for demonstration', ['demo.example.com', 'test.example.com'])
        ...
        validation_report = manager.get_validation_report(str(manager.default_config_path))
...
def demo_external_tool_migration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        zapret_config = '--dpi-desync=fake --dpi-desync-fooling=md5sig --dpi-desync-ttl=8'
        ...
        goodbyedpi_config = manager.migrator.migrate_goodbyedpi_config(goodbyedpi_params)
