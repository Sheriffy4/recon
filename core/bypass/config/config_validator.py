...
class ConfigurationValidator:
    ...
    def _get_known_attacks(self) -> Set[str]:
        ...
        return {
            # TCP attacks
            'tcp_fragmentation', 'tcp_multisplit', 'tcp_disorder',
            'tcp_fake_packet', 'tcp_rst_injection', 'tcp_bad_checksum',
            'tcp_md5_signature', 'tcp_bad_sequence',
            
            # HTTP attacks
            'http_host_case', 'http_host_dot', 'http_host_tab',
            'http_host_padding', 'http_domain_case', 'http_method_space',
            'http_unix_eol', 'http_header_modification',
            
            # TLS attacks
            'tls_record_split', 'tls_fragmentation', 'tls_sni_modification',
            'tls_handshake_manipulation', 'tls_version_downgrade',
            
            # DNS attacks
            'dns_tunneling', 'dns_doh_evasion', 'dns_dot_evasion',
            'dns_cache_poisoning', 'dns_modification',
            
            # Timing attacks
            'packet_timing', 'jitter_injection', 'delay_evasion',
            'burst_traffic',
            
            # Obfuscation attacks
            'protocol_tunneling', 'payload_encryption', 'protocol_mimicry',
            'traffic_obfuscation'
        }
    ...
    def _validate_basic_structure(self, config: PoolConfiguration) -> List[ValidationError]:
        ...
        if not isinstance(config.version, ConfigurationVersion):
            errors.append(ValidationError(
                ...
                'version',
                'Use a valid ConfigurationVersion enum value'
            ))
        ...
        if not config.pools:
            errors.append(ValidationError(
                'error',
                'Configuration must contain at least one pool',
                'pools',
                'Add at least one strategy pool'
            ))
    ...
    def _validate_pool(self, pool: StrategyPool, path: str) -> List[ValidationError]:
        ...
        elif not re.match(r'^[a-zA-Z0-9_-]+$', pool.id):
            errors.append(ValidationError(
                'warning',
                'Pool ID should only contain alphanumeric characters, underscores, and hyphens',
                f'{path}.id'
            ))
        ...
        # Validate subdomain strategies
        for subdomain, strategy in pool.subdomains.items():
            errors.extend(self._validate_strategy(
                strategy, 
                f'{path}.subdomains[{subdomain}]'
            ))
        
        # Validate port strategies
        for port, strategy in pool.ports.items():
            if port not in self.valid_ports:
                errors.append(ValidationError(
                    'warning',
                    f'Unusual port number: {port}',
                    f'{path}.ports[{port}]',
                    f'Common ports are: {sorted(self.valid_ports)}'
                ))
        ...
        if not isinstance(pool.priority, int) or pool.priority < 0:
            errors.append(ValidationError(
                'error',
                'Pool priority must be a non-negative integer',
                f'{path}.priority'
            ))
    ...
    def _validate_strategy(self, strategy: BypassStrategy, path: str) -> List[ValidationError]:
        ...
        if not strategy.attacks:
            errors.append(ValidationError(
                'warning',
                'Strategy has no attacks defined',
                f'{path}.attacks',
                'Add at least one attack for the strategy to be effective'
            ))
        ...
        for attack in strategy.attacks:
            if attack not in self.known_attacks:
                errors.append(ValidationError(
                    ...
                    f'{path}.attacks',
                    f'Known attacks: {sorted(self.known_attacks)}'
                ))
        
        # Validate target ports
        ...
        if strategy.compatibility_mode not in valid_modes:
            errors.append(ValidationError(
                'warning',
                f'Unknown compatibility mode: {strategy.compatibility_mode}',
                f'{path}.compatibility_mode',
        ...
        if not (0.0 <= strategy.success_rate <= 1.0):
            errors.append(ValidationError(
                'warning',
                f'Success rate should be between 0.0 and 1.0, got {strategy.success_rate}',
                f'{path}.success_rate'
            ))
    ...
    def _validate_domains(self, domains: List[str], path: str) -> List[ValidationError]:
        ...
        for i, domain in enumerate(domains):
            ...
            if not re.match(r'^[a-zA-Z0-9.-]+$', domain):
                errors.append(ValidationError(
                    'warning',
                    f'Domain contains unusual characters: {domain}',
                    domain_path
                ))
            ...
            if domain.startswith('.') or domain.endswith('.'):
                errors.append(ValidationError(
                    'warning',
                    f'Domain starts or ends with dot: {domain}',
                    domain_path
                ))
    ...
    def _validate_default_pool(self, config: PoolConfiguration) -> List[ValidationError]:
        ...
        if config.default_pool:
            ...
            if config.default_pool not in pool_ids:
                errors.append(ValidationError(
                    'error',
                    f'Default pool "{config.default_pool}" not found in pools',
                    'default_pool',
    ...
    def _validate_auto_assignment_rules(self, rules: List[DomainRule]) -> List[ValidationError]:
        ...
        for i, rule in enumerate(rules):
            ...
            if not rule.pool_id or not isinstance(rule.pool_id, str):
                errors.append(ValidationError(
                    'error',
                    'Rule pool_id must be a non-empty string',
                    f'{rule_path}.pool_id'
                ))
    ...
    def _validate_cross_references(self, config: PoolConfiguration) -> List[ValidationError]:
        ...
        # Check auto-assignment rule references
        for i, rule in enumerate(config.auto_assignment_rules):
            if rule.pool_id not in pool_ids:
                errors.append(ValidationError(
                    'error',
                    f'Auto-assignment rule references non-existent pool: {rule.pool_id}',
                    f'auto_assignment_rules[{i}].pool_id',
                    f'Available pools: {sorted(pool_ids)}'
                ))
