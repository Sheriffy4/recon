...
class ConfigurationMigrator:
    ...
    def _convert_legacy_to_pool(self, legacy_config: LegacyConfiguration) -> PoolConfiguration:
        ...
        default_pool = StrategyPool(
            ...
            strategy=strategy,
            domains=["*"],  # Apply to all domains by default
            priority=1
        )
        
        # Create fallback strategy for failed cases
    ...
    def _parse_zapret_strategy(self, strategy_str: str) -> BypassStrategy:
        ...
        # Parse common zapret parameters
        ...
        split_pos_match = re.search(r'--dpi-desync-split-pos=(\d+)', strategy_str)
        if split_pos_match:
            parameters['split_pos'] = int(split_pos_match.group(1))
        
        seqovl_match = re.search(r'--dpi-desync-split-seqovl=(\d+)', strategy_str)
        if seqovl_match:
        ...
        return BypassStrategy(
            ...
            attacks=list(set(attacks)),  # Remove duplicates
    ...
    def migrate_zapret_config(self, zapret_config: str) -> PoolConfiguration:
        ...
        return PoolConfiguration(
            ...
            metadata={
                "migrated_from": "zapret",
                "original_config": zapret_config
            }
        )
    ...
    def migrate_goodbyedpi_config(self, goodbyedpi_params: List[str]) -> PoolConfiguration:
        ...
        # Map goodbyedpi parameters to attacks
        param_mapping = {
            '-p': 'http_host_padding',
            '-r': 'http_host_removal', 
            '-s': 'tcp_fragmentation',
