    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "test_settings": {
                ...
                "default_timeout": 30,
                "retry_count": 3,
                "results_dir": "recon/data/test_results",
            },
            "stability_settings": {
                "duration_minutes": 10,
                "interval_seconds": 60,
            ...
            "integration_settings": {
                "test_domains": ["httpbin.org", "example.com", "google.com"],
                "test_ports": [80, 443],
...
class TestRunner:
    ...
    async def _run_quick_tests(
        ...
        suite = ComprehensiveTestSuite(
            ...
            self.results_dir,
            self.config.get("test_settings.max_parallel_tests"),
        )
    ...
    async def _run_comprehensive_tests(
        ...
        return await suite.run_comprehensive_tests(
            ...
            stability_duration_minutes=self.config.get(
                "stability_settings.duration_minutes"
            ),
            benchmark_iterations=self.config.get("benchmark_settings.iterations"),
        )
    ...
    async def _run_regression_tests(
        ...
        return await suite.run_comprehensive_tests(
            include_stability=False, include_benchmarks=False, include_regression=True
        )
    ...
    def _get_filtered_attack_ids(self) -> List[str]:
        ...
        # Start with all attacks or enabled only
        ...
        if categories:
            ...
            for category_str in categories:
                try:
                    ...
                    category_attacks = self.attack_registry.list_attacks(
                        category=category
                    )
        ...
        if complexities:
            ...
            for complexity_str in complexities:
                try:
                    ...
                    complexity_attacks = self.attack_registry.list_attacks(
    ...
    def _generate_html_report(self, report: TestReport) -> str:
        ...
        for result in report.test_results:
            status_class = (
                "success" if result.status == TestStatus.PASSED else "failure"
            )
            html += f"""
                <tr>
...
async def main():
    ...
    parser.add_argument(
        "--baseline", type=Path, help="Baseline file for regression tests"
    )
    parser.add_argument(
        "--format",
    ...
    try:
        ...
        print("\nTest Summary:")
        print(
            f"Total: {report.total_tests}, Passed: {report.passed_tests}, "
            f"Failed: {report.failed_tests}, Success Rate: {report.success_rate:.1%}"
        )
