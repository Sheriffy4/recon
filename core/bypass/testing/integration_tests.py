...
try:
    from ..strategies.strategy_application import (
        EnhancedStrategySelector as StrategyApplicationEngine,
    )
except ImportError:
...
class WorkflowIntegrationTester:
    ...
    def __init__(self):
        ...
        if StrategyApplicationEngine and self.pool_manager:
            try:
                self.strategy_engine = StrategyApplicationEngine(
    ...
    async def test_complete_workflow(self) -> TestReport:
        ...
        LOG.info(
            f"Integration test completed: {report.passed_tests}/{report.total_tests} passed"
        )
        return report
    ...
    async def _test_pool_management(self) -> TestResult:
        ...
        try:
            ...
            pool = self.pool_manager.create_pool(
                name="integration_test_pool", description="Pool for integration testing"
            )
            ...
            if len(retrieved_pool.domains) != len(self.test_domains):
                raise Exception(
                    f"Expected {len(self.test_domains)} domains, got {len(retrieved_pool.domains)}"
                )

            # Test pool operations
            self.pool_manager.update_pool_strategy(
                pool.id, {"attack_ids": ["tcp_fragment_basic"]}
            )
    ...
    async def _test_strategy_application(self) -> TestResult:
        ...
        try:
            ...
            for port in [80, 443]:
                strategy = (
                    await self.strategy_engine.select_strategy_for_domain_and_port(
    ...
    async def _test_multi_port_handling(self) -> TestResult:
        ...
        try:
            ...
            for domain in self.test_domains:
                accessible_ports = (
                    await self.multi_port_handler.detect_accessible_ports(domain)
                )
                ...
                for port in [80, 443]:
                    if port in accessible_ports:
                        strategy = (
                            await self.multi_port_handler.get_port_specific_strategy(
                                domain, port
                            )
                        )
                        if strategy:
                            LOG.debug(
                                f"Port-specific strategy found for {domain}:{port}"
                            )
    ...
    async def _test_reliability_validation_integration(self) -> TestResult:
        ...
        try:
            ...
            for domain in self.test_domains:
                ...
                validation_result = (
                    await self.reliability_validator.multi_level_validation(domain)
                )
                if validation_result:
                    LOG.debug(
                        f"Multi-level validation for {domain}: {validation_result}"
                    )
    ...
    async def _test_end_to_end_processing(self) -> TestResult:
        ...
        try:
            ...
            accessible_ports = await self.multi_port_handler.detect_accessible_ports(
                test_domain
            )

            # Step 2: Strategy selection
            strategy = await self.strategy_engine.select_strategy_for_domain(
                test_domain
            )
            ...
            if baseline_accessibility is not None and post_accessibility is not None:
                result.performance_metrics = {
                    ...
                    "accessible_ports": (
                        len(accessible_ports) if accessible_ports else 0
                    ),
