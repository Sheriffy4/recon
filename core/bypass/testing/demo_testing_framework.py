...
async def demo_comprehensive_testing():
    ...
    report = await suite.run_comprehensive_tests(attack_ids=attack_ids, include_stability=True, include_benchmarks=True, include_regression=False, stability_duration_minutes=1, benchmark_iterations=10)
    ...
    for benchmark in report.benchmark_results:
        LOG.info(f'  {benchmark.attack_id}: {benchmark.average_time:.3f}s avg, {benchmark.success_rate:.1%} success')
    return report
...
async def demo_integration_testing():
    ...
    try:
        ...
        LOG.info(f'Integration Test Results: {report.passed_tests}/{report.total_tests} passed')
...
async def demo_category_testing():
    ...
    try:
        ...
        LOG.info(f'Category Test Results: {report.passed_tests}/{report.total_tests} passed')
        return report
    except Exception as e:
...
async def demo_performance_monitoring():
    ...
    def performance_callback(result):
        if result.performance_metrics:
            LOG.info(f"Performance - {result.test_case_id}: {result.duration:.3f}s, Memory: {result.performance_metrics.get('memory_usage_mb', 0):.1f}MB")
    suite.add_test_callback(performance_callback)
    attack_ids = registry.list_attacks(enabled_only=True)[:2]
    if attack_ids:
        report = await suite.run_quick_tests(attack_ids)
        LOG.info(f'Performance Monitoring Results: {report.passed_tests}/{report.total_tests} passed')
        return report
    else:
...
async def run_all_demos():
    ...
    LOG.info('=' * 60)
    demos = [('Basic Testing', demo_basic_testing), ('Comprehensive Testing', demo_comprehensive_testing), ('Integration Testing', demo_integration_testing), ('Test Runner', demo_test_runner), ('Category Testing', demo_category_testing), ('Performance Monitoring', demo_performance_monitoring), ('Error Handling', demo_error_handling)]
    results = {}
