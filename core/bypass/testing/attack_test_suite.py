...
from .test_models import (
    TestCase, TestResult, TestStatus, TestSeverity, ValidationMethod,
    BenchmarkResult, StabilityResult, TestSuite, TestReport
)
from ..attacks.modern_registry import ModernAttackRegistry
from ..attacks.attack_definition import AttackDefinition, AttackCategory, AttackComplexity
        ...
        try:
            ...
            execution_result = {
                ...
                'performance_metrics': {
                    ...
                    'memory_usage_mb': (process.memory_info().rss - initial_memory) / 1024 / 1024,
        ...
        for method in test_case.validation_methods:
            try:
                if method == ValidationMethod.HTTP_RESPONSE:
                    ...
                    validation_results[method] = await self.reliability_validator.validate_domain_accessibility(
    ...
    async def _multi_request_validation(self, test_case: TestCase) -> bool:
        ...
        try:
            ...
            for _ in range(total_requests):
                accessible = await self.reliability_validator.validate_domain_accessibility(
                    test_case.test_domain
                )
                if accessible:
                    success_count += 1
...
class StabilityTester:
    ...
    async def test_attack_stability(self, attack_id: str, duration_minutes: int = 30, 
                                  interval_seconds: int = 60) -> StabilityResult:
    ...
    async def run_regression_tests(self) -> Dict[str, Any]:
        ...
        for test_case_id, baseline_result in self.baseline_results.items():
            try:
                ...
                attack_id = baseline_result.test_case_id.split('_')[0]  # Assuming format: attackid_testcaseid
                attack_def = self.attack_registry.get_attack_definition(attack_id)
...
class ComprehensiveTestSuite:
    ...
    def __init__(self, attack_registry: ModernAttackRegistry, 
                 results_dir: Path = None, max_parallel_tests: int = 5):
        self.attack_registry = attack_registry
    ...
    async def run_comprehensive_tests(self, 
                                    ...
                                    benchmark_iterations: int = 50) -> TestReport:
    ...
    async def _run_stability_tests(self, attack_ids: List[str], 
                                 duration_minutes: int) -> List[StabilityResult]:
    ...
    async def _run_benchmarks(self, attack_ids: List[str], 
