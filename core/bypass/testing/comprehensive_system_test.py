...
from .integration_tests import WorkflowIntegrationTester, ComponentIntegrationTester
...
try:
    from ..strategies.strategy_application import EnhancedStrategySelector
except ImportError:
...
class SystemMetricsCollector:
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            'collection_duration_minutes': (self.metrics[-1].timestamp - self.metrics[0].timestamp).total_seconds() / 60,
    ...
    def _detect_memory_leak(self) -> bool:
        ...
        # If memory is increasing by more than 1MB per measurement, consider it a leak
        return slope > 1.0
    ...
    def _detect_performance_degradation(self) -> float:
        ...
        # Compare first 25% with last 25% of measurements
        ...
        # Return percentage increase in CPU usage
...
class ComprehensiveSystemValidator:
    ...
    async def run_comprehensive_validation(self) -> ComprehensiveValidationReport:
        ...
        try:
            ...
            report = self._generate_comprehensive_report(
                start_time, end_time, attack_results, strategy_results,
                stability_results, integration_results
            )
    ...
    async def _validate_all_attacks(self) -> List[AttackValidationResult]:
        ...
        # Test attacks in parallel batches
    ...
    async def _validate_single_attack(self, attack_id: str) -> AttackValidationResult:
        ...
        try:
            ...
            # Calculate stability score (run multiple times)
    ...
    async def _calculate_attack_performance(self, attack_id: str, iterations: int = 5) -> float:
        ...
        # Performance score: lower execution time = higher score
        avg_time = statistics.mean(execution_times)
        # Normalize to 0-1 scale (assuming 1000ms is baseline)
    ...
    async def _test_strategy_effectiveness(self) -> List[StrategyEffectivenessResult]:
        ...
        for domain in self.test_domains:
            try:
                # Test modern strategy effectiveness
                modern_result = await self._test_modern_strategy_effectiveness(domain)
                
                # Simulate legacy strategy effectiveness (for comparison)
                ...
                if legacy_result['success_rate'] > 0:
                    improvement_percent = ((modern_result['success_rate'] - legacy_result['success_rate']) / 
                                         legacy_result['success_rate']) * 100
                
                if legacy_result['avg_time_ms'] > 0:
                    performance_improvement_percent = ((legacy_result['avg_time_ms'] - modern_result['avg_time_ms']) / 
                                                     legacy_result['avg_time_ms']) * 100
    ...
    async def _test_modern_strategy_effectiveness(self, domain: str) -> Dict[str, float]:
        ...
        for _ in range(iterations):
            try:
                ...
                # Select strategy using modern system
                ...
                if strategy:
                    ...
                    # Test reliability if validator available
    ...
    async def _simulate_legacy_strategy_effectiveness(self, domain: str) -> Dict[str, float]:
        ...
        # Simulate legacy system with lower success rates and higher execution times
        # This is a simulation since we don't have the actual legacy system
        
        # Legacy system assumptions based on typical performance
        base_success_rate = 0.7  # 70% success rate
        base_execution_time = 150  # 150ms average
        base_reliability = 0.6  # 60% reliability
    ...
    async def _test_system_stability(self) -> StabilityTestResult:
        ...
        result = StabilityTestResult(
            ...
            avg_memory_usage_mb=metrics_summary.get('memory_usage_mb', {}).get('avg', 0),
            max_memory_usage_mb=metrics_summary.get('memory_usage_mb', {}).get('max', 0),
            memory_leaks_detected=metrics_summary.get('memory_leak_detected', False),
            system_crashes=system_crashes,
            performance_degradation_percent=metrics_summary.get('performance_degradation', 0)
        )
    ...
    async def _perform_stability_test_batch(self) -> List[bool]:
        ...
        # Test attack registry operations
        ...
        # Convert exceptions to False, successful results to True
    ...
    def _generate_comprehensive_report(self, 
                                     ...
                                     stability_results: StabilityTestResult,
                                     integration_results: Dict[str, Any]) -> ComprehensiveValidationReport:
        ...
        # Calculate overall strategy improvement
    ...
    def _assess_system_readiness(self, 
                               attack_results: List[AttackValidationResult],
                               strategy_results: List[StrategyEffectivenessResult],
                               stability_results: StabilityTestResult,
                               integration_results: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
        ...
        if attack_success_rate < 0.9:  # 90% threshold
            critical_issues.append(f"Attack success rate too low: {attack_success_rate:.1%} (minimum 90% required)")
        
        # Check stability
        if stability_results.error_rate_percent > 5:  # 5% error rate threshold
        ...
        if 'workflow' in integration_results:
            ...
            if workflow_success < 0.95:  # 95% threshold for integration
...
async def main():
    ...
    parser.add_argument('--phase', choices=['all', 'attacks', 'strategy', 'stability'], 
