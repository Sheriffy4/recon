    ...
    def to_native_format(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parameters": {
                ...
                "ttl": self.parameters.get("dpi_desync_ttl", {}).value if "dpi_desync_ttl" in self.parameters else None,
                "repeats": self.parameters.get("dpi_desync_repeats", {}).value if "dpi_desync_repeats" in self.parameters else 1,
                "window_size": self.parameters.get("wssize", {}).value if "wssize" in self.parameters else None,
                "auto_ttl": self.parameters.get("dpi_desync_autottl", {}).value if "dpi_desync_autottl" in self.parameters else None,
                "fake_tls": self.parameters.get("dpi_desync_fake_tls", {}).value if "dpi_desync_fake_tls" in self.parameters else None,
                "fake_http": self.parameters.get("dpi_desync_fake_http", {}).value if "dpi_desync_fake_http" in self.parameters else None,
                "seqovl": self.parameters.get("dpi_desync_split_seqovl", {}).value if "dpi_desync_split_seqovl" in self.parameters else None,
                "badseq_increment": self.parameters.get("dpi_desync_badseq_increment", {}).value if "dpi_desync_badseq_increment" in self.parameters else None
            },
            "metadata": {
                "source": "zapret",
...
class ZapretConfigParser:
    ...
    def _initialize_parameter_definitions(self):
        ...
        self.parameter_defs = {
            ...
            "dpi-desync": {
                ...
                "valid_values": ["split", "fake", "disorder", "split2", "tlsrec", "ipfrag1", "ipfrag2"]
            },
            "dpi-desync-fooling": {
                "type": "list", 
                "description": "Fooling methods for fake packets",
                "valid_values": ["badsum", "badseq", "md5sig", "tcp_md5sig", "hopbyhop", "destopt", "ipfrag1"]
            },
            ...
            "wssize": {
                "type": "integer",
                "description": "TCP window size limit"
            },
            
            # HTTP modifications
            "hostcase": {
                "type": "flag",
                "description": "Change Host header case"
            },
            "hostpad": {
                "type": "flag",
                "description": "Add padding to Host header"
            },
            "methodspace": {
                "type": "flag", 
            ...
            "unixeol": {
                "type": "flag",
                "description": "Use Unix EOL in HTTP headers"
            }
        }
    ...
    def _parse_parameter_value(self, value: Optional[str], param_type: str, param_def: Dict[str, Any]) -> Any:
    ...
    def validate_config(self, config: ZapretConfig) -> List[str]:
        ...
        if "fake" in config.desync_methods:
            if "dpi-desync-ttl" not in config.parameters and "dpi-desync-autottl" not in config.parameters:
                issues.append("Fake method requires TTL setting (--dpi-desync-ttl or --dpi-desync-autottl)")
