    ...
    def to_native_format(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parameters": {
                ...
                "fragment_size": self.parameters.get("fragment-size", {}).value if "fragment-size" in self.parameters else None,
                "window_size": self.parameters.get("window-size", {}).value if "window-size" in self.parameters else None,
                "fake_ttl": self.parameters.get("fake-ttl", {}).value if "fake-ttl" in self.parameters else None,
                "disorder_count": self.parameters.get("disorder-count", {}).value if "disorder-count" in self.parameters else None,
                "split_count": self.parameters.get("split-count", {}).value if "split-count" in self.parameters else None
            },
            "metadata": {
                "source": "byebyedpi",
    ...
    def _parse_parameter_value(self, value: Optional[str], param_type: str, param_def: Dict[str, Any]) -> Any:
        ...
        try:
            if param_type == "integer":
                ...
                if ',' in value:
                    return [int(x.strip()) if x.strip().isdigit() else x.strip() for x in value.split(',')]
                else:
