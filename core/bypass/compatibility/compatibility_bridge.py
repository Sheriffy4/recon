...
class CompatibilityBridge:
    ...
    def _generate_recommendations(
        ...
        if detection.confidence < 0.7:
            recommendations.append(
                f"Low detection confidence ({detection.confidence:.2f}). Consider manually specifying the tool type for better accuracy."
            )
        ...
        if validation_issues:
            recommendations.append(
                f"Configuration has {len(validation_issues)} validation issues. Review parameter combinations and values."
            )
        ...
        if failed_conversions:
            recommendations.append(
                f"Conversion failed for: {', '.join((t.value for t in failed_conversions))}. Some features may not be supported."
            )
    ...
    def suggest_optimal_tool(self, requirements: List[str]) -> Tuple[ExternalTool, str]:
        ...
        return (
            best_tool[0],
            reasoning_map.get(best_tool[0], "Best match for requirements"),
        )
