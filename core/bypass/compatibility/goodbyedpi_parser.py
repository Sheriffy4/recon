    ...
    def to_native_format(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parameters": {
                ...
                "max_payload": self.parameters.get("max-payload", {}).value if "max-payload" in self.parameters else None,
                "set_ttl": self.parameters.get("set-ttl", {}).value if "set-ttl" in self.parameters else None,
                "auto_ttl": "auto-ttl" in self.parameters,
                "wrong_checksum": "wrong-chksum" in self.parameters,
                "wrong_seq": "wrong-seq" in self.parameters,
                "ip_id": self.parameters.get("ip-id", {}).value if "ip-id" in self.parameters else None,
                "blacklist": self.parameters.get("blacklist", {}).value if "blacklist" in self.parameters else None
            },
            "metadata": {
                "source": "goodbyedpi",
...
class GoodbyeDPIParser:
    ...
    def _initialize_parameter_definitions(self):
        ...
        self.long_option_defs = {
            ...
            "auto-ttl": {
                "type": "flag",
                "description": "Automatically determine TTL"
            },
            "wrong-chksum": {
                "type": "flag",
            ...
            "native-frag": {
                "type": "flag",
                "description": "Use native fragmentation"
            },
            "reverse-frag": {
                "type": "flag",
                "description": "Reverse fragment order"
            },
            "ip-id": {
                "type": "integer",
                "description": "Set IP identification field"
            },
            "blacklist": {
                "type": "string",
            ...
            "dnsv6-port": {
                "type": "integer",
                "description": "IPv6 DNS server port"
            }
        }
