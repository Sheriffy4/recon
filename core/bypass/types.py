...
class BlockType(Enum):
    ...
    CONTENT = "content"                     # Обнаружена блокировка по содержимому (синоним HTTP_BLOCK_PAGE)
...
class PacketInfo:
    ...
    def summary(self) -> str:
        ...
        return (f"PacketInfo({self.src_ip}:{self.src_port} -> {self.dst_ip}:{self.dst_port} "
...
class EngineStats:
    ...
    packets: Dict[str, int] = field(default_factory=lambda: {
        "captured": 0, "processed": 0, "bypassed": 0, "dropped": 0, "error": 0
    })
    strategies: Dict[str, int] = field(default_factory=lambda: {
        "applied": 0, "success": 0, "failed": 0
    })
    bytes: Dict[str, int] = field(default_factory=lambda: {
        "processed": 0, "modified": 0
    })
    timing: Dict[str, float] = field(default_factory=lambda: {
        "start_time": time.time(), "total_processing_ms": 0.0
    })

    def to_dict(self) -> Dict[str, Any]:
        uptime = time.time() - self.timing['start_time']
        success_rate = (self.strategies['success'] / self.strategies['applied']
                        if self.strategies['applied'] > 0 else 0.0)
        return {
            "uptime_seconds": uptime,
            "packets": self.packets,
            "strategies": {**self.strategies, "success_rate": success_rate},
            "bytes": self.bytes,
            "avg_processing_ms": (self.timing['total_processing_ms'] / self.packets['processed']
                                  if self.packets['processed'] > 0 else 0.0)
        }

    def merge(self, other: "EngineStats") -> None:
        """Объединяет статистику из другого объекта."""
        for key in self.packets: self.packets[key] += other.packets.get(key, 0)
        for key in self.strategies: self.strategies[key] += other.strategies.get(key, 0)
        for key in self.bytes: self.bytes[key] += other.bytes.get(key, 0)
...
class EnhancedDPIFingerprint:
    ...
    def short_hash(self) -> str:
        ...
        import hashlib
        data_str = (f"{self.dpi_vendor_prediction}:{self.behavioral.rst_injection}:"
