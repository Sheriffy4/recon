...
from .sharing_models import (
    SharedStrategy, StrategyPackage, StrategyFeedback,
    ShareLevel, ValidationStatus, SyncResult
)
...
class CommunityDatabase:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    json.dumps(strategy.strategy_data), strategy.author, strategy.version,
                    strategy.share_level.value, strategy.validation_status.value,
                    strategy.trust_score, strategy.download_count,
                    strategy.success_reports, strategy.failure_reports,
                    json.dumps(strategy.tags), json.dumps(strategy.target_regions),
                    json.dumps(strategy.target_isps), strategy.created_at.isoformat(),
    ...
    async def search_strategies(
        ...
        try:
            ...
            if tags:
                # Simple tag matching - in production would use proper JSON queries
                for tag in tags:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    feedback.strategy_id, feedback.user_id, int(feedback.success),
                    feedback.region, feedback.isp, feedback.notes,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    json.dumps([s.id for s in package.strategies]),
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                return {
                    ...
                    "validated_strategies": row[1],
                    "high_trust_strategies": row[2],
                    "average_trust_score": row[3] or 0.0,
