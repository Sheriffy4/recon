...
class UpdateManager:
        ...
        try:
            if self.config_path.exists():
                ...
                for source_data in data.get('sources', []):
                    source = TrustedSource(id=source_data['id'], name=source_data['name'], url=source_data['url'], public_key=source_data['public_key'], trust_level=TrustLevel(source_data['trust_level']), auto_update=source_data.get('auto_update', False), sync_interval=source_data.get('sync_interval', 3600), enabled=source_data.get('enabled', True))
    ...
    def _create_default_config(self):
        ...
        default_sources = [{'id': 'official_recon', 'name': 'Official Recon Repository', 'url': 'https://api.recon-strategies.org/v1/strategies', 'public_key': 'default_public_key_placeholder', 'trust_level': TrustLevel.VERIFIED.value, 'auto_update': False, 'sync_interval': 3600, 'enabled': False}, {'id': 'community_verified', 'name': 'Community Verified Strategies', 'url': 'https://community.recon-strategies.org/api/verified', 'public_key': 'community_public_key_placeholder', 'trust_level': TrustLevel.HIGH.value, 'auto_update': False, 'sync_interval': 7200, 'enabled': False}]
    ...
    def _save_trusted_sources(self):
        ...
        try:
            ...
            for source in self.trusted_sources.values():
                source_data = {'id': source.id, 'name': source.name, 'url': source.url, 'public_key': source.public_key, 'trust_level': source.trust_level.value, 'auto_update': source.auto_update, 'sync_interval': source.sync_interval, 'enabled': source.enabled}
                if source.last_sync:
    ...
    async def _perform_sync(self, source: TrustedSource) -> SyncResult:
        ...
        try:
            ...
            for strategy_data in strategies:
                try:
                    ...
                    if validation_result.trust_score < min_trust_score:
                        self.logger.warning(f'Strategy {strategy.id} trust score too low: {validation_result.trust_score}')
                        continue
            ...
            self._save_trusted_sources()
            result.success = True
            self.logger.info(f'Sync completed: {result.strategies_added} added, {result.strategies_updated} updated')
        except Exception as e:
    ...
    def _parse_strategy_data(self, data: Dict[str, Any], source: TrustedSource) -> Optional[SharedStrategy]:
        """Parse strategy data from source into SharedStrategy object."""
        try:
            strategy = SharedStrategy(id=data['id'], name=data['name'], description=data.get('description', ''), strategy_data=data['strategy_data'], author=data.get('author', source.name), version=data['version'], share_level=ShareLevel.COMMUNITY, validation_status=ValidationStatus.PENDING, trust_score=0.0, tags=data.get('tags', []), target_regions=data.get('target_regions', []), target_isps=data.get('target_isps', []), signature=data.get('signature'))
    ...
    def _get_min_trust_score_for_level(self, trust_level: TrustLevel) -> float:
        ...
        trust_score_map = {TrustLevel.UNKNOWN: 0.3, TrustLevel.LOW: 0.4, TrustLevel.MEDIUM: 0.6, TrustLevel.HIGH: 0.7, TrustLevel.VERIFIED: 0.8}
    ...
    def get_source_stats(self) -> Dict[str, Any]:
        ...
        return {'total_sources': total, 'enabled_sources': enabled, 'auto_update_sources': auto_update, 'last_sync_times': {s.id: s.last_sync.isoformat() if s.last_sync else None for s in self.trusted_sources.values()}}