...
from .sharing_models import (
    TrustedSource, SharedStrategy, SyncResult, 
    TrustLevel, ValidationStatus, ShareLevel
)
        ...
        try:
            ...
            for strategy_data in strategies:
                try:
                    ...
                    if existing:
                        # Update existing strategy if newer
            ...
            result.success = True
            self.logger.info(
                f"Sync completed: {result.strategies_added} added, "
                f"{result.strategies_updated} updated"
            )
    ...
    def _parse_strategy_data(self, data: Dict[str, Any], source: TrustedSource) -> Optional[SharedStrategy]:
        ...
        try:
            strategy = SharedStrategy(
                ...
                share_level=ShareLevel.COMMUNITY,  # Strategies from trusted sources are community level
                validation_status=ValidationStatus.PENDING,
                trust_score=0.0,  # Will be calculated during validation
                tags=data.get('tags', []),
