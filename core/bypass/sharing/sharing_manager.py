...
class SharingManager:
        ...
        try:
            if self.config_path.exists():
                with open(self.config_path, 'r') as f:
                    data = json.load(f)
                    return SharingConfig(enable_sharing=data.get('enable_sharing', True), enable_auto_updates=data.get('enable_auto_updates', False), default_share_level=ShareLevel(data.get('default_share_level', 'private')), min_trust_score=data.get('min_trust_score', 0.7), max_strategies_per_source=data.get('max_strategies_per_source', 1000), validation_timeout=data.get('validation_timeout', 300), community_db_url=data.get('community_db_url', ''), private_key=data.get('private_key', ''), public_key=data.get('public_key', ''))
            else:
    ...
    def _save_config(self, config: SharingConfig):
        ...
        try:
            ...
            data = {'enable_sharing': config.enable_sharing, 'enable_auto_updates': config.enable_auto_updates, 'default_share_level': config.default_share_level.value, 'min_trust_score': config.min_trust_score, 'max_strategies_per_source': config.max_strategies_per_source, 'validation_timeout': config.validation_timeout, 'community_db_url': config.community_db_url, 'private_key': config.private_key, 'public_key': config.public_key}
    ...
    async def share_strategy(self, strategy_data: Dict[str, Any], name: str, description: str='', tags: List[str]=None, share_level: ShareLevel=None) -> Optional[SharedStrategy]:
        """Share a strategy with the community."""
        if not self.config.enable_sharing:
            self.logger.warning('Strategy sharing is disabled')
            return None
        try:
            strategy = SharedStrategy(id=str(uuid.uuid4()), name=name, description=description, strategy_data=strategy_data, author='local_user', version='1.0.0', share_level=share_level or self.config.default_share_level, validation_status=ValidationStatus.PENDING, trust_score=0.0, tags=tags or [])
    ...
    async def search_strategies(self, query: str='', tags: List[str]=None, author: str='', min_trust_score: float=None, limit: int=50) -> List[SharedStrategy]:
        """Search for strategies in the community database."""
        min_score = min_trust_score or self.config.min_trust_score
        return await self.community_db.search_strategies(query=query, tags=tags, author=author, min_trust_score=min_score, limit=limit)
    ...
    async def submit_feedback(self, strategy_id: str, success: bool, region: str='', isp: str='', notes: str='') -> bool:
        """Submit feedback for a strategy."""
        try:
            feedback = StrategyFeedback(strategy_id=strategy_id, user_id='local_user', success=success, region=region, isp=isp, notes=notes)
            return await self.community_db.add_feedback(feedback)
        except Exception as e:
            self.logger.error(f'Failed to submit feedback: {e}')
            return False

    async def create_strategy_package(self, name: str, description: str, strategy_ids: List[str]) -> Optional[StrategyPackage]:
        ...
        try:
            ...
            if not strategies:
                ...
                return None
            package = StrategyPackage(id=str(uuid.uuid4()), name=name, description=description, strategies=strategies, author='local_user', version='1.0.0')
    ...
    async def add_trusted_source(self, name: str, url: str, public_key: str, trust_level: TrustLevel=TrustLevel.MEDIUM, auto_update: bool=False) -> bool:
        """Add a new trusted source for strategy updates."""
        source = TrustedSource(id=str(uuid.uuid4()), name=name, url=url, public_key=public_key, trust_level=trust_level, auto_update=auto_update)
    ...
    async def get_sharing_stats(self) -> Dict[str, Any]:
        ...
        return {'database': db_stats, 'sources': source_stats, 'validation': validation_stats, 'config': {'sharing_enabled': self.config.enable_sharing, 'auto_updates_enabled': self.config.enable_auto_updates, 'min_trust_score': self.config.min_trust_score}}
    ...
    async def export_strategies(self, strategy_ids: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for sid in strategy_ids:
                strategy = await self.community_db.get_strategy(sid)
                if strategy:
                    strategies.append({'id': strategy.id, 'name': strategy.name, 'description': strategy.description, 'strategy_data': strategy.strategy_data, 'author': strategy.author, 'version': strategy.version, 'tags': strategy.tags, 'signature': strategy.signature})
            return {'export_version': '1.0', 'export_date': datetime.now().isoformat(), 'strategies': strategies}
        except Exception as e:
    ...
    async def import_strategies(self, import_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            for strategy_data in import_data.get('strategies', []):
                strategy = SharedStrategy(id=strategy_data['id'], name=strategy_data['name'], description=strategy_data['description'], strategy_data=strategy_data['strategy_data'], author=strategy_data['author'], version=strategy_data['version'], share_level=ShareLevel.PRIVATE, validation_status=ValidationStatus.PENDING, trust_score=0.0, tags=strategy_data.get('tags', []), signature=strategy_data.get('signature'))
                validation_result = await self.validator.validate_strategy(strategy)
