...
from .sharing_models import SharedStrategy, ShareLevel, ValidationStatus
...
async def test_basic_sharing():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        strategy_data = {
            ...
            "parameters": {"mss": 1200, "header": "User-Agent", "fragment_size": 64},
            "target_ports": [80, 443],
        }
...
async def test_strategy_validation():
    ...
    if not result.is_valid:
        print(
            f"✓ Invalid strategy correctly failed validation (Trust: {result.trust_score:.2f})"
        )
        print(f"  Issues: {result.issues}")
    else:
        print(
            f"✗ Invalid strategy incorrectly passed validation (Trust: {result.trust_score:.2f})"
        )
        print(f"  Issues: {result.issues}")
...
async def test_community_database():
    ...
    try:
        ...
        db.validator.validate_strategy = AsyncMock(
...
async def test_export_import():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager.validator.validate_strategy = AsyncMock(
            return_value=Mock(is_valid=True, trust_score=0.8)
        )
        ...
        if export_data and "strategies" in export_data:
            print(
                f"✓ Successfully exported {len(export_data['strategies'])} strategies"
            )
...
async def test_sharing_stats():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager.community_db.get_database_stats = AsyncMock(
            return_value={
                "total_strategies": 10,
        ...
        manager.update_manager.get_source_stats = Mock(
            return_value={
                "total_sources": 2,
                "enabled_sources": 1,
        ...
        manager.validator.get_validation_stats = Mock(
            return_value={
                "total": 10,
                "valid": 8,
                "invalid": 2,
