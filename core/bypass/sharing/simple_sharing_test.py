...
async def test_basic_sharing():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        strategy_data = {'attacks': ['tcp_fragment', 'http_header_modify'], 'parameters': {'mss': 1200, 'header': 'User-Agent', 'fragment_size': 64}, 'target_ports': [80, 443]}
        print('Sharing strategy...')
        shared_strategy = await manager.share_strategy(strategy_data=strategy_data, name='Test TCP Fragmentation Strategy', description='A test strategy combining TCP fragmentation with HTTP header modification', tags=['tcp', 'http', 'fragmentation', 'test'])
        if shared_strategy:
        ...
        feedback_success = await manager.submit_feedback(strategy_id=shared_strategy.id, success=True, region='US', isp='Test ISP', notes='Strategy works perfectly for bypassing test DPI!')
        if feedback_success:
            print('✓ Successfully submitted feedback')
        else:
            print('✗ Failed to submit feedback')
        print('\nSearching for strategies...')
        search_results = await manager.search_strategies(query='TCP', tags=['tcp'], limit=10)
...
async def test_strategy_validation():
    ...
    if result.is_valid:
        ...
        return False
    invalid_strategy = SharedStrategy(id='invalid_test', name='Invalid Test Strategy', description='An invalid strategy for testing', strategy_data={'attacks': [], 'parameters': 'invalid_format'}, author='test_user', version='1.0.0', share_level=ShareLevel.COMMUNITY, validation_status=ValidationStatus.PENDING, trust_score=0.0)
...
async def test_community_database():
    ...
    try:
        ...
        strategy = SharedStrategy(id='db_test_strategy', name='Database Test Strategy', description='A strategy for testing database operations', strategy_data={'attacks': ['tcp_fragment'], 'parameters': {'mss': 1200}}, author='test_user', version='1.0.0', share_level=ShareLevel.COMMUNITY, validation_status=ValidationStatus.VALIDATED, trust_score=0.8, tags=['tcp', 'database', 'test'])
...
async def test_export_import():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager = SharingManager(str(config_path))
        test_strategy = SharedStrategy(id='export_test_strategy', name='Export Test Strategy', description='A strategy for testing export functionality', strategy_data={'attacks': ['tcp_fragment', 'http_header_modify'], 'parameters': {'mss': 1200, 'header': 'User-Agent'}}, author='test_user', version='1.0.0', share_level=ShareLevel.COMMUNITY, validation_status=ValidationStatus.VALIDATED, trust_score=0.8, tags=['tcp', 'http', 'export'])
...
async def test_sharing_stats():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager = SharingManager(str(config_path))
        manager.community_db.get_database_stats = AsyncMock(return_value={'total_strategies': 10, 'validated_strategies': 8, 'high_trust_strategies': 5, 'average_trust_score': 0.75, 'total_downloads': 150})
        manager.update_manager.get_source_stats = Mock(return_value={'total_sources': 2, 'enabled_sources': 1, 'auto_update_sources': 0})
        manager.validator.get_validation_stats = Mock(return_value={'total': 10, 'valid': 8, 'invalid': 2, 'avg_trust_score': 0.75})
...
async def main():
    ...
    tests = [('Basic Sharing', test_basic_sharing), ('Strategy Validation', test_strategy_validation), ('Community Database', test_community_database), ('Export/Import', test_export_import), ('Sharing Statistics', test_sharing_stats)]
    passed = 0
    total = len(tests)
