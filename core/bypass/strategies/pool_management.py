...
class StrategyPoolManager:
    ...
    def list_pools(self) -> List[StrategyPool]:
        return sorted(self.pools.values(), key=lambda p: (p.priority.value, p.name))
    ...
    def set_subdomain_strategy(
        self, pool_id: str, subdomain: str, strategy: BypassStrategy
    ) -> bool:
        pool = self.pools.get(pool_id)
        if not pool:
            return False
    ...
    def auto_assign_domain(self, domain: str, **kwargs) -> Optional[str]:
        ...
        if self.default_pool_id:
            if self.add_domain_to_pool(self.default_pool_id, domain):
                self.logger.info(
                    f"Auto-assigned '{domain}' to default pool '{self.default_pool_id}'"
                )
    ...
    def merge_pools(
        self, pool_ids: List[str], new_name: str, new_strategy: BypassStrategy
        ...
        merged_pool = self.create_pool(
            new_name, new_strategy, f"Merged from pools: {', '.join(pool_ids)}"
        )
    ...
    def get_pool_statistics(self) -> Dict[str, Any]:
        ...
        for pool in self.pools.values():
            ...
            stats["pools_by_priority"][priority_name] = (
                stats["pools_by_priority"].get(priority_name, 0) + 1
            )
