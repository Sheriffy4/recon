...
class DPIConfig:
    ...
    def _validate_desync_mode(self):
        ...
        if self.desync_mode not in valid_modes:
            raise ValueError(f"Invalid desync_mode: {self.desync_mode}. "
        ...
        for pos in self.split_positions:
            if isinstance(pos, int):
                ...
                if pos.lower() not in ['sni']:
                    raise ValueError(f"Invalid string split position: {pos}. "
    ...
    def _validate_fooling_methods(self):
        ...
        for method in self.fooling_methods:
            if method not in valid_methods:
                raise ValueError(f"Invalid fooling method: {method}. "
    ...
    def has_sni_position(self) -> bool:
        ...
        return any(isinstance(pos, str) and pos.lower() == 'sni' 
                  for pos in self.split_positions)
...
class SplitConfig:
    ...
    def get_effective_positions(self, packet_size: int, sni_position: Optional[int] = None) -> List[int]:
        """
...
class FoolingConfig:
    ...
    def _validate_probabilities(self):
        ...
        if not 0.0 <= self.badsum_probability <= 1.0:
            raise ValueError(f"badsum_probability must be between 0.0 and 1.0: {self.badsum_probability}")
