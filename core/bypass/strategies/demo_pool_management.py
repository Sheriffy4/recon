...
from pool_management import (
    StrategyPoolManager, BypassStrategy, PoolPriority,
    analyze_domain_patterns, suggest_pool_strategies
)
...
def demo_basic_pool_operations():
    ...
    cdn_pool = manager.create_pool(
        "CDN Sites",
        strategies['cdn_bypass'],
        "Strategies for CDN-hosted content"
    )
    
    default_pool = manager.create_pool(
        "Default Sites",
        strategies['tcp_basic'],
    ...
    logger.info(f"Created {len(manager.pools)} pools with total {sum(len(p.domains) for p in manager.pools.values())} domains")
...
def demo_subdomain_strategies(manager, strategies):
    ...
    manager.set_subdomain_strategy(social_pool.id, "api.twitter.com", twitter_api_strategy)
    ...
    for domain, port in test_domains:
        ...
        logger.info(f"  {domain}:{port} -> {strategy.name if strategy else 'No strategy'}")
...
def demo_pool_merging(manager, strategies):
    ...
    video_pool = manager.create_pool(
        "Video Sites",
        strategies['multisplit'],
        "Strategies for video streaming sites"
    )
    ...
    merged_pool = manager.merge_pools(
        [video_pool.id, streaming_pool.id],
        "Media Streaming Sites",
        merged_strategy
    )
    
    logger.info(f"After merge: {len(manager.pools)} pools")
    logger.info(f"Merged pool has {len(merged_pool.domains)} domains and tags: {merged_pool.tags}")
...
def demo_pool_splitting(manager, strategies):
    ...
    logger.info(f"Splitting pool '{target_pool.name}' with {len(target_pool.domains)} domains")
