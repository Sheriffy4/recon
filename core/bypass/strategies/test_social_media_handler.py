...
class TestSocialMediaBypassHandler:
    ...
    async def test_youtube_optimization(self, social_media_handler):
        ...
        for domain, expected_strategy_prefix in test_cases:
            ...
            assert strategy.id.startswith(
                expected_strategy_prefix
            ), f"Wrong strategy for {domain}: {strategy.id}"
    ...
    async def test_twitter_optimization(self, social_media_handler):
        ...
        for domain, expected_strategy_prefix in test_cases:
            ...
            if "twimg.com" in domain:
                assert strategy.parameters.get(
                    "media_subdomain_handling", False
                ), "Media subdomain handling not enabled"
    ...
    async def test_instagram_optimization(self, social_media_handler):
        ...
        test_cases = [
            ...
            ("instagram.com", 80, "instagram_web_optimized"),
        ...
        for domain, port, expected_strategy_prefix in test_cases:
            strategy = await social_media_handler.optimize_instagram_access(
                domain, port
            )
            assert strategy.id.startswith(
                expected_strategy_prefix
            ), f"Wrong strategy for {domain}:{port}: {strategy.id}"
            if port == 80:
                assert strategy.parameters.get(
                    "http_port_fix", False
                ), "HTTP port fix not enabled"
                assert (
                    "http_manipulation" in strategy.attacks
                ), "HTTP manipulation not added for port 80"
    ...
    async def test_tiktok_optimization(self, social_media_handler):
        ...
        for domain, expected_strategy_prefix in test_cases:
            ...
            if "tiktokcdn.com" in domain:
                assert strategy.parameters.get(
                    "cdn_rotation_handling", False
                ), "CDN rotation handling not enabled"
    ...
    async def test_blocking_pattern_detection(self, social_media_handler):
        ...
        test_cases = [
            (
                ...
                443,
                {
                    BlockingPattern.DPI_CONTENT_INSPECTION,
                    BlockingPattern.THROTTLING,
                    BlockingPattern.SNI_BLOCKING,
                },
            ),
            (
                "pbs.twimg.com",
                443,
                {BlockingPattern.CDN_BLOCKING, BlockingPattern.SNI_BLOCKING},
            ),
        ...
        for domain, port, expected_patterns in test_cases:
            detected_patterns = await social_media_handler._detect_blocking_patterns(
                domain, port
            )
            assert detected_patterns.intersection(
                expected_patterns
            ), f"Expected patterns not detected for {domain}:{port}"
    ...
    async def test_blocking_pattern_optimizations(self, social_media_handler):
        ...
        test_cases = [
            ...
            ({BlockingPattern.CDN_BLOCKING}, None),
            ({BlockingPattern.HTTP_HOST_BLOCKING}, "http_manipulation"),
        ]
        for blocking_patterns, expected_attack in test_cases:
            optimized_strategy = (
                social_media_handler._apply_blocking_pattern_optimizations(
                    base_strategy, blocking_patterns
                )
            )
            if expected_attack:
                assert (
                    expected_attack in optimized_strategy.attacks
                ), f"Expected attack {expected_attack} not added"
            if BlockingPattern.CDN_BLOCKING in blocking_patterns:
                assert optimized_strategy.parameters.get(
                    "cdn_optimization", False
                ), "CDN optimization not enabled"
                assert (
                    optimized_strategy.parameters.get("split_count", 0) >= 5
                ), "Split count not increased for CDN blocking"
    ...
    async def test_get_optimized_strategy(self, social_media_handler):
        ...
        for domain, port, media_type in test_cases:
            strategy = await social_media_handler.get_optimized_strategy(
                domain, port, media_type
            )
    ...
    async def test_platform_effectiveness_testing(self, social_media_handler):
        ...
        for domain in test_domains:
            ...
            assert (
                "strategy_id" in results
            ), f"Strategy ID missing from results for {domain}"
            assert "tests" in results, f"Tests missing from results for {domain}"
            assert (
                "overall_success" in results
            ), f"Overall success missing from results for {domain}"
            assert (
                "success_rate" in results
            ), f"Success rate missing from results for {domain}"
            expected_tests = ["connectivity", "speed", "content"]
            for test_name in expected_tests:
                assert (
                    test_name in results["tests"]
                ), f"Test {test_name} missing for {domain}"
                assert (
                    "success" in results["tests"][test_name]
                ), f"Success field missing in {test_name} test for {domain}"
            platform = social_media_handler._detect_platform(domain)
            if platform != PlatformType.GENERIC:
                assert (
                    "platform_specific" in results["tests"]
                ), f"Platform-specific tests missing for {domain}"
    ...
    def test_platform_recommendations(self, social_media_handler):
        ...
        for domain, expected_platform in test_cases:
            ...
            for strategy, confidence, reason in recommendations:
                ...
                ), f"Invalid strategy type for {domain}"
                assert (
                    0.0 <= confidence <= 1.0
                ), f"Invalid confidence score for {domain}: {confidence}"
            ...
            ), f"Recommendations not sorted by confidence for {domain}"
    ...
    def test_configuration_persistence(self, social_media_handler, temp_config_file):
        ...
        new_handler = SocialMediaBypassHandler(
            social_media_handler.subdomain_handler, temp_config_file
        )
        ...
        assert (
            loaded_strategy.platform == PlatformType.YOUTUBE
        ), "Platform not loaded correctly"
        assert (
            loaded_strategy.effectiveness_score == 0.85
        ), "Effectiveness score not loaded correctly"
        assert (
            loaded_strategy.strategy.id == "test_strategy"
        ), "Strategy ID not loaded correctly"
    ...
    async def test_youtube_specific_features(self, social_media_handler):
        ...
        ), "Video acceleration not enabled"
        assert (
            strategy.parameters.get("burst_size", 0) > 0
        ), "Burst size not set for video acceleration"
        ...
        ), "Mobile optimization not enabled"
        assert strategy.parameters.get(
            "cdn_fallback", False
        ), "CDN fallback not enabled for video content"

    @pytest.mark.asyncio
    async def test_instagram_http_port_issues(self, social_media_handler):
        """Test Instagram HTTP port issue handling."""
        strategy_http = await social_media_handler.optimize_instagram_access(
            "instagram.com", 80
        )
        assert strategy_http.parameters.get(
            "http_port_fix", False
        ), "HTTP port fix not enabled"
        assert (
            "http_manipulation" in strategy_http.attacks
        ), "HTTP manipulation not added for port 80"
        strategy_https = await social_media_handler.optimize_instagram_access(
            "instagram.com", 443
        )
        assert (
            not strategy_https.parameters.get("http_port_fix", False) or 443 != 80
        ), "HTTP port fix incorrectly enabled for HTTPS"
    ...
    async def test_twitter_media_subdomain_handling(self, social_media_handler):
        ...
        media_strategy = await social_media_handler.optimize_twitter_access(
            "pbs.twimg.com"
        )
        assert media_strategy.parameters.get(
            "media_subdomain_handling", False
        ), "Media subdomain handling not enabled"
        assert (
            media_strategy.parameters.get("split_count", 0) >= 4
        ), "Split count not increased for media subdomains"
        assert media_strategy.parameters.get(
    ...
    async def test_tiktok_cdn_rotation_handling(self, social_media_handler):
        ...
        cdn_strategy = await social_media_handler.optimize_tiktok_access(
            "v16-web.tiktokcdn.com"
        )
        assert cdn_strategy.parameters.get(
            "cdn_rotation_handling", False
        ), "CDN rotation handling not enabled"
        assert (
            cdn_strategy.parameters.get("split_count", 0) >= 8
        ), "Split count not increased for CDN rotation"
        assert cdn_strategy.parameters.get(
            "mobile_optimization", False
        ), "Mobile optimization not enabled"
    ...
    async def test_live_stream_optimizations(self, social_media_handler):
        ...
        for domain in live_domains:
            ...
            assert (
                jitter_ms <= 10
            ), f"Jitter too high for live stream {domain}: {jitter_ms}ms"
            burst_size = strategy.parameters.get("burst_size", 0)
            assert (
                burst_size >= 3
            ), f"Burst size too low for live stream {domain}: {burst_size}"
    ...
    def test_strategy_effectiveness_updates(self, social_media_handler):
        ...
        test_results = {
            ...
            "success_rate": 0.9,
            "tests": {"speed": {"success": True, "improvement_percent": 35.5}},
        }
        test_strategy = BypassStrategy(
        ...
        social_media_handler._update_strategy_effectiveness(
            "youtube.com", test_strategy, test_results
        )
        assert (
            len(social_media_handler.platform_strategies) > 0
        ), "No platform strategies added"
        added_strategy = None
        for key, platform_strategy in social_media_handler.platform_strategies.items():
            if platform_strategy.strategy.id == test_strategy.id:
                ...
                break
        assert (
            added_strategy is not None
        ), "Test strategy not found in platform strategies"
...
class TestSocialMediaIntegration:
    ...
    def full_setup(self):
        ...
        subdomain_config = tempfile.NamedTemporaryFile(
            mode="w", suffix=".json", delete=False
        )
        social_config = tempfile.NamedTemporaryFile(
            mode="w", suffix=".json", delete=False
        )
        ...
        try:
            ...
            subdomain_handler = SubdomainStrategyHandler(
                pool_manager, subdomain_config.name
            )
            social_handler = SocialMediaBypassHandler(
                subdomain_handler, social_config.name
            )
            yield {
    ...
    async def test_fallback_to_subdomain_handler(self, full_setup):
        ...
        strategy = await social_handler.get_optimized_strategy(
            "unknown-platform.com", 443
        )
        assert strategy is not None, "No fallback strategy provided"
        assert (
            "fallback" in strategy.id or "pool" in strategy.id
        ), "Did not fallback to subdomain handler"
...
class TestSocialMediaPerformance:
    ...
    async def test_effectiveness_testing_performance(self, performance_handler):
        ...
        ), "Concurrent testing not faster than sequential"
        assert len(concurrent_results) == len(
            test_domains
        ), "Not all concurrent tests completed"
    ...
    try:
        ...
        async def test_optimizations():
            for domain in test_domains:
                ...
                print(
                    f"  {domain} -> {strategy.name} ({len(strategy.attacks)} attacks)"
                )
