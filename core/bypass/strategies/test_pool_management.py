# recon/core/bypass/strategies/test_pool_management.py
"""
...
from .pool_management import (
    StrategyPoolManager, StrategyPool, BypassStrategy, DomainRule,
    PoolPriority, analyze_domain_patterns, suggest_pool_strategies
)
...
class TestBypassStrategy:
    ...
    def test_strategy_creation(self):
        ...
        assert strategy.target_ports == [443]  # default
...
class TestDomainRule:
    ...
    def test_rule_creation(self):
        ...
        assert rule.pattern == r".*\.google\.com$"
...
class TestStrategyPoolManager:
    ...
    def setup_method(self):
        """Setup test fixtures"""
        # Use temporary file for configuration
    ...
    def test_add_domain_to_pool(self):
        ...
        # Test adding to non-existent pool
    ...
    def test_get_strategy_for_domain(self):
        ...
        # Test domain not in any pool
        strategy = self.manager.get_strategy_for_domain("unknown.com")
        assert strategy is None  # No fallback set
    ...
    def test_auto_assignment_rules(self):
        ...
        # Add assignment rule
        self.manager.add_assignment_rule(r".*\.google\.com$", pool.id, priority=5)
    ...
    def test_rule_priority(self):
        ...
        self.manager.add_assignment_rule(r".*\.example\.com$", pool1.id, priority=1)
        self.manager.add_assignment_rule(r".*\.example\.com$", pool2.id, priority=5)
    ...
    def test_default_pool(self):
        ...
        # Domain should be assigned to default pool
    ...
    def test_split_pool(self):
        ...
        strategies = {
            "google": BypassStrategy(id="google_strategy", name="Google Strategy", attacks=["tcp_fragmentation"]),
            "social": BypassStrategy(id="social_strategy", name="Social Strategy", attacks=["http_manipulation"])
        }
        
        new_pools = self.manager.split_pool(pool.id, domain_groups, strategies)
        
        assert len(new_pools) == 2
        assert pool.id not in self.manager.pools  # Original pool should be removed
        
        # Check that domains are correctly distributed
    ...
    def test_configuration_save_load(self):
        ...
        pool = self.manager.create_pool("Test Pool", self.test_strategy, "Test description")
        self.manager.add_domain_to_pool(pool.id, "example.com")
        self.manager.add_assignment_rule(r".*\.test\.com$", pool.id, priority=3)
        self.manager.set_default_pool(pool.id)
        ...
        assert new_manager.assignment_rules[0].pattern == r".*\.test\.com$"
