...
class PacketModifier(IPacketModifier):
    ...
    def split_packet(self, packet: bytes, positions: List[int]) -> List[bytes]:
        ...
        logger.debug(
            f"Split packet of size {len(packet)} into {len(parts)} parts at positions {sorted_positions}"
        )
        return parts
    ...
    def _validate_split_result(
        self, original_packet: bytes, parts: List[bytes], positions: List[int]
    ):
        """
    ...
    def create_tcp_segments(
        ...
        try:
            ...
            for i, part in enumerate(parts):
                ...
                logger.debug(
                    f"Created TCP segment {i+1}/{len(parts)} with payload size {len(part)}"
                )
    ...
    def _parse_packet_headers(
        self, packet: bytes
    ) -> Tuple[Optional[bytes], Optional[bytes], bytes]:
        """
        ...
        try:
            ...
            if len(packet) < tcp_start + self.min_tcp_header_size:
                logger.debug("Packet too small for TCP header")
            ...
            logger.debug(
                f"Parsed headers: IP={len(ip_header)}, TCP={len(tcp_header)}, payload={len(payload)}"
            )
    ...
    def _create_tcp_packet(
        self, ip_header: bytes, tcp_header: bytes, payload: bytes, segment_index: int
    ) -> bytes:
        """
        ...
        try:
            ...
            logger.debug(
                f"Created TCP packet: total_len={len(new_packet)}, payload_len={len(payload)}"
            )
            return new_packet
    ...
    def update_sequence_numbers(self, packets: List[bytes]) -> List[bytes]:
        ...
        try:
            ...
            for i, packet in enumerate(packets):
                ...
                if not seq_info:
                    logger.warning(
                        f"Could not extract TCP sequence info from packet {i}, using as-is"
                    )
                ...
                if i == 0:
                    ...
                    logger.debug(
                        f"Packet {i}: updating seq from {seq_info['seq_num']} to {new_seq}"
                    )
                ...
                logger.debug(
                    f"Packet {i}: payload_size={payload_size}, next_offset={current_seq_offset}"
                )

            logger.debug(
                f"Successfully updated sequence numbers for {len(updated_packets)} packets"
            )
