...
def create_mock_registry():
    ...
    class MockAttackRegistry:
        def __init__(self):
            self.attacks = {
                'tcp_fragmentation': MockAttackDefinition('tcp_fragmentation', 'STABLE'),
                'http_manipulation': MockAttackDefinition('http_manipulation', 'STABLE'),
                ...
                'protocol_obfuscation': MockAttackDefinition('protocol_obfuscation', 'EXPERIMENTAL')
            }
...
def demo_basic_functionality():
    ...
    from strategy_application import EnhancedStrategySelector, ConflictResolution
...
def demo_pool_management(selector, pool_manager):
    ...
    pools['cdn'] = pool_manager.create_pool(
        "CDN Sites",
        strategies['cdn'],
        "Optimized for CDN-hosted content"
    )
...
def demo_conflict_resolution(selector):
    ...
    for strategy in conflicting_strategies:
        ...
        print(f"    Last Tested: {strategy.last_tested.strftime('%Y-%m-%d') if strategy.last_tested else 'Never'}")
