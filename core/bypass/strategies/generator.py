...
from core.bypass.attacks.modern_registry import ModernAttackRegistry, get_modern_registry
...
class StrategyGenerator:
    ...
    def __init__(self, attack_registry: ModernAttackRegistry, learning_cache = None): # Optional[AdaptiveLearningCache]
        self.attack_registry = attack_registry
        self.learning_cache = learning_cache
        self.rules = [
            {"if": ["vulnerable_to_fragmentation"], "then": "tcp_multisplit", "score": 0.9, "params": {"split_count": 3}},
            {"if": ["vulnerable_to_fragmentation"], "then": "ip_fragmentation_disorder", "score": 0.85},
            {"if": ["vulnerable_to_sni_case"], "then": "sni_manipulation", "score": 0.95, "params": {"manipulation_type": "random_case"}},
            {"if": ["vulnerable_to_bad_checksum_race"], "then": "badsum_race", "score": 1.0},
            {"if": ["is_stateful", "rst_injection_detected"], "then": "faked_disorder", "score": 0.8},
            {"if": ["is_stateful", "rst_injection_detected"], "then": "seqovl", "score": 0.75},
        ]

    def generate_strategies(self, fingerprint: DPIFingerprint, count: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        if self.learning_cache:
            ...
            for strategy in candidate_strategies:
                if strategy["name"] in rec_map:
                    ...
                    strategy["estimated_score"] = (strategy["estimated_score"] * 0.4) + (historical_success_rate * 0.6)
        ...
        if len(unique_strategies) < count:
            LOG.debug(f"Generated {len(unique_strategies)} strategies from rules, need {count}. Adding generic fallbacks.")
    ...
    def generate_combo_strategies(self, fingerprint: DPIFingerprint, best_single_strategies: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        combo_strategy = {
            ...
            "estimated_score": (strat1.get("estimated_score", 0.5) + strat2.get("estimated_score", 0.5)) / 2 * 0.9, # Combo is slightly less certain
