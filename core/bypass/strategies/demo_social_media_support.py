...
async def demonstrate_platform_detection():
    ...
    subdomain_handler = SubdomainStrategyHandler(
        pool_manager, "demo_subdomain_config.json"
    )
    social_handler = SocialMediaBypassHandler(
        subdomain_handler, "demo_social_config.json"
    )
...
async def demonstrate_youtube_optimization():
    ...
    youtube_domains = [
        ("www.youtube.com", "Main YouTube interface"),
        ("r1---sn-4g5e6nez.googlevideo.com", "Video content delivery"),
        ("i.ytimg.com", "Thumbnail images"),
        ("youtube.com/shorts/abc123", "YouTube Shorts"),
        ("youtube.com/live/stream123", "Live streaming"),
    ]
...
async def demonstrate_twitter_optimization():
    ...
    twitter_domains = [
        ...
        ("upload.twitter.com", "Upload service"),
    ]
...
async def demonstrate_instagram_optimization():
    ...
    instagram_test_cases = [
        ("instagram.com", 443, "Main Instagram interface (HTTPS)"),
        ("instagram.com", 80, "Main Instagram interface (HTTP - with port issues)"),
        ("scontent.cdninstagram.com", 443, "Instagram media CDN"),
        ("instagram.com/stories", 443, "Instagram Stories"),
        ("instagram.com/reels", 443, "Instagram Reels"),
        ("i.instagram.com", 443, "Instagram API"),
    ]
    ...
    for domain, port, description in instagram_test_cases:
        ...
        # Highlight HTTP port issue handling
...
async def demonstrate_tiktok_optimization():
    ...
    tiktok_domains = [
        ...
        ("tiktok.com/live/stream123", "TikTok live streaming"),
...
async def demonstrate_blocking_pattern_detection():
    ...
    test_cases = [
        ("r1---sn-4g5e6nez.googlevideo.com", 443, "YouTube video content"),
        ("pbs.twimg.com", 443, "Twitter media"),
        ("instagram.com", 80, "Instagram HTTP"),
        ("v16-web.tiktokcdn.com", 443, "TikTok CDN"),
    ]
...
async def demonstrate_strategy_recommendations():
    ...
    for domain in test_domains:
        ...
        for i, (strategy, confidence, reason) in enumerate(recommendations[:3], 1):
            ...
            print(
                f"     Attacks: {', '.join(strategy.attacks[:3])}{'...' if len(strategy.attacks) > 3 else ''}"
            )
...
async def demonstrate_configuration_management():
    ...
    # Test some domains to generate platform strategies
    ...
    # Show statistics
    stats = (
        social_handler.get_platform_statistics()
        if hasattr(social_handler, "get_platform_statistics")
        else {}
    )
    ...
    if social_handler.platform_strategies:
        ...
        for key, platform_strategy in list(social_handler.platform_strategies.items())[
            :5
        ]:
            print(
                f"  - {platform_strategy.platform.value}: {platform_strategy.strategy.name}"
            )
            print(f"    Effectiveness: {platform_strategy.effectiveness_score:.1%}")
            if platform_strategy.avg_speed_improvement > 0:
                print(
                    f"    Speed Improvement: {platform_strategy.avg_speed_improvement:.1f}%"
                )
