...
class SubdomainStrategyHandler:
    ...
    def __init__(self, pool_manager: StrategyPoolManager, config_path: str='subdomain_config.json'):
    ...
    def set_subdomain_strategy(self, domain: str, strategy: BypassStrategy, platform: PlatformType=None, subdomain_type: SubdomainType=None) -> bool:
        ...
        if not subdomain_type:
            subdomain_type = self._get_subdomain_type(domain, platform)
        subdomain_strategy = SubdomainStrategy(subdomain=domain, subdomain_type=subdomain_type, platform=platform, strategy=strategy)
        self.subdomain_strategies[domain] = subdomain_strategy
        self._save_configuration()
        LOG.info(f'Set strategy for {domain}: {strategy.name}')
        return True

    def test_subdomain_strategy(self, domain: str, strategy: BypassStrategy=None) -> Dict[str, Any]:
        """
        ...
        if domain in self.subdomain_strategies:
            ...
            self._save_configuration()
        result = {'success': success, 'latency_ms': latency_ms, 'strategy_id': strategy.id, 'timestamp': datetime.now().isoformat()}
        LOG.info(f"Test result for {domain}: {('SUCCESS' if success else 'FAILED')} ({latency_ms:.1f}ms)")
        return result
    ...
    def get_platform_statistics(self) -> Dict[str, Any]:
        ...
        stats = {'total_subdomains': len(self.subdomain_strategies), 'platforms': {}, 'subdomain_types': {}, 'success_rates': {}}
    ...
    def _initialize_platform_configs(self) -> None:
        ...
        youtube_patterns = [SubdomainPattern('^www\\.youtube\\.com$', SubdomainType.WEB_INTERFACE, PlatformType.YOUTUBE, 'Main YouTube interface', 10), SubdomainPattern('^m\\.youtube\\.com$', SubdomainType.WEB_INTERFACE, PlatformType.YOUTUBE, 'Mobile YouTube interface', 9), SubdomainPattern('^.*\\.googlevideo\\.com$', SubdomainType.MEDIA_CONTENT, PlatformType.YOUTUBE, 'YouTube video content', 10, True), SubdomainPattern('^i\\.ytimg\\.com$', SubdomainType.STATIC_ASSETS, PlatformType.YOUTUBE, 'YouTube thumbnails', 8), SubdomainPattern('^.*\\.youtube\\.com$', SubdomainType.API_ENDPOINT, PlatformType.YOUTUBE, 'YouTube API endpoints', 5)]
        youtube_strategies = {SubdomainType.WEB_INTERFACE: BypassStrategy(id='youtube_web', name='YouTube Web Interface', attacks=['http_manipulation', 'tls_evasion'], parameters={'split_pos': 'midsld', 'ttl': 2}), SubdomainType.MEDIA_CONTENT: BypassStrategy(id='youtube_video', name='YouTube Video Content', attacks=['tcp_fragmentation', 'packet_timing'], parameters={'split_count': 5, 'ttl': 3, 'fake_sni': True}), SubdomainType.STATIC_ASSETS: BypassStrategy(id='youtube_assets', name='YouTube Static Assets', attacks=['tcp_fragmentation'], parameters={'split_pos': 3, 'ttl': 1}), SubdomainType.API_ENDPOINT: BypassStrategy(id='youtube_api', name='YouTube API', attacks=['http_manipulation'], parameters={'split_pos': 'midsld', 'ttl': 2})}
        self.platform_configs[PlatformType.YOUTUBE] = PlatformConfiguration(platform=PlatformType.YOUTUBE, base_domains=['youtube.com', 'googlevideo.com', 'ytimg.com'], subdomain_patterns=youtube_patterns, default_strategies=youtube_strategies, special_rules={'video_acceleration': True, 'thumbnail_optimization': True, 'api_rate_limiting': False})
        twitter_patterns = [SubdomainPattern('^(www\\.)?twitter\\.com$', SubdomainType.WEB_INTERFACE, PlatformType.TWITTER, 'Main Twitter interface', 10), SubdomainPattern('^mobile\\.twitter\\.com$', SubdomainType.WEB_INTERFACE, PlatformType.TWITTER, 'Mobile Twitter interface', 9), SubdomainPattern('^.*\\.twimg\\.com$', SubdomainType.MEDIA_CONTENT, PlatformType.TWITTER, 'Twitter media content', 10, True), SubdomainPattern('^api\\.twitter\\.com$', SubdomainType.API_ENDPOINT, PlatformType.TWITTER, 'Twitter API', 8), SubdomainPattern('^upload\\.twitter\\.com$', SubdomainType.UPLOAD, PlatformType.TWITTER, 'Twitter upload service', 7), SubdomainPattern('^abs\\.twimg\\.com$', SubdomainType.STATIC_ASSETS, PlatformType.TWITTER, 'Twitter static assets', 6)]
        twitter_strategies = {SubdomainType.WEB_INTERFACE: BypassStrategy(id='twitter_web', name='Twitter Web Interface', attacks=['http_manipulation', 'tls_evasion'], parameters={'split_pos': 'midsld', 'ttl': 2}), SubdomainType.MEDIA_CONTENT: BypassStrategy(id='twitter_media', name='Twitter Media Content', attacks=['tcp_fragmentation', 'http_manipulation'], parameters={'split_pos': 5, 'ttl': 3, 'fake_sni': True}), SubdomainType.API_ENDPOINT: BypassStrategy(id='twitter_api', name='Twitter API', attacks=['http_manipulation'], parameters={'split_pos': 'midsld', 'ttl': 1}), SubdomainType.UPLOAD: BypassStrategy(id='twitter_upload', name='Twitter Upload', attacks=['tcp_fragmentation', 'packet_timing'], parameters={'split_count': 3, 'ttl': 2}), SubdomainType.STATIC_ASSETS: BypassStrategy(id='twitter_assets', name='Twitter Static Assets', attacks=['tcp_fragmentation'], parameters={'split_pos': 3, 'ttl': 1})}
        self.platform_configs[PlatformType.TWITTER] = PlatformConfiguration(platform=PlatformType.TWITTER, base_domains=['twitter.com', 'twimg.com'], subdomain_patterns=twitter_patterns, default_strategies=twitter_strategies, special_rules={'media_optimization': True, 'api_authentication': True, 'upload_acceleration': True})
        instagram_patterns = [SubdomainPattern('^(www\\.)?instagram\\.com$', SubdomainType.WEB_INTERFACE, PlatformType.INSTAGRAM, 'Main Instagram interface', 10), SubdomainPattern('^.*\\.cdninstagram\\.com$', SubdomainType.MEDIA_CONTENT, PlatformType.INSTAGRAM, 'Instagram media CDN', 10, True), SubdomainPattern('^.*\\.fbcdn\\.net$', SubdomainType.MEDIA_CONTENT, PlatformType.INSTAGRAM, 'Facebook CDN for Instagram', 9, True), SubdomainPattern('^i\\.instagram\\.com$', SubdomainType.API_ENDPOINT, PlatformType.INSTAGRAM, 'Instagram API', 8), SubdomainPattern('^upload\\.instagram\\.com$', SubdomainType.UPLOAD, PlatformType.INSTAGRAM, 'Instagram upload service', 7)]
        instagram_strategies = {SubdomainType.WEB_INTERFACE: BypassStrategy(id='instagram_web', name='Instagram Web Interface', attacks=['http_manipulation', 'tls_evasion'], parameters={'split_pos': 'midsld', 'ttl': 2}), SubdomainType.MEDIA_CONTENT: BypassStrategy(id='instagram_media', name='Instagram Media Content', attacks=['tcp_fragmentation', 'packet_timing'], parameters={'split_count': 7, 'ttl': 3, 'fake_sni': True}), SubdomainType.API_ENDPOINT: BypassStrategy(id='instagram_api', name='Instagram API', attacks=['http_manipulation'], parameters={'split_pos': 'midsld', 'ttl': 1}), SubdomainType.UPLOAD: BypassStrategy(id='instagram_upload', name='Instagram Upload', attacks=['tcp_fragmentation', 'http_manipulation'], parameters={'split_count': 5, 'ttl': 2})}
        self.platform_configs[PlatformType.INSTAGRAM] = PlatformConfiguration(platform=PlatformType.INSTAGRAM, base_domains=['instagram.com', 'cdninstagram.com', 'fbcdn.net'], subdomain_patterns=instagram_patterns, default_strategies=instagram_strategies, special_rules={'media_optimization': True, 'cdn_acceleration': True, 'upload_chunking': True})
        tiktok_patterns = [SubdomainPattern('^(www\\.)?tiktok\\.com$', SubdomainType.WEB_INTERFACE, PlatformType.TIKTOK, 'Main TikTok interface', 10), SubdomainPattern('^.*\\.tiktokcdn\\.com$', SubdomainType.MEDIA_CONTENT, PlatformType.TIKTOK, 'TikTok video CDN', 10, True), SubdomainPattern('^.*\\.musical\\.ly$', SubdomainType.MEDIA_CONTENT, PlatformType.TIKTOK, 'Legacy Musical.ly content', 8, True), SubdomainPattern('^api.*\\.tiktok\\.com$', SubdomainType.API_ENDPOINT, PlatformType.TIKTOK, 'TikTok API', 7)]
        tiktok_strategies = {SubdomainType.WEB_INTERFACE: BypassStrategy(id='tiktok_web', name='TikTok Web Interface', attacks=['http_manipulation', 'tls_evasion'], parameters={'split_pos': 'midsld', 'ttl': 2}), SubdomainType.MEDIA_CONTENT: BypassStrategy(id='tiktok_video', name='TikTok Video Content', attacks=['tcp_fragmentation', 'packet_timing'], parameters={'split_count': 8, 'ttl': 4, 'fake_sni': True}), SubdomainType.API_ENDPOINT: BypassStrategy(id='tiktok_api', name='TikTok API', attacks=['http_manipulation'], parameters={'split_pos': 'midsld', 'ttl': 1})}
        self.platform_configs[PlatformType.TIKTOK] = PlatformConfiguration(platform=PlatformType.TIKTOK, base_domains=['tiktok.com', 'tiktokcdn.com', 'musical.ly'], subdomain_patterns=tiktok_patterns, default_strategies=tiktok_strategies, special_rules={'video_optimization': True, 'mobile_priority': True, 'cdn_acceleration': True})
    ...
    def _get_platform_strategy(self, domain: str, platform: PlatformType, subdomain_type: SubdomainType, port: int) -> Optional[BypassStrategy]:
        ...
        if strategy:
            customized_strategy = BypassStrategy(id=f'{strategy.id}_{domain}', name=f'{strategy.name} for {domain}', attacks=strategy.attacks.copy(), parameters=strategy.parameters.copy(), target_ports=strategy.target_ports.copy() if hasattr(strategy, 'target_ports') else [port], priority=strategy.priority if hasattr(strategy, 'priority') else 1)
            if port == 80:
    ...
    def _cache_subdomain_strategy(self, domain: str, platform: PlatformType, subdomain_type: SubdomainType, strategy: BypassStrategy) -> None:
    ...
    def _get_general_strategies_for_type(self, subdomain_type: SubdomainType) -> List[BypassStrategy]:
        """Get general strategies for a subdomain type."""
        strategies = []
        if subdomain_type == SubdomainType.MEDIA_CONTENT:
            strategies.append(BypassStrategy(id='general_media', name='General Media Strategy', attacks=['tcp_fragmentation', 'packet_timing'], parameters={'split_count': 5, 'ttl': 3}))
        elif subdomain_type == SubdomainType.API_ENDPOINT:
            strategies.append(BypassStrategy(id='general_api', name='General API Strategy', attacks=['http_manipulation'], parameters={'split_pos': 'midsld', 'ttl': 1}))
        elif subdomain_type == SubdomainType.UPLOAD:
            strategies.append(BypassStrategy(id='general_upload', name='General Upload Strategy', attacks=['tcp_fragmentation', 'http_manipulation'], parameters={'split_count': 3, 'ttl': 2}))
        else:
            strategies.append(BypassStrategy(id='general_web', name='General Web Strategy', attacks=['http_manipulation', 'tls_evasion'], parameters={'split_pos': 'midsld', 'ttl': 2}))
        return strategies
            ...
            if config_file.exists():
                ...
                for domain, strategy_data in data.get('subdomain_strategies', {}).items():
                    try:
                        ...
                        subdomain_strategy = SubdomainStrategy(subdomain=domain, subdomain_type=SubdomainType(strategy_data['subdomain_type']), platform=PlatformType(strategy_data['platform']), strategy=strategy, success_rate=strategy_data.get('success_rate', 0.0), avg_latency_ms=strategy_data.get('avg_latency_ms', 0.0), test_count=strategy_data.get('test_count', 0), failure_count=strategy_data.get('failure_count', 0), notes=strategy_data.get('notes', ''))
                        if strategy_data.get('last_tested'):
                            subdomain_strategy.last_tested = datetime.fromisoformat(strategy_data['last_tested'])
                        self.subdomain_strategies[domain] = subdomain_strategy
                    except Exception as e:
                        LOG.error(f'Failed to load strategy for {domain}: {e}')
                LOG.info(f'Loaded {len(self.subdomain_strategies)} subdomain strategies from {self.config_path}')
        except Exception as e:
    ...
    def _save_configuration(self) -> None:
        ...
        try:
            data = {'subdomain_strategies': {}, 'last_updated': datetime.now().isoformat()}
            for domain, subdomain_strategy in self.subdomain_strategies.items():
                strategy_data = {'subdomain_type': subdomain_strategy.subdomain_type.value, 'platform': subdomain_strategy.platform.value, 'strategy': {'id': subdomain_strategy.strategy.id, 'name': subdomain_strategy.strategy.name, 'attacks': subdomain_strategy.strategy.attacks, 'parameters': subdomain_strategy.strategy.parameters}, 'success_rate': subdomain_strategy.success_rate, 'avg_latency_ms': subdomain_strategy.avg_latency_ms, 'test_count': subdomain_strategy.test_count, 'failure_count': subdomain_strategy.failure_count, 'notes': subdomain_strategy.notes}
            ...
            LOG.info(f'Saved {len(self.subdomain_strategies)} subdomain strategies to {self.config_path}')
        except Exception as e:
...
def analyze_subdomain_structure(domain: str) -> Dict[str, Any]:
    ...
    parts = domain.split('.')
    analysis = {'full_domain': domain, 'parts': parts, 'depth': len(parts) - 2 if len(parts) >= 2 else 0, 'tld': parts[-1] if parts else '', 'sld': parts[-2] if len(parts) >= 2 else '', 'subdomains': parts[:-2] if len(parts) > 2 else [], 'is_subdomain': len(parts) > 2, 'subdomain_levels': []}
    if analysis['is_subdomain']:
        for i in range(len(analysis['subdomains'])):
            level_parts = analysis['subdomains'][i:] + [analysis['sld'], analysis['tld']]
...
def suggest_subdomain_tests(domain: str) -> List[Dict[str, Any]]:
    ...
    tests = []
    tests.append({'type': 'connectivity', 'description': f'Test basic connectivity to {domain}', 'ports': [80, 443], 'timeout': 10})
    if platform == PlatformType.YOUTUBE:
        if subdomain_type == SubdomainType.MEDIA_CONTENT:
            tests.append({'type': 'video_streaming', 'description': 'Test video streaming capability', 'test_urls': [f'https://{domain}/test_video'], 'expected_content_type': 'video/*'})
    elif platform == PlatformType.TWITTER:
        if subdomain_type == SubdomainType.MEDIA_CONTENT:
            tests.append({'type': 'image_loading', 'description': 'Test image loading capability', 'test_urls': [f'https://{domain}/test_image.jpg'], 'expected_content_type': 'image/*'})
    recommendations = handler.get_subdomain_recommendations(domain)
    for strategy, confidence in recommendations:
        tests.append({'type': 'strategy_test', 'description': f'Test {strategy.name} strategy', 'strategy': strategy, 'confidence': confidence})
    return tests
