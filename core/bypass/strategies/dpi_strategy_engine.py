...
class DPIStrategyEngine(IDPIStrategy):
        ...
        try:
            ...
            try:
                if not self.should_split_packet(packet):
                    logger.debug(
                        f"Packet of size {len(packet)} does not require DPI strategy application"
                    )
                    return [packet]
    ...
    def get_split_positions(self, packet: bytes) -> List[int]:
        ...
        try:
            ...
            logger.debug(
                f"After priority handling: {prioritized_positions} from original: {positions}"
            )

            # Filter out invalid positions
            valid_positions = [
                pos
                for pos in prioritized_positions
    ...
    def _resolve_position_conflicts(
        ...
        limits the number of splits per packet while preserving priority order.

        Args:
            packet: The packet bytes
            positions: List of candidate positions in priority order

        Returns:
            List of conflict-resolved positions in priority order
        ...
        # but return results in priority order
        min_fragment_size = 3  # Minimum bytes per fragment

        # Create sorted positions for fragment validation
        sorted_positions = sorted(unique_positions)
        valid_sorted_positions = []
    ...
    def _get_sni_position_from_splits(
        self, packet: bytes, split_positions: List[int]
    ) -> Optional[int]:
        """
    ...
    def handle_strategy_failure(
        ...
        logger.error(
            f"Strategy failure in {context}: {error}",
        ...
        if self._is_critical_error(error):
            logger.warning(
                f"Critical error detected in {context}, consider reviewing configuration"
            )
    ...
    def log_strategy_application(
        ...
        log_data = {
            ...
            "is_tls_client_hello": (
                self._sni_detector.is_client_hello(packet) if packet else False
            ),
        }
        ...
        try:
            ...
            if len(result_packets) > 1:
                ...
                if len(reconstructed) != len(original_packet):
                    logger.error(
                        f"Strategy result validation failed: size mismatch. "
