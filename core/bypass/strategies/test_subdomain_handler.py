...
class TestSubdomainPattern(unittest.TestCase):
    ...
    def setUp(self):
        self.youtube_pattern = SubdomainPattern(
            pattern=r"^www\.youtube\.com$",
            subdomain_type=SubdomainType.WEB_INTERFACE,
            platform=PlatformType.YOUTUBE,
            description="Main YouTube interface",
        )

        self.video_pattern = SubdomainPattern(
...
class TestSubdomainStrategy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.subdomain_strategy = SubdomainStrategy(
    ...
    def test_metrics_update_multiple(self):
        ...
        # Latency should be exponential moving average
...
class TestSubdomainStrategyHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create pool manager and handler
    ...
    def test_platform_detection(self):
        ...
        test_cases = [
            ...
            ("r1---sn-4g5e6nls.googlevideo.com", PlatformType.YOUTUBE),
        ...
        for domain, expected_platform in test_cases:
            ...
            self.assertEqual(
                detected_platform, expected_platform, f"Failed for domain: {domain}"
            )

    def test_subdomain_type_detection(self):
        """Test subdomain type detection."""
        test_cases = [
            ("www.youtube.com", PlatformType.YOUTUBE, SubdomainType.WEB_INTERFACE),
            (
                "r1---sn-4g5e6nls.googlevideo.com",
                PlatformType.YOUTUBE,
                SubdomainType.MEDIA_CONTENT,
            ),
            ("i.ytimg.com", PlatformType.YOUTUBE, SubdomainType.STATIC_ASSETS),
            ("pbs.twimg.com", PlatformType.TWITTER, SubdomainType.MEDIA_CONTENT),
            ("api.twitter.com", PlatformType.TWITTER, SubdomainType.API_ENDPOINT),
            ("upload.twitter.com", PlatformType.TWITTER, SubdomainType.UPLOAD),
        ]

        for domain, platform, expected_type in test_cases:
            detected_type = self.handler._get_subdomain_type(domain, platform)
            self.assertEqual(
                detected_type, expected_type, f"Failed for domain: {domain}"
            )
    ...
    def test_strategy_retrieval_youtube(self):
        ...
        for domain in test_domains:
            ...
            self.assertIn(
                "youtube",
                strategy.id.lower(),
                f"Strategy ID should contain 'youtube' for {domain}",
            )
    ...
    def test_strategy_retrieval_twitter(self):
        ...
        for domain in test_domains:
            ...
            self.assertIn(
                "twitter",
                strategy.id.lower(),
                f"Strategy ID should contain 'twitter' for {domain}",
            )
    ...
    def test_strategy_retrieval_instagram(self):
        ...
        for domain in test_domains:
            ...
            self.assertIn(
                "instagram",
                strategy.id.lower(),
                f"Strategy ID should contain 'instagram' for {domain}",
            )
    ...
    def test_auto_discovery(self):
        ...
        for base_domain in base_domains:
            ...
            # Should discover at least some subdomains for known platforms
            if base_domain != "example.com":
                self.assertGreater(
                    len(discovered), 0, f"Should discover subdomains for {base_domain}"
                )
    ...
    def test_recommendations(self):
        ...
        for domain in test_domains:
            ...
            self.assertGreater(
                len(recommendations), 0, f"Should have recommendations for {domain}"
            )
    ...
    def test_configuration_persistence(self):
        ...
        # Set strategy and save
        self.handler.set_subdomain_strategy(domain, strategy)

        # Create new handler and verify strategy is loaded
    ...
    def test_platform_statistics(self):
        ...
        test_strategies = [
            ("www.youtube.com", PlatformType.YOUTUBE, SubdomainType.WEB_INTERFACE),
            ("pbs.twimg.com", PlatformType.TWITTER, SubdomainType.MEDIA_CONTENT),
            ("instagram.com", PlatformType.INSTAGRAM, SubdomainType.WEB_INTERFACE),
        ]

        for domain, platform, subdomain_type in test_strategies:
            strategy = BypassStrategy(
                id=f"test_{domain}",
                name=f"Test Strategy for {domain}",
                attacks=["tcp_fragmentation"],
            )
            self.handler.set_subdomain_strategy(
                domain, strategy, platform, subdomain_type
            )
...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_analyze_subdomain_structure(self):
        ...
        test_cases = [
            ...
            {
                "domain": "r1---sn-4g5e6nls.googlevideo.com",
                "expected": {
                    "depth": 1,
                    "tld": "com",
                    "sld": "googlevideo",
                    "is_subdomain": True,
                    "subdomains": ["r1---sn-4g5e6nls"],
                },
            },
            {
                "domain": "youtube.com",
...
class TestIntegration(unittest.TestCase):
    ...
    def test_fallback_behavior(self):
        ...
        for domain in unknown_domains:
            # Should still get some strategy (fallback)
            strategy = self.manager.get_strategy_for_domain(domain)
            # May be None if no fallback is configured, which is acceptable
...
def run_simple_test():
    ...
    try:
        ...
        print(
            f"✅ YouTube strategy: {youtube_strategy.name if youtube_strategy else 'None'}"
        )

        # Test Twitter
        twitter_strategy = manager.get_strategy_for_domain("pbs.twimg.com")
        print(
            f"✅ Twitter media strategy: {twitter_strategy.name if twitter_strategy else 'None'}"
        )

        # Test Instagram
        instagram_strategy = manager.get_strategy_for_domain(
            "scontent.cdninstagram.com"
        )
        print(
            f"✅ Instagram media strategy: {instagram_strategy.name if instagram_strategy else 'None'}"
        )
