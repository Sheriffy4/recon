...
class SocialMediaBypassHandler:
    ...
    async def get_optimized_strategy(self, domain: str, port: int = 443, 
                                   media_type: MediaType = None) -> Optional[BypassStrategy]:
        """
    ...
    async def _get_platform_optimized_strategy(self, domain: str, port: int, platform: PlatformType,
                                             media_type: MediaType, blocking_patterns: Set[BlockingPattern]) -> Optional[BypassStrategy]:
    ...
    def _apply_blocking_pattern_optimizations(self, strategy: BypassStrategy, 
                                           blocking_patterns: Set[BlockingPattern]) -> BypassStrategy:
    ...
    async def _test_content_accessibility(self, domain: str, strategy: BypassStrategy) -> Dict[str, Any]:
        ...
        return {
            "success": success,
            "content_type": "media" if any(cdn in domain for cdn in ["googlevideo", "twimg", "cdninstagram", "tiktokcdn"]) else "web",
            "accessibility_score": success_rate
        }
    
    async def _test_platform_specific_features(self, domain: str, platform: PlatformType, 
                ...
                for key, strategy_data in data.get('platform_strategies', {}).items():
                    try:
                        ...
                        platform_strategy = PlatformSpecificStrategy(
                            ...
                            last_updated=datetime.fromisoformat(strategy_data['last_updated']) if strategy_data.get('last_updated') else None
                        )
        ...
        try:
            ...
            for key, platform_strategy in self.platform_strategies.items():
                data['platform_strategies'][key] = {
                    ...
                    'last_updated': platform_strategy.last_updated.isoformat() if platform_strategy.last_updated else None
                }
