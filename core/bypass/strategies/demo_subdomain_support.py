...
def demo_subdomain_support():
    ...
    try:
        ...
        sample_domains = ["www.youtube.com", "pbs.twimg.com", "scontent.cdninstagram.com"]
        ...
        print("✅ Platform-specific strategy selection (YouTube, Twitter, Instagram, TikTok)")
...
def demo_real_world_scenarios():
    ...
    try:
        ...
        for domain in youtube_video_domains:
            ...
            print(f"  {domain}")
            print(f"    Strategy: {strategy.name}")
            print(f"    Optimized for: Video streaming with high throughput")
            print(f"    Attacks: {', '.join(strategy.attacks)}")
            
            # Test the strategy
            result = manager.test_subdomain_strategy(domain)
            print(f"    Test result: {'✅ Success' if result['success'] else '❌ Failed'} ({result['latency_ms']:.1f}ms)")
            print()
        ...
        social_platforms = {
            ...
            "Instagram": ["instagram.com", "scontent.cdninstagram.com", "i.instagram.com"],
        ...
        for config in enterprise_configs:
            success = manager.set_subdomain_strategy(config["domain"], config["strategy"])
            if success:
        ...
        print(f"  ⚠️  Needs optimization: {worst_latency['domain']} ({worst_latency['avg_latency']:.1f}ms)")
