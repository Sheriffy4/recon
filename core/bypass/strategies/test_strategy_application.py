...
from ..attacks.attack_definition import (
    AttackDefinition,
    AttackCategory,
    AttackComplexity,
    AttackStability,
...
class TestEnhancedStrategySelector:
    ...
    def setup_method(self):
        ...
        self.selector = EnhancedStrategySelector(
            ...
            attack_registry=self.mock_registry,
            user_preferences_path=self.temp_file.name,
        )
    ...
    def _setup_mock_attacks(self):
        """Set up mock attack definitions."""
        self.mock_attacks = {
            "tcp_fragmentation": AttackDefinition(
                id="tcp_fragmentation",
                name="TCP Fragmentation",
                description="TCP packet fragmentation attack",
                category=AttackCategory.TCP_FRAGMENTATION,
                complexity=AttackComplexity.SIMPLE,
                stability=AttackStability.STABLE,
            ),
            "http_manipulation": AttackDefinition(
                id="http_manipulation",
                name="HTTP Manipulation",
                description="HTTP header manipulation attack",
                category=AttackCategory.HTTP_MANIPULATION,
                complexity=AttackComplexity.MODERATE,
                stability=AttackStability.STABLE,
            ),
            "tls_evasion": AttackDefinition(
                id="tls_evasion",
                name="TLS Evasion",
                description="TLS handshake evasion attack",
        ...
        self.mock_registry.get_attack_definition.side_effect = (
            mock_get_attack_definition
        )
    ...
    def _create_test_strategies(self):
        ...
        self.test_strategies = {
            ...
            "general_strategy": BypassStrategy(
                id="general_strategy",
    ...
    def _create_test_pools(self):
        """Create test pools."""
        # Social media pool
        social_pool = self.pool_manager.create_pool(
            "Social Media",
            self.test_strategies["social_strategy"],
        ...
        # Video platform pool
        video_pool = self.pool_manager.create_pool(
            "Video Platforms",
            self.test_strategies["video_strategy"],
            "For video platforms",
        )
        ...
        # General pool
        general_pool = self.pool_manager.create_pool(
            "General", self.test_strategies["general_strategy"], "For general sites"
        )
    ...
    def test_strategy_selection_with_user_preference(self):
        ...
        assert strategy is not None
        # Should prefer user preference over pool strategy
    ...
    def test_auto_assign_domain(self):
        ...
        # Verify domain was added to appropriate pool
    ...
    def test_conflict_resolution_user_preference(self):
        ...
        strategies = [
            ...
            self.test_strategies["video_strategy"],
            BypassStrategy(
                id="user_pref_test.com",
                name="User Preference for test.com",
                attacks=["tcp_fragmentation"],
                parameters={"ttl": 1},
            ),
        ]

        # Mock the to_zapret_format method for the user preference strategy
        strategies[2].to_zapret_format = lambda: "--dpi-desync=fake --dpi-desync-ttl=1"

        resolved = self.selector.resolve_strategy_conflicts(
            "test.com", strategies, ConflictResolution.USER_PREFERENCE
        )
    ...
    def test_conflict_resolution_success_rate(self):
        ...
        strategies = [
            self.test_strategies["social_strategy"],  # 0.85
            self.test_strategies["video_strategy"],  # 0.75
            self.test_strategies["general_strategy"],  # 0.65
        ]
    ...
    def test_conflict_resolution_pool_priority(self):
        ...
        # Create strategies that belong to pools with different priorities
        strategies = [
            self.test_strategies["social_strategy"],  # HIGH priority
            self.test_strategies["video_strategy"],  # NORMAL priority
            self.test_strategies["general_strategy"],  # LOW priority
        ]

        resolved = self.selector.resolve_strategy_conflicts(
            "test.com", strategies, ConflictResolution.POOL_PRIORITY
        )

        assert resolved.id == "social_strategy"  # Highest priority pool
    ...
    def test_strategy_scoring(self):
        ...
        score = self.selector._score_strategy(
            strategy, "youtube.com", 443, domain_analysis
        )
    ...
    def test_reliability_score_calculation(self):
        ...
        # Should be high since we mocked stable attacks
    ...
    def test_compatibility_score_calculation(self):
        ...
        # Social media domain with social strategy
        domain_analysis = self.selector._analyze_domain("youtube.com")
        strategy = self.test_strategies["social_strategy"]

        score = self.selector._calculate_compatibility_score(strategy, domain_analysis)
        assert score > 0.5  # Should get bonus for social media match

        # Regular domain with general strategy
    ...
    def test_user_preference_conversion(self):
        ...
        user_pref = UserPreference(
            domain="test.com",
            strategy="--dpi-desync=fake --dpi-desync-ttl=1 --dpi-desync-split-pos=3",
            success_rate=0.8,
        )
    ...
    def test_strategy_merging(self):
        ...
        assert len(merged.attacks) >= 2  # Should have attacks from both strategies
        assert len(merged.target_ports) >= 1  # Should have ports from both strategies

        # Check that attacks are unique
    ...
    def test_update_user_preference(self):
        ...
        self.selector.update_user_preference(
            domain="test.com",
            strategy="--dpi-desync=fake --dpi-desync-ttl=1",
            success_rate=0.9,
            latency_ms=200.0,
    ...
    def test_find_similar_pools(self):
        ...
        domain_analysis = self.selector._analyze_domain("tiktok.com")  # Social media

        similar_pools = self.selector._find_similar_pools(domain_analysis)

        # Should find the social media pool
    ...
    def test_pool_creation_for_domain(self):
        ...
        # Should either find existing social media pool or create new one

        # Test unknown domain type
        domain_analysis = self.selector._analyze_domain("unknown-site.xyz")
        pool = self.selector._find_or_create_pool_for_domain(domain_analysis)

        assert pool is not None
        # Should create general pool
    ...
    def test_user_preferences_persistence(self):
        ...
        # Create new selector to test loading
        new_selector = EnhancedStrategySelector(
            pool_manager=self.pool_manager,
            attack_registry=self.mock_registry,
            user_preferences_path=self.temp_file.name,
        )
    ...
    def test_multiple_user_preferences_format(self):
        ...
        test_data = {
            "preferences": {
                ...
                "twitter.com": {
                    "strategy": "--dpi-desync=fake --dpi-desync-ttl=2",
                    "success_rate": 0.75,
...
class TestStrategyScore:
    ...
    def test_strategy_score_with_criteria(self):
        ...
        score = StrategyScore(
            ...
            total_score=0.85,
            criteria_scores={
                SelectionCriteria.SUCCESS_RATE: 0.9,
                SelectionCriteria.LATENCY: 0.8,
            },
            confidence=0.85,
            reasoning=["High success rate", "Low latency"],
        )
...
class TestDomainAnalysis:
    ...
    def test_domain_analysis_creation(self):
        ...
        analysis = DomainAnalysis(
            domain="www.youtube.com", tld="com", sld="youtube.com", subdomains=["www"]
        )
        ...
        assert analysis.sld == "youtube.com"
        assert analysis.subdomains == ["www"]
        assert not analysis.is_social_media  # Default value
        assert analysis.estimated_complexity == 1  # Default value
...
class TestStrategyApplicationIntegration:
    ...
    def test_end_to_end_strategy_selection(self):
        ...
        # Should prefer user preference due to higher success rate

    def test_auto_assignment_workflow(self):
        """Test automatic domain assignment workflow."""
        # 1. Auto-assign a social media domain
        pool_id = self.selector.auto_assign_domain("instagram.com")
        assert pool_id is not None
        ...
        # 3. Auto-assign another social media domain
        pool_id2 = self.selector.auto_assign_domain("tiktok.com")

        # 4. Should be assigned to same pool (or similar pool)
    ...
    def test_conflict_resolution_workflow(self):
        ...
        # 1. Create multiple strategies for same domain
        ...
        # 2. Create pools with different priorities
        ...
        # 3. Add same domain to both pools (simulate conflict)
        self.pool_manager.add_domain_to_pool(pool1.id, "conflict.com")
        # Note: pool manager should handle this by removing from previous pool

        # 4. Resolve conflict
        strategies = [strategy1, strategy2]
        resolved = self.selector.resolve_strategy_conflicts(
            "conflict.com", strategies, ConflictResolution.HIGHEST_SUCCESS_RATE
        )

        # 5. Should select strategy with highest success rate
...
if __name__ == "__main__":
    ...
    # This would normally use pytest, but for now just test imports
    try:
        ...
        selector = EnhancedStrategySelector(
            pool_manager, mock_registry, "/tmp/test_prefs.json"
        )
