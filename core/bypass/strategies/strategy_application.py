...
class UserPreference:
    ...
    @classmethod
    def from_best_strategy_json(cls, data: Dict[str, Any], domain: str='default') -> 'UserPreference':
        """Create UserPreference from best_strategy.json format."""
        return cls(domain=domain, strategy=data.get('strategy', ''), success_rate=data.get('success_rate', 0.0), avg_latency_ms=data.get('avg_latency_ms', 0.0), fingerprint_used=data.get('fingerprint_used', False), dpi_type=data.get('dpi_type', 'unknown'), dpi_confidence=data.get('dpi_confidence', 0.0))
...
class EnhancedStrategySelector:
    ...
    def __init__(self, pool_manager: StrategyPoolManager, attack_registry: ModernAttackRegistry, user_preferences_path: str='recon/best_strategy.json'):
        self.pool_manager = pool_manager
        self.attack_registry = attack_registry
        self.user_preferences_path = user_preferences_path
        self.user_preferences: Dict[str, UserPreference] = {}
        self.selection_weights = {SelectionCriteria.SUCCESS_RATE: 0.3, SelectionCriteria.LATENCY: 0.2, SelectionCriteria.RELIABILITY: 0.2, SelectionCriteria.USER_PREFERENCE: 0.15, SelectionCriteria.COMPATIBILITY: 0.1, SelectionCriteria.FRESHNESS: 0.05}
        self.conflict_resolution_order = [ConflictResolution.USER_PREFERENCE, ConflictResolution.POOL_PRIORITY, ConflictResolution.HIGHEST_SUCCESS_RATE, ConflictResolution.LOWEST_LATENCY]
        self.domain_patterns = {'social_media': ['.*youtube\\.com.*', '.*twitter\\.com.*', '.*instagram\\.com.*', '.*tiktok\\.com.*', '.*facebook\\.com.*', '.*vk\\.com.*', '.*telegram\\.org.*', '.*discord\\.com.*'], 'video_platforms': ['.*youtube\\.com.*', '.*vimeo\\.com.*', '.*twitch\\.tv.*', '.*netflix\\.com.*', '.*hulu\\.com.*'], 'cdn_providers': ['.*cloudflare\\.com.*', '.*fastly\\.com.*', '.*akamai\\.com.*', '.*amazonaws\\.com.*', '.*googleusercontent\\.com.*'], 'news_sites': ['.*bbc\\.com.*', '.*cnn\\.com.*', '.*reuters\\.com.*', '.*nytimes\\.com.*', '.*guardian\\.com.*']}
        self._load_user_preferences()

    def select_strategy(self, domain: str, port: int=443, context: Optional[Dict[str, Any]]=None) -> Optional[BypassStrategy]:
        """
        ...
        LOG.info(f"Selected strategy '{best_score.strategy_id}' for {domain} (score: {best_score.total_score:.3f}, confidence: {best_score.confidence:.3f})")
    ...
    def resolve_strategy_conflicts(self, domain: str, conflicting_strategies: List[BypassStrategy], resolution_method: ConflictResolution=None) -> BypassStrategy:
        """
        ...
        LOG.info(f'Resolving strategy conflict for {domain} ({len(conflicting_strategies)} strategies)')
    ...
    def update_user_preference(self, domain: str, strategy: str, success_rate: float=None, latency_ms: float=None, **kwargs) -> None:
        """
        ...
        """
        preference = UserPreference(domain=domain, strategy=strategy, success_rate=success_rate or 0.0, avg_latency_ms=latency_ms or 0.0, last_updated=datetime.now(), **kwargs)
        self.user_preferences[domain] = preference
        self._save_user_preferences()
        LOG.info(f'Updated user preference for {domain}: {strategy}')

    def get_strategy_recommendations(self, domain: str, count: int=3) -> List[Tuple[BypassStrategy, float]]:
        """
    ...
    def _analyze_domain(self, domain: str) -> DomainAnalysis:
        ...
        parts = domain.split('.')
        analysis = DomainAnalysis(domain=domain, tld=parts[-1] if parts else '', sld='.'.join(parts[-2:]) if len(parts) >= 2 else domain, subdomains=parts[:-2] if len(parts) > 2 else [])
    ...
    def _get_candidate_strategies(self, domain: str, port: int, domain_analysis: DomainAnalysis) -> List[BypassStrategy]:
    ...
    def _score_strategy(self, strategy: BypassStrategy, domain: str, port: int, domain_analysis: DomainAnalysis, context: Optional[Dict[str, Any]]=None) -> StrategyScore:
    ...
    def _calculate_reliability_score(self, strategy: BypassStrategy) -> float:
        ...
        for attack_id in strategy.attacks:
            ...
            if definition:
                stability_scores = {'STABLE': 1.0, 'MOSTLY_STABLE': 0.8, 'EXPERIMENTAL': 0.6, 'UNSTABLE': 0.3, 'DEPRECATED': 0.1}
                stability_name = definition.stability.name if hasattr(definition.stability, 'name') else str(definition.stability)
    ...
    def _calculate_compatibility_score(self, strategy: BypassStrategy, domain_analysis: DomainAnalysis) -> float:
        ...
        if domain_analysis.is_social_media and 'social' in strategy.name.lower():
            score += 0.3
        if domain_analysis.is_video_platform and any((attack in strategy.attacks for attack in ['multisplit', 'timing'])):
            score += 0.2
    ...
    def _find_or_create_pool_for_domain(self, domain_analysis: DomainAnalysis) -> Optional[StrategyPool]:
        ...
        if domain_analysis.is_social_media:
            ...
            return self.pool_manager.create_pool('Social Media Sites', strategy, 'Automatically created for social media domains')
        elif domain_analysis.is_video_platform:
            strategy = self._create_video_platform_strategy()
            return self.pool_manager.create_pool('Video Platforms', strategy, 'Automatically created for video platform domains')
        elif domain_analysis.is_cdn:
            strategy = self._create_cdn_strategy()
            return self.pool_manager.create_pool('CDN Sites', strategy, 'Automatically created for CDN domains')
        else:
            strategy = self._create_general_strategy()
            return self.pool_manager.create_pool('General Sites', strategy, 'Automatically created for general domains')

    def _generate_strategies_for_domain(self, domain_analysis: DomainAnalysis) -> List[BypassStrategy]:
    ...
    def _create_social_media_strategy(self) -> BypassStrategy:
        ...
        return BypassStrategy(id='auto_social_media', name='Auto Social Media Strategy', attacks=['http_manipulation', 'tls_evasion'], parameters={'split_pos': 'midsld', 'ttl': 2}, target_ports=[443, 80], priority=2)

    def _create_video_platform_strategy(self) -> BypassStrategy:
        """Create strategy optimized for video platforms."""
        return BypassStrategy(id='auto_video_platform', name='Auto Video Platform Strategy', attacks=['tcp_fragmentation', 'packet_timing'], parameters={'split_count': 5, 'ttl': 3}, target_ports=[443, 80, 8080], priority=2)

    def _create_cdn_strategy(self) -> BypassStrategy:
        """Create strategy optimized for CDN sites."""
        return BypassStrategy(id='auto_cdn', name='Auto CDN Strategy', attacks=['tcp_fragmentation'], parameters={'split_pos': 3, 'ttl': 1}, target_ports=[443, 80], priority=1)

    def _create_general_strategy(self) -> BypassStrategy:
        """Create general-purpose strategy."""
        return BypassStrategy(id='auto_general', name='Auto General Strategy', attacks=['tcp_fragmentation'], parameters={'split_pos': 3, 'ttl': 2}, target_ports=[443, 80], priority=1)
    ...
    def _convert_user_preference_to_strategy(self, user_pref: UserPreference) -> Optional[BypassStrategy]:
        ...
        try:
            ...
            return BypassStrategy(id=f'user_pref_{user_pref.domain}', name=f'User Preference for {user_pref.domain}', attacks=attacks, parameters=parameters, success_rate=user_pref.success_rate, last_tested=user_pref.last_updated)
        except Exception as e:
            LOG.error(f'Failed to convert user preference: {e}')
            return None

    def _apply_conflict_resolution(self, domain: str, strategies: List[BypassStrategy], method: ConflictResolution) -> Optional[BypassStrategy]:
    ...
    def _merge_strategies(self, strategies: List[BypassStrategy]) -> BypassStrategy:
        ...
        for attack in all_attacks:
            if attack not in seen:
                ...
                seen.add(attack)
        return BypassStrategy(id='merged_strategy', name='Merged Strategy', attacks=unique_attacks, parameters=all_parameters, target_ports=list(all_ports), priority=max((s.priority for s in strategies)))
    ...
    def _save_user_preferences(self) -> None:
        ...
        try:
            if len(self.user_preferences) == 1 and 'default' in self.user_preferences:
                ...
                data = {'strategy': pref.strategy, 'success_rate': pref.success_rate, 'avg_latency_ms': pref.avg_latency_ms, 'fingerprint_used': pref.fingerprint_used, 'dpi_type': pref.dpi_type, 'dpi_confidence': pref.dpi_confidence, 'last_updated': pref.last_updated.isoformat()}
            else:
                data = {'preferences': {}}
                for domain, pref in self.user_preferences.items():
                    data['preferences'][domain] = {'strategy': pref.strategy, 'success_rate': pref.success_rate, 'avg_latency_ms': pref.avg_latency_ms, 'fingerprint_used': pref.fingerprint_used, 'dpi_type': pref.dpi_type, 'dpi_confidence': pref.dpi_confidence, 'last_updated': pref.last_updated.isoformat()}
