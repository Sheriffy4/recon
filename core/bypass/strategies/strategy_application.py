...
try:
    from .pool_management import StrategyPool, StrategyPoolManager, BypassStrategy, DomainRule
...
class UserPreference:
    ...
    domain: str
    strategy: str  # zapret format strategy string
...
class EnhancedStrategySelector:
    ...
    def __init__(self, pool_manager: StrategyPoolManager, attack_registry: ModernAttackRegistry,
        ...
        self.domain_patterns = {
            'social_media': [
                ...
                r'.*tiktok\.com.*', r'.*facebook\.com.*', r'.*vk\.com.*',
                r'.*telegram\.org.*', r'.*discord\.com.*'
            ],
            'video_platforms': [
                r'.*youtube\.com.*', r'.*vimeo\.com.*', r'.*twitch\.tv.*',
                r'.*netflix\.com.*', r'.*hulu\.com.*'
            ],
            'cdn_providers': [
                r'.*cloudflare\.com.*', r'.*fastly\.com.*', r'.*akamai\.com.*',
                r'.*amazonaws\.com.*', r'.*googleusercontent\.com.*'
            ],
            'news_sites': [
                r'.*bbc\.com.*', r'.*cnn\.com.*', r'.*reuters\.com.*',
                r'.*nytimes\.com.*', r'.*guardian\.com.*'
            ]
        }
    ...
    def select_strategy(self, domain: str, port: int = 443, 
                       context: Optional[Dict[str, Any]] = None) -> Optional[BypassStrategy]:
        """
        ...
        LOG.info(f"Selected strategy '{best_score.strategy_id}' for {domain} "
    ...
    def resolve_strategy_conflicts(self, domain: str, conflicting_strategies: List[BypassStrategy],
                                 resolution_method: ConflictResolution = None) -> BypassStrategy:
        """
        ...
        # Use specified method or default order
    ...
    def update_user_preference(self, domain: str, strategy: str, success_rate: float = None,
                             latency_ms: float = None, **kwargs) -> None:
        """
    ...
    def _analyze_domain(self, domain: str) -> DomainAnalysis:
        ...
        # Estimate complexity based on domain characteristics
        complexity = 1
        ...
        # Suggest ports based on domain type
    ...
    def _get_candidate_strategies(self, domain: str, port: int, 
                                domain_analysis: DomainAnalysis) -> List[BypassStrategy]:
        """Get candidate strategies for a domain."""
        candidates = []
        
        # 1. Check if domain is in any pool
        ...
        if user_pref:
            # Convert user preference to BypassStrategy
            strategy = self._convert_user_preference_to_strategy(user_pref)
            if strategy:
                candidates.append(strategy)
        
        # 3. Get strategies from pools with similar domains
        ...
        # 4. Generate strategies based on domain analysis
    ...
    def _score_strategy(self, strategy: BypassStrategy, domain: str, port: int,
                       domain_analysis: DomainAnalysis, context: Optional[Dict[str, Any]] = None) -> StrategyScore:
        ...
        # Assume average latency of 500ms, score inversely
        ...
        # Reliability score based on attack stability
        ...
        # Freshness score (how recently tested/updated)
        ...
        score.confidence = min(1.0, total * 1.2)  # Boost confidence slightly
    ...
    def _calculate_reliability_score(self, strategy: BypassStrategy) -> float:
        ...
        if not strategy.attacks:
            return 0.5  # Neutral score for strategies without attacks
        ...
        for attack_id in strategy.attacks:
            ...
            if definition:
                # Convert stability enum to score
                stability_scores = {
                    'STABLE': 1.0,
                    'MOSTLY_STABLE': 0.8,
                    'EXPERIMENTAL': 0.6,
    ...
    def _calculate_user_preference_score(self, strategy: BypassStrategy, domain: str) -> float:
        ...
        # Check if strategy matches user preference
        strategy_str = strategy.to_zapret_format()
        if strategy_str == user_pref.strategy:
            return 1.0
        
        # Partial match based on similar attacks
    ...
    def _calculate_compatibility_score(self, strategy: BypassStrategy, domain_analysis: DomainAnalysis) -> float:
        ...
        # Boost score for strategies that match domain type
    ...
    def _calculate_freshness_score(self, strategy: BypassStrategy) -> float:
        ...
        if not strategy.last_tested:
            return 0.3  # Low score for never tested
    ...
    def _find_similar_pools(self, domain_analysis: DomainAnalysis) -> List[StrategyPool]:
        ...
        for pool in self.pool_manager.list_pools():
            # Check if pool has domains with similar characteristics
            similarity_score = 0
        ...
        # Sort by similarity (pools with more similar domains first)
    ...
    def _find_or_create_pool_for_domain(self, domain_analysis: DomainAnalysis) -> Optional[StrategyPool]:
        ...
        # Try to find existing pool based on domain characteristics
        for pool in self.pool_manager.list_pools():
            # Check if pool tags match domain tags
        ...
        # Create new pool based on domain analysis
        if domain_analysis.is_social_media:
            ...
            return self.pool_manager.create_pool(
                "CDN Sites",
                strategy,
                "Automatically created for CDN domains"
            )
        else:
            # Create general pool
            ...
            return self.pool_manager.create_pool(
                "General Sites",
                strategy,
                "Automatically created for general domains"
            )
    ...
    def _generate_strategies_for_domain(self, domain_analysis: DomainAnalysis) -> List[BypassStrategy]:
        ...
        # Always include a general strategy
    ...
    def _convert_user_preference_to_strategy(self, user_pref: UserPreference) -> Optional[BypassStrategy]:
        ...
        try:
            # Parse zapret format strategy string
            attacks = []
            parameters = {}
            ...
            if not attacks:
                attacks = ['tcp_fragmentation']  # Default
    ...
    def _apply_conflict_resolution(self, domain: str, strategies: List[BypassStrategy],
                                 method: ConflictResolution) -> Optional[BypassStrategy]:
        ...
        if method == ConflictResolution.USER_PREFERENCE:
            ...
            # Find strategies that belong to pools and sort by pool priority
            pool_strategies = []
            for strategy in strategies:
                for pool in self.pool_manager.list_pools():
...
if __name__ == "__main__":
    ...
    # This would normally be tested with proper test framework
