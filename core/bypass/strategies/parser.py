...
class UnifiedStrategyParser:
    ...
    ZAPRET_TO_ATTACK_MAP = {
        ...
        "fakeddisorder": {
            ...
            "param_mapping": {"dpi_desync_split_pos": "split_pos"},
        },
        "multisplit": {
            "attack_name": "tcp_multisplit",
            "param_mapping": {
                "dpi_desync_split_count": "split_count",
                "dpi_desync_split_seqovl": "overlap_size",
            },
        },
        "multidisorder": {
            "attack_name": "tcp_multidisorder",
            "param_mapping": {"dpi_desync_split_count": "split_count"},
        },
        "seqovl": {
            "attack_name": "tcp_seqovl",
            "param_mapping": {
                "dpi_desync_split_pos": "split_pos",
                "dpi_desync_split_seqovl": "overlap_size",
            },
        },
        "split2": {
            "attack_name": "tcp_split",
            "param_mapping": {"dpi_desync_split_pos": "split_pos"},
        },
        "tlsrec": {
            "attack_name": "tls_record_splitting",
            "param_mapping": {"tlsrec": "split_pos"},
        },
        # Техники "гонки" и обмана (Fooling)
        "fake": {
            "attack_name": "ttl_fake_race",
            "param_mapping": {"dpi_desync_ttl": "ttl", "dpi_desync_repeats": "repeats"},
        },
    ...
    def _translate_parsed_to_engine_task(
        self, parsed_params: Dict[str, Any]
        ...
        all_techniques = parsed_params.get("dpi-desync", []) + parsed_params.get(
            "dpi-desync-fooling", []
        )
        ...
        for technique in set(all_techniques):  # Используем set для удаления дубликатов
            if technique in self.ZAPRET_TO_ATTACK_MAP:
                ...
                stages.append(
                    {
                        "type": mapping["attack_name"],
                        "name": mapping["attack_name"],
                        "params": stage_params,
                    }
                )
