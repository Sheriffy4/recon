    ...
    try:
        ...
        for domain in youtube_domains:
            ...
            if strategy:
                ...
                assert (
                    "youtube" in strategy.id.lower()
                ), f"Strategy should be YouTube-specific for {domain}"
            else:
        ...
        for domain in twitter_domains:
            ...
            if strategy:
                print(f"✅ {domain}: {strategy.name}")
                assert (
                    "twitter" in strategy.id.lower()
                ), f"Strategy should be Twitter-specific for {domain}"
            else:
                print(f"❌ {domain}: No strategy found")
                assert False, f"Should find strategy for {domain}"
        ...
        for domain in instagram_domains:
            ...
            if strategy:
                print(f"✅ {domain}: {strategy.name}")
                assert (
                    "instagram" in strategy.id.lower()
                ), f"Strategy should be Instagram-specific for {domain}"
            else:
                print(f"❌ {domain}: No strategy found")
                assert False, f"Should find strategy for {domain}"

        # Test TikTok subdomain handling
        tiktok_domains = ["www.tiktok.com", "v16-web.tiktokcdn.com"]
        ...
        for domain in tiktok_domains:
            ...
            if strategy:
                print(f"✅ {domain}: {strategy.name}")
                assert (
                    "tiktok" in strategy.id.lower()
                ), f"Strategy should be TikTok-specific for {domain}"
            else:
                print(f"❌ {domain}: No strategy found")
                assert False, f"Should find strategy for {domain}"
        ...
        for domain in test_domains:
            ...
            assert isinstance(
                analysis["is_subdomain"], bool
            ), "is_subdomain should be boolean"
            assert isinstance(
                analysis["subdomain_levels"], list
            ), "subdomain_levels should be list"
        ...
        assert (
            retrieved_strategy.id == custom_strategy.id
        ), "Retrieved strategy should match set strategy"
        ...
        assert (
            "latency_ms" in test_result
        ), "Test result should contain latency_ms field"
        assert (
            "strategy_id" in test_result
        ), "Test result should contain strategy_id field"
        ...
        assert isinstance(
            stats["total_subdomains"], int
        ), "Total subdomains should be integer"
...
def test_subdomain_patterns():
    ...
    try:
        ...
        test_cases = [
            ...
            (
                "r1---sn-4g5e6nls.googlevideo.com",
                subdomain_handler.PlatformType.YOUTUBE,
            ),
        ...
        for domain, expected_platform in test_cases:
            ...
            if detected_platform == expected_platform:
                ...
                print(
                    f"❌ {domain} -> Expected: {expected_platform.value}, Got: {detected_platform.value}"
                )
        ...
        type_test_cases = [
            (
                "www.youtube.com",
                subdomain_handler.PlatformType.YOUTUBE,
                subdomain_handler.SubdomainType.WEB_INTERFACE,
            ),
            (
                "r1---sn-4g5e6nls.googlevideo.com",
                subdomain_handler.PlatformType.YOUTUBE,
                subdomain_handler.SubdomainType.MEDIA_CONTENT,
            ),
            (
                "i.ytimg.com",
                subdomain_handler.PlatformType.YOUTUBE,
                subdomain_handler.SubdomainType.STATIC_ASSETS,
            ),
            (
                "pbs.twimg.com",
                subdomain_handler.PlatformType.TWITTER,
                subdomain_handler.SubdomainType.MEDIA_CONTENT,
            ),
            (
                "api.twitter.com",
                subdomain_handler.PlatformType.TWITTER,
                subdomain_handler.SubdomainType.API_ENDPOINT,
            ),
        ]
        ...
        for domain, platform, expected_type in type_test_cases:
            ...
            if detected_type == expected_type:
                ...
                print(
                    f"❌ {domain} -> Expected: {expected_type.value}, Got: {detected_type.value}"
                )
...
def test_configuration_persistence():
    ...
    try:
        ...
        new_handler = subdomain_handler.SubdomainStrategyHandler(
            pool_manager, config_path
        )
        ...
        assert (
            retrieved_strategy.id == strategy.id
        ), "Retrieved strategy should match saved strategy"
