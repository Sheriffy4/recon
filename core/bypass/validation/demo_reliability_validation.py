    ...
    for domain, port in test_domains:
        ...
        try:
            ...
            for val_result in result.validation_results:
                ...
                print(f"    {status} {val_result.method.value}: {val_result.response_time:.2f}s")
    ...
    for strategy_id, domain, port in strategies:
        ...
        try:
            ...
            for i, acc_result in enumerate(result.accessibility_results):
                print(f"    Iteration {i+1}: {acc_result.status.value} "
...
async def demo_batch_validation():
    ...
    try:
        ...
        sorted_results = sorted(results, key=lambda r: r.effectiveness_score, reverse=True)
...
async def demo_false_positive_detection():
    ...
    try:
        ...
        results = []
        for i in range(5):
            result = await validator.multi_level_accessibility_check(test_domain, test_port)
            results.append(result)
            print(f"  Check {i+1}: {result.status.value} "
...
async def demo_performance_analysis():
    ...
    for scenario_name, domain, port in scenarios:
        ...
        try:
            ...
            for i in range(3):
                ...
                print(f"  Iteration {i+1}: {total_time:.2f}s total, "
            ...
            performance_data.append({
                ...
                'consistency': 1.0 - (max(iteration_times) - min(iteration_times)) / avg_total_time
            })
    ...
    if performance_data:
        ...
        print(f"Fastest Scenario: {fastest['scenario']} ({fastest['avg_total_time']:.2f}s)")
        print(f"Most Reliable: {most_reliable['scenario']} ({most_reliable['avg_reliability']:.2f})")
        print(f"Most Consistent: {most_consistent['scenario']} ({most_consistent['consistency']:.2f})")
...
async def demo_convenience_functions():
    ...
    try:
        ...
        print(f"Domain Accessibility Result:")
        print(f"  Status: {result.status.value}")
    ...
    try:
        ...
        print(f"Strategy Reliability Result:")
        print(f"  Effectiveness: {result.effectiveness_score:.2f}")
        print(f"  Reliability Level: {result.reliability_level.value}")
        print(f"  Recommendation: {result.recommendation}")
    except Exception as e:
