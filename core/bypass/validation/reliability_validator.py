...
class ReliabilityValidator:
    ...
    async def _run_validation_method(
        ...
        try:
            if method == ValidationMethod.HTTP_RESPONSE:
                ...
                return await self._validate_payload_verification(
                    domain, port, start_time
                )
            else:
    ...
    async def _validate_http_response(
        self, domain: str, port: int, start_time: float
    ) -> ValidationResult:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url) as response:
                    ...
                    success = (
                        response.status == 200 and not is_blocked and len(content) > 100
                    )
    ...
    async def _validate_content_check(
        self, domain: str, port: int, start_time: float
    ) -> ValidationResult:
    ...
    async def _validate_timing_analysis(
        self, domain: str, port: int, start_time: float
    ) -> ValidationResult:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                suspicious_patterns = [
                    avg_timing
                    > self.performance_baselines["max_response_time"],  # Too slow
                    timing_variance > avg_timing * 0.5,  # High variance
                    any(
                        t > self.timeout * 0.8 for t in timings
                    ),  # Near-timeout responses
                ]

                success = (
                    not any(suspicious_patterns)
                    and avg_timing < self.performance_baselines["max_response_time"]
                )
    ...
    async def _validate_multi_request(
        self, domain: str, port: int, start_time: float
    ) -> ValidationResult:
    ...
    async def _validate_dns_resolution(
        ...
        try:
            ...
            result = await loop.run_in_executor(
                self._thread_pool, lambda: resolver.resolve(domain, "A")
            )
    ...
    async def _validate_ssl_handshake(
        self, domain: str, port: int, start_time: float
    ) -> ValidationResult:
    ...
    async def _validate_header_analysis(
        self, domain: str, port: int, start_time: float
    ) -> ValidationResult:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.head(url) as response:
                    ...
                    expected_present = sum(
                        1 for h in expected_headers if h in present_headers
                    )

                    # Check for blocking indicators in headers
                    blocking_headers = ["x-blocked", "x-filtered", "x-censored"]
                    blocking_detected = any(
                        h in present_headers for h in blocking_headers
                    )

                    success = (
                        response.status == 200
                        and expected_present >= 2
                        and not blocking_detected
                    )
    ...
    async def _validate_payload_verification(
        self, domain: str, port: int, start_time: float
    ) -> ValidationResult:
                ...
                async with session.get(url) as response:
                    ...
                    if is_text:
                        ...
                        has_html_structure = any(
                            tag in text_content.lower() for tag in html_tags
                        )
    ...
    async def _collect_baseline_measurements(
        ...
        for result in validation_results:
            ...
            if result.success and result.response_time > 0:
                time_penalty = min(
                    ...
                    / self.performance_baselines["max_response_time"],
                    1.0,
                )
                score *= (
                    1.0 - time_penalty * 0.2
                )  # Up to 20% penalty for slow responses
    ...
    def _detect_false_positive_in_results(
        self, validation_results: List[ValidationResult]
    ) -> bool:
        ...
        status_codes = [
            r.status_code for r in validation_results if r.status_code is not None
        ]
        if len(status_codes) > 1:
            ...
            if (
                consistency_rate
                < self.false_positive_thresholds["status_code_consistency"]
            ):
                return True
    ...
    def _determine_accessibility_status(
        self, validation_results: List[ValidationResult], reliability_score: float
    ) -> AccessibilityStatus:
    ...
    def _calculate_effectiveness_score(
        ...
        normalized_improvement = min(
            improvement_factor / 2.0, 1.0
        )  # Cap at 2x improvement
    ...
    def _calculate_consistency_score(
        self, accessibility_results: List[AccessibilityResult]
    ) -> float:
        ...
        bypass_scores = [r.bypass_effectiveness for r in accessibility_results]
        bypass_variance = (
            statistics.stdev(bypass_scores) if len(bypass_scores) > 1 else 0.0
        )
    ...
    def _calculate_performance_score(
        ...
        if avg_response_time <= 1.0:  # Excellent performance
            ...
            performance_score = (
                1.0 - (avg_response_time - 1.0) / (max_acceptable_time - 1.0) * 0.5
            )
    ...
    def _generate_strategy_recommendation(
        ...
        if reliability_level in [
            ...
            if consistency_score >= 0.7:
                ...
                return (
                    "Use with extensive testing - moderate and inconsistent reliability"
                )
    ...
    async def batch_validate_strategies(
        ...
        """
        self.logger.info(
            f"Batch validating {len(strategy_domain_pairs)} strategy-domain pairs"
        )
    ...
    def generate_reliability_report(
        ...
        if avg_consistency < 0.7:
            recommendations.append(
                "Low consistency detected - strategies may be unstable"
            )
        ...
        if avg_performance < 0.6:
            recommendations.append(
                "Performance optimization needed - response times are high"
            )
