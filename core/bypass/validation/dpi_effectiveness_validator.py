...
class DPIEffectivenessValidator:
    ...
    def validate_attack_effectiveness(
        self, attack: BaseAttack, target: DPITestTarget, test_count: int = 10
    ) -> EffectivenessResult:
        """
    ...
    def _test_network_bypass(
        self, target: DPITestTarget, result: AttackResult, context: AttackContext
    ) -> Tuple[bool, float]:
    ...
    def validate_multiple_attacks(
        ...
        self.logger.info(
            f"Starting validation of {len(attacks)} attacks against {len(targets)} targets"
        )
        ...
        for attack in attacks:
            for target in targets:
                try:
                    ...
                    self.logger.error(
                        f"Validation failed for {attack.__class__.__name__} vs {target.name}: {e}"
                    )
    ...
    def _calculate_target_performance(
        self, results: List[EffectivenessResult]
    ...
    def _calculate_attack_performance(
        self, results: List[EffectivenessResult]
    ) -> Dict[str, Dict[str, float]]:
    ...
    def _generate_recommendations(self, results: List[EffectivenessResult]) -> List[str]:
        ...
        if avg_success_rate < 0.5:
            recommendations.append(
                "Overall success rate is low - consider improving attack techniques"
            )
        ...
        for attack_name, rates in attack_performance.items():
            ...
            if avg_rate < 0.3:
                recommendations.append(
                    f"{attack_name} shows poor performance - consider optimization"
                )
            elif avg_rate > 0.8:
                recommendations.append(
                    f"{attack_name} shows excellent performance - consider as primary attack"
                )
        ...
        if len(detected_bypasses) > len(results) * 0.2:  # More than 20% detected
            recommendations.append(
                "High bypass detection rate - consider more subtle attack techniques"
            )
        ...
        if response_times:
            ...
            if avg_response_time > 5.0:  # More than 5 seconds
                recommendations.append(
                    "High response times detected - consider optimizing attack performance"
                )

        return recommendations

    def compare_with_zapret(
        self, attacks: List[BaseAttack], targets: Optional[List[DPITestTarget]] = None
    ) -> Dict[str, Any]:
        """
        ...
        zapret_performance = self._simulate_zapret_performance(
            attacks, targets or self.test_targets
        )
        ...
        comparison = {
            ...
            "response_time_improvement": (
                ...
                / our_report.performance_metrics["average_response_time"]
    ...
    def _simulate_zapret_performance(
        self, attacks: List[BaseAttack], targets: List[DPITestTarget]
    ) -> Dict[str, Any]:
    ...
    def _detailed_comparison(
        self, our_report: ValidationReport, zapret_performance: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            "success_rate_comparison": {
                ...
                "difference": our_report.overall_success_rate
            ...
            "effectiveness_distribution": our_report.performance_metrics[
    ...
    def generate_effectiveness_dashboard(self, report: ValidationReport) -> Dict[str, Any]:
        ...
        return {
            ...
            "timeline": [
                {"timestamp": r.timestamp, "success_rate": r.overall_success_rate}
...
def validate_attack_effectiveness(
    attack: BaseAttack, target_host: str, target_port: int = 80, test_count: int = 10
) -> EffectivenessResult:
    """
