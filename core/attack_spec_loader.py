...
class AttackSpecLoader:
    ...
    def get_validation_rules(self, attack_name: str, category: Optional[str] = None) -> List[ValidationRule]:
        """
    ...
    def validate_parameters(self, attack_name: str, params: Dict[str, Any]) -> List[str]:
        ...
        for param_name, param_value in params.items():
            ...
            if not self._validate_type(param_value, expected_type):
                errors.append(f"Parameter {param_name} has wrong type: expected {expected_type}, got {type(param_value).__name__}")
            
            # Range validation
            if param_spec.min is not None and param_value < param_spec.min:
                errors.append(f"Parameter {param_name} below minimum: {param_value} < {param_spec.min}")
            
            if param_spec.max is not None and param_value > param_spec.max:
                errors.append(f"Parameter {param_name} above maximum: {param_value} > {param_spec.max}")
                ...
                if isinstance(param_value, list):
                    for val in param_value:
                        if val not in param_spec.allowed_values:
                            errors.append(f"Parameter {param_name} has invalid value: {val} not in {param_spec.allowed_values}")
                elif param_value not in param_spec.allowed_values:
                    errors.append(f"Parameter {param_name} has invalid value: {param_value} not in {param_spec.allowed_values}")
