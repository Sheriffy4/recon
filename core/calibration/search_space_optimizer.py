...
class StrategyIntentEngine:
    ...
    def propose_intents(self, fingerprint: Optional[DPIFingerprint], 
                       failure_history: List[str] = None) -> List[StrategyIntent]:
        """
    ...
    def _get_default_intents(self) -> List[StrategyIntent]:
        ...
        return [
            ...
            StrategyIntent(
                key="basic_ttl_manipulation",
                priority=0.5,
                rationale="Базовая манипуляция TTL"
            )
        ]
    
    def _filter_by_failure_history(self, intents: List[StrategyIntent], 
                                  failure_history: List[str]) -> List[StrategyIntent]:
...
class TargetedStrategyGenerator:
    ...
    def generate_strategies(self, fingerprint: Optional[DPIFingerprint], 
                          max_strategies: int = 15) -> List[GeneratedStrategy]:
        """
    ...
    def _generate_strategies_for_intent(self, intent: StrategyIntent, 
                                      fingerprint: Optional[DPIFingerprint]) -> List[GeneratedStrategy]:
    ...
    def _generate_parameters_for_attack(self, attack_name: str, intent: StrategyIntent, 
                                       fingerprint: Optional[DPIFingerprint]) -> List[Dict[str, Any]]:
...
class NegativeKnowledgeManager:
            ...
            for domain, entries in data.items():
                ...
                for sig, entry_data in entries.items():
                    ...
                    if entry_data.get("retry_after"):
                        entry_data["retry_after"] = datetime.fromisoformat(entry_data["retry_after"])
    ...
    def add_failed_strategy(self, domain: str, strategy: GeneratedStrategy, 
                           failure_reason: str, confidence: float = 0.8):
        ...
        entry = NegativeKnowledgeEntry(
            ...
            attack_type=strategy.attack_combination[0] if strategy.attack_combination else "unknown",
    ...
    def filter_strategies(self, domain: str, strategies: List[GeneratedStrategy]) -> List[GeneratedStrategy]:
...
class SearchSpaceOptimizer:
    ...
    def optimize_strategies(self, domain: str, fingerprint: Optional[DPIFingerprint], 
                          max_strategies: int = 15) -> Tuple[List[GeneratedStrategy], Dict[str, Any]]:
        """
        ...
        all_strategies = self.strategy_generator.generate_strategies(fingerprint, max_strategies * 2)
    ...
    def record_strategy_result(self, domain: str, strategy: GeneratedStrategy, 
...
def optimize_strategies_for_domain(domain: str, fingerprint: Optional[DPIFingerprint] = None, 
                                 max_strategies: int = 15) -> List[GeneratedStrategy]:
    """
