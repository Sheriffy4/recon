...
try:
    from core.strategy_failure_analyzer import StrategyFailureAnalyzer, FailureReport, Strategy, TestResult, TrialArtifacts
    SFA_AVAILABLE = True
except ImportError:
    SFA_AVAILABLE = False
...
class EnhancedStrategyCalibrator(StrategyCalibrator):
    ...
    async def calibrate_domain(self, domain: str, budget: CalibrationBudget) -> CalibrationResult:
        ...
        try:
            ...
            if self.fingerprint_service:
                ...
                LOG.info(f"🔍 DPI fingerprint: тип={fingerprint.dpi_type.value}, режим={fingerprint.dpi_mode.value}")
            ...
            LOG.info(f"🎯 Сгенерировано {len(strategies)} стратегий, отфильтровано до {len(filtered_strategies)}")
            ...
            for i, strategy in enumerate(filtered_strategies):
                ...
                if failure_pattern_detector.should_stop_early(last_failure_causes, consecutive_failures):
                ...
                adapted_strategy = strategy_feedback_system.adapt_strategy(strategy, result.failure_reports)
                ...
                if test_result.success:
                    ...
                    if self.failure_analyzer and test_result.pcap_file:
                        ...
                        if self.fingerprint_service:
                            await self._update_fingerprint_from_failure(
                                domain, failure_report
                            )
                            result.fingerprint_updated = True
                        
                        # Сохраняем negative knowledge
                        await self._save_negative_knowledge_entry(domain, adapted_strategy, failure_report)
                    
                    # Обратная связь для улучшения генерации стратегий
                    strategy_feedback_system.record_failure(adapted_strategy, test_result, failure_report)
                    ...
                    if consecutive_failures >= 3 and failure_report:
                        ...
                        if new_strategies:
                            ...
                            LOG.info(f"➕ Добавлено {len(new_strategies)} адаптивных стратегий: {', '.join(strategy_names)}")
                        else:
            ...
            if hasattr(failure_pattern_detector, 'detected_patterns') and failure_pattern_detector.detected_patterns:
                LOG.info(f"🔍 Обнаружено паттернов неудач: {len(failure_pattern_detector.detected_patterns)}")
            
            if hasattr(strategy_feedback_system, 'adaptation_history') and strategy_feedback_system.adaptation_history:
                LOG.info(f"🔧 Выполнено адаптаций стратегий: {len(strategy_feedback_system.adaptation_history)}")
    ...
    async def _generate_targeted_strategies(self, domain: str, fingerprint: Optional[DPIFingerprint]) -> List[BypassStrategy]:
    ...
    def _filter_negative_knowledge(self, strategies: List[BypassStrategy], domain: str) -> List[BypassStrategy]:
    ...
    async def _test_strategy_with_capture(self, domain: str, strategy: BypassStrategy) -> EnhancedTestResult:
        """
    ...
    async def _analyze_failure(self, pcap_file: str, strategy: BypassStrategy, domain: str) -> FailureReport:
    ...
    async def _save_negative_knowledge_entry(self, domain: str, strategy: BypassStrategy, failure_report: FailureReport):
        ...
        self.negative_knowledge[domain][strategy_key] = {
            ...
            "attack_type": strategy.attack_type,
            "parameters": strategy.parameters,
            "failure_cause": failure_report.root_cause.value if hasattr(failure_report.root_cause, 'value') else str(failure_report.root_cause),
            "confidence": failure_report.confidence,
    ...
    async def _generate_adaptive_strategies(self, 
                                          domain: str, 
                                          fingerprint: Optional[DPIFingerprint], 
                                          failure_report: FailureReport,
                                          feedback_system: "StrategyFeedbackSystem") -> List[BypassStrategy]:
        """
    ...
    def _create_strategy_from_intent(self, intent_key: str, parameters: Dict[str, Any]) -> Optional[BypassStrategy]:
    ...
    def should_stop_early(self, failure_causes: List, consecutive_failures: int) -> bool:
        ...
        if len(failure_causes) >= self.pattern_threshold:
            ...
            if len(failure_causes) >= 4:
                ...
                if (str(last_four[0]) == str(last_four[2]) and 
                    str(last_four[1]) == str(last_four[3]) and 
                    str(last_four[0]) != str(last_four[1])):
                    pattern_key = f"cycle_{last_four[0]}_{last_four[1]}"
                    if pattern_key not in self.detected_patterns:
                        self.detected_patterns[pattern_key] = 1
                        LOG.info(f"🛑 Обнаружен циклический паттерн неудач: {last_four[0]} ↔ {last_four[1]}")
                        return True
            ...
            if len(failure_causes) >= 5:
                ...
                if max_count >= 4:  # 4 из 5 последних неудач
                    ...
                    if pattern_key not in self.detected_patterns:
                        self.detected_patterns[pattern_key] = 1
                        LOG.info(f"🛑 Обнаружена доминирующая причина неудач: {dominant_cause} ({max_count}/5)")
                        return True
...
class StrategyFeedbackSystem:
    ...
    def adapt_strategy(self, strategy: BypassStrategy, failure_reports: List[FailureReport]) -> BypassStrategy:
        """
        ...
        for report in failure_reports[-3:]:  # Последние 3 неудачи
            ...
            # Адаптация для фрагментации
                ...
                if strategy.attack_type == "multisplit" and "split_count" in adapted_params:
                    ...
                    adaptations_made.append(f"split_count: {current_count} -> {adapted_params['split_count']}")
    ...
    def record_failure(self, strategy: BypassStrategy, test_result: EnhancedTestResult, failure_report: Optional[FailureReport]):
        ...
        if failure_report:
            self.failure_patterns[strategy_key]["failure_causes"].append(str(failure_report.root_cause))
            ...
            if pattern["count"] >= 2:  # Стратегия была успешной минимум 2 раза
                ...
                if pattern["parameters"]:
                    ...
                    suggestion = BypassStrategy(
                        ...
                        success_rate=pattern["count"] / (pattern["count"] + 1),  # Приблизительная оценка
                        test_count=0
                    )
    ...
    def _generate_counter_strategies(self, failure_report: FailureReport) -> List[BypassStrategy]:
        ...
        if "rst_injection" in failure_cause.lower():
            ...
            counter_strategies.append(BypassStrategy(
...
class AdaptiveBudgetManager:
    ...
    def should_stop(self, result: CalibrationResult) -> bool:
        ...
        if result.successful_strategies and result.total_trials > 0:
            ...
            if success_rate > 0.5:
                remaining_time = self.original_budget.max_time_seconds - (
                    datetime.now() - self.original_budget.start_time
                ).total_seconds()
                
                if remaining_time > 0:
                    # Виртуально продлеваем время
                    extended_time = remaining_time * self.time_extension_factor
                    LOG.info(f"⏰ Продление времени тестирования: +{extended_time - remaining_time:.1f}с")
                    return False
...
async def calibrate_domain_enhanced(domain: str, max_trials: int = 10, max_time: int = 300, stop_on_success: bool = True) -> CalibrationResult:
    """
