...
class HybridEngine:
    ...
    async def execute_strategy_real_world_from_dict(self, strategy_dict: Dict[str, Any], test_sites: List[str], target_ips: Set[str], dns_cache: Dict[str, str], target_port: int=443, initial_ttl: Optional[int]=None, fingerprint: Optional[DPIFingerprint]=None) -> Tuple[str, int, int, float]:
        """
    ...
    async def test_strategies_hybrid(self, strategies: List[Dict[str, Any]], test_sites: List[str], ips: Set[str], dns_cache: Dict[str, str], port: int, domain: str, fast_filter: bool=True, initial_ttl: Optional[int]=None, enable_fingerprinting: bool=False, use_modern_engine: bool=True) -> List[Dict]:
        """
        ...
        for i, strategy_dict in enumerate(strategies_to_test):
            ...
            result_status, successful_count, total_count, avg_latency = await self.execute_strategy_real_world_from_dict(strategy_dict, test_sites, ips, dns_cache, port, initial_ttl, fingerprint)
            success_rate = successful_count / total_count if total_count > 0 else 0.0
            result_data = {'strategy_dict': strategy_dict, 'strategy': strategy_str_repr, 'result_status': result_status, 'successful_sites': successful_count, 'total_sites': total_count, 'success_rate': success_rate, 'avg_latency_ms': avg_latency, 'fingerprint_used': fingerprint is not None, 'dpi_type': fingerprint.dpi_type.value if fingerprint else None, 'dpi_confidence': fingerprint.confidence if fingerprint else None}
    ...
    def _enhance_strategies_with_registry(self, strategies: List[str], fingerprint: Optional[DPIFingerprint], domain: str, port: int) -> List[str]:
        ...
        if fingerprint:
            if fingerprint.rst_injection_detected:
                tcp_attacks = self.attack_registry.list_attacks(category=self.attack_registry.get_attack_definition(available_attacks[0]).category if available_attacks else None, enabled_only=True)
