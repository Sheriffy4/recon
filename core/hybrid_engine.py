...
from utils.strategy_normalizer import normalize_zapret_string
...
try:
    ...
    logging.getLogger("hybrid_engine").warning(
        f"Modern bypass engine not available: {e}"
    )
...
try:
    ...
    logging.getLogger("hybrid_engine").warning(
        f"Advanced fingerprinting not available: {e}"
    )
...
class HybridEngine:
    ...
    def _translate_zapret_to_engine_task(self, params: Dict) -> Optional[Dict]:
        ...
        if task_type in ["fakedisorder", "multidisorder", "multisplit"]:
            ...
            if any((p.get("type") == 127 for p in split_pos_raw)):
                task_params["split_pos"] = 127
            else:
                positions = [
    ...
    async def fingerprint_target(
        self, domain: str, port: int = 443, force_refresh: bool = False
        ...
        try:
            ...
            fingerprint = await self.advanced_fingerprinter.fingerprint_target(
                target=domain, port=port, force_refresh=force_refresh
            )
            analysis_time = time.time() - start_time
            self.fingerprint_stats["fingerprints_created"] += 1
            LOG.info(
                f"DPI fingerprinting completed for {domain}:{port} in {analysis_time:.2f}s"
            )
            LOG.info(
                f"Detected DPI type: {fingerprint.dpi_type.value} (confidence: {fingerprint.confidence:.2f})"
            )
            return fingerprint
    ...
    def _adapt_strategies_for_fingerprint(
        self, strategies: List[str], fingerprint: DPIFingerprint
    ) -> List[str]:
        """
        Adapt and prioritize strategies based on DPI fingerprint with enhanced hints support.
        ...
        LOG.info(
            f"Adapting strategies for DPI type: {dpi_type.value} (confidence: {confidence:.2f})"
        )
        
        if hints:
            LOG.info(f"Using strategy hints: {hints}")
        
        # 1) Если QUIC блокируется — принудительно ориентируемся на TLS-стратегии
        ...
        # 2) Если чувствительность к SNI — используем split-tls=sni + multisplit
        ...
        # 3) Если h2 не договаривается — принуждаем http/1.1 (в движке это может выражаться как отсутствие h2 имитации)
        if "prefer_http11" in hints:
            adapted_strategies.append("--dpi-desync=fake,fakeddisorder --dpi-desync-split-pos=3 --dpi-desync-ttl=4 --dpi-desync-fooling=badsum")

        # 4) CDN/медиа — агрессивный multisplit
        ...
        # 5) Базовые правила по типу DPI (как было раньше)
        if dpi_type == DPIType.ROSKOMNADZOR_TSPU:
            ...
            adapted_strategies.extend(
                self._prioritize_strategies(strategies, priority_patterns)
            )
            ...
            priority_patterns = [
                "--dpi-desync=fake.*disorder",
                "--dpi-desync-fooling=.*seq",
            ]
            adapted_strategies.extend(
                self._prioritize_strategies(strategies, priority_patterns)
            )
        
        # Additional characteristic-based adaptations
        if (
            fingerprint.rst_injection_detected
            and fingerprint.connection_reset_timing < 100
        ):
            adapted_strategies.extend(
                [
        ...
        # Объединяем (без дублей) и нормализуем
        seen = set()
        unique_strategies = []
        for strategy in adapted_strategies + strategies:
            # Normalize strategy parameters to ensure they're within safe ranges
            normalized_strategy = normalize_zapret_string(strategy)
            if normalized_strategy not in seen:
                seen.add(normalized_strategy)
                unique_strategies.append(normalized_strategy)
        
        LOG.info(
            f"Adapted {len(strategies)} strategies to {len(unique_strategies)} fingerprint-aware strategies"
        )
        ...
        if fingerprint:
            if fingerprint.rst_injection_detected:
                tcp_attacks = self.attack_registry.list_attacks(
                    category=(
                        self.attack_registry.get_attack_definition(
                            available_attacks[0]
                        ).category
                ...
                LOG.info(
                    f"Found {len(tcp_attacks)} TCP-based attacks for RST injection DPI"
                )
    ...
    def _generate_registry_strategies(
        self,
        available_attacks: List[str],
    ...
    def assign_domain_to_pool(
        self, domain: str, port: int = 443, strategy: Optional["BypassStrategy"] = None
    ) -> bool:
        """
    ...
    def get_pool_strategy_for_domain(
    ...
    def validate_strategy_reliability(
        self, domain: str, strategy: "BypassStrategy", port: int = 443
    ) -> Optional[float]:
        """
    ...
    def _prioritize_strategies(
        self, strategies: List[str], priority_patterns: List[str]
    ) -> List[str]:
        """
