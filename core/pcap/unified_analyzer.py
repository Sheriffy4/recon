...
class UnifiedPCAPAnalyzer:
                ...
                if tcp_payload[0] == 0x16 and tcp_payload[1] == 0x03:
                    ...
                    if len(tcp_payload) >= 6 and tcp_payload[5] == 0x01:
                        ...
                        clienthello = ClientHelloInfo(
                            ...
                            raw_bytes=tcp_payload[:record_length + 5] if len(tcp_payload) >= record_length + 5 else tcp_payload
                        )
    ...
    def detect_split(self, packets: List[bytes], clienthellos: List[ClientHelloInfo]) -> Optional[SplitInfo]:
        """
        ...
        for idx, packet_data in enumerate(packets):
            try:
                ...
                if ttl < 20:
                    ...
                    self.logger.debug(
                        f"Found fake packet at index {idx}: TTL={ttl}, seq={seq}"
                    )
    ...
    def detect_fooling(self, packets: List[bytes]) -> List[str]:
        ...
        if invalid_checksum_count > 0:
            ...
            self.logger.info(f"Detected badsum: {invalid_checksum_count} packets with invalid checksums")
