...
class StrategyMetricsCollector:
    ...
    def start_conn(self, domain: str, ip: str):
        self.connections[domain] = {"ip": ip, "start": time.time(),
                                    "sent": 0, "recv": 0, "success": False, "events": []}
        self.connections[domain]["events"].append(("start", time.time()))
    def sent(self, domain: str, n: int): self._mark(domain, "sent", n)
    def recv(self, domain: str, n: int): self._mark(domain, "recv", n)
    def ok(self, domain: str): self._mark(domain, "ok", 0, success=True)
    def fail(self, domain: str, reason: str): self._mark(domain, f"fail:{reason}", 0)
    def _mark(self, domain: str, event: str, n: int, success: bool = False):
        c = self.connections.get(domain)
        if not c: return
        if event == "sent": c["sent"] += n
        elif event == "recv": c["recv"] += n
        if success: c["success"] = True
        c["events"].append((event, time.time(), n))
    def agg(self) -> Dict[str, Any]:
        total = len(self.connections); ok = sum(1 for c in self.connections.values() if c["success"])
        return {"total_connections": total, "successful_connections": ok,
    ...
    def per_domain(self) -> Dict[str, Dict[str, Any]]:
        ...
        for domain, c in self.connections.items():
            out[domain] = {"success": c["success"], "ip": c["ip"],
                           "data_transferred": c["sent"] + c["recv"],
                           "latency_ms": (c["events"][-1][1] - c["events"][0][1]) * 1000
                                         if len(c["events"]) > 1 else 0}
        return out
...
class EnhancedPacketCapturer:
    ...
    def __init__(self, pcap_file: str = "", bpf: Optional[str] = None,
                 interface: Optional[str] = None, max_packets: int = 0, max_seconds: int = 0):
    ...
    def analyze_pcap_file(self, pcap_file: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
        ...
        for sid, _, _ in windows:
            res[sid] = {"total_packets": 0, "syn_packets": 0, "rst_packets": 0,
                        "tls_clienthellos": 0, "tls_serverhellos": 0,
                        "data_packets": 0, "total_bytes": 0,
                        "success_indicator": False, "success_score": 0.0}
        for pkt in pkts:
            try: ts = float(getattr(pkt, "time", 0.0))
            except Exception: ts = 0.0
            for sid, st, en in windows:
                if st <= ts <= en:
                    r = res[sid]; r["total_packets"] += 1
                    try:
                        if TCP in pkt:
                            flags = pkt[TCP].flags
                            ...
                            if Raw in pkt:
                                ...
                                if payload:
                                    ...
                                    if len(payload) > 5 and payload[0] == 0x16:
                                        if payload[5] == 0x01: r["tls_clienthellos"] += 1
                                        elif payload[5] == 0x02: r["tls_serverhellos"] += 1
                    except Exception:
                        pass
                    break
        ...
        if TCP in pkt:
            ...
            if Raw in pkt:
                ...
                if payload:
                    ...
                    if len(payload) > 5 and payload[0] == 0x16:
                        if payload[5] == 0x01: tls_clienthellos += 1
                        elif payload[5] == 0x02: tls_serverhellos += 1
    success_indicator = tls_serverhellos > 0
    return {"total_packets": len(pkts), "tcp_handshakes": syn_packets,
            "tls_clienthellos": tls_clienthellos, "tls_serverhellos": tls_serverhellos,
            "rst_packets": rst_packets, "data_packets": data_packets,
            "total_bytes": total_bytes, "success_indicator": success_indicator,
            "success_score": tls_serverhellos / max(1, tls_clienthellos)}

def create_enhanced_packet_capturer(pcap_file: str, target_ips: Set[str], port: int = 443,
                                    interface: Optional[str] = None) -> EnhancedPacketCapturer:
    """
    ...
    return EnhancedPacketCapturer(pcap_file=pcap_file, bpf=bpf, interface=interface,
                                  max_packets=10000, max_seconds=300)
