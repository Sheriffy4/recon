...
class CaptureConstants:
    ...
    VERIFICATION_DURATION = 15  # Extended capture for TLS handshake
    NORMAL_DURATION = 8  # Extended for TLS handshake capture
...
class TemporaryPCAPCapturer:
    ...
    def __init__(
        self, temp_dir: Optional[str] = None, max_file_size_mb: int = 50, auto_cleanup: bool = True
    ):
        """
    ...
    def _build_filter_string(
        self, verification_mode: bool = False, target_ip: Optional[str] = None
    ) -> str:
        """
        ...
        """
        # CRITICAL FIX: Always use broader filter to capture TLS handshake
        # Don't optimize too aggressively as we need to capture the actual connections
        ...
        port_conditions = " or ".join(f"tcp.DstPort == {p} or tcp.SrcPort == {p}" for p in ports)

        # Include target IP if available for better filtering
        if target_ip:
            return f"tcp and host {target_ip} and ({port_conditions})"
        else:
        ...
        try:
            ...
            if self.capture_mode == "pydivert":
                capture_thread = threading.Thread(
                    target=self._capture_with_windivert, args=(session,), daemon=True
                )
                session.capture_method = "windivert"
            elif self.capture_mode == "scapy":
                capture_thread = threading.Thread(
                    target=self._capture_with_scapy, args=(session,), daemon=True
                )
    ...
    def _capture_with_windivert(self, session: CaptureSession):
        ...
        try:
            ...
            with pydivert.WinDivert(session.filter_str, priority=500) as w:
                    ...
                    try:
                        ...
                        if packet is None:
                            ...
                            if timeout_count >= CaptureConstants.MAX_CONSECUTIVE_TIMEOUTS:
                                self.logger.warning(
                                    f"Too many timeouts ({timeout_count}), stopping"
                                )
                                break
                            continue
                        ...
                        # Convert to Scapy packet with Ethernet header
                        try:
                            raw_data = bytes(packet.raw)

                            # CRITICAL FIX: WinDivert provides IP packets without Ethernet headers
                            # We need to add Ethernet header for proper PCAP format
                            from scapy.all import Ether, IP

                            ip_packet = IP(raw_data)
                            # Add Ethernet header (dummy MAC addresses for PCAP compatibility)
                            eth_packet = (
                                Ether(dst="00:00:00:00:00:00", src="00:00:00:00:00:01") / ip_packet
                            )

                            packets.append(eth_packet)
                            session.packets_captured += 1

                            if len(packets) <= 5:
                                self.logger.debug(
                                    f"Captured packet {len(packets)}: {len(raw_data)} bytes (IP->Ethernet)"
                                )
                ...
                self.logger.info(
                    f"WinDivert capture ended: {capture_actual:.2f}s, {len(packets)} packets"
                )
    ...
    def _capture_with_scapy(self, session: CaptureSession):
        ...
        try:
            ...
            self.logger.info(
                f"🔍 {mode_str} MODE: Scapy capture ({capture_duration}s, max {max_packets} packets)"
            )
            ...
            try:
                ...
                def packet_handler(packet):
                    ...
                    try:
                        ...
                        if packets_written <= 5:
                            self.logger.debug(
                                f"Captured packet {packets_written}: {packet.summary()}"
                            )
                ...
                while (
                    (time.monotonic() - start_time) < capture_duration
                    and packets_written < max_packets
                    ...
                    try:
                        ...
                        self.logger.error(
                            "Permission denied. Install Npcap and run as Admin on Windows"
                        )
                        break
                    except Exception as e:
...
class EnhancedBypassEngineAdapter:
    ...
    async def test_strategy_with_capture(
        ...
        with self.pcap_capturer.capture_session(domain) as session:
            ...
            self.logger.info(
                f"Strategy test completed. Captured {session.packets_captured} packets"
            )

            return result

    async def _test_strategy_fallback(
        self, domain: str, strategy: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
