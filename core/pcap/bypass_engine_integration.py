...
class WindowsBypassEngineWithCapture:
    ...
    def __init__(self, bypass_engine, enable_capture: bool = True):
        ...
        if (
            ...
            and self.pcap_capturer
            and self.pcap_capturer.is_capture_available()
        ):
            self.logger.info("✅ PCAP capture enabled for strategy testing")
        elif enable_capture:
            self.logger.warning(
                "⚠️ PCAP capture requested but unavailable - running in fallback mode"
            )
        else:
            self.logger.info("ℹ️ PCAP capture disabled - running without individual capture")
    ...
    def test_strategy_with_analysis(
        ...
        self.logger.info(
            f"🧪 Testing strategy for {domain}: {strategy.get('attack', strategy.get('type', 'unknown'))}"
        )
        ...
        try:
            if verification_mode:
                try:
                    ...
                    metadata = {"domain": domain, "strategy": strategy, "verification_mode": True}
                    strategy_id = operation_logger.start_strategy_log(
                        strategy_name=strategy_name, domain=domain, metadata=metadata
                    )
            ...
            if pcap_file:
                ...
                try:
                    ...
                    metrics_collector.record_validation_result(
                        ...
                        strategy=strategy_for_engine.get(
                            "attack", strategy_for_engine.get("type", "unknown")
                        ),
                ...
                if not clienthello_validation.get("valid", False):
                    self.logger.warning(
                        f"⚠️ ClientHello validation failed: {clienthello_validation.get('reason')}"
                    )
                    self.logger.warning(
                        f"💡 Recommendation: {clienthello_validation.get('recommendation')}"
                    )
                else:
                    self.logger.info(
        ...
        try:
            ...
            if hasattr(self.bypass_engine, "test_strategy_like_testing_mode"):
                try:
                    ...
                    self.logger.info(
                        f"🔧 Bypass engine test result: success={success}, error={error}"
                    )
            ...
            self.logger.warning(
                "⚠️ Bypass engine test method not available, using requests fallback"
            )
            ...
            try:
                ...
                self.logger.info(
                    f"🌐 Request result: status={response.status_code}, success={success}"
                )
                return success, error
    ...
    def _validate_success_with_pcap(
        ...
        try:
            ...
            flows_with_sni = [(key, meta) for key, meta in flows.items() if meta["has_sni"]]
            self.logger.info(
                f"📊 Потоков с ClientHello(SNI={target_domain}) в окне: " f"{len(flows_with_sni)}"
            )
    ...
    def test_strategy_as_service(
        ...
        """
        capture_mode = (
            "WITH individual PCAP capture"
            if enable_capture
            else "WITHOUT individual PCAP capture (shared capture mode)"
        )
            ...
            try:
                with self.pcap_capturer.capture_session(
                    ...
                    if hasattr(self.bypass_engine, "test_strategy_as_service"):
                        self.logger.info(
                            "[SERVICE] Using UnifiedBypassEngine.test_strategy_as_service()"
                        )
                        try:
                        ...
                        self.logger.warning(
                            "[SERVICE] test_strategy_as_service not available, using fallback"
                        )
                ...
                if isinstance(result, dict) and pcap_file:
                    ...
                    clienthello_validation = self.validate_clienthello_size(pcap_file)
                    try:
                        from core.metrics.clienthello_metrics import (
        ...
        # Фоллбек: без индивидуального PCAP захвата
        if enable_capture:
            self.logger.warning(
                "[SERVICE] Individual PCAP capture failed, testing without individual capture"
            )
        else:
            self.logger.info(
                "[SERVICE] Individual PCAP capture disabled (using shared capture), testing without individual capture"
            )

        if hasattr(self.bypass_engine, "test_strategy_as_service"):
            self.logger.info(
                "[SERVICE] Using UnifiedBypassEngine.test_strategy_as_service() (no capture)"
            )
            try:
    ...
    def validate_clienthello_size(self, pcap_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if avg_size < self.MIN_CLIENT_HELLO_SIZE:
                ...
                self.logger.warning(
                    "⚠️ DPI systems easily analyze and block small ClientHello packets"
                )
        ...
        try:
            if hasattr(self.bypass_engine, "is_windivert_active"):
                ...
                if is_active:
                    ...
                    self.logger.warning(
                        "⚠️ WinDivert is not active - strategies may not be applied!"
                    )
            ...
            if hasattr(self.bypass_engine, "get_mode"):
                ...
                if mode == "testing":
                    self.logger.info(
                        "✅ Testing mode confirmed - WinDivert should intercept packets"
                    )
    ...
    def enable_discovery_mode(self):
        """Enable discovery mode - delegates to underlying bypass engine."""
        if hasattr(self.bypass_engine, "enable_discovery_mode"):
            self.bypass_engine.enable_discovery_mode()
            self.logger.info("🔍 Discovery mode enabled in WindowsBypassEngineWithCapture")
        else:
    ...
    def disable_discovery_mode(self):
        ...
        if hasattr(self.bypass_engine, "disable_discovery_mode"):
            self.bypass_engine.disable_discovery_mode()
            self.logger.info("🔍 Discovery mode disabled in WindowsBypassEngineWithCapture")
        else:
            self.logger.warning("⚠️ Underlying bypass engine does not support discovery mode")
    ...
    def is_discovery_mode_active(self):
        ...
        return predicate_is_discovery_mode_active(self.bypass_engine)
