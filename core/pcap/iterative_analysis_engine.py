...
class IterativeAnalysisEngine:
    ...
    async def analyze_pcap_iteration(
        ...
        try:
            ...
            LOG.info(
                f"✅ Итерация {iteration_number} завершена за {analysis_time:.2f}s: "
    ...
    async def _update_dpi_knowledge(
        self,
        domain: str,
        pcap_analysis: PCAPAnalysisResult,
        failed_strategy: Any,
        ...
        try:
            ...
            updates.append({"type": "evolution_timeline_updated", "entry": evolution_entry})
    ...
    def _calculate_knowledge_confidence(self, knowledge: DPIKnowledge) -> float:
        ...
        pattern_confidence = sum(knowledge.blocking_patterns.values()) / len(
    ...
    async def _correlate_with_previous_iterations(
        self, domain: str, pcap_analysis: PCAPAnalysisResult, iteration_context: IterationContext
    ) -> Dict[str, Any]:
        """Корреляция с предыдущими итерациями"""
        correlation_results = {
        ...
        try:
            ...
            strategy_effectiveness = self._analyze_strategy_effectiveness_trends(
                previous_iterations
            )
    ...
    def _analyze_strategy_effectiveness_trends(
        self, previous_iterations: List[IterationContext]
    ) -> Dict[str, Any]:
    ...
    def _determine_learning_phase(
        self, domain: str, iteration_context: IterationContext
    ) -> LearningPhase:
            ...
            if domain in self.iteration_history:
                ...
                for iteration in recent_iterations:
                    if iteration.strategies_tested:
                        success_rate = len(iteration.successful_strategies) / len(
                            iteration.strategies_tested
                        )
                        total_success_rate += success_rate

                avg_success_rate = (
                    total_success_rate / len(recent_iterations) if recent_iterations else 0
                )
    ...
    async def _generate_adaptive_strategies(
        ...
        try:
            ...
            if learning_phase == LearningPhase.EXPLORATION:
                ...
                strategies = await self._generate_adaptation_strategies(
                    pcap_analysis, knowledge, iteration_context
                )
            ...
            LOG.info(
                f"🎯 Сгенерировано {len(adaptive_strategies)} адаптивных стратегий "
    ...
    async def _generate_exploration_strategies(
        self, pcap_analysis: PCAPAnalysisResult, knowledge: Optional[DPIKnowledge]
    ) -> List[Dict[str, Any]]:
        ...
        base_strategies = [
            ...
            {
                "attack": "disorder",
                "params": {"split_pos": random.randint(2, 10), "fooling": "badsum"},
            },
            {"attack": "split", "params": {"split_pos": "sni", "fooling": "badsum"}},
            {
                "attack": "multisplit",
                "params": {"split_count": random.randint(4, 12), "split_pos": "sni"},
            },
        ]
    ...
    async def _generate_exploitation_strategies(
        self, pcap_analysis: PCAPAnalysisResult, knowledge: Optional[DPIKnowledge]
    ) -> List[Dict[str, Any]]:
        ...
        if knowledge and knowledge.effective_strategies:
            ...
            sorted_strategies = sorted(
                knowledge.effective_strategies.items(), key=lambda x: x[1], reverse=True
            )
    ...
    async def _generate_evolution_strategies(
        self,
        pcap_analysis: PCAPAnalysisResult,
        knowledge: Optional[DPIKnowledge],
        iteration_context: IterationContext,
        ...
        try:
            ...
            LOG.info(
                f"🧬 Эволюция: создано {len(strategies)} стратегий из {len(offspring_genes)} генов"
            )
    ...
    def _select_genes_for_reproduction(self) -> List[StrategyGene]:
        ...
        for _ in range(min(10, len(self.strategy_gene_pool))):
            tournament = random.sample(
                list(self.strategy_gene_pool.values()),
    ...
    def _crossover_genes(
        self, parent1: StrategyGene, parent2: StrategyGene
        ...
        child1 = StrategyGene(
            ...
            parent_genes=[
                f"{parent1.attack_type}_{parent1.generation}",
    ...
    def _gene_to_strategy(
        self, gene: StrategyGene, pcap_analysis: PCAPAnalysisResult
    ) -> Dict[str, Any]:
    ...
    def update_gene_fitness(
        self, strategy_name: str, success: bool, performance_metrics: Dict[str, Any]
    ):
    ...
    def _get_or_create_iteration_context(
        self, domain: str, iteration_number: int, failed_strategy: Any
    ) -> IterationContext:
        ...
        context = IterationContext(
            iteration_number=iteration_number, domain=domain, phase=LearningPhase.EXPLORATION
        )
        ...
        try:
            ...
            if Path(self.knowledge_file).exists():
                ...
                for domain, data in knowledge_data.items():
                    ...
                    knowledge.last_updated = datetime.fromisoformat(
        ...
        try:
            ...
            for domain, knowledge in self.knowledge_base.items():
                knowledge_data[domain] = {
                    "blocking_patterns": {
                        k.value: v for k, v in knowledge.blocking_patterns.items()
                    },
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        stats["knowledge_base"] = {
            ...
            "total_blocking_patterns": sum(
                len(k.blocking_patterns) for k in self.knowledge_base.values()
            ),
            "total_effective_strategies": sum(
                len(k.effective_strategies) for k in self.knowledge_base.values()
            ),
            "average_confidence": (
                sum(k.confidence for k in self.knowledge_base.values()) / len(self.knowledge_base)
        ...
        stats["iterations"] = {
            ...
            "average_iterations_per_domain": (
                sum(len(history) for history in self.iteration_history.values())
        ...
        try:
            ...
            for knowledge in self.knowledge_base.values():
                ...
                knowledge.dpi_evolution_timeline = [
                    entry
                    for entry in knowledge.dpi_evolution_timeline
            ...
            for domain, iterations in self.iteration_history.items():
                ...
                self.iteration_history[domain] = [
                    iteration for iteration in iterations if iteration.start_time > cutoff_date
                ]
