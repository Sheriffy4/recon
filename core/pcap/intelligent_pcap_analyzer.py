...
from ..intelligence.utils.dns_utils import extract_dns_answers
...
class IntelligentPCAPAnalyzer:
    ...
    async def analyze_pcap_file(
        self, pcap_file: str, domain: str, strategy_context: Optional[Dict] = None
    ) -> PCAPAnalysisResult:
        """
        ...
        try:
            ...
            dpi_signatures = await self._extract_dpi_signatures(
                packets, blocking_type, analysis_results
            )
            ...
            LOG.info(
                f"✅ Анализ завершен за {analysis_time:.2f}s: {blocking_type.value} (confidence: {confidence:.2f})"
            )
    ...
    async def _analyze_rst_injections(self, packets: List, domain: str) -> Dict[str, Any]:
        ...
        LOG.debug(
            f"🔍 RST анализ: {len(rst_packets)} RST пакетов, {len(suspicious_rsts)} подозрительных"
        )
    ...
    async def _analyze_tls_handshake(self, packets: List, domain: str) -> Dict[str, Any]:
        ...
        sni_filtering_suspected = (
            client_hello_count > 0 and server_hello_count == 0 and len(tls_alerts) == 0
        )
        ...
        LOG.debug(
            f"🔍 TLS анализ: {client_hello_count} CH, {server_hello_count} SH, {len(tls_alerts)} alerts"
        )
    ...
    async def _analyze_dns_responses(self, packets: List, domain: str) -> Dict[str, Any]:
            ...
            if DNS in packet:
                if packet[DNS].qr == 0:  # Query
                    ...
                    response_info = {
                        ...
                        "src_ip": packet[IP].src,
                        "answers": extract_dns_answers(packet),  # Используем общую утилиту
                    }
        ...
        LOG.debug(
            f"🔍 DNS анализ: {len(dns_queries)} запросов, {len(dns_responses)} ответов, {len(suspicious_responses)} подозрительных"
        )
    ...
    async def _analyze_fragmentation(self, packets: List, domain: str) -> Dict[str, Any]:
        ...
        for frag_id, fragments in fragment_groups.items():
            if len(fragments) > 1:
                ...
                for frag in fragments:
                    ...
                    expected_offset = (
                        frag["frag_offset"] + (frag["length"] - 20) // 8
                    )  # IP header = 20 bytes
        ...
        analysis = {
            ...
            "confidence": (
                min(len(reassembly_issues) * 0.5, 1.0)
                if len(reassembly_issues) > 0
                else 0.2 if len(fragmented_packets) > 0 else 0.0
            ),
        }

        LOG.debug(
            f"🔍 Фрагментация: {len(fragmented_packets)} фрагментов, {len(fragment_groups)} групп, {len(reassembly_issues)} проблем"
        )
    ...
    async def _analyze_timing_patterns(self, packets: List, domain: str) -> Dict[str, Any]:
        ...
        if intervals:
            ...
            suspicious_gaps = [
                interval for interval in intervals if interval > 5.0
            ...
            analysis = {
                ...
                "confidence": (
                    min(len(suspicious_gaps) * 0.3, 1.0) if len(suspicious_gaps) > 0 else 0.1
                ),
            }
        else:
            analysis = {"total_packets": len(packets), "confidence": 0.0}

        LOG.debug(f"🔍 Тайминг: {len(packets)} пакетов, {len(intervals)} интервалов")

        return analysis

    def _determine_blocking_type(
        ...
        if confidences:
            ...
            LOG.info(
                f"🎯 Определен тип блокировки: {best_type.value} (confidence: {best_confidence:.2f})"
            )
    ...
    async def _extract_dpi_signatures(
        self, packets: List, blocking_type: BlockingType, analysis_results: Dict[str, Any]
    ) -> List[DPISignature]:
    ...
    def _generate_recommendations(
        self, blocking_type: BlockingType, analysis_results: Dict[str, Any]
    ) -> List[str]:
    ...
    def get_analysis_statistics(self) -> Dict[str, Any]:
        ...
        avg_time = (
            self.stats["analysis_time_total"] / self.stats["files_analyzed"]
