...
class StrategyAnalysisResult:
    ...
    detected_attacks: List[str] = field(default_factory=list)  # Обнаруженные базовые атаки (Task 3.1)
    fake_packets_detected: int = 0  # Количество фейковых пакетов (Task 3.2)
    combo_attacks: List[str] = field(default_factory=list)  # Выявленная комбинация атак (в порядке применения)
...
class PCAPAnalyzer:
        ...
        try:
            ...
            for i, pkt in enumerate(packets):
                ...
                try:
                    ...
                    tcp_payload = (
                        tcp_data[tcp_header_size:]
                        if len(tcp_data) > tcp_header_size
                        else b""
                    )
                    ...
                    self.logger.debug(
                        f"⚠️ Ошибка валидации checksum для пакета {i}: {e}"
                    )
                    checksums[packet_id] = False
            
            valid_count = sum(1 for v in checksums.values() if v)
            self.logger.debug(
                f"🔍 Проверено {len(checksums)} checksums, валидных: {valid_count}"
            )
            ...
            differences.append(
                {
                    "type": "sni_value",
                    "description": (
                        f"Expected SNI '{expected_sni}', found {analysis.sni_values}"
                    ),
    ...
    def _filter_packets_by_timestamp(self, packets: List[RawPacket], test_start_time: float, time_window: float = 5.0) -> List[RawPacket]:
        ...
        try:
            ...
            self.logger.debug(f"🔍 Фильтрация пакетов: test_start={test_start_time}, window=[{test_window_start:.3f}, {test_window_end:.3f}]")
            ...
            self.logger.info(f"✅ Отфильтровано {len(filtered_packets)}/{len(packets)} пакетов в окне [{test_window_start:.3f}, {test_window_end:.3f}]")
    ...
    def _detect_disorder(self, packets: List[RawPacket]) -> bool:
        ...
        try:
            ...
            real_packets_data: List[Tuple[int, int, int, int]] = []  # (seq_num, packet_idx, payload_size, ttl)
    ...
    def _determine_strategy_type_from_attacks(
        ...
        main_attacks = [
            a for a in detected_attacks if a not in FOOLING_LABELS
        ]
    ...
    def _determine_strategy_type(
        ...
        try:
            ...
            tcp_packets = [p for p in packets if p.protocol == ProtocolType.TCP]
            tcp_with_payload = [
                p for p in packets if p.protocol == ProtocolType.TCP and p.payload
            ]
    ...
    def _find_anomalies(
        ...
        try:
            ...
            if invalid_checksums:
                anomalies.append(
                    f"Invalid checksums in {len(invalid_checksums)} packets"
                )
    ...
    def _calculate_similarity(
        ...
        if expected_attacks:
            checks += 1
            if set(expected_attacks) == set(
                analysis.combo_attacks or analysis.detected_attacks
            ):
                score += 1.0
    ...
    def _get_cache_key(self, pcap_file: str, test_start_time: Optional[float] = None, strategy_name: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            for i in range(min(len(tcp_segments), len(log_segments))):
                ...
                if log_flags is not None and pcap_seg.tcp_flags != log_flags:
                    seg_diff.append(f"Flags: PCAP=0x{pcap_seg.tcp_flags:02X}, Log=0x{log_flags:02X}")
                ...
                if log_payload_len != pcap_payload_len:
                    seg_diff.append(f"Payload length: PCAP={pcap_payload_len}, Log={log_payload_len}")
                
                if seg_diff:
                    mismatches.append({
                        "segment_index": i + 1,
                        "differences": seg_diff
                    })
            ...
            if result["match"]:
                self.logger.info(f"✅ PCAP matches operation log perfectly: {len(tcp_segments)} segments")
            else:
                self.logger.warning(
