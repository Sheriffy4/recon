...
def load_pcap_metadata(pcap_file: str, strategy_name: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
                ...
                for strategy_file in temp_dir.glob("strategy_*.json"):
                    try:
                        ...
                        if metadata.get('domain') == domain:
                            LOG.debug(f"✅ Loaded PCAP metadata from {strategy_file} (matched by domain)")
                            return metadata
                    except Exception:
                        continue
