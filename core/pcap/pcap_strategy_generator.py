...
class PCAPStrategyGenerator:
    ...
    async def generate_strategies_from_pcap(
        self, pcap_analysis: PCAPAnalysisResult, max_strategies: int = 10
        ...
        LOG.info(
            f"🎯 Генерация стратегий для {pcap_analysis.blocking_type.value} "
        ...
        try:
            ...
            base_strategies = self._get_base_strategies_for_blocking_type(
    ...
    async def _generate_strategy_from_template(
        self, template: Dict, pcap_analysis: PCAPAnalysisResult
    ) -> Optional[PCAPGeneratedStrategy]:
    ...
    def _extract_relevant_evidence(
        self, pcap_analysis: PCAPAnalysisResult, template: Dict
    ) -> Dict[str, Any]:
    ...
    async def _optimize_strategy_parameters(
        self, strategy: PCAPGeneratedStrategy, pcap_analysis: PCAPAnalysisResult
    ):
            ...
            for param_name, param_value in strategy.parameters.items():
                if param_name in self.parameter_optimizers:
                    ...
                    if optimized_value != param_value:
                        LOG.debug(
                            f"🔧 Оптимизирован {param_name}: {param_value} -> {optimized_value}"
                        )
    ...
    async def _optimize_ttl_parameter(
        self, current_ttl: int, pcap_analysis: PCAPAnalysisResult, strategy: PCAPGeneratedStrategy
    ) -> int:
    ...
    async def _optimize_split_pos_parameter(
        self, current_pos, pcap_analysis: PCAPAnalysisResult, strategy: PCAPGeneratedStrategy
    ):
    ...
    async def _optimize_split_count_parameter(
        self, current_count: int, pcap_analysis: PCAPAnalysisResult, strategy: PCAPGeneratedStrategy
    ) -> int:
    ...
    def _prioritize_strategies(
        self, strategies: List[PCAPGeneratedStrategy], pcap_analysis: PCAPAnalysisResult
    ) -> List[PCAPGeneratedStrategy]:
        ...
        try:
            for strategy in strategies:
                ...
                # Финальный приоритет
                final_priority = min(
                    base_priority + confidence_bonus + signature_bonus + specificity_bonus, 1.0
                )
                strategy.priority = final_priority

                # Оценка ожидаемого успеха
                strategy.expected_success_rate = self._estimate_success_rate(
            ...
            for i, strategy in enumerate(strategies[:5]):  # Топ 5
                LOG.debug(
                    f"  {i+1}. {strategy.name}: priority={strategy.priority:.2f}, "
    ...
    async def generate_adaptive_parameters(
        self, base_strategy: Dict, pcap_signatures: List[DPISignature]
    ) -> Dict[str, Any]:
    ...
    def create_strategy_explanation(
        self, strategy: PCAPGeneratedStrategy, pcap_analysis: PCAPAnalysisResult
    ) -> str:
        ...
        if pcap_analysis.recommendations:
            explanation_parts.append(
                f"Рекомендации: {', '.join(pcap_analysis.recommendations[:3])}"
            )
