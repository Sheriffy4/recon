...
try:
    ...
    print(
        "[ERROR] Scapy is required for this analysis. Please run: pip install scapy",
        file=sys.stderr,
    )
    sys.exit(1)
...
class RSTTriggerAnalyzer:
    ...
    def analyze(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"[ERROR] Could not read PCAP file '{self.pcap_path}': {e}",
                file=sys.stderr,
            )
            return []
        ...
        for i, packet in enumerate(packets):
            ...
            if packet[TCP].flags.R and is_from_server:
                if flow["client_packets"]:
                    ...
                    if (
                        flow["server_ttl"] is not None
                        and abs(packet[IP].ttl - flow["server_ttl"]) > 5
                    ):
                    ...
                    flow["rst_triggers"].append(
                        {
                            ...
                            "trigger_payload_hex": (
                                trigger_packet.getlayer(Raw).load[:64].hex()
                                if trigger_packet.haslayer(Raw)
                                else ""
                            ),
                        }
                    )
    ...
    def print_report(self, triggers: List[Dict[str, Any]]):
        ...
        for i, trigger in enumerate(triggers, 1):
            ...
            if trigger["dpi_injection_suspected"]:
                print("  - 🔴 ОБНАРУЖЕНА ВЕРОЯТНАЯ ИНЪЕКЦИЯ DPI!")
                print(
                    f"  - TTL RST-пакета ({trigger['rst_packet_ttl']}) сильно отличается от TTL сервера ({trigger['server_base_ttl']})."
                )
            else:
                print(
                    f"  - 🟢 TTL RST-пакета ({trigger['rst_packet_ttl']}) похож на TTL сервера ({trigger['server_base_ttl']})."
                )
                print(
                    "     Это может быть легитимный RST от самого сервера (например, порт закрыт)."
                )
        print("\n" + "=" * 60)
