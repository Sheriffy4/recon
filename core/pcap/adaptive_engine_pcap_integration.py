...
class AdaptiveEnginePCAPIntegration:
    ...
    async def analyze_failure_with_pcap(self, domain: str, strategy: Any, 
                                      test_result: Dict[str, Any]) -> Optional[PCAPAnalysisResult]:
        """
        ...
        try:
            ...
            LOG.info(f"🔍 Запуск PCAP анализа для {domain} после неудачи стратегии {getattr(strategy, 'name', 'unknown')}")
            ...
            LOG.info(f"✅ PCAP анализ завершен за {analysis_time:.2f}s: "
    ...
    async def generate_strategies_from_pcap(self, pcap_analysis: PCAPAnalysisResult, 
                                          max_strategies: int = 5) -> List[PCAPGeneratedStrategy]:
        """
    ...
    async def correlate_with_historical_data(self, pcap_analysis: PCAPAnalysisResult) -> Dict[str, Any]:
        """
        Корреляция PCAP данных с историческими успехами
        ...
        try:
            ...
            LOG.info(f"🔗 Корреляция завершена: {len(similar_cases)} похожих случаев, "
    ...
    def _find_similar_cases(self, pcap_analysis: PCAPAnalysisResult) -> List[Dict[str, Any]]:
        ...
        try:
            for cache_key, cached_data in self.analysis_cache.items():
                ...
                # Проверяем схожесть
                if (cached_analysis.blocking_type == pcap_analysis.blocking_type and
                    cached_analysis.domain != pcap_analysis.domain and
                    abs(cached_analysis.confidence - pcap_analysis.confidence) < 0.3):
                    ...
                    similar_case = {
                        ...
                        "similarity_score": self._calculate_similarity_score(pcap_analysis, cached_analysis)
                    }
    ...
    def _calculate_similarity_score(self, analysis1: PCAPAnalysisResult, 
                                  analysis2: PCAPAnalysisResult) -> float:
    ...
    def _find_successful_strategies_for_blocking_type(self, blocking_type: BlockingType) -> List[Dict[str, Any]]:
    ...
    def _generate_correlation_recommendations(self, pcap_analysis: PCAPAnalysisResult,
                                            similar_cases: List[Dict], 
                                            successful_strategies: List[Dict]) -> List[str]:
    ...
    async def integrate_with_strategy_failure_analyzer(self, failure_analyzer, 
        ...
        try:
            integration_result = {
                ...
                "sfa_integration": {
                    "enhanced_failure_report": True,
                    "pcap_evidence_added": True
                }
            }
    ...
    def get_integration_statistics(self) -> Dict[str, Any]:
        ...
        if self.pcap_analyzer:
            stats.update({
                "pcap_analyzer_stats": self.pcap_analyzer.get_analysis_statistics()
            })
    ...
    async def export_pcap_analysis_report(self, output_file: str = "pcap_analysis_report.json"):
        ...
        try:
            ...
            for cached_data in self.analysis_cache.values():
                ...
                if blocking_type not in report["analysis_cache_summary"]["blocking_types_distribution"]:
                    report["analysis_cache_summary"]["blocking_types_distribution"][blocking_type] = 0
            ...
            report["analysis_cache_summary"]["domains_analyzed"] = len(report["analysis_cache_summary"]["domains_analyzed"])
            if confidences:
                report["analysis_cache_summary"]["average_confidence"] = sum(confidences) / len(confidences)
...
def integrate_pcap_analysis_into_adaptive_engine(adaptive_engine):
    ...
    try:
        ...
        adaptive_engine.generate_strategies_from_pcap = pcap_integration.generate_strategies_from_pcap
        adaptive_engine.correlate_pcap_with_history = pcap_integration.correlate_with_historical_data
