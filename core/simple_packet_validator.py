...
class SimplePacketValidator:
    ...
    def validate_pcap(
        self, pcap_file: str, attack_type: str = None, params: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
    ...
    def validate_seq_numbers(
        ...
        """
        params = params or {}
        result = {"passed": True, "errors": [], "warnings": [], "details": []}
        ...
        if attack_type == "fakeddisorder":
            if not fake_packets:
                ...
                result["errors"].append(
                    f"Expected at least 2 real packets, found {len(real_packets)}"
                )
                result["passed"] = False
            else:
            ...
            if len(packets) < 2:
                result["errors"].append(
                    f"Expected at least 2 packets for {attack_type}, found {len(packets)}"
                )
                result["passed"] = False
            else:
