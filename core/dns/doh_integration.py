...
class DoHIntegration:
    ...
    async def resolve_with_fallback(
        self, domain: str, timeout: float = 10.0, retry_on_failure: bool = True
    ) -> Set[str]:
        """
        ...
        use_doh = self.enable_doh and (not self.auto_detect_blocking or self.should_use_doh(domain))
        ...
        if use_doh:
            try:
                ...
                ips = await asyncio.wait_for(self.doh_resolver.resolve_all(domain), timeout=timeout)
        ...
        try:
            ...
            results = await asyncio.wait_for(
                loop.getaddrinfo(domain, None, family=socket.AF_INET), timeout=timeout
            )
            ...
            if ips:
                if fallback_reason:
                    LOG.info(
                        f"System DNS resolved {domain} -> {ips} (fallback reason: {fallback_reason})"
                    )
                else:
    ...
    def should_use_doh(self, domain: str) -> bool:
        ...
        if domain in self.failed_system_dns and self.failed_system_dns[domain] >= 2:
            LOG.info(
                f"Domain {domain} has {self.failed_system_dns[domain]} failed system DNS attempts, using DoH"
            )
    ...
    def _record_system_dns_failure(self, domain: str):
        ...
        LOG.debug(
            f"Recorded system DNS failure for {domain} (count: {self.failed_system_dns[domain]})"
        )
