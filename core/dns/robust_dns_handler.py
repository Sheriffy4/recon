...
class RobustDNSHandler:
    ...
    def __init__(self, timeout: float=5.0, max_retries: int=3):
        ...
        self.doh_resolvers = {'cloudflare': DoHResolver('cloudflare'), 'google': DoHResolver('google'), 'quad9': DoHResolver('quad9')}
        self.resolution_methods = [self._resolve_via_doh_cloudflare, self._resolve_via_doh_google, self._resolve_via_doh_quad9, self._resolve_via_system_dns, self._resolve_via_getaddrinfo]
        ...
        for method in self.resolution_methods:
            try:
                ...
                if result.success and result.ip:
                    ...
                    LOG.info(f'Successfully resolved {domain} -> {result.ip} via {result.method} (latency: {result.latency_ms:.1f}ms)')
                    return result.ip
                else:
    ...
    def resolve_multiple_domains(self, domains: List[str], max_workers: int=10) -> Dict[str, Optional[str]]:
        """
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_domain = {executor.submit(self.resolve_with_fallback, domain): domain for domain in domains}
    ...
    def _resolve_via_doh(self, domain: str, provider: str) -> DNSResolutionResult:
        ...
        try:
            ...
            if ip and self.validate_ip_resolution(domain, ip):
                return DNSResolutionResult(domain=domain, ip=ip, method=f'DoH-{provider}', success=True, latency_ms=latency_ms)
            else:
                return DNSResolutionResult(domain=domain, ip=None, method=f'DoH-{provider}', success=False, error='Invalid or empty IP result', latency_ms=latency_ms)
        except Exception as e:
            latency_ms = (time.time() - start_time) * 1000
            return DNSResolutionResult(domain=domain, ip=None, method=f'DoH-{provider}', success=False, error=str(e), latency_ms=latency_ms)
    ...
    def _resolve_via_system_dns(self, domain: str) -> DNSResolutionResult:
        ...
        try:
            ...
            if self.validate_ip_resolution(domain, ip):
                return DNSResolutionResult(domain=domain, ip=ip, method='system-dns', success=True, latency_ms=latency_ms)
            else:
                return DNSResolutionResult(domain=domain, ip=None, method='system-dns', success=False, error='Invalid IP result', latency_ms=latency_ms)
        except Exception as e:
            latency_ms = (time.time() - start_time) * 1000
            return DNSResolutionResult(domain=domain, ip=None, method='system-dns', success=False, error=str(e), latency_ms=latency_ms)
    ...
    def _resolve_via_getaddrinfo(self, domain: str) -> DNSResolutionResult:
        ...
        try:
            ...
            if addr_info:
                ...
                latency_ms = (time.time() - start_time) * 1000
                if self.validate_ip_resolution(domain, ip):
                    return DNSResolutionResult(domain=domain, ip=ip, method='getaddrinfo', success=True, latency_ms=latency_ms)
                else:
                    return DNSResolutionResult(domain=domain, ip=None, method='getaddrinfo', success=False, error='Invalid IP result', latency_ms=latency_ms)
            else:
                latency_ms = (time.time() - start_time) * 1000
                return DNSResolutionResult(domain=domain, ip=None, method='getaddrinfo', success=False, error='No address info returned', latency_ms=latency_ms)
        except Exception as e:
            latency_ms = (time.time() - start_time) * 1000
            return DNSResolutionResult(domain=domain, ip=None, method='getaddrinfo', success=False, error=str(e), latency_ms=latency_ms)
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        with self.cache_lock:
            return {'cache_size': len(self.cache), 'cached_domains': list(self.cache.keys())}