...
class DependencyAnalyzer:
    ...
    def _analyze_file(self, module_name: str, content: str, target_modules: List[str]):
        ...
        try:
            ...
            if imports_targets or any(target in module_name for target in target_modules):
            ...
            if node.module:
                for target in target_modules:
                    if target in node.module:
                        ...
                        for item in items:
                            self.imports_map[importer].add(f"from {node.module} import {item}")
            ...
            if isinstance(node, ast.ClassDef):
                ...
                for base in node.bases:
                    if isinstance(base, ast.Name):
                        ...
                        self.inheritance_map[f"{module_name}.{class_name}"].add(ast.unparse(base))
    ...
    def _generate_report(self, target_modules: List[str]) -> Dict:
        ...
        for module, calls in self.function_calls.items():
            relevant_calls = [c for c in calls if any(
...
def main():
    ...
    for dep in report['critical_dependencies']:
        print(f"  {dep['class']}: used by {len(dep['used_by'])} modules (Risk: {dep['risk_level']})")
