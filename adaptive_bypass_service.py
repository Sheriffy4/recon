...
try:
    ...
    from monitoring_system import MonitoringSystem, ConnectionHealth
    CLI_AVAILABLE = True
...
try:
    from intelligent_bypass_monitor import (
        IntelligentBypassMonitor, TrafficPattern, BypassStrategy, DPIAnalyzer
    )
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('adaptive_bypass_service.log'),
...
class AdaptiveBypassService:
    ...
    async def start_service(self):
        ...
        if (self.intelligent_monitor and 
            self.config["monitoring"]["enable_traffic_monitoring"]):
    ...
    async def _site_monitoring_loop(self):
        ...
        while self.running:
            try:
                ...
                sites_batch = list(self.sites_to_monitor)[:self.config["sites"]["max_concurrent_tests"]]
    ...
    async def _performance_monitoring_loop(self):
        ...
        while self.running:
            try:
                ...
                # Выводим статистику
                logger.info(f"📈 Статистика: {stats['sites_monitored']} сайтов, "
    ...
    async def _api_get_strategies(self, request):
        ...
        from aiohttp import web
        strategies = {
            site: strategy.to_dict() 
            for site, strategy in self.active_strategies.items()
        }
...
def main():
    ...
    parser.add_argument("--config", default="adaptive_bypass_config.json",
