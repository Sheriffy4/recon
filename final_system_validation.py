...
async def test_attack_adapter_multisplit():
    ...
    try:
        ...
        success = (
            result.status == AttackStatus.SUCCESS and
            result.has_segments() and
...
def test_cli_basic():
    ...
    try:
        ...
        print(f"   ⏰ TIMEOUT (but this is expected for network operations)")
...
def test_cli_multisplit():
    ...
    try:
        ...
        result = subprocess.run([
            sys.executable, "cli.py", "nnmclub.to", "--debug", 
            "--strategy", "--dpi-desync=multisplit --dpi-desync-split-count=5 --dpi-desync-split-seqovl=20 --dpi-desync-fooling=badsum"
        ], capture_output=True, text=True, timeout=120)
        
        success = result.returncode == 0
        
        # Проверяем, что в выводе есть признаки успешного выполнения
        has_attack_success = "Attack tcp_multisplit executed" in result.stdout and "success" in result.stdout
        ...
        print(f"   Result: {'✅ PASS' if success and has_attack_success else '❌ FAIL'}")
...
async def main():
    ...
    if passed == total:
        ...
        print('   python cli.py nnmclub.to --debug --strategy "--dpi-desync=multisplit --dpi-desync-split-count=5 --dpi-desync-split-seqovl=20 --dpi-desync-fooling=badsum"')
