...
def apply_forced_override_patch(file_path):
    ...
    try:
        ...
        replacements = [
            ...
            (
                r'(\{[^}]*"type"[^}]*\})',
                lambda m: add_no_fallbacks_to_config(m.group(1)),
            ),
            # Добавляем forced override в вызовы apply_bypass
            (
                r"(apply_bypass\([^)]*\))",
                lambda m: add_forced_to_apply_bypass(m.group(1, forced=True)),
            ),
            # Добавляем логирование forced override
            (
                r"(logger\.info\([^)]*bypass[^)]*\))",
                lambda m: add_forced_logging(m.group(1)),
            ),
        ]
...
'''
    ...
    print("✅ Создан скрипт проверки: verify_forced_override.py")
