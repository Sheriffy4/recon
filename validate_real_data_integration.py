...
class RealDataValidator:
            ...
            if file_path.exists():
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        self.logger.info(
                            f"Loaded {len(additional)} strategies from {strategy_file}"
                        )
                except Exception as e:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            self.logger.info("[OK] AttackRegistry initialized")
            ...
            self.logger.info("[OK] AttackDispatcher initialized")

            # Initialize strategy loader
            self.strategy_loader = UnifiedStrategyLoader()
            self.logger.info("[OK] UnifiedStrategyLoader initialized")

            # Initialize bypass engine (mock config)
            config = Config()
            self.bypass_engine = WindowsBypassEngine(config)
            self.logger.info("[OK] WindowsBypassEngine initialized")
    ...
    def validate_strategy_parsing(self) -> List[ValidationResult]:
        ...
        self.logger.info("[INFO] Validating strategy parsing with real data...")
        ...
        for domain, strategy_cmd in test_strategies:
            ...
            try:
                if self.strategy_loader:
                    ...
                    parsed = self.strategy_loader._parse_zapret_style(strategy_cmd)
                    ...
                    result = ValidationResult(
                        ...
                        attack_type=(
                            parsed.type if hasattr(parsed, "type") else "unknown"
                        ),
                        parameters=parsed.params if hasattr(parsed, "params") else {},
                        validation_details={
                            "domain": domain,
                    ...
                    self.logger.info(
                        f"[OK] Parsed strategy for {domain}: {parsed.type if hasattr(parsed, 'type') else 'unknown'}"
                    )
                    ...
                    self.logger.info(f"[SIM] Simulated parsing for {domain}")
                ...
                self.logger.error(f"[FAIL] Failed to parse strategy for {domain}: {e}")
    ...
    def validate_attack_dispatch(self) -> List[ValidationResult]:
        ...
        self.logger.info("[INFO] Validating attack dispatch with real data...")
        ...
        for attack_type, real_params in real_attack_types.items():
            ...
            try:
                if self.attack_dispatcher:
                    ...
                    self.logger.info(
                        f"[OK] Dispatched {attack_type}: {len(recipe) if recipe else 0} packets"
                    )
                    ...
                    self.logger.info(f"[SIM] Simulated dispatch for {attack_type}")
                ...
                self.logger.error(f"[FAIL] Failed to dispatch {attack_type}: {e}")
    ...
    def validate_baseline_compatibility(self) -> List[ValidationResult]:
        ...
        self.logger.info("[INFO] Validating baseline data compatibility...")
        ...
        for baseline_name, baseline_data in self.baseline_data.items():
            ...
            try:
                ...
                if "results" in baseline_data:
                    ...
                    for attack_data in sample_attacks:
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Baseline attack validation failed: {attack_name}: {e}"
                            )
                    ...
                    self.logger.info(
                        f"[OK] Baseline {baseline_name}: {successful_attacks}/{total_attacks} attacks validated"
                    )
                ...
                self.logger.error(
                    f"[FAIL] Baseline validation failed for {baseline_name}: {e}"
                )
    ...
    def validate_parameter_handling(self) -> List[ValidationResult]:
        ...
        self.logger.info("[INFO] Validating parameter handling with real values...")
            ...
            try:
                if self.attack_registry:
                    ...
                    validation_result = self.attack_registry.validate_parameters(
                        test_case["attack_type"], test_case["params"]
                    )
                    ...
                    self.logger.info(
                        f"[OK] Parameter validation for {test_case['name']}: {validation_result}"
                    )
                    ...
                    self.logger.info(
                        f"[SIM] Simulated parameter validation for {test_case['name']}"
                    )
                ...
                self.logger.error(
                    f"[FAIL] Parameter validation failed for {test_case['name']}: {e}"
                )
    ...
    def validate_end_to_end_flow(self) -> List[ValidationResult]:
        ...
        self.logger.info("[INFO] Validating end-to-end attack flow...")
            ...
            if domain in self.real_strategies:
                ...
                try:
                    ...
                    if self.strategy_loader and self.bypass_engine:
                        # Parse strategy
                        parsed_strategy = self.strategy_loader._parse_zapret_style(
                            strategy_cmd
                        )

                        # Create mock packet
                        _ = self._create_mock_packet(domain)  # Test packet creation
                        ...
                        result = ValidationResult(
                            ...
                            validation_details={
                                ...
                                "parsed_strategy": (
                                    parsed_strategy.to_dict()
                                    if hasattr(parsed_strategy, "to_dict")
                                    else str(parsed_strategy)
                                ),
                            },
                        )

                        self.logger.info(f"[OK] End-to-end flow validated for {domain}")
                        ...
                        self.logger.info(
                            f"[SIM] Simulated end-to-end flow for {domain}"
                        )
                    ...
                    self.logger.error(
                        f"[FAIL] End-to-end validation failed for {domain}: {e}"
                    )
            ...
            try:
                ...
                if "--dpi-desync=" in strategy_cmd:
                    ...
                    if "--dpi-desync-split-pos=" in strategy_cmd:
                        pos_part = strategy_cmd.split("--dpi-desync-split-pos=")[
                            1
                        ].split()[0]
                        params["split_pos"] = (
                            int(pos_part) if pos_part.isdigit() else pos_part
                        )
                    ...
                    if "--dpi-desync-fooling=" in strategy_cmd:
                        fooling_part = strategy_cmd.split("--dpi-desync-fooling=")[
                            1
                        ].split()[0]
                        params["fooling"] = [fooling_part]

                    if "--dpi-desync-split-count=" in strategy_cmd:
                        count_part = strategy_cmd.split("--dpi-desync-split-count=")[
                            1
                        ].split()[0]
                        params["split_count"] = int(count_part)
                        # For multisplit, generate positions based on split_count
                        if attack_type == "multisplit" and "split_pos" in params:
                            base_pos = params["split_pos"]
                            if isinstance(base_pos, int):
                                params["positions"] = [
                                    base_pos + i for i in range(int(count_part))
                                ]
                            else:
                                params["positions"] = [base_pos] * int(count_part)

                    if "--dpi-desync-split-seqovl=" in strategy_cmd:
                        ovl_part = strategy_cmd.split("--dpi-desync-split-seqovl=")[
                            1
                        ].split()[0]
    ...
    def run_full_validation(self) -> Dict[str, Any]:
        ...
        self.logger.info("[START] Starting comprehensive real data validation...")
        ...
        for suite_name, validation_func in validation_suites:
            self.logger.info(f"\n[SUITE] Running {suite_name} validation...")
            ...
            try:
                ...
                suite_summaries[suite_name] = {
                    ...
                    "success_rate": success_rate,
                    "avg_execution_time": (
                        sum(r.execution_time for r in suite_results) / total_tests
                        if total_tests > 0
                        else 0
                    ),
                }
                ...
                self.logger.info(
                    f"[RESULT] {suite_name}: {successful_tests}/{total_tests} tests passed ({success_rate:.1%})"
                )

            except Exception as e:
                self.logger.error(f"[ERROR] {suite_name} validation failed: {e}")
                suite_summaries[suite_name] = {
                    "total_tests": 0,
                    "successful_tests": 0,
                    "failed_tests": 1,
        ...
        # Save report (handle bytes serialization)
        report_file = (
            self.output_dir
            / f"real_data_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        self.logger.info(
            f"Success Rate: {report['validation_summary']['overall_success_rate']:.1%}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Real data validation for attack dispatch refactor"
    )
    parser.add_argument(
        "--output-dir", default="real_data_validation", help="Output directory"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--suite",
        choices=["parsing", "dispatch", "baseline", "params", "e2e", "all"],
        default="all",
    ...
    print("[INFO] Real Data Validation for Attack Dispatch Refactor")
    print("=" * 60)
    ...
    try:
        if args.suite == "all":
            _ = validator.run_full_validation()  # Run validation but don't use report
        else:
            # Run specific suite
            suite_map = {
            ...
            print(f"\n[RESULTS] {args.suite.upper()} VALIDATION RESULTS")
