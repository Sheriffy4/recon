...
class SubdomainDetector:
    ...
    async def _test_subdomain_connection(self, subdomain: str, timeout: float = 3.0) -> bool:
        """Тестирует доступность поддомена."""
        try:
            # Пробуем HTTPS подключение
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(subdomain, 443),
                timeout=timeout
            )
            writer.close()
            ...
            try:
                ...
                reader, writer = await asyncio.wait_for(
                    asyncio.open_connection(subdomain, 80),
                    timeout=timeout
                )
                writer.close()
                await writer.wait_closed()
...
async def main():
    ...
    monitor_parser.add_argument('--interval', type=int, default=30, help='Интервал проверки в секундах')
