...
class AdvancedPcapAnalyzer:
        ...
        while f.tell() < file_size - 12:
            ...
            try:
                ...
                if block_type == 0x00000006:  # Enhanced Packet Block
                    packet_data = self._parse_enhanced_packet_block(
                        f, pos, block_length
                    )
                    if packet_data:
                ...
                if len(packet_data) >= 34:
                    ...
                    if protocol == 6 and len(packet_data) >= 54:
                        ...
                        if dst_port in [80, 443, 8080, 8443]:
                            packet_info["service"] = (
                                "HTTP" if dst_port in [80, 8080] else "HTTPS"
                            )
    ...
    def _generate_report(self):
        ...
        print("\n🔄 Топ соединений:")
        connection_stats = {
            conn: len(packets) for conn, packets in self.connections.items()
        }
    ...
    def _analyze_bypass_effectiveness(self):
        ...
        https_packets = sum(
            ...
            if p.get("dst_port") == 443 or p.get("src_port") == 443
        )
        https_percentage = (
            (https_packets / total_packets) * 100 if total_packets > 0 else 0
        )
    ...
    def _save_detailed_report(self):
        ...
        report = {
            ...
            "statistics": {
                ...
                "total_size": sum(self.packet_sizes),
                "avg_packet_size": (
                    sum(self.packet_sizes) / len(self.packets) if self.packets else 0
                ),
                "duration": (
                    max(self.timestamps) - min(self.timestamps)
                    if len(self.timestamps) > 1
                    else 0
                ),
            },
            ...
            "top_connections": {
                conn: len(packets)
                for conn, packets in list(self.connections.items())[:20]
            },
        }
...
def main():
    ...
    if success:
        ...
        print("\n💡 РЕКОМЕНДАЦИИ:")
        print("   • Проверьте work_pcap_analysis_report.json для детальной статистики")
        print("   • Если HTTPS трафика много - обход работает хорошо")
        print("   • Если мало трафика - проверьте настройки обхода")
        print("   • Обратите внимание на разнообразие соединений")
    else:
        print("\n❌ Анализ не удался")
