...
def test_ml_prediction_scenario():
    ...
    try:
        ...
        LOG.info(f"INFO     ML predicted strategy for {target_ip} ({domain}): {predicted_strategy} (confidence: {confidence:.2f})")
        ...
        import asyncio
        result = asyncio.run(adapter.execute_attack_by_name(predicted_strategy, context))
...
def main():
    ...
    if passed == total:
        ...
        LOG.info("The AttackStatus error might be intermittent or environment-specific.")
    else:
