...
class StrategySync:
    ...
    def sync_best_to_strategies(self, domain_specific: bool = True) -> bool:
        ...
        if domain_specific and successful_domains:
            ...
            print(
                f"🎯 Adding domain-specific strategies for {len(successful_domains)} domains:"
            )
    ...
    def merge_strategy_updates(self, update_file: str) -> bool:
        ...
        if self.save_json(strategies_data, self.strategies_file):
            ...
            print("❌ Failed to merge strategies")
            return False
    ...
    def status(self) -> None:
        ...
        print(
            f"📁 strategies.json: {'✅ Found' if strategies_exists else '❌ Missing'}"
        )
        ...
        if best_exists:
            ...
            if best_data:
                print(
                    f"📋 Best strategy available: {'✅ Yes' if best_data else '❌ No'}"
                )
        ...
        if strategies_exists:
            ...
            if strategies_data:
                ...
                print(
                    f"📝 Strategy domains: {', '.join(list(strategies_data.keys())[:5])}{'...' if len(strategies_data) > 5 else ''}"
                )
...
def main():
    ...
    parser.add_argument("--update-file", type=str, help="Strategy update file to merge")
