...
# Попытка импортировать Rich для красивого вывода
try:
    ...
    # Создаем классы-заглушки, если Rich не установлен
    class Console:
...
# --- Блок для запуска скрипта напрямую и настройки путей ---
if __name__ == "__main__" and __package__ is None:
    # Этот блок позволяет запускать cli.py как `python cli.py`
    # и исправляет пути для корректного импорта модулей из пакета recon
...
# Импортируем менеджер сигнатур для отображения статистики
...
def run_command(command: list, needs_admin: bool = False, capture_output: bool = False):
    ...
    if needs_admin and not is_admin():
        console.print(
            "[bold red]Ошибка:[/bold red] Для этой операции требуются права администратора."
        )
        console.print(
            "Пожалуйста, перезапустите скрипт из терминала, запущенного от имени администратора."
        )
        return None

    try:
        # Если setup.py запущен с --debug, передаем этот флаг дальше
...
def check_files():
    ...
    if not os.path.exists(SITES_FILE):
        console.print(
            f"[yellow]Файл '{SITES_FILE}' не найден. Создаем файл с примерами...[/yellow]"
        )
        try:
            ...
            console.print(
                f"[bold red]Не удалось создать '{SITES_FILE}': {e}[/bold red]"
            )
...
def show_main_menu():
    ...
    while True:
        ...
        status_text.append(" | ", style="dim")
        status_text.append(
            f"📖 Сигнатур в базе: {stats.get('total_signatures', 0)}", style="blue"
        )
        ...
        console.print(
            Panel(
                panel_content,
                title="[bold cyan]Recon DPI Bypass Manager[/bold cyan]",
                border_style="blue",
            )
        )
...
def action_find_strategy():
    ...
    console.print(
        Panel(
            "[bold]Поиск лучшей стратегии[/bold]\n\n"
            "Сейчас будет запущен процесс анализа и тестирования различных техник обхода.\n"
            f"Он будет использовать домены из файла [cyan]'{SITES_FILE}'[/cyan].\n"
            "Процесс может занять несколько минут.",
            title="[1] Поиск стратегии",
            border_style="green",
        )
    )
    ...
    if os.path.exists(STRATEGY_FILE):
        ...
        console.print(
            "Попробуйте отредактировать список сайтов или запустить поиск еще раз."
        )
...
def action_start_service():
    ...
    console.print(
        Panel(
            "[bold]Запуск службы обхода[/bold]\n\n"
            "Служба будет перехватывать трафик к доменам из "
            f"[cyan]'{SITES_FILE}'[/cyan] и применять к ним лучшую найденную стратегию.\n"
            "Для остановки службы просто закройте это окно или нажмите [bold]Ctrl+C[/bold].",
            title="[2] Запуск службы",
            border_style="green",
        )
    )
    ...
    if not os.path.exists(STRATEGY_FILE):
        console.print(
            f"[bold yellow]Предупреждение:[/bold yellow] Файл '{STRATEGY_FILE}' не найден."
        )
