...
from .tool_detector import ExternalTool, ToolDetector
...
class SyntaxConverter:
    ...
    def _initialize_attack_mappings(self):
        ...
        # Native to external tool attack mappings
        self.native_to_zapret = {
            "tcp_fragmentation": "split",
            "tcp_fake_disorder": "fake,disorder",
            "tcp_multisplit": "split",
            "tcp_seqovl": "fake,split",
            "tls_record_splitting": "tlsrec",
            "ttl_fake_race": "fake",
            "badsum_fooling": "badsum",
            "badseq_fooling": "badseq",
            "md5sig_fooling": "md5sig",
            "http_header_case": "hostcase",
            "http_method_space": "methodspace",
            "http_unix_eol": "unixeol",
            "http_host_padding": "hostpad",
        }

        self.native_to_goodbyedpi = {
            "tcp_fragmentation": "-f",
            "http_header_modification": "-m",
            "fake_packet_injection": "-e",
            "http_persistence_fix": "-p",
            "fragment_replacement": "-r",
            "sni_removal": "-s",
            "wrong_checksum": "-w",
        }

        self.native_to_byebyedpi = {
            "tcp_splitting": "--split-pos",
            "packet_disorder": "--disorder",
            "fake_packet_injection": "--fake-packet",
            "http_modification": "--http-modify",
            "tls_modification": "--tls-modify",
        }
    ...
    def convert(
        ...
        # Auto-detect source tool if not specified
        if source_tool is None:
            ...
            if source_tool == ExternalTool.UNKNOWN:
                return ConversionResult(
                    ...
                    warnings=warnings,
                    errors=["Could not detect source tool format"],
        ...
        try:
            ...
            if parsed_config is None:
                return ConversionResult(
                    ...
                    target_tool=target_tool,
                    warnings=warnings,
                    errors=["Failed to parse source configuration"],
                    metadata={},
                )

            # Convert to native format first
            native_config = self._to_native_format(parsed_config, source_tool)

            # Convert from native to target format
            ...
            # Validate conversion
            validation_warnings = self._validate_conversion(
                native_config, converted_config, target_tool
            )
    ...
    def _to_native_format(
        ...
        if source_tool == ExternalTool.ZAPRET and hasattr(
            ...
            return {
                ...
                "parameters": {},
                "metadata": {
                    "source": source_tool.value,
                    "raw_config": str(parsed_config),
                },
            }

    def _from_native_format(
        self, native_config: Dict[str, Any], target_tool: ExternalTool
    ) -> Dict[str, Any]:
    ...
    def _native_to_zapret(self, native_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Map attack types to zapret methods
    ...
    def _native_to_goodbyedpi(self, native_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Map methods to goodbyedpi flags
    ...
    def _native_to_byebyedpi(self, native_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Map methods to byebyedpi options
        methods = parameters.get("methods", [])
        for method in methods:
    ...
    def _validate_conversion(
        ...
        # Check for attack type compatibility
        attack_type = native_config.get("attack_type", "")
        if (
    ...
    def batch_convert(
        self, configs: List[str], target_tool: ExternalTool
