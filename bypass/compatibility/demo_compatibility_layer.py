...
from .compatibility_bridge import get_compatibility_bridge
...
def demo_tool_detection():
    ...
    test_configs = [
        ("--dpi-desync=split --dpi-desync-split-pos=2", "Zapret basic split"),
        (
            "--dpi-desync=fake,disorder --dpi-desync-ttl=8 --dpi-desync-fooling=badsum",
            "Zapret advanced",
        ),
        ("-f 2 -e -m", "GoodbyeDPI basic"),
        (
            "goodbyedpi.exe -f 2 --wrong-chksum --set-ttl 64",
            "GoodbyeDPI with executable",
        ),
        ("--split-pos 2,10 --disorder --fake-packet", "ByeByeDPI"),
        ('{"attack_type": "tcp_fragmentation", "parameters": {}}', "Native JSON"),
        ("unknown command format", "Unknown format"),
    ]
    ...
    for config, description in test_configs:
        ...
        if detection.detected_features:
            print(f"  Features: {detection.detected_features[:3]}...")  # Show first 3
...
def demo_configuration_parsing():
    ...
    print("\n--- ByeByeDPI Parsing ---")
    byebyedpi_config = (
        "--split-pos 2,10,20 --disorder --disorder-count 3 --fake-packet --fake-ttl 8"
    )
...
def demo_syntax_conversion():
    ...
    test_configs = {
        ExternalTool.ZAPRET: "--dpi-desync=split --dpi-desync-split-pos=2 --dpi-desync-ttl=8",
        ExternalTool.GOODBYEDPI: "-f 2 -e -m --set-ttl 64",
        ExternalTool.BYEBYEDPI: "--split-pos 2,10 --disorder --fake-packet --fake-ttl 8",
    }
            ...
            if target_tool != source_tool:
                ...
                if result.success:
                    if target_tool == ExternalTool.NATIVE:
                        print(
                            f"  → {target_tool.value}: {result.converted_config['attack_type']}"
                        )
                    else:
                    ...
                    print(
                        f"  → {target_tool.value}: FAILED ({result.errors[0] if result.errors else 'Unknown error'})"
                    )
...
def demo_compatibility_analysis():
    ...
    test_configs = [
        "--dpi-desync=fake,split --dpi-desync-fooling=badsum,md5sig --dpi-desync-split-pos=2,midsld --dpi-desync-ttl=8",
        "-f 2 -e -m -p -s --wrong-chksum --set-ttl 64",
        "--split-pos 2,10,20 --disorder --disorder-count 3 --fake-packet --fake-ttl 8 --http-modify",
    ]
    ...
    for i, config in enumerate(test_configs, 1):
        ...
        print(
            f"Detected tool: {report.detected_tool.value} (confidence: {report.detection_confidence:.2f})"
        )
        ...
        successful_conversions = [
            tool.value
            for tool, result in report.conversion_results.items()
            if result.success
        ]
        failed_conversions = [
            tool.value
...
def demo_tool_capabilities():
    ...
    for tool in tools:
        ...
        for strength in capabilities["strengths"][:3]:  # Show first 3
            print(f"  + {strength}")

        print(f"Limitations ({len(capabilities['limitations'])}):")
        for limitation in capabilities["limitations"][:2]:  # Show first 2
            print(f"  - {limitation}")
...
def demo_optimal_tool_suggestion():
    ...
    scenarios = [
        (["simple", "windows", "easy"], "Simple Windows setup"),
        (["advanced", "complex", "linux"], "Advanced Linux setup"),
        (["cross-platform", "integration", "api"], "Cross-platform integration"),
        (["migration", "compatibility", "advanced"], "Migration project"),
    ]
...
def demo_real_world_examples():
    ...
    examples = {
        "Basic YouTube bypass (Zapret)": "--dpi-desync=split --dpi-desync-split-pos=midsld",
        "Advanced multi-method (Zapret)": "--dpi-desync=fake,split,disorder --dpi-desync-fooling=badsum --dpi-desync-split-pos=2,10 --dpi-desync-ttl=8",
        "Simple fragmentation (GoodbyeDPI)": "-f 2",
        "Comprehensive bypass (GoodbyeDPI)": "-f 2 -e -m -p -s --wrong-chksum",
        "Modern approach (ByeByeDPI)": "--split-pos 2,10 --disorder --http-modify",
    }
    ...
    for description, config in examples.items():
        ...
        # Convert to native for standardization
