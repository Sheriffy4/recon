...
from .config_models import ConfigurationBackup, ConfigurationVersion
...
class BackupManager:
    ...
    def create_backup(
        ...
        backup = ConfigurationBackup(
            id=backup_id,
            original_path=str(config_path.absolute()),
    ...
    def restore_backup(self, backup_id: str, target_path: Optional[str] = None) -> str:
        ...
        if target_path.exists():
            current_backup_id = self.create_backup(
                str(target_path), f"Auto-backup before restore of {backup_id}"
            )
