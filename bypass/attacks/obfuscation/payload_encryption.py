# recon/core/bypass/attacks/obfuscation/payload_encryption.py
"""
...
class XORPayloadEncryptionAttack(BaseAttack):
    ...
    def _generate_encryption_key(
        ...
        if strategy == "random":
            ...
            # Use TCP sequence number as key source
...
class ChaCha20PayloadEncryptionAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            key = random.randbytes(32)  # ChaCha20 uses 256-bit keys
    ...
    def _generate_nonce(self, strategy: str, context: AttackContext) -> bytes:
        ...
        if strategy == "random":
            return random.randbytes(12)  # ChaCha20 uses 96-bit nonces
        elif strategy == "counter":
            # Use packet counter as nonce
...
class MultiLayerEncryptionAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        try:
            ...
            # Add noise between layers if requested
            if add_noise:
