...
try:
    from .zapret_strategy import ZapretStrategy, ZapretConfig, create_zapret_strategy
    from .native_combo_engine import (
...
# Preset configurations based on real-world effectiveness
ZAPRET_PRESETS = {
    "default": {
...
class ZapretIntegration:
    ...
    async def execute_preset(
        ...
        # Method 1: Use combo engine if available
        if self.combo_engine and preset_name in self.registered_presets:
            try:
                ...
                # Convert combo result to attack result
                if result.attack_results:
                    return result.attack_results[
                        0
                    ]  # Return first (and only) attack result
                else:
                    return AttackResult(
    ...
    async def execute_custom(
        ...
        # Add custom configuration info to result
        if result.details:
        ...
        async def test_preset(preset_name: str) -> tuple:
            async with semaphore:
                try:
                    ...
                    return preset_name, AttackResult(
                        success=False,
                        technique_used=f"zapret_{preset_name}",
                        error_message=str(e),
                    )

        # Execute all preset tests
