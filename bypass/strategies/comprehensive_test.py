...
def test_comprehensive_functionality():
    ...
    social_pool = manager.create_pool(
        "Social Media Sites", social_strategy, "Optimized for social media"
    )
    ...
    default_pool = manager.create_pool(
        "Default Sites", default_strategy, "Default strategy"
    )
    ...
    manager.add_assignment_rule(
        pattern=r".*(cloudflare|fastly|akamai|cdn).*", pool_id=cdn_pool.id, priority=8
    )
    ...
    test_cases = [
        ...
        (
            ...
            443,
            "Default Strategy",
        ),  # Should use fallback via default pool
    ]
    ...
    for domain, port, expected_name in test_cases:
        ...
        print(
            f"  {status} {domain}:{port} -> {actual_name} (expected: {expected_name})"
        )
    ...
    streaming_pool = manager.create_pool(
        "Streaming Services", cdn_strategy, "Streaming services"
    )
    ...
    merged_pool = manager.merge_pools(
        [video_pool.id, streaming_pool.id], "Media Sites", merged_strategy
    )
    pools_after = len(manager.pools)

    if merged_pool and pools_after == pools_before - 1:
        print(
            f"  ✓ Successfully merged pools: {len(merged_pool.domains)} domains, tags: {merged_pool.tags}"
        )
    else:
    ...
    if len(social_pool.domains) >= 2:
        domain_groups = {
            ...
            "social": [
                d
                for d in social_pool.domains
                if "twitter" in d or "facebook" in d or "instagram" in d
            ],
        }
        ...
        if len(domain_groups["video"]) > 0 and len(domain_groups["social"]) > 0:
            ...
            pools_before = len(manager.pools)
            new_pools = manager.split_pool(
                social_pool.id, domain_groups, split_strategies
            )
            pools_after = len(manager.pools)
