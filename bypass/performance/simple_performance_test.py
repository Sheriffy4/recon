...
async def test_performance_optimizer():
    ...
    try:
        ...
        print(
            f"✅ Optimization completed: {result.improvement_percentage:.2f}% improvement"
        )

        # Test system health
        health = await optimizer.get_system_health()
        print(
            f"✅ System health: Load {health.system_load:.2f}, Uptime {health.uptime/3600:.1f}h"
        )
...
async def test_strategy_optimizer():
    ...
    try:
        ...
        selected = await optimizer.optimize_strategy_selection(
        ...
        print(
            f"✅ Algorithm parameters optimized: {params.get('current_level', 'unknown')}"
        )
