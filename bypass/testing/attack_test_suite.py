...
class TestExecutor:
    ...
    async def execute_test(
        self, test_case: TestCase, attack_registry: ModernAttackRegistry
    ) -> TestResult:
        """Execute a single test case."""
        start_time = datetime.now()

        result = TestResult(
            test_case_id=test_case.id, status=TestStatus.RUNNING, start_time=start_time
        )

        try:
            # Get attack definition and instance
            ...
            attack_instance = attack_registry.create_attack_instance(
                test_case.attack_id
            )
            if not attack_instance:
                result.status = TestStatus.ERROR
                result.error_message = (
                    f"Failed to create attack instance for {test_case.attack_id}"
                )
                return result

            # Execute with safety controls
            execution_result = await self._execute_with_safety(
                attack_instance, test_case, attack_def
            )

            # Validate results
            validation_results = await self._validate_test_results(
            ...
            result.reliability_score = sum(validation_results.values()) / len(
                validation_results
            )
    ...
    async def _execute_with_safety(
        self, attack_instance, test_case: TestCase, attack_def: AttackDefinition
    ) -> Dict[str, Any]:
        ...
        try:
            # Execute the attack test
            # This is a simplified execution - in real implementation,
            # you would create proper AttackContext and execute the attack
            execution_result = {
                "success": True,
                "response_time": time.time() - start_time,
                "performance_metrics": {
                    "execution_time_ms": (time.time() - start_time) * 1000,
                    "memory_usage_mb": (process.memory_info().rss - initial_memory)
                    / 1024
                    / 1024,
                    "cpu_usage_percent": process.cpu_percent() - initial_cpu_percent,
                },
            }
            ...
            try:
                if method == ValidationMethod.HTTP_RESPONSE:
                    ...
                    validation_results[method] = (
                        await self.reliability_validator.validate_domain_accessibility(
                            test_case.test_domain
                        )
                    )
                    ...
                    validation_results[method] = await self._multi_request_validation(
                        test_case
                    )
    ...
    async def _multi_request_validation(self, test_case: TestCase) -> bool:
        ...
        try:
            ...
            # Consider successful if at least 2/3 requests succeed
...
class StabilityTester:
    ...
    async def test_attack_stability(
        self, attack_id: str, duration_minutes: int = 30, interval_seconds: int = 60
    ) -> StabilityResult:
        """Test attack stability over time."""
        LOG.info(
            f"Starting stability test for {attack_id} (duration: {duration_minutes}m)"
        )
        ...
        if len(execution_times) > 1:
            ...
            if first_half and second_half:
                ...
                performance_degradation = (
                    (avg_second - avg_first) / avg_first if avg_first > 0 else 0.0
                )
        ...
        result = StabilityResult(
            ...
            failure_patterns=list(set(failure_patterns)),  # Remove duplicates
            error_types=error_types,
            performance_degradation=performance_degradation,
        )

        LOG.info(
            f"Stability test completed for {attack_id}: {stability_score:.2%} success rate"
        )
        return result
...
class PerformanceBenchmarker:
    ...
    async def benchmark_attack(
        self, attack_id: str, iterations: int = 100
    ) -> BenchmarkResult:
        """Benchmark attack performance."""
        LOG.info(
            f"Starting performance benchmark for {attack_id} ({iterations} iterations)"
        )
        ...
        result = BenchmarkResult(
            attack_id=attack_id,
            test_name=f"benchmark_{iterations}_iterations",
        ...
        LOG.info(
            f"Benchmark completed for {attack_id}: {success_rate:.2%} success rate, {avg_time:.3f}s avg time"
        )
        return result
...
class RegressionTester:
        ...
        try:
            ...
            for k, v in baseline_data.get("results", {}).items():
                ...
                result = TestResult(
                    ...
                    status=TestStatus(v["status"]),
                    start_time=datetime.fromisoformat(v["start_time"]),
                )
                result.end_time = (
                    datetime.fromisoformat(v["end_time"]) if v.get("end_time") else None
                )
    ...
    async def run_regression_tests(self) -> Dict[str, Any]:
        ...
        # Test all attacks that have baseline results
        for test_case_id, baseline_result in self.baseline_results.items():
            try:
                # Find the attack and test case
                attack_id = baseline_result.test_case_id.split("_")[
                    0
                ]  # Assuming format: attackid_testcaseid
                attack_def = self.attack_registry.get_attack_definition(attack_id)
                ...
                # Run current test
                current_result = await self.test_executor.execute_test(
                    test_case, self.attack_registry
                )
                current_results[test_case_id] = current_result

                # Compare with baseline
                regression_issue = self._compare_with_baseline(
    ...
    def _compare_with_baseline(
        ...
        # Check for significant performance degradation (>50% slower)
        if baseline.duration > 0 and current.duration > baseline.duration * 1.5:
            issues.append(
                f"Performance degradation: {current.duration:.3f}s vs {baseline.duration:.3f}s baseline"
            )

        # Check for reliability score degradation (>20% drop)
        if (
            baseline.reliability_score > 0
            and current.reliability_score < baseline.reliability_score * 0.8
        ):
            issues.append(
                f"Reliability degradation: {current.reliability_score:.2f} vs {baseline.reliability_score:.2f} baseline"
            )
        ...
        if issues:
            return {
                ...
                "baseline_reliability": baseline.reliability_score,
                "current_reliability": current.reliability_score,
            }
...
class ComprehensiveTestSuite:
    ...
    async def run_comprehensive_tests(
        ...
        stability_duration_minutes: int = 10,
        ...
        start_time = datetime.now()
        report = TestReport(
            suite_id=f"comprehensive_{start_time.strftime('%Y%m%d_%H%M%S')}",
        ...
        try:
            ...
            if include_stability:
                ...
                stability_results = await self._run_stability_tests(
                    attack_ids, stability_duration_minutes
                )
            ...
            if include_benchmarks:
                ...
                benchmark_results = await self._run_benchmarks(
                    attack_ids, benchmark_iterations
                )
            ...
            if include_regression:
                ...
                # Add regression results to report (could extend TestReport to include this)
        ...
        LOG.info(
            f"Results: {report.passed_tests}/{report.total_tests} passed ({report.success_rate:.1%})"
        )
    ...
    async def _run_basic_tests(self, attack_ids: List[str]) -> List[TestResult]:
        ...
        if len(test_cases) <= self.max_parallel_tests:
            # Run all in parallel
            tasks = [
                self.test_executor.execute_test(test_case, self.attack_registry)
    ...
    async def _run_stability_tests(
        self, attack_ids: List[str], duration_minutes: int
    ) -> List[StabilityResult]:
        """Run stability tests for selected attacks."""
        results = []

        # Run stability tests sequentially to avoid resource conflicts
        for attack_id in attack_ids:
            try:
                result = await self.stability_tester.test_attack_stability(
                    attack_id, duration_minutes
                )
    ...
    async def _run_benchmarks(
        self, attack_ids: List[str], iterations: int
    ...
    async def run_quick_tests(
        self, attack_ids: Optional[List[str]] = None
    ) -> TestReport:
    ...
    async def run_complexity_tests(self, complexity: AttackComplexity) -> TestReport:
        ...
        attack_ids = self.attack_registry.list_attacks(
            complexity=complexity, enabled_only=True
        )
        return await self.run_comprehensive_tests(attack_ids=attack_ids)
    ...
    def generate_test_summary(self, report: TestReport) -> str:
        ...
        if report.benchmark_results:
            ...
            avg_success_rate = statistics.mean(
                [br.success_rate for br in report.benchmark_results]
            )
        ...
        if report.stability_results:
            ...
            avg_stability = statistics.mean(
                [sr.stability_score for sr in report.stability_results]
            )
...
async def run_attack_test(
    attack_id: str, registry: ModernAttackRegistry = None
) -> TestResult:
