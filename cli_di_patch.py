...
async def initialize_cli_services(args):
    ...
    try:
        ...
        if args.debug:
            ...
            console.print(f"[dim]Registered services: {len(container_info.get('registered_services', []))}[/dim]")
...
def create_manual_fingerprint_engine(domain_ip: str, port: int):
    ...
    return UltimateAdvancedFingerprintEngine(prober=prober, classifier=classifier, attack_adapter=attack_adapter, debug=False)

async def create_strategy_generator_from_di(cli_integration, fingerprint_dict: Dict[str, Any], args):
    """
    ...
    return AdvancedStrategyGenerator(fingerprint_dict=fingerprint_dict, history=[], parameter_optimizer=parameter_optimizer)
...
async def run_hybrid_mode_with_di(args, report_logger, cli_integration, services):
    ...
    """
    attack_adapter = services.attack_adapter if hasattr(services, 'attack_adapter') else services['attack_adapter']
    result_processor = services.result_processor if hasattr(services, 'result_processor') else services['result_processor']
    if cli_integration:
        domain_services = cli_integration.create_domain_specific_services('example.com', '1.2.3.4', 443)
