...
async def initialize_cli_services(args):
    ...
    try:
        ...
        # Display container info if debug enabled
        if args.debug:
...
# Utility function to replace the service creation block in CLI
...
# Example of how to update a function in CLI to use DI
async def run_hybrid_mode_with_di(args, report_logger, cli_integration, services):
    ...
    """
    # Use services from DI instead of creating them manually
    attack_adapter = (
    ...
    # Rest of the function remains the same, but uses DI services
    # ... (existing hybrid mode logic)

    # Example of creating domain-specific services
    if cli_integration:
