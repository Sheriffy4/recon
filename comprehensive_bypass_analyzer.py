...
class UnifiedPcapAnalyzer:
    ...
    def __init__(self, pcap_file: str = "work.pcap"):
        ...
        self.blocked_domains = {
            ...
            'facebook.com', 'youtube.com', 'telegram.org', 'twitter.com'
        }
    ...
    def analyze(self) -> Dict:
        ...
        LOG.info(f"Анализ завершен: {results['summary'].get('successful_connections', 0)} успешных соединений из {results['summary'].get('total_attempts', 0)}")
            ...
            try:
                ...
                if any(b in domain for b in self.blocked_domains):
                    for i in range(dns.ancount):
                        if hasattr(dns.an[i], 'rdata') and dns.an[i].type == 1:  # A record
    ...
    def _analyze_tcp_packet_improved(self, packet, results):
        ...
        elif tcp.flags.A and not tcp.flags.S and not tcp.flags.F and not tcp.flags.R:  # Pure ACK
            conn['has_ack'] = True
    ...
    def _post_process_analysis(self, results):
        ...
        for conn_key, conn in results.get('connections', {}).items():
            ...
            if conn['has_syn'] or conn['has_data']:
                ...
                if conn['has_syn'] and conn['has_syn_ack'] and conn['has_ack']:
                    ...
                    is_successful = True
                    LOG.debug(f"Успешное соединение (есть данные {conn['data_bytes']} байт): {conn_key}")
                ...
                if is_successful:
                    ...
                    # Проверяем таймаут
                    duration = conn['last_time'] - conn['start_time'] if conn['start_time'] else 0
                    if duration > 5.0 and not conn['has_syn_ack']:
                        if conn['domain']:
                            results['domain_stats'][conn['domain']]['failed_timeout'] += 1
                        LOG.debug(f"Неуспешное соединение (таймаут {duration:.1f}s): {conn_key}")
        ...
        results['summary']['success_rate'] = (successful_connections / total_attempts * 100) if total_attempts > 0 else 0
        results['summary']['bypass_active'] = sum(results['bypass_indicators'].values()) > 0
        ...
        for domain, stats in results['domain_stats'].items():
            if stats['connections'] > 0:
                ...
                avg_ttl = sum(stats['avg_ttl']) / len(stats['avg_ttl']) if stats['avg_ttl'] else 0
                stats['avg_ttl_value'] = avg_ttl
                LOG.info(f"Домен {domain}: {stats['successful']}/{stats['connections']} успешных ({stats['success_rate']:.1f}%)")
        
        # Отладочная информация
        LOG.info(f"Отладочная статистика:")
                ...
                if ext_type == 0x0000:  # SNI extension
                    ...
                    if ext_len >= 5:
                        ...
                        if payload[cursor+2] == 0x00:  # host_name type
                            name_len = int.from_bytes(payload[cursor+3:cursor+5], 'big')
                            name_start = cursor + 5
                            if name_start + name_len <= cursor + ext_len:
                                sni = payload[name_start:name_start + name_len].decode('ascii', 'ignore')
                                return {'sni': sni}
...
class ImprovedComprehensiveAnalyzer:
    ...
    def _load_current_strategies(self) -> Dict:
        ...
        strategy_files = ['strategies.json', 'best_strategy.json', 'optimized_strategies.json']
            ...
            if os.path.exists(filename):
                try:
                    with open(filename, 'r', encoding='utf-8') as f:
                        ...
                        LOG.info(f"Загружены стратегии из {filename}: {len(loaded)} записей")
                except Exception as e:
    ...
    def _generate_domain_specific_recommendations(self) -> Dict:
        ...
        for domain, stats in domain_stats.items():
            ...
            if success_rate < 30:
                ...
                domain_recs[domain] = self._get_optimized_strategy(domain, rst_count, data_transferred)
            elif rst_count > 3:
    ...
    def _get_optimized_strategy(self, domain: str, rst_count: int, data_transferred: int) -> str:
    ...
    def _get_anti_rst_strategy(self, domain: str) -> str:
        ...
        if success_rate < 30:
            recs['problems_found'].append(f"Очень низкая успешность соединений ({success_rate:.1f}%)")
            recs['immediate_actions'].append("Срочно пересмотреть стратегии обхода")
        elif success_rate < 70:
            recs['problems_found'].append(f"Недостаточная успешность соединений ({success_rate:.1f}%)")
            recs['immediate_actions'].append("Оптимизировать стратегии для проблемных доменов")
    ...
    def _generate_smart_recommendations(self) -> Dict:
        ...
        if not dns_analysis.get('queries'):
            recs['optimization_potential'].append("Можно улучшить DNS разрешение через DoH")
        
        # Анализ TLS
        tls_analysis = self.pcap_results.get('tls', {})
        if len(tls_analysis.get('client_hellos', [])) > len(tls_analysis.get('server_hellos', [])):
            ratio = len(tls_analysis.get('server_hellos', [])) / max(len(tls_analysis.get('client_hellos', [])), 1)
            if ratio < 0.5:
                recs['problems_found'].append(f"Много неудачных TLS handshake ({ratio:.1%})")
        
        # Конфигурация сервиса
        recs['service_config'] = {
            'bypass_detected': sum(self.pcap_results.get('bypass_indicators', {}).values()) > 0,
            'recommended_ttl': 4,
            'recommended_strategy': 'fake,disorder' if success_rate > 50 else 'multisplit',
            'enable_monitoring': True
        }
        
        # Оптимизации
        if success_rate > 80:
            recs['optimization_potential'].append("Система работает хорошо, можно снизить агрессивность")
        else:
            recs['optimization_potential'].append("Использовать ECH (Encrypted Client Hello) для скрытия SNI")
        ...
        if tls_analysis.get('client_hellos') and not tls_analysis.get('server_hellos'):
            recs['problems_found'].append("TLS блокировка - ClientHello отправляются, но нет ServerHello")
            recs['immediate_actions'].append("Использовать стратегии разделения TLS handshake")
    ...
    def _print_comprehensive_report(self):
        ...
        print(f"  • Успешных соединений: {pcap_summary.get('successful_connections', 0)}")
        print(f"  • Успешность: {pcap_summary.get('success_rate', 0):.1f}%")
        print(f"  • Обход активен: {'Да' if pcap_summary.get('bypass_active') else 'Нет'}")
        ...
        if domain_stats:
            ...
            for domain, stats in domain_stats.items():
                success_rate = stats.get('success_rate', 0)
                status = "✅" if success_rate > 70 else "⚠️" if success_rate > 30 else "❌"
    ...
    def _save_optimized_config(self):
        ...
        if self.recommendations.get('optimized_strategies'):
            with open("optimized_strategies_final.json", 'w', encoding='utf-8') as f:
                json.dump(self.recommendations['optimized_strategies'], f, indent=2, ensure_ascii=False)
        ...
        print(f"💾 Сохранено: comprehensive_analysis_report.json")
        ...
        if self.recommendations.get('hosts_entries'):
            with open("recommended_hosts_entries.txt", 'w', encoding='utf-8') as f:
                ...
                for entry in list(set(self.recommendations['hosts_entries'])):
                    f.write(f"{entry}\n")
            print(f"💾 Сохранено: recommended_hosts_entries.txt")
