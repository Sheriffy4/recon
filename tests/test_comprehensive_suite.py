...
class TestEndToEndFingerprinting(unittest.TestCase):
    ...
    def test_cache_integration(self):
        ...
        # Create test fingerprint
        fingerprint = DPIFingerprint(
            target="cached-site.com", dpi_type=DPIType.COMMERCIAL_DPI, confidence=0.9
        )
...
class TestPerformanceBenchmarks(unittest.TestCase):
    ...
    def test_fingerprinting_speed_benchmark(self, mock_dns, mock_http, mock_tcp):
        ...
        try:
            ...
            self.assertLess(
                duration, 5.0, "Single fingerprint should complete within 5 seconds"
            )
            self.assertIsNotNone(fingerprint)
    ...
    def test_batch_fingerprinting_benchmark(self, mock_dns, mock_http, mock_tcp):
        ...
        try:
            ...
            successful_fingerprints = [
                fp for fp in fingerprints if isinstance(fp, DPIFingerprint)
            ]

            self.assertGreater(len(successful_fingerprints), 0)
            self.assertLess(
                duration, 30.0, "Batch fingerprinting should complete within 30 seconds"
            )

            avg_time_per_fingerprint = duration / len(successful_fingerprints)
            self.assertLess(
                avg_time_per_fingerprint,
                5.0,
                "Average time per fingerprint should be under 5s",
            )
    ...
    def test_cache_performance_benchmark(self):
        ...
        # Performance assertions
        self.assertLess(
            write_duration, 5.0, "Cache writes should complete within 5 seconds"
        )
        self.assertLess(
            read_duration, 1.0, "Cache reads should complete within 1 second"
        )
    ...
    def test_strategy_generation_performance(self):
        ...
        self.assertLess(
            duration, 2.0, "Strategy generation should complete within 2 seconds"
        )
...
class TestStressTests(unittest.TestCase):
    ...
    def test_concurrent_fingerprinting_stress(self, mock_dns, mock_http, mock_tcp):
        ...
        try:
            ...
            self.assertGreater(
                len(successful_results), 40, "At least 80% of requests should succeed"
            )
            self.assertLess(
                len(failed_results), 10, "Less than 20% of requests should fail"
            )
            self.assertLess(
                duration, 60.0, "Stress test should complete within 60 seconds"
            )
    ...
    def test_cache_concurrent_access_stress(self):
        ...
        # Stress test assertions
        self.assertGreater(
            len(successful_reads), 180, "At least 90% of reads should succeed"
        )
        self.assertLess(
            write_duration, 10.0, "Concurrent writes should complete within 10 seconds"
        )
        self.assertLess(
            read_duration, 5.0, "Concurrent reads should complete within 5 seconds"
        )

        print(
            f"Concurrent write duration (200 items, 10 threads): {write_duration:.3f}s"
        )
    ...
    def test_memory_usage_stress(self):
        ...
        for i in range(1000):
            fp = DPIFingerprint(
                ...
                confidence=0.8,
                raw_metrics={
                    f"metric_{j}": f"value_{j}" for j in range(50)
                },  # Add bulk data
            )
        ...
        self.assertLess(
            memory_increase,
            200,
            "Memory increase should be less than 200MB for 1000 fingerprints",
        )
        self.assertLess(
            after_processing_memory - initial_memory,
            300,
        ...
        print(
            f"After creation: {after_creation_memory:.1f}MB (+{memory_increase:.1f}MB)"
        )
...
class TestRegressionTests(unittest.TestCase):
    ...
    def test_strategy_generation_regression(self):
        ...
        strategies_with_fp = generator.generate_strategies(
            fingerprint=fingerprint, count=10
        )

        # Test without fingerprint
        strategies_without_fp = generator.generate_strategies(
            fingerprint=None, count=10
        )
