...
"""
import json
from recon.tests.tool_detector import ToolDetector, ExternalTool
from recon.tests.zapret_parser import ZapretConfigParser
from recon.tests.goodbyedpi_parser import GoodbyeDPIParser
from recon.tests.byebyedpi_parser import ByeByeDPIParser
from recon.tests.syntax_converter import SyntaxConverter
from recon.tests.compatibility_bridge import CompatibilityBridge
...
class TestToolDetector:
    ...
    def test_detect_zapret_format(self):
        ...
        zapret_commands = ['--dpi-desync=split --dpi-desync-split-pos=2', '--dpi-desync=fake,disorder --dpi-desync-ttl=8', '--dpi-desync-fooling=badsum --wssize=1024']
    ...
    def test_detect_goodbyedpi_format(self):
        ...
        goodbyedpi_commands = ['-f 2 -e -m', 'goodbyedpi.exe -f 2 --wrong-chksum', '-p -r -s --max-payload 1024']
    ...
    def test_detect_byebyedpi_format(self):
        ...
        byebyedpi_commands = ['--split-pos 2,10 --disorder', 'byebyedpi --fake-packet --fake-ttl 8', '--http-modify --fragment-size 1024']
    ...
    def test_detect_native_format(self):
        ...
        native_configs = ['{"attack_type": "tcp_fragmentation", "parameters": {}}', '{"strategy_id": "test", "attacks": []}']
...
class TestZapretParser:
    ...
    def test_parse_fake_disorder(self):
        ...
        command = '--dpi-desync=fake,disorder --dpi-desync-ttl=8 --dpi-desync-fooling=badsum'
        config = self.parser.parse(command)
...
class TestSyntaxConverter:
    ...
    def test_zapret_to_native(self):
        ...
        zapret_command = '--dpi-desync=split --dpi-desync-split-pos=2 --dpi-desync-ttl=8'
        result = self.converter.convert(zapret_command, ExternalTool.NATIVE, ExternalTool.ZAPRET)
    ...
    def test_goodbyedpi_to_native(self):
        ...
        result = self.converter.convert(goodbyedpi_command, ExternalTool.NATIVE, ExternalTool.GOODBYEDPI)
        assert result.success
    ...
    def test_native_to_zapret(self):
        ...
        native_config = {'attack_type': 'zapret_combo', 'parameters': {'desync_methods': ['split'], 'split_positions': [{'type': 'absolute', 'value': 2}], 'ttl': 8}}
        result = self.converter.convert(json.dumps(native_config), ExternalTool.ZAPRET, ExternalTool.NATIVE)
        assert result.success
    ...
    def test_zapret_to_goodbyedpi(self):
        ...
        result = self.converter.convert(zapret_command, ExternalTool.GOODBYEDPI, ExternalTool.ZAPRET)
        assert result.success or len(result.warnings) > 0

    def test_batch_conversion(self):
        """Test batch conversion of multiple configurations."""
        configs = ['--dpi-desync=split --dpi-desync-split-pos=2', '-f 2 -e', '--split-pos 2,10']
    ...
    def test_conversion_validation(self):
        ...
        zapret_command = '--dpi-desync=fake,split --dpi-desync-fooling=md5sig --dpi-desync-ttl=8'
        result = self.converter.convert(zapret_command, ExternalTool.GOODBYEDPI, ExternalTool.ZAPRET)
...
class TestCompatibilityBridge:
    ...
    def test_migration_summary(self):
        ...
        from recon.tests.syntax_converter import ConversionResult
...
class TestIntegration:
    ...
    def test_end_to_end_zapret_conversion(self):
        ...
        assert report.detected_tool == ExternalTool.ZAPRET
        assert report.parsing_success
        native_result = self.bridge.convert_configuration(zapret_config, ExternalTool.NATIVE)
        assert native_result.success
        zapret_result = self.bridge.convert_configuration(json.dumps(native_result.converted_config), ExternalTool.ZAPRET, ExternalTool.NATIVE)
    ...
    def test_cross_tool_conversion(self):
        ...
        zapret_result = self.bridge.convert_configuration(goodbyedpi_config, ExternalTool.ZAPRET)
        byebyedpi_result = self.bridge.convert_configuration(goodbyedpi_config, ExternalTool.BYEBYEDPI)
    ...
    def test_error_handling(self):
        ...
        invalid_configs = ['completely invalid syntax', '--invalid-zapret-option=value', '-z invalid_goodbyedpi_flag']
        for config in invalid_configs:
            report = self.bridge.analyze_configuration(config)
            assert report.detected_tool == ExternalTool.UNKNOWN or not report.parsing_success or len(report.validation_issues) > 0

def run_compatibility_tests():
    """Run all compatibility layer tests."""
    test_classes = [TestToolDetector, TestZapretParser, TestGoodbyeDPIParser, TestByeByeDPIParser, TestSyntaxConverter, TestCompatibilityBridge, TestIntegration]
    total_tests = 0
    passed_tests = 0
    failed_tests = []
