...
from core.bypass.strategies.config_models import DPIConfig, SplitConfig, FoolingConfig, TCPPacketInfo
from core.bypass.strategies.exceptions import DPIStrategyError, ConfigurationError
...
class TestStrategyIntegration:
    ...
    def test_sni_plus_numeric_position_priority(self):
        ...
        # If SNI was found, it should be prioritized
        ...
        if sni_position is not None and sni_position in positions:
            ...
            assert positions[0] == sni_position
    ...
    def test_sni_priority_over_numeric_positions(self):
        ...
        tls_packet = self._create_tls_client_hello_with_sni_at_position("test.com", target_position=10)
    ...
    def test_multiple_strategy_combinations(self):
        ...
        test_cases = [
            # Numeric only
            {
                'config': DPIConfig(desync_mode="split", split_positions=[3, 10], fooling_methods=[], enabled=True),
                'expected_split': True
            },
            # SNI only
            {
            ...
            {
                'config': DPIConfig(desync_mode="split", split_positions=[], fooling_methods=["badsum"], enabled=True),
                'expected_split': False
            },
            # All combined
            {
                'config': DPIConfig(desync_mode="split", split_positions=[3, "sni"], fooling_methods=["badsum"], enabled=True),
                'expected_split': True
            }
        ]
    ...
    def test_fallback_scenarios_malformed_tls(self):
        ...
        malformed_tls = b'\x16\x03\x03\x00\x50' + b'\xFF' * 80  # Looks like TLS but malformed
    ...
    def test_performance_integration_multiple_packets(self):
        ...
        packets = [
            self._create_tls_client_hello_with_sni(f"test{i}.com")
            for i in range(20)
        ]
    ...
    def test_component_interaction_badsum_application(self):
        ...
        should_apply = self.checksum_fooler.should_apply_badsum(tcp_info, is_first_part=True)
        
        if should_apply:
            # Apply badsum and verify
            modified_packet, result = self.checksum_fooler.apply_badsum(tls_packet, tcp_info)
    ...
    def _create_tls_client_hello_with_sni_at_position(self, hostname: str, target_position: int) -> bytes:
    ...
    def _create_http_packet(self) -> bytes:
        ...
        ip_header = struct.pack('!BBHHHBBH4s4s',
            0x45, 0x00, len(http_request) + 40,  # Version, TOS, Total length
            0x1234, 0x4000,  # ID, Flags
        ...
        tcp_header = struct.pack('!HHIIBBHHH',
            54321, 80,  # Source port, Dest port (HTTP)
            1000, 2000,  # Seq, Ack
            0x50, 0x18,  # Data offset, Flags
            65535, 0x1234, 0  # Window, Checksum, Urgent
        )
    ...
    def _create_large_tls_client_hello(self) -> bytes:
        ...
        base_packet = bytearray(self._create_tls_client_hello_with_sni("large-packet-test.example.com"))
    ...
    def _extract_tcp_checksum_if_present(self, packet: bytes) -> int:
        ...
        try:
            ...
            return struct.unpack('!H', packet[tcp_checksum_offset:tcp_checksum_offset + 2])[0]
        except:
            return None
...
class TestStrategyIntegrationAdvanced:
    ...
    def test_concurrent_packet_processing(self):
        ...
        # Create test packets
        packets = [
            TestStrategyIntegration()._create_tls_client_hello_with_sni(f"concurrent{i}.com")
            for i in range(10)
        ]
        ...
        for i, engine in enumerate(engines):
            ...
            end_idx = start_idx + packets_per_thread if i < len(engines) - 1 else len(packets)
            thread_packets = packets[start_idx:end_idx]
            
            thread = threading.Thread(target=process_packets, args=(engine, thread_packets))
    ...
    def test_memory_usage_large_volume(self):
        ...
        for i in range(100):
            packet = TestStrategyIntegration()._create_tls_client_hello_with_sni(f"memory-test-{i}.com")
            result = engine.apply_strategy(packet)
    ...
    def test_stress_test_error_conditions(self):
        ...
        problematic_packets = [
            ...
            TestStrategyIntegration()._create_tls_client_hello_with_sni(""),  # Empty SNI
            TestStrategyIntegration()._create_tls_client_hello_with_sni("a" * 300),  # Very long SNI
        ]
    ...
    def test_configuration_hot_swap(self):
        ...
        packet = TestStrategyIntegration()._create_tls_client_hello_with_sni("hotswap-test.com")
