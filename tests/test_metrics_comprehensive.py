...
async def demonstrate_protocol_metrics():
    ...
    with patch('asyncio.open_connection') as mock_open_connection:
        ...
        def mock_connection(*args, **kwargs):
            nonlocal call_count
            mock_reader.read = AsyncMock(return_value=http_responses[call_count % len(http_responses)])
            call_count += 1
...
def demonstrate_validation():
    ...
    invalid_metrics.protocols['http'] = ProtocolMetrics(protocol='http', success_rate=1.5)  # Invalid success rate
...
def demonstrate_aggregation():
    ...
    for i in range(3):
        ...
        metrics.protocols['http'] = ProtocolMetrics(
            protocol='http', 
            success_rate=0.8 - i * 0.1  # 0.8, 0.7, 0.6
        )
    ...
    print(f"  Max packet loss: {aggregated.network.packet_loss_rate:.2f} (expected: 0.2)")
    print(f"  Average HTTP success rate: {aggregated.protocols['http'].success_rate:.2f} (expected: 0.7)")
...
async def demonstrate_error_handling():
    ...
    with patch('asyncio.open_connection') as mock_open_connection:
        ...
        try:
            result = await collector.collect_comprehensive_metrics("unreachable.example.com", 80)
