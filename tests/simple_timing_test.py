...
def test_timing_attacks():
    ...
    try:
        config = JitterConfiguration(
            jitter_type=JitterType.UNIFORM, jitter_amplitude_ms=5.0, packets_per_burst=3
        )
    ...
    try:
        ...
        print(
            f"   Burst sequence: {[(size, f'{interval:.1f}ms') for size, interval in burst_sequence[:3]]}"
        )
