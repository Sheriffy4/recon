...
from recon.tests.training_data import TrainingDataGenerator, FeatureEngineer, TrainingExample
from recon.tests.model_trainer import ModelTrainer, ModelEvaluationMetrics
try:
    import sklearn
...
class TestTrainingDataGenerator(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        expected_types = {'ROSKOMNADZOR_TSPU', 'ROSKOMNADZOR_DPI', 'COMMERCIAL_DPI', 'FIREWALL_BASED', 'ISP_TRANSPARENT_PROXY', 'CLOUDFLARE_PROTECTION', 'GOVERNMENT_CENSORSHIP', 'UNKNOWN'}
    ...
    def test_metrics_completeness(self):
        ...
        required_metrics = ['rst_ttl', 'rst_latency_ms', 'connection_latency_ms', 'dns_resolution_time_ms', 'handshake_time_ms']
        for example in self.generator.training_examples:
            for metric in required_metrics:
                self.assertIn(metric, example.metrics, f'Missing {metric} in {example.dpi_type} example')
...
class TestFeatureEngineer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.generator = TrainingDataGenerator()
        self.sample_training_data = self.generator.get_training_data(include_synthetic=False)[:5]
...
class TestModelTrainer(unittest.TestCase):
    ...
    def test_train_model_with_evaluation(self):
        ...
        metrics = self.trainer.train_model_with_evaluation(training_data=training_data, test_size=0.3, cv_folds=3)
    ...
    def test_evaluation_metrics_serialization(self):
        ...
        metrics = ModelEvaluationMetrics(accuracy=0.85, precision_macro=0.83, precision_micro=0.85, recall_macro=0.82, recall_micro=0.85, f1_macro=0.82, f1_micro=0.85, f1_weighted=0.84, cross_val_scores=[0.8, 0.85, 0.9], cross_val_mean=0.85, cross_val_std=0.04, confusion_matrix=[[10, 2], [1, 12]], classification_report='test report', feature_importance={'feature1': 0.5, 'feature2': 0.3}, class_distribution={'class1': 15, 'class2': 10})
    ...
    def test_feature_importance_report(self):
        ...
        self.trainer.evaluation_metrics = ModelEvaluationMetrics(accuracy=0.85, precision_macro=0.83, precision_micro=0.85, recall_macro=0.82, recall_micro=0.85, f1_macro=0.82, f1_micro=0.85, f1_weighted=0.84, cross_val_scores=[0.8, 0.85, 0.9], cross_val_mean=0.85, cross_val_std=0.04, confusion_matrix=[[10, 2], [1, 12]], classification_report='test report', feature_importance={'feature1': 0.5, 'feature2': 0.3, 'feature3': 0.2, 'feature4': 0.1, 'feature5': 0.05}, class_distribution={'class1': 15, 'class2': 10})
    ...
    def test_training_report_generation(self):
        ...
        self.trainer.evaluation_metrics = ModelEvaluationMetrics(accuracy=0.85, precision_macro=0.83, precision_micro=0.85, recall_macro=0.82, recall_micro=0.85, f1_macro=0.82, f1_micro=0.85, f1_weighted=0.84, cross_val_scores=[0.8, 0.85, 0.9], cross_val_mean=0.85, cross_val_std=0.04, confusion_matrix=[[10, 2], [1, 12]], classification_report='Precision    Recall  F1-Score   Support\nClass1         0.83      0.85      0.84        15', feature_importance={'feature1': 0.5, 'feature2': 0.3}, class_distribution={'ROSKOMNADZOR_TSPU': 15, 'COMMERCIAL_DPI': 10})
...
class TestIntegration(unittest.TestCase):
    ...
    def test_end_to_end_training_pipeline(self):
        ...
        with patch.object(trainer.training_data_generator, 'generate_synthetic_variations') as mock_synthetic:
            mock_synthetic.return_value = trainer.training_data_generator.training_examples[:3]
    ...
    def test_training_data_quality_validation(self):
        ...
        for example in generator.training_examples:
            self.assertIsInstance(example.dpi_type, str)
            self.assertIn(example.dpi_type, ['UNKNOWN', 'ROSKOMNADZOR_TSPU', 'ROSKOMNADZOR_DPI', 'COMMERCIAL_DPI', 'FIREWALL_BASED', 'ISP_TRANSPARENT_PROXY', 'CLOUDFLARE_PROTECTION', 'GOVERNMENT_CENSORSHIP'])
            required_metrics = ['rst_ttl', 'rst_latency_ms', 'connection_latency_ms', 'dns_resolution_time_ms', 'handshake_time_ms']
            for metric in required_metrics:
    ...
    def test_feature_engineering_consistency(self):
        ...
        for key, value in features1.items():
            self.assertIsInstance(value, (int, float), f'Feature {key} is not numeric: {type(value)}')
