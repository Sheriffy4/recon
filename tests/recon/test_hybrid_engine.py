...
# Mock pydivert before it's imported by other modules
sys.modules['pydivert'] = Mock()
sys.modules['pydivert.packet'] = Mock()
sys.modules['pydivert.windivert_dll'] = Mock()
...
class TestHybridEngine:
    ...
    def hybrid_engine(self):
        ...
        # Disable fingerprinting and modern bypass for core tests
    ...
    async def test_connectivity_success(self, mock_get, hybrid_engine, test_data):
    ...
    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession.get')
    async def test_connectivity_failure_timeout(self, mock_get, hybrid_engine, test_data):
        """Test _test_sites_connectivity for a failed connection due to timeout."""
        # Configure the session mock to raise a timeout
    ...
    async def test_execute_strategy_real_world_success(self, mock_bypass_engine, hybrid_engine, test_data):
        ...
        mock_engine_instance.start.return_value = Mock() # mock the thread
        ...
        hybrid_engine._test_sites_connectivity = AsyncMock(return_value={
            site: ("WORKING", "1.1.1.1", 100.0, 200) for site in test_data["test_sites"]
        })
    ...
    async def test_test_strategies_hybrid_logic(self, hybrid_engine, test_data):
        ...
        # A list of strategies for testing
        strategies_to_test = [
            {"name": "fakedisorder", "params": {"split_pos": 3}},
            {"name": "multisplit", "params": {"positions": [1, 5]}}
        ]

        # Mock the underlying execution method to test sorting and result processing
        ...
        # Mock fingerprinting to be disabled for this test
        ...
        results = await hybrid_engine.test_strategies_hybrid(
            ...
            enable_fingerprinting=False  # Explicitly disable for this test
        )

        assert len(results) == 2
        # The results should be sorted by success_rate (descending)
        ...
        # Verify that execute_strategy_real_world_from_dict was called for each strategy
    ...
    async def test_execute_strategy_real_world_failure(self, mock_bypass_engine, hybrid_engine, test_data):
        ...
        # Mock connectivity test to return failure
        hybrid_engine._test_sites_connectivity = AsyncMock(return_value={
            site: ("TIMEOUT", "1.1.1.1", 5000.0, 0) for site in test_data["test_sites"]
        })
