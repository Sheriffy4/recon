...
from core.bypass.attacks.stateful_fragmentation import (
...
CLIENT_HELLO_PAYLOAD = (
    ...
    + b"example.com"
    + b"\x00\x0b\x00\x04\x03\x00\x01\x02\x00\x0a"
)
...
async def test_stateful_fragmentation_attack():
    ...
    context = AttackContext(
        dst_ip="1.1.1.1", dst_port=443, payload=CLIENT_HELLO_PAYLOAD
    )
...
async def test_client_hello_split_attack():
    ...
    record2_len = int.from_bytes(
        modified_payload[record2_start + 3 : record2_start + 5], "big"
    )
