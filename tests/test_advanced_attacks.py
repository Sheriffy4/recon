...
def test_advanced_attacks():
    ...
    print(
        f"Атаки с 'sni' или 'alpn': {[k for k in registry.attacks.keys() if 'sni' in k or 'alpn' in k]}"
    )
    ...
    # Проверяем TLS атаки
    tls_attacks = ["sni_manipulation", "alpn_manipulation", "grease_injection"]
    ...
    for attack_name, params in test_cases:
        ...
        try:
            ...
            if not attack_info:
                print("  ✗ Атака не найдена в реестре")
                continue
            ...
            context = AttackContext(
                dst_ip="127.0.0.1", dst_port=443, payload=test_payload, params=params
            )
            ...
            if hasattr(result, "status"):
                ...
                if result.status == AttackStatus.SUCCESS:
                    print("  ✓ Успешно выполнена")
                    if hasattr(result, "segments"):
                        print(f"  Сегментов: {len(result.segments)}")
                else:
                    print(f"  ✗ Ошибка: {result.error_message}")
            else:
                print("  ✓ Выполнена (старый формат)")
