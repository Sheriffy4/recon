...
from core.cli.dpi_parameter_parser import create_dpi_parameter_parser
...
class EndToEndValidator:
        ...
        for config in self.test_configurations:
            ...
            try:
                ...
                if result.success:
                    ...
                    self.logger.warning(f"Test {config.test_name} failed: {result.errors}")
    ...
    def run_single_test(self, config: TestConfiguration) -> TestResult:
        ...
        try:
            ...
            performance_metrics = {
                ...
                'strategy_applications': analysis_results.get('strategy_applications', 0),
    ...
    def _conduct_real_world_test(self, config: TestConfiguration, pcap_file: str) -> bool:
        """
    ...
    def _create_mock_packets(self, config: TestConfiguration) -> List:
        ...
        # Base packet
        base_packet = IP(src="192.168.1.100", dst="142.250.74.14") / TCP(sport=12345, dport=443) / Raw(load=client_hello_data)
        ...
        if not config.split_positions:
            ...
            if "3" in config.split_positions:
                # Split at position 3
                part1 = IP(src="192.168.1.100", dst="142.250.74.14") / TCP(sport=12345, dport=443, seq=1000) / Raw(load=client_hello_data[:3])
                part2 = IP(src="192.168.1.100", dst="142.250.74.14") / TCP(sport=12345, dport=443, seq=1003) / Raw(load=client_hello_data[3:])
                ...
                part1 = IP(src="192.168.1.100", dst="142.250.74.14") / TCP(sport=12345, dport=443, seq=1000) / Raw(load=client_hello_data[:10])
                part2 = IP(src="192.168.1.100", dst="142.250.74.14") / TCP(sport=12345, dport=443, seq=1010) / Raw(load=client_hello_data[10:])
                ...
                sni_position = 43
                part1 = IP(src="192.168.1.100", dst="142.250.74.14") / TCP(sport=12345, dport=443, seq=1000) / Raw(load=client_hello_data[:sni_position])
                part2 = IP(src="192.168.1.100", dst="142.250.74.14") / TCP(sport=12345, dport=443, seq=1000+sni_position) / Raw(load=client_hello_data[sni_position:])
    ...
    def _analyze_pcap_file(self, pcap_file: str, config: TestConfiguration) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            analysis['split_effectiveness'] = (
                analysis['split_packets_detected'] / max(analysis['tcp_packets'], 1)
            )
            analysis['badsum_effectiveness'] = (
                analysis['badsum_packets_detected'] / max(analysis['tcp_packets'], 1)
            )
    ...
    def _create_mock_analysis_results(self, config: TestConfiguration) -> Dict[str, Any]:
    ...
    def _analyze_strategy_effectiveness(self) -> Dict[str, Any]:
        ...
        for result in self.test_results:
            ...
            if "3" in config.split_positions:
                ...
                effectiveness['split_position_3']['avg_applications'] += result.analysis_results.get('position_3_splits', 0)
                
            if "10" in config.split_positions:
                effectiveness['split_position_10']['tests'] += 1
                if result.success:
                    effectiveness['split_position_10']['successes'] += 1
                effectiveness['split_position_10']['avg_applications'] += result.analysis_results.get('position_10_splits', 0)
                
            if "sni" in config.split_positions:
                effectiveness['split_position_sni']['tests'] += 1
                if result.success:
                    effectiveness['split_position_sni']['successes'] += 1
                effectiveness['split_position_sni']['avg_applications'] += result.analysis_results.get('sni_splits_detected', 0)
                
            if "badsum" in config.fooling_methods:
                effectiveness['badsum_fooling']['tests'] += 1
                if result.success:
                    effectiveness['badsum_fooling']['successes'] += 1
                effectiveness['badsum_fooling']['avg_applications'] += result.analysis_results.get('badsum_packets_detected', 0)
            ...
            if len(config.split_positions) > 1 or len(config.fooling_methods) > 0:
                ...
                effectiveness['combined_strategies']['avg_applications'] += result.analysis_results.get('strategy_applications', 0)
        
        # Calculate success rates
        for strategy in effectiveness.values():
            if strategy['tests'] > 0:
                strategy['success_rate'] = strategy['successes'] / strategy['tests']
                strategy['avg_applications'] = strategy['avg_applications'] / strategy['tests']
            else:
    ...
    def _analyze_performance(self) -> Dict[str, Any]:
        ...
        durations = [result.performance_metrics.get('test_duration', 0) for result in self.test_results]
        packets_processed = [result.performance_metrics.get('packets_processed', 0) for result in self.test_results]
        ...
        return {
            ...
            'average_packets_per_test': sum(packets_processed) / max(len(packets_processed), 1),
            'throughput_packets_per_second': sum(packets_processed) / max(sum(durations), 1) if durations else 0
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if len(successful_tests) < len(self.test_results) * 0.8:
            recommendations.append("Overall success rate is below 80%. Consider reviewing strategy implementation.")
            
        # Check for specific strategy issues
        split_3_tests = [r for r in self.test_results if "3" in r.config.split_positions]
        if split_3_tests and all(not r.success for r in split_3_tests):
            recommendations.append("Split position 3 strategy is failing consistently. Review implementation.")
            
        split_10_tests = [r for r in self.test_results if "10" in r.config.split_positions]
        if split_10_tests and all(not r.success for r in split_10_tests):
            recommendations.append("Split position 10 strategy is failing consistently. Review implementation.")
        ...
        if sni_tests and all(not r.success for r in sni_tests):
            recommendations.append("SNI split strategy is failing consistently. Review SNI detection logic.")
            
        badsum_tests = [r for r in self.test_results if "badsum" in r.config.fooling_methods]
        if badsum_tests and all(not r.success for r in badsum_tests):
            recommendations.append("Badsum fooling strategy is failing consistently. Review checksum manipulation.")
            
        # Performance recommendations
        avg_duration = sum(r.performance_metrics.get('test_duration', 0) for r in self.test_results) / max(len(self.test_results), 1)
        if avg_duration > 60:  # More than 1 minute per test
            recommendations.append("Test duration is high. Consider optimizing strategy application performance.")
            
        if not recommendations:
            recommendations.append("All strategies are performing well. Continue with current implementation.")
    ...
    def _identify_issues(self) -> List[str]:
        ...
        for result in self.test_results:
            if result.errors:
                issues.extend([f"{result.test_name}: {error}" for error in result.errors])
        ...
        if len(failed_tests) > len(self.test_results) * 0.5:
            issues.append("More than 50% of tests failed - major implementation issues likely")
