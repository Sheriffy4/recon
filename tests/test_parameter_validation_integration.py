...
class TestParameterValidationIntegration(unittest.TestCase):
    ...
    def test_attack_registry_validation(self):
        ...
        self.assertTrue(
            result.is_valid, f"Valid parameters should pass: {result.error_message}"
        )
        ...
        self.assertFalse(
            result.is_valid, "Missing required parameter should fail validation"
        )
        ...
        self.assertFalse(
            result.is_valid, "Invalid fooling method should fail validation"
        )
    ...
    def test_cli_validation_integration(self):
        ...
        validated = self.searcher._validate_attack_parameters(
            "fakeddisorder", valid_genes
        )
    ...
    def test_registry_enhancement_in_loader(self):
        ...
        for attack_type in registry_attacks:
            ...
            if metadata and metadata.required_params:
                self.assertIn(
                    attack_type,
                    self.loader.required_params,
                    f"Attack type {attack_type} should have required_params",
                )
                self.assertEqual(
                    self.loader.required_params[attack_type],
    ...
    def test_parameter_normalization_with_registry(self):
        ...
        normalized = self.loader._normalize_params_with_registry(
            "fakeddisorder", minimal_params
        )
    ...
    def test_fallback_validation(self):
        ...
        with patch(
            "core.bypass.attacks.attack_registry.get_attack_registry"
        ) as mock_registry:
    ...
    def test_special_parameter_values(self):
        ...
        result = self.registry.validate_parameters("fakeddisorder", invalid_params)
        self.assertFalse(
            result.is_valid, "Invalid special split_pos should fail validation"
        )
    ...
    def test_positions_parameter_validation(self):
        ...
        self.assertFalse(
            result.is_valid, "Positions with invalid values should fail validation"
        )
...
def run_validation_tests():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(
        TestParameterValidationIntegration
    )
