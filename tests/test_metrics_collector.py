...
from recon.tests.metrics_collector import MetricsCollector, TimingMetricsCollector, NetworkMetricsCollector, ProtocolMetricsCollector, BaseMetricsCollector, TimingMetrics, NetworkMetrics, ProtocolMetrics, ComprehensiveMetrics
from recon.tests.advanced_models import MetricsCollectionError
...
class TestTimingMetrics(unittest.TestCase):
    ...
    def test_timing_metrics_with_values(self):
        ...
        metrics = TimingMetrics(latency_ms=15.5, jitter_ms=2.3, packet_timing=packet_timing, connection_time_ms=8.2, first_byte_time_ms=12.1, total_time_ms=25.7, timeout_occurred=True, retransmission_count=2)
...
class TestBaseMetricsCollector(unittest.TestCase):
    ...
    def test_validate_metrics_valid(self):
        ...
        metrics = {'latency_ms': 15.5, 'connection_time_ms': 8.2, 'total_time_ms': 25.7, 'success_rate': 0.8}
...
class TestTimingMetricsCollector(unittest.TestCase):
    ...
    async def test_collect_metrics_with_timeouts(self, mock_open_connection):
        ...
        mock_open_connection.side_effect = [asyncio.TimeoutError(), (AsyncMock(), AsyncMock()), (AsyncMock(), AsyncMock())]
    ...
    def test_get_timing_trends_with_data(self):
        ...
        for i in range(5):
            self.collector.timing_history.append({'timestamp': time.time() + i, 'latency_ms': 10.0 + i, 'jitter_ms': 1.0 + i * 0.1})
        trends = self.collector.get_timing_trends()
...
class TestProtocolMetricsCollector(unittest.TestCase):
    ...
    async def test_collect_metrics_auto_protocol(self, mock_open_connection):
        ...
        mock_open_connection.return_value = (mock_reader, mock_writer)
        result = await self.collector.collect_metrics('test.example.com', 80, protocol='auto')
        self.assertEqual(result['protocol'], 'http')
...
class TestMetricsCollector(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertIsInstance(self.collector.protocol_collector, ProtocolMetricsCollector)
    ...
    async def test_collect_comprehensive_metrics_success(self, mock_protocol, mock_network, mock_timing):
        """Test successful comprehensive metrics collection"""
        mock_timing.return_value = {'latency_ms': 15.5, 'jitter_ms': 2.3, 'packet_timing': [10, 12, 14], 'connection_time_ms': 8.2, 'first_byte_time_ms': 12.1, 'total_time_ms': 25.7, 'timeout_occurred': False, 'retransmission_count': 0}
        mock_network.return_value = {'packet_loss_rate': 0.1, 'out_of_order_packets': 0, 'duplicate_packets': 0, 'fragmented_packets': 0, 'mtu_discovery_blocked': False, 'icmp_responses': [], 'tcp_window_scaling': True, 'tcp_options': ['mss', 'wscale']}
        mock_protocol.return_value = {'protocol': 'https', 'success_rate': 0.8, 'error_codes': [200], 'response_sizes': [1234], 'header_modifications': {}, 'content_modifications': False, 'redirect_responses': 0, 'blocked_responses': 0}
        result = await self.collector.collect_comprehensive_metrics('test.example.com', 443)
    ...
    @patch.object(NetworkMetricsCollector, 'collect_metrics')
    @patch.object(ProtocolMetricsCollector, 'collect_metrics')
    async def test_collect_comprehensive_metrics_with_errors(self, mock_protocol, mock_network, mock_timing):
        """Test comprehensive metrics collection with some collector failures"""
        mock_timing.return_value = {'latency_ms': 15.5, 'jitter_ms': 2.3, 'packet_timing': []}
    ...
    async def test_collect_comprehensive_metrics_selective_collection(self):
        ...
        with patch.object(self.collector.timing_collector, 'collect_metrics') as mock_timing:
            mock_timing.return_value = {'latency_ms': 15.5}
            result = await self.collector.collect_comprehensive_metrics('test.example.com', 443, include_timing=True, include_network=False, include_protocol=False)
            self.assertIsInstance(result, ComprehensiveMetrics)
    ...
    def test_validate_comprehensive_metrics_invalid(self):
        ...
        metrics = ComprehensiveMetrics(target='')
        metrics.timing.latency_ms = -5.0
        metrics.network.packet_loss_rate = 1.5
        metrics.protocols['http'] = ProtocolMetrics(protocol='http', success_rate=1.5)
        ...
        self.assertTrue(any(('Packet loss rate must be between 0.0 and 1.0' in error for error in errors)))
        self.assertTrue(any(('Success rate for http must be between 0.0 and 1.0' in error for error in errors)))
        self.assertTrue(any(('Reliability score must be between 0.0 and 1.0' in error for error in errors)))
    ...
    def test_aggregate_metrics_multiple_items(self):
        ...
        self.assertEqual(result.target, 'test.example.com')
        self.assertEqual(result.timing.latency_ms, 15.0)
        self.assertEqual(result.network.packet_loss_rate, 0.2)
        self.assertEqual(result.protocols['http'].success_rate, 0.7)
...
class TestMetricsCollectorIntegration(unittest.TestCase):
    ...
    async def test_end_to_end_collection_mock(self):
        ...
        with patch('asyncio.open_connection') as mock_open_connection:
            ...
            mock_reader.read = AsyncMock(return_value=b'HTTP/1.1 200 OK\r\n\r\n')
            mock_open_connection.return_value = (mock_reader, mock_writer)
            result = await self.collector.collect_comprehensive_metrics('httpbin.org', 80)
            self.assertIsInstance(result, ComprehensiveMetrics)
...
if __name__ == '__main__':
    ...
    test_classes = [TestTimingMetrics, TestNetworkMetrics, TestProtocolMetrics, TestComprehensiveMetrics, TestBaseMetricsCollector, TestTimingMetricsCollector, TestNetworkMetricsCollector, TestProtocolMetricsCollector, TestMetricsCollector, TestMetricsCollectorIntegration]
