...
from recon.tests.config_models import PoolConfiguration, BypassStrategy, StrategyPool, ConfigurationVersion
from recon.tests.config_migrator import ConfigurationMigrator
from recon.tests.config_validator import ConfigurationValidator
from recon.tests.config_manager import ConfigurationManager
from recon.tests.backup_manager import BackupManager
...
class TestConfigurationMigrator:
    ...
    def test_migrate_legacy_to_pool_success(self):
        ...
        legacy_config = {'strategy': '--dpi-desync=multisplit --dpi-desync-split-count=5 --dpi-desync-split-pos=4', 'result_status': 'PARTIAL_SUCCESS', 'successful_sites': 3, 'total_sites': 5, 'success_rate': 0.6, 'avg_latency_ms': 250.5, 'fingerprint_used': True, 'dpi_type': 'roskomnadzor', 'dpi_confidence': 0.8}
        legacy_path = Path(self.temp_dir) / 'legacy.json'
        with open(legacy_path, 'w') as f:
            json.dump(legacy_config, f)
        target_path = Path(self.temp_dir) / 'pool_config.json'
        result = self.migrator.migrate_legacy_to_pool(str(legacy_path), str(target_path))
    ...
    def test_parse_zapret_strategy(self):
        ...
        assert 'tcp_multisplit' in strategy.attacks or 'tcp_fragmentation' in strategy.attacks
    ...
    def test_validation_after_migration(self):
        ...
        legacy_config = {'strategy': '--dpi-desync=multisplit --dpi-desync-split-count=3', 'result_status': 'SUCCESS', 'success_rate': 0.9}
        legacy_path = Path(self.temp_dir) / 'legacy.json'
        with open(legacy_path, 'w') as f:
            json.dump(legacy_config, f)
        ...
        result = self.migrator.migrate_legacy_to_pool(str(legacy_path), str(target_path))
        assert result.success
        validation_issues = self.migrator.validate_migration(str(legacy_path), str(target_path))
...
class TestConfigurationValidator:
    ...
    def test_validate_valid_configuration(self):
        ...
        strategy = BypassStrategy(id='test_strategy', name='Test Strategy', attacks=['tcp_fragmentation', 'http_host_case'], parameters={'split_pos': 2}, target_ports=[443])
        pool = StrategyPool(id='test_pool', name='Test Pool', description='Test pool', strategy=strategy, domains=['example.com'], priority=1)
        config = PoolConfiguration(version=ConfigurationVersion.POOL_V1, pools=[pool], default_pool='test_pool')
    ...
    def test_validate_invalid_attacks(self):
        ...
        strategy = BypassStrategy(id='test_strategy', name='Test Strategy', attacks=['invalid_attack', 'another_invalid'], target_ports=[443])
        pool = StrategyPool(id='test_pool', name='Test Pool', description='Test pool', strategy=strategy, domains=['example.com'])
    ...
    def test_validate_duplicate_pool_ids(self):
        ...
        pool1 = StrategyPool(id='duplicate_id', name='Pool 1', description='First pool', strategy=strategy, domains=['example.com'])
        pool2 = StrategyPool(id='duplicate_id', name='Pool 2', description='Second pool', strategy=strategy, domains=['test.com'])
        config = PoolConfiguration(version=ConfigurationVersion.POOL_V1, pools=[pool1, pool2])
        errors = self.validator.validate_configuration(config)
        error_count = len([e for e in errors if e.level == 'error' and 'Duplicate pool ID' in e.message])
        assert error_count >= 1

    def test_validate_invalid_default_pool(self):
        """Test validation catches invalid default pool reference."""
        strategy = BypassStrategy(id='test_strategy', name='Test Strategy', attacks=['tcp_fragmentation'], target_ports=[443])
        pool = StrategyPool(id='existing_pool', name='Existing Pool', description='Pool that exists', strategy=strategy, domains=['example.com'])
        config = PoolConfiguration(version=ConfigurationVersion.POOL_V1, pools=[pool], default_pool='nonexistent_pool')
        errors = self.validator.validate_configuration(config)
        error_count = len([e for e in errors if e.level == 'error' and 'Default pool' in e.message])
        assert error_count >= 1

    def test_validate_invalid_ports(self):
        """Test validation catches invalid port numbers."""
        strategy = BypassStrategy(id='test_strategy', name='Test Strategy', attacks=['tcp_fragmentation'], target_ports=[0, 70000, -1])
        pool = StrategyPool(id='test_pool', name='Test Pool', description='Test pool', strategy=strategy, domains=['example.com'])
        config = PoolConfiguration(version=ConfigurationVersion.POOL_V1, pools=[pool])
        errors = self.validator.validate_configuration(config)
...
class TestConfigurationManager:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.temp_dir = tempfile.mkdtemp()
        self.config_manager = ConfigurationManager(config_dir=str(Path(self.temp_dir) / 'config'), backup_dir=str(Path(self.temp_dir) / 'backups'))
    ...
    def test_migrate_legacy_configuration(self):
        ...
        with open(legacy_path, 'w') as f:
            json.dump(legacy_config, f)
        target_path = Path(self.temp_dir) / 'migrated.json'
        result = self.config_manager.migrate_legacy_configuration(str(legacy_path), str(target_path))
        assert result.success
        assert target_path.exists()
    ...
    def test_add_and_remove_pool(self):
        ...
        config = self.config_manager._create_default_configuration()
        strategy = BypassStrategy(id='new_strategy', name='New Strategy', attacks=['tcp_fragmentation'], target_ports=[443])
        new_pool = self.config_manager.create_pool('new_pool', 'New Pool', strategy, 'Test pool for adding/removing')
    ...
    def test_validation_integration(self):
        ...
        invalid_config = {'version': 'pool_v1', 'pools': [{'id': '', 'name': 'Invalid Pool', 'strategy': {'id': 'invalid_strategy', 'name': 'Invalid Strategy', 'attacks': ['nonexistent_attack'], 'target_ports': [70000]}}]}
...
def test_integration_workflow():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        legacy_config = {'strategy': '--dpi-desync=multisplit --dpi-desync-split-count=5 --dpi-desync-fooling=badsum', 'result_status': 'PARTIAL_SUCCESS', 'successful_sites': 3, 'total_sites': 5, 'success_rate': 0.6}
        ...
        error_count = len([e for e in errors if e.level == 'error'])
        assert error_count == 0
        new_strategy = BypassStrategy(id='custom_strategy', name='Custom Strategy', attacks=['http_host_case', 'tls_sni_modification'], target_ports=[443])
        new_pool = manager.create_pool('custom_pool', 'Custom Pool', new_strategy, 'Custom pool for specific domains', ['custom.example.com'])
