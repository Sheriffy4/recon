...
from core.bypass.strategies.exceptions import DPIStrategyError, SNINotFoundError
...
class TestSNIDetector:
    ...
    def test_is_client_hello_valid_tls12(self):
        ...
        packet = struct.pack('!BHHBH', 0x16, 0x0303, 0x0020, 0x01, 0x001C)
        packet += b'\x00' * 28  # Padding
    ...
    def test_is_client_hello_valid_tls13(self):
        ...
        packet = struct.pack('!BHHBH', 0x16, 0x0304, 0x0020, 0x01, 0x001C)
        packet += b'\x00' * 28  # Padding
    ...
    def test_parse_tls_extensions_multiple_extensions(self):
        ...
        assert isinstance(result, dict)
        assert len(result) > 1  # Should have multiple extensions
        assert 0x0000 in result  # SNI extension
    ...
    def test_is_valid_hostname_valid(self):
        ...
        for hostname in valid_hostnames:
            assert self.detector._is_valid_hostname(hostname) is True, f"Failed for {hostname}"
    ...
    def test_is_valid_hostname_invalid(self):
        ...
        for hostname in invalid_hostnames:
            assert self.detector._is_valid_hostname(hostname) is False, f"Should fail for {hostname}"
...
class TestSNIDetectorIntegration:
    ...
    def _create_large_client_hello(self) -> bytes:
        ...
        detector = SNIDetector()
        base_packet = bytearray(detector._create_client_hello_with_sni("large.example.com"))
