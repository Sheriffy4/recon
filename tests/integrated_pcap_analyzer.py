...
class IntegratedPCAPAnalyzer:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        # Create file handler
        log_file = (
            self.output_dir / f"analyzer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    def analyze_pcap_comprehensive(
        self, pcap_file: str, expected_strategies: List[str]
    ...
    def _perform_basic_analysis(
        self, pcap_file: str, result: IntegratedAnalysisResult
    ) -> None:
        """Perform basic packet counting and statistics."""
        if not SCAPY_AVAILABLE:
            self.logger.warning(
                "Scapy not available - using mock data for basic analysis"
            )
        ...
        try:
            ...
            self.logger.info(
                f"Basic analysis: {result.total_packets} total, {result.tcp_packets} TCP, {result.tls_packets} TLS"
            )
    ...
    def _perform_strategy_validation(
        self,
        pcap_file: str,
        ...
        try:
            ...
            validation_result = self.strategy_validator.validate_pcap_file(
            ...
            self.logger.info(
                f"Strategy validation completed: {len(result.strategy_validations)} strategies validated"
            )
                    ...
                    timing_analysis["burst_patterns"].append(
                        {
                            ...
                            "duration": sum(
                                timing_analysis["inter_packet_intervals"][
                                    current_burst[0] : current_burst[-1]
                                ]
                            ),
                        }
                    )
    ...
    def _calculate_effectiveness_and_recommendations(
        ...
        if result.strategy_validations:
            passed_validations = sum(
                1 for v in result.strategy_validations if v.validation_passed
            )
            ...
            avg_confidence = (
                sum(v.confidence_score for v in result.strategy_validations)
                / total_validations
            )

            # Combine scores
            result.effectiveness_score = (validation_score * 0.7) + (
                avg_confidence * 0.3
            )
        else:
    ...
    def _generate_recommendations(self, result: IntegratedAnalysisResult) -> None:
        ...
        failed_strategies = [
            v for v in result.strategy_validations if not v.validation_passed
        ]
        successful_strategies = [
            v for v in result.strategy_validations if v.validation_passed
        ]
        ...
        if result.effectiveness_score >= 0.8:
            ...
            recommendations.append(
                "⚠️ DPI strategies are partially effective - some improvements needed"
            )
        else:
            recommendations.append(
        ...
        for strategy in failed_strategies:
            if strategy.strategy_name == "split_3":
                recommendations.append(
                    "🔧 Split position 3 not detected - verify packet splitting logic"
                )
                ...
                recommendations.append(
                    "🔧 Badsum not detected - verify checksum manipulation"
                )
        ...
        if result.tcp_packets == 0:
            ...
            recommendations.append(
                "⚠️ No TLS traffic detected - verify HTTPS connections"
            )

        # Performance recommendations
        if result.total_packets > 1000:
            recommendations.append(
                "📊 Large packet capture - consider filtering for better analysis"
            )
    ...
    def _generate_human_readable_report(self, result: IntegratedAnalysisResult) -> str:
        ...
        for validation in result.strategy_validations:
            ...
            lines.append(
                f"{validation.strategy_name}: {status} (confidence: {validation.confidence_score:.2f})"
            )
        ...
        if result.youtube_analysis:
            ...
            if isinstance(result.youtube_analysis, dict):
                lines.append(
                    f"DNS Queries: {result.youtube_analysis.get('dns_queries', 'N/A')}"
                )
                lines.append(
                    f"RST Packets: {result.youtube_analysis.get('rst_packets', 'N/A')}"
                )
                lines.append(
                    f"HTTPS Connections: {result.youtube_analysis.get('https_connections', 'N/A')}"
                )
            lines.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Integrated PCAP analysis for DPI strategy validation"
    )
    parser.add_argument("pcap_file", help="PCAP file to analyze")
    parser.add_argument(
        "--strategies",
        nargs="+",
        default=["split_3", "split_10", "split_sni", "badsum"],
        help="Expected strategies to validate",
    )
    parser.add_argument(
        "--output-dir", default="pcap_analysis_results", help="Output directory"
    )
    parser.add_argument(
    ...
    print("🔍 Starting integrated PCAP analysis")
