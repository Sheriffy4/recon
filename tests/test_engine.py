...
class TestHybridEngine:
    ...
    def hybrid_engine(self):
        ...
        return HybridEngine(debug=True, enable_advanced_fingerprinting=False, enable_modern_bypass=False)

    @pytest.fixture
    def test_data(self):
        """Common test data"""
        return {'domain': 'test-domain.com', 'port': 443, 'test_sites': ['https://test-domain.com', 'https://another-site.com'], 'ips': {'1.1.1.1', '2.2.2.2'}, 'dns_cache': {'test-domain.com': '1.1.1.1', 'another-site.com': '2.2.2.2'}, 'strategy_str': '--dpi-desync=fake --dpi-desync-ttl=2', 'strategy_dict': {'name': 'fakedisorder', 'params': {'split_pos': 3}}}
    ...
    async def test_connectivity_success(self, mock_session, hybrid_engine, test_data):
        ...
        results = await hybrid_engine._test_sites_connectivity(sites=test_data['test_sites'], dns_cache=test_data['dns_cache'])
    ...
    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession')
    async def test_connectivity_failure_timeout(self, mock_session, hybrid_engine, test_data):
    ...
    async def test_execute_strategy_real_world_success(self, mock_bypass_engine, hybrid_engine, test_data):
        ...
        hybrid_engine._test_sites_connectivity = AsyncMock(return_value={site: ('WORKING', '1.1.1.1', 100.0, 200) for site in test_data['test_sites']})
        result_status, successful_count, total_count, avg_latency = await hybrid_engine.execute_strategy_real_world(strategy_str=test_data['strategy_str'], test_sites=test_data['test_sites'], target_ips=test_data['ips'], dns_cache=test_data['dns_cache'])
    ...
    async def test_test_strategies_hybrid_logic(self, hybrid_engine, test_data):
        ...
        strategies_to_test = [{'name': 'fakedisorder', 'params': {'split_pos': 3}}, {'name': 'multisplit', 'params': {'positions': [1, 5]}}]
        ...
        hybrid_engine.execute_strategy_real_world_from_dict = AsyncMock(side_effect=mock_execute_strategy)
        hybrid_engine.fingerprint_target = AsyncMock(return_value=None)
        results = await hybrid_engine.test_strategies_hybrid(strategies=strategies_to_test, test_sites=test_data['test_sites'], ips=test_data['ips'], dns_cache=test_data['dns_cache'], port=test_data['port'], domain=test_data['domain'], enable_fingerprinting=False)
    ...
    @pytest.mark.asyncio
    @patch('core.hybrid_engine.BypassEngine')
    async def test_execute_strategy_real_world_failure(self, mock_bypass_engine, hybrid_engine, test_data):
        """Test execute_strategy_real_world with failed connectivity."""
        mock_engine_instance = Mock()
        mock_engine_instance.start.return_value = Mock()
        mock_bypass_engine.return_value = mock_engine_instance
        hybrid_engine._test_sites_connectivity = AsyncMock(return_value={site: ('TIMEOUT', '1.1.1.1', 5000.0, 0) for site in test_data['test_sites']})
        result_status, successful_count, total_count, avg_latency = await hybrid_engine.execute_strategy_real_world(strategy_str=test_data['strategy_str'], test_sites=test_data['test_sites'], target_ips=test_data['ips'], dns_cache=test_data['dns_cache'])
