...
try:
    from attack_catalog import get_catalog, AttackCategory, AttackComplexity, ExternalTool
    ...
    def basic_validation():
        ...
        if catalog_file.exists():
            ...
            checks = [
                ("ComprehensiveAttackCatalog class", "class ComprehensiveAttackCatalog" in content),
                ("TCP fragmentation attacks", "_register_tcp_fragmentation_attacks" in content),
                ("HTTP manipulation attacks", "_register_http_manipulation_attacks" in content),
...
def test_compatibility_matrix():
    ...
    if mapping:
        print(f"Zapret mapping for fake_disorder:")
    ...
    command = matrix.convert_to_tool_command("fake_disorder", ExternalTool.ZAPRET, parameters)
    if command:
...
def test_attack_metadata():
    ...
    if metadata:
        print(f"Metadata for tlsrec_split:")
    ...
    for attack_id, attack in catalog.attacks.items():
        ...
        if metadata and metadata.effectiveness_score >= 0.8:
            high_effectiveness_attacks.append((attack.name, metadata.effectiveness_score))
    
    print(f"\nHigh effectiveness attacks (>= 0.8):")
    for name, score in sorted(high_effectiveness_attacks, key=lambda x: x[1], reverse=True)[:10]:
        print(f"  - {name}: {score}")
...
def test_export_functionality():
    ...
    if catalog.export_catalog(catalog_file):
        ...
        print(f"❌ Failed to export catalog")
        return
    ...
    if matrix.export_compatibility_matrix(matrix_file):
        ...
        try:
            ...
            print(f"  Exported mappings for {data['metadata']['total_attacks']} attacks")
        ...
        print(f"❌ Failed to export compatibility matrix")
        return
...
def test_real_world_scenarios():
    ...
    for attack_id, attack in catalog.attacks.items():
        metadata = catalog.get_metadata_by_id(attack_id)
        if (metadata and 
            metadata.effectiveness_score >= 0.8 and 
            metadata.stability_score >= 0.7 and
            attack.complexity in [AttackComplexity.ADVANCED, AttackComplexity.EXPERT]):
    ...
    for name, score in sorted(sophisticated_attacks, key=lambda x: x[1], reverse=True)[:5]:
        print(f"    - {name}: {score}")
    ...
    for attack_id, attack in catalog.attacks.items():
        metadata = catalog.get_metadata_by_id(attack_id)
        if (metadata and 
            metadata.resource_usage == "low" and
            metadata.stability_score >= 0.8 and
            attack.complexity in [AttackComplexity.SIMPLE, AttackComplexity.MODERATE]):
    ...
    for name, score in sorted(lightweight_attacks, key=lambda x: x[1], reverse=True)[:5]:
        print(f"    - {name}: {score}")
