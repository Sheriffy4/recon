...
class TestDPIStrategyEngine:
    ...
    def test_should_apply_disabled(self):
        ...
        config = DPIConfig(
            enabled=False, desync_mode="split", split_positions=[3], fooling_methods=[]
        )
    ...
    def test_should_split_packet_no_positions(self):
        ...
        config = DPIConfig(
            enabled=True, desync_mode="split", split_positions=[], fooling_methods=[]
        )
        engine = DPIStrategyEngine(config)
        packet = b"A" * 100
    ...
    def test_should_split_packet_tls_client_hello(self):
        ...
        with patch.object(
            self.engine._sni_detector, "is_client_hello", return_value=True
        ):
    ...
    def test_get_split_positions_success(self):
        ...
        with patch.object(
            self.engine._position_resolver,
            "resolve_positions",
    ...
    def test_apply_priority_handling_sni_first(self):
        ...
        with patch.object(
            ...
            with patch.object(
                self.engine._sni_detector, "find_sni_position", return_value=80
            ):
    ...
    def test_apply_strategy_success(self):
        ...
        with patch.object(self.engine, "should_split_packet", return_value=True):
            with patch.object(self.engine, "get_split_positions", return_value=[50]):
                with patch.object(
                    self.engine._packet_modifier,
                    "split_packet",
                    return_value=expected_parts,
                ):
                    with patch.object(
                        self.engine,
                        "_apply_fooling_strategies",
                        return_value=expected_parts,
                    ):
    ...
    def test_apply_fooling_strategies_success(self):
        ...
        mock_fooler = Mock()
        mock_fooler.should_apply_badsum.side_effect = [
            True,
            False,
            False,
        ]  # Only first packet
    ...
    def test_log_strategy_application(self):
        ...
        # Should not raise any exception
        self.engine.log_strategy_application(
            packet, result, split_positions, processing_time
        )
        ...
        with patch.object(self.engine, "should_split_packet", return_value=True):
            with patch.object(self.engine, "get_split_positions", return_value=[80]):
                with patch.object(
                    self.engine._packet_modifier,
                    "split_packet",
                    return_value=[b"part1", b"part2"],
                ):
                    with patch.object(
                        self.engine, "_get_sni_position_from_splits", return_value=80
                    ):
                        with patch.object(
                            self.engine,
                            "_apply_fooling_strategies",
        ...
        with patch.object(self.engine, "should_split_packet", return_value=True):
            with patch.object(self.engine, "get_split_positions", return_value=[50]):
                with patch.object(
                    ...
                    with patch.object(
                        self.engine, "_get_sni_position_from_splits", return_value=None
                    ):
                        with patch.object(
                            self.engine,
                            "_apply_fooling_strategies",
