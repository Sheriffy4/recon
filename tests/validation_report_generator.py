...
class ValidationReportGenerator:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        # Create file handler
        log_file = self.output_dir / f"report_gen_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def generate_comprehensive_report(self, 
                                    ...
                                    additional_data: Optional[Dict[str, Any]] = None) -> ComprehensiveValidationReport:
        """
    ...
    def _process_end_to_end_results(self, results: Dict[str, Any], report: ComprehensiveValidationReport) -> None:
        """Process end-to-end test results."""
        ...
        if 'validation_summary' in results:
            ...
            report.validation_summary.successful_tests = summary.get('successful_tests', 0)
            ...
            report.validation_summary.total_packets_analyzed = summary.get('total_packets_analyzed', 0)
    ...
    def _process_pcap_analysis_results(self, results: List[Dict[str, Any]], report: ComprehensiveValidationReport) -> None:
        """Process PCAP analysis results."""
        ...
        for analysis in results:
            ...
            if 'total_packets' in analysis:
                report.validation_summary.total_packets_analyzed += analysis['total_packets']
        ...
        if effectiveness_count > 0:
            report.validation_summary.average_effectiveness = total_effectiveness / effectiveness_count
            
    def _process_additional_data(self, data: Dict[str, Any], report: ComprehensiveValidationReport) -> None:
    ...
    def _calculate_summary_statistics(self, report: ComprehensiveValidationReport) -> None:
        ...
        for test_result in report.test_results:
            if isinstance(test_result, dict) and 'config' in test_result:
                ...
                strategies = config.get('split_positions', []) + config.get('fooling_methods', [])
        ...
        report.validation_summary.strategies_working = len([s for s in strategy_counts if strategy_successes.get(s, 0) > 0])
        report.validation_summary.strategies_failing = report.validation_summary.total_strategies_tested - report.validation_summary.strategies_working
        
        # Count total issues
        report.validation_summary.total_issues_found = len(set(report.issues_and_limitations))
        
    def _analyze_strategy_performance(self, report: ComprehensiveValidationReport) -> None:
            ...
            if isinstance(test_result, dict) and 'config' in test_result:
                ...
                for strategy in strategies:
                    ...
                    if f'{strategy}_applications' in analysis:
                        strategy_data[strategy]['applications'] += analysis[f'{strategy}_applications']
                ...
                for validation in pcap_result['strategy_validations']:
                    ...
                    if strategy_name in strategy_data:
                        if validation.get('issues'):
                            strategy_data[strategy_name]['issues'].extend(validation['issues'])
    ...
    def _generate_recommendations_and_next_steps(self, report: ComprehensiveValidationReport) -> None:
        ...
        if report.validation_summary.success_rate >= 0.9:
            report.recommendations.append("✅ Excellent performance - DPI strategies are highly effective")
            ...
            report.recommendations.append("⚠️ Good performance with room for improvement")
            report.next_steps.extend([
            ...
            report.recommendations.append("❌ Poor performance - significant issues need addressing")
            report.next_steps.extend([
        ...
        for strategy_name, performance in report.strategy_performance.items():
            if performance.success_rate < 0.5:
                report.recommendations.append(f"🔧 {strategy_name} strategy needs significant improvement (success rate: {performance.success_rate:.1%})")
                report.next_steps.append(f"Debug and fix {strategy_name} implementation")
                
        # Performance recommendations
        if report.validation_summary.total_packets_analyzed > 10000:
            report.recommendations.append("📊 Large-scale testing completed - results are statistically significant")
        elif report.validation_summary.total_packets_analyzed < 100:
            report.recommendations.append("📊 Limited packet analysis - consider more extensive testing")
            report.next_steps.append("Conduct longer capture sessions for more comprehensive analysis")
            
        # Issue-based recommendations
        if report.validation_summary.total_issues_found > 10:
            report.recommendations.append("⚠️ Multiple issues detected - prioritize fixing critical problems")
    ...
    def _save_report(self, report: ComprehensiveValidationReport) -> None:
        ...
        report_dict = {
            ...
            'strategy_performance': {k: asdict(v) for k, v in report.strategy_performance.items()},
    ...
    def _generate_text_report(self, report: ComprehensiveValidationReport) -> str:
        ...
        for strategy_name, performance in report.strategy_performance.items():
            status = "✅" if performance.success_rate >= 0.8 else "⚠️" if performance.success_rate >= 0.5 else "❌"
            lines.append(f"{status} {strategy_name.upper()}")
            lines.append(f"  Success Rate: {performance.success_rate:.1%} ({performance.successful_tests}/{performance.tests_conducted})")
            ...
            if performance.issues_found:
                ...
                if len(performance.issues_found) > 3:
                    lines.append(f"    ... and {len(performance.issues_found) - 3} more")
            lines.append("")
        ...
        if report.pcap_analysis_results:
            total_packets = sum(r.get('total_packets', 0) for r in report.pcap_analysis_results)
            total_tcp = sum(r.get('tcp_packets', 0) for r in report.pcap_analysis_results)
            total_tls = sum(r.get('tls_packets', 0) for r in report.pcap_analysis_results)
            ...
            effectiveness_scores = [r.get('effectiveness_score', 0) for r in report.pcap_analysis_results if 'effectiveness_score' in r]
    ...
    def _generate_executive_summary(self, report: ComprehensiveValidationReport) -> str:
        ...
        lines.append("=" * 80)
        lines.append(f"Report ID: {report.report_id}")
        lines.append(f"Date: {datetime.fromisoformat(report.generation_timestamp).strftime('%B %d, %Y')}")
        lines.append("")
        ...
        if summary.success_rate >= 0.9:
            ...
            lines.append("🟡 GOOD: DPI strategies are mostly effective with minor issues")
        else:
            lines.append("🔴 POOR: DPI strategies have significant effectiveness issues")
            
        lines.append(f"• Overall Success Rate: {summary.success_rate:.1%}")
        lines.append(f"• Strategies Working: {summary.strategies_working}/{summary.total_strategies_tested}")
        ...
        for strategy_name, performance in report.strategy_performance.items():
            ...
            lines.append(f"• {strategy_name}: {status} ({performance.success_rate:.1%})")
        lines.append("")
        
        # Critical Issues
        critical_issues = [issue for issue in report.issues_and_limitations if any(word in issue.lower() for word in ['critical', 'failed', 'error', 'not working'])]
        if critical_issues:
        ...
        if summary.success_rate >= 0.9:
            lines.append("The DPI strategy implementation is highly effective and ready for")
            lines.append("production deployment. Continue monitoring and periodic validation.")
        elif summary.success_rate >= 0.7:
            lines.append("The DPI strategy implementation shows good results but requires")
            lines.append("addressing identified issues before full production deployment.")
        else:
            lines.append("The DPI strategy implementation has significant issues that must")
            lines.append("be resolved before deployment. Recommend thorough review and fixes.")
...
def main():
    ...
    parser.add_argument("--end-to-end-results", help="JSON file with end-to-end test results")
    parser.add_argument("--pcap-analysis-results", nargs="+", help="JSON files with PCAP analysis results")
    parser.add_argument("--output-dir", default="validation_reports", help="Output directory")
    ...
    if args.pcap_analysis_results:
        for file_path in args.pcap_analysis_results:
            try:
                ...
                print(f"Warning: Could not load PCAP analysis results from {file_path}: {e}")
    ...
    try:
        ...
        report = generator.generate_comprehensive_report(
            end_to_end_results=end_to_end_data,
            pcap_analysis_results=pcap_data
        )
