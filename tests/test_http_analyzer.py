...
class TestHTTPAnalyzer:
    ...
    def mock_response(self):
        ...
        response_context.text = AsyncMock(return_value="<html><body>Test content</body></html>")
    ...
    async def test_make_request_connection_reset(self, analyzer):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            mock_session.get = AsyncMock(side_effect=aiohttp.ClientError("Connection reset by peer"))
            mock_session.__aenter__ = AsyncMock(return_value=mock_session)
            mock_session.__aexit__ = AsyncMock(return_value=None)
    ...
    async def test_http_method_restrictions(self, analyzer):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            for method in ['get', 'post', 'put', 'delete', 'head', 'options', 'patch']:
                setattr(mock_session, method, AsyncMock(side_effect=lambda *args, **kwargs: mock_request_behavior(method, *args, **kwargs)))
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            async def mock_request_behavior(method, url, *args, **kwargs):
                ...
                if 'vpn' in str(data) or 'proxy' in str(data):
                    ...
                    response_context.text = AsyncMock(return_value="<html>This content is blocked</html>")
                    return response
                ...
                if '/blocked' in url:
                    ...
                    response_context.text = AsyncMock(return_value="<html>Redirecting...</html>")
                    return response
                
                # Normal response
                response_context.status = 200
                response_context.headers = {'Content-Type': 'text/html'}
                response_context.text = AsyncMock(return_value="<html><body>Normal content</body></html>")
                return response
    ...
    async def test_bytewise_packet_processing(self, analyzer):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            async def mock_response_with_raw_packets(*args, **kwargs):
                ...
                if hasattr(analyzer, 'use_bytewise_processing') and analyzer.use_bytewise_processing:
