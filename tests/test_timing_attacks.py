...
from ..timing_controller import TimingStrategy, PreciseTimingController
...
class TestTimingBase:
    ...
    def setup_method(self):
        """Setup test fixtures."""
        self.config = TimingConfiguration(
            base_delay_ms=10.0,
            min_delay_ms=1.0,
            max_delay_ms=100.0,
        ...
        # Create mock timing attack for testing base functionality
    ...
    def test_timing_configuration_validation(self):
        ...
        # Test invalid min/max delays
        config = TimingConfiguration(
            min_delay_ms=-5.0, max_delay_ms=5.0, base_delay_ms=10.0
        )

        assert config.min_delay_ms == 0.0  # Should be corrected
    ...
    def test_delay_sequence_generation(self):
        ...
        assert all(
            self.config.min_delay_ms <= d <= self.config.max_delay_ms for d in delays
        )
    ...
    def test_adaptive_delay_calculation(self):
        ...
        self.attack.last_response_time = 50.0  # Fast response

        delay = self.attack._calculate_adaptive_delay(1)
        assert delay > 0

        # Test slow response
        self.attack.last_response_time = 2000.0  # Slow response
        delay_slow = self.attack._calculate_adaptive_delay(2)
        assert delay_slow < delay  # Should reduce delay for slow responses
...
class TestJitterInjection:
    ...
    def setup_method(self):
        """Setup test fixtures."""
        self.config = JitterConfiguration(
    ...
    def test_jitter_configuration_validation(self):
        ...
        config = JitterConfiguration(
            jitter_amplitude_ms=-5.0, gaussian_stddev_ms=-1.0, packets_per_burst=0
        )

        assert config.jitter_amplitude_ms == 0.0  # Should be corrected
        assert config.gaussian_stddev_ms == 1.0  # Should be corrected
        assert config.packets_per_burst == 1  # Should be corrected
    ...
    def test_uniform_jitter_calculation(self):
        ...
        assert len(set(jitter_values)) > 10  # Should have variety
    ...
    def test_gaussian_jitter_calculation(self):
        ...
        # Check that jitter values are clamped to amplitude
    ...
    def test_periodic_jitter_calculation(self):
        ...
        # Check that jitter values are within amplitude bounds
        assert all(-10.0 <= j <= 10.0 for j in jitter_values)

        # Check that we have a periodic pattern (approximately)
        # Values should follow sine wave pattern
        assert jitter_values[0] != jitter_values[5]  # Should vary
    ...
    def test_jitter_sequence_generation(self):
        ...
        assert all(delay >= 0 for delay in sequence)  # No negative delays
    ...
    def test_jitter_statistics(self):
        ...
        # Execute some jitter operations
...
class TestDelayEvasion:
    ...
    def setup_method(self):
        """Setup test fixtures."""
        self.config = DelayEvasionConfiguration(
    ...
    def test_delay_configuration_validation(self):
        ...
        config = DelayEvasionConfiguration(
            progression_factor=0.5,  # Invalid
            max_progression_steps=0,  # Invalid
            fibonacci_multiplier=-1.0,  # Invalid
            packets_per_delay=0,  # Invalid
        )

        assert config.progression_factor == 1.5  # Should be corrected
        assert config.max_progression_steps == 1  # Should be corrected
        assert config.fibonacci_multiplier == 1.0  # Should be corrected
        assert config.packets_per_delay == 1  # Should be corrected
    ...
    def test_progressive_delay_generation(self):
        ...
        # Each delay should be larger than the previous (progressive)
    ...
    def test_exponential_delay_generation(self):
        ...
        # Should follow exponential pattern (until max is reached)
        for i in range(1, len(delays)):
    ...
    def test_fibonacci_delay_generation(self):
        ...
        # Should follow Fibonacci-based pattern
        # First few values should be based on Fibonacci sequence
        assert delays[0] == self.config.fibonacci_multiplier * 1  # F(0) = 1
        assert delays[1] == self.config.fibonacci_multiplier * 1  # F(1) = 1
    ...
    def test_sine_wave_delay_generation(self):
        ...
        assert len(delays) == 10
        assert all(delay >= 0 for delay in delays)  # No negative delays

        # Should have wave-like variation
    ...
    def test_random_walk_delay_generation(self):
        ...
        assert len(delays) == 10
        assert all(10.0 <= delay <= 100.0 for delay in delays)  # Within bounds
    ...
    def test_custom_delay_generation(self):
        ...
        assert len(delays) == 8
        # Should repeat the custom sequence
    ...
    def test_pattern_effectiveness_tracking(self):
        ...
        # Execute attack to populate effectiveness data
...
class TestBurstTraffic:
    ...
    def setup_method(self):
        """Setup test fixtures."""
        self.config = BurstConfiguration(
    ...
    def test_burst_configuration_validation(self):
        ...
        config = BurstConfiguration(
            min_burst_size=0,  # Invalid
            max_burst_size=5,
            default_burst_size=10,  # Invalid (> max)
            concurrent_streams=0,  # Invalid
            total_bursts=0,  # Invalid
        )

        assert config.min_burst_size == 1  # Should be corrected
        assert config.default_burst_size <= config.max_burst_size  # Should be corrected
        assert config.concurrent_streams == 1  # Should be corrected
        assert config.total_bursts == 1  # Should be corrected
    ...
    def test_exponential_burst_size_calculation(self):
        ...
        # Should increase exponentially (until max is reached)
        assert sizes[0] == 2  # 2 * 2^0 = 2
        assert sizes[1] == 4  # 2 * 2^1 = 4
        assert sizes[2] == 8  # 2 * 2^2 = 8
    ...
    def test_fibonacci_burst_size_calculation(self):
        ...
        # Should follow Fibonacci pattern (scaled to bounds)
        assert all(
            self.config.min_burst_size <= size <= self.config.max_burst_size
            for size in sizes
        )
        assert len(sizes) == 5
    ...
    def test_random_burst_size_calculation(self):
        ...
        # Should be within bounds and have variation
        assert all(3 <= size <= 10 for size in sizes)
        assert len(set(sizes)) > 1  # Should have variety
    ...
    def test_burst_interval_calculation(self):
        ...
        assert len(set(intervals)) > 1  # Should have variety
    ...
    def test_burst_sequence_generation(self):
        ...
        # Check burst sizes and intervals
    ...
    def test_concurrent_burst_execution(self):
        ...
        assert isinstance(result, AttackResult)
        # Should have executed bursts from multiple streams
        assert (
            result.packets_sent
            >= self.config.total_bursts * self.config.default_burst_size
        )

    def test_burst_statistics(self):
        """Test burst statistics collection."""
        # Execute attack to populate statistics
        self.attack.execute(self.context)
    ...
    def test_adaptive_burst_size(self):
        ...
        # Test fast response (should increase burst size)
        self.attack.last_response_time = 20.0  # Fast
        size1 = self.attack._calculate_burst_size(0)

        # Test slow response (should decrease burst size)
        self.attack.last_response_time = 100.0  # Slow
...
class TestTimingIntegration:
    ...
    def test_timing_attack_configuration(self):
        ...
        # Test jitter attack configuration
        jitter_attack = JitterInjectionAttack()
        jitter_attack.configure_jitter(
            jitter_type=JitterType.GAUSSIAN, amplitude_ms=15.0
        )
        ...
        # Test delay attack configuration
        delay_attack = DelayEvasionAttack()
        delay_attack.configure_delay_pattern(
            DelayPattern.EXPONENTIAL, progression_factor=2.0
        )
        ...
        # Test burst attack configuration
        burst_attack = BurstTrafficAttack()
        burst_attack.configure_burst_pattern(
            BurstType.RANDOM, min_burst_size=3, max_burst_size=15
        )
    ...
    def test_timing_attack_state_reset(self):
        ...
        for attack in attacks:
            # Execute attack to populate state
            attack.execute(self.context)
            ...
            # State should be reset (specific checks depend on attack type)
            assert True  # Basic test that reset doesn't crash
