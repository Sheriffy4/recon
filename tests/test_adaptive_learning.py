...
from cli import (
    AdaptiveLearningCache, StrategyPerformanceRecord
)
...
def test_domain_recommendations(learning_cache):
    ...
    assert recommendations[0][1] > recommendations[1][1]  # Success rates should be ordered
...
def test_dpi_recommendations(learning_cache):
    ...
    assert len(recommendations) == 2
    assert recommendations[0][1] > recommendations[1][1]  # Should be ordered by success rate
