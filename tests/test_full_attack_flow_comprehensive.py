...
class TestComprehensiveFullAttackFlow:
    ...
    def setup_method(self):
        ...
        self.techniques.apply_md5sig_fooling.return_value = bytearray(b"md5_fooled_packet")
    ...
    def test_all_registered_attacks_full_cycle(self):
        ...
        for attack_type in attack_types:
            ...
            assert metadata is not None, f"No metadata found for attack type '{attack_type}'"
            ...
            assert validation_result.is_valid, f"Parameter validation failed for '{attack_type}': {validation_result.error_message}"
            
            # Step 2: Get handler
            handler = self.registry.get_attack_handler(attack_type)
            assert handler is not None, f"No handler found for attack type '{attack_type}'"
            
            # Step 3: Dispatch attack
            try:
                result = self.dispatcher.dispatch_attack(attack_type, params, payload, {})
                assert result is not None, f"Dispatcher returned None for attack type '{attack_type}'"
    ...
    def test_fakeddisorder_comprehensive_flow(self):
        ...
        for test_case in test_cases:
            ...
            if params.get("split_pos") == "sni":
                with patch.object(self.dispatcher, '_find_sni_position', return_value=20):
                    result = self.dispatcher.dispatch_attack(attack_type, params, payload, {})
            else:
                result = self.dispatcher.dispatch_attack(attack_type, params, payload, {})
    ...
    def test_seqovl_comprehensive_flow(self):
        ...
        test_cases = [
            ...
            {"split_pos": "cipher", "overlap_size": 10, "fake_ttl": 3}  # Special split_pos
        ]
        ...
        for params in test_cases:
            ...
            if params.get("split_pos") == "cipher":
                with patch.object(self.dispatcher, '_find_cipher_position', return_value=25):
                    result = self.dispatcher.dispatch_attack(attack_type, params, payload, {})
            else:
                result = self.dispatcher.dispatch_attack(attack_type, params, payload, {})
    ...
    def test_multidisorder_comprehensive_flow(self):
        ...
        for params in test_cases:
            ...
            if params.get("split_pos") == "midsld":
                with patch.object(self.dispatcher, '_find_midsld_position', return_value=30):
                    result = self.dispatcher.dispatch_attack(attack_type, params, payload, {})
            else:
                result = self.dispatcher.dispatch_attack(attack_type, params, payload, {})
    ...
    def test_disorder_variants_comprehensive_flow(self):
        ...
        result1 = self.dispatcher.dispatch_attack("disorder", {"split_pos": 7}, payload, {})
        ...
        result2 = self.dispatcher.dispatch_attack("disorder2", {"split_pos": 12}, payload, {})
    ...
    def test_multisplit_and_split_comprehensive_flow(self):
        ...
        result2 = self.dispatcher.dispatch_attack("split", {"split_pos": 20}, payload, {})
        assert result2 is not None
        self.techniques.apply_multisplit.assert_called()
                    ...
                    with patch.object(self.dispatcher, '_find_sni_position', return_value=25):
                        with patch.object(self.dispatcher, '_find_midsld_position', return_value=35):
                            try:
                                result = self.dispatcher.dispatch_attack(attack_type, params, payload, {})
                                assert result is not None
                                print(f"✅ {attack_type} with split_pos='{special_value}' - SUCCESS")
                            except Exception as e:
                                print(f"⚠️ {attack_type} with split_pos='{special_value}' failed: {e}")
    ...
    def test_parameter_edge_cases_flow(self):
        ...
        edge_cases = [
            ...
            {
                "attack": "seqovl", 
                "params": {"split_pos": len(payload)-1, "overlap_size": len(payload)//2, "fake_ttl": 255},
    ...
    def test_performance_all_attacks_flow(self):
        ...
        for attack_type in attack_types:
            ...
            try:
                ...
                assert execution_time < 0.1, f"Attack '{attack_type}' too slow: {execution_time:.3f}s"
    ...
    def _prepare_attack_parameters(self, attack_type: str, metadata, payload: bytes) -> Dict[str, Any]:
...
class TestAttackFlowErrorHandling:
    ...
    def test_invalid_parameters_flow(self):
        ...
        for case in invalid_cases:
            ...
            validation_result = self.registry.validate_parameters(case["attack"], case["params"])
            if not validation_result.is_valid:
                assert case["expected_error"] in validation_result.error_message
                print(f"✅ Invalid params correctly rejected: {validation_result.error_message}")
            else:
                print(f"⚠️ Validation passed unexpectedly for {case['attack']} with {case['params']}")
                # Test that dispatcher handles it gracefully
                try:
                    result = self.dispatcher.dispatch_attack(case["attack"], case["params"], payload, {})
                    print(f"⚠️ Dispatcher also handled invalid params gracefully")
                except Exception as e:
        ...
        for attack_type in unknown_attacks:
            ...
            try:
                ...
                if result is not None:
                    print(f"⚠️ Unknown attack '{attack_type}' returned result instead of failing")
    ...
    def test_technique_method_failure_flow(self):
        ...
        self.techniques.apply_fakeddisorder.side_effect = RuntimeError("Technique method failed")
        
        with pytest.raises(RuntimeError):
            self.dispatcher.dispatch_attack("fakeddisorder", {"split_pos": 5}, payload, {})
