...
def test_special_position_validation():
    ...
    for i, test_case in enumerate(test_cases):
        try:
            ...
            print(
                f"✅ Test {i+1}: Valid strategy loaded - {test_case['type']} with params {test_case['params']}"
            )
...
def test_normalization():
    ...
    try:
        strategy = loader.load_strategy(test_case)
        if (
            strategy.params["split_pos"] == "cipher"
    ...
    try:
        strategy = loader.load_strategy(test_case)
        if (
            isinstance(strategy.params["fooling"], list)
            and len(strategy.params["fooling"]) == 2
        ):
