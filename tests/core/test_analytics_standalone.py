...
class SimpleMetricsCollector:
    ...
    async def record_attack_result(
        self, attack_id: str, success: bool, response_time: float
    ):
...
class SimpleAnalyticsEngine:
    ...
    async def record_attack_result(
        ...
        await self.metrics_collector.record_attack_result(
            attack_id, success, response_time
        )
        ...
        for attack_id, metrics in self.metrics_collector.attack_metrics.items():
            if metrics.success_rate < 0.5 and metrics.total_attempts > 5:
                recommendations.append(
                    f"Review configuration for {attack_id} (low success rate)"
                )
    ...
    with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as f:
        ...
        try:
            ...
            for attack in attacks:
                ...
                if analytics:
                    print(
                        f"  - {attack}: {analytics['success_rate']:.2f} success rate, "
            ...
            print(
                f"✓ System overview: {summary['total_attacks']} attacks, "
            ...
            print(
                f"  Overall Success Rate: {final_overview['overall_success_rate']:.2%}"
            )
            print(
                f"  Average Response Time: {final_overview['avg_response_time']:.2f}s"
            )
