...
async def test_monitoring_system_telemetry():
    ...
    try:
        ...
        assert (
            f"{test_domain}:443" in status_report["sites"]
        ), f"Сайт {test_domain} не найден в отчете"

        # Проверяем статистику современного обхода
        assert (
            status_report["modern_bypass_enabled"] == True
        ), "Modern bypass должен быть включен в отчете"

        # Проверяем статистику компонентов
        if monitoring.attack_registry:
            assert (
                "attack_registry_stats" in status_report
            ), "Отсутствует attack_registry_stats"
            print("✅ Attack registry статистика включена в отчет")

        if monitoring.pool_manager:
            assert (
                "pool_manager_stats" in status_report
            ), "Отсутствует pool_manager_stats"
...
async def test_engine_monitoring_integration():
    ...
    try:
        ...
        monitoring_config = MonitoringConfig(
            check_interval_seconds=10, enable_auto_recovery=True
        )
        ...
        ), "Телеметрия движка должна быть словарем"
        ...
        assert successes >= len(
            test_ips
        ), f"Недостаточно успешных операций: {successes}"
...
async def test_telemetry_data_consistency():
    ...
    try:
        ...
        assert (
            monitoring_timestamp is not None
        ), "Отсутствует timestamp в отчете мониторинга"

        print(
            f"✅ Временные метки: движок={engine_start_ts}, мониторинг={monitoring_timestamp}"
        )
        ...
        assert engine_targets >= len(
            test_operations
        ), f"Недостаточно целей в движке: {engine_targets}"
        assert monitoring_sites >= len(
            test_operations
        ), f"Недостаточно сайтов в мониторинге: {monitoring_sites}"
        ...
        ), f"Недостаточно успехов: {successes} < {expected_successes}"
        assert (
            failures >= expected_failures
        ), f"Недостаточно неудач: {failures} < {expected_failures}"

        print(f"✅ Счетчики: успехи={successes}, неудачи={failures}")
        print(
            f"✅ Цели в движке: {engine_targets}, сайты в мониторинге: {monitoring_sites}"
        )
...
async def main():
    ...
    print("\n" + "=" * 80)
    print("📊 Результаты тестирования интеграции телеметрии:")
