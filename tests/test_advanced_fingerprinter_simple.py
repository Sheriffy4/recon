# recon/core/fingerprint/test_advanced_fingerprinter_simple.py
"""
...
class TestAdvancedFingerprinteSimple:
    ...
    def test_get_stats(self):
        ...
        try:
            ...
            required_fields = [
                'fingerprints_created', 'cache_hits', 'cache_misses',
                'ml_classifications', 'fallback_classifications', 'errors',
                'total_analysis_time', 'cache_hit_rate', 'avg_analysis_time'
            ]
    ...
    async def test_health_check(self):
        ...
        try:
            ...
            expected_components = [
                'cache', 'ml_classifier', 'metrics_collector',
                'tcp_analyzer', 'http_analyzer', 'dns_analyzer'
            ]
            ...
            for component in expected_components:
                ...
                assert components[component]['status'] in [
                    'healthy', 'disabled', 'unhealthy', 'untrained'
                ]
    ...
    def test_heuristic_classification_patterns(self):
        ...
        try:
            ...
            # Test ROSKOMNADZOR_TSPU pattern (fast RST + DNS hijacking + header filtering)
            fingerprint1 = DPIFingerprint(
                target="test1.com:443",
                rst_injection_detected=True,
                dns_hijacking_detected=True,
                http_header_filtering=True,
                connection_reset_timing=50.0  # Fast reset
            )
            
            dpi_type, confidence = fingerprinter._heuristic_classification(fingerprint1)
            assert dpi_type == DPIType.ROSKOMNADZOR_TSPU  # Should be TSPU due to fast reset (< 100ms)
            assert confidence > 0.5
            
            # Test COMMERCIAL_DPI pattern
            fingerprint2 = DPIFingerprint(
                target="test2.com:443",
            ...
            # Test ISP_TRANSPARENT_PROXY pattern
            fingerprint3 = DPIFingerprint(
                target="test3.com:443",
            ...
            # Test UNKNOWN pattern (no clear indicators)
    ...
    def test_extract_ml_features(self):
        ...
        try:
            ...
            # Create fingerprint with various features
            fingerprint = DPIFingerprint(
                ...
                http_header_filtering=True,
                dns_hijacking_detected=True,
                handshake_anomalies=['anomaly1', 'anomaly2'],
                http_method_restrictions=['POST'],
            ...
            expected_features = [
                ...
                'tcp_options_filtering', 'connection_reset_timing', 'handshake_anomalies_count',
                'mss_clamping_detected', 'tcp_timestamp_manipulation', 'http_header_filtering',
                'content_inspection_depth', 'user_agent_filtering', 'host_header_manipulation',
                'http_method_restrictions_count', 'content_type_filtering', 'redirect_injection',
                'http_response_modification', 'keep_alive_manipulation', 'dns_hijacking_detected',
                'dns_response_modification', 'dns_query_filtering', 'doh_blocking', 'dot_blocking',
                'dns_cache_poisoning', 'dns_timeout_manipulation', 'recursive_resolver_blocking',
                'dns_over_tcp_blocking', 'edns_support', 'supports_ipv6', 'geographic_restrictions',
                'time_based_filtering', 'packet_size_limitations', 'protocol_whitelist_count',
                'analysis_duration'
            ]
    ...
    def test_calculate_reliability_score(self):
        ...
        try:
            ...
            # High reliability fingerprint (multiple analysis methods, high confidence)
            fingerprint1 = DPIFingerprint(
                target="test1.com:443",
                confidence=0.9,
            ...
            assert reliability1 > 0.8  # Should be high
            
            # Low reliability fingerprint (few methods, low confidence)
            fingerprint2 = DPIFingerprint(
                target="test2.com:443",
                confidence=0.2,
            ...
            assert reliability2 < 0.3  # Should be low
            
            # Medium reliability fingerprint
            fingerprint3 = DPIFingerprint(
                target="test3.com:443",
                confidence=0.6,
                analysis_methods_used=['tcp_analysis', 'http_analysis'],
                rst_injection_detected=True,
                http_header_filtering=True,
                dns_hijacking_detected=True
            )
            ...
            assert 0.3 < reliability3 < 0.8  # Should be medium
    ...
    async def test_context_manager(self):
        ...
        try:
            ...
            # After context exit, fingerprinter should be closed
            # (We can't easily test internal state, but no exceptions should occur)
    ...
    async def test_fingerprint_with_mocked_analyzers(self):
        ...
        try:
            config = FingerprintingConfig(
                enable_cache=False,  # Disable cache to avoid complications
                enable_ml=False,     # Disable ML to use heuristics
            ...
            mock_tcp_result = {
                ...
                'handshake_anomalies': ['test_anomaly'],
