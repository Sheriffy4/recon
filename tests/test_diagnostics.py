...
try:
    from core.fingerprint.diagnostics import (
        MetricsCollector, HealthChecker, DiagnosticLogger, DiagnosticSystem,
        PerformanceMetric, HealthCheckResult, DiagnosticReport,
        get_diagnostic_system, setup_logging, monitor_operation
    )
...
class TestDiagnosticSystem(unittest.TestCase):
    ...
    def test_get_system_info(self, mock_boot_time, mock_disk_usage, mock_memory, mock_cpu_count):
        """Test system information collection."""
        # Mock system information
        mock_cpu_count.return_value = 4
        mock_memory.return_value = Mock(total=8000000000)
        mock_disk_usage.return_value = Mock(total=1000000000, free=500000000, used=500000000)
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_complete_fingerprinting_workflow_monitoring(self):
        ...
        self.diagnostic_system.record_analyzer_operation(
            analyzer="tcp", target=target, duration=0.5, success=True,
            result={'rst_injection_detected': True}
        )
        
        self.diagnostic_system.record_analyzer_operation(
            analyzer="http", target=target, duration=0.8, success=True,
            result={'http_header_filtering': True}
        )
        
        self.diagnostic_system.record_analyzer_operation(
            analyzer="dns", target=target, duration=0.3, success=True,
        ...
        self.diagnostic_system.record_cache_operation("get", target, hit=False, duration=0.001)
