...
class TestCLIIntegration:
    ...
    def test_genes_to_zapret_strategy_all_attack_types(self):
        ...
        test_attacks = [
            "fakeddisorder", "seqovl", "multidisorder", "disorder", "multisplit",
            "split", "fake", "disorder2", "wssize_limit", "tlsrec_split"
        ]
        ...
        for attack_type in test_attacks:
            if attack_type in attack_types:
                ...
                if attack_type in ["fakeddisorder", "seqovl", "disorder", "split", "fake"]:
                    genes["split_pos"] = "5"
                ...
                assert strategy is not None, f"Failed to generate strategy for {attack_type}"
                assert len(strategy) > 0, f"Generated empty strategy for {attack_type}"
                assert isinstance(strategy, str), f"Generated strategy is not a string for {attack_type}"
                assert "dpi-desync" in strategy, f"Generated strategy missing dpi-desync for {attack_type}"
...
class TestRealStrategyIntegration:
    ...
    def test_load_and_execute_real_strategy_files(self):
        ...
        mock_strategies = [
            ...
            {
                "type": "seqovl",
                "params": {"split_pos": 5, "overlap_size": 10}
            },
            {
                "type": "multidisorder",
    ...
    def test_strategy_parameter_validation_with_real_data(self):
        ...
        for strategy in test_strategies:
            ...
            assert validation_result.is_valid, f"Strategy {strategy['type']} failed validation: {validation_result.error_message}"
    ...
    def test_real_strategy_execution_with_complex_parameters(self):
        ...
        for strategy in complex_strategies:
            ...
            # Execute through dispatcher
            with patch.object(self.dispatcher, '_find_sni_position', return_value=76):  # Mock SNI position resolution
                result = self.dispatcher.dispatch_attack(
                    strategy["type"], strategy["params"], payload, packet_info
                )
            
            assert result is not None, f"Strategy {strategy['type']} returned None"
            assert isinstance(result, list), f"Strategy {strategy['type']} should return a list"
            
            # Verify the result contains valid segments
            for segment in result:
                assert isinstance(segment, tuple), "Each segment should be a tuple"
                assert len(segment) == 3, "Each segment should have 3 elements (data, offset, options)"
    ...
    def test_real_strategy_with_edge_case_payloads(self):
        ...
        for i, payload in enumerate(edge_case_payloads):
            ...
            assert validation_result.is_valid, f"Strategy validation failed for payload {i}"
            ...
            try:
                ...
                assert "Invalid parameters" in str(e), f"Unexpected error for payload {i}: {e}"
    ...
    def test_real_strategy_performance_with_large_payloads(self):
        ...
        for strategy in strategies:
            ...
            assert validation_result.is_valid, f"Strategy {strategy['type']} failed validation"
            ...
            assert execution_time < 0.1, f"Strategy {strategy['type']} took too long: {execution_time*1000:.2f}ms"
...
class TestPerformanceIntegration:
    ...
    def test_end_to_end_dispatch_performance(self):
        ...
        for attack_type, params in test_strategies:
            for _ in range(10):
                self.dispatcher.dispatch_attack(attack_type, params, payload, packet_info)
        ...
        for attack_type, params in test_strategies:
            ...
            for _ in range(iterations):
                result = self.dispatcher.dispatch_attack(attack_type, params, payload, packet_info)
            ...
            assert avg_time < 0.002, f"{attack_type} dispatch too slow: {avg_time*1000:.3f}ms per call"
    ...
    def test_registry_lookup_performance(self):
        ...
        attack_types = ["fakeddisorder", "seqovl", "multidisorder", "disorder", "multisplit", "split", "fake"]
        ...
        for _ in range(iterations):
            for attack_type in attack_types:
                ...
                validation = self.registry.validate_parameters(attack_type, {"split_pos": 3})
        
        end_time = time.time()
        total_time = end_time - start_time
        avg_time_per_operation = total_time / (iterations * len(attack_types) * 2)  # 2 operations per iteration
        
        # Should be very fast (less than 0.1ms per operation)
        assert avg_time_per_operation < 0.0001, f"Registry operations too slow: {avg_time_per_operation*1000:.3f}ms per operation"
        
        print(f"Registry performance: {avg_time_per_operation*1000:.3f}ms per operation")
    ...
    def test_concurrent_dispatch_performance(self):
        ...
        test_tasks = [
            ...
            ("seqovl", {"split_pos": 10, "overlap_size": 20, "fake_ttl": 2}, b"Payload 2", {}),
        ...
        for _ in range(num_threads):
            t = threading.Thread(target=dispatch_worker, args=(task_queue, result_queue))
            t.start()
            threads.append(t)
        ...
        assert avg_time_per_dispatch < 0.01, f"Concurrent dispatch too slow: {avg_time_per_dispatch*1000:.3f}ms per dispatch"
        ...
        assert success_count == iterations * len(test_tasks), f"Expected {iterations * len(test_tasks)} successful operations, got {success_count}"
        
        print(f"Concurrent performance: {avg_time_per_dispatch*1000:.3f}ms per dispatch with {num_threads} threads")
...
class TestBackwardCompatibility:
        ...
        legacy_strategies = [
            {
                ...
                "params": {
                    "split_pos": 76,
                    "fake_ttl": 3  # Legacy parameter name
                }
            },
            {
                "type": "seqovl",
    ...
    def test_alias_compatibility(self):
        ...
        for alias, canonical in alias_tests:
            ...
            canonical_validation = self.registry.validate_parameters(canonical, test_params)
