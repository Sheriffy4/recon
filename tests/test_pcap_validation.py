...
class PCAPAnalyzer:
        ...
        for i, packet in enumerate(self.packets):
            ...
            if checksum_info:
                results["checksum_analysis"].append(
                    {"packet_index": i, "checksum_info": checksum_info}
                )
        ...
        if tcp_payloads:
            ...
            return {
                ...
                "original_payload_size": (
                    len(original_payload) if original_payload else 0
                ),
    ...
    def _analyze_packet_checksum(self, packet_data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            is_badsum = self._is_likely_badsum_pattern(
...
class TestPCAPValidation:
    ...
    def test_pcap_validation_packet_reconstruction(self):
        ...
        reconstruction_analysis = analyzer.analyze_packet_reconstruction(
            original_packet
        )
    ...
    def test_pcap_validation_multiple_split_positions(self):
        ...
        if len(result_packets) > 2:
            total_splits = (
                split_analysis["position_3_splits"]
    ...
    def test_pcap_validation_youtube_scenario(self):
        ...
        for i, packet in enumerate(result_packets):
            analyzer.add_packet(
                packet, metadata={"packet_type": "youtube_tls", "sequence": i}
            )
    ...
    def test_create_test_pcap_files(self):
        ...
        for test_case in test_cases:
            ...
            # Create original packet
            original_packet = self._create_test_tcp_packet_with_payload(
                test_case["payload"]
            )
    ...
    def test_automated_pcap_analysis(self):
        ...
        for name, pcap_data in pcap_files.items():
            ...
            analyzer.add_packet(
                pcap_data["original_packet"], metadata={"type": "original"}
            )
    ...
    def test_pcap_validation_comprehensive_report(self):
        ...
        for name, results in analysis_results.items():
            try:
                # Count packets
                report["summary"]["total_packets_analyzed"] += results[
                    "split_analysis"
                ]["total_packets"]
                ...
                if (
                    reconstruction.get("can_reconstruct") is False
                    and reconstruction.get("reason") != "no_split_packets"
                ):
...
class TestPCAPValidationAdvanced:
    ...
    def test_real_world_pcap_simulation(self):
        ...
        real_world_scenarios = [
            ...
            {"name": "twitter_access", "hostname": "twitter.com", "payload_size": 128},
        ]
        ...
        for scenario in real_world_scenarios:
            ...
            tls_payload = TestPCAPValidation()._create_tls_client_hello_packet(
                scenario["hostname"]
            )
            # Pad to desired size
            ...
            original_packet = TestPCAPValidation()._create_test_tcp_packet_with_payload(
                tls_payload
            )
    ...
    def test_pcap_validation_performance_metrics(self):
        ...
        for size in [64, 128, 256, 512, 1024, 1500]:  # Various packet sizes
            tls_payload = TestPCAPValidation()._create_tls_client_hello_packet(
                f"perf-test-{size}.com"
            )
            if len(tls_payload) < size:
                tls_payload += b"\x00" * (size - len(tls_payload))
            packet = TestPCAPValidation()._create_test_tcp_packet_with_payload(
                tls_payload
            )
        ...
        performance_metrics["summary"] = {
            "avg_strategy_time_ms": sum(performance_metrics["processing_times"])
            / len(performance_metrics["processing_times"])
            * 1000,
            "avg_analysis_time_ms": sum(performance_metrics["analysis_times"])
            / len(performance_metrics["analysis_times"])
            * 1000,
            ...
            "total_processing_time_ms": sum(performance_metrics["processing_times"])
            * 1000,
        ...
        assert (
            performance_metrics["summary"]["avg_strategy_time_ms"] < 100
        )  # Less than 100ms average
        assert (
            performance_metrics["summary"]["avg_analysis_time_ms"] < 50
        )  # Less than 50ms average
        assert (
            performance_metrics["summary"]["max_strategy_time_ms"] < 500
        )  # Less than 500ms max
