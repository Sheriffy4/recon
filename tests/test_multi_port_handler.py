...
from recon.tests.multi_port_handler import MultiPortHandler, ProtocolFamily, PortStrategy, PortTestResult, BypassResult
from recon.types import BlockType
...
class TestMultiPortHandler:
    ...
    def sample_attacks(self):
        ...
        attacks = []
        http_attack = AttackDefinition(id='http_host_header_case', name='HTTP Host Header Case Manipulation', description='Manipulate case of HTTP Host header', category=AttackCategory.HTTP_MANIPULATION, complexity=AttackComplexity.SIMPLE, stability=AttackStability.STABLE, supported_protocols=['tcp'], supported_ports=[80, 443], effectiveness_score=0.8)
        attacks.append(http_attack)
        tls_attack = AttackDefinition(id='tls_sni_fragmentation', name='TLS SNI Fragmentation', description='Fragment TLS SNI extension', category=AttackCategory.TLS_EVASION, complexity=AttackComplexity.MODERATE, stability=AttackStability.STABLE, supported_protocols=['tcp'], supported_ports=[443], effectiveness_score=0.9)
        attacks.append(tls_attack)
        dns_attack = AttackDefinition(id='dns_fragmentation', name='DNS Query Fragmentation', description='Fragment DNS queries', category=AttackCategory.DNS_TUNNELING, complexity=AttackComplexity.SIMPLE, stability=AttackStability.STABLE, supported_protocols=['udp'], supported_ports=[53], effectiveness_score=0.7)
    ...
    def test_add_remove_port_strategy(self, handler):
        ...
        custom_strategy = PortStrategy(port=8443, protocol_family=ProtocolFamily.SECURE_FAMILY, preferred_attacks=['custom_attack'], requires_tls=True)
    ...
    async def test_domain_accessibility_testing(self, handler):
        ...
        with patch.object(handler, '_test_single_port') as mock_test:
            mock_test.side_effect = [PortTestResult(port=80, accessible=True, response_time_ms=100.0, block_type=BlockType.NONE), PortTestResult(port=443, accessible=False, response_time_ms=5000.0, block_type=BlockType.TIMEOUT)]
    ...
    async def test_port_specific_strategy_application(self, handler, sample_attacks):
        ...
        with patch.object(handler, '_test_single_port') as mock_test:
            mock_test.return_value = PortTestResult(port=443, accessible=True, response_time_ms=200.0, block_type=BlockType.NONE)
            result = await handler.apply_port_specific_strategy('example.com', 443, 'test_strategy', sample_attacks)
    ...
    async def test_strategy_application_failure(self, handler, sample_attacks):
        ...
        with patch.object(handler, '_test_single_port') as mock_test:
            mock_test.return_value = PortTestResult(port=80, accessible=False, response_time_ms=5000.0, block_type=BlockType.TIMEOUT)
            result = await handler.apply_port_specific_strategy('blocked.example.com', 80, 'test_strategy', sample_attacks)
    ...
    def test_optimal_port_selection(self, handler):
        ...
        test_results = {80: PortTestResult(port=80, accessible=True, response_time_ms=100.0), 443: PortTestResult(port=443, accessible=True, response_time_ms=150.0)}
        optimal_port = handler.get_optimal_port_for_domain('example.com', test_results)
        assert optimal_port == 443
        test_results = {80: PortTestResult(port=80, accessible=True, response_time_ms=100.0), 443: PortTestResult(port=443, accessible=False, response_time_ms=5000.0)}
        optimal_port = handler.get_optimal_port_for_domain('example.com', test_results)
        assert optimal_port == 80
        test_results = {80: PortTestResult(port=80, accessible=False, response_time_ms=5000.0), 443: PortTestResult(port=443, accessible=False, response_time_ms=5000.0)}
        optimal_port = handler.get_optimal_port_for_domain('example.com', test_results)
        assert optimal_port == 443

    def test_protocol_requirements_detection(self, handler):
        """Test protocol requirements detection."""
        test_results = {80: PortTestResult(port=80, accessible=True, response_time_ms=100.0), 443: PortTestResult(port=443, accessible=True, response_time_ms=150.0), 53: PortTestResult(port=53, accessible=False, response_time_ms=1000.0)}
        required_ports = handler.detect_protocol_requirements('example.com', test_results)
        assert 80 in required_ports
        assert 443 in required_ports
        assert 53 not in required_ports
        test_results = {80: PortTestResult(port=80, accessible=False, response_time_ms=5000.0, block_type=BlockType.TIMEOUT), 443: PortTestResult(port=443, accessible=False, response_time_ms=5000.0, block_type=BlockType.RST_INJECTION)}
        required_ports = handler.detect_protocol_requirements('example.com', test_results)
        assert 80 in required_ports
        assert 443 in required_ports
    ...
    def test_cache_functionality(self, handler):
        ...
        test_result = PortTestResult(port=80, accessible=True, response_time_ms=100.0, block_type=BlockType.NONE)
    ...
    def test_supported_ports_management(self, handler):
        ...
        custom_strategy = PortStrategy(port=8080, protocol_family=ProtocolFamily.HTTP_FAMILY)
...
class TestPortStrategy:
    ...
    def test_port_strategy_initialization(self):
        ...
        https_strategy = PortStrategy(port=443, protocol_family=ProtocolFamily.SECURE_FAMILY)
        assert https_strategy.requires_tls is True
        assert https_strategy.supports_sni is True
        assert https_strategy.validation_method == 'tls_handshake'
        http_strategy = PortStrategy(port=80, protocol_family=ProtocolFamily.HTTP_FAMILY)
    ...
    def test_port_strategy_custom_configuration(self):
        ...
        custom_strategy = PortStrategy(port=8443, protocol_family=ProtocolFamily.SECURE_FAMILY, preferred_attacks=['custom_tls_attack'], blocked_attacks=['unsafe_attack'], default_timeout=60, custom_headers={'X-Custom': 'value'})
...
class TestPortTestResult:
    ...
    def test_port_test_result_initialization(self):
        ...
        result = PortTestResult(port=443, accessible=True, response_time_ms=150.0, protocol_detected='https', tls_version='TLSv1.3')
        assert result.port == 443
        assert result.accessible is True
    ...
    def test_port_test_result_with_block_type(self):
        ...
        result = PortTestResult(port=80, accessible=False, response_time_ms=5000.0, block_type=BlockType.TIMEOUT, error_message='Connection timed out')
        assert result.port == 80
        assert result.accessible is False
        assert result.block_type == BlockType.TIMEOUT
...
class TestBypassResult:
    ...
    def test_bypass_result_success(self):
        ...
        result = BypassResult(success=True, port=443, strategy_used='tls_fragmentation', execution_time_ms=250.0, attacks_applied=['tls_sni_fragmentation', 'tcp_window_scaling'], metadata={'test_key': 'test_value'})
        assert result.success is True
        assert result.port == 443
    ...
    def test_bypass_result_failure(self):
        ...
        result = BypassResult(success=False, port=80, strategy_used='http_manipulation', execution_time_ms=100.0, error_message='No suitable attacks found')
        assert result.success is False
        assert result.port == 80
...
async def test_integration_multi_port_workflow():
    ...
    attacks = [AttackDefinition(id='http_test_attack', name='HTTP Test Attack', description='Test HTTP attack', category=AttackCategory.HTTP_MANIPULATION, complexity=AttackComplexity.SIMPLE, stability=AttackStability.STABLE, supported_protocols=['tcp'], supported_ports=[80, 443], effectiveness_score=0.8), AttackDefinition(id='tls_test_attack', name='TLS Test Attack', description='Test TLS attack', category=AttackCategory.TLS_EVASION, complexity=AttackComplexity.MODERATE, stability=AttackStability.STABLE, supported_protocols=['tcp'], supported_ports=[443], effectiveness_score=0.9)]
    with patch.object(handler, '_test_single_port') as mock_test:
        mock_test.side_effect = [PortTestResult(port=80, accessible=True, response_time_ms=100.0, block_type=BlockType.NONE), PortTestResult(port=443, accessible=True, response_time_ms=150.0, block_type=BlockType.NONE), PortTestResult(port=443, accessible=True, response_time_ms=120.0, block_type=BlockType.NONE)]
        accessibility_results = await handler.test_domain_accessibility('example.com', [80, 443])
        ...
        optimal_port = handler.get_optimal_port_for_domain('example.com', accessibility_results)
        assert optimal_port == 443
        bypass_result = await handler.apply_port_specific_strategy('example.com', optimal_port, 'integration_test_strategy', attacks)
