...
class TestMultiPortHandler:
    ...
    def test_attack_selection_for_port(self, handler, sample_attacks):
        ...
        assert (
            "tls_sni_fragmentation" not in http_attack_ids
        )  # TLS attack shouldn't be selected for HTTP
        ...
        assert (
            "http_host_header_case" in https_attack_ids
        )  # HTTP attack should work on HTTPS too
        ...
        assert (
            "http_host_header_case" not in dns_attack_ids
        )  # HTTP attack shouldn't work on DNS
