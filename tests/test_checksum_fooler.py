...
from core.bypass.strategies.exceptions import ChecksumCalculationError
...
class TestChecksumFooler:
    ...
    def test_calculate_bad_checksum_error_handling(self):
        ...
        with patch.object(self.fooler, 'calculate_bad_checksum', side_effect=Exception("Test error")):
    ...
    def test_should_apply_badsum_small_packet(self):
        ...
        tcp_info = self._create_https_tcp_info()
        context = {'packet_size': 30}  # Below minimum size
        
        result = self.fooler.should_apply_badsum(tcp_info, is_first_part=True, packet_context=context)
    ...
    def test_apply_badsum_success(self):
        ...
        modified_packet, checksum_result = self.fooler.apply_badsum(packet_data, tcp_info)
    ...
    def test_verify_checksum_modification_valid(self):
        ...
        result = self.fooler.verify_checksum_modification(original_packet, modified_packet)
    ...
    def test_restore_original_checksum(self):
        ...
        restored_packet = self.fooler.restore_original_checksum(modified_packet, original_checksum)
    ...
    def test_apply_badsum_to_first_part_only(self):
        ...
        modified_parts, checksum_results = self.fooler.apply_badsum_to_first_part_only(packet_parts, tcp_infos)
    ...
    def test_is_likely_badsum_pattern_true(self):
        ...
        result = self.fooler._is_likely_badsum_pattern(badsum_checksum, correct_checksum)
    ...
    def test_is_likely_badsum_pattern_false(self):
        ...
        result = self.fooler._is_likely_badsum_pattern(random_checksum, correct_checksum)
    ...
    def test_classify_checksum_error(self):
        ...
        assert self.fooler._classify_checksum_error(correct ^ 0xDEAD, correct) == "intentional_badsum"
        assert self.fooler._classify_checksum_error(correct + 1, correct) == "single_bit_error"
        assert self.fooler._classify_checksum_error(0x0000, correct) == "zero_checksum"
        assert self.fooler._classify_checksum_error(0xFFFF, correct) == "all_ones_checksum"
    
    def test_create_test_packet_with_badsum(self):
        """Test creating test packet with badsum."""
        base_packet = self._create_tcp_packet()
        
        modified_packet, test_info = self.fooler.create_test_packet_with_badsum(base_packet)
    ...
    def test_calculate_correct_tcp_checksum(self):
        ...
        packet = self._create_tcp_packet_with_checksum(0x0000)  # Zero checksum for calculation
...
class TestChecksumFoolerIntegration:
    ...
    def test_multiple_packet_parts_scenario(self, checksum_fooler):
        ...
        modified_parts, results = checksum_fooler.apply_badsum_to_first_part_only(packet_parts, tcp_infos)
    ...
    def test_error_recovery_malformed_packets(self, checksum_fooler):
        ...
        assert error_count == len(malformed_packets)
