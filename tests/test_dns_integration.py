# recon/core/fingerprint/test_dns_integration.py
"""
...
from .dns_analyzer import DNSAnalyzer, DNSResponse, DNSQuery, DNSRecordType
from .advanced_models import DPIFingerprint, DPIType, ConfidenceLevel
# from .metrics_collector import MetricsCollector  # Skip for now
...
class TestDNSIntegration:
    ...
    #     """Create MetricsCollector instance for integration testing"""
    ...
    async def test_dns_analysis_with_metrics_collector(self, analyzer):
        ...
        # Mock DNS analyzer methods to return predictable results
        with patch.object(analyzer, '_detect_dns_hijacking', new_callable=AsyncMock) as mock_hijacking, \
             patch.object(analyzer, '_detect_response_modification', new_callable=AsyncMock) as mock_modification, \
             patch.object(analyzer, '_test_doh_blocking', new_callable=AsyncMock) as mock_doh, \
             patch.object(analyzer, '_test_dot_blocking', new_callable=AsyncMock) as mock_dot, \
             patch.object(analyzer, '_detect_cache_poisoning', new_callable=AsyncMock) as mock_poisoning, \
             patch.object(analyzer, '_test_edns_support', new_callable=AsyncMock) as mock_edns, \
             patch.object(analyzer, '_test_dns_over_tcp', new_callable=AsyncMock) as mock_tcp, \
             patch.object(analyzer, '_test_recursive_resolver_blocking', new_callable=AsyncMock) as mock_resolver, \
             patch.object(analyzer, '_detect_timeout_manipulation', new_callable=AsyncMock) as mock_timeout:
            
            # Setup mock returns for blocked scenario
            ...
            mock_modification.return_value = {
                ...
                'modification_details': [{'record_type': 'A', 'suspicious_patterns': ['null_ip_response']}]
            }
            mock_doh.return_value = {
                'doh_blocking': True,
                'doh_details': {'blocked_servers': ['cloudflare', 'google'], 'working_servers': []}
            }
            mock_dot.return_value = {
                'dot_blocking': True,
                'dot_details': {'blocked_servers': ['cloudflare', 'google'], 'working_servers': []}
            }
            mock_poisoning.return_value = {
                'dns_cache_poisoning': True,
                'poisoning_details': {'inconsistent_responses': [['1.2.3.4'], ['5.6.7.8']]}
            }
            ...
            mock_resolver.return_value = {
                ...
                'resolver_details': {'blocked_resolvers': ['8.8.8.8', '1.1.1.1'], 'working_resolvers': []}
            }
            ...
            # Verify comprehensive blocking detection
            ...
            # Verify detailed results are present
    ...
    async def test_dns_analysis_clean_environment(self, analyzer):
        ...
        with patch.object(analyzer, '_detect_dns_hijacking', new_callable=AsyncMock) as mock_hijacking, \
            ...
            # Setup mock returns for clean environment
            mock_hijacking.return_value = {'dns_hijacking_detected': False}
            mock_modification.return_value = {'dns_response_modification': False}
            mock_doh.return_value = {
                'doh_blocking': False,
                'doh_details': {'blocked_servers': [], 'working_servers': ['cloudflare', 'google']}
            }
            mock_dot.return_value = {
                'dot_blocking': False,
                'dot_details': {'blocked_servers': [], 'working_servers': ['cloudflare', 'google']}
            }
            mock_poisoning.return_value = {'dns_cache_poisoning': False}
            mock_edns.return_value = {'edns_support': True, 'edns_details': {'edns_version': 0}}
            mock_tcp.return_value = {
            ...
            mock_resolver.return_value = {
                ...
                'resolver_details': {'blocked_resolvers': [], 'working_resolvers': ['8.8.8.8', '1.1.1.1']}
            }
            mock_timeout.return_value = {
    ...
    async def test_dns_analysis_partial_blocking(self, analyzer):
        ...
        with patch.object(analyzer, '_detect_dns_hijacking', new_callable=AsyncMock) as mock_hijacking, \
            ...
            # Setup mixed blocking scenario
            mock_hijacking.return_value = {'dns_hijacking_detected': False}
            mock_modification.return_value = {'dns_response_modification': False}
            mock_doh.return_value = {
                'doh_blocking': True,  # DoH blocked
                'doh_details': {'blocked_servers': ['cloudflare'], 'working_servers': ['google']}
            }
            mock_dot.return_value = {
                'dot_blocking': False,  # DoT working
                'dot_details': {'blocked_servers': [], 'working_servers': ['cloudflare', 'google']}
            }
            mock_poisoning.return_value = {'dns_cache_poisoning': False}
            mock_edns.return_value = {'edns_support': True}
            mock_tcp.return_value = {
                'dns_over_tcp_blocking': True,  # TCP blocked
                'tcp_details': {'tcp_successful': False, 'udp_successful': True}
            }
            mock_resolver.return_value = {
                'recursive_resolver_blocking': False,
            ...
            # Verify partial blocking detected
            assert dns_results['doh_blocking'] is True
            assert dns_results['dot_blocking'] is False
            assert dns_results['dns_over_tcp_blocking'] is True
            assert dns_results['dns_hijacking_detected'] is False
    ...
    async def test_dns_analysis_error_handling(self, analyzer):
        ...
        with patch.object(analyzer, '_detect_dns_hijacking', new_callable=AsyncMock) as mock_hijacking, \
            ...
            # Simulate errors in some methods
            mock_hijacking.side_effect = Exception("Network error")
            mock_modification.return_value = {'dns_response_modification': False}
            mock_doh.side_effect = Exception("DoH server unreachable")
            
            # Run DNS analysis - should not crash
            dns_results = await analyzer.analyze_dns_behavior(target)
            
            # Verify analysis completed despite errors
            assert isinstance(dns_results, dict)
            assert 'analysis_duration' in dns_results
            assert 'analysis_error' in dns_results or 'dns_response_modification' in dns_results
    ...
    async def test_dns_query_methods_integration(self, analyzer):
        ...
        with patch('socket.socket') as mock_socket, \
             patch.object(analyzer, '_query_dns_udp', new_callable=AsyncMock) as mock_udp:
    ...
    async def test_dns_blocking_pattern_analysis(self, analyzer):
        ...
        # Test suspicious response detection
        query = DNSQuery(
            timestamp=time.time(),
    ...
    async def test_dns_analysis_performance(self, analyzer):
        ...
        with patch.object(analyzer, '_detect_dns_hijacking', new_callable=AsyncMock) as mock_hijacking, \
            ...
            # Analysis should complete quickly with mocked methods
            assert actual_duration < 1.0  # Less than 1 second
...
class TestDNSRealWorldScenarios:
    ...
    async def test_government_censorship_scenario(self, analyzer):
        ...
        with patch.object(analyzer, '_detect_dns_hijacking', new_callable=AsyncMock) as mock_hijacking, \
            ...
            # Government censorship typically involves comprehensive blocking
            mock_hijacking.return_value = {
                'dns_hijacking_detected': True,
            ...
            # Verify comprehensive blocking pattern
            blocking_indicators = [
            ...
            # Most indicators should be True for government censorship
    ...
    async def test_isp_transparent_proxy_scenario(self, analyzer):
        ...
        with patch.object(analyzer, '_detect_dns_hijacking', new_callable=AsyncMock) as mock_hijacking, \
            ...
            # ISP proxy typically involves selective blocking
            mock_hijacking.return_value = {'dns_hijacking_detected': False}
            mock_modification.return_value = {
                'dns_response_modification': True,
                'modification_details': [{'suspicious_patterns': ['private_ip_response']}]
            }
            mock_doh.return_value = {'doh_blocking': False}  # DoH usually works
            mock_dot.return_value = {'dot_blocking': False}  # DoT usually works
            mock_poisoning.return_value = {'dns_cache_poisoning': False}
            mock_edns.return_value = {'edns_support': True}
            mock_tcp.return_value = {'dns_over_tcp_blocking': False}
            mock_resolver.return_value = {'recursive_resolver_blocking': False}
            ...
            # Verify selective blocking pattern
            assert dns_results['dns_response_modification'] is True
            assert dns_results['doh_blocking'] is False
            assert dns_results['dot_blocking'] is False
            assert dns_results['dns_hijacking_detected'] is False
