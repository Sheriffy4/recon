...
class TestBypassEngineAPI(AioHTTPTestCase):
    ...
    async def test_update_pool(self):
        ...
        resp = await self.client.request(
            "PUT", f"/api/bypass/pools/{pool.id}", json=update_data
        )
        self.assertEqual(resp.status, 200)
    ...
    async def test_add_domain_to_pool(self):
        ...
        resp = await self.client.request(
            "POST", f"/api/bypass/pools/{pool.id}/domains", json=domain_data
        )
        self.assertEqual(resp.status, 200)
        updated_pool = self.pool_manager.get_pool(pool.id)
    ...
    async def test_import_config(self):
        ...
        resp = await self.client.request(
            "POST", "/api/bypass/config/import", json=config_data
        )
        self.assertEqual(resp.status, 200)
        data = await resp.json()
        self.assertTrue(data["success"])
...
class TestBypassDashboard(AioHTTPTestCase):
    ...
    async def get_application(self):
        ...
        api = BypassEngineAPI(
            pool_manager=pool_manager, attack_registry=attack_registry
        )
...
class TestRealTimeFeatures:
    ...
    def setup_method(self):
        ...
        self.attack_registry = ModernAttackRegistry()
        self.api = BypassEngineAPI(
            pool_manager=self.pool_manager, attack_registry=self.attack_registry
        )
    ...
    async def test_strategy_test_session(self):
        ...
        self.api.test_runner.run_strategy_test = AsyncMock(
            return_value={"success": True, "response_time": 150, "accessibility": True}
        )
        test_id = "test_123"
        domain = "example.com"
...
class TestStrategySharing:
    ...
    def test_strategy_validation_on_import(self):
        ...
        invalid_config = {
            "pools": [{"name": "Invalid Pool", "domains": ["example.com"]}]
        }
...
def run_integration_tests():
    ...
    try:
        ...
        api_routes = [
            ...
            if r.resource.canonical.startswith("/api/bypass")
        ]
        dashboard_routes = [
            r for r in app.router.routes() if r.resource.canonical.startswith("/bypass")
        ]
        print(
            f"✅ Routes configured: {len(api_routes)} API routes, {len(dashboard_routes)} dashboard routes"
        )
