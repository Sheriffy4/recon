...
class TestTCPAnalyzerIntegration:
    ...
    async def test_tcp_analyzer_basic_functionality(self):
        ...
        with patch.object(
            analyzer, "_resolve_target", return_value="127.0.0.1"
        ) as mock_resolve, patch.object(
            analyzer, "_analyze_basic_connections"
        ) as mock_basic, patch.object(
            analyzer, "_analyze_rst_injection"
        ) as mock_rst, patch.object(
            analyzer, "_analyze_window_manipulation"
        ) as mock_window, patch.object(
            analyzer, "_analyze_sequence_numbers"
        ) as mock_seq, patch.object(
            analyzer, "_analyze_fragmentation_handling"
        ) as mock_frag, patch.object(
            analyzer, "_analyze_tcp_options"
        ) as mock_options:
    ...
    async def test_tcp_analyzer_error_handling(self):
        ...
        with patch.object(
            analyzer, "_resolve_target", side_effect=Exception("DNS failed")
        ):
    ...
    def test_tcp_analysis_result_serialization(self):
        ...
        expected_fields = [
            ...
            "window_size_variations",
            "window_scaling_blocked",
            "sequence_number_anomalies",
            "seq_prediction_difficulty",
            "ack_number_manipulation",
