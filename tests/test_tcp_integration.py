...
from recon.tests.tcp_analyzer import TCPAnalyzer
...
class TestTCPAnalyzerIntegration:
    ...
    async def test_tcp_analyzer_basic_functionality(self):
        ...
        with patch.object(analyzer, '_resolve_target', return_value='127.0.0.1') as mock_resolve, patch.object(analyzer, '_analyze_basic_connections') as mock_basic, patch.object(analyzer, '_analyze_rst_injection') as mock_rst, patch.object(analyzer, '_analyze_window_manipulation') as mock_window, patch.object(analyzer, '_analyze_sequence_numbers') as mock_seq, patch.object(analyzer, '_analyze_fragmentation_handling') as mock_frag, patch.object(analyzer, '_analyze_tcp_options') as mock_options:
    ...
    def test_tcp_analysis_result_serialization(self):
        ...
        from recon.tests.tcp_analyzer import TCPAnalysisResult
        ...
        expected_fields = ['target', 'timestamp', 'rst_injection_detected', 'rst_source_analysis', 'rst_timing_patterns', 'rst_ttl_analysis', 'tcp_window_manipulation', 'window_size_variations', 'window_scaling_blocked', 'sequence_number_anomalies', 'seq_prediction_difficulty', 'ack_number_manipulation', 'fragmentation_handling', 'mss_clamping_detected', 'fragment_timeout_ms', 'tcp_options_filtering', 'tcp_timestamp_manipulation', 'connection_state_tracking', 'syn_flood_protection', 'reliability_score', 'analysis_errors']
