...
def test_problematic_strategy():
    ...
    try:
        ...
        if normalized.type == "fakeddisorder":
            ...
            print(
                f"❌ ОШИБКА: fake,fakeddisorder парсится как {normalized.type}, ожидался fakeddisorder"
            )
            return False
...
def test_various_fake_combinations():
    ...
    try:
        ...
        test_cases = [
            ...
            (
                "--dpi-desync=fake,multidisorder --dpi-desync-split-pos=3 --dpi-desync-ttl=3",
                "fakeddisorder",
            ),
            (
                "--dpi-desync=fake,fakeddisorder --dpi-desync-split-pos=3 --dpi-desync-ttl=3",
                "fakeddisorder",
            ),
            # fake + non-disorder -> fake
            (
                "--dpi-desync=fake,split --dpi-desync-split-pos=3 --dpi-desync-ttl=3",
                "fake",
            ),
            (
                "--dpi-desync=fake,seqovl --dpi-desync-split-pos=3 --dpi-desync-ttl=3 --dpi-desync-split-seqovl=20",
                "fake",
            ),
            # Порядок не важен
            (
                "--dpi-desync=disorder,fake --dpi-desync-split-pos=3 --dpi-desync-ttl=3",
                "fakeddisorder",
            ),
            (
                "--dpi-desync=disorder2,fake --dpi-desync-split-pos=3 --dpi-desync-ttl=3",
                "fakeddisorder",
            ),
        ]
