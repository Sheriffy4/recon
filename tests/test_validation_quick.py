...
def test_validation_integration():
    ...
    try:
        ...
        result = registry.validate_parameters(
            "fakeddisorder", {"split_pos": 3, "ttl": 300}
        )
        print(
            f"   ❌ Invalid TTL validation: {result.is_valid} - {result.error_message}"
        )

        # Test valid parameters
        result = registry.validate_parameters(
            "fakeddisorder", {"split_pos": 3, "ttl": 4}
        )
        ...
        corrected = searcher._validate_attack_parameters(
            "fakeddisorder", {"split_pos": 3, "ttl": 300}
        )
        ...
        strategy = loader.load_strategy(
            {"type": "fakeddisorder", "params": {"split_pos": 3, "ttl": 4}}
        )
        ...
        result = registry.validate_parameters(
            "multisplit", {"positions": [1, "invalid", 5]}
        )
