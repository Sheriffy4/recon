...
def test_capability_detector():
    ...
    assert recommended in ["native", "emulated", "compatibility"], f"Invalid recommended mode: {recommended}"
...
def test_mode_controller():
    ...
    # Test initialization
    assert controller.get_current_mode() == OperationMode.AUTO, "Should start in AUTO mode"
    ...
    assert OperationMode.COMPATIBILITY in available_modes, "Compatibility mode should always be available"
    ...
    try:
        ...
        assert controller.get_current_mode() == OperationMode.COMPATIBILITY, "Should be in compatibility mode"
