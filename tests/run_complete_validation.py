...
def run_complete_validation(target_domain: str = "youtube.com",
    ...
    try:
        ...
        logger.info(f"✅ End-to-end tests completed. Results saved to {end_to_end_file}")
        ...
        if not pcap_files:
            logger.warning("No PCAP files found from end-to-end tests. Creating mock analysis.")
            ...
            for pcap_file in pcap_files:
                ...
                try:
                    analysis_result = pcap_analyzer.analyze_pcap_comprehensive(pcap_file, expected_strategies)
                    ...
                    workflow_results['errors'].append(f"PCAP analysis failed for {pcap_file}: {e}")
...
def print_workflow_summary(results: Dict[str, Any]) -> None:
    ...
    if results['pcap_analysis_results']:
        ...
        total_packets = sum(r.get('total_packets', 0) for r in results['pcap_analysis_results'])
        avg_effectiveness = sum(r.get('effectiveness_score', 0) for r in results['pcap_analysis_results']) / max(pcap_count, 1)
    ...
    if results['final_report']:
        ...
        if 'validation_summary' in report:
            ...
            print(f"  Effectiveness Score: {summary.get('effectiveness_score', 0):.2f}/1.00")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run complete DPI strategy validation workflow")
    parser.add_argument("--domain", default="youtube.com", help="Target domain to test")
    parser.add_argument("--duration", type=int, default=30, help="Capture duration per test (seconds)")
    parser.add_argument("--output-dir", default="complete_validation_results", help="Output directory")
