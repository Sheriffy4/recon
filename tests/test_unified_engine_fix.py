    ...
    try:
        ...
        if engine_task:
            ...
            checks = [
                ...
                (
                    "fooling сохранен",
                    "badsum" in params.get("fooling", [])
                    and "badseq" in params.get("fooling", []),
                ),
                ("forced = True", engine_task.get("forced") == True),
                ("no_fallbacks = True", engine_task.get("no_fallbacks") == True),
            ]
...
def test_strategy_comparison():
    ...
    try:
        ...
        if service_task and test_task:
            ...
            if (
                "split_seqovl" not in service_params
                and "split_seqovl" not in test_params
            ):
                print("    ✅ split_seqovl удален из обеих стратегий")
            else:
                print(
                    f"    ❌ split_seqovl найден: служба={service_params.get('split_seqovl')}, тест={test_params.get('split_seqovl')}"
                )
                all_match = False
...
def main():
    ...
    results.append(
        (
            "_ensure_engine_task with fakeddisorder",
            test_ensure_engine_task_with_fakeddisorder(),
        )
    )
    ...
    if passed == total:
        print(
            "🎉 ВСЕ ТЕСТЫ ПРОШЛИ! unified_bypass_engine теперь применяет совместимость."
        )
        return True
    else:
