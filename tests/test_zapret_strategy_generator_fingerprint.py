...
class TestZapretStrategyGeneratorFingerprint(unittest.TestCase):
    ...
    def test_generate_strategies_with_fingerprint(self):
        ...
        strategies = self.generator.generate_strategies(
            fingerprint=self.roskomnadzor_tspu_fingerprint,
            count=10
        )
    ...
    def test_generate_strategies_without_fingerprint(self):
        ...
        proven_found = any(strategy in self.generator.PROVEN_WORKING for strategy in strategies)
    ...
    def test_dpi_type_specific_strategies(self):
        ...
        commercial_strategies = self.generator.generate_strategies(
            fingerprint=self.commercial_dpi_fingerprint,
            count=20
        )
        
        government_strategies = self.generator.generate_strategies(
            fingerprint=self.government_censorship_fingerprint,
            count=20
        )
    ...
    def test_get_dpi_type_strategies(self):
        ...
        tspu_strategies = self.generator._get_dpi_type_strategies(DPIType.ROSKOMNADZOR_TSPU)
        self.assertIsInstance(tspu_strategies, list)
        self.assertGreater(len(tspu_strategies), 0)
        
        # Test COMMERCIAL_DPI strategies
        commercial_strategies = self.generator._get_dpi_type_strategies(DPIType.COMMERCIAL_DPI)
        ...
        gov_strategies = self.generator._get_dpi_type_strategies(DPIType.GOVERNMENT_CENSORSHIP)
    ...
    def test_characteristic_based_strategies(self):
        ...
        low_ttl_found = any('--dpi-desync-ttl=1' in strategy or '--dpi-desync-ttl=2' in strategy 
                           for strategy in rst_strategies)
        self.assertTrue(low_ttl_found, "Should include low TTL strategies for RST injection")
        
        # Test deep content inspection characteristic
        deep_inspection_strategies = self.generator._get_characteristic_based_strategies(
            self.commercial_dpi_fingerprint
        )
        
        # Should contain aggressive segmentation for deep inspection
        multisplit_found = any('multisplit' in strategy for strategy in deep_inspection_strategies)
        self.assertTrue(multisplit_found, "Should include multisplit for deep content inspection")
    ...
    def test_confidence_based_ranking(self):
        ...
        low_conf_strategies = self.generator.generate_strategies(
            fingerprint=self.low_confidence_fingerprint,
            count=10
        )
    ...
    def test_strategy_complexity_calculation(self):
        ...
        simple_complexity = self.generator._calculate_strategy_complexity(simple_strategy)
        complex_complexity = self.generator._calculate_strategy_complexity(complex_strategy)
    ...
    def test_strategy_aggressiveness_calculation(self):
        ...
        mild_aggressiveness = self.generator._calculate_strategy_aggressiveness(mild_strategy)
        aggressive_aggressiveness = self.generator._calculate_strategy_aggressiveness(aggressive_strategy)
    ...
    def test_fingerprint_aware_vs_generic_strategies(self):
        ...
        self.assertLess(len(overlap), min(len(fingerprint_strategies), len(generic_strategies)))
    ...
    def test_government_censorship_specific_strategies(self):
        ...
        very_low_ttl_found = any('--dpi-desync-ttl=1' in strategy for strategy in strategies)
        high_repeats_found = any('--dpi-desync-repeats=' in strategy and 
                                int(strategy.split('--dpi-desync-repeats=')[1].split()[0]) >= 3 
                                for strategy in strategies if '--dpi-desync-repeats=' in strategy)
        
        self.assertTrue(very_low_ttl_found, "Should include very low TTL for government censorship")
        self.assertTrue(high_repeats_found, "Should include high repeats for government censorship")
    ...
    def test_commercial_dpi_specific_strategies(self):
        ...
        self.assertTrue(advanced_fooling_found or high_ttl_found, 
    ...
    def test_packet_size_limitation_strategies(self):
        ...
        fine_segmentation_found = any('--dpi-desync-split-count=6' in strategy or
                                    '--dpi-desync-split-pos=1,2,3,4,5' in strategy
                                    for strategy in strategies)
        
        self.assertTrue(fine_segmentation_found, 
    ...
    def test_dns_blocking_strategies(self):
        ...
        aggressive_found = any('multidisorder' in strategy or 'multisplit' in strategy 
                             for strategy in strategies)
        
        self.assertTrue(aggressive_found, "Should include aggressive strategies for DNS blocking")
    
    def test_empty_fingerprint_fallback(self):
        """Test behavior with minimal fingerprint data."""
        minimal_fingerprint = DPIFingerprint(
            target="minimal-site.com",
            dpi_type=DPIType.UNKNOWN,
            confidence=0.0
        )
    ...
    def test_strategy_count_limits(self):
        ...
        for count in [5, 10, 15, 25, 50]:
            ...
            self.assertEqual(len(strategies), count, f"Should generate exactly {count} strategies")
    ...
    def test_strategy_uniqueness(self):
        ...
        self.assertEqual(len(strategies), len(unique_strategies), 
...
class TestZapretStrategyGeneratorIntegration(unittest.TestCase):
    ...
    def test_full_workflow_high_confidence(self):
        ...
        strategies = self.generator.generate_strategies(fingerprint=fingerprint, count=20)
        ...
        for strategy in strategies[:5]:  # Check top 5 strategies
            if any(keyword in strategy for keyword in ['fake', 'multidisorder', 'badsum', 'badseq']):
                relevant_strategies += 1
        
        self.assertGreaterEqual(relevant_strategies, 3, 
    ...
    def test_full_workflow_low_confidence(self):
        ...
        strategies = self.generator.generate_strategies(fingerprint=fingerprint, count=15)
        ...
        proven_included = any(strategy in self.generator.PROVEN_WORKING for strategy in strategies)
        self.assertTrue(proven_included, "Should include proven strategies for low confidence")
    ...
    def test_performance_with_large_strategy_count(self):
        ...
        strategies = self.generator.generate_strategies(fingerprint=fingerprint, count=100)
        
        end_time = time.time()
        execution_time = end_time - start_time
        
        # Should complete within reasonable time (less than 5 seconds)
        self.assertLess(execution_time, 5.0, "Should generate 100 strategies within 5 seconds")
