            ...
            if strategy_str:
                try:
                    ...
                    if normalized.type == expected_type:
                        print(
                            f"✅ {domain}: {strategy_str[:50]}... → {normalized.type}"
                        )
                    else:
                        print(
                            f"❌ {domain}: {strategy_str[:50]}... → {normalized.type} (ожидался {expected_type})"
                        )
                        all_correct = False
...
def test_service_initialization():
    ...
    try:
        ...
        for domain, strategy in service.domain_strategies.items():
            if "fake" in strategy and (
                "disorder" in strategy or "disorder2" in strategy
            ):
