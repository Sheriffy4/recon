...
def test_current_dispatch_problem():
    ...
    test_cases = [
        ...
        (
            "seqovl",
            {"split_pos": 5, "overlap_size": 20},
        ),  # ❌ overlap_size игнорируется!
...
def test_proposed_dispatch_fix():
    ...
    if all_correct:
        print(
            "  ✅ ИСПРАВЛЕНО: Каждая атака использует свой метод с правильными параметрами!"
        )
        return True
    else:
...
def test_parameter_extraction():
    ...
    for attack_type, raw_params, expected_param in test_cases:
        ...
        if expected_param and expected_param in result_params:
            print(
                f"    ✅ {attack_type}: {expected_param} = {result_params[expected_param]}"
            )
...
def main():
    ...
    if passed >= 3:  # Первый тест должен провалиться (демонстрация проблемы)
        ...
        print("⚠️ Нужно доработать логику исправления.")
        return False
