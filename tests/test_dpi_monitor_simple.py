# recon/core/fingerprint/test_dpi_monitor_simple.py
"""
...
from .dpi_behavior_monitor import (
    MonitoringConfig, PerformanceMonitor, BehaviorAnalyzer,
    AlertSeverity, MonitoringState, BehaviorChange, MonitoringAlert
)
...
def test_performance_monitor_basic():
    ...
    # Test adaptive interval with no load
    interval = monitor.get_adaptive_interval(60, 30, 300, 80.0, 85.0)
    assert interval == 60  # Should return base interval when not overloaded
...
def test_behavior_analyzer_known_patterns():
    ...
    # Check that known patterns contain expected elements
...
def test_behavior_analyzer_extract_signature():
    ...
    fingerprint.content_inspection_depth = 1500  # > 1000
    fingerprint.connection_reset_timing = 50.0  # < 100
