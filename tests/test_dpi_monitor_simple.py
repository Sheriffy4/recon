...
from recon.tests.dpi_behavior_monitor import MonitoringConfig, PerformanceMonitor, BehaviorAnalyzer, AlertSeverity, MonitoringState, BehaviorChange, MonitoringAlert
...
def test_monitoring_config_custom():
    ...
    config = MonitoringConfig(check_interval_seconds=120, enable_alerts=False, alert_retention_days=14)
...
def test_behavior_change_creation():
    ...
    change = BehaviorChange(target='example.com:443', timestamp=datetime.now(), change_type='new_blocking', old_fingerprint=None, new_fingerprint=fingerprint, confidence=0.8, details={'test': 'data'})
...
def test_monitoring_alert_creation():
    ...
    fingerprint = Mock()
    alert = MonitoringAlert(id='test123', target='example.com:443', timestamp=datetime.now(), severity=AlertSeverity.HIGH, title='Test Alert', description='Test description', fingerprint=fingerprint, suggested_actions=['action1', 'action2'])
...
def test_behavior_change_to_dict():
    ...
    change = BehaviorChange(target='example.com:443', timestamp=timestamp, change_type='test_change', old_fingerprint=None, new_fingerprint=fingerprint, confidence=0.7, details={'key': 'value'})
    data = change.to_dict()
...
def test_monitoring_alert_to_dict():
    ...
    fingerprint.to_dict.return_value = {'test': 'data'}
    timestamp = datetime.now()
    alert = MonitoringAlert(id='test123', target='example.com:443', timestamp=timestamp, severity=AlertSeverity.MEDIUM, title='Test Alert', description='Test description', fingerprint=fingerprint, suggested_actions=['action1'], acknowledged=True, resolved=False)
    data = alert.to_dict()
...
def test_behavior_analyzer_extract_signature():
    ...
    expected_elements = {'rst_injection_detected', 'http_header_filtering', 'dns_hijacking_detected', 'content_inspection_depth_high', 'fast_connection_reset'}
