...
def dpi_config_basic():
    ...
    return DPIConfig(
        desync_mode="split", split_positions=[3, 10], fooling_methods=[], enabled=True
    )
...
def dpi_config_sni_only():
    ...
    return DPIConfig(
        desync_mode="split", split_positions=["sni"], fooling_methods=[], enabled=True
    )
...
def dpi_config_disabled():
    ...
    return DPIConfig(
        desync_mode="split", split_positions=[3, 10], fooling_methods=[], enabled=False
    )
...
def sample_packets():
    ...
    def create_tcp_packet(
        payload: bytes, src_port: int = 54321, dst_port: int = 443
    ) -> bytes:
        ...
        tcp_header = struct.pack(
            "!HHIIBBHHH", src_port, dst_port, 1000, 2000, 0x50, 0x18, 65535, 0x1234, 0
        )
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "pcap: mark test as PCAP validation test")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if (
            "test_position_resolver" in item.nodeid
            or "test_sni_detector" in item.nodeid
            or "test_checksum_fooler" in item.nodeid
            or "test_dpi_strategy_engine" in item.nodeid
        ):
