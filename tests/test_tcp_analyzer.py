...
from .tcp_analyzer import (
    TCPAnalyzer, TCPAnalysisResult, TCPConnectionAttempt, 
    RSTSource, NetworkAnalysisError
)
...
class TestTCPAnalyzer:
    ...
    async def test_basic_connections_analysis_success(self, tcp_analyzer):
        ...
        with patch('asyncio.open_connection') as mock_open_conn:
            ...
            assert not result.rst_injection_detected  # Should be False for successful connections
    ...
    async def test_basic_connections_analysis_timeouts(self, tcp_analyzer):
        ...
        with patch('asyncio.open_connection') as mock_open_conn:
            ...
            assert result.connection_state_tracking  # High timeout rate indicates state tracking
    ...
    async def test_window_manipulation_analysis(self, tcp_analyzer):
        ...
        for i, window_size in enumerate([1024, 8192, 16384, 32768, 65535]):
            mock_sock = MockSocket(should_fail=(i % 2 == 1))  # Fail every other connection
    ...
    async def test_fragmentation_analysis_with_scapy(self, tcp_analyzer, mock_scapy_packet):
        ...
        # Mock Scapy functions
        with patch('recon.core.fingerprint.tcp_analyzer.send') as mock_send, \
             patch('recon.core.fingerprint.tcp_analyzer.sr1') as mock_sr1:
    ...
    async def test_mss_clamping_detection(self, tcp_analyzer, mock_scapy_packet):
        ...
        with patch('recon.core.fingerprint.tcp_analyzer.sr1') as mock_sr1:
            ...
            mock_tcp = Mock()
            mock_tcp.options = [('MSS', 1460)]  # Clamped to 1460 even for larger requests
            mock_scapy_packet.__getitem__.return_value = mock_tcp
            mock_sr1.return_value = mock_scapy_packet
    ...
    async def test_syn_flood_protection_detection(self, tcp_analyzer, mock_scapy_packet):
        ...
        with patch('recon.core.fingerprint.tcp_analyzer.sr1') as mock_sr1:
            ...
            responses = [mock_scapy_packet, mock_scapy_packet, None, None, None, 
    ...
    def test_reliability_score_calculation(self, tcp_analyzer):
        ...
        result.connection_attempts = [
            TCPConnectionAttempt(time.time(), "192.168.1.1", 443, 12345, 1000, 0, 65535, success=True),
            TCPConnectionAttempt(time.time(), "192.168.1.1", 443, 12346, 1001, 0, 65535, success=True),
            TCPConnectionAttempt(time.time(), "192.168.1.1", 443, 12347, 1002, 0, 65535, success=False),
        ]
    ...
    async def test_full_tcp_analysis_success(self, tcp_analyzer):
        ...
        with patch.object(tcp_analyzer, '_resolve_target', return_value="192.168.1.1"):
            ...
            with patch.object(tcp_analyzer, '_analyze_basic_connections') as mock_basic, \
                 patch.object(tcp_analyzer, '_analyze_rst_injection') as mock_rst, \
                 patch.object(tcp_analyzer, '_analyze_window_manipulation') as mock_window, \
                 patch.object(tcp_analyzer, '_analyze_sequence_numbers') as mock_seq, \
                 patch.object(tcp_analyzer, '_analyze_fragmentation_handling') as mock_frag, \
                 patch.object(tcp_analyzer, '_analyze_tcp_options') as mock_options:
    ...
    async def test_tcp_analysis_partial_failure(self, tcp_analyzer):
        ...
        with patch.object(tcp_analyzer, '_resolve_target', return_value="192.168.1.1"):
            with patch.object(tcp_analyzer, '_analyze_basic_connections') as mock_basic, \
                 patch.object(tcp_analyzer, '_analyze_rst_injection', side_effect=Exception("RST analysis failed")) as mock_rst, \
                 patch.object(tcp_analyzer, '_analyze_window_manipulation') as mock_window, \
                 patch.object(tcp_analyzer, '_analyze_sequence_numbers') as mock_seq, \
                 patch.object(tcp_analyzer, '_analyze_fragmentation_handling') as mock_frag, \
                 patch.object(tcp_analyzer, '_analyze_tcp_options') as mock_options:
