...
def test_multidisorder_with_split_pos():
    ...
    try:
        ...
        recipe = dispatcher.dispatch_attack(
            "multidisorder", params, test_payload, packet_info
        )
...
def test_multidisorder_with_positions():
    ...
    try:
        ...
        params = {"positions": [5, 10, 15], "fake_ttl": 3, "fooling": ["badseq"]}
        ...
        if recipe:
            logger.info(
                f"✅ multidisorder с positions: {len(recipe)} сегментов сгенерировано"
            )
            return True
        else:
...
def main():
    ...
    tests = [test_multidisorder_with_split_pos, test_multidisorder_with_positions]
