...
class PacketCapture:
    ...
    def get_connection(
        self, src_addr: str, src_port: int, dst_addr: str, dst_port: int
    ...
    def _update_connection_stats(
        self, ip_packet: IPv4Packet, transport_packet: any, protocol: str
    ):
    ...
    def _process_packets(self):
        ...
        while self.running:
            ...
            try:
                ...
                if ip_packet.protocol == 6:  # TCP
                    tcp_packet = TCPPacket.parse(ip_packet.payload)
                    modified = self.packet_engine.process_tcp_packet(
                    ...
                    if udp_packet.dst_port in [443, 80]:
                        try:
                            quic_packet = self.packet_engine.parse_quic_packet(
                                udp_packet.payload
                            )
                            if quic_packet:
                                modified = self.packet_engine.process_quic_initial(
                                    quic_packet
                                )
                        except:
                            pass
