...
def test_basic_functionality():
    ...
    print(f"✓ Created online learning system")
    ...
    print(f"✓ High confidence correct prediction: {'learned' if result1 else 'skipped'} (expected: skipped)")
    ...
    print(f"✓ Incorrect prediction: {'learned' if result2 else 'skipped'} (expected: learned)")
    ...
    print(f"✓ Low confidence prediction: {'learned' if result3 else 'skipped'} (expected: skipped)")
...
def test_learning_modes():
    ...
    for mode in [LearningMode.CONSERVATIVE, LearningMode.MODERATE, LearningMode.AGGRESSIVE]:
        online_learning = OnlineLearningSystem(
            ml_classifier=mock_classifier,
            learning_mode=mode,
            buffer_size=10
...
def test_ab_testing():
    ...
    with unittest.mock.patch('online_learning.MLClassifier', return_value=test_classifier):
        config = ABTestConfig(
        ...
        if success:
            ...
            dpi_type, confidence, model_used = online_learning.classify_with_ab_test(sample_metrics)
            print(f"  Classification result: {dpi_type} (confidence: {confidence:.2f}, model: {model_used})")
