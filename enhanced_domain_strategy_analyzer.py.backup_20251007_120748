...
class EnhancedDomainStrategyAnalyzer:
    def __init__(self, pcap_file="work.pcap", report_file=None):
        ...
        self.domain_success_rates = defaultdict(lambda: {'attempts': 0, 'successes': 0, 'failures': 0})
        ...
        self.strategy_effectiveness = defaultdict(lambda: {'total': 0, 'success': 0, 'domains': set()})
    ...
    def _load_target_domains(self) -> Set[str]:
        ...
        if sites_file.exists():
            ...
            if not domains:
                print(f"⚠️ Warning: Could not load sites.txt with any encoding")
        ...
        try:
            ...
            if eth_type == 0x0800 and len(packet_data) >= 34:
                ...
                if protocol == 6 and len(packet_data) >= 14 + ihl + 20:
                    tcp_start = 14 + ihl
                    tcp_header = struct.unpack('!HHLLBBHHH', packet_data[tcp_start:tcp_start+20])
                    ...
                    if dst_port == 443 or src_port == 443:
                        self._analyze_tls_packet(packet_data, src_ip, dst_ip, src_port, dst_port, timestamp, flags)
    ...
    def _analyze_tls_packet(self, packet_data, src_ip, dst_ip, src_port, dst_port, timestamp, tcp_flags):
        ...
        try:
            ...
            if (packet_data[tls_start] == 0x16 and  # Handshake
                len(packet_data) > tls_start + 5 and 
            ...
            elif packet_data[tls_start] == 0x16 and len(packet_data) > tls_start + 5 and packet_data[tls_start + 5] == 0x02:  # ServerHello
    ...
    def _is_target_domain(self, domain):
        ...
        interesting_domains = {
            'instagram.com', 'x.com', 'twitter.com', 'youtube.com', 'facebook.com',
            'rutracker.org', 'nnmclub.to', 'telegram.org'
        }
    ...
    def _track_connection_success(self, dst_ip, src_ip, timestamp):
        ...
        for handshake in reversed(self.tls_handshakes):
            if (handshake['dst_ip'] == src_ip and 
                handshake['src_ip'] == dst_ip and 
                abs(handshake['timestamp'] - timestamp) < 10):
    ...
    def _generate_enhanced_report(self):
        ...
        for domain in sorted(self.domain_connections.keys()):
            ...
            success_pct = min(100.0, (successes / max(total_attempts, 1) * 100)) if total_attempts > 0 else 0
            ...
            print(f"  {status_icon} {domain:<40} | {total_attempts:>3} attempts | {successes:>3} success | {success_pct:>5.1f}% | {status}")
            if strategies:
    ...
    def _generate_strategy_recommendations(self):
        ...
        for domain, connections in self.domain_connections.items():
            ...
            total_attempts = len(connections)
            successes = success_rate['successes']
            success_pct = (successes / total_attempts * 100) if total_attempts > 0 else 0
        ...
        for domain in high_success_domains:
            strategies = list(self.domain_strategies[domain])
            print(f"  ✅ {domain} - Patterns: {', '.join(strategies) if strategies else 'Standard'}")
    ...
    def _generate_strategies_json_recommendation(self):
        ...
        print(f"📊 CLI Discovery Results Summary:")
        print(f"  • Best Strategy: {strategy_str}")
        print(f"  • Success Rate: {successful_sites}/{total_sites} ({successful_sites/total_sites*100:.1f}%)")
        print(f"  • Total Strategies Tested: {len(all_results)}")
        print(f"  • Working Strategies Found: {len([r for r in all_results if r.get('successful_sites', 0) > 0])}")
        ...
        if pcap_domains:
            ...
            successful_domains = list(target_domains)[:successful_sites] if successful_sites > 0 else []
            failed_domains = list(target_domains)[successful_sites:] if successful_sites < len(target_domains) else []
        ...
        if failed_domains and len(all_results) > 1:
            ...
            alt_strategy = self._convert_to_zapret_format(second_best.get('strategy', ''))
        ...
        if failed_domains and len(all_results) > 1:
            alt_strategies = [self._convert_to_zapret_format(r.get('strategy', '')) for r in all_results[1:4]]  # Top 3 alternatives
        ...
        recommendation = {
            ...
            'strategy_options': {
                'option_1_best_for_successful': {domain: zapret_strategy for domain in successful_domains},
                'option_3_comprehensive': comprehensive_strategies
            },
            'zapret_formats': {
                'best_strategy': zapret_strategy,
                'alternatives': [self._convert_to_zapret_format(r.get('strategy', '')) for r in all_results[1:4]]
            }
        }
        ...
        print(f"   2. Merge with existing strategies.json")
        print(f"   3. Test with recon_service.py")
    ...
    def _save_detailed_analysis(self, domain_results, missing_domains):
        ...
        output_file = f"enhanced_domain_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
