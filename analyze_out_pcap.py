        ...
        if IP in packet:
            ...
            if TCP in packet:
                ...
                if hasattr(packet, 'load') and packet.load:
                    ...
                    if len(packet.load) > 5 and packet.load[0] in [0x16, 0x14, 0x15, 0x17]:
                        stats['tls_packets'] += 1
                        ...
                        if packet.load[0] == 0x15:  # Alert
                            stats['handshake_failures'].append({
                                ...
                                'dst': packet[IP].dst,
                                'dport': tcp_layer.dport,
                                'alert_level': packet.load[1] if len(packet.load) > 1 else 0,
                                'alert_description': packet.load[2] if len(packet.load) > 2 else 0
                            })
                ...
                if DNS in packet:
                    ...
                    if dns_layer.qr == 0:  # Query
                        if dns_layer.qd:
                            ...
                            if any(blocked in domain for blocked in ['x.com', 'instagram.com', 'youtube.com', 'facebook.com']):
...
def generate_report(stats):
    ...
    if stats['rst_packets']:
        ...
        for rst in stats['rst_packets'][:10]:
            print(f"  {rst['src']}:{rst['sport']} -> {rst['dst']}:{rst['dport']} (seq: {rst['seq']})")
    
    if stats['handshake_failures']:
        print(f"\nTLS HANDSHAKE FAILURES (first 10):")
        for failure in stats['handshake_failures'][:10]:
            print(f"  {failure['src']} -> {failure['dst']}:{failure['dport']} Alert: {failure['alert_level']}.{failure['alert_description']}")
    ...
    for dest, attempts in sorted(stats['connection_attempts'].items(), key=lambda x: x[1], reverse=True)[:15]:
    ...
    if len(stats['rst_packets']) > 0:
        print(f"⚠️  High number of RST packets ({len(stats['rst_packets'])}) indicates connection resets")
    ...
    if stats['dns_packets'] == 0:
        print(f"⚠️  No DNS packets detected - may indicate DNS blocking or alternative resolution")
    ...
    if syn_count > 0:
        ...
        print(f"📊 Connection success ratio: {success_ratio:.2%} ({syn_ack_count}/{syn_count})")
        if success_ratio < 0.5:
            print(f"⚠️  Low connection success rate indicates blocking")
...
def main():
    ...
    if stats:
        ...
        with open('out_pcap_analysis.json', 'w') as f:
            ...
            json_stats['protocol_distribution'] = dict(json_stats['protocol_distribution'])
