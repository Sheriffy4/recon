...
class MigrationValidationTool:
    ...
    def validate_single_attack(
        self, attack_class: type, reference_attack_class: Optional[type] = None
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            if reference_attack_class:
                regression_score, regression_issues = self._test_regression(
                    attack, reference_attack_class
                )
            ...
            overall_functionality = (
                functionality_score + interface_score + test_results["regression"]
            ) / 3
            ...
            validation_passed = (
                ...
                and functionality_score >= 0.8
                and interface_score >= 0.9
            )
    ...
    def _test_compatibility(self, attack: BaseAttack) -> Tuple[float, List[str]]:
        ...
        try:
            ...
            try:
                ...
                result = self.compatibility_manager.execute_with_fallback(
                    attack, test_context
                )
                ...
                if result.status == AttackStatus.SUCCESS:
                    ...
                    issues.append(
                        f"Compatibility execution failed: {result.error_message}"
                    )
                    score = min(score, 0.5)
                        ...
                        for j, segment in enumerate(result._segments):
                            if not isinstance(segment, tuple) or len(segment) != 3:
                                ...
                                if not isinstance(payload, bytes):
                                    issues.append(
                                        f"Test {i+1}, Segment {j+1}: Payload not bytes"
                                    )
                                if not isinstance(offset, int) or offset < 0:
                                    issues.append(
                                        f"Test {i+1}, Segment {j+1}: Invalid offset"
                                    )
                                if not isinstance(options, dict):
                                    issues.append(
                                        f"Test {i+1}, Segment {j+1}: Options not dict"
                                    )
                else:
                    issues.append(
                        f"Test {i+1}: Execution failed - {result.error_message}"
                    )
        ...
        try:
            ...
            if times:
                ...
                metrics["execution_consistency"] = (
                    1.0 - (max_time - min_time) / avg_time if avg_time > 0 else 0.0
                )
        ...
        try:
            ...
            if hasattr(attack, "estimate_effectiveness"):
                ...
                if (
                    ...
                    and 0.0 <= effectiveness <= 1.0
                ):
                    score += 0.1
                else:
                    issues.append(
                        "estimate_effectiveness should return float between 0.0 and 1.0"
                    )
    ...
    def _test_regression(
        self, migrated_attack: BaseAttack, reference_attack: type
        ...
        try:
            ...
            for context in self.test_contexts[:3]:  # Test subset
                try:
                    ...
                    reference_eff = (
                        reference.estimate_effectiveness(context)
                        if hasattr(reference, "estimate_effectiveness")
                        else 0.5
                    )

                    effectiveness_scores.append(
                        migrated_eff / reference_eff if reference_eff > 0 else 1.0
                    )

                except Exception:
                    effectiveness_scores.append(0.5)  # Neutral score on error

            avg_effectiveness_ratio = sum(effectiveness_scores) / len(
                effectiveness_scores
            )

            if avg_effectiveness_ratio < 0.8:
                issues.append(
                    f"Effectiveness regression: {avg_effectiveness_ratio:.2f} of reference"
                )
            ...
            success_ratio = (
                migrated_successes / reference_successes
                if reference_successes > 0
                else 1.0
            )

            if success_ratio < 1.0:
                issues.append(
                    f"Success rate regression: {success_ratio:.2f} of reference"
                )
    ...
    def _generate_recommendations(
        self, test_results: Dict[str, Any], issues: List[str]
    ) -> List[str]:
        ...
        if test_results.get("performance", 0) < 0.7:
            ...
            recommendations.append(
                "Consider reducing segment count or payload processing"
            )

        # Functionality recommendations
        if test_results.get("functionality", 0) < 0.8:
            recommendations.append(
                "Fix functionality issues across different payload types"
            )
        ...
        if test_results.get("regression", 1.0) < 0.8:
            recommendations.append(
                "Address regression issues compared to reference implementation"
            )
            recommendations.append(
                "Ensure migrated attack maintains or improves effectiveness"
            )
    ...
    def validate_multiple_attacks(
        self, attack_classes: List[type], reference_classes: Optional[List[type]] = None
        ...
        for i, attack_class in enumerate(attack_classes):
            reference_class = (
                reference_classes[i]
                if reference_classes and i < len(reference_classes)
                else None
            )
    ...
    def generate_report(
        ...
        if not output_file:
            ...
            output_file = (
                self.output_dir / f"migration_validation_report_{timestamp}.json"
            )
        ...
        avg_compatibility = (
            sum(r.compatibility_score for r in results) / total_attacks
            if total_attacks > 0
            else 0
        )
        avg_performance = (
            sum(r.performance_score for r in results) / total_attacks
            if total_attacks > 0
            else 0
        )
        avg_functionality = (
            sum(r.functionality_score for r in results) / total_attacks
            if total_attacks > 0
            else 0
        )
    ...
    def print_summary(self, results: List[ValidationResult]):
        ...
        for result in results:
            ...
            print(
                f"  {result.attack_name}: {status} (C:{result.compatibility_score:.2f} P:{result.performance_score:.2f} F:{result.functionality_score:.2f})"
            )
...
def main():
    ...
    parser.add_argument(
        "--attack-file", type=Path, help="Python file containing attack class"
    )
    parser.add_argument(
        "--attack-class", type=str, help="Attack class name to validate"
    )
    parser.add_argument(
        "--reference-file",
        type=Path,
        help="Python file containing reference attack class",
    )
    parser.add_argument(
        "--reference-class", type=str, help="Reference attack class name"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if args.attack_file and args.attack_class:
        ...
        if args.reference_file and args.reference_class:
            reference_class = load_attack_class_from_file(
                args.reference_file, args.reference_class
            )
        ...
        print(
            "Usage: python migration_validation_tool.py --attack-file <file> --attack-class <class>"
        )
        print(
            "Example: python migration_validation_tool.py --attack-file my_attack.py --attack-class MyAttack"
        )
        sys.exit(1)
