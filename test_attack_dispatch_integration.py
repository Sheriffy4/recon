...
def test_attack_registry():
    ...
    try:
        ...
        expected_attacks = [
            "fakeddisorder", "seqovl", "multidisorder", 
            "disorder", "disorder2", "multisplit", "split", "fake"
        ]
            ...
            try:
                ...
                for i, segment in enumerate(recipe):
                    if not isinstance(segment, tuple) or len(segment) != 3:
                        logger.error(f"❌ {test_case['name']}: неправильная структура сегмента {i}")
                        break
                    
                    data, offset, options = segment
                    if not isinstance(data, bytes):
                        logger.error(f"❌ {test_case['name']}: данные сегмента {i} не bytes")
                        break
                    
                    if not isinstance(offset, int):
                        logger.error(f"❌ {test_case['name']}: смещение сегмента {i} не int")
                        break
                    
                    if not isinstance(options, dict):
                        logger.error(f"❌ {test_case['name']}: опции сегмента {i} не dict")
                        break
                else:
                    logger.info(f"✅ {test_case['name']}: {len(recipe)} сегментов сгенерировано")
...
def test_parameter_validation():
    ...
    try:
        ...
        validation_tests = [
            ...
            ('fakeddisorder', {'split_pos': 'invalid'}, False),  # Неправильный split_pos
            ('seqovl', {'split_pos': 10, 'overlap_size': -1}, False),  # Отрицательный overlap_size
        ]
        ...
        for attack_type, params, should_be_valid in validation_tests:
            ...
            if result.is_valid == should_be_valid:
                ...
                logger.error(f"❌ {attack_type} с параметрами {params}: ожидалось {should_be_valid}, получено {result.is_valid}")
    ...
    try:
        ...
        for special_value in special_values:
            try:
                ...
                if recipe:
                    logger.info(f"✅ {special_value}: {len(recipe)} сегментов сгенерировано")
                else:
