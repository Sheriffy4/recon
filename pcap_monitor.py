    ...
    def monitor_all(self):
        ...
        for pcap_file in self.pcap_files:
            ...
            if result is None:
                ...
                modified = datetime.fromtimestamp(result['modified']).strftime('%H:%M:%S')
                print(f"   ✅ {result['format']} | {size_mb:.1f} МБ | ~{result['packets']:,} пакетов | {modified}")
    ...
    def _analyze_results(self, results):
        ...
        if len(valid_files) > 1:
            ...
            for filename in valid_files:
                ...
                print(f"   {status}: {filename} - {size_mb:.1f} МБ, ~{packets:,} пакетов")
