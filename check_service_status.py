        ...
        for domain in x_com_domains:
            if domain in strategies:
                ...
                if "multidisorder" in strategy and "autottl=2" in strategy:
                    print("  ✅ Contains expected multidisorder + autottl=2")
                else:
                    print("  ⚠️  May not contain expected parameters")
            else:
...
def provide_recommendations(service_running, config_ok, log_results):
    ...
    if isinstance(log_results, dict):
        if not log_results["has_ip_mappings"]:
            print(
                "  3. Check IP mapping - service may not be resolving x.com correctly"
            )
...
def main():
    ...
    if isinstance(log_results, dict):
        ...
        all_good = (
            service_running
            and config_ok
            and log_results["has_ip_mappings"]
            and log_results["has_autottl"]
            and log_results["has_bypass"]
            and not log_results["has_errors"]
        )
    else:
        all_good = False
