...
class DiagnosticSystem:
    ...
    def __init__(self, attack_adapter: 'AttackAdapter', debug: bool=False):
        ...
        self.stats = {'events_logged': 0, 'errors_detected': 0, 'patterns_identified': 0, 'reports_generated': 0, 'monitoring_cycles': 0, 'attack_results_logged': 0, 'attack_failures_analyzed': 0, 'registry_validations': 0}
        self.thresholds = {'max_processing_time_ms': 100.0, 'min_success_rate': 0.8, 'max_error_rate': 0.1, 'health_score_warning': 0.7, 'health_score_critical': 0.5, 'attack_latency_warning': 50.0, 'attack_latency_critical': 100.0}
        self.logger.info('Enhanced DiagnosticSystem initialized with unified attack system integration')
    ...
    def _update_attack_metrics(self, attack_name: str, result: AttackResult):
        ...
        try:
            ...
            if attack_name not in self.attack_metrics:
                ...
                self.attack_metrics[attack_name] = AttackPerformanceMetrics(attack_name=attack_name, category=category, total_executions=0, successful_executions=0, failed_executions=0, avg_latency_ms=0.0, success_rate=0.0, error_patterns=[], last_used=current_time, health_status='healthy')
            ...
            metrics.health_status = self._determine_attack_health_status(metrics.success_rate, metrics.avg_latency_ms)
            self._update_category_health(metrics.category)
        except Exception as e:
            self.logger.error(f'Error updating attack metrics for {attack_name}: {e}')

    def _analyze_attack_failure(self, attack_name: str, result: AttackResult, context: Optional[AttackContext]):
        ...
        try:
            ...
            if attack_name not in self.attack_failures:
                self.attack_failures[attack_name] = AttackFailureAnalysis(attack_name=attack_name, failure_type=failure_type, frequency=0, first_occurrence=current_time, last_occurrence=current_time, error_messages=[], troubleshooting_steps=[], alternative_attacks=[])
            ...
            failure_analysis.troubleshooting_steps = self._generate_troubleshooting_steps(attack_name, failure_type, result)
            failure_analysis.alternative_attacks = self._suggest_alternative_attacks(attack_name, context)
        except Exception as e:
            self.logger.error(f'Error analyzing attack failure for {attack_name}: {e}')

    def _calculate_attack_performance_metrics(self, attack_name: str, results: List[Dict]) -> AttackPerformanceMetrics:
        """Calculate performance metrics for an attack."""
        try:
            total_executions = len(results)
            successful_executions = sum((1 for r in results if r['result'].status == AttackStatus.SUCCESS))
            ...
            return AttackPerformanceMetrics(attack_name=attack_name, category=category, total_executions=total_executions, successful_executions=successful_executions, failed_executions=failed_executions, avg_latency_ms=avg_latency, success_rate=success_rate, error_patterns=error_patterns, last_used=max((r['timestamp'] for r in results)), health_status=health_status)
        except Exception as e:
            self.logger.error(f'Error calculating attack performance metrics: {e}')
            return AttackPerformanceMetrics(attack_name=attack_name, category='unknown', total_executions=0, successful_executions=0, failed_executions=0, avg_latency_ms=0.0, success_rate=0.0, error_patterns=[], last_used=time.time(), health_status='unknown')
    ...
    def start_monitoring(self, fast_bypass_engine: 'FastBypassEngine'):
        ...
        self.monitoring_thread = threading.Thread(target=self._monitoring_loop, daemon=True, name='DiagnosticMonitoring')
    ...
    def log_attack_result(self, attack_result: AttackResult, attack_name: str, context: Optional[AttackContext]=None):
        """
        ...
        try:
            current_time = time.time()
            self.attack_results.append({'timestamp': current_time, 'attack_name': attack_name, 'result': attack_result, 'context': context})
            ...
            if self.debug:
                self.logger.debug(f'🎯 Attack logged: {attack_name} | Status: {attack_result.status.value} | Latency: {attack_result.latency_ms:.2f}ms | Packets: {attack_result.packets_sent} | Bytes: {attack_result.bytes_sent}')
    ...
    def log_packet_processing(self, packet: 'pydivert.Packet', action: str, technique_used: Optional[str]=None, strategy_type: Optional[str]=None, processing_time_ms: float=0.0, success: bool=True, error_message: Optional[str]=None, byte_level_info: Optional[Dict[str, Any]]=None):
        """
        ...
        try:
            ...
            event = PacketProcessingEvent(timestamp=time.time(), packet_size=packet_size, src_addr=src_addr, dst_addr=dst_addr, src_port=src_port, dst_port=dst_port, protocol=protocol, action=action, technique_used=technique_used, processing_time_ms=processing_time_ms, error_message=error_message, strategy_type=strategy_type, success=success)
            self.packet_events.append(event)
            self.stats['events_logged'] += 1
            if technique_used:
                self._update_technique_metrics(technique_used, success, processing_time_ms, error_message)
            if not success and error_message:
                self.error_events.append(event)
                self.stats['errors_detected'] += 1
                self._analyze_failure_pattern(event)
            if self.debug:
                self.logger.debug(f'📦 Packet processed: {src_addr}:{src_port} -> {dst_addr}:{dst_port} ({protocol}, {packet_size}B) | Action: {action} | Technique: {technique_used} | Time: {processing_time_ms:.2f}ms | Success: {success}')
                if byte_analysis:
    ...
    def analyze_bypass_effectiveness(self, time_window_minutes: int=60) -> Dict[str, Any]:
        ...
        try:
            ...
            if not recent_events:
                return {'time_window_minutes': time_window_minutes, 'total_events': 0, 'bypass_effectiveness': 0.0, 'combined_stats': combined_stats, 'processor_stats': processor_stats, 'message': 'No recent events to analyze'}
            ...
            technique_stats = defaultdict(lambda: {'total': 0, 'successful': 0, 'failed': 0, 'avg_processing_time': 0.0, 'processing_times': []})
            ...
            for technique, stats in technique_stats.items():
                ...
                avg_time = statistics.mean(stats['processing_times']) if stats['processing_times'] else 0.0
                technique_effectiveness[technique] = {'total_uses': stats['total'], 'successful_uses': stats['successful'], 'failed_uses': stats['failed'], 'effectiveness': effectiveness, 'avg_processing_time_ms': avg_time, 'performance_score': self._calculate_technique_performance_score(effectiveness, avg_time)}
            processing_times = [e.processing_time_ms for e in recent_events if e.processing_time_ms > 0]
            performance_metrics = {}
            if processing_times:
                performance_metrics = {'avg_processing_time_ms': statistics.mean(processing_times), 'median_processing_time_ms': statistics.median(processing_times), 'p95_processing_time_ms': self._calculate_percentile(processing_times, 95), 'p99_processing_time_ms': self._calculate_percentile(processing_times, 99), 'min_processing_time_ms': min(processing_times), 'max_processing_time_ms': max(processing_times)}
            domain_stats = defaultdict(lambda: {'total': 0, 'bypassed': 0, 'successful': 0, 'failed': 0, 'techniques_used': set(), 'avg_processing_time': 0.0, 'processing_times': []})
            ...
            for domain, stats in domain_stats.items():
                ...
                avg_time = statistics.mean(stats['processing_times']) if stats['processing_times'] else 0.0
                health = EngineHealthCheck(debug=False).evaluate_strategy_health({'success_count': stats['successful'], 'fail_count': stats['failed'], 'avg_latency_ms': avg_time})
                domain_analysis[domain] = {'total_events': stats['total'], 'bypassed_events': stats['bypassed'], 'success_rate': success_rate, 'bypass_rate': bypass_rate, 'techniques_used': list(stats['techniques_used']), 'avg_processing_time_ms': avg_time, 'health': health}
            ...
            health_score = self._calculate_comprehensive_health_score(overall_effectiveness, performance_metrics.get('avg_processing_time_ms', 0), error_rate, len(technique_effectiveness))
            results = {'time_window_minutes': time_window_minutes, 'total_events': total_events, 'bypass_effectiveness': overall_effectiveness, 'combined_stats': combined_stats, 'processor_stats': processor_stats, 'technique_effectiveness': technique_effectiveness, 'performance_metrics': performance_metrics, 'domain_analysis': domain_analysis, 'protocol_distribution': dict(protocol_stats), 'error_patterns': dict(error_patterns), 'health_score': health_score, 'recommendations': self._generate_effectiveness_recommendations(overall_effectiveness, error_rate, technique_effectiveness)}
            self.logger.debug(f'Effectiveness analysis completed: overall={overall_effectiveness:.2f}')
            return results
        except Exception as e:
    ...
    def analyze_attack_failures(self, time_window_minutes: int=60) -> Dict[str, Any]:
        ...
        try:
            ...
            failed_attacks = [r for r in recent_results if r['result'].status != AttackStatus.SUCCESS]
            if not failed_attacks:
                return {'time_window_minutes': time_window_minutes, 'total_failures': 0, 'failure_analysis': {}, 'message': 'No attack failures in the specified time window'}
            ...
            overall_recommendations = self._generate_failure_troubleshooting_recommendations(failure_analysis)
            return {'time_window_minutes': time_window_minutes, 'total_failures': len(failed_attacks), 'failure_analysis': failure_analysis, 'overall_recommendations': overall_recommendations, 'critical_attacks': [name for name, analysis in failure_analysis.items() if analysis['severity'] == 'critical'], 'analysis_timestamp': time.time()}
        except Exception as e:
    ...
    def analyze_failure_patterns(self) -> Dict[str, Any]:
        ...
        try:
            processor_stats = {}
            if hasattr(self, 'fast_bypass_engine') and hasattr(self.fast_bypass_engine, 'packet_processor'):
                processor_stats = self.fast_bypass_engine.packet_processor.get_stats()
            if not self.error_events:
                return {'total_errors': 0, 'processor_stats': processor_stats, 'patterns': [], 'message': 'No error events to analyze'}
            ...
            recommendations = self._generate_failure_recommendations(pattern_analysis, processor_stats)
            return {'total_errors': len(self.error_events), 'processor_stats': processor_stats, 'patterns': pattern_analysis, 'recommendations': recommendations, 'critical_issues': [p for p in pattern_analysis if p['severity'] == 'critical'], 'analysis_timestamp': time.time()}
        except Exception as e:
    ...
    def generate_performance_report(self) -> PerformanceReport:
        ...
        try:
            ...
            if total_events == 0 and total_attack_results == 0:
                return PerformanceReport(report_timestamp=current_time, total_packets_processed=0, bypass_success_rate=0.0, avg_processing_time_ms=0.0, top_performing_techniques=[], problematic_techniques=[], top_performing_attacks=[], problematic_attacks=[], attack_category_performance={}, optimization_recommendations=['No data available for analysis'], system_health_score=0.0)
            ...
            processing_times = [e.processing_time_ms for e in self.packet_events if e.processing_time_ms > 0]
            ...
            recommendations = self._generate_optimization_recommendations(bypass_success_rate, avg_processing_time, technique_performance, attack_performance)
            health_score = self._calculate_health_score(bypass_success_rate, avg_processing_time, attack_category_performance)
            report = PerformanceReport(report_timestamp=current_time, total_packets_processed=total_events, bypass_success_rate=bypass_success_rate, avg_processing_time_ms=avg_processing_time, top_performing_techniques=top_performing_techniques, problematic_techniques=problematic_techniques, top_performing_attacks=top_performing_attacks, problematic_attacks=problematic_attacks, attack_category_performance=attack_category_performance, optimization_recommendations=recommendations, system_health_score=health_score)
            ...
            return PerformanceReport(report_timestamp=current_time, total_packets_processed=0, bypass_success_rate=0.0, avg_processing_time_ms=0.0, top_performing_techniques=[], problematic_techniques=[], top_performing_attacks=[], problematic_attacks=[], attack_category_performance={}, optimization_recommendations=[f'Error generating report: {e}'], system_health_score=0.0)

    def analyze_attack_performance(self, time_window_minutes: int=60) -> Dict[str, AttackPerformanceMetrics]:
        """
    ...
    def monitor_attack_effectiveness(self, attack_name: str, time_window_minutes: int=30) -> Dict[str, Any]:
        """
        ...
        try:
            cutoff_time = time.time() - time_window_minutes * 60
            attack_results = [r for r in self.attack_results if r['attack_name'] == attack_name and r['timestamp'] >= cutoff_time]
            if not attack_results:
                return {'attack_name': attack_name, 'time_window_minutes': time_window_minutes, 'total_executions': 0, 'effectiveness': 0.0, 'message': 'No recent executions found'}
            total_executions = len(attack_results)
            successful_executions = sum((1 for r in attack_results if r['result'].status == AttackStatus.SUCCESS))
            latencies = [r['result'].latency_ms for r in attack_results if r['result'].latency_ms > 0]
            avg_latency = statistics.mean(latencies) if latencies else 0.0
            effectiveness = successful_executions / total_executions if total_executions > 0 else 0.0
            error_patterns = defaultdict(int)
            for r in attack_results:
                if r['result'].status != AttackStatus.SUCCESS and r['result'].error_message:
                    error_type = self._categorize_attack_error(r['result'].error_message)
                    error_patterns[error_type] += 1
            return {'attack_name': attack_name, 'time_window_minutes': time_window_minutes, 'total_executions': total_executions, 'successful_executions': successful_executions, 'failed_executions': total_executions - successful_executions, 'effectiveness': effectiveness, 'avg_latency_ms': avg_latency, 'error_patterns': dict(error_patterns), 'health_status': self._determine_attack_health_status(effectiveness, avg_latency), 'recommendations': self._generate_attack_recommendations(attack_name, effectiveness, error_patterns)}
        except Exception as e:
    ...
    def track_attack_success_rates(self, time_window_minutes: int=30) -> Dict[str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            for attack_name, results in attack_groups.items():
                ...
                tracking_data[attack_name] = {'total_executions': total_executions, 'successful_executions': successful_executions, 'failed_executions': total_executions - successful_executions, 'success_rate': success_rate, 'avg_latency_ms': avg_latency, 'alert_level': alert_level, 'last_execution': max((r['timestamp'] for r in results))}
                if alert_level in ['warning', 'critical']:
                    alerts.append({'attack_name': attack_name, 'alert_level': alert_level, 'success_rate': success_rate, 'avg_latency_ms': avg_latency, 'message': self._generate_alert_message(attack_name, alert_level, success_rate, avg_latency)})
            return {'time_window_minutes': time_window_minutes, 'tracking_data': tracking_data, 'alerts': alerts, 'total_attacks_tracked': len(tracking_data), 'attacks_with_alerts': len(alerts), 'timestamp': time.time()}
        except Exception as e:
    ...
    def get_real_time_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, 'fast_bypass_engine'):
                combined_stats = self.fast_bypass_engine.get_combined_stats()
            recent_attack_results = [r for r in self.attack_results if r['timestamp'] >= recent_cutoff]
            attack_stats = {'total_attack_executions': len(recent_attack_results), 'successful_attacks': len([r for r in recent_attack_results if r['result'].status == AttackStatus.SUCCESS]), 'failed_attacks': len([r for r in recent_attack_results if r['result'].status != AttackStatus.SUCCESS]), 'unique_attacks_used': len(set((r['attack_name'] for r in recent_attack_results)))}
            return {'timestamp': current_time, 'events_per_minute': events_per_minute, 'recent_success_rate': success_rate, 'total_events': len(self.packet_events), 'total_errors': len(self.error_events), 'active_techniques': len(self.technique_metrics), 'monitoring_active': self.monitoring_active, 'combined_stats': combined_stats, 'attack_stats': attack_stats, 'system_stats': self.stats.copy()}
        except Exception as e:
    ...
    def validate_attack_registry_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results = {'total_attacks': len(all_attacks), 'total_categories': len(categories), 'healthy_attacks': [], 'problematic_attacks': [], 'missing_attacks': [], 'category_health': {}, 'overall_health_score': 0.0, 'validation_timestamp': time.time()}
            healthy_count = 0
            for attack_name, attack_class in all_attacks.items():
                try:
                    ...
                    validation_results['healthy_attacks'].append({'name': attack_name, 'category': attack_instance.category, 'protocols': attack_instance.supported_protocols, 'description': attack_instance.description})
                    healthy_count += 1
                except Exception as e:
                    validation_results['problematic_attacks'].append({'name': attack_name, 'error': str(e), 'class_name': attack_class.__name__})
            ...
            for category in categories:
                ...
                category_healthy = len([a for a in validation_results['healthy_attacks'] if a['category'] == category])
                category_total = len(category_attacks)
                category_health_score = category_healthy / category_total if category_total > 0 else 0.0
                validation_results['category_health'][category] = {'total_attacks': category_total, 'healthy_attacks': category_healthy, 'health_score': category_health_score}
            validation_results['overall_health_score'] = healthy_count / len(all_attacks) if all_attacks else 0.0
            ...
            validation_results['recommendations'] = self._generate_registry_health_recommendations(validation_results)
            if self.debug:
                self.logger.info(f"🏥 Registry health check: {healthy_count}/{len(all_attacks)} attacks healthy ({validation_results['overall_health_score']:.1%})")
            return validation_results
        except Exception as e:
            self.logger.error(f'Error validating attack registry health: {e}')
            return {'error': str(e), 'validation_timestamp': time.time(), 'overall_health_score': 0.0}
    ...
    def export_diagnostics(self, filepath: str) -> bool:
        ...
        """
        try:
            export_data = {'export_timestamp': time.time(), 'stats': self.stats, 'technique_metrics': {name: asdict(metrics) for name, metrics in self.technique_metrics.items()}, 'attack_metrics': {name: asdict(metrics) for name, metrics in self.attack_metrics.items()}, 'attack_failures': {name: asdict(failure) for name, failure in self.attack_failures.items()}, 'category_health': self.category_health, 'failure_patterns': {name: asdict(pattern) for name, pattern in self.failure_patterns.items()}, 'recent_events': [asdict(event) for event in list(self.packet_events)[-1000:]], 'recent_errors': [asdict(event) for event in list(self.error_events)[-100:]]}
    ...
    def _monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                if effectiveness.get('overall_effectiveness', 0) < self.thresholds['health_score_critical']:
                    self.logger.warning(f"⚠️ Critical performance issue detected: Effectiveness={effectiveness.get('overall_effectiveness', 0):.2f}")
                for technique_name, metrics in self.technique_metrics.items():
                    if metrics.success_rate < self.thresholds['min_success_rate']:
                        self.logger.warning(f'⚠️ Technique {technique_name} underperforming: Success rate={metrics.success_rate:.2f}')
    ...
    def _update_technique_metrics(self, technique_name: str, success: bool, processing_time_ms: float, error_message: Optional[str]):
        """Update performance metrics for a technique."""
        if technique_name not in self.technique_metrics:
            self.technique_metrics[technique_name] = TechniquePerformanceMetrics(technique_name=technique_name, total_applications=0, successful_applications=0, failed_applications=0, avg_processing_time_ms=0.0, success_rate=0.0, error_patterns=[], optimal_parameters={}, last_used=time.time())
        ...
        if processing_time_ms > 0:
            ...
            metrics.avg_processing_time_ms = (current_avg * (total - 1) + processing_time_ms) / total
    ...
    def _analyze_failure_pattern(self, event: PacketProcessingEvent):
        ...
        if pattern_key not in self.failure_patterns:
            self.failure_patterns[pattern_key] = FailurePattern(pattern_type=pattern_key, frequency=0, first_occurrence=event.timestamp, last_occurrence=event.timestamp, affected_domains=set(), error_messages=[], suggested_fixes=[])
    ...
    def _analyze_error_pattern(self, error_type: str, events: List[PacketProcessingEvent]) -> Dict[str, Any]:
        """Analyze a specific error pattern."""
        return {'error_type': error_type, 'frequency': len(events), 'first_seen': min((e.timestamp for e in events)), 'last_seen': max((e.timestamp for e in events)), 'affected_domains': list(set((e.dst_addr for e in events))), 'severity': self._determine_severity(error_type, len(events)), 'suggested_fixes': self._get_error_fixes(error_type)}
    ...
    def _analyze_packet_bytes(self, packet: 'pydivert.Packet', additional_info: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        """
    ...
    def _analyze_ipv4_header(self, raw_data: bytes) -> Dict[str, Any]:
        ...
        try:
            if len(raw_data) < 20:
                return analysis
            analysis['header_length'] = (raw_data[0] & 15) * 4
            ...
            analysis['flags'] = flags_and_frag >> 13 & 7
            analysis['fragment_offset'] = flags_and_frag & 8191
            analysis['is_fragmented'] = analysis['fragment_offset'] > 0 or analysis['flags'] & 1 != 0
    ...
    def _analyze_ipv6_header(self, raw_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis['traffic_class'] = version_class_label >> 20 & 255
            analysis['flow_label'] = version_class_label & 1048575
    ...
    def _analyze_tcp_bytes(self, raw_data: bytes, packet: 'pydivert.Packet') -> Dict[str, Any]:
        """Analyze TCP header and options bytes."""
        analysis = {}
        try:
            ip_version = raw_data[0] >> 4 & 15
            ip_header_len = (raw_data[0] & 15) * 4 if ip_version == 4 else 40
            if len(raw_data) < ip_header_len + 20:
                return analysis
            tcp_start = ip_header_len
            tcp_header_len = (raw_data[tcp_start + 12] >> 4 & 15) * 4
            analysis['tcp_header_length'] = tcp_header_len
            analysis['sequence_number'] = struct.unpack('!I', raw_data[tcp_start + 4:tcp_start + 8])[0]
            analysis['ack_number'] = struct.unpack('!I', raw_data[tcp_start + 8:tcp_start + 12])[0]
            flags_byte = raw_data[tcp_start + 13]
            analysis['tcp_flags'] = {'FIN': bool(flags_byte & 1), 'SYN': bool(flags_byte & 2), 'RST': bool(flags_byte & 4), 'PSH': bool(flags_byte & 8), 'ACK': bool(flags_byte & 16), 'URG': bool(flags_byte & 32)}
            analysis['window_size'] = struct.unpack('!H', raw_data[tcp_start + 14:tcp_start + 16])[0]
            analysis['checksum'] = struct.unpack('!H', raw_data[tcp_start + 16:tcp_start + 18])[0]
            analysis['urgent_pointer'] = struct.unpack('!H', raw_data[tcp_start + 18:tcp_start + 20])[0]
    ...
    def _analyze_udp_bytes(self, raw_data: bytes, packet: 'pydivert.Packet') -> Dict[str, Any]:
        ...
        try:
            ...
            analysis['udp_checksum'] = struct.unpack('!H', raw_data[udp_start + 6:udp_start + 8])[0]
        except Exception as e:
    ...
    def _analyze_payload_bytes(self, payload: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            if payload_bytes.startswith(b'GET ') or payload_bytes.startswith(b'POST ') or payload_bytes.startswith(b'PUT ') or payload_bytes.startswith(b'DELETE '):
            ...
            if len(payload_bytes) >= 1:
                ...
                elif first_byte & 64 != 0:
                    analysis['protocol_hints'].append('QUIC_Short_Header')
            analysis['entropy'] = self._calculate_entropy(payload_bytes[:min(256, len(payload_bytes))])
        ...
        try:
            while i < len(options_data):
                if options_data[i] == 0:
                    ...
                    option_info = {'type': option_type, 'length': option_length, 'data': option_data.hex() if option_data else None}
                    if option_type == 2 and option_length == 4:
                        option_info['name'] = 'MSS'
                        option_info['value'] = struct.unpack('!H', option_data)[0]
                    elif option_type == 3 and option_length == 3:
                        option_info['name'] = 'Window_Scale'
                        option_info['value'] = option_data[0]
                    elif option_type == 4 and option_length == 2:
                        option_info['name'] = 'SACK_Permitted'
                    elif option_type == 8 and option_length == 10:
    ...
    def _analyze_byte_patterns(self, data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            dpi_patterns = [b'Host:', b'User-Agent:', b'Content-Type:', b'Accept:', b'Connection:', b'GET /', b'POST /', b'HTTP/1.1', b'HTTP/2.0']
    ...
    def _log_byte_level_details(self, byte_analysis: Dict[str, Any]):
        ...
        try:
            ...
            if 'is_fragmented' in byte_analysis and byte_analysis['is_fragmented']:
                self.logger.debug(f"🧩 Fragmented packet: offset={byte_analysis.get('fragment_offset', 'unknown')}")
            ...
            if 'tcp_options' in byte_analysis and byte_analysis['tcp_options']:
                options = [opt.get('name', f"Type_{opt.get('type', 'unknown')}") for opt in byte_analysis['tcp_options']]
                self.logger.debug(f"⚙️ TCP options: {', '.join(options)}")
            if 'protocol_hints' in byte_analysis and byte_analysis['protocol_hints']:
                self.logger.debug(f"🔍 Protocol hints: {', '.join(byte_analysis['protocol_hints'])}")
    ...
    def _log_tls_packet_details(self, payload: bytes):
        ...
        try:
            ...
            self.logger.debug(f'🔒 TLS Details: Type=0x{tls_type:02x}, Version=0x{tls_version:04x}, Length={tls_length}, Payload={len(payload_bytes)}B')
            if tls_type == 22 and len(payload_bytes) > 5 and (payload_bytes[5] == 1):
    ...
    def _calculate_technique_performance_score(self, effectiveness: float, avg_time_ms: float) -> float:
    ...
    def _calculate_comprehensive_health_score(self, effectiveness: float, avg_time_ms: float, error_rate: float, num_techniques: int) -> float:
    ...
    def _generate_effectiveness_recommendations(self, effectiveness: float, error_rate: float, technique_stats: Dict[str, Dict]) -> List[str]:
        """Generate recommendations based on effectiveness analysis."""
        recommendations = []
        if effectiveness < 0.5:
            recommendations.append('🔴 Critical: Overall bypass effectiveness is very low. Review strategy configuration.')
        elif effectiveness < 0.7:
            recommendations.append('🟡 Warning: Bypass effectiveness could be improved. Consider optimizing techniques.')
        if error_rate > 0.2:
            recommendations.append('🔴 Critical: High error rate detected. Check packet validation and processing.')
        elif error_rate > 0.1:
            recommendations.append('🟡 Warning: Elevated error rate. Monitor for processing issues.')
        if technique_stats:
            best_techniques = sorted(technique_stats.items(), key=lambda x: x[1]['performance_score'], reverse=True)[:3]
            worst_techniques = sorted(technique_stats.items(), key=lambda x: x[1]['performance_score'])[:2]
            if best_techniques:
            ...
            if worst_techniques and worst_techniques[0][1]['performance_score'] < 0.3:
                ...
                if worst_names:
                    recommendations.append(f"⚠️ Underperforming techniques: {', '.join(worst_names)}")
        return recommendations

    def _get_error_fixes(self, error_type: str) -> List[str]:
        """Get suggested fixes for error type."""
        fixes = {'winerror_87': ['Enable packet validation before processing', 'Use RobustPacketProcessor for safe reconstruction', 'Check packet size limits (avoid >1500 bytes)', 'Implement localhost packet filtering'], 'packet_validation': ['Verify IP header integrity', 'Check minimum packet size requirements', 'Validate TCP/UDP headers'], 'timeout': ['Increase processing timeout values', 'Optimize technique processing speed', 'Check network connectivity'], 'checksum_error': ['Recalculate checksums after modification', 'Use PacketBuilder for proper assembly', 'Verify byte-level operations'], 'localhost_handling': ['Implement proper localhost filtering', 'Skip bypass for 127.0.0.1 addresses', 'Use RobustPacketProcessor.handle_localhost_packets()'], 'packet_reconstruction': ['Use safe reconstruction methods', 'Handle large packets specially', 'Implement fallback mechanisms'], 'technique_failure': ['Review technique parameters', 'Check payload size requirements', 'Implement technique fallbacks']}
        return fixes.get(error_type, ['Review error logs for specific details'])

    def _generate_failure_recommendations(self, patterns: List[Dict[str, Any]], processor_stats: Dict[str, int]) -> List[str]:
        ...
        if processor_stats.get('reconstruction_errors', 0) > 50:
            recommendations.append('Packet reconstruction issues - check RobustPacketProcessor configuration')
        ...
        if len(patterns) > 5:
            recommendations.append('Multiple error patterns detected - comprehensive system review needed')
        return recommendations

    def _generate_optimization_recommendations(self, success_rate: float, avg_processing_time: float, technique_performance: List[Tuple[str, float]], attack_performance: Optional[List[Tuple[str, float]]]=None) -> List[str]:
        """Generate optimization recommendations."""
        recommendations = []
        if success_rate < self.thresholds['min_success_rate']:
            recommendations.append(f'Low success rate ({success_rate:.2f}) - review strategy effectiveness')
        if avg_processing_time > self.thresholds['max_processing_time_ms']:
            recommendations.append(f'High processing time ({avg_processing_time:.2f}ms) - optimize techniques')
        ...
        if good_techniques:
            recommendations.append(f"Focus on high-performing techniques: {', '.join(good_techniques)}")
        if attack_performance:
            poor_attacks = [a[0] for a in attack_performance if a[1] < 0.5]
            if poor_attacks:
                recommendations.append(f"Poor performing attacks: {', '.join(poor_attacks[:5])} - consider alternatives")
            good_attacks = [a[0] for a in attack_performance[:3] if a[1] > 0.8]
            if good_attacks:
                recommendations.append(f"High-performing attacks: {', '.join(good_attacks)} - prioritize usage")
        for category, health_score in self.category_health.items():
            if health_score < 0.5:
                recommendations.append(f"Category '{category}' underperforming ({health_score:.1%}) - review attack selection")
        return recommendations

    def _calculate_health_score(self, effectiveness: float, processing_time: float, category_performance: Optional[Dict[str, float]]=None) -> float:
        ...
        try:
            ...
            if processing_time <= max_acceptable_time:
                ...
                performance_score = max(0.0, 1.0 - (processing_time - max_acceptable_time) / max_acceptable_time)
            total_events = len(self.packet_events)
            error_events = len(self.error_events)
            error_rate = error_events / total_events if total_events > 0 else 0.0
            error_score = max(0.0, 1.0 - error_rate / self.thresholds['max_error_rate'])
            category_score = 1.0
            if category_performance:
                category_scores = list(category_performance.values())
                if category_scores:
                    category_score = statistics.mean(category_scores)
            health_score = effectiveness_score * 0.4 + performance_score * 0.25 + error_score * 0.2 + category_score * 0.15
    ...
    def log_attack_result(self, attack_result: AttackResult, domain: str=None):
        ...
        """
        try:
            self.logger.debug(f'Logging attack result: {attack_result.technique_used} - {attack_result.status.value}')
    ...
    def analyze_attack_performance(self, attack_name: str) -> Optional[AttackPerformanceMetrics]:
        """
        ...
        try:
            if attack_name not in self.attack_metrics:
                ...
                return None
            metrics = self.attack_metrics[attack_name]
            if metrics.success_rate >= 0.8 and metrics.avg_latency_ms <= self.thresholds['attack_latency_warning']:
                metrics.health_status = 'healthy'
            elif metrics.success_rate >= 0.6 and metrics.avg_latency_ms <= self.thresholds['attack_latency_critical']:
                metrics.health_status = 'warning'
            else:
                metrics.health_status = 'critical'
            self.logger.info(f'Attack {attack_name} performance: {metrics.success_rate:.2%} success rate, {metrics.avg_latency_ms:.1f}ms avg latency, status: {metrics.health_status}')
            return metrics
        except Exception as e:
        ...
        try:
            ...
            if attack_name in self.attack_failures:
                ...
                if failure_analysis.alternative_attacks:
                    recommendations.append(f"Consider alternative attacks: {', '.join(failure_analysis.alternative_attacks)}")
            if attack_name in self.attack_metrics:
                metrics = self.attack_metrics[attack_name]
                if metrics.avg_latency_ms > self.thresholds['attack_latency_critical']:
                    recommendations.append('Attack latency is high - consider optimizing parameters')
                if metrics.success_rate < 0.5:
                    recommendations.append('Low success rate - attack may not be suitable for current target')
            ...
            if not unique_recommendations:
                unique_recommendations = ['No specific troubleshooting data available', 'Check network connectivity and target accessibility', 'Verify attack parameters are appropriate for target', 'Consider trying attacks from different categories']
            self.logger.info(f'Generated {len(unique_recommendations)} troubleshooting recommendations for {attack_name}')
    ...
    def track_attack_success_rates(self) -> Dict[str, float]:
        ...
        try:
            ...
            for attack_name, metrics in self.attack_metrics.items():
                ...
                if metrics.success_rate < self.thresholds['min_success_rate']:
                    self.logger.warning(f'ALERT: Attack {attack_name} has low success rate: {metrics.success_rate:.2%}')
                if metrics.avg_latency_ms > self.thresholds['attack_latency_critical']:
                    self.logger.warning(f'ALERT: Attack {attack_name} has high latency: {metrics.avg_latency_ms:.1f}ms')
    ...
    def validate_attack_registry_health(self) -> Dict[str, Any]:
        ...
        try:
            self.stats['registry_validations'] += 1
            health_report = {'timestamp': time.time(), 'registry_healthy': True, 'total_attacks': 0, 'categories': [], 'unavailable_attacks': [], 'category_health': {}, 'recommendations': []}
            registry_stats = self.attack_registry.get_stats()
            health_report['total_attacks'] = registry_stats.get('total_attacks', 0)
            health_report['categories'] = list(registry_stats.get('categories', {}).keys())
            all_attacks = self.attack_registry.list_attacks()
            unavailable_attacks = []
            for attack_name in all_attacks[:10]:
                try:
            ...
            for category in health_report['categories']:
                ...
                if not category_health:
                    ...
                    health_report['recommendations'].append(f'Category {category} has no available attacks')
            if unavailable_attacks:
                health_report['registry_healthy'] = False
                health_report['recommendations'].append(f'{len(unavailable_attacks)} attacks are unavailable')
            if health_report['total_attacks'] < 50:
                health_report['recommendations'].append('Low number of available attacks - consider expanding registry')
            if health_report['registry_healthy']:
                self.logger.info(f"AttackRegistry health check passed: {health_report['total_attacks']} attacks available")
            else:
                self.logger.warning(f"AttackRegistry health issues detected: {len(health_report['recommendations'])} problems")
            return health_report
        except Exception as e:
            self.logger.error(f'Error validating attack registry health: {e}')
            return {'timestamp': time.time(), 'registry_healthy': False, 'error': str(e), 'recommendations': ['Registry validation failed - check system integrity']}
    ...
    def _update_attack_metrics(self, attack_result: AttackResult, domain: str=None):
        ...
        try:
            ...
            if not attack_name:
                return
            attack_info = self.attack_adapter.get_attack_info(attack_name)
            category = attack_info.get('category', 'unknown') if attack_info else 'unknown'
            if attack_name not in self.attack_metrics:
                self.attack_metrics[attack_name] = AttackPerformanceMetrics(attack_name=attack_name, category=category, total_executions=0, successful_executions=0, failed_executions=0, avg_latency_ms=0.0, success_rate=0.0, error_patterns=[], last_used=time.time(), health_status='healthy')
            metrics = self.attack_metrics[attack_name]
            metrics.total_executions += 1
            metrics.last_used = time.time()
            if attack_result.status == AttackStatus.SUCCESS:
                ...
                metrics.failed_executions += 1
                if attack_result.error_message and attack_result.error_message not in metrics.error_patterns:
            ...
            if metrics.avg_latency_ms == 0.0:
                ...
                metrics.avg_latency_ms = metrics.avg_latency_ms * 0.8 + attack_result.latency_ms * 0.2
        except Exception as e:
    ...
    def _analyze_attack_failure(self, attack_result: AttackResult, domain: str=None):
        ...
        try:
            ...
            failure_analysis.frequency += 1
            failure_analysis.last_occurrence = current_time
            if attack_result.error_message and attack_result.error_message not in failure_analysis.error_messages:
                failure_analysis.error_messages.append(attack_result.error_message)
            troubleshooting_steps = self._generate_troubleshooting_steps(failure_type, attack_result.error_message)
            ...
            if attack_info:
                category = attack_info.get('category', 'unknown')
                alternatives = self.attack_adapter.get_available_attacks(category=category)
                failure_analysis.alternative_attacks = [a for a in alternatives if a != attack_name][:3]
        except Exception as e:
    ...
    def _update_category_health(self, attack_result: AttackResult):
        ...
        try:
            ...
            if category not in self.category_health:
                ...
                self.category_health[category] = self.category_health[category] * 0.9 + success_score * 0.1
        except Exception as e:
    ...
    def _get_common_error_patterns(self, error_patterns: List[str]) -> Dict[str, str]:
        ...
        for error in error_patterns:
            ...
            if 'timeout' in error_lower:
                ...
                common_errors['connection'] = 'Verify target is accessible and not blocking connections'
    ...
    def _get_category_troubleshooting(self, category: str) -> List[str]:
        ...
        category_recommendations = {'tcp': ['Check if target supports TCP connections', 'Verify firewall is not blocking TCP traffic', 'Consider adjusting TCP-specific parameters'], 'ip': ['Ensure IP fragmentation is supported by network path', 'Check MTU settings and fragmentation policies', 'Verify IP-level access to target'], 'tls': ['Confirm target uses TLS/SSL', 'Check TLS version compatibility', 'Verify certificate validation settings'], 'http': ['Ensure target is an HTTP/HTTPS service', 'Check HTTP method and header support', 'Verify content-type handling'], 'payload': ['Check payload size limits', 'Verify encoding/encryption compatibility', 'Consider payload inspection policies'], 'tunneling': ['Ensure tunneling protocols are not blocked', 'Check for deep packet inspection', 'Verify tunnel endpoint accessibility'], 'combo': ['Check if individual attack components work', 'Verify timing and sequencing parameters', 'Consider reducing combo complexity']}
        return category_recommendations.get(category, [])

    def _generate_troubleshooting_steps(self, failure_type: str, error_message: str=None) -> List[str]:
        """Generate troubleshooting steps based on failure type."""
        steps = []
        if failure_type == 'timeout':
            steps.extend(['Increase attack timeout value', 'Check network connectivity to target', 'Verify target is not rate limiting connections'])
        elif failure_type == 'error':
            steps.extend(['Check attack parameters are valid', 'Verify target compatibility with attack', 'Review error logs for specific details'])
        elif failure_type == 'blocked':
            steps.extend(['Target may be blocking this attack type', 'Try alternative attacks from same category', 'Consider using combo attacks for evasion'])
        elif failure_type == 'invalid_params':
            steps.extend(['Verify all required parameters are provided', 'Check parameter value ranges and types', 'Review attack documentation for parameter requirements'])
        if error_message:
            error_lower = error_message.lower()
    ...
    def _generate_attack_recommendations(self, attack_name: str, effectiveness: float, error_patterns: Dict) -> List[str]:
        """Generate recommendations for improving attack effectiveness."""
        recommendations = []
        if effectiveness < 0.5:
            recommendations.append(f'Attack {attack_name} has low effectiveness ({effectiveness:.1%}). Consider using alternative attacks.')
        if 'timeout' in error_patterns and error_patterns['timeout'] > 2:
            recommendations.append(f'Attack {attack_name} experiencing frequent timeouts. Consider increasing timeout or checking network connectivity.')
        if 'blocked' in error_patterns and error_patterns['blocked'] > 1:
            recommendations.append(f'Attack {attack_name} being blocked. Try different attack parameters or alternative techniques.')
        if 'invalid_parameters' in error_patterns:
            recommendations.append(f'Attack {attack_name} has parameter issues. Review attack configuration.')
        if not recommendations:
            recommendations.append(f'Attack {attack_name} is performing well. No immediate action needed.')
        return recommendations

    def _analyze_attack_failure_pattern(self, attack_name: str, failures: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            failure_rate = total_failures / max(1, len([r for r in self.attack_results if r['attack_name'] == attack_name]))
            if failure_rate > 0.8:
                severity = 'critical'
                ...
                severity = 'info'
            troubleshooting_steps = self._generate_troubleshooting_steps(attack_name, max(failure_types.keys(), key=failure_types.get) if failure_types else 'unknown', failures[0]['result'] if failures else None)
            return {'attack_name': attack_name, 'total_failures': total_failures, 'failure_types': dict(failure_types), 'error_messages': error_messages, 'severity': severity, 'failure_rate': failure_rate, 'troubleshooting_steps': troubleshooting_steps, 'alternative_attacks': self._suggest_alternative_attacks(attack_name, failures[0].get('context'))}
        except Exception as e:
    ...
    def _generate_troubleshooting_steps(self, attack_name: str, failure_type: str, result: Optional[AttackResult]) -> List[str]:
        """Generate troubleshooting steps for attack failures."""
        steps = []
        if failure_type == 'timeout':
            steps.extend([f'Increase timeout value for {attack_name}', 'Check network connectivity to target', 'Verify target is responsive', 'Consider using faster alternative attacks'])
        elif failure_type == 'blocked':
            steps.extend([f'Try different parameters for {attack_name}', 'Use alternative attacks from same category', 'Check if target has updated DPI rules', 'Consider combo attacks to bypass detection'])
        elif failure_type == 'invalid_params':
            steps.extend([f'Review parameter configuration for {attack_name}', 'Check parameter types and ranges', 'Validate target compatibility', 'Use default parameters as baseline'])
        elif failure_type == 'connection_failed':
            steps.extend(['Verify target IP and port are correct', 'Check network connectivity', 'Ensure target service is running', 'Try basic connectivity test first'])
        else:
            steps.extend([f'Review logs for {attack_name} execution', 'Check system resources and permissions', 'Verify attack prerequisites are met', 'Try simpler attacks first'])
        return steps

    def _suggest_alternative_attacks(self, attack_name: str, context: Optional[AttackContext]) -> List[str]:
        ...
        try:
            ...
            category_attacks = self.attack_adapter.get_available_attacks(category=category, protocol=protocol)
    ...
    def _generate_failure_troubleshooting_recommendations(self, failure_analysis: Dict[str, Dict]) -> List[str]:
        """Generate overall troubleshooting recommendations."""
        recommendations = []
        critical_attacks = [name for name, analysis in failure_analysis.items() if analysis.get('severity') == 'critical']
        if critical_attacks:
            recommendations.append(f'Critical: {len(critical_attacks)} attacks have high failure rates. Immediate attention required.')
        ...
        if all_failure_types:
            ...
            recommendations.append(f'Most common failure type: {most_common_failure}. Focus troubleshooting efforts here.')
        if len(failure_analysis) > 10:
            recommendations.append('Many attacks experiencing failures. Check system resources and network connectivity.')
        recommendations.append('Review attack configurations and consider updating parameters based on current network conditions.')
        return recommendations

    def _determine_alert_level(self, success_rate: float, avg_latency: float) -> str:
        """Determine alert level based on metrics."""
        if success_rate < self.thresholds['health_score_critical'] or avg_latency > self.thresholds['attack_latency_critical']:
            return 'critical'
        elif success_rate < self.thresholds['health_score_warning'] or avg_latency > self.thresholds['attack_latency_warning']:
            return 'warning'
        else:
            return 'normal'

    def _generate_alert_message(self, attack_name: str, alert_level: str, success_rate: float, avg_latency: float) -> str:
    ...
    def _update_category_health(self, category: str):
        ...
        try:
            category_attacks = [metrics for metrics in self.attack_metrics.values() if metrics.category == category]
    ...
    def _get_expected_attack_list(self) -> List[str]:
        ...
        return ['tcp_segmentation', 'tcp_timing', 'tcp_manipulation', 'ip_fragmentation', 'ip_header_manipulation', 'tls_record_manipulation', 'tls_confusion', 'http_header_attacks', 'http_method_attacks', 'payload_encryption', 'payload_obfuscation', 'dns_tunneling', 'icmp_tunneling', 'multi_layer_combo', 'adaptive_combo']

    def _generate_registry_health_recommendations(self, validation_results: Dict[str, Any]) -> List[str]:
        """Generate recommendations based on registry health validation."""
        recommendations = []
        health_score = validation_results['overall_health_score']
        if health_score < 0.8:
            recommendations.append(f'Registry health is below optimal ({health_score:.1%}). Review problematic attacks.')
        if validation_results['problematic_attacks']:
            recommendations.append(f"{len(validation_results['problematic_attacks'])} attacks failed validation. Check implementation and dependencies.")
        if validation_results['missing_attacks']:
            recommendations.append(f"{len(validation_results['missing_attacks'])} expected attacks are missing. Verify attack registration.")
        for category, health_info in validation_results['category_health'].items():
            if health_info['health_score'] < 0.7:
                recommendations.append(f"Category '{category}' has low health score ({health_info['health_score']:.1%}). Review attacks in this category.")
        if not recommendations:
            recommendations.append('Attack registry is healthy. All attacks are properly registered and functional.')
        return recommendations
    ...
    def evaluate_domain_effectiveness(self, domain: str, start_bypass_cb=None, stop_bypass_cb=None, use_https: bool=True) -> Dict[str, Any]:
        ...
        return {'domain': domain, 'verdict': report.verdict, 'reason': report.reason, 'baseline': {'success': report.baseline.success, 'latency_ms': report.baseline.latency_ms, 'status_code': report.baseline.status_code, 'error': report.baseline.error}, 'bypass': {'success': report.bypass.success, 'latency_ms': report.bypass.latency_ms, 'status_code': report.bypass.status_code, 'error': report.bypass.error}}
    ...
    def evaluate_strategy_health(self, success_count: int, fail_count: int, avg_latency_ms: float) -> Dict[str, Any]:
        ...
        stats = {'success_count': success_count, 'fail_count': fail_count, 'avg_latency_ms': avg_latency_ms}
