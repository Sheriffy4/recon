...
class AutomatedStrategyWorkflow:
        ...
        try:
            ...
            if os.path.exists("strategies_update.json"):
                ...
                cmd = [
                    ...
                    "--action", "merge",
                    "--update-file", "strategies_update.json"
                ]
                ...
                if result.returncode == 0:
                    ...
                    self.results['synchronization_results']['status'] = 'success'
                    self.results['synchronization_results']['method'] = 'best_strategy_sync'
                    self.results['synchronization_results']['output'] = result.stdout
                    return True
                else:
                    print(f"❌ Strategy synchronization failed: {result.stderr}")
    ...
    def _generate_detailed_json_report(self, data: Dict):
        ...
        report_file = os.path.join(self.output_dir, f"comprehensive_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    ...
    def _generate_markdown_report(self, data: Dict):
        ...
        report_file = os.path.join(self.output_dir, f"analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md")
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- **PCAP File:** `{workflow.get('input_files', {}).get('pcap', 'N/A')}`\\n")
            f.write(f"- **Report File:** `{workflow.get('input_files', {}).get('report', 'N/A')}`\\n")
            f.write(f"- **Analysis Status:** {workflow.get('analysis_results', {}).get('status', 'Unknown')}\\n")
            f.write(f"- **Sync Status:** {workflow.get('synchronization_results', {}).get('status', 'Unknown')}\\n\\n")
            ...
            if 'enhanced_domain_analysis' in data:
                ...
                f.write(f"- **Total Handshakes:** {summary.get('total_handshakes', 0)}\\n")
                f.write(f"- **Domains Detected:** {summary.get('total_domains_detected', 0)}\\n")
                f.write(f"- **Successful Connections:** {summary.get('successful_connections', 0)}\\n\\n")
                
                # Domain results table
                domain_results = domain_data.get('domain_results', [])
                if domain_results:
                    f.write("### Domain-Specific Results\\n\\n")
                    f.write("| Domain | Attempts | Successes | Success Rate | Status |\\n")
                    f.write("|--------|----------|-----------|--------------|--------|\\n")
                    ...
                    for result in domain_results:
                        ...
                        f.write(f"| {domain} | {attempts} | {successes} | {success_rate:.1f}% | {status} |\\n")
            ...
            if 'domain_strategy_recommendations' in data:
                ...
                f.write(f"- **Best Strategy:** `{analysis_summary.get('best_strategy', 'N/A')}`\\n")
                f.write(f"- **Success Rate:** {analysis_summary.get('success_rate', 'N/A')}\\n")
                f.write(f"- **Successful Domains:** {len(analysis_summary.get('successful_domains', []))}\\n")
                f.write(f"- **Failed Domains:** {len(analysis_summary.get('failed_domains', []))}\\n\\n")
            
            # Current strategies
            if 'current_strategies' in data:
                strategies = data['current_strategies']
                f.write(f"## Current Strategies Configuration\\n\\n")
                ...
                for domain, strategy in list(strategies.items())[:10]:  # Limit to first 10
    ...
    def _generate_strategy_recommendations(self, data: Dict):
        ...
        with open(rec_file, 'w', encoding='utf-8') as f:
            ...
            f.write("The strategies have been automatically updated. Review the changes:\\n\\n")
            f.write("```bash\\n")
            ...
            if 'domain_strategy_recommendations' in data:
                ...
                if 'option_3_comprehensive' in strategy_options:
                    ...
                    f.write(f"The following {len(comprehensive)} domain-specific strategies have been applied:\\n\\n")
    ...
    def run_workflow(self) -> bool:
        ...
        if success:
            ...
            print(f"   - Updated strategies.json")
            print("\\n✅ Next steps:")
...
def main():
    ...
    parser.add_argument('--pcap', type=str, default='work.pcap',
                       help='PCAP file to analyze')
    parser.add_argument('--report', type=str,
                       help='CLI report file (auto-detected if not specified)')
    parser.add_argument('--output-dir', type=str, default='workflow_results',
