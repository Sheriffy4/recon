...
class StrategyVariant:
    ...
    def to_string(self) -> str:
        return (
            f"order={self.segment_order}, ttl={self.fake_ttl}, "
...
class StrategyBruteforcer:
    ...
    def generate_variants(self, base_strategy: Dict = None) -> List[StrategyVariant]:
        ...
        priority_variants = [
            # Cloudflare
            StrategyVariant(
                "fake_first", 1, 76, 336, ["badsum"], 2, 0x10, 0x18, "original"
            ),
            StrategyVariant(
                "fake_first", 2, 76, 160, ["badsum"], 1, 0x10, 0x18, "original"
            ),
            # Meta/Instagram
            StrategyVariant(
                "fake_first", 1, 40, 20, ["badsum"], 5, 0x10, 0x18, "reduced"
            ),
            StrategyVariant(
                "real_first", 1, 3, 0, ["badsum", "badseq"], 7, 0x18, 0x10, "original"
            ),
            # Fastly
            StrategyVariant(
                "fake_first", 2, 100, 50, ["md5sig"], 3, 0x10, 0x18, "original"
            ),
            # Росkomnadzor
            StrategyVariant("fake_first", 3, 3, 0, [], 1, 0x10, 0x18, "original"),
            StrategyVariant(
                "fake_first", 1, "midsld", 20, ["badsum"], 2, 0x10, 0x18, "original"
            ),
        ]
    ...
    def _generate_mutations(
        self, base_variant: StrategyVariant
        ...
        # Инверсия порядка
        mutant = StrategyVariant(**base_variant.__dict__)
        mutant.segment_order = (
            "real_first" if base_variant.segment_order == "fake_first" else "fake_first"
        )
        mutations.append(mutant)
    ...
    async def test_variant(self, variant: StrategyVariant) -> TestResult:
        ...
        try:
            ...
            if len(result) >= 5:
                status, success_count, total_count, avg_latency, site_results = result[
                    :5
                ]
            else:
    ...
    async def run_bruteforce(
        self, max_variants: int = 50, early_stop_threshold: float = 0.9
    ):
        ...
        for i, variant in enumerate(variants, 1):
            ...
            self.logger.info(
                f"Result: {result.success_rate:.1%} success, {result.avg_latency:.1f}ms latency"
            )

            # Ранняя остановка при достижении хорошего результата
            if result.success_rate >= early_stop_threshold:
                self.logger.info(
                    f"Early stop: found variant with {result.success_rate:.1%} success rate"
                )
                break
            ...
            if i % 10 == 0 and i < len(variants) - 10:
                ...
                best_so_far = sorted(
                    self.results, key=lambda r: r.success_rate, reverse=True
                )[:3]
    ...
    def generate_report(self) -> Dict:
        ...
        sorted_results = sorted(
            self.results, key=lambda r: (r.success_rate, -r.avg_latency), reverse=True
        )
    ...
    def _analyze_parameters(self) -> Dict:
        ...
        for result in self.results:
            ...
            # Fooling
            fooling_key = (
                ",".join(sorted(result.variant.fooling))
                if result.variant.fooling
                else "none"
            )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if best.variant.segment_order == "fake_first":
            recommendations.append(
                "Use fake_first segment order (send fake segment before real)"
            )
        else:
            recommendations.append(
                "Use real_first segment order (send real segment before fake)"
            )
        ...
        if best.variant.fooling:
            recommendations.append(
                f"Use fooling methods: {', '.join(best.variant.fooling)}"
            )
        else:
