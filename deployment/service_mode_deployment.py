...
class ServiceModeDeployment:
    ...
    def __init__(self, config: ServiceDeploymentConfig):
        ...
        self.logger.info(
            f"   Unified Engine: {'ENABLED' if self.config.enable_unified_engine else 'DISABLED'}"
        )
        self.logger.info(
            f"   Monitoring: {'ENABLED' if self.config.enable_monitoring else 'DISABLED'}"
        )
        self.logger.info(
            f"   Failure Tracking: {'ENABLED' if self.config.enable_failure_tracking else 'DISABLED'}"
        )
        self.logger.info(
            f"   Rollback on Failure: {'ENABLED' if self.config.enable_rollback_on_failure else 'DISABLED'}"
        )
    ...
    def deploy_to_service_mode(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._validate_service_mode_readiness():
                deployment_results["issues_found"].append(
                    "Service mode readiness validation failed"
                )
            ...
            if not service_started:
                deployment_results["issues_found"].append(
                    "Failed to start service with unified engine"
                )
                return deployment_results
            ...
            if self.config.enable_failure_tracking:
                ...
                if self._should_trigger_rollback(failure_analysis):
                    self.logger.warning(
                        "Failure threshold exceeded - triggering rollback"
                    )
            ...
            critical_issues = [
                issue
                for issue in deployment_results["issues_found"]
                if "critical" in issue.lower() or "failed" in issue.lower()
            ]
            deployment_results["deployment_successful"] = (
                len(critical_issues) == 0
                and not deployment_results["rollback_triggered"]
            )

            self.logger.info("Service mode deployment completed")
            self.logger.info(
                f"   Success: {deployment_results['deployment_successful']}"
            )
            self.logger.info(
                f"   Rollback Triggered: {deployment_results['rollback_triggered']}"
            )
            self.logger.info(
                f"   Issues Found: {len(deployment_results['issues_found'])}"
            )
    ...
    def _validate_service_mode_readiness(self) -> bool:
        ...
        try:
            ...
            test_strategy = test_loader.load_strategy(
                "--dpi-desync=multidisorder --dpi-desync-split-pos=3"
            )
            forced_config = test_loader.create_forced_override(test_strategy)

            if not forced_config.get("no_fallbacks", False) or not forced_config.get(
                "forced", False
            ):
                self.logger.error(
                    "Strategy loader not creating proper forced overrides"
                )
                return False
            ...
            if os.path.exists(testing_results_dir):
                ...
                testing_files = [
                    f
                    for f in os.listdir(testing_results_dir)
                    if f.startswith("testing_deployment_results_")
                ]
                if testing_files:
                    ...
                    if not testing_results.get("deployment_successful", False):
                        self.logger.warning(
                            "Testing environment deployment was not successful"
                        )
                        return False
                else:
    ...
    def _enable_unified_engine_service_mode(self):
        ...
        self.logger.info(
            f"     Force Override: {self.unified_engine.config.force_override}"
        )
        self.logger.info(f"     Debug Mode: {self.unified_engine.config.debug}")
        self.logger.info(
            f"     Diagnostics: {self.unified_engine.config.enable_diagnostics}"
        )
        self.logger.info(
            f"     Strategy Logging: {self.unified_engine.config.log_all_strategies}"
        )
        ...
        try:
            ...
            for domain, ip in domain_to_ip.items():
                ...
                try:
                    ...
                    forced_config = strategy_loader.create_forced_override(
                        normalized_strategy
                    )
                    strategy_map[ip] = forced_config

                    self.logger.info(
                        f"Loaded strategy for {domain} ({ip}): {normalized_strategy.type}"
                    )
                except Exception as e:
            ...
            if strategy_map:
                ...
                self.logger.info(
                    f"Unified engine started for {len(target_ips)} targets"
                )
    ...
    def _monitor_domain_opening(self) -> Dict[str, Any]:
            ...
            for domain in self.config.test_domains:
                try:
                    ...
                    if total_domain_tests > 0:
                        domain_results["success_rate"] = (
                            domain_results["success_count"] / total_domain_tests
                        ) * 100

                    self.logger.info(
                        f"Domain {domain}: {'SUCCESS' if test_result['success'] else 'FAILED'} "
                    ...
                    monitoring_results["issues_detected"].append(
                        f"Domain test error for {domain}: {e}"
                    )
        ...
        if total_tests > 0:
            monitoring_results["overall_success_rate"] = (
                successful_tests / total_tests
            ) * 100

        self.logger.info("Domain monitoring completed:")
        self.logger.info(f"   Total Tests: {total_tests}")
        self.logger.info(f"   Successful: {successful_tests}")
        self.logger.info(
            f"   Success Rate: {monitoring_results['overall_success_rate']:.1f}%"
        )
    ...
    def _update_service_metrics(
        self, unified_engine: UnifiedBypassEngine, domain_to_ip: Dict[str, str]
    ):
        """
        ...
        try:
            ...
            self.metrics["strategy_applications"] = engine_diag[
                "strategy_applications_count"
            ]

            # Log periodic status
            self.logger.info("Service metrics update:")
            self.logger.info(
                f"   Forced Overrides: {self.metrics['forced_override_count']}"
            )
            self.logger.info(
                f"   Strategy Applications: {self.metrics['strategy_applications']}"
            )
            self.logger.info(f"   Domains Tested: {self.metrics['domains_tested']}")
            self.logger.info(
                f"   Success Rate: {(self.metrics['successful_domains'] / max(1, self.metrics['domains_tested']) * 100):.1f}%"
            )
    ...
    def _analyze_failures(self) -> Dict[str, Any]:
        ...
        if total_tests > 0:
            failure_analysis["failure_rate"] = (
                len(self.metrics["failure_details"]) / total_tests
            ) * 100
        ...
        if failure_analysis["total_failures"] == 0:
            failure_analysis["recommendations"].append(
                "No failures detected - service deployment successful"
            )
        elif failure_analysis["failure_rate"] < 10:
            failure_analysis["recommendations"].append(
                "Low failure rate - monitor and investigate specific failures"
            )
        elif failure_analysis["failure_rate"] < self.config.failure_threshold_percent:
            failure_analysis["recommendations"].append(
                "Moderate failure rate - investigate patterns and optimize"
            )
        else:
            failure_analysis["recommendations"].append(
                "High failure rate - consider rollback and investigation"
            )
        ...
        self.logger.info(
            f"   Critical Failures: {len(failure_analysis['critical_failures'])}"
        )
    ...
    def _should_trigger_rollback(self, failure_analysis: Dict[str, Any]) -> bool:
        ...
        if failure_analysis["critical_failures"]:
            self.logger.warning(
                f"Critical failures detected: {failure_analysis['critical_failures']}"
            )
            return True
    ...
    def _trigger_rollback(self) -> Dict[str, Any]:
        ...
        try:
            ...
            rollback_result["rollback_actions"].append(
                "Stopped service with unified engine"
            )
            ...
            rollback_result["rollback_actions"].append(
                "Would restore previous service configuration"
            )
    ...
    def _collect_service_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "service_metrics": {
                ...
                "success_rate": (
                    self.metrics["successful_domains"]
                    / max(1, self.metrics["domains_tested"])
                    * 100
                ),
                "total_failures": len(self.metrics["failure_details"]),
        ...
        self.logger.info("Service metrics collected:")
        self.logger.info(f"   Deployment Duration: {deployment_duration:.2f}s")
        self.logger.info(f"   Domains Tested: {self.metrics['domains_tested']}")
        self.logger.info(
            f"   Success Rate: {metrics['service_metrics']['success_rate']:.1f}%"
        )
        self.logger.info(
            f"   Forced Overrides: {self.metrics['forced_override_count']}"
        )
    ...
    def _generate_service_recommendations(
        ...
        if deployment_results["deployment_successful"]:
            recommendations.append(
                "Service mode deployment successful - unified engine working correctly"
            )
        else:
            recommendations.append(
                "Service mode deployment has issues - investigate and resolve"
            )

        # Check rollback status
        if deployment_results["rollback_triggered"]:
            recommendations.append(
                "CRITICAL: Rollback was triggered - investigate failures before retry"
            )
        ...
        if overall_success_rate >= 90:
            recommendations.append(
                f"Excellent success rate ({overall_success_rate:.1f}%) - deployment is highly successful"
            )
        elif overall_success_rate >= 70:
            recommendations.append(
                f"Good success rate ({overall_success_rate:.1f}%) - minor optimization may be beneficial"
            )
        elif overall_success_rate >= 50:
            recommendations.append(
                f"Moderate success rate ({overall_success_rate:.1f}%) - investigate failing domains"
            )
        else:
            recommendations.append(
                f"Low success rate ({overall_success_rate:.1f}%) - significant issues need resolution"
            )
        ...
        if failure_analysis.get("total_failures", 0) == 0:
            ...
            if failure_rate > self.config.failure_threshold_percent:
                recommendations.append(
                    f"CRITICAL: Failure rate ({failure_rate:.1f}%) exceeds threshold"
                )
        ...
        if service_metrics.get("forced_override_count", 0) > 0:
            recommendations.append(
                "Forced override mechanism working correctly in service mode"
            )
        else:
            recommendations.append(
                "WARNING: No forced overrides detected - verify configuration"
            )
