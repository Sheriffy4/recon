...
class PostDeploymentLogMonitor:
    ...
    def analyze_forced_override_usage(self, log_content: str, log_file: str) -> List[Dict]:
    ...
    def analyze_domain_success_rates(self, log_content: str, log_file: str) -> Dict:
        ...
        domain_stats = defaultdict(lambda: {'attempts': 0, 'successes': 0, 'failures': 0})
        lines = log_content.split('\n')
        
        for line in lines:
            # Extract domain from line
            domain_match = re.search(r'(youtube\.com|rutracker\.org|x\.com|instagram\.com)', line, re.IGNORECASE)
            if domain_match:
    ...
    def monitor_logs(self) -> Dict:
        ...
        for log_file in log_files:
            try:
                ...
                forced_overrides = self.analyze_forced_override_usage(content, str(log_file))
                ...
                for domain, stats in domain_stats.items():
                    if domain not in all_domain_stats:
                        all_domain_stats[domain] = {
                            'attempts': 0, 'successes': 0, 'failures': 0, 'files': []
                        }
        ...
        self.results['summary'] = {
            ...
            'critical_errors': len([e for e in all_errors if 'CRITICAL' in e.get('error_line', '').upper()]),
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"- **Forced override usages:** {summary['total_forced_override_usages']}")
        report.append(f"- **Forced override detected:** {'✅ Yes' if summary['forced_override_detected'] else '❌ No'}")
        ...
        if self.results['forced_override_usage']:
            ...
            report.append("")
            for i, usage in enumerate(self.results['forced_override_usage'][:5]):  # Show first 5
        ...
        if summary['critical_errors'] > 0:
            report.append("- 🚨 **Critical errors detected** - immediate investigation required")
        if not summary['forced_override_detected']:
            report.append("- ⚠️  **No forced override usage detected** - verify unified engine is active")
        if summary['total_errors'] > 10:
            report.append("- 🔍 **High error count** - review error patterns and root causes")
        
        # Check domain success rates
        for domain, stats in self.results['domain_success_rates'].items():
            if stats['success_rate'] < 80:
                report.append(f"- 📉 **{domain} success rate low** ({stats['success_rate']}%) - investigate failures")
        
        if not self.results['domain_success_rates']:
            report.append("- 📊 **No domain statistics** - ensure logging is properly configured")
...
def main():
    ...
    print(f"Forced override detected: {'✅ Yes' if summary['forced_override_detected'] else '❌ No'}")
