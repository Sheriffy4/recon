...
from core.bypass.performance.segment_performance_optimizer import SegmentPerformanceOptimizer
...
class AnomalyDetector:
    ...
    def is_anomaly(self, metric_name: str, value: float, threshold_factor: float = 2.0) -> bool:
...
class ProductionMonitoringSystem:
    ...
    def __init__(self, config: MonitoringConfig):
        ...
        # Metrics storage
        self.metrics_history = deque(maxlen=int(config.metrics_retention_hours * 3600 / config.metrics_collection_interval))
        self.active_alerts = {}
    ...
    async def _collect_current_metrics(self) -> SystemMetrics:
        ...
        if recent_executions:
            ...
            execution_times = [ex.get('execution_time', 0) * 1000 for ex in recent_executions]
            avg_response_time = statistics.mean(execution_times) if execution_times else 0
            p95_response_time = statistics.quantiles(execution_times, n=20)[18] if len(execution_times) > 5 else avg_response_time
        else:
            success_rate = 1.0
    ...
    async def _collect_segment_metrics(self, recent_executions: List[Dict]) -> Dict[str, float]:
    ...
    async def _collect_error_metrics(self, recent_executions: List[Dict]) -> Dict[str, Any]:
        ...
        failed_executions = [ex for ex in recent_executions if not ex.get('success', True)]
        error_rate = len(failed_executions) / len(recent_executions)
        
        # Count critical errors and warnings
        critical_errors = sum(1 for ex in failed_executions if ex.get('error_severity') == 'critical')
        warnings = sum(1 for ex in failed_executions if ex.get('error_severity') == 'warning')
    ...
    async def _check_performance_thresholds(self, metrics: SystemMetrics):
        ...
        if metrics.success_rate < (1.0 - self.config.failure_rate_threshold):
            await self._create_alert(
                ...
                metadata={'success_rate': metrics.success_rate, 'threshold': 1.0 - self.config.failure_rate_threshold}
            )
        ...
        if metrics.average_response_time_ms > self.config.response_time_threshold_ms:
            await self._create_alert(
                ...
                metadata={'response_time_ms': metrics.average_response_time_ms, 'threshold_ms': self.config.response_time_threshold_ms}
            )
        ...
        if metrics.memory_usage_mb > self.config.memory_usage_threshold_mb:
            await self._create_alert(
                ...
                metadata={'memory_mb': metrics.memory_usage_mb, 'threshold_mb': self.config.memory_usage_threshold_mb}
            )
        ...
        if metrics.cpu_usage_percent > self.config.cpu_usage_threshold_percent:
            await self._create_alert(
                ...
                metadata={'cpu_percent': metrics.cpu_usage_percent, 'threshold_percent': self.config.cpu_usage_threshold_percent}
            )
    ...
    async def _create_alert(self, alert_type: str, severity: str, title: str, message: str, metadata: Dict[str, Any] = None):
        """Create a new alert."""
    ...
    async def _check_alert_resolution(self):
        ...
        for alert_id, alert in self.active_alerts.items():
            ...
            if alert.type == 'high_failure_rate':
                should_resolve = latest_metrics.success_rate >= (1.0 - self.config.failure_rate_threshold * 0.8)
            elif alert.type == 'high_response_time':
                should_resolve = latest_metrics.average_response_time_ms <= self.config.response_time_threshold_ms * 0.9
            elif alert.type == 'high_memory_usage':
                should_resolve = latest_metrics.memory_usage_mb <= self.config.memory_usage_threshold_mb * 0.9
            elif alert.type == 'high_cpu_usage':
                should_resolve = latest_metrics.cpu_usage_percent <= self.config.cpu_usage_threshold_percent * 0.9
    ...
    async def _process_alert_notifications(self):
        ...
        if self.active_alerts:
            ...
            if critical_alerts:
                self.logger.critical(f"🚨 {len(critical_alerts)} critical alerts active")
    
    async def _save_metrics_snapshot(self, metrics: SystemMetrics):
        """Save metrics snapshot to file."""
        
        metrics_file = Path(self.config.monitoring_data_dir) / "metrics" / f"metrics_{int(metrics.timestamp)}.json"
    ...
    async def _save_alert(self, alert: Alert):
        ...
        alert_file = Path(self.config.monitoring_data_dir) / "alerts" / f"alert_{alert.id}.json"
    ...
    async def _update_dashboard_data(self):
        ...
        dashboard_data = {
            ...
            'critical_alerts_count': sum(1 for alert in self.active_alerts.values() if alert.severity == 'critical'),
            'current_metrics': {
    ...
    async def _establish_performance_baseline(self):
        ...
        await asyncio.sleep(min(baseline_duration, 60))  # Wait up to 1 minute for initial data
        ...
        if len(self.metrics_history) >= 5:
            ...
            self.performance_baseline = {
                ...
                'avg_attacks_per_minute': statistics.mean([m.attacks_per_minute for m in recent_metrics])
            }
    ...
    async def _check_for_anomalies(self, metrics: SystemMetrics):
        ...
        if metrics.average_response_time_ms > baseline['avg_response_time_ms'] * 2:
            await self._create_alert(
                ...
                metadata={'current': metrics.average_response_time_ms, 'baseline': baseline['avg_response_time_ms']}
            )
        ...
        if metrics.success_rate < baseline['avg_success_rate'] * 0.8:
            await self._create_alert(
                ...
                metadata={'current': metrics.success_rate, 'baseline': baseline['avg_success_rate']}
            )
    ...
    def get_metrics_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_errors': sum([m.critical_errors + m.warnings for m in recent_metrics])
        }
    ...
    def get_alert_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_alerts_today': sum(1 for alert in self.alert_history if time.time() - alert.timestamp <= 86400),
            'recent_alerts': [
                {
                    'id': alert.id,
                    'type': alert.type,
