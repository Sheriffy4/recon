...
class NotworkPcapAnalyzer:
    ...
    def analyze_tls_handshakes(self):
        ...
        for packet in self.packets:
            if packet.haslayer(IP) and packet.haslayer(TCP):
                ...
                if ip_dst in self.blocked_ips or any(
                    ip_dst.startswith(prefix)
                    for prefix in ["157.240", "172.66", "104.244", "199.232"]
                ):
                            ...
                            if hasattr(client_hello, "ext") and client_hello.ext:
                                for ext in client_hello.ext:
                                    if hasattr(ext, "servernames") and ext.servernames:
                                        ...
                                        print(
                                            f"  🔐 TLS ClientHello к {ip_dst} (SNI: {sni})"
                                        )
                                        break
                            else:
                    ...
                    # Успешное TLS рукопожатие (ServerHello)
                        ...
                        packet.haslayer(TLS)
                        and hasattr(packet[TLS], "msg")
                        and len(packet[TLS].msg) > 0
                    ):
    ...
    def analyze_http_traffic(self):
        ...
        for packet in self.packets:
            if packet.haslayer(HTTPRequest):
                ...
                status_code = (
                    resp.Status_Code.decode("utf-8") if resp.Status_Code else "unknown"
                )
    ...
    def analyze_bypass_effectiveness(self):
        ...
        for packet in self.packets:
            if packet.haslayer(IP) and packet.haslayer(TCP):
                ...
                if prev_packet and packet.haslayer(TLS):
                    if (
                        prev_packet.haslayer(IP)
                        and prev_packet[IP].dst == ip_layer.dst
                        and prev_packet.haslayer(TCP)
                        and prev_packet[TCP].dport == tcp_layer.dport
                    ):
        ...
        print(
            f"  📊 Фрагментированные пакеты: {bypass_indicators['fragmented_packets']}"
        )
    ...
    def analyze_connection_timeouts(self):
        ...
        for packet in self.packets:
            if packet.haslayer(IP) and packet.haslayer(TCP):
                ...
                if ip_dst in self.blocked_ips or any(
                    ip_dst.startswith(prefix)
                    for prefix in ["157.240", "172.66", "104.244"]
                ):
        ...
        for conn_key, conn in connections.items():
            ...
            if duration > 5.0 and not conn["established"]:  # Таймаут > 5 секунд
                ...
                print(
                    f"  ⏱️ Таймаут: {conn_key} ({duration:.1f}s, {conn['packets']} пакетов)"
                )
    ...
    def detect_blocking_methods(self):
        ...
        for method in blocking_methods:
            ...
            print(
                f"  {severity_icon} {method['method']}: {method['description']} (x{method['count']})"
            )
    ...
    def run_full_analysis(self):
        ...
        print("🔍 === Анализ файла notwork.pcap ===")
        print("Цель: выяснить почему заблокированные сайты не открывались\n")
    ...
    def print_summary(self):
        ...
        if results["tcp_resets"] > results["successful_connections"]:
            print("  • Много TCP RST пакетов - провайдер активно блокирует соединения")

        if results["tls_handshakes"] > 0 and results["successful_connections"] == 0:
            print("  • TLS рукопожатия не завершаются - DPI блокирует TLS трафик")

        if results["timeouts"] > 0:
            print(
                "  • Соединения зависают - возможна блокировка на уровне маршрутизации"
            )

        bypass_indicators = results.get("bypass_indicators", {})
        if sum(bypass_indicators.values()) == 0:
            print("  • Признаки обхода не обнаружены - стратегии могут не работать")
        ...
        if blocking_methods:
            ...
            if high_severity:
                ...
                print("\n  💡 Попробуйте:")
                print("    1. Изменить стратегию обхода на более агрессивную")
                print("    2. Использовать комбинацию методов (fake + split + ttl)")
                print("    3. Настроить DoH в браузере")
                print("    4. Добавить рабочие IP в hosts файл")

        print("\n🔧 Для улучшения обхода запустите:")
        print("  python simple_cli.py setup-hosts")
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("📄 Результаты сохранены в analysis_results")
        else:
            print("\n❌ Анализ не удался")
