...
class WorkPcapFocusedAnalyzer:
                        ...
                        if 0 < captured_len < 65536:
                            ...
                            if len(packet_data) == captured_len:
                                self._analyze_packet(
                                    packet_data, 0
                                )  # timestamp not critical for this analysis
                                packet_count += 1
                    except:
                        pass
                ...
                if protocol == 6 and len(packet_data) >= 54:
                    ...
                    if dst_port == 443 or src_port == 443:
                        self._analyze_tls_packet(
                            packet_data, src_ip, dst_ip, src_port, dst_port, timestamp
                        )

                    # Check for HTTP (port 80)
                    elif dst_port == 80 or src_port == 80:
                        self._analyze_http_packet(
                            packet_data, src_ip, dst_ip, src_port, dst_port, timestamp
                        )
    ...
    def _analyze_tls_packet(
        self, packet_data, src_ip, dst_ip, src_port, dst_port, timestamp
    ):
            ...
            if (
                ...
                if sni:
                    handshake_info = {
                        ...
                        "sni": sni,
                        "is_target": any(
                            domain in sni for domain in self.target_domains
                        ),
                    }
    ...
    def _generate_focused_report(self):
        ...
        for domain, count in domain_stats.most_common():
            ...
            if is_problematic:
                print(
                    f"   ⚠️  {domain} - PROBLEMATIC SUBDOMAIN (needs specific strategy)"
                )
        ...
        print("\n💡 RECOMMENDATIONS:")
        print(
            "   1. Ensure enhanced strategies.json is loaded (33 strategies vs current 14)"
        )
