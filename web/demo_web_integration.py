...
async def demo_web_integration():
    ...
    for pool_data in config['pools']:
        print(f"  - {pool_data['name']}: {len(pool_data['domains'])} domains, "
    ...
    for pool in pool_manager.list_pools():
        ...
        print(f"    Strategy: {pool.strategy.name} ({len(pool.strategy.attacks)} attacks)")
    ...
    print(f"✅ Web application configured:")
    ...
    print(f"📊 Total domains: {sum(len(p.domains) for p in pool_manager.pools.values())}")
