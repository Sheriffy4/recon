...
from core.bypass.strategies.pool_management import (
    StrategyPoolManager, StrategyPool, BypassStrategy, PoolPriority
)
...
class BypassEngineAPI:
    ...
    def __init__(self, pool_manager: StrategyPoolManager, 
        ...
        if not AIOHTTP_AVAILABLE:
            raise ImportError("aiohttp is required for web API. Install with: pip install aiohttp")
    ...
    def setup_routes(self, app: Application):
        ...
        app.router.add_post('/api/bypass/pools/{pool_id}/domains', self.add_domain_to_pool)
        app.router.add_delete('/api/bypass/pools/{pool_id}/domains/{domain}', self.remove_domain_from_pool)
        
        # Pool subdomain strategies
        app.router.add_post('/api/bypass/pools/{pool_id}/subdomains', self.set_subdomain_strategy)
        app.router.add_delete('/api/bypass/pools/{pool_id}/subdomains/{subdomain}', self.remove_subdomain_strategy)
        ...
        app.router.add_post('/api/bypass/attacks/{attack_id}/enable', self.enable_attack)
        app.router.add_post('/api/bypass/attacks/{attack_id}/disable', self.disable_attack)
        
        # Attack testing endpoints
        app.router.add_post('/api/bypass/attacks/{attack_id}/test', self.test_attack)
        app.router.add_get('/api/bypass/attacks/{attack_id}/test-results', self.get_attack_test_results)
        ...
        app.router.add_get('/api/bypass/strategies/test/{test_id}', self.get_test_status)
    ...
    async def create_pool(self, request: Request) -> Response:
        ...
        try:
            ...
            strategy = BypassStrategy(
                id=strategy_data.get('id', f"strategy_{data['name'].lower().replace(' ', '_')}"),
    ...
    async def get_pool(self, request: Request) -> Response:
        ...
        try:
            ...
            strategy_dict = {
                ...
                'last_tested': pool.strategy.last_tested.isoformat() if pool.strategy.last_tested else None
            }
        ...
        try:
            ...
            if 'strategy' in data:
                ...
                if 'compatibility_mode' in strategy_data:
                    pool.strategy.compatibility_mode = strategy_data['compatibility_mode']
    ...
    async def add_domain_to_pool(self, request: Request) -> Response:
        ...
        try:
            ...
            if success:
                ...
                return web.json_response({
                    'success': True,
                    'message': f'Domain "{domain}" added to pool'
                })
            else:
                return web.json_response({
                    'success': False,
                    'error': 'Failed to add domain to pool'
                }, status=400)
    ...
    async def remove_domain_from_pool(self, request: Request) -> Response:
        ...
        try:
            ...
            if success:
                ...
                return web.json_response({
                    'success': True,
                    'message': f'Domain "{domain}" removed from pool'
                })
            else:
                return web.json_response({
                    'success': False,
                    'error': 'Failed to remove domain from pool'
                }, status=400)
    ...
    async def set_subdomain_strategy(self, request: Request) -> Response:
        ...
        try:
            ...
            if not subdomain or not strategy_data:
                return web.json_response({
                    'success': False,
                    'error': 'Subdomain and strategy are required'
                }, status=400)
    ...
    async def remove_subdomain_strategy(self, request: Request) -> Response:
        ...
        try:
            ...
            if subdomain in pool.subdomains:
                ...
                return web.json_response({
                    'success': False,
                    'error': 'Subdomain strategy not found'
                }, status=404)
            ...
            for attack_id in attack_ids:
                ...
                if definition:
                    attack_dict = {
                        ...
                        'last_tested': definition.last_tested.isoformat() if definition.last_tested else None,
    ...
    async def get_attack(self, request: Request) -> Response:
        ...
        try:
            ...
            test_results_data = [tr.to_dict() for tr in test_results[-10:]]  # Last 10 results
    ...
    async def _run_all_attacks_test(self, test_session_id: str, attack_ids: List[str]):
        ...
        try:
            ...
            await self.broadcast_update({
                ...
                'success_rate': successful_tests / test_data['total_attacks'] if test_data['total_attacks'] > 0 else 0
            })
    ...
    async def enable_attack(self, request: Request) -> Response:
        ...
        try:
            ...
            if success:
                ...
                return web.json_response({
                    'success': True,
                    'message': f'Attack "{attack_id}" enabled'
                })
            else:
                return web.json_response({
                    'success': False,
                    'error': 'Failed to enable attack'
                }, status=400)
    ...
    async def disable_attack(self, request: Request) -> Response:
        ...
        try:
            ...
            if success:
                ...
                return web.json_response({
                    'success': True,
                    'message': f'Attack "{attack_id}" disabled'
                })
            else:
                return web.json_response({
                    'success': False,
                    'error': 'Failed to disable attack'
                }, status=400)
        ...
        try:
            ...
            for pool_data in config['pools']:
                try:
                    ...
                    strategy = BypassStrategy(
                        id=strategy_data.get('id', f"imported_{pool_data['name'].lower().replace(' ', '_')}"),
                        name=strategy_data.get('name', f"Strategy for {pool_data['name']}"),
                        attacks=strategy_data.get('attacks', []),
                        parameters=strategy_data.get('parameters', {}),
                        target_ports=strategy_data.get('target_ports', [443]),
                        compatibility_mode=strategy_data.get('compatibility_mode', 'native'),
                    ...
                    if 'subdomains' in pool_data:
                        for subdomain, sub_strategy_data in pool_data['subdomains'].items():
                            sub_strategy = BypassStrategy(
                                id=sub_strategy_data.get('id', f"sub_{subdomain}"),
                                name=sub_strategy_data.get('name', f"Strategy for {subdomain}"),
                                attacks=sub_strategy_data.get('attacks', []),
                                parameters=sub_strategy_data.get('parameters', {}),
                                target_ports=sub_strategy_data.get('target_ports', [443]),
                                compatibility_mode=sub_strategy_data.get('compatibility_mode', 'native')
                            )
                        ...
                        for port_str, port_strategy_data in pool_data['ports'].items():
                            try:
                                ...
                                port_strategy = BypassStrategy(
                                    ...
                                    name=port_strategy_data.get('name', f"Strategy for port {port}"),
                                    attacks=port_strategy_data.get('attacks', []),
                                    parameters=port_strategy_data.get('parameters', {}),
                                    target_ports=port_strategy_data.get('target_ports', [port]),
                                    compatibility_mode=port_strategy_data.get('compatibility_mode', 'native')
                                )
                    ...
                    errors.append(f"Failed to import pool '{pool_data.get('name', 'unknown')}': {e}")
            ...
            if errors:
                ...
                result['message'] = f"Imported {imported_count} pools with {len(errors)} errors"
            else:
        ...
        try:
            health = {
                ...
                'components': {
                    ...
                    'attack_registry': 'healthy' if self.attack_registry else 'unavailable',
                    'test_runner': 'healthy' if self.test_runner else 'unavailable',
                    'reliability_validator': 'healthy' if self.reliability_validator else 'unavailable'
                }
            }
    ...
    async def _run_strategy_test(self, test_id: str, domain: str, strategy: BypassStrategy):
