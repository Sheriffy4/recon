...
class OptimizedCLIWorkflow:
    ...
    def __init__(self, attack_adapter, result_processor, args):
        ...
        self.args = args
        self.optimizer = create_workflow_optimizer(attack_adapter, result_processor, args)
        self.execution_mode = detect_execution_mode(args)
        LOG.info(f'Initialized optimized CLI workflow in {self.execution_mode.value} mode')
    ...
    async def run_optimized_hybrid_mode(self, report_logger, domains, dns_cache):
        ...
        if not optimized_groups:
            ...
            return
        console.print(f'Optimized grouping: {len(optimized_groups)} groups (vs {len(domains)} individual domains)')
        report_logger.info(f'Domain grouping optimization: {len(optimized_groups)} groups for {len(domains)} domains')
        ...
        for group_id, group_domains in optimized_groups.items():
            if group_id.startswith('pending_'):
                ...
                should_skip, cached_fp = self.optimizer.should_skip_fingerprinting(representative_domain, ip)
                if should_skip and cached_fp:
                    console.print(f'  [green]✓ Using cached fingerprint for {len(group_domains)} domains[/green]')
                    fingerprinted_groups[cached_fp.short_hash()] = {'domains': group_domains, 'fingerprint': cached_fp, 'best_result': None}
                else:
                    console.print(f'  [cyan]Fingerprinting {representative_domain} (representing {len(group_domains)} domains)[/cyan]')
                    ...
                    fingerprint_engine = UltimateAdvancedFingerprintEngine(prober=prober, classifier=classifier, attack_adapter=self.attack_adapter, debug=self.args.debug)
                    with capture_session(f'tcp and host {ip}') as (packets, _):
                        context = AttackContext(dst_ip=ip, dst_port=self.args.port, domain=representative_domain, params={'type': 'baseline'})
                        await self.attack_adapter.execute_attack_by_name('tcp_http_combo', context)
                    fp = await fingerprint_engine.create_comprehensive_fingerprint(representative_domain, target_ips=[ip], packets=packets)
                    for domain in group_domains:
                        self.optimizer.cache_fingerprint_result(domain, ip, fp)
                    fingerprinted_groups[fp.short_hash()] = {'domains': group_domains, 'fingerprint': fp, 'best_result': None}
                    console.print(f'  [green]✓ Fingerprinted and cached for {len(group_domains)} domains[/green]')
            else:
        ...
        for fp_hash, group_data in fingerprinted_groups.items():
            ...
            if hasattr(self.args, 'count'):
                strategy_generator = AdvancedStrategyGenerator(fingerprint_dict=fp.to_dict(), history=[], parameter_optimizer=None)
                strategy_plan = strategy_generator.generate_strategies(count=self.args.count)
                attack_names = [task.get('name') for task in strategy_plan if task.get('name')]
                optimized_order = self.optimizer.optimize_parameter_testing_order(attack_names, fp)
                ...
                for attack_name in optimized_order:
                    for task in strategy_plan:
                        if task.get('name') == attack_name:
                            ...
                            break
                console.print(f'  [cyan]Optimized attack order: {optimized_order[:3]}...[/cyan]')
            else:
                optimized_strategy_plan = []
            await self._test_optimized_strategy_group(group_data, optimized_strategy_plan, report_logger)
        ...
        console.print(f"  Execution mode: {workflow_stats.get('execution_mode', 'unknown')}")
        console.print(f"  Cached fingerprints: {workflow_stats.get('cached_fingerprints', 0)}")
        console.print(f"  Cache hit rate: {workflow_stats.get('cache_hit_rate', 0):.1%}")
    ...
    async def _test_optimized_strategy_group(self, group_data, strategy_plan, report_logger):
        """Test strategies on a group with optimizations."""
        fp = group_data['fingerprint']
        group_domains = group_data['domains']
        use_fast_mode = self.optimizer.should_use_fast_mode(len(group_domains))
        if use_fast_mode:
            console.print(f'  [yellow]Using fast mode for {len(group_domains)} domains[/yellow]')
            representative_domain = group_domains[0]
        ...
        for i, strategy_task in enumerate(strategy_plan):
            ...
            for domain in test_domains:
                ...
                context = AttackContext(dst_ip=ip, dst_port=self.args.port, domain=domain, payload=test_payload, params=strategy_task.get('params', {}), debug=self.args.debug)
                attack_result = await self.attack_adapter.execute_attack_by_name(attack_name, context)
                processed = self.result_processor.process_attack_result(attack_result, attack_name)
            ...
            if successful_tests:
                ...
                strategy_summary = {'task': strategy_task, 'success_rate': success_rate, 'avg_latency_ms': avg_latency, 'bypass_effective': True}
                if best_result is None or success_rate > best_result['success_rate']:
                    best_result = strategy_summary
                console.print(f'    [green]✓ {attack_name}: {success_rate:.0%} success[/green]')
                if success_rate >= 0.9:
                    console.print('    [bold green]Early termination - found highly effective strategy[/bold green]')
                    break
            else:
                console.print(f'    [red]✗ {attack_name}: No success[/red]')
        group_data['best_result'] = best_result
        if best_result:
            console.print(f"  [bold green]Best for group: {best_result['task'].get('name')} ({best_result['success_rate']:.0%} success)[/bold green]")
        else:
            console.print('  [bold red]No effective strategies found for this group[/bold red]')
