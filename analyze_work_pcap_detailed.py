...
try:
    ...
    print("❌ Требуется установка: pip install scapy")
    print(f"Ошибка: {e}")
    sys.exit(1)
                ...
                try:
                    ...
                    if len(payload) > 100:
                        ...
                        for i in range(40, len(payload) - 20):
                            if (
                                payload[i] == 0x00
                                and payload[i + 1] == 0x00  # SNI extension type
                                and i + 9 < len(payload)
                            ):
                                ...
                                try:
                                    ...
                                    if name_len_pos + 2 < len(payload):
                                        name_len = int.from_bytes(
                                            payload[name_len_pos : name_len_pos + 2],
                                            "big",
                                        )
                                        ...
                                        if (
                                            name_len > 0
                                            and name_len < 300
                                            and name_start + name_len <= len(payload)
                                        ):
                                            sni = payload[
...
def analyze_work_pcap():
        ...
        if packet.haslayer(IP):
            ...
            if packet.haslayer(TCP):
                ...
                if tcp_layer.flags & 0x08:  # PSH flag
                    if (
                        packet_size < 100
                    ):  # Small packets with PSH might indicate splitting
    ...
    for domain, stats in sorted(domain_stats.items()):
        if stats["packets"] > 5:  # Show only domains with significant traffic
            ...
            avg_size = (
                sum(stats["packet_sizes"]) / len(stats["packet_sizes"])
            ...
            if len(stats["timestamps"]) > 1:
                ...
                quick_succession = sum(
                    1 for interval in intervals if interval < 0.001
                )  # < 1ms
                if quick_succession > 0:
                    print(
                        f"   ⚡ Быстрые пакеты (< 1мс): {quick_succession} - возможное разделение"
                    )
    ...
    if suspicious_patterns:
        ...
        for pattern in suspicious_patterns[:5]:
            print(
                f"   📦 Пакет #{pattern['packet_num']}: {pattern['type']} "
    ...
    if fragmented_packets:
        for frag in fragmented_packets[:10]:
            print(
                f"   📦 Пакет #{frag['packet_num']}: {frag['dst_ip']}:{frag['dst_port']} "
