...
def profile_validation_report_creation(
    profiler: PerformanceProfiler, orchestrator: CLIValidationOrchestrator
):
    ...
    with profiler.profile_operation("create_validation_report") as metrics:
        report = orchestrator.create_validation_report(
            pcap_validation=pcap_validation, baseline_comparison=comparison
        )
...
def profile_output_formatting(
    ...
    with profiler.profile_operation("format_output_plain") as metrics:
        output = orchestrator.format_validation_output(
            report, use_colors=False, verbose=False
        )
    ...
    with profiler.profile_operation("format_output_verbose") as metrics:
        output = orchestrator.format_validation_output(
            report, use_colors=True, verbose=True
        )
...
def analyze_optimization_opportunities(profiler: PerformanceProfiler):
    ...
    sorted_metrics = sorted(
        profiler.metrics, key=lambda m: m.execution_time, reverse=True
    )

    logger.info("\nTop 5 slowest operations:")
    for i, metric in enumerate(sorted_metrics[:5], 1):
        logger.info(f"  {i}. {metric.operation_name}: {metric.execution_time:.4f}s")

    # Analyze formatting operations
    format_metrics = [
        m for m in profiler.metrics if "format_output" in m.operation_name
    ]
    if format_metrics:
    ...
    if len(startup_metrics) >= 2:
        no_val = next(
            (m for m in startup_metrics if "no_validation" in m.operation_name), None
        )
        with_val = next(
            (m for m in startup_metrics if "with_validation" in m.operation_name), None
        )

        if no_val and with_val:
            overhead = with_val.execution_time - no_val.execution_time
            overhead_pct = (
                (overhead / no_val.execution_time) * 100
                if no_val.execution_time > 0
                else 0
            )
...
def main():
    ...
    report = profiler.generate_report(
        "cli_validation_orchestrator", include_recommendations=True
    )
