...
def analyze_attack_signature(attack_class: type) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'required_params': [p for p, info in params.items() if info['default'] is None],
            'optional_params': [p for p, info in params.items() if info['default'] is not None]
        }
...
def main():
    ...
    print("=" * 80)
    no_params = [name for name, info in analysis_results.items() 
    ...
    print("=" * 80)
    many_params = [(name, info) for name, info in analysis_results.items() 
                   if info.get('param_count', 0) > 5]
    for attack, info in sorted(many_params, key=lambda x: x[1]['param_count'], reverse=True):
        params = list(info['parameters'].keys())
        print(f"  - {attack} ({info['param_count']} params): {', '.join(params[:5])}...")
    ...
    if conflicts:
        for test_param, conflict_list in conflicts.items():
            ...
            for conflict in conflict_list:
                print(f"  - {conflict['attack']}: available params = {conflict['available_params']}")
    else:
