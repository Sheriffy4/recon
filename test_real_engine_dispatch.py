...
def test_primitives_methods():
    ...
    try:
        ...
        tests = [
            ("apply_fakeddisorder", lambda: techniques.apply_fakeddisorder(test_payload, 3, 3, ["badsum"])),
            ("apply_seqovl", lambda: techniques.apply_seqovl(test_payload, 5, 20, 3, ["badsum"])),
            ("apply_multidisorder", lambda: techniques.apply_multidisorder(test_payload, [1, 5, 10], ["badsum"], 3)),
            ("apply_disorder", lambda: techniques.apply_disorder(test_payload, 7, False)),
            ("apply_multisplit", lambda: techniques.apply_multisplit(test_payload, [3, 6, 9], [])),
            ("apply_fake_packet_race", lambda: techniques.apply_fake_packet_race(test_payload, 2, ["badsum"]))
        ]
