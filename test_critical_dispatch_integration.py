...
class MockPacket:
    ...
    def _create_tls_clienthello(self):
        ...
        tls_header = b'\x16\x03\x01\x02\x00'  # TLS Record: Handshake, version 3.1, length 512
        ...
        sni_extension = b'\x00\x00\x00\x0c\x00\x0a\x00\x00\x07example\x03com\x00'  # SNI extension
        
        return tls_header + handshake_header + version + random_bytes + session_id + cipher_suites + compression + extensions_length + sni_extension
...
def test_dispatch_integration():
    ...
    test_scenarios = [
        ...
        },
        {
            ...
            "strategy": {
                "type": "seqovl", 
                "params": {"split_pos": 5, "overlap_size": 20, "ttl": 3, "fooling": ["badsum"]}
            },
        ...
        },
        {
            ...
            "strategy": {
                "type": "multidisorder",
                "params": {"positions": [1, 5, 10], "ttl": 3, "fooling": ["badsum", "badseq"]}
            },
    ...
    for scenario in test_scenarios:
        ...
        try:
            ...
            with patch.object(engine.techniques, scenario['expected_method']) as mock_method:
                ...
                engine.apply_bypass(packet, mock_divert, scenario['strategy'], forced=True)
                ...
                if mock_method.called:
                    ...
                    if call_args:
                        ...
                        print(f"  📋 Параметры: args={len(args)}, kwargs={list(kwargs.keys())}")
                        
                        # Специфичные проверки для каждого типа
                        if scenario['name'] == 'seqovl' and len(args) >= 3:
                            overlap_size = args[2] if len(args) > 2 else kwargs.get('overlap_size')
                            if overlap_size == 20:
                                print(f"  ✅ overlap_size корректно передан: {overlap_size}")
                            else:
    ...
    class MockEngine:
        ...
        def apply_bypass(self, packet, w, strategy_task, forced=True):
            ...
            if task_type == "fakeddisorder":
                return self.techniques.apply_fakeddisorder(payload, sp, fake_ttl, params.get("fooling", []))
            elif task_type == "seqovl":
                ovl = int(params.get("overlap_size", 20))
                return self.techniques.apply_seqovl(payload, sp, ovl, fake_ttl, params.get("fooling", []))
            elif task_type == "multidisorder":
                positions = params.get("positions", [1, 5, 10])
                return self.techniques.apply_multidisorder(payload, positions, params.get("fooling", ["badsum"]), fake_ttl)
                ...
                return self.techniques.apply_multisplit(payload, positions, params.get("fooling", []))
            elif task_type == "fake":
                return self.techniques.apply_fake_packet_race(payload, fake_ttl, params.get("fooling", ["badsum"]))
