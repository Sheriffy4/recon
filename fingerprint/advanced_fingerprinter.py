...
class AdvancedFingerprinter:
    ...
    def _heuristic_classification(
        ...
        # >>>>> НОВЫЙ КОД: Используем block_type для более точной классификации <<<<<
        if fingerprint.block_type == "ssl_block":
            # Это очень сильный индикатор РКН-like систем
            return DPIType.ROSKOMNADZOR_DPI, 0.8

        if fingerprint.block_type == "rst" and fingerprint.rst_injection_detected:
            if fingerprint.connection_reset_timing < 100: # Быстрый RST
                return DPIType.ROSKOMNADZOR_TSPU, 0.75
            else:
                return DPIType.FIREWALL_BASED, 0.65

        if fingerprint.block_type == "timeout":
            # Таймауты часто указывают на пассивные или государственные системы
            if fingerprint.dns_hijacking_detected:
                return DPIType.GOVERNMENT_CENSORSHIP, 0.7
            else:
                return DPIType.ISP_TRANSPARENT_PROXY, 0.6
        # >>>>> КОНЕЦ НОВОГО КОДА <<<<<

        # Russian DPI patterns (остаются как дополнительное подтверждение)
        if (
