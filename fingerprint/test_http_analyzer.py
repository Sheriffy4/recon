...
class TestHTTPAnalyzer:
    ...
    async def test_make_request_connection_reset(self, analyzer):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            mock_session = MagicMock()
            # Use a generic exception that contains "Connection reset" in the message
    ...
    async def test_http_method_restrictions(self, analyzer):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            # Set up all HTTP method mocks
            mock_session.get = AsyncMock(
                side_effect=lambda *args, **kwargs: mock_request_side_effect(
                    "GET", *args, **kwargs
                )
            )
    ...
    async def test_content_inspection_detection(self, analyzer):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            # Mock responses: block content with keywords
            def mock_post_side_effect(*args, **kwargs):
                ...
                # Block content with "forbidden" keyword
    ...
    async def test_redirect_injection_detection(self, analyzer):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            # Mock responses: redirect for certain paths
            def mock_get_side_effect(url, *args, **kwargs):
                mock_response = MagicMock()
                mock_response.__aenter__ = AsyncMock(return_value=mock_response)
                mock_response.__aexit__ = AsyncMock(return_value=None)

                # Redirect for blocked paths
                if "/blocked" in url or "/forbidden" in url:
                    mock_response.status = 302
                    mock_response.headers = {
                        "Location": "https://government-warning.com/blocked",
    ...
    async def test_response_modification_detection(self, analyzer):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            # Mock responses with injected content
            mock_response = MagicMock()
            mock_response.status = 200
            mock_response.headers = {
                "Content-Type": "text/html",
                "X-Blocked-By": "DPI-System",  # Suspicious header
            }
            mock_response.text = AsyncMock(
                return_value="<html><body>This site is blocked by government</body></html>"
            )
            mock_response.__aenter__ = AsyncMock(return_value=mock_response)
            mock_response.__aexit__ = AsyncMock(return_value=None)
    ...
    async def test_connection_behavior_analysis(self, analyzer):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            # Mock responses: different behavior for keep-alive vs close
            def mock_get_side_effect(*args, **kwargs):
                headers = kwargs.get("headers", {})
    ...
    async def test_encoding_handling_analysis(self, analyzer):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            # Mock responses: block chunked encoding
            def mock_request_side_effect(method, *args, **kwargs):
                headers = kwargs.get("headers", {})
    ...
    def test_calculate_reliability_score(self, analyzer):
        ...
        assert score > 0.5  # Should be reasonably high with mostly successful requests
