# recon/core/fingerprint/online_learning_demo.py
"""
...
def create_sample_metrics(dpi_type: str = "ROSKOMNADZOR_TSPU") -> Dict[str, Any]:
    ...
    # Modify metrics based on DPI type
...
def simulate_classification_with_errors(true_type: str, confidence: float) -> tuple:
    ...
    # Simulate classification errors based on confidence
    if confidence > 0.9:
        error_rate = 0.05
    elif confidence > 0.8:
        error_rate = 0.1
...
def demo_basic_online_learning():
    ...
    # Create ML classifier and train it
    ...
    # Simulate incoming predictions and feedback
    ...
    for i in range(30):
        ...
        predicted_type, confidence = simulate_classification_with_errors(
            true_type, random.uniform(0.6, 0.95)
        )

        # Add to online learning system
        learned = online_learning.add_learning_example(
            metrics=metrics,
        ...
        if learned:
            print(
                f"   Example {i+1}: {predicted_type} -> {true_type} "
                f"(conf: {confidence:.2f}) [LEARNED]"
            )
        else:
            print(
                f"   Example {i+1}: {predicted_type} -> {true_type} "
                f"(conf: {confidence:.2f}) [SKIPPED]"
            )
...
def demo_learning_modes():
    ...
    for mode in modes:
        ...
        for i in range(total_count):
            # Generate examples with varying confidence
            confidence = random.uniform(0.5, 0.95)
            true_type = random.choice(["ROSKOMNADZOR_TSPU", "COMMERCIAL_DPI"])
            predicted_type, conf = simulate_classification_with_errors(
                true_type, confidence
            )
...
def demo_ab_testing():
    ...
    # Create control and test models
    ...
    # Train test model with slightly different data
    test_trainer = ModelTrainer("demo_test_model.joblib")
    test_trainer.train_model_with_evaluation(
        training_data[10:50]
    ...
    ab_config = ABTestConfig(
        ...
        control_model_path="demo_control_model.joblib",
        test_model_path="demo_test_model.joblib",
        traffic_split=0.5,  # 50/50 split
        min_samples=10,  # Small for demo
    ...
    # Simulate traffic and collect results
    print("\n3. Simulating traffic...")
    dpi_types = ["ROSKOMNADZOR_TSPU", "COMMERCIAL_DPI", "GOVERNMENT_CENSORSHIP"]

    for i in range(25):  # More than min_samples to trigger conclusion
        true_type = random.choice(dpi_types)
        metrics = create_sample_metrics(true_type)

        # Classify with A/B test
        predicted_type, confidence, model_used = online_learning.classify_with_ab_test(
            metrics
        )

        # Simulate some classification errors
        ...
        # Record result
        online_learning.record_ab_test_result(
            metrics=metrics,
            predicted_type=predicted_type,
        ...
        print(
            f"   Request {i+1}: {model_used} model -> {predicted_type} "
            f"(actual: {actual_type}, conf: {confidence:.2f})"
        )
...
def demo_performance_monitoring():
    ...
    # Create online learning system with sensitive retraining threshold
    online_learning = OnlineLearningSystem(
        ml_classifier=classifier,
        learning_mode=LearningMode.MODERATE,
        performance_window_size=8,  # Small window for demo
        retraining_threshold=0.3,  # Sensitive threshold for demo
    )

    print("1. Establishing baseline performance...")

    # Establish good baseline with correct predictions
    for i in range(8):
        ...
        online_learning.add_learning_example(
            metrics=metrics,
            predicted_type=true_type,  # All correct
            actual_type=true_type,
            confidence=0.85,
            source="automatic",
        )
    ...
    for i in range(8):
        true_type = "ROSKOMNADZOR_TSPU"
        predicted_type = "COMMERCIAL_DPI"  # All wrong
        metrics = create_sample_metrics(true_type)
...
def main():
    ...
    try:
        ...
        if online_learning:
            ...
            print(
                f"  Total examples: {final_stats['statistics']['total_examples_received']}"
            )
            print(
                f"  Examples learned from: {final_stats['statistics']['examples_learned_from']}"
            )
            print(
                f"  Retraining events: {final_stats['statistics']['retraining_events']}"
            )
            print(
                f"  A/B tests completed: {final_stats['statistics']['ab_tests_completed']}"
            )
        ...
        # Clean up demo files
        demo_files = [
            "demo_online_classifier.joblib",
            "demo_modes_classifier.joblib",
            "demo_control_model.joblib",
            "demo_test_model.joblib",
            "demo_performance_classifier.joblib",
