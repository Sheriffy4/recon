...
class OnlineLearningIntegrator:
    ...
    def classify_with_learning(
        ...
        if self.online_learning and self.online_learning.active_ab_test:
            ...
            LOG.debug(
                f"A/B test classification: {dpi_type} (confidence: {confidence:.3f}, "
                f"model: {model_used})"
            )
        else:
    ...
    def add_user_feedback(self, feedback: FeedbackData) -> bool:
        ...
        try:
            ...
            if learned:
                ...
                LOG.info(
                    f"User feedback processed: {predicted_type} -> {feedback.user_reported_type} "
    ...
    def add_validation_result(
        ...
        try:
            ...
            if learned:
                self.stats["learning_examples_added"] += 1
                LOG.debug(
                    f"Validation result processed: {predicted_type} -> {validated_type} "
                    f"for {target}"
                )
