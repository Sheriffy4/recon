# recon/cli_simple.py - Упрощенная рабочая версия на основе v111
...
# Попробуем импортировать из старой структуры
try:
...
async def run_simple_mode(args):
    ...
    console.print(
        Panel(
            "[bold cyan]Recon: Simple DPI Bypass Finder[/bold cyan]",
            title="Simple Mode",
            expand=False,
        )
    )
    ...
    if not OLD_STRUCTURE_AVAILABLE:
        console.print(
            "[bold red]Error:[/bold red] Required components from old structure are not available."
        )
    ...
    default_domains = (
        [args.target] if not args.domains_file else [config.DEFAULT_DOMAIN]
    )
    ...
    if args.strategy:
        ...
        # Создаем простой фингерпринт для генератора
    ...
    if not working_strategies:
        ...
        try:
            ...
            console.print(
                "[green]💾 Best strategy saved to 'best_strategy.json'[/green]"
            )
        ...
        console.print(
            "Служба автоматически подхватит найденную стратегию из 'best_strategy.json'."
        )
        console.print("=" * 50 + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Recon: Simple DPI Bypass Finder (based on working v111)",
    ...
    parser.add_argument(
        ...
        help="Target host (e.g., rutracker.org) or a file with domains (if -d is used).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--domains-file",
        action="store_true",
        help="Treat 'target' as a file with a list of domains.",
    )
    parser.add_argument(
        "-c",
        "--count",
