...
class TCPSessionAwareAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        if current_offset < len(payload):
            ...
            segments.append(
                (chunk2, current_offset, {"flags": 0x08, "delay_ms": 10})  # PSH only
            )
        ...
        if current_offset < len(payload):
            ...
            segments.append(
                (chunk3, current_offset, {"flags": 0x38, "delay_ms": 3})  # PSH+ACK+URG
            )
...
class SequenceManipulationAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        segments.append((middle_chunk, middle_start, {"delay_ms": 5, "ttl": 64}))

        # Segment 2: Send end part second
        end_chunk = payload[middle_end:]

        segments.append((end_chunk, middle_end, {"delay_ms": 10, "ttl": 64}))

        # Segment 3: Send beginning part last (creates reordering)
        begin_chunk = payload[:middle_start]

        segments.append((begin_chunk, 0, {"delay_ms": 15, "ttl": 64}))
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        for i in range(0, len(payload), chunk_size):
            ...
            if packet_id == 1:
                ...
                options = {"flags": 0x38, "delay_ms": packet_id * 3}  # PSH+ACK+URG
...
def demonstrate_tcp_session_context():
    ...
    if result2.has_segments():
        for i, (payload_data, seq_offset, options) in enumerate(result2.segments):
            print(
                f"   Segment {i+1}: {len(payload_data)} bytes at offset {seq_offset}, options: {options}"
            )
    print()
