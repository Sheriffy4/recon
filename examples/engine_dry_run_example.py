...
class EngineDryRunDemo:
    ...
    def __init__(self):
        ...
        self.config = EngineConfig(
            debug=True, timeout=30.0, packet_buffer_size=65535, log_packets=True
        )
    ...
    def demonstrate_multiple_attack_scenarios(self):
        ...
        for scenario_key, scenario_result in results["results"].items():
            ...
            if scenario_result.get("simulation_time_ms"):
                logger.info(
                    f"    Simulation time: {scenario_result['simulation_time_ms']:.3f}ms"
                )
    ...
    async def demonstrate_performance_analysis(self):
        ...
        for i in range(num_iterations):
            ...
            logger.info(
                f"  Iteration {i+1}: {result.status.value} ({iteration_time:.3f}ms total)"
            )
        ...
        logger.info(
            f"Average simulation time: {sum(simulation_times) / len(simulation_times):.3f}ms"
        )
        ...
        logger.info(
            f"Simulation time variance: {max(simulation_times) - min(simulation_times):.3f}ms"
        )
    ...
    async def demonstrate_validation_testing(self):
        ...
        for scenario in test_scenarios:
            ...
            try:
                ...
                logger.info(
                    f"  Validation: {'✓' if validation_result['segments_valid'] else '✗'}"
                )
        ...
        logger.info(
            f"Validation testing summary: {successful_validations}/{len(test_scenarios)} scenarios passed"
        )
    ...
    async def demonstrate_engine_integration(self):
        ...
        if "engine_stats" in result.metadata:
            ...
            logger.info(
                f"  - Packets processed: {engine_stats.get('packets_processed', 0)}"
            )
        ...
        logger.info("Current engine statistics:")
        ...
        logger.info(
            f"  - Total sessions: {diag_stats['diagnostic_system']['total_sessions']}"
        )
        logger.info(
            f"  - Segments diagnosed: {diag_stats['diagnostic_system']['total_segments_processed']}"
        )
