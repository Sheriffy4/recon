...
def demonstrate_tcp_timing_attacks():
    ...
    for variant_name, attack in timing_variants:
        ...
        for i, (segment_payload, seq_offset, options) in enumerate(result._segments):
            print(f"    Segment {i+1}: {len(segment_payload)} bytes, offset={seq_offset}, delay={options['delay_ms']:.1f}ms")
...
def demonstrate_urgent_pointer_attacks():
    ...
    for variant_name, attack in urgent_variants:
        ...
        for i, (segment_payload, seq_offset, options) in enumerate(result._segments):
            ...
            print(f"    Segment {i+1}: {len(segment_payload)} bytes, URG={is_urgent}, ptr={urgent_ptr}")
            if is_urgent:
                print(f"      Urgent data detected in segment")
...
def demonstrate_window_scaling_attacks():
    ...
    payload = b"PUT /api/config HTTP/1.1\r\nHost: config.example.com\r\nContent-Length: 200\r\n\r\n" + b"config_data=" + b"X" * 180
...
def demonstrate_payload_obfuscation_attacks():
    ...
    obfuscation_variants = [
        ...
        ("Custom Multi-Method", create_payload_obfuscation_attack(
    ...
    for variant_name, attack in obfuscation_variants:
        ...
        for i, (segment_payload, seq_offset, options) in enumerate(result._segments):
            ...
            preview = segment_payload[:30] if len(segment_payload) > 30 else segment_payload
            
            print(f"    Segment {i+1}: {len(segment_payload)} bytes at offset {seq_offset}")
            print(f"      Preview: {preview}...")
        
        print(f"  Total obfuscated size: {total_obfuscated_size} bytes (vs {len(payload)} original)")
...
def demonstrate_attack_combinations():
    ...
    for combo_name, attacks in combinations:
        ...
        for attack in attacks:
            ...
            print(f"  {attack.name}: {len(result._segments)} segments, {effectiveness:.1%} effective")
        
        print(f"  Combined: {total_segments} total segments, {combined_effectiveness:.1%} combined effectiveness")
    ...
    for scenario_name, context in scenarios:
        ...
        for attack_name, attack in test_attacks:
            try:
                ...
                print(f"  {attack_name}: {result.status}, {len(result._segments)} segments, {effectiveness:.1%}")
...
def demonstrate_performance_analysis():
    ...
    payload = b"GET /performance-test HTTP/1.1\r\nHost: test.example.com\r\n\r\n" + b"X" * 1000
    ...
    performance_tests = [
        ...
        ("Complex Obfuscation", create_mixed_obfuscation_attack(per_segment_obfuscation=True)),
    ...
    for test_name, attack in performance_tests:
        ...
        avg_segment_size = total_payload_size / len(result._segments) if result._segments else 0
