...
from core.bypass.engines.native_pydivert_engine import (
...
class MockSegmentAttack:
    ...
    async def execute(self, context: AttackContext) -> AttackResult:
        ...
        if self.name == "fakeddisorder":
            ...
            return AttackResult(
                status=AttackStatus.FAILED, error_message="Unknown attack"
            )
    ...
    def _create_fakeddisorder_segments(self, payload: bytes) -> AttackResult:
        ...
        if split_pos >= len(payload):
            return AttackResult(
                status=AttackStatus.FAILED, error_message="Payload too small"
            )
...
def demonstrate_enhanced_context_creation():
    ...
    try:
        ...
        print(
            f"  Connection: {context.src_ip}:{context.src_port} -> {context.dst_ip}:{context.dst_port}"
        )
        print(f"  TCP Seq: {context.tcp_seq}, ACK: {context.tcp_ack}")
        print(
            f"  TCP Flags: 0x{context.tcp_flags:02x} ({context.get_tcp_flags_string()})"
        )
...
def demonstrate_segments_orchestration():
    ...
    for i, (payload_data, seq_offset, options) in enumerate(segments):
        print(
            f"  Segment {i+1}: {len(payload_data)} bytes, offset={seq_offset}, options={options}"
        )
    ...
    for i, segment in enumerate(segments):
        try:
            ...
            print(
                f"  Packet {i+1}: {packet_info.packet_size} bytes, "
...
def demonstrate_attack_integration():
    ...
    # Create context
    context = AttackContext(dst_ip="192.168.1.100", dst_port=443, payload=test_payload)
    ...
    async def test_attacks():
        ...
        if result1.status == AttackStatus.SUCCESS and result1.has_segments():
            ...
            for i, (payload_data, seq_offset, options) in enumerate(segments):
                print(
                    f"     Segment {i+1}: {len(payload_data)} bytes, offset={seq_offset}, {options}"
                )
        else:
...
def demonstrate_timing_control():
    ...
    for i, (payload_data, seq_offset, options) in enumerate(timing_segments):
        ...
        total_delay += delay
        print(
            f"  Segment {i+1}: {len(payload_data)} bytes, delay={delay}ms, cumulative={total_delay}ms"
        )

    print(f"\nTotal execution time (delays only): {total_delay}ms")
    print(
        "Note: In real execution, packet construction and transmission time would be added."
    )
    print()
...
def demonstrate_error_handling():
    ...
    builder = SegmentPacketBuilder()
    context = AttackContext(dst_ip="192.168.1.100", dst_port=443, tcp_seq=1000000)
    ...
    for case in error_cases:
        ...
        try:
            ...
            is_valid, error_msg = validate_segments_for_building(
                case["segments"], context
            )
            ...
            if is_valid:
                ...
                for segment in case["segments"]:
                    packet_info = builder.build_segment(
                        segment[0], segment[1], segment[2], context
                    )
