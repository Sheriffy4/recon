...
def simulate_packet_construction(
...
def demonstrate_caching_benefits():
    ...
    print(
        f"Testing with {len(repeated_segments)} segments ({len(base_segments)} unique)"
    )
    ...
    start_time = time.time()
    metrics_no_cache = optimizer_no_cache.optimize_segment_execution(
        repeated_segments, execution_func_no_cache
    )
    time_no_cache = time.time() - start_time

    # Test with caching
    config_with_cache = OptimizationConfig(
        enable_packet_caching=True, max_cache_size=10
    )
    ...
    start_time = time.time()
    metrics_with_cache = optimizer_with_cache.optimize_segment_execution(
        repeated_segments, execution_func_with_cache
    )
...
def demonstrate_memory_pooling():
    ...
    pool_stats = optimizer_with_pool.get_performance_stats().get(
        "memory_pool_stats", {}
    )
...
def demonstrate_performance_benchmarking():
    ...
    benchmark_results = optimizer.benchmark_performance(
        segments, execution_func, iterations=5
    )
    ...
    print(
        f"  Total segments processed: {benchmark_results['total_segments_processed']}"
    )
    ...
    print(
        f"  Throughput: {benchmark_results['throughput_segments_per_sec']:.1f} segments/sec"
    )
    ...
    if profiler_stats:
        ...
        for operation, stats in profiler_stats.items():
            print(
                f"    {operation}: {stats['avg_time']*1000:.2f}ms avg ({stats['count']} calls)"
            )
...
def demonstrate_real_world_scenario():
    ...
    for i in range(10):
        ...
        http_segments.append(
            (payload, i * len(payload), {"ttl": 64, "delay_ms": i * 10})
        )
    ...
    for i in range(5):
        ...
        http_segments.append(
            (payload, 1000 + i * len(payload), {"ttl": 63, "delay_ms": 50})
        )
    ...
    if "cache_stats" in stats:
        ...
        print(
            f"  Cache: {cache['hits']} hits, {cache['misses']} misses ({cache['hit_rate']:.1%} hit rate)"
        )

    if "memory_pool_stats" in stats:
        pool = stats["memory_pool_stats"]
        print(
            f"  Memory Pool: {pool['reused']} reused, {pool['allocated']} allocated ({pool['reuse_rate']:.1%} reuse rate)"
        )
    ...
    if "profiler_stats" in stats:
        ...
        for operation, op_stats in stats["profiler_stats"].items():
            print(
                f"    {operation}: {op_stats['avg_time']*1000:.2f}ms avg ({op_stats['count']} calls)"
            )
