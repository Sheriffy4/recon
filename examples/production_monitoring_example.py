    ...
    try:
        ...
        if alerts['active_alerts'] > 0:
            ...
            for alert in alerts['recent_alerts'][-3:]:  # Show last 3 alerts
                ...
                print(f"  Type: {alert['type']}, Time: {time.ctime(alert['timestamp'])}")
        else:
        ...
        if monitoring_dir.exists():
            ...
            if dashboard_file.exists():
                ...
                try:
                    ...
                    print(f"  Dashboard status: {dashboard_data.get('system_status', 'unknown')}")
                    print(f"  Active alerts: {dashboard_data.get('active_alerts_count', 0)}")
                except Exception as e:
...
async def production_deployment_monitoring_example():
    ...
    try:
        ...
        if 'error' not in deployment_summary:
            ...
            print(f"Average response time: {deployment_summary['avg_response_time_ms']:.1f}ms")
            print(f"Maximum response time: {deployment_summary['max_response_time_ms']:.1f}ms")
            print(f"Average memory usage: {deployment_summary['avg_memory_usage_mb']:.1f}MB")
            print(f"Maximum memory usage: {deployment_summary['max_memory_usage_mb']:.1f}MB")
        ...
        if alert_summary['critical_alerts'] == 0 and deployment_summary.get('avg_success_rate', 0) > 0.9:
