...
def demonstrate_attack_variants():
    ...
    for variant_name, attack in variants:
        ...
        print(f"  Split Position: {result.metadata['split_position']} ({result.metadata['split_ratio']:.1%})")
        print(f"  Fake Delay: {result.metadata['config']['fake_delay_ms']}ms")
        print(f"  Effectiveness: {effectiveness:.1%}")
        print(f"  Checksum Corruption: {result.metadata['config']['corrupt_fake_checksum']}")
        print()
...
def demonstrate_payload_analysis():
    ...
    payloads = [
        ...
        ("HTTP POST", b"POST /api/data HTTP/1.1\r\nHost: api.com\r\nContent-Length: 15\r\n\r\n{\"key\":\"value\"}"),
        ("TLS Handshake", b"\x16\x03\x01\x00\x50\x01\x00\x00\x4c\x03\x03" + b"\x00" * 32 + b"blocked.com"),
    ...
    for payload_type, payload in payloads:
        ...
        print(f"  Fake:     {fake_payload[:50]}{'...' if len(fake_payload) > 50 else ''}")
...
def demonstrate_timing_analysis():
    ...
    timing_configs = [
        ("Fast", FakedDisorderConfig(fake_delay_ms=5, part2_delay_ms=2, part1_delay_ms=1)),
        ("Medium", FakedDisorderConfig(fake_delay_ms=15, part2_delay_ms=8, part1_delay_ms=5)),
        ("Slow", FakedDisorderConfig(fake_delay_ms=30, part2_delay_ms=15, part1_delay_ms=10))
    ]
    
    for config_name, config in timing_configs:
        attack = FakedDisorderAttack(name=f"timing_{config_name.lower()}", config=config)
        ...
        total_delay = config.fake_delay_ms + config.part2_delay_ms + config.part1_delay_ms
...
def demonstrate_validation_and_error_handling():
    ...
    test_cases = [
        ("Valid Context", AttackContext(
            dst_ip="1.2.3.4", dst_port=80,
            payload=b"Valid payload for testing",
            connection_id="valid_test"
        )),
        ("Empty Payload", AttackContext(
            dst_ip="1.2.3.4", dst_port=80,
            payload=b"",
            connection_id="empty_test"
        )),
        ("Short Payload", AttackContext(
            dst_ip="1.2.3.4", dst_port=80,
            payload=b"short",
            connection_id="short_test"
        )),
        ("Invalid TCP Seq", AttackContext(
            dst_ip="1.2.3.4", dst_port=80,
            payload=b"Valid payload for testing",
            tcp_seq=-1,
