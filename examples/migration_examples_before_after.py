        ...
        try:
            ...
            return AttackResult(
                ...
                metadata={
                    ...
                    "segment_count": len(segments),
                    "total_delay_ms": sum(
                        opts.get("delay_ms", 0) for _, _, opts in segments
                    ),
                },
            )
...
class PacketManipulationAttack_Migrated(BaseAttack):
    ...
    def estimate_effectiveness(self, context: AttackContext) -> float:
        ...
        fragment_count = (
            len(context.payload) + self.fragment_size - 1
        ) // self.fragment_size
        if fragment_count > 3:
            base_effectiveness += 0.1
    ...
    def get_attack_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "effectiveness": "high",
            "config": {"base_ttl": self.base_ttl, "fragment_size": self.fragment_size},
            "advantages": [
...
class StateBasedAttack_Migrated(BaseAttack):
        ...
        try:
            ...
            return AttackResult(
                ...
                metadata={
                    ...
                    "requests_seen": self.state["requests_seen"],
                    "pattern_used": (
                        self.state["patterns"][-1] if self.state["patterns"] else "none"
                    ),
                },
            )
    ...
    def _create_aggressive_segments(
        self, payload: bytes
    ) -> List[Tuple[bytes, int, Dict[str, Any]]]:
    ...
    def _create_moderate_segments(
        ...
        for i in range(2):
            ...
            options = {"delay_ms": i * 30, "ttl": 64 - i}

            segments.append((chunk, start, options))

        return segments

    def _create_minimal_segments(
        self, payload: bytes
    ) -> List[Tuple[bytes, int, Dict[str, Any]]]:
...
def demonstrate_timing_attack_migration():
    ...
    for i, (payload, offset, options) in enumerate(migrated_result._segments):
        ...
        print(
            f"    Segment {i+1}: {len(payload)} bytes, offset={offset}, delay={delay}ms"
        )
...
def demonstrate_state_based_migration():
    ...
    for i, payload in enumerate(test_payloads):
        ...
        print(
            f"  Request {i+1}: {pattern} pattern, {len(result.modified_payload)} bytes"
        )
    ...
    for i, payload in enumerate(test_payloads):
        ...
        print(
            f"  Request {i+1}: {pattern} pattern, {segment_count} segments, {effectiveness:.1%} effective"
        )
