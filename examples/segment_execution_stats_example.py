...
def demonstrate_basic_statistics():
    ...
    print(f"  Throughput: {completed_session.throughput_segments_per_sec:.1f} segments/sec")
...
def demonstrate_performance_monitoring():
    ...
    for session_num in range(3):
        ...
        for i in range(segment_count):
            ...
            stats_collector.update_segment_phase(segment_metrics, ExecutionPhase.TIMING, timing_time)
            stats_collector.update_segment_phase(segment_metrics, ExecutionPhase.TRANSMISSION, transmission_time)
            
            # Simulate some failures
            success = not (i == segment_count - 1 and session_num == 2)  # Last segment of last session fails
    ...
    print(f"  Global throughput: {global_stats.global_throughput_segments_per_sec:.1f} segments/sec")
    print(f"  Average timing accuracy: {global_stats.global_avg_timing_accuracy_percent:.1f}%")
...
def demonstrate_performance_analysis():
    ...
    print(f"  Average execution time: {recent_perf.get('avg_execution_time_ms', 0):.2f}ms")
    print(f"  Throughput: {recent_perf.get('throughput_segments_per_sec', 0):.1f} segments/sec")
    ...
    if "construction_time_ms" in timing_analysis:
        ...
        print(f"  Construction time - Avg: {construction.get('avg', 0):.2f}ms, "
    ...
    if "transmission_time_ms" in timing_analysis:
        ...
        print(f"  Transmission time - Avg: {transmission.get('avg', 0):.2f}ms, "
    ...
    if "timing_accuracy_error_ms" in timing_analysis:
        ...
        print(f"  Timing accuracy error - Avg: {accuracy.get('avg', 0):.3f}ms, "
    ...
    if "ttl_modifications" in mod_analysis:
        ...
        print(f"  TTL modifications: {ttl_mods.get('count', 0)} ({ttl_mods.get('percentage', 0):.1f}%)")
    
    if "checksum_corruptions" in mod_analysis:
        checksum_corr = mod_analysis["checksum_corruptions"]
        print(f"  Checksum corruptions: {checksum_corr.get('count', 0)} ({checksum_corr.get('percentage', 0):.1f}%)")
...
def demonstrate_recent_sessions_analysis():
    ...
    for i, session in enumerate(recent_sessions):
        ...
        print(f"    Segments: {session.total_segments} (Success: {session.successful_segments})")
...
def demonstrate_engine_integration():
    ...
    print("3. Phase timing is automatically recorded (validation, construction, timing, transmission)")
    ...
    print("- engine.get_segment_execution_statistics() - Get detailed segment statistics")
    ...
    print("- engine.get_diagnostic_statistics() - Get diagnostic and segment statistics")
...
def demonstrate_monitoring_dashboard():
    ...
    # Overview section
    print("\\n🔍 OVERVIEW")
    print(f"  Sessions: {global_stats.active_sessions} active, {global_stats.completed_sessions} completed")
    print(f"  Segments: {global_stats.total_segments_processed} processed, {global_stats.total_successful_segments} successful")
    ...
    print("\\n⚡ PERFORMANCE")
    recent_perf = performance_summary.get("recent_performance", {})
    print(f"  Throughput: {global_stats.global_throughput_segments_per_sec:.1f} segments/sec")
    ...
    print(f"  Window Size Modifications: {global_stats.total_window_size_modifications}")
    
    # Recent activity section
    print("\\n📈 RECENT ACTIVITY")
    for i, session in enumerate(recent_sessions[-3:]):  # Last 3 sessions
        status_icon = "✅" if session.success_rate_percent == 100 else "⚠️" if session.success_rate_percent > 50 else "❌"
        print(f"  {status_icon} {session.session_id}: {session.total_segments} segments, "
    ...
    if global_stats.global_success_rate_percent < 90:
        alerts.append(f"Low success rate: {global_stats.global_success_rate_percent:.1f}%")
    
    if global_stats.global_avg_timing_accuracy_percent < 85:
        alerts.append(f"Poor timing accuracy: {global_stats.global_avg_timing_accuracy_percent:.1f}%")
    
    if global_stats.global_throughput_segments_per_sec < 10:
        alerts.append(f"Low throughput: {global_stats.global_throughput_segments_per_sec:.1f} segments/sec")
