...
class SegmentDiagnosticsDemo:
    ...
    def __init__(self):
        ...
        configure_segment_diagnostics(
            detailed_logging=True, max_events_per_segment=50, max_sessions_history=10
        )
    ...
    def demonstrate_basic_segment_logging(self):
        ...
        logger.info(
            f"Session completed with {summary.successful_segments}/{summary.total_segments} successful segments"
        )
    ...
    def demonstrate_multi_segment_session(self):
        ...
        for config in segments_config:
            ...
            if config["options"].get("delay_ms", 0) > 0:
                ...
                self.diagnostic_logger.log_timing_phase(
                    segment_data, timing_measurement
                )
            ...
            logger.info(
                f"Segment {config['id']} {'succeeded' if config['success'] else 'failed'}"
            )

        # End session and analyze results
        summary = self.diagnostic_logger.end_session(session_id)

        logger.info("Multi-segment session completed:")
    ...
    def demonstrate_error_scenarios(self):
        ...
        logger.info("Error scenarios completed:")
        logger.info(f"  - Total segments: {summary.total_segments}")
    ...
    def demonstrate_performance_analysis(self):
        ...
        logger.info(
            f"  - Min/Max segment time: {summary.min_segment_time_ms:.1f}ms / {summary.max_segment_time_ms:.1f}ms"
        )
    ...
    def demonstrate_global_statistics(self):
        ...
        logger.info(
            f"  - Total segments processed: {final_stats['total_segments_processed']}"
        )
        logger.info(
            f"  - Total execution time: {final_stats['total_execution_time_ms']:.1f}ms"
        )
        logger.info(
            f"  - Average time per session: {final_stats['average_execution_time_per_session_ms']:.1f}ms"
        )
