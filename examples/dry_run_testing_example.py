...
class ExampleSegmentAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        logger.info(
            f"Executing {self.name} with context: {context.dst_ip}:{context.dst_port}"
        )
        ...
        segments = [
            ...
            (
                b"fake_decoy_data",
                0,
                {"ttl": 1, "delay_ms": 15, "flags": 0x18},  # PSH+ACK
            ),
            # Real packet part 1 with checksum corruption
            (
                context.payload[: len(context.payload) // 2],
                0,
                {"ttl": 64, "bad_checksum": True, "delay_ms": 5},
            ),
            # Real packet part 2 with normal settings
            (
                context.payload[len(context.payload) // 2 :],
                len(context.payload) // 2,
                {"ttl": 64, "delay_ms": 2, "window_size": 32768},
            ),
        ]
...
class ExamplePayloadAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        logger.info(
            f"Modified payload from {len(original_payload)} to {len(modified_payload)} bytes"
        )
...
class DryRunTestingDemo:
    ...
    def __init__(self):
        ...
        self.config = IntegrationConfig(
            debug_mode=True, cache_attack_results=False, attack_timeout_seconds=30
        )
    ...
    def create_test_context(
        self, target_ip: str = "1.2.3.4", target_port: int = 443
    ) -> AttackContext:
    ...
    async def demonstrate_segment_attack_dry_run(self):
        ...
        logger.info(
            f"Simulation time: {dry_result.metadata.get('simulation_time_ms', 0):.3f}ms"
        )
    ...
    async def demonstrate_payload_attack_dry_run(self):
        ...
        if dry_result.metadata.get("payload_modified"):
            logger.info(
                f"Original payload size: {dry_result.metadata.get('original_payload_size', 0)} bytes"
            )
            logger.info(
                f"Modified payload size: {dry_result.metadata.get('modified_payload_size', 0)} bytes"
            )
    ...
    async def demonstrate_comparison_dry_vs_real(self):
        ...
        logger.info(
            f"  Both successful: {dry_result.status == real_result.status == AttackStatus.SUCCESS}"
        )

        # Compare segments
        dry_segments = (
            len(dry_result._segments) if hasattr(dry_result, "_segments") else 0
        )
        real_segments = (
            len(real_result._segments) if hasattr(real_result, "_segments") else 0
        )
        logger.info(f"  Segments generated: dry={dry_segments}, real={real_segments}")

        # Dry run specific features
        logger.info(
            f"  Dry run validation: {'✓' if dry_result.metadata.get('segments_valid') else '✗'}"
        )
        logger.info(
            f"  Dry run analysis: {'✓' if 'segment_analysis' in dry_result.metadata else '✗'}"
        )
    ...
    async def demonstrate_validation_testing(self):
        ...
        test_scenarios = [
            ...
            },
            {
                "name": "Invalid IP",
                "context": AttackContext(
                    dst_ip="invalid_ip", dst_port=443, payload=b"test", protocol="tcp"
                ),
        ...
        for scenario in test_scenarios:
            ...
            try:
                ...
                logger.info(
                    f"  Validation: {'✓' if result.metadata.get('segments_valid', True) else '✗'}"
                )

                if result.metadata.get("validation_errors"):
                    logger.info(
                        f"  Errors: {len(result.metadata['validation_errors'])}"
                    )
    ...
    async def demonstrate_performance_analysis(self):
        ...
        for i in range(num_runs):
            ...
            logger.info(
                f"  Run {i+1}: {result.metadata.get('simulation_time_ms', 0):.3f}ms"
            )
        ...
        logger.info(
            f"  Average segments per run: {dry_run_stats['average_segments_per_run']:.1f}"
        )
        logger.info(
            f"  Validation error rate: {dry_run_stats['validation_error_rate']:.2%}"
        )
