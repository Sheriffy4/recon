...
def demonstrate_ttl_manipulation():
    ...
    for ttl in ttl_values:
        ...
        print(f"   TTL {ttl}: packet size {packet_info.packet_size}, actual TTL {actual_ttl}")
...
def demonstrate_sequence_offsets():
    ...
    for i, offset in enumerate(offsets):
        ...
        print(f"   Chunk {i}: offset {offset}, expected seq {expected_seq}, actual seq {actual_seq}")
...
def demonstrate_batch_building():
    ...
    for i, info in enumerate(packet_infos):
        print(f"     Packet {i+1}: {info.packet_size} bytes, TTL={info.ttl}, "
...
def demonstrate_packet_analysis():
    ...
    print(f"     Acknowledgment: {struct.unpack('!I', packet[tcp_start+8:tcp_start+12])[0]}")
    print(f"     Flags: 0x{packet[tcp_start+13]:02x}")
    print(f"     Window: {struct.unpack('!H', packet[tcp_start+14:tcp_start+16])[0]}")
    print(f"     Checksum: 0x{struct.unpack('!H', packet[tcp_start+16:tcp_start+18])[0]:04x}")
    
    # Analyze payload
    payload_start = 40  # 20 IP + 20 TCP
    actual_payload = packet[payload_start:]
    print(f"   Payload: {len(actual_payload)} bytes")
    print(f"     Content: {actual_payload[:50]}{'...' if len(actual_payload) > 50 else ''}")
