...
def demonstrate_basic_usage():
    ...
    for i, (segment_payload, seq_offset, options) in enumerate(result._segments):
        ...
        print(f"    Content Preview: {segment_payload[:30]}{'...' if len(segment_payload) > 30 else ''}")
        print()
...
def demonstrate_custom_configuration():
    ...
    # Test payload
    payload = b"GET /blocked-resource HTTP/1.1\r\nHost: blocked.example.com\r\nUser-Agent: CustomAgent/1.0\r\nAccept: */*\r\n\r\n" * 3
...
def demonstrate_segment_analysis():
    ...
    print(f"Analyzing {len(result._segments)} segments from {len(payload)}-byte payload:")
    print()
    ...
    for i, (segment_payload, seq_offset, options) in enumerate(segments_sorted):
        ...
        print(f"  Segment {i+1}: offset={seq_offset:3d}, size={segment_size:2d}, "
    ...
    print(f"Overhead: {total_bytes - len(payload)} bytes ({((total_bytes - len(payload)) / len(payload) * 100):.1f}%)")
...
def demonstrate_timing_patterns():
    ...
    timing_configs = [
        ("Linear", MultisplitConfig(
            split_count=6, base_delay_ms=5.0, delay_variation_ms=2.0, exponential_backoff=False
        )),
        ("Exponential", MultisplitConfig(
            split_count=6, base_delay_ms=2.0, delay_variation_ms=1.0, exponential_backoff=True, backoff_multiplier=1.5
        )),
        ("High Variation", MultisplitConfig(
            split_count=6, base_delay_ms=3.0, delay_variation_ms=8.0, exponential_backoff=False
        )),
        ("Aggressive Backoff", MultisplitConfig(
            split_count=6, base_delay_ms=1.0, delay_variation_ms=0.5, exponential_backoff=True, backoff_multiplier=2.5
        ))
    ]
    
    for config_name, config in timing_configs:
        attack = MultisplitAttack(name=f"timing_{config_name.lower().replace(' ', '_')}", config=config)
        result = attack.execute(context)
...
def demonstrate_effectiveness_analysis():
    ...
    test_scenarios = [
        ...
        ("Large HTTP", b"POST /api/data HTTP/1.1\r\nHost: api.com\r\n" + b"data=" + b"X" * 1000),
        ("Binary Data", b"\x01\x02\x03\x04" * 100),
        ("JSON API", b'{"query": "' + b"SELECT * FROM table WHERE " + b"condition=" * 20 + b'"}'),
    ...
    for scenario_name, payload in test_scenarios:
        ...
        print(f"{scenario_name:<15} {effectiveness_scores[0]:<12} {effectiveness_scores[1]:<12} {effectiveness_scores[2]:<12}")
