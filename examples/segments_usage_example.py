...
class ExampleFakedDisorderAttack(BaseAttack):
    ...
    def execute(self, context: AttackContext) -> AttackResult:
        ...
        segments = [
            ...
            (part2, split_pos, {"delay_ms": 10}),  # Delay to create timing gap
        ...
        while offset < len(payload):
            ...
            segments.append(
                (
                    chunk,
                    offset,
                    {"delay_ms": delay, "bad_checksum": corrupt_checksum, "ttl": ttl},
                )
            )
...
def demonstrate_segments_usage():
    ...
    helper_result = AttackResultHelper.create_segments_result(
        technique_used="helper_demo", segments=segments
    )

    print(
        f"   Helper result has segments: {AttackResultHelper.has_segments(helper_result)}"
    )
