...
class PacketConstructionDemo:
        ...
        for i, segment in enumerate(segments):
            ...
            try:
                packet_info = builder.build_segment(
                    payload_data, seq_offset, options, context
                )

                logger.info(f"Segment {i+1} built:")
                logger.info(f"  - Payload: {len(payload_data)} bytes")
                logger.info(
                    f"  - Sequence: {packet_info.tcp_seq} (offset: {seq_offset})"
                )
    ...
    def demonstrate_precise_timing(self):
        ...
        logger.info(
            f"  - Average accuracy: {stats.get('average_accuracy_percentage', 0):.1f}%"
        )
    ...
    def demonstrate_comprehensive_integration(self):
        ...
        segments = [
            ...
            (b"fake_data", 0, {"ttl": 1, "delay_ms": 15.5, "flags": 0x18}),
            ...
            (
                b"real_part2",
                10,
                {"ttl": 64, "delay_ms": 2.1, "flags": 0x10},  # ACK only
            ),
        ]
        ...
        for i, segment in enumerate(segments):
            ...
            try:
                ...
                if delay_ms > 0:
                    timing_result = timing_controller.delay(delay_ms)
                    logger.info("Timing delay applied:")
                    logger.info(
                        f"  - Requested: {timing_result.requested_delay_ms:.3f}ms"
                    )
        ...
        logger.info(
            f"Builder - Total build time: {builder_stats['total_build_time_ms']:.3f}ms"
        )
        logger.info(
            f"Builder - TTL modifications: {builder_stats['ttl_modifications']}"
        )
        logger.info(
            f"Builder - Checksum corruptions: {builder_stats['checksum_corruptions']}"
        )
        logger.info(f"Timing - Total delays: {timing_stats.get('total_delays', 0)}")
        logger.info(
            f"Timing - Average accuracy: {timing_stats.get('average_accuracy_percentage', 0):.1f}%"
        )
