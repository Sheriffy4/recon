...
from recon.core.monitoring_system import MonitoringSystem, MonitoringConfig, load_monitoring_config
...
class MonitoringCLI:
    ...
    async def start_monitoring(self, args):
        ...
        if args.web_interface:
            try:
                self.web_server = MonitoringWebServer(self.monitoring_system, config.web_interface_port)
                await self.web_server.start()
            except ImportError:
                console.print("[red]❌ Web interface requires aiohttp. Install with: pip install aiohttp[/red]")
    ...
    async def interactive_loop(self):
        ...
        if not RICH_AVAILABLE:
            console.print("[yellow]Interactive mode requires rich. Install with: pip install rich[/yellow]")
            await self.simple_loop()
            return
...
async def main():
    ...
    parser.add_argument("-c", "--config", default="monitoring_config.json", 
                       help="Configuration file (default: monitoring_config.json)")
    
    # Режимы работы
    parser.add_argument("-i", "--interactive", action="store_true", 
                       help="Interactive mode with live status updates")
    parser.add_argument("-w", "--web-interface", action="store_true",
