...
def test_advanced_attacks_integration():
    ...
    try:
        from core.bypass.engine.attack_dispatcher import AttackDispatcher, ADVANCED_ATTACKS_AVAILABLE
        ...
        print(f'Dispatcher initialized with {len(dispatcher._advanced_attacks)} advanced attacks')
        ...
        try:
            result = dispatcher.dispatch_attack('fakeddisorder', test_params, test_payload, test_packet_info)
            print(f'✅ FakeDisorder test: {len(result)} segments generated')
            if result:
                print(f'   First segment: {len(result[0][0])} bytes, offset: {result[0][1]}')
                print(f'   Segment options: {result[0][2]}')
                print(f'   ✅ Asyncio conflict resolved - no event loop errors!')
        ...
        try:
            result = dispatcher.dispatch_attack('multidisorder', test_params, test_payload, test_packet_info)
        ...
        try:
            result = dispatcher.dispatch_attack('seqovl', seqovl_params, test_payload, test_packet_info)
