...
def analyze_work_pcap():
    ...
    try:
        ...
        for i, packet in enumerate(packets):
            if i % 5000 == 0:
                print(f"  Обработано: {i:,}/{len(packets):,} ({i/len(packets)*100:.1f}%)")
        ...
        if client_hello_count > 0 and server_hello_count > 0:
            ...
            print(f"❌ TLS handshake не завершен")
        ...
        if server_hello_count > 0:
            ...
            print(f"   Это означает, что TLS соединения устанавливаются успешно")
        else:
            print(f"❌ Обход не работает - нет Server Hello пакетов")
            print(f"   TLS соединения блокируются на уровне Client Hello")
        
        # Рекомендации
        print(f"\n💡 РЕКОМЕНДАЦИИ")
        
        if server_hello_count > client_hello_count * 0.5:
            print(f"🎉 Система обхода работает эффективно!")
            print(f"   Более 50% TLS соединений успешны")
        elif server_hello_count > 0:
            print(f"⚠️  Система обхода работает частично")
            print(f"   Рекомендуется оптимизировать стратегии")
        else:
            print(f"🔧 Система обхода требует настройки")
            print(f"   Попробуйте другие стратегии или параметры")
