...
class ImprovedTimeoutHandler:
    ...
    def create_ssl_context(
        self, target_domain: str = None, evasive: bool = True
    ) -> ssl.SSLContext:
        ...
        context.verify_mode = (
            ssl.CERT_NONE if not self.config.verify_ssl else ssl.CERT_REQUIRED
        )
    ...
    async def connect_with_retry(
        self, host: str, port: int, target_domain: str = None
    ) -> Tuple[bool, Optional[Exception], Dict[str, Any]]:
        ...
        for attempt in range(self.config.max_retries):
            ...
            try:
                ...
                if attempt == 0:
                    ...
                    success, error = await self._attempt_standard_connection(
                        host, port, target_domain, connection_info
                    )
                elif attempt == 1:
                    # Second attempt: no SNI
                    success, error = await self._attempt_no_sni_connection(
                        host, port, connection_info
                    )
                else:
                    # Third attempt: minimal TLS
                    success, error = await self._attempt_minimal_tls_connection(
                        host, port, connection_info
                    )
                ...
                last_exception = e
                connection_info["errors"].append(
                    f"Attempt {attempt + 1} exception: {e}"
                )
    ...
    async def _attempt_standard_connection(
        self, host: str, port: int, target_domain: str, info: Dict
        ...
        try:
            ...
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(
                    host, port, ssl=context, server_hostname=target_domain or host
                ),
    ...
    async def _attempt_no_sni_connection(
    ...
    async def _attempt_minimal_tls_connection(
        self, host: str, port: int, info: Dict
    ) -> Tuple[bool, Optional[Exception]]:
    ...
    async def test_connection(
        self, host: str, port: int = 443, target_domain: str = None
    ) -> Dict[str, Any]:
        ...
        if not success and error:
            if isinstance(error, asyncio.TimeoutError):
                ...
                if "handshake" in str(error).lower():
                    ...
                    result["recommendations"].extend(
                        ["Increase handshake timeout", "Use TLS fragmentation"]
                    )
...
async def test_domain_connections(
    domains: List[str], port: int = 443
) -> Dict[str, Dict]:
    """Test connections to multiple domains and return results."""

    config = ConnectionConfig(
        connect_timeout=10.0, handshake_timeout=15.0, max_retries=3
    )
