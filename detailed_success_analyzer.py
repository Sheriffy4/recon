...
class DetailedSuccessAnalyzer:
    ...
    def calculate_success_scores(self):
        ...
        for conn_key, conn in self.connection_analysis.items():
            ...
            # TLS этапы
            if conn["tls_hello_sent"]:
                score += 2
            if conn["tls_response_received"]:
                score += 5

            # Передача данных
            if conn["data_transferred"]:
                score += 3
            ...
            # Объем данных
            if conn["total_bytes"] > 1000:
                score += 1
            if conn["total_bytes"] > 5000:
                score += 2
    ...
    def print_connection_details(self):
        ...
        for conn_key, conn in sorted_connections:
            ...
            print(
                f"   Оценка: {conn['success_score']}/16 | Длительность: {duration:.1f}s | Пакетов: {len(conn['packets'])}"
            )
            print(
                f"   Этапы: SYN:{conn['syn_sent']} → SYN+ACK:{conn['syn_ack_received']} → EST:{conn['established']}"
            )
            print(
                f"   TLS: Hello:{conn['tls_hello_sent']} → Response:{conn['tls_response_received']}"
            )
            print(
                f"   Данные: {conn['total_bytes']} байт | Передача:{conn['data_transferred']}"
            )
            ...
            for i, state in enumerate(conn["states"][:5]):
                ...
                print(
                    f"     {i+1}. {rel_time:.3f}s: {state['flags']} ({state['size']}b, TTL:{state['ttl']})"
                )
    ...
    def analyze_success_patterns(self):
        ...
        if not self.success_patterns:
            print("❌ Успешных соединений не найдено")
            return
    ...
    def analyze_failure_patterns(self):
        ...
        if not self.failure_patterns:
            print("✅ Неудачных соединений не найдено")
            return
    ...
    def generate_recommendations(self):
        ...
        if self.success_patterns:
            ...
            # Анализируем TTL
            ttls = [
                state["ttl"] for state in success_conn["states"] if state["ttl"] > 0
            ]
            if ttls:
                common_ttl = max(set(ttls), key=ttls.count)
                recommendations.append(
                    f"Используйте TTL={common_ttl} (найден в успешном соединении)"
                )
            ...
            if sizes:
                ...
                recommendations.append(
                    f"Оптимальный размер пакетов: ~{avg_size:.0f} байт"
                )
        ...
        tls_failures = sum(
            1 for conn in self.failure_patterns if not conn["tls_response_received"]
        )
        if tls_failures > 0:
            recommendations.append(
                "TLS блокировка обнаружена - используйте разделение ClientHello"
            )
    ...
    def run_analysis(self):
        ...
        print("🔍 === Детальный анализ успехов и неудач ===")
        print(f"Файл: {self.pcap_file}")
        print("Цель: понять почему rutracker.org сработал\n")
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("\n🔧 Следующие шаги:")
            print("  1. Примените рекомендации из анализа")
            print("  2. Перезапустите службу обхода")
            print("  3. Протестируйте rutracker.org снова")
