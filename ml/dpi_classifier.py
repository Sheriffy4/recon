...
class DPIClassifier:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        # Known DPI vendors and types for classification
        self.known_vendors = [
            "Cisco",
            "Fortinet",
            "Palo Alto",
        ...
        self.behavioral_features = [
            ...
            "stateful_connection_limit",
            "deep_packet_inspection_depth",
            "anti_evasion_count",
        ]
        ...
        if SKLEARN_AVAILABLE:
            ...
            LOG.warning(
                "Scikit-learn not available, DPI classifier will use rule-based fallback"
            )
    ...
    def _initialize_models(self):
        ...
        # Vendor prediction model - focuses on implementation characteristics
        self.vendor_model = RandomForestClassifier(
            n_estimators=100, max_depth=10, random_state=42, class_weight="balanced"
        )

        # Type prediction model - focuses on deployment patterns
        self.type_model = GradientBoostingClassifier(
    ...
    def extract_behavioral_features(self, profile: DPIBehaviorProfile) -> np.ndarray:
        ...
        # Core DPI capabilities (binary features)
        ...
        # Timing sensitivity (aggregate score)
        timing_scores = (
        ...
        # Protocol handling sophistication
        ...
        # Detection pattern sophistication
    ...
    def predict_dpi_vendor_and_type(
        ...
        try:
            ...
            predicted_vendor = self.vendor_encoder.inverse_transform(
                [vendor_classes[vendor_idx]]
            )[0]
            ...
            predicted_type = self.type_encoder.inverse_transform(
                [type_classes[type_idx]]
            )[0]
            ...
            if hasattr(self.vendor_model, "feature_importances_"):
                for i, importance in enumerate(self.vendor_model.feature_importances_):
                    if i < len(self.behavioral_features):
                        feature_importance[self.behavioral_features[i]] = float(
                            importance
                        )

            # Create probability dictionaries
            vendor_probabilities = {
                self.vendor_encoder.inverse_transform([cls])[0]: float(prob)
                for cls, prob in zip(vendor_classes, vendor_probs)
            }

            type_probabilities = {
                self.type_encoder.inverse_transform([cls])[0]: float(prob)
                for cls, prob in zip(type_classes, type_probs)
            }

            # Generate reasoning
            reasoning = self._generate_ml_reasoning(
                profile, feature_importance, predicted_vendor, predicted_type
            )

            return DPIClassificationResult(
                predicted_vendor=predicted_vendor,
                predicted_type=predicted_type,
                confidence=overall_confidence,
                vendor_probabilities=vendor_probabilities,
    ...
    def _rule_based_classification(
        ...
        # Rule-based vendor detection
        if (
            profile.ml_detection
            and profile.deep_packet_inspection_depth
            ...
            profile.checksum_validation
            and profile.tcp_state_tracking_depth
            and profile.tcp_state_tracking_depth >= 3
        ):
            vendor = "Cisco"
        ...
        if (
            profile.stateful_connection_limit
            and profile.stateful_connection_limit > 500000
        ):
            dpi_type = "ISP_DPI"
            reasoning.append("High connection limit suggests ISP-level DPI")
            confidence += 0.2
        elif (
            profile.anti_evasion_techniques
            and len(profile.anti_evasion_techniques) >= 4
        ):
            ...
            reasoning.append(
                "Signature-based detection suggests traditional enterprise firewall"
            )
            confidence += 0.1
        ...
        return DPIClassificationResult(
            predicted_vendor=vendor,
            predicted_type=dpi_type,
            confidence=confidence,
            vendor_probabilities={vendor: confidence},
            type_probabilities={dpi_type: confidence},
            feature_importance={},
            classification_reasoning=reasoning,
        )
    ...
    def _generate_ml_reasoning(
        ...
        # Explain top 3 most important features
        for feature, importance in sorted_features[:3]:
            if importance > 0.1:  # Only explain significant features
                if feature == "supports_ip_frag":
                    if profile.supports_ip_frag:
                        reasoning.append(
                            f"IP fragmentation support (importance: {importance:.2f}) indicates {vendor}"
                        )
                    else:
                        reasoning.append(
                            f"IP fragmentation blocking (importance: {importance:.2f}) suggests {dpi_type}"
                        )
                elif feature == "rst_latency_ms":
                    if profile.rst_latency_ms and profile.rst_latency_ms < 100:
                        reasoning.append(
                            f"Fast RST response (importance: {importance:.2f}) characteristic of {vendor}"
                        )
                elif feature == "anti_evasion_count":
                    count = len(profile.anti_evasion_techniques)
                    reasoning.append(
                        f"Anti-evasion technique count ({count}, importance: {importance:.2f}) suggests {dpi_type}"
                    )
    ...
    def train_model(self, training_data: List[Tuple[DPIBehaviorProfile, str, str]]):
        ...
        vendor_score = cross_val_score(
            self.vendor_model, X_scaled, vendor_encoded, cv=5
        ).mean()

        # Train type model
        self.type_model.fit(X_scaled, type_encoded)
        type_score = cross_val_score(
            self.type_model, X_scaled, type_encoded, cv=5
        ).mean()

        self.is_trained = True

        LOG.info(
            f"Model training completed - Vendor accuracy: {vendor_score:.3f}, Type accuracy: {type_score:.3f}"
        )
            ...
            if hasattr(self.vendor_model, "feature_importances_"):
                info["feature_importances"] = {
                    ...
                    for feature, importance in zip(
                        self.behavioral_features, self.vendor_model.feature_importances_
                    )
                }
