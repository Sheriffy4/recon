...
class AdvancedStrategyGenerator:
    ...
    def generate_strategies(
        ...
        if self.fp_object:
            if getattr(self.fp_object, "is_stateful_dpi", False):
                LOG.info(
                    "DPI fingerprint suggests stateful inspection. Prioritizing stateful attacks."
                )
                stateful_techniques = self._map_category_to_techniques(
            ...
            if getattr(self.fp_object, "has_weak_tls_parser", False):
                LOG.info(
                    "DPI fingerprint suggests a weak TLS parser. Prioritizing TLS record evasion."
                )
                tls_evasion_techniques = self._map_category_to_techniques(
                    "tls_record_evasion"
                )
    ...
    def _map_category_to_techniques(self, category: str) -> List[str]:
        ...
        category_mapping = {
            ...
            "stateful_tcp_manipulation": ["stateful_fragment", "advanced_overlap"],
            "tls_record_evasion": ["client_hello_split", "tls_record_padding"],
            "ip_fragmentation": [
    ...
    def generate_ab_test_variations(
        ...
        if not attack_name:
            LOG.warning(
                "Cannot generate A/B test variations for a task without a name."
            )
            return []
        if not self.parameter_optimizer:
            LOG.warning(
                "Parameter optimizer not available, cannot generate A/B test variations."
            )
            return []
        param_ranges = self.parameter_optimizer.generate_parameter_ranges(attack_name)
        if param_to_test not in param_ranges:
            LOG.warning(
                f"Parameter '{param_to_test}' not found in ranges for attack '{attack_name}'."
            )
            return []
        ...
        LOG.info(
            f"Generated {len(variations)} A/B test variations for '{attack_name}' on parameter '{param_to_test}'."
        )
        return variations
