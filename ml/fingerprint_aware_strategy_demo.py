...
def demo_characteristic_based_strategies():
    ...
    for test_case in test_cases:
        ...
        char_strategies = generator._get_characteristic_based_strategies(test_case['fingerprint'])
        if char_strategies:
            print(f"   💡 Characteristic-specific techniques: {len(char_strategies)} strategies")
...
def demo_confidence_based_ranking():
    ...
    high_conf_strategies = generator.generate_strategies(
        fingerprint=high_confidence_fp, 
        count=5
    )
    ...
    print(f"   • High confidence uses targeted strategies for {high_confidence_fp.dpi_type.value}")
    print(f"   • Low confidence falls back to proven general strategies")
    print(f"   • Top strategies are different: {high_conf_strategies[0] != low_conf_strategies[0]}")
...
def demo_fallback_mechanism():
    ...
    proven_included = any(strategy in generator.PROVEN_WORKING for strategy in generic_strategies)
    print(f"\n✅ Includes proven working strategies: {proven_included}")
    print(f"📊 Total proven working strategies available: {len(generator.PROVEN_WORKING)}")
...
def demo_strategy_analysis():
    ...
    for i, strategy in enumerate(test_strategies, 1):
        ...
        print(f"\n{i}. {strategy}")
        print(f"   Complexity: {complexity:.2f}/1.0 | Aggressiveness: {aggressiveness:.2f}/1.0")
...
def demo_integration_with_existing_system():
    ...
    try:
        ...
        strategies = generator.generate_strategies(fingerprint=old_style_fingerprint, count=3)
    ...
    print(f"Performance overhead: {((fp_time - generic_time) / generic_time * 100):.1f}%")
