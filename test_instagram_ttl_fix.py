...
def test_instagram_strategy():
    ...
    try:
        ...
        has_autottl = 'autottl' in normalized.params and normalized.params['autottl'] is not None
...
def test_ttl_priority():
    ...
    try:
        ...
        test_cases = [
            # Только ttl
            ("--dpi-desync=fake,disorder --dpi-desync-split-pos=3 --dpi-desync-ttl=5", "ttl", 5),
            
            # Только autottl
            ("--dpi-desync=fake,disorder --dpi-desync-split-pos=3 --dpi-desync-autottl=2", "autottl", 2),
            
            # И ttl, и autottl - должен выбрать autottl
            ("--dpi-desync=fake,disorder --dpi-desync-split-pos=3 --dpi-desync-ttl=5 --dpi-desync-autottl=2", "autottl", 2),
        ]
        ...
        for strategy_str, expected_param, expected_value in test_cases:
            try:
                ...
                if expected_param == "ttl":
                    if normalized.params.get('ttl') == expected_value and not normalized.params.get('autottl'):
                        print(f"✅ TTL приоритет: {strategy_str[:40]}... → ttl={expected_value}")
                    else:
                        print(f"❌ TTL приоритет: {strategy_str[:40]}... → ttl={normalized.params.get('ttl')}, autottl={normalized.params.get('autottl')}")
                        all_passed = False
                elif expected_param == "autottl":
                    if normalized.params.get('autottl') == expected_value and not normalized.params.get('ttl'):
                        print(f"✅ AutoTTL приоритет: {strategy_str[:40]}... → autottl={expected_value}")
                    else:
                        print(f"❌ AutoTTL приоритет: {strategy_str[:40]}... → ttl={normalized.params.get('ttl')}, autottl={normalized.params.get('autottl')}")
                        all_passed = False
