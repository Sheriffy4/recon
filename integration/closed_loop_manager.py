# recon/core/integration/closed_loop_manager.py
"""
Closed Loop Manager
...
class ClosedLoopManager(IClosedLoopManager):
    ...
    def __init__(
        ...
        self.convergence_threshold = 0.9  # Stop if effectiveness > 90%
        self.improvement_threshold = 0.05  # Minimum improvement to continue
        self.strategies_per_iteration = 10
        self.max_strategies_to_test = 5  # Test only top N strategies per iteration
    ...
    async def run_closed_loop(
        ...
        try:
            ...
            self.logger.info(
                f"Baseline established: success={baseline_result.success}, "
                f"latency={baseline_result.latency_ms:.1f}ms"
            )

            # If baseline is already successful, we may still want to optimize latency
            if baseline_result.success:
                result.analysis_summary.append(
                    f"Domain {domain} is accessible without bypass"
                )
                self.baseline_effectiveness = (
                    0.5  # Set moderate baseline for optimization
                )
            else:
                result.analysis_summary.append(
            ...
            # Step 2: Create initial comprehensive fingerprint with behavioral profiling
            ...
            # Step 3: Create behavioral profile for enhanced analysis
            ...
            # Step 4: Load learning history
            learning_history = await self.learning_memory.load_learning_history(
                fingerprint_hash
            )
            if learning_history:
                self.logger.info(
                    f"Loaded learning history: {len(learning_history.successful_attacks)} "
                    f"successful attacks, best effectiveness: {learning_history.best_effectiveness:.2f}"
                )

            # === MAIN CLOSED LOOP ITERATIONS ===
            for iteration in range(self.max_iterations):
                ...
                # Run single iteration with enhanced planning
                ...
                # Collect test results for cumulative analysis
                ...
                # Analyze failure patterns using FailureAnalyzer
                ...
                if iteration_result.best_effectiveness > self.best_effectiveness:
                    ...
                    self.logger.info(
                        f"New best effectiveness: {self.best_effectiveness:.2f} "
                        f"(improvement: +{improvement:.2f})"
                    )
                else:
                    self.iterations_without_improvement += 1
                    self.logger.info(
                        f"No improvement in iteration {self.current_iteration} "
                        f"({self.iterations_without_improvement} consecutive)"
                    )
                ...
                if iteration < self.max_iterations - 1:
                    ...
                    # Update behavioral profile with new insights
                    behavioral_profile = (
            ...
            self.logger.info(
                f"Enhanced closed loop completed: {result.total_iterations} iterations, "
                f"final effectiveness: {result.final_effectiveness:.2f}, "
    ...
    async def _run_enhanced_iteration(
        ...
        try:
            # === PHASE 2: PLAN (Enhanced Strategy Generation) ===

            # Generate strategies with failure analysis integration
            ...
            self.logger.info(
                f"Generated {iteration_result.strategies_generated} strategies, "
                f"testing top {iteration_result.strategies_tested}"
            )
            ...
            for i, strategy in enumerate(strategies_to_test):
                ...
                try:
                    ...
                    if (
                        ...
                        self.logger.info(
                            f"New best in iteration: {effectiveness_result.effectiveness_score:.2f} "
                            f"with {strategy['name']}"
                        )
            ...
            # === PHASE 4: ANALYZE (Enhanced with FailureAnalyzer) ===
            ...
            if test_results:
                ...
                # Log key insights
                for pattern in failure_analysis.failure_patterns[:3]:  # Top 3 patterns
                    self.logger.info(
            ...
            self.logger.info(
                f"Iteration {self.current_iteration} completed in {iteration_time:.1f}s: "
                f"best_effectiveness={iteration_result.best_effectiveness:.2f}"
            )
    ...
    async def _run_single_iteration(
        ...
        # Step 1: Generate strategies based on current fingerprint and previous failure analysis
        fingerprint_dict = fingerprint.__dict__ if fingerprint else {}
        history_list = []
        if learning_history:
            # Convert learning history to format expected by strategy generator
            for (
                attack_name,
                success_rate,
        ...
        # Initialize strategy generator with current context
        strategy_gen = AdvancedStrategyGenerator(
            fingerprint_dict=fingerprint_dict,
            history=history_list,
            doh_success=False,  # Could be determined from fingerprint
        )

        # Get failure analysis from previous iteration if available
        ...
        # Generate strategies for this iteration, informed by failure analysis
        if previous_failure_analysis:
        ...
        # Step 2: Test strategies using adaptive attacks
        tested_strategies = 0
        best_effectiveness = 0.0
        effectiveness_results = []

        # Limit number of strategies to test to avoid excessive runtime
        ...
        for i, strategy in enumerate(strategies_to_test):
            try:
                ...
                # Create adaptive attack for this strategy
                adaptive_attack = LearningAdaptiveAttack(
                    effectiveness_tester=self.effectiveness_tester,
                    learning_memory=self.learning_memory,
                )

                # Execute attack (this is a simplified version - real implementation would be more complex)
        ...
        # Step 3: Analyze results and update learning
        analysis_notes = []
        ...
        if effectiveness_results:
            ...
            # Extract strategic recommendations from failure analysis
            strategic_recommendations = failure_analysis.strategic_recommendations

            # Log failure analysis insights
            ...
            for result in effectiveness_results:
                await self.learning_memory.save_learning_result(
                    ...
                    parameters={},  # Would extract from strategy
            ...
            # Create empty failure analysis for consistency
        ...
        self.logger.info(
            f"Iteration {self.current_iteration} completed in {iteration_time:.1f}s: "
            f"best_effectiveness={best_effectiveness:.2f}, "
            f"improvement={improvement_over_baseline:.2f}"
        )
    ...
    async def _execute_strategy_with_adaptive_attack(
        ...
        try:
            ...
            # 1. Convert strategy dict to AttackContext
            # 2. Execute the adaptive attack
            # 3. Measure real effectiveness

            # For now, simulate by testing baseline vs bypass
            baseline_result = await self.effectiveness_tester.test_baseline(
                domain, port
            )

            # Simulate attack execution (would be real in production)
    ...
    async def should_continue_loop(
        ...
        # Check if we have any improvement over baseline
        if (
    ...
    async def _refine_fingerprint_from_results(
        ...
        # 1. Обновляем показатели успешности техник в фингерпринте
        for result in test_results:
            attack_name = result.bypass.attack_name
            effectiveness = result.effectiveness_score

            # Используем скользящее среднее для обновления
        ...
        # 2. Делаем выводы на основе успешных/неуспешных атак
        if (
        ...
        # 3. Переклассифицируем DPI с новыми данными
    ...
    # === NEW: Enhanced Closed Loop Supporting Methods ===
    ...
    def _prioritize_strategies_by_behavioral_profile(
        ...
        def calculate_strategy_priority(strategy: Dict[str, Any]) -> float:
            ...
            # Prioritize based on identified weaknesses
            ...
            # Prioritize based on DPI capabilities
            if (
                not behavioral_profile.supports_ip_frag
                and "fragmentation" in strategy_name.lower()
            ):
                priority_score -= 0.2  # Lower priority if fragmentation is blocked

            if (
                not behavioral_profile.checksum_validation
                and "badsum" in strategy_name.lower()
            ):
                priority_score += 0.2  # Higher priority if checksum validation is weak

            # Prioritize modern protocol attacks if supported
            if behavioral_profile.ech_support is False and (
                "ech" in strategy_name.lower() or "modern" in strategy_name.lower()
            ):
                priority_score += 0.2

            # Prioritize traffic mimicry for sophisticated DPI
    ...
    async def _integrate_failure_analysis(
        ...
        merged_analysis = FailureAnalysisResult(
            ...
            next_iteration_focus=new_analysis.next_iteration_focus,  # Use latest focus
    ...
    async def _generate_strategic_recommendations(
        ...
        # Behavioral profile based recommendations
        ...
        # Learning history based recommendations
    ...
    async def _update_learning_memory_with_behavioral_insights(
        ...
        """
        try:
            # Create behavioral insights dictionary
            behavioral_insights = {
                "dpi_capabilities": {
            ...
            # Store behavioral insights in learning memory
    ...
    async def _generate_final_analysis_summary(
        ...
        """
        try:
            # Add behavioral profile summary
            result.analysis_summary.append(
                f"DPI Behavioral Profile: {len(behavioral_profile.identified_weaknesses)} weaknesses, "
                f"{len(behavioral_profile.anti_evasion_techniques)} anti-evasion techniques"
            )
            ...
            if result.iterations:
                ...
                result.analysis_summary.append(
                    f"Best iteration: #{best_iteration.iteration_number} "
                    f"with {best_iteration.best_effectiveness:.2f} effectiveness"
                )
