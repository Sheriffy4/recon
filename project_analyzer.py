...
class ProjectAnalyzer:
    ...
    def scan_python_files(self):
        ...
        for file_path in self.project_root.rglob("*.py"):
            ...
            if any(
                part in file_path.parts for part in [".venv", "__pycache__", ".git"]
            ):
                continue
    ...
    def categorize_modules(self):
        ...
        for file_path, info in self.modules_info.items():
            ...
            # CLI инструменты
            if (
                "cli" in rel_path.lower()
                or info["has_main"]
                or "main" in info["functions"]
                or any("argparse" in imp for imp in info["imports"])
            ):
                categories["cli_tools"].append(rel_path)

            # Основные движки
            elif (
                "engine" in rel_path.lower()
                or "bypass" in rel_path.lower()
            ...
            # Анализаторы
                ...
                "analyzer" in rel_path.lower()
                or "pcap" in rel_path.lower()
                or "analyze" in rel_path.lower()
            ):
                categories["analyzers"].append(rel_path)

            # Тесты
            elif "test" in rel_path.lower() or "demo" in rel_path.lower():
                categories["tests"].append(rel_path)

            # Утилиты
            elif (
                "util" in rel_path.lower()
                or "helper" in rel_path.lower()
                or "tool" in rel_path.lower()
            ):
