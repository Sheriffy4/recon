...
def test_strategy_parsing():
    ...
    try:
        ...
        test_cases = [
            ...
            ("--dpi-desync=fake,fakeddisorder --dpi-desync-split-pos=3", "fakeddisorder"),
        ...
        for strategy_str, expected_type in test_cases:
            try:
                ...
                if normalized.type == expected_type:
                    ...
                    print(f"❌ {strategy_str[:30]}... → {normalized.type} (ожидался {expected_type})")
                    all_passed = False
            ...
            if strategy_str:
                try:
                    ...
                    if normalized.type == expected_type:
                        ...
                        print(f"❌ {domain}: {normalized.type} (ожидался {expected_type})")
                        all_correct = False
        ...
        for strategy_str in strategies:
            try:
                ...
                if forced_config.get('type') != normalized.type:
                    print(f"❌ Неправильный тип в forced_config: {forced_config.get('type')} != {normalized.type}")
                    all_correct = False
