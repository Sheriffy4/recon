...
class AdvancedSetupManager:
    ...
    def load_module_info(self):
        ...
        self.modules = {
            'cli_tools': [
                {'file': 'cli.py', 'name': 'Основной CLI', 'desc': 'Полнофункциональный CLI с PCAP анализом'},
                {'file': 'simple_cli.py', 'name': 'Простой CLI', 'desc': 'Упрощенный CLI для быстрых операций'},
                {'file': 'smart_bypass_cli.py', 'name': 'Smart Bypass CLI', 'desc': 'Умный CLI с автоматическим обходом'},
                {'file': 'subdomain_detector.py', 'name': 'Детектор поддоменов', 'desc': 'Автоматическое обнаружение заблокированных поддоменов'},
                {'file': 'browser_network_monitor.py', 'name': 'Монитор браузера', 'desc': 'Мониторинг сетевых запросов браузера'}
            ],
            'analyzers': [
                {'file': 'simple_pcap_analyzer.py', 'name': 'PCAP Анализатор', 'desc': 'Анализ PCAP файлов для диагностики'},
                {'file': 'comprehensive_bypass_analyzer.py', 'name': 'Комплексный анализатор', 'desc': 'Детальный анализ эффективности обхода'},
                {'file': 'x_com_subdomain_analyzer.py', 'name': 'X.com анализатор', 'desc': 'Специализированный анализатор для X.com'},
                {'file': 'project_analyzer.py', 'name': 'Анализатор проекта', 'desc': 'Анализ структуры и модулей проекта'}
            ],
            'tests': [
                {'file': 'quick_test.py', 'name': 'Быстрый тест', 'desc': 'Быстрая проверка системы обхода'},
                {'file': 'simple_bypass_test.py', 'name': 'Тест обхода', 'desc': 'Тестирование стратегий обхода'},
                {'file': 'final_bypass_test.py', 'name': 'Финальный тест', 'desc': 'Комплексное тестирование системы'}
            ],
            'utilities': [
                {'file': 'setup_hosts_bypass.py', 'name': 'Настройка hosts', 'desc': 'Управление hosts файлом'},
                {'file': 'apply_improved_strategies.py', 'name': 'Применение стратегий', 'desc': 'Применение улучшенных стратегий'},
                {'file': 'adaptive_strategy_finder.py', 'name': 'Поиск стратегий', 'desc': 'Адаптивный поиск оптимальных стратегий'}
            ],
            'monitors': [
    ...
    def run_command(self, command: list, needs_admin: bool = False, capture_output: bool = False):
        """Запускает команду."""
        if needs_admin and not self.is_admin():
            self.console.print("[bold red]Ошибка:[/bold red] Требуются права администратора.")
            return None
    ...
    def show_main_menu(self):
        ...
        while True:
            ...
            status_text.append(" | ", style="dim")
            status_text.append(f"📖 Сигнатур: {stats.get('total_signatures', 0)}", style="blue")
...
[0] Выход
    ...
    def show_cli_tools_menu(self):
        ...
        while True:
            ...
            table = Table(title="CLI Инструменты", show_header=True, header_style="bold magenta")
    ...
    def run_cli_tool(self, tool):
        ...
        if tool['file'] == 'cli.py':
            ...
            if mode == "1":
                ...
                command = [tool['file'], self.sites_file, "--domains-file", "--evolutionary"]
            elif mode == "3":
                command = [tool['file'], self.sites_file, "--domains-file", "--individual"]
            ...
            cmd_choice = Prompt.ask("Выберите команду", choices=["1", "2", "3", "4"], default="1")
            ...
            if cmd_choice == "1":
                ...
                command = [tool['file'], "test-multi", "x.com", "instagram.com", "rutracker.org"]
            elif cmd_choice == "4":
            ...
            cmd_choice = Prompt.ask("Выберите команду", choices=["1", "2", "3"], default="1")
            
            if cmd_choice == "1":
                command = [tool['file'], "test-multiple", "x.com", "instagram.com", "rutracker.org"]
            elif cmd_choice == "2":
    ...
    def show_analyzers_menu(self):
        ...
        while True:
            ...
            table = Table(title="Анализаторы и диагностика", show_header=True, header_style="bold green")
            table.add_column("№", style="dim", width=3)
            table.add_column("Название", style="cyan")
            table.add_column("Описание", style="white")
    ...
    def show_tests_menu(self):
        ...
        while True:
            ...
            table = Table(title="Тесты и проверки", show_header=True, header_style="bold yellow")
            table.add_column("№", style="dim", width=3)
            table.add_column("Название", style="cyan")
            table.add_column("Описание", style="white")
    ...
    def show_utilities_menu(self):
        ...
        while True:
            ...
            table = Table(title="Утилиты и настройки", show_header=True, header_style="bold blue")
            table.add_column("№", style="dim", width=3)
            table.add_column("Название", style="cyan")
            table.add_column("Описание", style="white")
    ...
    def show_monitoring_menu(self):
        ...
        while True:
            ...
            table = Table(title="Мониторинг и автоматизация", show_header=True, header_style="bold red")
            table.add_column("№", style="dim", width=3)
            table.add_column("Название", style="cyan")
            table.add_column("Описание", style="white")
            ...
            try:
                ...
                if 0 <= idx < len(self.modules['monitors']):
                    ...
                    if 'monitor' in monitor['file']:
                        # Мониторинг доменов
                        domains = Prompt.ask("Введите домены для мониторинга (через пробел)", default="x.com instagram.com").split()
    ...
    def action_start_service(self):
        ...
        if not os.path.exists(self.strategy_file):
            self.console.print(f"[yellow]Файл стратегии не найден.[/yellow]")
