...
async def test_direct_connection():
    ...
    try:
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            async with session.get('https://x.com', headers=HEADERS, allow_redirects=True) as response:
...
async def test_ip_connection(ip):
    ...
    try:
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            headers['Host'] = 'x.com'
            async with session.get(f'https://{ip}', headers=headers, allow_redirects=True) as response:
                await response.content.readexactly(1)
                latency = (time.time() - start_time) * 1000
                print(f"  ✅ УСПЕХ: Статус {response.status}, задержка {latency:.1f}ms")
                return True
        ...
        try:
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                ...
                async with session.get('https://x.com', headers=HEADERS, allow_redirects=True) as response:
                    await response.content.readexactly(1)
                    latency = (time.time() - start_time) * 1000
                    print(f"  ✅ УСПЕХ С ОБХОДОМ: Статус {response.status}, задержка {latency:.1f}ms")
...
async def main():
    ...
    if passed == 0:
        print("🚨 x.com полностью недоступен - возможно, проблема с интернетом или блокировкой")
    elif passed < total:
