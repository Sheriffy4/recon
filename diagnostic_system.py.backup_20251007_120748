...
class DiagnosticSystem:
    ...
    def log_attack_result(
        ...
        try:
            ...
            if self.debug:
                self.logger.debug(
                    f"🎯 Attack logged: {attack_name} | Status: {attack_result.status.value} | Latency: {attack_result.latency_ms:.2f}ms | Packets: {attack_result.packets_sent} | Bytes: {attack_result.bytes_sent}"
                )
    ...
    def log_packet_processing(
        ...
        try:
            ...
            if self.debug:
                self.logger.debug(
                    f"📦 Packet processed: {src_addr}:{src_port} -> {dst_addr}:{dst_port} ({protocol}, {packet_size}B) | Action: {action} | Technique: {technique_used} | Time: {processing_time_ms:.2f}ms | Success: {success}"
                )
                if byte_analysis:
    ...
    def export_diagnostics(self, filepath: str) -> bool:
        ...
        try:
            export_data = {
                ...
                "recent_events": [
                    asdict(event) for event in list(self.packet_events)[-1000:]
                ],
                "recent_errors": [
                    asdict(event) for event in list(self.error_events)[-100:]
                ],
            }
        ...
        while self.monitoring_active:
            try:
                ...
                if (
                    ...
                    self.logger.warning(
                        f"⚠️ Critical performance issue detected: Effectiveness={effectiveness.get('overall_effectiveness', 0):.2f}"
                    )
                for technique_name, metrics in self.technique_metrics.items():
                    if metrics.success_rate < self.thresholds["min_success_rate"]:
                        self.logger.warning(
                            f"⚠️ Technique {technique_name} underperforming: Success rate={metrics.success_rate:.2f}"
                        )
    ...
    def _analyze_ipv4_header(self, raw_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis["is_fragmented"] = (
                analysis["fragment_offset"] > 0 or analysis["flags"] & 1 != 0
            )
    ...
    def _analyze_tcp_bytes(
        ...
        try:
            ...
            analysis["tcp_flags"] = {
                "FIN": bool(flags_byte & 1),
                "SYN": bool(flags_byte & 2),
                "RST": bool(flags_byte & 4),
                "PSH": bool(flags_byte & 8),
                "ACK": bool(flags_byte & 16),
                "URG": bool(flags_byte & 32),
            }
    ...
    def _log_tls_packet_details(self, payload: bytes):
        ...
        try:
            ...
            self.logger.debug(
                f"🔒 TLS Details: Type=0x{tls_type:02x}, Version=0x{tls_version:04x}, Length={tls_length}, Payload={len(payload_bytes)}B"
            )
