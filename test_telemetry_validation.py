...
def test_telemetry_basic_functionality():
    ...
    try:
        ...
        assert isinstance(initial_telemetry["duration_sec"], (int, float)), "duration_sec должно быть числом"
...
def test_telemetry_after_attack_dispatch():
    ...
    try:
        ...
        print(f"📊 Начальная телеметрия - segments: {initial_segments}, fake_packets: {initial_fake_packets}")
        ...
        assert test_ip in updated_telemetry["per_target"], f"IP {test_ip} не найден в per_target"
        
        target_data = updated_telemetry["per_target"][test_ip]
        assert "high_level_success" in target_data, "Отсутствует high_level_success"
        assert target_data["high_level_success"] == True, "high_level_success должен быть True"
        assert "high_level_outcome_ts" in target_data, "Отсутствует high_level_outcome_ts"
        
        print(f"✅ Телеметрия для {test_ip}: success={target_data['high_level_success']}")
        
        # Проверяем aggregate счетчики
        aggregate = updated_telemetry["aggregate"]
        assert "high_level_successes" in aggregate, "Отсутствует high_level_successes в aggregate"
        assert aggregate["high_level_successes"] >= 1, "high_level_successes должен быть >= 1"
...
def test_telemetry_structure_consistency():
    ...
    try:
        ...
        for i, snapshot in enumerate(snapshots[1:], 1):
            ...
            assert base_keys == current_keys, f"Структура телеметрии изменилась в снимке {i}"
        ...
        for i in range(1, len(durations)):
            assert durations[i] >= durations[i-1], f"duration_sec не увеличивается: {durations[i-1]} -> {durations[i]}"
        
        print(f"✅ duration_sec корректно увеличивается: {durations[0]:.3f} -> {durations[-1]:.3f}")
...
def test_telemetry_attack_dispatcher_integration():
    ...
    try:
        ...
        assert hasattr(engine, '_attack_dispatcher'), "AttackDispatcher не инициализирован"
        ...
        for ip in test_ips:
            assert ip in updated_telemetry["per_target"], f"IP {ip} не найден в телеметрии"
            print(f"✅ Телеметрия для {ip} записана")
        
        # Проверяем aggregate счетчики
        successes = updated_telemetry["aggregate"].get("high_level_successes", 0)
        assert successes >= len(test_ips), f"Недостаточно успешных операций: {successes} < {len(test_ips)}"
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"📊 Результаты тестирования телеметрии:")
    ...
    if failed == 0:
        ...
        print("✅ Телеметрия работает корректно после рефакторинга диспетчеризации атак")
        return True
    else:
