...
class AdaptiveStrategyFinder:
    ...
    def _define_attacks(self) -> List[AttackConfig]:
        ...
        return [
            ...
            AttackConfig(
                ...
                method="fakeddisorder",
                params={"split_seqovl": 336, "autottl": 2, "fooling": "md5sig,badsum,badseq", "repeats": 1, "split_pos": 76, "ttl": 1},
            ...
            AttackConfig(
                ...
                method="multisplit",
                params={"split_count": 7, "split_seqovl": 30, "fooling": "badsum", "repeats": 3, "ttl": 4},
            ...
            AttackConfig(
                ...
                params={"split_pos": 3, "split_seqovl": 20, "fooling": "badsum", "ttl": 2},
    ...
    def _generate_combinations(self, max_complexity: int = 3) -> List[Dict]:
        ...
        for attack1, attack2 in combinations(self.available_attacks, 2):
            ...
            if total_complexity <= max_complexity + 2:  # Позволяем немного больше для комбинаций
                combinations_list.append({
        ...
        for attack1, attack2, attack3 in combinations(simple_attacks, 3):
            total_complexity = attack1.complexity + attack2.complexity + attack3.complexity
    ...
    async def test_strategy(self, domain: str, strategy_string: str, strategy_name: str) -> TestResult:
        ...
        try:
            ...
            if results and len(results) > 0:
                ...
                score = 0.0
                if success:
                    score = 100.0 - (latency / 10)  # Базовая оценка минус штраф за задержку
    ...
    async def find_best_strategy_for_domain(self, domain: str, max_tests: int = 20) -> Dict:
        ...
        prioritized_attacks = sorted(simple_strategies, 
        ...
        for i, attack in enumerate(test_attacks):
            ...
            print(f"{attack.name:<30} {status:<10} {result.latency_ms:<10.0f} {result.score:<.1f}")
            ...
            if result.success and result.score > 80:
                ...
                for variation in variations[:3]:  # Максимум 3 вариации
                    ...
                    var_result = await self.test_strategy(domain, var_strategy, f"{attack.name}_var")
                    results.append(var_result)
                    
                    var_status = "✅ Успех" if var_result.success else "❌ Неудача"
                    print(f"{f'{attack.name}_var':<30} {var_status:<10} {var_result.latency_ms:<10.0f} {var_result.score:<.1f}")
        ...
        if successful_results:
            ...
            return {
                ...
                "zapret_string": self._find_strategy_string_by_name(best_result.strategy_name),
            ...
            return {
                ...
                "success_rate": 0.0,
                "zapret_string": None,
                "adaptive_insights": {"recommendation": "Try different network conditions or DPI detection"}
            }
    ...
    def _generate_strategy_variations(self, base_attack: AttackConfig) -> List[AttackConfig]:
        ...
        if "fooling" in base_attack.params and base_attack.params["fooling"] == "badsum":
    ...
    def _generate_adaptive_insights(self, results: List[TestResult], domain: str) -> Dict:
        ...
        if successful:
            insights["best_methods"] = list(set([r.strategy_name.split("_")[0] for r in successful]))
            insights["avg_successful_latency"] = sum(r.latency_ms for r in successful) / len(successful)
            insights["recommendation"] = "Found working strategies, consider optimizing parameters"
        else:
            insights["recommendation"] = "No strategies worked, may need different approach or network analysis"
            if failed:
    ...
    def _classify_domain_type(self, domain: str) -> str:
        ...
        if any(social in domain for social in ["x.com", "twitter", "instagram", "facebook"]):
            return "social_media"
    ...
    async def optimize_multiple_domains(self, domains: List[str], max_tests_per_domain: int = 15) -> Dict:
        ...
        for i, domain in enumerate(domains):
            ...
            domain_result = await self.find_best_strategy_for_domain(domain, adaptive_max_tests)
            ...
            if domain_result["best_strategy"]:
                ...
                print(f"✅ Найдена лучшая стратегия: {domain_result['best_strategy']['name']}")
                ...
                if "adaptive_insights" in domain_result:
                    ...
                    if "best_methods" in insights:
                        print(f"   Лучшие методы: {', '.join(insights['best_methods'])}")
            else:
            ...
            if learned_patterns:
                success_rate = sum(1 for r in results.values() if r["best_strategy"]) / len(results)
                print(f"   📊 Общий прогресс: {success_rate:.1%} успешных доменов")
        
        # Генерируем общие рекомендации
        general_recommendations = self._generate_general_recommendations(results, learned_patterns)
    ...
    def _calculate_adaptive_test_count(self, learned_patterns: Dict, domain: str, base_max: int) -> int:
        ...
        if domain_type in learned_patterns and learned_patterns[domain_type]["success_count"] > 0:
    ...
    def _update_learned_patterns(self, learned_patterns: Dict, domain: str, result: Dict):
        ...
        pattern["avg_latency"] = (pattern["avg_latency"] * (pattern["success_count"] - 1) + current_latency) / pattern["success_count"]
        
        # Добавляем успешный метод
        strategy_method = result["best_strategy"]["name"].split("_")[0]
        pattern["best_methods"].add(strategy_method)
    
    def _generate_general_recommendations(self, results: Dict, learned_patterns: Dict) -> Dict:
        """Генерирует общие рекомендации на основе всех результатов."""
        all_results = results if isinstance(results, dict) and "domain_results" not in results else results.get("domain_results", results)
        
        successful_domains = [r for r in all_results.values() if r["best_strategy"]]
        total_domains = len(all_results)
        
        recommendations = {
            "overall_success_rate": len(successful_domains) / total_domains if total_domains > 0 else 0,
        ...
        if successful_domains:
            ...
            avg_latency = sum(r["best_result"].latency_ms for r in successful_domains) / len(successful_domains)
    ...
    def _generate_optimization_summary(self, results: Dict) -> Dict:
        ...
        return {
            ...
            "avg_tests_per_domain": sum(len(r["all_results"]) for r in all_results.values()) / len(all_results) if all_results else 0,
            "total_strategy_tests": sum(len(r["all_results"]) for r in all_results.values())
        }
    ...
    def generate_optimized_config(self, optimization_results: Dict) -> Dict:
        ...
        config = {
            ...
            "optimization_summary": {
                ...
                "successful_domains": len([r for r in domain_results.values() if r["best_strategy"]]),
                "average_success_rate": sum(r["success_rate"] for r in domain_results.values()) / len(domain_results) if domain_results else 0,
        ...
        for domain, result in domain_results.items():
            if result["best_strategy"]:
                ...
                config["domain_patterns"][domain] = {
                    ...
                    "domain_type": result.get("adaptive_insights", {}).get("domain_type", "unknown"),
                    "confidence": self._calculate_strategy_confidence(result)
                }
            else:
                # Интеллектуальная fallback стратегия на основе типа домена
                domain_type = self._classify_domain_type(domain)
                fallback_strategy = self._get_intelligent_fallback(domain_type, learned_patterns)
        ...
        for domain_type, pattern in learned_patterns.items():
            if pattern["success_count"] > 0:
                best_method = max(pattern["best_methods"], key=lambda x: pattern["successful_strategies"].count(x))
                config["fallback_strategies"][domain_type] = self._generate_fallback_for_method(best_method)
        
        # Добавляем глобальные рекомендации
        if recommendations:
            config["global_insights"] = {
                "most_successful_methods": recommendations.get("most_successful_methods", []),
                "avg_successful_latency": recommendations.get("avg_successful_latency", 0),
    ...
    def _calculate_strategy_confidence(self, result: Dict) -> float:
        ...
        confidence = min(1.0, base_confidence + latency_bonus + score_bonus - test_penalty)
        return round(confidence, 2)
    
    def _get_intelligent_fallback(self, domain_type: str, learned_patterns: Dict) -> str:
        """Возвращает интеллектуальную fallback стратегию для типа домена."""
        # Если есть обученные паттерны для этого типа
        if domain_type in learned_patterns and learned_patterns[domain_type]["success_count"] > 0:
            pattern = learned_patterns[domain_type]
            best_method = max(pattern["best_methods"], 
    ...
    def save_results(self, results: Dict, filename: str = "adaptive_optimization_results.json"):
...
async def main():
    ...
    try:
        ...
        results = await finder.optimize_multiple_domains(test_domains, max_tests_per_domain=10)
        ...
        print(f"Успешных доменов: {config['optimization_summary']['successful_domains']}/{config['optimization_summary']['total_domains']}")
        print(f"Средняя успешность: {config['optimization_summary']['average_success_rate']:.1f}%")
        print(f"Конфигурация сохранена в adaptive_strategies.json")
