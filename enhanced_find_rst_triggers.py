...
try:
    from core import UnifiedBypassEngine, UnifiedEngineConfig, UnifiedStrategyLoader, NormalizedStrategy
...
if not UNIFIED_ENGINE_AVAILABLE:
    try:
        ...
        LOG.warning("⚠️  Using legacy bypass engine - may not match service mode behavior")
    except ImportError as e:
...
class DPIFingerprintAnalyzer:
    ...
    def _test_with_bypass_engine(self, config: StrategyTestConfig) -> Tuple[bool, float]:
        ...
        try:
            ...
            if test_result.get('forced_override'):
                LOG.debug(f"✅ Strategy tested with FORCED OVERRIDE: {config.get_description()}")
            else:
                LOG.warning(f"⚠️  Strategy tested WITHOUT forced override: {config.get_description()}")
            
            # Log telemetry if available
            if 'telemetry_delta' in test_result:
                delta = test_result['telemetry_delta']
                LOG.debug(f"   Telemetry: segments={delta.get('segments_sent', 0)}, "
    ...
    def validate_engine_compatibility(self) -> Dict[str, Any]:
        ...
        if UNIFIED_ENGINE_AVAILABLE and self.unified_engine:
            ...
            validation['issues'].append("UnifiedBypassEngine not available - using fallback")
            validation['matches_service_mode'] = False
...
def compare_with_service_mode(domain: str, strategy_string: str) -> Dict[str, Any]:
    ...
    if not UNIFIED_ENGINE_AVAILABLE:
        comparison['differences'].append("UnifiedBypassEngine not available - cannot compare")
        return comparison
    ...
    try:
        ...
        service_engine = UnifiedBypassEngine(UnifiedEngineConfig(
            debug=True,
        ...
        if testing_result and service_result:
            ...
            both_success = testing_result.get('success') and service_result.get('success')
            both_fail = not testing_result.get('success') and not service_result.get('success')
            
            comparison['identical_behavior'] = both_success or both_fail
            
            # Check for differences
            if testing_result.get('forced_override') != service_result.get('forced_override'):
...
Examples:
  ...
  python enhanced_find_rst_triggers.py --domain x.com --compare-service-mode --strategy "fakeddisorder(ttl=1)"
    ...
    parser.add_argument(
        "--strategy",
        help="Specific strategy to test (for comparison mode)"
    )
