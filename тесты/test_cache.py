...
from recon.тесты.cache import FingerprintCache, CachedFingerprint
from recon.тесты.advanced_models import DPIFingerprint, DPIType, CacheError
...
class TestCachedFingerprint:
    ...
    def test_expiry_logic(self):
        ...
        cached_expired = CachedFingerprint(
            fingerprint=fingerprint, timestamp=time.time() - 7200, ttl=3600
        )
...
class TestFingerprintCache:
    ...
    def test_persistence_expired_filtering(self, temp_cache_file, sample_fingerprint):
        ...
        expired_cached = CachedFingerprint(
            fingerprint=sample_fingerprint, timestamp=time.time() - 7200, ttl=3600
        )
    ...
    def test_background_cleanup(self, temp_cache_file, sample_fingerprint):
        ...
        cache = FingerprintCache(
            cache_file=temp_cache_file, cleanup_interval=1, auto_save=False
        )
