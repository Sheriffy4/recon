# recon/core/fingerprint/test_tcp_analyzer.py
"""
...
# Import the TCP analyzer
from .tcp_analyzer import (
    TCPAnalyzer,
    TCPAnalysisResult,
...
def mock_scapy_packet():
    ...
    # Mock TCP layer
    mock_tcp = Mock()
    mock_tcp.flags = 0x04  # RST flag
    mock_tcp.window = 65535
    mock_tcp.options = [("MSS", 1460), ("WScale", 3)]

    # Configure packet to return appropriate layers using side_effect
    ...
    # Use MagicMock to properly support __getitem__
...
class TestTCPAnalyzer:
    ...
    async def test_resolve_target_success(self, tcp_analyzer):
        ...
        with patch("socket.getaddrinfo") as mock_getaddrinfo:
            ...
            # Mock asyncio.get_event_loop().getaddrinfo
    ...
    async def test_basic_connections_analysis_success(self, tcp_analyzer):
        ...
        with patch("asyncio.open_connection") as mock_open_conn:
            ...
            assert (
                not result.rst_injection_detected
            )  # Should be False for successful connections
    ...
    async def test_basic_connections_analysis_rst_injection(self, tcp_analyzer):
        ...
        # Mock connections that fail with ConnectionResetError
        with patch("asyncio.open_connection") as mock_open_conn:
            ...
            assert len(result.connection_attempts) > 0
            rst_attempts = sum(
                1 for attempt in result.connection_attempts if attempt.rst_received
            )
    ...
    async def test_basic_connections_analysis_timeouts(self, tcp_analyzer):
        ...
        with patch("asyncio.open_connection") as mock_open_conn:
            ...
            timeout_attempts = sum(
                1 for attempt in result.connection_attempts if attempt.timeout_occurred
            )
            assert timeout_attempts > 0
            assert (
                result.connection_state_tracking
            )  # High timeout rate indicates state tracking
    ...
    async def test_rst_injection_analysis_limited(self, tcp_analyzer):
        ...
        # Force limited analysis (no raw sockets)
        tcp_analyzer.use_raw_sockets = False

        # Mock some successful connections and some RST failures
        connection_results = [
            (MockStreamReader(), MockStreamWriter()),  # Success
            ConnectionResetError("Reset"),  # RST
            ConnectionResetError("Reset"),  # RST
            (MockStreamReader(), MockStreamWriter()),  # Success
            ConnectionResetError("Reset"),  # RST
        ]
        ...
        with patch("asyncio.open_connection") as mock_open_conn:
            ...
            # Should detect middlebox if RST timing is fast
            assert result.rst_source_analysis in [
                RSTSource.MIDDLEBOX.value,
                RSTSource.SERVER.value,
            ]

    @pytest.mark.asyncio
    @patch("recon.core.fingerprint.tcp_analyzer.SCAPY_AVAILABLE", True)
    async def test_rst_injection_analysis_with_scapy(
        self, tcp_analyzer, mock_scapy_packet
    ):
    ...
    def test_analyze_rst_source_middlebox(self, tcp_analyzer, mock_scapy_packet):
        ...
        # Configure mock packet for middlebox detection
        mock_ip = mock_scapy_packet[Mock()]
        mock_ip.src = "192.168.1.100"  # Different from target
        mock_ip.ttl = 64

        mock_tcp = Mock()
        mock_tcp.options = []  # No options indicates middlebox
        mock_tcp.window = 0  # Default window size
    ...
    def test_analyze_rst_source_server(self, tcp_analyzer, mock_scapy_packet):
        ...
        # Configure mock packet for server detection
        mock_ip = mock_scapy_packet[Mock()]
        mock_ip.src = "192.168.1.1"  # Same as target
        mock_ip.ttl = 64

        mock_tcp = Mock()
        mock_tcp.options = [("MSS", 1460), ("WScale", 3)]  # Has options
        mock_tcp.window = 8192  # Non-default window size
    ...
    async def test_window_manipulation_analysis(self, tcp_analyzer):
        ...
        with patch("socket.socket") as mock_socket_class:
            ...
            # Should detect manipulation if not all window sizes work
            if len(result.window_size_variations) < 4:  # Less than 80% of test windows
    ...
    @pytest.mark.asyncio
    @patch("recon.core.fingerprint.tcp_analyzer.SCAPY_AVAILABLE", True)
    async def test_fragmentation_analysis_with_scapy(
        self, tcp_analyzer, mock_scapy_packet
    ):
        ...
        # Mock Scapy functions
        with patch("recon.core.fingerprint.tcp_analyzer.send") as mock_send, patch(
            "recon.core.fingerprint.tcp_analyzer.sr1"
        ) as mock_sr1:

            # Configure mock response for successful reassembly
            mock_tcp = Mock()
            mock_tcp.flags = 0x12  # SYN-ACK flags
            mock_scapy_packet.__getitem__.return_value = mock_tcp
            mock_sr1.return_value = mock_scapy_packet

            await tcp_analyzer._analyze_fragmentation_handling(
                result, "192.168.1.1", 443
            )

            assert result.fragmentation_handling in [
                "reassembled",
                "blocked",
                "unknown",
            ]
            mock_send.assert_called()  # Should have sent fragment
    ...
    async def test_mss_clamping_detection(self, tcp_analyzer, mock_scapy_packet):
        ...
        # Mock Scapy sr1 to return responses with clamped MSS
        with patch("recon.core.fingerprint.tcp_analyzer.sr1") as mock_sr1:
            # Configure mock TCP response with clamped MSS
            mock_tcp = Mock()
            mock_tcp.options = [
                ("MSS", 1460)
            ]  # Clamped to 1460 even for larger requests
            mock_scapy_packet.__getitem__.return_value = mock_tcp
            mock_sr1.return_value = mock_scapy_packet

            await tcp_analyzer._test_mss_clamping(result, "192.168.1.1", 443)

            # Should detect clamping when we send MSS 9000 but get back 1460
    ...
    async def test_tcp_options_analysis(self, tcp_analyzer, mock_scapy_packet):
        ...
        # Mock Scapy sr1 to return responses without certain options
        with patch("recon.core.fingerprint.tcp_analyzer.sr1") as mock_sr1:
            # Configure mock TCP response with filtered options
            mock_tcp = Mock()
            mock_tcp.options = [("MSS", 1460)]  # Only MSS, other options filtered
            mock_scapy_packet.__getitem__.return_value = mock_tcp
            mock_sr1.return_value = mock_scapy_packet

            await tcp_analyzer._analyze_tcp_options(result, "192.168.1.1", 443)

            # Should detect filtered options
            assert len(result.tcp_options_filtering) >= 0

    @pytest.mark.asyncio
    @patch("recon.core.fingerprint.tcp_analyzer.SCAPY_AVAILABLE", True)
    async def test_syn_flood_protection_detection(
        self, tcp_analyzer, mock_scapy_packet
    ):
        """Test SYN flood protection detection"""
        result = TCPAnalysisResult(target="test.com")

        # Mock Scapy sr1 to return fewer responses (simulating rate limiting)
        with patch("recon.core.fingerprint.tcp_analyzer.sr1") as mock_sr1:
            # Return response only for first few attempts
            responses = [
                mock_scapy_packet,
                mock_scapy_packet,
                None,
            ...
            # Should detect SYN flood protection due to low response rate
    ...
    def test_reliability_score_calculation(self, tcp_analyzer):
        ...
        result.connection_attempts = [
            TCPConnectionAttempt(
                time.time(), "192.168.1.1", 443, 12345, 1000, 0, 65535, success=True
            ),
            TCPConnectionAttempt(
                time.time(), "192.168.1.1", 443, 12346, 1001, 0, 65535, success=True
            ),
            TCPConnectionAttempt(
                time.time(), "192.168.1.1", 443, 12347, 1002, 0, 65535, success=False
            ),
        ]
        result.rst_timing_patterns = [50.0, 55.0, 52.0]  # Consistent timing
    ...
    def test_reliability_score_with_errors(self, tcp_analyzer):
        ...
        # Add errors to reduce reliability
        result.analysis_errors = ["Error 1", "Error 2", "Error 3"]
        result.connection_attempts = []  # No successful attempts
        ...
        assert 0.0 <= score <= 1.0
        assert score < 0.5  # Should be low due to errors and lack of data
    ...
    async def test_full_tcp_analysis_success(self, tcp_analyzer):
        ...
        with patch.object(tcp_analyzer, "_resolve_target", return_value="192.168.1.1"):
            ...
            with patch.object(
                tcp_analyzer, "_analyze_basic_connections"
            ) as mock_basic, patch.object(
                tcp_analyzer, "_analyze_rst_injection"
            ) as mock_rst, patch.object(
                tcp_analyzer, "_analyze_window_manipulation"
            ) as mock_window, patch.object(
                tcp_analyzer, "_analyze_sequence_numbers"
            ) as mock_seq, patch.object(
                tcp_analyzer, "_analyze_fragmentation_handling"
            ) as mock_frag, patch.object(
                tcp_analyzer, "_analyze_tcp_options"
            ) as mock_options:
    ...
    async def test_tcp_analysis_with_network_error(self, tcp_analyzer):
        ...
        with patch.object(
            tcp_analyzer,
            "_resolve_target",
            side_effect=NetworkAnalysisError("DNS failed"),
        ):
    ...
    async def test_tcp_analysis_partial_failure(self, tcp_analyzer):
        ...
        # Mock DNS resolution success but analysis method failures
        with patch.object(tcp_analyzer, "_resolve_target", return_value="192.168.1.1"):
            with patch.object(
                tcp_analyzer, "_analyze_basic_connections"
            ) as mock_basic, patch.object(
                tcp_analyzer,
                "_analyze_rst_injection",
                side_effect=Exception("RST analysis failed"),
            ) as mock_rst, patch.object(
                tcp_analyzer, "_analyze_window_manipulation"
            ) as mock_window, patch.object(
                tcp_analyzer, "_analyze_sequence_numbers"
                ...
                # Should not raise exception, but should record error
                result = await tcp_analyzer.analyze_tcp_behavior("test.com", 443)
...
class TestTCPAnalyzerIntegration:
    ...
    async def test_tcp_analyzer_integration_with_real_network(self):
        ...
        # Test with a well-known server (Google DNS)
        try:
    ...
    async def test_tcp_analyzer_with_localhost(self):
        ...
        try:
            ...
            # Even localhost might not have port 80 open
