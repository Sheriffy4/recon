# recon/core/bypass/safety/test_safety_framework.py
...
from .attack_sandbox import AttackSandbox, SandboxConstraints
...
class MockAttack(BaseAttack):
    ...
    def __init__(
        self,
        attack_id: str = "test_attack",
        execution_time: float = 0.1,
        should_fail: bool = False,
        result_status: AttackStatus = AttackStatus.SUCCESS,
    ):
        super().__init__()
        self.id = attack_id
...
class TestResourceManager:
    ...
    def test_resource_limits_validation(self):
        ...
        invalid_limits = ResourceLimits(
            max_execution_time_seconds=-1, max_memory_mb=0, max_cpu_percent=150
        )
    ...
    def test_concurrent_attack_limit(self):
        ...
        # Remove one monitor and try again
        manager.remove_monitor("attack1")
        monitor3 = manager.create_monitor("attack3")  # Should succeed
...
class TestAttackSandbox:
    ...
    def test_sandbox_monitor_creation(self):
        ...
        assert len(violations) == 0  # No violations initially

    def test_file_operation_monitoring(self):
        """Test file operation monitoring."""
        constraints = SandboxConstraints(
            max_file_operations=2, forbidden_paths={"/etc/passwd"}
        )
    ...
    def test_threading_constraints(self):
        ...
        monitor2.record_thread_creation()  # Should succeed

        with pytest.raises(SandboxViolationError):
            monitor2.record_thread_creation()  # Should fail
...
class TestEmergencyStopManager:
    ...
    def test_emergency_stop_request(self):
        ...
        # Request stop
        success = manager.request_stop(
            "test_attack", StopReason.USER_REQUEST, "Test stop", StopPriority.HIGH
        )
        assert success
    ...
    def test_stop_all_attacks(self):
        ...
        # Stop all
        count = manager.request_stop_all(
            StopReason.SYSTEM_INSTABILITY, "System overload", StopPriority.CRITICAL
        )
    ...
    def test_stop_conditions(self):
        ...
        def test_condition():
            trigger_count[0] += 1
            return trigger_count[0] >= 3  # Trigger after 3 checks

        from .emergency_stop import StopCondition

        condition = StopCondition(
            name="test_condition",
            check_function=test_condition,
            reason=StopReason.CRITICAL_ERROR,
        ...
        # Create controller to be stopped
        controller = manager.create_controller("test_attack")

        # Wait for condition to trigger
        time.sleep(0.5)

        # Check if stop was triggered (this depends on the monitoring loop)
        # In a real scenario, the condition would trigger a stop
...
class TestSafetyValidator:
    ...
    def test_post_execution_validation(self):
        ...
        context = AttackContext(dst_ip="1.1.1.1", dst_port=443, payload=b"test payload")
        result = AttackResult(
            status=AttackStatus.SUCCESS, latency_ms=100.0, packets_sent=1
        )
    ...
    def test_validation_failures(self):
        ...
        invalid_context = AttackContext(
            ...
            payload=b"x" * (65 * 1024),  # Too large payload
        )
    ...
    def test_custom_validation_checks(self):
        ...
        from .safety_validator import ValidationCheck

        custom_validation = ValidationCheck(
            name="custom_port_check",
            description="Check for forbidden port 1337",
...
class TestSafetyController:
    ...
    def test_attack_timeout(self):
        ...
        # Attack that takes longer than timeout
    ...
    def test_validation_failure_handling(self):
        ...
        attack = MockAttack()
        # Invalid context that should fail validation
        invalid_context = AttackContext(
            dst_ip="", dst_port=0  # Missing IP  # Invalid port
        )
    ...
    def test_emergency_stop_during_execution(self):
        ...
        controller = SafetyController()
        attack = MockAttack(execution_time=1.0)  # Long-running attack
        context = AttackContext(dst_ip="1.1.1.1", dst_port=443)

        def trigger_stop():
            time.sleep(0.1)  # Wait a bit then trigger stop
            controller.emergency_stop_attack("test_attack", "Test emergency stop")

        # Start stop trigger in background
        stop_thread = threading.Thread(target=trigger_stop, daemon=True)
        stop_thread.start()

        # This should raise EmergencyStopError
    ...
    def test_configuration_updates(self):
        ...
        new_config = SafetyConfiguration(
            validation_level=ValidationLevel.STRICT, default_attack_timeout=30.0
        )
    ...
    def test_disabled_components(self):
        ...
        assert controller.validator is not None  # Always enabled

        # Should still be able to execute attacks
        attack = MockAttack()
        context = AttackContext(dst_ip="1.1.1.1", dst_port=443)
...
def test_integration_all_components():
    ...
    # Configure strict safety
    config = SafetyConfiguration(
        resource_limits=ResourceLimits(
            max_execution_time_seconds=5.0, max_memory_mb=50.0, max_concurrent_attacks=2
        ),
    ...
    # Execute attack with all safety components
    attack = MockAttack(execution_time=0.1)
    context = AttackContext(dst_ip="1.1.1.1", dst_port=443, payload=b"test payload")
    ...
    # Check post-execution validation
