...
def domain_status():
    ...
    return {"test1.com": "BLOCKED", "test2.com": "WORKING", "test3.com": "BLOCKED"}
...
def test_save_report_error_handling(
    reporter, test_results, domain_status, fingerprints
):
...
def test_report_with_evolution_data(
    reporter, test_results, domain_status, fingerprints
):
...
def test_report_without_fingerprints(reporter, test_results, domain_status):
    ...
    report = reporter.generate_report(
        test_results=test_results, domain_status=domain_status, args=args
    )
