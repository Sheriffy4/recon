...
def evolutionary_searcher():
    ...
    return SimpleEvolutionarySearcher(
        population_size=5, generations=2, mutation_rate=0.2
    )
...
def test_evolutionary_chromosome_initialization():
    ...
    genes = {"type": "fakedisorder", "ttl": 3, "split_pos": 3}
...
def test_evolutionary_chromosome_crossover():
    ...
    parent1 = EvolutionaryChromosome(
        genes={"type": "fakedisorder", "ttl": 3, "split_pos": 3}
    )
    parent2 = EvolutionaryChromosome(
        genes={"type": "multisplit", "ttl": 5, "split_pos": 5, "overlap_size": 10}
    )
...
def test_evolutionary_searcher_initialization():
    ...
    searcher = SimpleEvolutionarySearcher(
        population_size=10, generations=3, mutation_rate=0.2
    )
...
def test_evolutionary_searcher_create_population_with_learning():
    ...
    learning_cache.get_domain_recommendations = Mock(
        return_value=[("fakedisorder", 0.8), ("multisplit", 0.6)]
    )
    learning_cache.get_dpi_recommendations = Mock(
        return_value=[("seqovl", 0.7), ("badsum_race", 0.5)]
    )

    population = searcher.create_initial_population(
        learning_cache=learning_cache, domain="test.com", dpi_hash="abc123"
    )
...
@pytest.mark.asyncio
async def test_evolutionary_searcher_evaluate_fitness(
    evolutionary_searcher, hybrid_engine
):
...
async def test_evolutionary_searcher_evolve(evolutionary_searcher, hybrid_engine):
    ...
    assert all(
        "best_fitness" in gen for gen in evolutionary_searcher.best_fitness_history
    )
...
async def test_evolutionary_search_integration():
    ...
    searcher = SimpleEvolutionarySearcher(
        population_size=3, generations=2, mutation_rate=0.2
    )
    hybrid_engine = Mock()
