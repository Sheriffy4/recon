...
class ComprehensivePcapVerifier:
    ...
    def __init__(self, zapret_pcap: str = "zapret.pcap", recon_pcap: str = "out2.pcap"):
        ...
        self.analysis_results = {
            ...
            "files": {"zapret": zapret_pcap, "recon": recon_pcap},
            "analysis": {},
        }
    ...
    def find_matching_flows(
        self, zapret_packets: List, recon_packets: List
    ) -> Dict[str, Dict]:
    ...
    def analyze_packet_headers(self, zapret_pkt, recon_pkt, pkt_num: int) -> Dict:
        ...
        for field in ip_fields:
            ...
            if zapret_val != recon_val:
                analysis["differences"].append(
                    f"IP.{field}: zapret={zapret_val}, recon={recon_val}"
                )
        ...
        if TCP in zapret_pkt and TCP in recon_pkt:
            ...
            for field in tcp_fields:
                ...
                if zapret_val != recon_val:
                    analysis["differences"].append(
                        f"TCP.{field}: zapret={zapret_val}, recon={recon_val}"
                    )
    ...
    def analyze_tcp_options(self, zapret_pkt, recon_pkt) -> Dict:
        ...
        analysis = {"zapret_options": [], "recon_options": [], "differences": []}
        ...
        if analysis["zapret_options"] != analysis["recon_options"]:
            ...
            analysis["differences"].append(
                f"Zapret options: {analysis['zapret_options']}"
            )
            analysis["differences"].append(
                f"Recon options: {analysis['recon_options']}"
            )
    ...
    def analyze_timing(self, zapret_flow: List, recon_flow: List) -> Dict:
        ...
        analysis = {"zapret_timing": [], "recon_timing": [], "timing_differences": []}
            ...
            analysis["zapret_timing"].append(
                {
                    ...
                    "relative_time": (
                        float(pkt.time) - float(zapret_flow[0][1].time)
                        if i > 0
                        else 0.0
                    ),
                }
            ...
            analysis["recon_timing"].append(
                {
                    ...
                    "timestamp": float(pkt.time),
                    "relative_time": (
                        float(pkt.time) - float(recon_flow[0][1].time) if i > 0 else 0.0
                    ),
                }
            )
    ...
    def check_tcp_retransmissions(self, flow_packets: List) -> Dict:
        ...
        analysis = {"retransmissions_found": [], "os_retransmissions": []}
    ...
    def analyze_rst_packets(self, flow_packets: List) -> Dict:
        ...
        analysis = {"rst_packets": [], "rst_sources": {}}
    ...
    def perform_comprehensive_analysis(self) -> Dict:
        ...
        flow_analysis = {
            ...
            "packet_count": {"zapret": len(zapret_flow), "recon": len(recon_flow)},
            "packet_analysis": [],
            "timing_analysis": {},
        ...
        flow_analysis["retransmission_analysis"] = self.check_tcp_retransmissions(
            recon_flow
        )
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"Packet count - Zapret: {flow['packet_count']['zapret']}, Recon: {flow['packet_count']['recon']}"
        )
        report.append("")
        ...
        if flow["timing_analysis"]["timing_differences"]:
            ...
            for timing in flow["timing_analysis"]["timing_differences"]:
                report.append(
                    f"Packet {timing['packet']}: Zapret={timing['zapret_delta_ms']:.3f}ms, "
        ...
        if flow["retransmission_analysis"]["retransmissions_found"]:
            ...
            report.append("-" * 40)
            report.append(
                f"Total retransmissions found: {len(flow['retransmission_analysis']['retransmissions_found'])}"
            )
            report.append(
                f"OS retransmissions (TTL=128): {len(flow['retransmission_analysis']['os_retransmissions'])}"
            )

            if flow["retransmission_analysis"]["os_retransmissions"]:
                report.append(
                    "WARNING: OS retransmissions detected - this may indicate timing issues!"
                )
            report.append("")
        ...
        if flow["rst_analysis"]["rst_packets"]:
            ...
            report.append("-" * 40)
            report.append(
                f"Total RST packets: {len(flow['rst_analysis']['rst_packets'])}"
            )
        ...
        total_differences = sum(
            len(pkt["differences"]) for pkt in flow["packet_analysis"]
        )
        ...
        if flow["rst_analysis"]["rst_packets"]:
            report.append(
                f"⚠ {len(flow['rst_analysis']['rst_packets'])} RST packets found"
            )
        else:
