...
async def check_domain_simple(domain):
        ...
        for provider_name, server_url in doh_servers:
            try:
                ...
                async with session.get(
                    server_url, params=params, headers=headers, timeout=5
                ) as response:
    ...
    if not doh_ips:
        print("DoH: Все провайдеры недоступны")
    ...
    if block_type != "none":
        ...
        print("   - Настройте DoH в браузере")
...
async def test_connection_simple(domain, port=443):
    ...
    try:
        ...
        reader, writer = await asyncio.wait_for(
            asyncio.open_connection(domain, port), timeout=5.0
        )
        ...
        for domain in domains:
            ...
            for server_url in doh_servers:
                try:
                    ...
                    async with session.get(
                        server_url, params=params, headers=headers, timeout=3
                    ) as response:
                        if response.status == 200:
...
async def setup_hosts_simple():
    ...
    if platform.system().lower() == "windows":
        ...
        print("Для редактирования запустите блокнот от имени администратора")
    else:
...
def check_hosts_simple():
    ...
    try:
        ...
        if found_entries:
            ...
            print("   Используйте: python simple_cli.py setup-hosts")
        ...
        if suspicious:
            ...
            for line_num, entry in suspicious:
                print(f"  {line_num:3d}: {entry}")
            print("   Рекомендуется удалить или закомментировать эти строки")

    except FileNotFoundError:
        print(f"❌ Файл hosts не найден: {hosts_path}")
    except PermissionError:
        print("❌ Нет прав для чтения файла hosts")
    ...
    async with aiohttp.ClientSession() as session:
        for domain in test_domains:
            ...
            try:
                start_time = asyncio.get_event_loop().time()
                reader, writer = await asyncio.wait_for(
                    asyncio.open_connection(domain, 443), timeout=3.0
                )
                end_time = asyncio.get_event_loop().time()
...
Поддерживаемые домены:
  ...
  • nnmclub.to
    ...
    multi_parser = subparsers.add_parser(
        "test-multi", help="Тестировать несколько доменов"
    )
