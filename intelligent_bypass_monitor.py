...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('intelligent_bypass_monitor.log'),
...
class StrategyCalibrator:
    ...
    def calibrate_strategy(self, domain: str, blocking_type: str, 
                          suggested_strategies: List[str]) -> Optional[BypassStrategy]:
        ...
        for strategy_type in suggested_strategies:
            ...
            for attack_name, attack_class in attacks.items():
                ...
                # Калибруем параметры
                best_params = self._calibrate_parameters(
                    domain, attack_name, base_params
                )
    ...
    def _get_base_parameters(self, attack_name: str, blocking_type: str) -> Dict[str, Any]:
        ...
        return base_params.get(blocking_type, {
            "split_pos": [2, 3],
            "ttl": [2, 3],
            "fooling": ["badsum"],
            "repeats": [2]
    ...
    def _calibrate_parameters(self, domain: str, attack_name: str, 
    ...
    def _test_parameters(self, domain: str, attack_name: str, 
...
class IntelligentBypassMonitor:
    ...
    def _run_calibration(self, domain: str, blocking_type: str, 
    ...
    def start(self):
        ...
        monitoring_thread = threading.Thread(
            target=self.traffic_monitor.start_monitoring,
            daemon=True
        )
    ...
    def _print_stats(self):
        ...
        logger.info(f"   Проанализировано паттернов: {self.monitoring_stats.get('patterns_analyzed', 0)}")
        logger.info(f"   Обнаружено блокировок: {self.monitoring_stats.get('blocked_domains_detected', 0)}")
        logger.info(f"   Успешных соединений: {self.monitoring_stats.get('successful_connections', 0)}")
        logger.info(f"   Откалибровано стратегий: {self.monitoring_stats.get('strategies_calibrated', 0)}")
...
def main():
    ...
    parser.add_argument("--config", default="intelligent_bypass_config.json",
