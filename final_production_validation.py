...
def test_exact_production_scenario():
    ...
    try:
        ...
        LOG.info(
            "INFO     ML predicted strategy for 104.21.32.39 (rutracker.org): tcp_window_scaling (confidence: 0.60)"
        )
        ...
        LOG.info("✅ Background fingerprinting successful")
        ...
        LOG.info(
            f"✅ Dynamic combo attack completed successfully: {combo_result.status}"
        )
...
def test_error_conditions():
    ...
    try:
        ...
        invalid_context = AttackContext(
            dst_ip="", dst_port=0, payload=b"test"  # Invalid IP  # Invalid port
        )
